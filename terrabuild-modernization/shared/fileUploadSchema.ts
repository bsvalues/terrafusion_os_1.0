...
export const fileUploads = pgTable('file_uploads', {
  ...
  status: text('status').default('uploaded').notNull(), // Status: uploaded, processing, completed, error
  processedItems: text('processed_items').default('0'),
...
export const insertFileUploadSchema = createInsertSchema(fileUploads)
  .omit({ id: true, uploadedAt: true } as const);
