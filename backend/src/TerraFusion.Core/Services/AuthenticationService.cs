  ...
  public class AuthenticationService : IAuthenticationService
  {
    ...
    public async Task<string> GenerateJwtTokenAsync(string userId, string email, IEnumerable<string> roles)
    {
      try
      {
        ...
        // CX-16: Derive permission claims from roles
        var permissions = ResolvePermissionsForRoles(roleList);

        // CX-16: Resolve county context (dev default or future directory lookup)
        var countyId = _configuration["DefaultCounty:Id"] ?? "";
        var countyCode = _configuration["DefaultCounty:Code"] ?? "";
        ...
        if (!string.IsNullOrEmpty(countyId))
          customClaims["countyId"] = countyId;
        if (!string.IsNullOrEmpty(countyCode))
          customClaims["countyCode"] = countyCode;

        var token = _jwtTokenService.GenerateAccessToken(userId, email, roleList, customClaims);

        _logger.LogInformation("Generated JWT token for user {UserId} with email {EmailMasked}, perms={PermCount}",
            userId, MaskEmail(email), permissions.Length);
    ...
    /// </summary>
    private static string[] ResolvePermissionsForRoles(string[] roles)
    {
      ...
      foreach (var role in roles)
      {
        switch (role)
        {
          ...
          case "PropertyAssessor":
            perms.UnionWith(new[]
            {
                            "read:dossier", "write:dossier",
                            "read:properties", "read:parcel",
                            "access:costforge", "calculate:property-cost",
                            "read:cost-breakdown", "read:cost-comparison",
                            "read:cost-forecast", "read:cost-factors", "read:cost-matrix",
                            "read:system-status", "read:performance-metrics",
                            "ecosystem:view",
                        });
            break;
          ...
          case "Administrator":
            perms.UnionWith(new[]
            {
                            ...
                            "read:cost-breakdown", "read:cost-comparison",
                            "read:cost-forecast", "read:cost-factors", "read:cost-matrix",
                            "read:system-status", "read:ai-agents", "manage:ai-agents",
                            "read:performance-metrics", "sync:external-systems",
