...
public class CanonicalDebugController : ControllerBase
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            var d1 = await rawLandingSvc.LandParcelGeomsAsync(
                "53005", bentonCountyId, operatorName, cancellationToken);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            if (!skipGeometry)
            {
                var gisD1 = await gisRawSvc.LandParcelGeomsAsync("53005", bentonCountyId, operatorName, cancellationToken);
    ...
    // ════════════════════════════════════════════════════════════════════
    // READBACK-F1 re-projection repair (2026-06-08).
    //   Re-projects canonical_tf.tf_land + tf_improvement from EXISTING
    //   truth (truth_pacs.land_current / imprv_current) onto the CURRENT
    //   live parcel identity (source_xref → tf_parcel), fixing the
    //   parcel-identity fork found in production readback F1.
    //   Projection-only: iterates each existing truth promotion batch and
    //   calls the (already-current-identity-resolving) canonical projector.
    //   Does NOT re-drain PACS, does NOT mutate legacy_pacs_raw/truth_pacs,
    //   does NOT touch the parcel spine. Geometry is repaired separately via
    //   gis-pop-1/reproject-canonical. Idempotent (each projector clears its
    //   own prior canonical rows by truth key before re-projecting).
    // ════════════════════════════════════════════════════════════════════
    [HttpPost("readback-f1/reproject-land-imprv")]
    [Microsoft.AspNetCore.Authorization.AllowAnonymous]
    public async Task<IActionResult> RunReadbackF1ReprojectLandImprv(
        [FromServices] TerraFusion.Core.Sync.PacsLandCanonical.IPacsLandCanonicalProjector landProjector,
        [FromServices] TerraFusion.Core.Sync.PacsImprvCanonical.IPacsImprvCanonicalProjector imprvProjector,
        [FromBody] ReadbackF1Request? request,
        CancellationToken cancellationToken = default)
    {
        var operatorName = string.IsNullOrWhiteSpace(request?.OperatorName)
            ? "readback-f1-reproject-land-imprv"
            : request.OperatorName.Trim();
        try
        {
            var beforeLand = await _db.TfLands.CountAsync(cancellationToken);
            var beforeImprv = await _db.TfImprovements.CountAsync(cancellationToken);

            var landBatches = await _db.TruthPacsLandCurrents
                .Select(t => t.PromotionLoadBatchId).Distinct()
                .ToListAsync(cancellationToken);
            var imprvBatches = await _db.TruthPacsImprvCurrents
                .Select(t => t.PromotionLoadBatchId).Distinct()
                .ToListAsync(cancellationToken);

            int landOk = 0, landFail = 0, imprvOk = 0, imprvFail = 0;
            var failures = new List<object>();

            foreach (var b in landBatches)
            {
                cancellationToken.ThrowIfCancellationRequested();
                var r = await landProjector.ProjectAsync(b, operatorName, cancellationToken);
                if (string.Equals(r.Status, "COMPLETED", StringComparison.OrdinalIgnoreCase)) landOk++;
                else { landFail++; failures.Add(new { lane = "land", batch = b, r.Status, r.ErrorSummary }); }
            }
            foreach (var b in imprvBatches)
            {
                cancellationToken.ThrowIfCancellationRequested();
                var r = await imprvProjector.ProjectAsync(b, operatorName, cancellationToken);
                if (string.Equals(r.Status, "COMPLETED", StringComparison.OrdinalIgnoreCase)) imprvOk++;
                else { imprvFail++; failures.Add(new { lane = "imprv", batch = b, r.Status, r.ErrorSummary }); }
            }

            var afterLand = await _db.TfLands.CountAsync(cancellationToken);
            var afterImprv = await _db.TfImprovements.CountAsync(cancellationToken);

            _logger.LogInformation(
                "[ReadbackF1] reproject COMPLETE land(ok={Lo}/fail={Lf} {Bl}batches {BeforeL}->{AfterL}) imprv(ok={Io}/fail={If} {Bi}batches {BeforeI}->{AfterI})",
                landOk, landFail, landBatches.Count, beforeLand, afterLand,
                imprvOk, imprvFail, imprvBatches.Count, beforeImprv, afterImprv);

            return Ok(new
            {
                operatorName,
                land = new { batches = landBatches.Count, ok = landOk, fail = landFail, beforeLand, afterLand },
                imprv = new { batches = imprvBatches.Count, ok = imprvOk, fail = imprvFail, beforeImprv, afterImprv },
                failures,
            });
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "[ReadbackF1] reproject-land-imprv FAILED");
            return StatusCode(500, new { error = ex.Message, type = ex.GetType().Name });
        }
    }

    public sealed record ReadbackF1Request(string? OperatorName);
}
