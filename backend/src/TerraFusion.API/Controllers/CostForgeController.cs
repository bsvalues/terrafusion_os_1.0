...
public class CostForgeController : ControllerBase
{
  ...
  /// <summary>
  /// Get full cost matrix catalog — all building types, regions, quality/condition
  /// factors, depreciation config, and component breakdowns.
  /// For audit, inspection, and model transparency.
  /// </summary>
  [HttpGet("matrices")]
  [RequiresPermission("read:cost-matrix")]
  public async Task<ActionResult<CostMatrixCatalogDto>> GetCostMatrices()
  {
    try
    {
      await _auditLogger.LogDataAccessAsync("CostMatrixCatalog", "all", "READ",
          User.FindFirst("sub")?.Value);

      var result = await _costForgeService.GetCostMatricesAsync();
      return Ok(result);
    }
    catch (Exception ex)
    {
