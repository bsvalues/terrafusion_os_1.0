...
// WORKBENCH-V0.3 SLICE-L: v0.3 bridge implementation.
// Spawns the existing tools/sync/identity-runner.mjs Node.js tool and
// returns raw pipe-delimited stdout for the frontend to parse.
// No C# identity logic — the SQL is the single source of truth.

/// <summary>
/// Singleton service that runs the identity drift detector as a child process.
/// The 409 guard (concurrent run prevention) uses <see cref="Interlocked"/>
/// compare-exchange on <c>_running</c>.
/// </summary>
public sealed class IdentityRunnerService : IIdentityRunnerService
{
    ...
    public async Task<IdentityRunResult> RunAsync(CancellationToken cancellationToken = default)
    {
        // 409 guard — only one concurrent run allowed.
        if (Interlocked.CompareExchange(ref _running, 1, 0) != 0)
        {
            return new IdentityRunResult
            {
                ExitCode   = 2,
                Stdout     = string.Empty,
                Stderr     = string.Empty,
    ...
    private IReadOnlyDictionary<string, string> BuildPgEnvVars()
    {
        ...
        // identity-runner.mjs connects to the same database as the API.
