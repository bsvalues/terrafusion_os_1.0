...
// WORKBENCH-V0.3 SLICE-K: v0.3 bridge implementation.
// Spawns the existing tools/sync/pack-validator-runner.mjs Node.js tool and
// returns raw pipe-delimited stdout for the frontend to parse.
// No C# validator logic — the SQL is the single source of truth.

/// <summary>
/// Singleton service that runs the Harris PACS pack validator as a child process.
/// The 409 guard (concurrent run prevention) uses <see cref="Interlocked"/>
/// compare-exchange on <c>_running</c>.
/// </summary>
public sealed class PackValidatorRunnerService : IPackValidatorRunnerService
{
    ...
    public PackValidatorRunnerService(
    ...
    public async Task<PackValidatorRunResult> RunAsync(CancellationToken cancellationToken = default)
    {
        // 409 guard — only one concurrent run allowed.
        if (Interlocked.CompareExchange(ref _running, 1, 0) != 0)
        {
            return new PackValidatorRunResult
            {
                ExitCode  = 2,
                Stdout    = string.Empty,
                Stderr    = string.Empty,
    ...
    private IReadOnlyDictionary<string, string> BuildPgEnvVars()
    {
        ...
        // pack-validator-runner.mjs connects to the same database as the API.
        // If values are absent the runner falls back to its own defaults
