...
/// <summary>
/// Executes one hash-pinned Dais appeal-workflow module and schema in a disposable,
...
/// </summary>
public sealed class DaisAppealWorkflowProcessHost : IDaisAppealWorkflowProcessHost
{
    ...
    private static readonly string RunnerSource = $$"""
        ...
        const denyNetwork = () => {
          throw new Error('Network access is denied by the Dais appeal-workflow host.');
        };
        ...
        for await (const chunk of process.stdin) {
          ...
          if (inputBytes > {{MaximumInputBytes}}) {
            throw new Error('Dais appeal-workflow exchange exceeds 1 MiB.');
          }
          input += chunk;
        }
        ...
        if (typeof workflow.validateDaisExchange !== 'function' ||
            typeof workflow.normalizeJson !== 'function') {
          throw new Error('Dais module does not expose the required exact functions.');
        }
        const violations = await workflow.validateDaisExchange(schema, exchange);
        if (!Array.isArray(violations)) {
          throw new Error('Dais validation did not return a violation array.');
        }
    ...
    public DaisAppealWorkflowProcessHost(
    ...
    internal DaisAppealWorkflowProcessHost(
        string nodeExecutablePath,
        TimeSpan? timeout,
        ...
        Func<string, int, Exception?>? cleanupFailureFactory = null)
    {
        ...
        if (_timeout <= TimeSpan.Zero || _timeout > TimeSpan.FromSeconds(30))
        {
            throw new ArgumentOutOfRangeException(
                nameof(timeout),
                "Dais appeal-workflow timeout must be greater than zero and no more than 30 seconds.");
        }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        DaisAppealWorkflowProcessResult? result = null;
        ...
        try
        {
            var canonicalModulePath = ValidateArtifactPath(
                modulePath,
                ".mjs",
                DaisAppealWorkflowFailure.InvalidModulePath,
                DaisAppealWorkflowFailure.ModuleNotFound,
                DaisAppealWorkflowFailure.UnsupportedModuleType,
                "module");
            var canonicalSchemaPath = ValidateArtifactPath(
                schemaPath,
                ".json",
                DaisAppealWorkflowFailure.InvalidSchemaPath,
                DaisAppealWorkflowFailure.SchemaNotFound,
                DaisAppealWorkflowFailure.UnsupportedSchemaType,
                "schema");
            ...
            var exchange = ValidateExchange(appealWorkflowExchangeJson);
            ...
            if (!string.Equals(sourceModuleHash, expectedModuleHash, StringComparison.Ordinal))
            {
                throw Fail(
                    ...
                    $"Dais source module hash mismatch: expected {expectedModuleHash}, found {sourceModuleHash}.");
            }
            ...
            {
                throw Fail(
                    DaisAppealWorkflowFailure.SourceSchemaHashMismatch,
                    $"Dais source schema hash mismatch: expected {expectedSchemaHash}, found {sourceSchemaHash}.");
            }

            invocationDirectory = Path.Combine(
                _temporaryRoot,
                "terrafusion-dais-appeal-workflow-host",
                Guid.NewGuid().ToString("N"));
            Directory.CreateDirectory(invocationDirectory);
            var copiedModulePath = Path.Combine(invocationDirectory, "project-dais-appeal-workflow.mjs");
            var copiedSchemaPath = Path.Combine(invocationDirectory, "dais.appeal-workflow.v1.schema.json");
            var runnerPath = Path.Combine(invocationDirectory, "dais-appeal-workflow-runner.mjs");
            ...
            {
                throw Fail(
                    DaisAppealWorkflowFailure.CopiedModuleHashMismatch,
                    $"Disposable Dais module hash mismatch: expected {expectedModuleHash}, found {copiedModuleHash}.");
            }
            ...
            {
                throw Fail(
                    DaisAppealWorkflowFailure.CopiedSchemaHashMismatch,
                    $"Disposable Dais schema hash mismatch: expected {expectedSchemaHash}, found {copiedSchemaHash}.");
            }
        ...
        {
            result = Failure(DaisAppealWorkflowFailure.Cancelled, "Dais validation was cancelled.");
        }
        catch (Exception ex) when (
            ex is IOException or UnauthorizedAccessException or CryptographicException)
        {
            result = Failure(
                DaisAppealWorkflowFailure.ProcessStartFailed,
                $"Dais validation preparation failed closed: {ex.Message}");
        }
        finally
        {
        ...
        {
            return Failure(
                DaisAppealWorkflowFailure.CleanupFailed,
                $"Dais invocation cleanup failed: {invocationDirectory}",
    ...
    private async Task<DaisAppealWorkflowProcessResult> InvokeNodeAsync(
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            stdoutTask = ReadBoundedAsync(
                ...
                DaisAppealWorkflowFailure.StandardOutputTooLarge,
                linkedCts.Token);
            stderrTask = ReadBoundedAsync(
                process.StandardError,
                MaximumStandardErrorBytes,
                DaisAppealWorkflowFailure.StandardErrorTooLarge,
                linkedCts.Token);
            ...
            {
                throw Fail(
                    DaisAppealWorkflowFailure.NonZeroExit,
                    $"Dais validation exited with code {process.ExitCode}: {BoundMessage(stderr)}");
            }
            if (string.IsNullOrWhiteSpace(stdout))
            {
                throw Fail(DaisAppealWorkflowFailure.MissingOutput, "Dais validation produced no output.");
            }
        ...
        catch (OperationCanceledException)
        {
            ...
            throw Fail(
                cancellationToken.IsCancellationRequested
                    ...
                    : DaisAppealWorkflowFailure.Timeout,
                cancellationToken.IsCancellationRequested
                    ? "Dais validation was cancelled."
                    : $"Dais validation exceeded the {_timeout.TotalSeconds.ToString(CultureInfo.InvariantCulture)}-second limit.");
        }
        finally
        {
    ...
    private static DaisAppealWorkflowProcessResult ValidateOutput(
        string stdout,
        ExchangeIdentity exchange,
        string sourceModuleHash,
        string copiedModuleHash,
        ...
        string copiedSchemaHash)
    {
        ...
        catch (JsonException ex)
        {
            throw Fail(
                ...
                $"Invalid Dais JSON output: {ex.Message}");
        }
        ...
        using (document)
        {
            ...
            {
                throw Fail(DaisAppealWorkflowFailure.InvalidOutput, "accepted must be a boolean.");
            }
                ...
                !string.Equals(resultCountyId, exchange.ResultCountyId, StringComparison.Ordinal))
            {
                throw Fail(
                    ...
                    "Dais output county identities must match the submitted exchange.");
            }
            ...
            var violations = new List<DaisAppealWorkflowViolation>();
            foreach (var violation in violationsElement.EnumerateArray())
            {
                ...
                violations.Add(new DaisAppealWorkflowViolation(violationClass, message));
            }
            ...
            if (accepted)
            {
                if (violations.Count != 0 || normalizedElement.ValueKind != JsonValueKind.String)
                {
                    throw Fail(
                        DaisAppealWorkflowFailure.InvalidOutput,
                        "Accepted Dais output requires no violations and normalized exchange JSON.");
                }
                    ...
                    !string.Equals(normalized, exchange.CanonicalJson, StringComparison.Ordinal))
                {
                    throw Fail(
                        DaisAppealWorkflowFailure.IdentityMismatch,
                        "Accepted Dais output must normalize the submitted exchange without mutation.");
                }

                return new DaisAppealWorkflowProcessResult(
                    DaisAppealWorkflowOutcome.Accepted,
                    DaisAppealWorkflowFailure.None,
                    normalized,
                    Array.Empty<DaisAppealWorkflowViolation>(),
                    requestCountyId,
                    resultCountyId,
                    sourceModuleHash,
                    copiedModuleHash,
            ...
            if (violations.Count == 0 || normalizedElement.ValueKind != JsonValueKind.Null)
            {
                throw Fail(
                    DaisAppealWorkflowFailure.InvalidOutput,
                    "Rejected Dais output requires at least one violation and no normalized exchange.");
            }
    ...
    private static ExchangeIdentity ValidateExchange(string exchangeJson)
    {
        ...
        {
            throw Fail(DaisAppealWorkflowFailure.InvalidExchange, "Dais exchange is required.");
        }
        if (Utf8WithoutBom.GetByteCount(exchangeJson) > MaximumInputBytes)
        {
            throw Fail(DaisAppealWorkflowFailure.InputTooLarge, "Dais exchange exceeds 1 MiB.");
        }
        ...
        try
        {
            ...
            var requestCountyId = RequireString(
                request,
                "countyId",
                DaisAppealWorkflowFailure.InvalidExchange);
        ...
        {
            throw Fail(DaisAppealWorkflowFailure.InvalidExchange, $"Invalid Dais exchange: {ex.Message}");
        }
    }
    ...
    private static string ValidateArtifactPath(
        ...
        DaisAppealWorkflowFailure invalidPathFailure,
        DaisAppealWorkflowFailure missingFailure,
        DaisAppealWorkflowFailure unsupportedTypeFailure,
        string artifactName)
    {
        ...
        {
            throw Fail(invalidPathFailure, $"Dais {artifactName} must not be a link or reparse point.");
        }
    ...
    private static string ValidateExpectedHash(string expectedHash, string artifactName)
    {
        ...
        {
            throw Fail(
                DaisAppealWorkflowFailure.InvalidExpectedHash,
        ...
        string objectName)
    {
        if (element.ValueKind != JsonValueKind.Object)
        {
            throw Fail(DaisAppealWorkflowFailure.InvalidOutput, $"Dais {objectName} must be an object.");
        }
    ...
    private static string RequireString(
        JsonElement parent,
        string name,
        DaisAppealWorkflowFailure failure = DaisAppealWorkflowFailure.InvalidOutput)
    {
    ...
    private static async Task<string> ReadBoundedAsync(
        StreamReader reader,
        int maximumBytes,
        DaisAppealWorkflowFailure overflowFailure,
        CancellationToken cancellationToken)
    {
    ...
    private static HostFailureException Fail(DaisAppealWorkflowFailure failure, string message) =>
        new(failure, message);

    private static DaisAppealWorkflowProcessResult Failure(
    ...
    {
        public HostFailureException(DaisAppealWorkflowFailure failure, string message)
            : base(message)
        {
            Failure = failure;
        }

        public DaisAppealWorkflowFailure Failure { get; }
    }
    ...
    internal static string PermissionFlagForVersion(string nodeVersion)
    {
        ...
        {
            throw new ArgumentException(
                "Dais appeal-workflow validation requires a supported Node 20 or newer runtime.",
                nameof(nodeVersion));
        }
