  ...
  public class JwtTokenService : IJwtTokenService
  {
    ...
    public string GenerateAccessToken(string userId, string email, string[] roles, Dictionary<string, object>? customClaims = null)
    {
      ...
      if (customClaims != null)
      {
        ...
        {
          // CX-16: Support IEnumerable<string> as multiple claims with same key
          // (e.g., "perm" → ["read:dossier", "write:dossier"] produces two "perm" claims)
          if (claim.Value is IEnumerable<string> values)
          {
            foreach (var v in values)
              claims.Add(new Claim(claim.Key, v));
          }
          else
          {
