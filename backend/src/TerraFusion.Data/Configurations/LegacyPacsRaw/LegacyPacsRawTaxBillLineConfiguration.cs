...
/// <summary>REVENUE-SPINE: EF config for legacy_pacs_raw.tax_bill_line.</summary>
public sealed class LegacyPacsRawTaxBillLineConfiguration
    : IEntityTypeConfiguration<LegacyPacsRawTaxBillLine>
{
    public void Configure(EntityTypeBuilder<LegacyPacsRawTaxBillLine> builder)
    {
        builder.ToTable("tax_bill_line", schema: "legacy_pacs_raw");
        ...
        builder.HasIndex(x => new { x.PropId, x.TaxYr })
            .HasDatabaseName("ix_legacy_pacs_raw_tax_bill_line_prop_year");
        builder.HasIndex(x => x.LoadBatchId)
            .HasDatabaseName("ix_legacy_pacs_raw_tax_bill_line_loadbatch");
    }
}
