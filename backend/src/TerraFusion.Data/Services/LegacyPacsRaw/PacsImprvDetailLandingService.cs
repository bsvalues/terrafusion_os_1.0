...
/// <summary>
/// Slice C1-B: drains an <see cref="IPacsImprvDetailSource"/> into
/// <c>legacy_pacs_raw.imprv_detail</c> with full provenance and
/// records four C1-B promotion gates:
///
/// <list type="bullet">
///   <item><c>imprv-detail-distribution</c> — informational; type-cd histogram.</item>
///   <item><c>imprv-detail-key-uniqueness</c> — FAIL when any 5-key
...
///   <item><c>imprv-detail-aggregate</c> — informational; reports
///   sums of <c>ImprvDetVal</c> + <c>ImprvDetArea</c>.</item>
/// </list>
/// </summary>
public sealed class PacsImprvDetailLandingService : IPacsImprvDetailLandingService
{
    private const int BatchSize = 1000;

    private readonly TerraFusionDbContext _db;
    private readonly ILogger<PacsImprvDetailLandingService> _logger;

    public PacsImprvDetailLandingService(
        TerraFusionDbContext db,
        ILogger<PacsImprvDetailLandingService> logger)
    {
        _db = db;
        _logger = logger;
    }

    public async Task<PacsImprvDetailLandingResult> LandImprvDetailsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var rowsLanded = 0;
        var pending = 0;
        var keyCounts = new Dictionary<(short, short, int, long, long), int>();
        var typeHistogram = new Dictionary<string, int>(StringComparer.Ordinal);
        decimal imprvDetValSum = 0m;
        decimal imprvDetAreaSum = 0m;
        ...
        try
        {
                ...
                .ConfigureAwait(false))
            {
                ...
                _db.LegacyPacsRawImprvDetails.Add(new LegacyPacsRawImprvDetail
                {
                    ...
                    ImprvDetTypeCd = src.ImprvDetTypeCd,
                    ImprvDetMethCd = src.ImprvDetMethCd,
                    ImprvDetClassCd = src.ImprvDetClassCd,
                    ImprvDetSubClassCd = src.ImprvDetSubClassCd,
                    ConditionCd = src.ConditionCd,
                    ImprvDetArea = src.ImprvDetArea,
                    ImprvDetVal = src.ImprvDetVal,
                ...
                var key = (src.PropValYr, src.SupNum, src.PropId, src.ImprvId, src.ImprvDetId);
                keyCounts[key] = keyCounts.TryGetValue(key, out var c) ? c + 1 : 1;

                if (!string.IsNullOrEmpty(src.ImprvDetTypeCd))
                {
                    typeHistogram[src.ImprvDetTypeCd] =
                        typeHistogram.TryGetValue(src.ImprvDetTypeCd, out var tc) ? tc + 1 : 1;
                }
                if (src.ImprvDetVal.HasValue) imprvDetValSum += src.ImprvDetVal.Value;
                if (src.ImprvDetArea.HasValue) imprvDetAreaSum += src.ImprvDetArea.Value;
            ...
            await WriteGatesAsync(
                ...
                typeHistogram, imprvDetValSum, imprvDetAreaSum,
            ...
            _logger.LogInformation(
                "PACS imprv_detail landing COMPLETED. batch={BatchId} rows={Rows} duplicates={Dups} types={Types} valSum={VS} areaSum={AS}",
                batch.LoadBatchId, rowsLanded, duplicateKeyViolations,
                typeHistogram.Count, imprvDetValSum, imprvDetAreaSum);

            return new PacsImprvDetailLandingResult
            {
                LoadBatchId = batch.LoadBatchId,
                Status = "COMPLETED",
                ...
                ImprvDetValSum = imprvDetValSum,
                ImprvDetAreaSum = imprvDetAreaSum,
            };
        }
        catch (OperationCanceledException)
        {
            ...
            _logger.LogWarning(
                "PACS imprv_detail landing CANCELLED. batch={BatchId}", batch.LoadBatchId);
            throw;
        }
        catch (Exception ex) when (ex is not OperationCanceledException)
        {
            ...
            _logger.LogError(ex,
                "PACS imprv_detail landing FAILED. batch={BatchId} summary={Summary}",
    ...
    private async Task WriteGatesAsync(
        ...
        decimal imprvDetValSum,
        decimal imprvDetAreaSum,
        CancellationToken cancellationToken)
    {
        var now = DateTime.UtcNow;

        // 1) imprv-detail-distribution — informational PASS.
        ...
        {
            LoadBatchId = batch.LoadBatchId,
            GateName = "imprv-detail-distribution",
        ...
        // 2) imprv-detail-key-uniqueness — doctrine invariant.
        _db.SyncBridgePromotionGateResults.Add(new PromotionGateResult
        {
            LoadBatchId = batch.LoadBatchId,
            GateName = "imprv-detail-key-uniqueness",
            GateStage = "SOURCE_TO_RAW",
            ...
            Detail = duplicateKeyViolations == 0
                ? "every (year, sup, prop_id, imprv_id, imprv_det_id) is unique"
                : $"{duplicateKeyViolations} 5-key tuples appeared more than once",
            ExecutedAt = now,
        });

        // 3) provenance-coverage — assert from DB.
        var unprovenanced = await _db.LegacyPacsRawImprvDetails
        ...
        // 4) imprv-detail-aggregate — informational; sums.
        _db.SyncBridgePromotionGateResults.Add(new PromotionGateResult
        {
            LoadBatchId = batch.LoadBatchId,
            GateName = "imprv-detail-aggregate",
            GateStage = "SOURCE_TO_RAW",
            Status = "PASS",
            Expected = "informational",
            Actual = rowsLanded.ToString(CultureInfo.InvariantCulture),
            Detail = $"imprvDetValSum={imprvDetValSum.ToString(CultureInfo.InvariantCulture)} " +
                     $"imprvDetAreaSum={imprvDetAreaSum.ToString(CultureInfo.InvariantCulture)}",
            ExecutedAt = now,
        });
    ...
    internal static string ComputeRowHash(PacsSourceImprvDetail src)
    {
        var seed = string.Join("|",
            ...
            src.ImprvDetMethCd ?? "",
            src.ImprvDetClassCd ?? "",
            src.ImprvDetSubClassCd ?? "",
            src.ConditionCd ?? "",
            src.ImprvDetArea?.ToString(CultureInfo.InvariantCulture) ?? "",
            src.ImprvDetVal?.ToString(CultureInfo.InvariantCulture) ?? "",
            src.NumUnits?.ToString(CultureInfo.InvariantCulture) ?? "",
            src.YrBuilt?.ToString(CultureInfo.InvariantCulture) ?? "");
