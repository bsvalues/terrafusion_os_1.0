...
import { WorkflowEngine, WorkflowDefinition, WorkflowStep, WorkflowState } from '../workflow';
...
describe('WorkflowEngine', () => {
  ...
  test('should handle errors in workflow steps', async () => {
    ...
    await expect(workflowEngine.executeWorkflow('errorWorkflow', {})).rejects.toThrow(
      'Error executing workflow step errorStep: Test error'
    );
  ...
  test('should throw error for non-existent workflow', async () => {
    await expect(workflowEngine.executeWorkflow('nonExistentWorkflow', {})).rejects.toThrow(
      'Workflow nonExistentWorkflow not found'
    );
  });
});
