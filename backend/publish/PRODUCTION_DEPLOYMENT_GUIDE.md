...
This guide provides step-by-step instructions for deploying Terrafusion OS 1.0
into production using the standalone version.
...
- **API Status**: http://localhost:\${{TF_API_PORT:-5000}}/health
- **Main Endpoint**: http://localhost:\${{TF_API_PORT:-5000}}/
- **Test Endpoint**: http://localhost:\${{TF_API_PORT:-5000}}/api/test
...
- **Firewall**: Ensure port \${{TF_API_PORT:-5000}} is open
...
- Check if port \${{TF_API_PORT:-5000}} is available
...
Once the API is running and responding to health checks, your Terrafusion OS 1.0
is successfully deployed in production!
