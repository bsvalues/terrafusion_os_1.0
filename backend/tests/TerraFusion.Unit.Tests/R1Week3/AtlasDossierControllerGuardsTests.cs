...
public class AtlasDossierControllerGuardsTests
{
  ...
  private static AtlasController CreateAtlasController(TerraFusionDbContext db)
  {
    var service = new AtlasService(db, NullLogger<AtlasService>.Instance);
    return new AtlasController(db, NullLogger<AtlasController>.Instance, service);
  }
  ...
  public async Task Atlas_InvalidParcelId_ReturnsBadRequest()
  {
    ...
    var controller = CreateAtlasController(db);
  ...
  public async Task Atlas_SameCountyParcel_ReturnsExplicitGeometryUnavailable()
  {
    ...
    Assert.NotEqual(JsonValueKind.Null, root.GetProperty("centroid").ValueKind);
  }
