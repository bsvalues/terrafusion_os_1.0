...
using AtlasService = TerraFusion.Core.Services.AtlasService;
...
public sealed class R1Week5CxR1ClosureTests
{
  ...
  [Fact]
  public async Task AtlasController_GetLayers_WithoutCountyClaims_ReturnsForbid()
  {
    using var db = CreateDbContext(nameof(AtlasController_GetLayers_WithoutCountyClaims_ReturnsForbid));
    var atlasService = new AtlasService(db, NullLogger<AtlasService>.Instance);
    var controller = new AtlasController(db, NullLogger<AtlasController>.Instance, atlasService);
    AttachPrincipal(controller, CreateEmptyPrincipal());

    var result = await controller.GetLayers();
  ...
  public async Task AtlasController_SearchParcels_WithoutCountyClaims_ReturnsForbid()
  {
    ...
    var result = await controller.SearchParcels(new TerraFusion.Core.Services.AtlasParcelSearchRequest { Query = "123 Main" });
