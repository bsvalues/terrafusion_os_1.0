...
using PiltService = TerraFusion.Core.Services.IPiltService;
...
public sealed class R1Week5CxR1ClosureTests
{
  ...
  [Fact]
  public async Task PiltController_Status_WithoutCountyClaims_ReturnsForbid()
  {
    await using var db = CreateDbContext(nameof(PiltController_Status_WithoutCountyClaims_ReturnsForbid));
    var piltService = new Mock<PiltService>(MockBehavior.Strict);
    var controller = new PiltController(NullLogger<PiltController>.Instance, piltService.Object, db);
    AttachPrincipal(controller, CreateEmptyPrincipal());
