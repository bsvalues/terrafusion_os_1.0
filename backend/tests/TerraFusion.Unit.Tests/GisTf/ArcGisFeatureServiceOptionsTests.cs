...
public sealed class ArcGisFeatureServiceOptionsTests
{
    ...
    {
        const string countyA = "53005"; // Benton WA
        const string countyB = "53011"; // Clark WA
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
    ...
    [Fact]
    public void GetForCounty_UnknownFips_ReturnsNull()
    {
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                ["ArcGisFeatureServices:Counties:53005:ParcelFeatureServiceUrl"]
    ...
    [Fact]
    public void GetForCounty_IsCaseInsensitive_OnFipsString()
    {
        // Config keys may arrive in any casing (unusual for FIPS but
        // the dictionary contract is case-insensitive regardless).
        const string mixedKey = "WA-BENTON"; // hypothetical alphanumeric FIPS variant
        var configuration = new ConfigurationBuilder()
            .AddInMemoryCollection(new Dictionary<string, string?>
            {
                [$"ArcGisFeatureServices:Counties:{mixedKey}:ParcelFeatureServiceUrl"]
                    = "https://example/FeatureServer/0",
            })
            .Build();
        ...
        bound.GetForCounty("wa-benton").Should().NotBeNull();
        bound.GetForCounty("WA-BENTON").Should().NotBeNull();
    }
