...
export class AISwarmCoordinator extends EventEmitter {
  ...
  private async distributeAgentsToModules(): Promise<void> {
    ...
    const sortedModules = Object.entries(this.MODULE_PRIORITY)
      .sort(([,a], [,b]) => b - a);
  ...
  public async allocateTask(task: Partial<TaskAllocation>): Promise<string[]> {
    ...
    const assignedAgents = candidateAgents.slice(0, task.estimatedDuration ? 
      Math.ceil(task.estimatedDuration / 1000) : 1);
  ...
  private generateCapabilities(tier: AgentTier): string[] {
    const baseCapabilities = {
      [AgentTier.CommandBrain]: [
        'strategic-planning', 'decision-making', 'resource-allocation',
        'system-optimization', 'multi-module-coordination'
      ],
      [AgentTier.SwarmCoordinator]: [
        'task-coordination', 'agent-management', 'load-balancing',
        'performance-monitoring', 'conflict-resolution'
      ],
      [AgentTier.AdvancedProcessor]: [
        'complex-analysis', 'pattern-recognition', 'data-processing',
        'algorithm-execution', 'quality-assurance'
      ],
      [AgentTier.SpecialistWorker]: [
        'domain-expertise', 'task-execution', 'data-validation',
        'report-generation', 'user-interaction'
      ],
      [AgentTier.MicroOptimizer]: [
        'fine-tuning', 'performance-optimization', 'error-correction',
        'detail-analysis', 'efficiency-improvement'
      ]
    };
  ...
  private calculateModuleAgentRequirement(moduleName: string, priority: number): Record<AgentTier, number> {
  ...
  private updateSwarmMetrics(): void {
    ...
    this.metrics = {
      ...
      errorRate: agents.reduce((sum, a) => sum + a.performance.errorCount, 0) / Math.max(1, agents.reduce((sum, a) => sum + a.performance.tasksCompleted, 0))
    };
  }

  private calculateSwarmCoherence(): number {
    const agents = Array.from(this.agents.values());
    const avgPerformance = agents.reduce((sum, a) => sum + a.performance.successRate, 0) / agents.length;
    const performanceVariance = agents.reduce((sum, a) => sum + Math.pow(a.performance.successRate - avgPerformance, 2), 0) / agents.length;
