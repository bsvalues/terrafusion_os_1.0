...
export abstract class BaseDevOpsAgent extends EventEmitter {
  ...
  async initialize(): Promise<void> {
    ...
    console.log(`✅ Agent ready: ${this.config.name} (${this.config.capabilities.length} capabilities)`);
  ...
  async executeTask(payload: DevOpsTaskPayload): Promise<any> {
    if (this.activeTasks.size >= this.config.maxConcurrentTasks) {
      throw new Error(`Agent ${this.config.name} at max capacity (${this.config.maxConcurrentTasks} tasks)`);
    }
  ...
  private startMetricsCollection(): void {
    setInterval(() => {
      ...
      this.metrics.resourceUtilization.network = this.activeTasks.size > 0 ? Math.random() * 100 : 0;
  ...
  getHealthScore(): number {
    const successRate = this.metrics.tasksExecuted > 0 ? 
      this.metrics.successfulTasks / this.metrics.tasksExecuted : 1.0;
