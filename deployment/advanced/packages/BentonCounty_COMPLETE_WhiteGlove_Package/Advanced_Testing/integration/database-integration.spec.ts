...
describe('Database Integration Tests', () => {
  ...
  describe('Basic Database Operations', () => {
    test(
      'Should perform health checks on all databases',
      async () => {
    ...
    test(
      'Should save and retrieve app data correctly',
      async () => {
        const testData = {
          testKey: 'test-value',
          timestamp: Date.now(),
    ...
    test(
      'Should handle data isolation between apps',
      async () => {
        const testData = {
    ...
    test(
      ...
      async () => {
        ...
        for (let i = 0; i < CONCURRENT_OPERATIONS; i++) {
          const promise = db.save_app_data(appId, `concurrent-key-${i}`, {
  ...
  describe('Transaction Tests', () => {
    test(
      'Should maintain ACID properties in transactions',
      async () => {
        ...
        try {
          const result = await db.execute_transaction(async tx => {
            ...
            await tx.execute('INSERT INTO app_data (app_id, key, value) VALUES (?, ?, ?)', [
              'test-transaction-1',
              'tx-key-1',
              JSON.stringify({ value: 'tx-value-1' }),
            ]);

            await tx.execute('INSERT INTO app_data (app_id, key, value) VALUES (?, ?, ?)', [
              'test-transaction-1',
              'tx-key-2',
              JSON.stringify({ value: 'tx-value-2' }),
            ]);
      ...
      async () => {
        ...
        try {
          await db.execute_transaction(async tx => {
            // Valid operation
            await tx.execute('UPDATE app_data SET value = ? WHERE app_id = ? AND key = ?', [
              JSON.stringify({ value: 'should-not-persist' }),
              'test-rollback',
              'rollback-key',
            ]);
  ...
  describe('Performance and Stress Tests', () => {
    test('Should handle high-volume operations efficiently', async () => {
      ...
      console.log(
        `   - Write rate: ${((STRESS_TEST_OPERATIONS / writeTime) * 1000).toFixed(2)} ops/sec`
      );
    ...
    test(
      ...
      async () => {
        ...
        results.forEach(r => {
          console.log(
            `   - ${r.appId}: Active=${r.stats.active_connections}, Max=${r.stats.max_connections}`
          );
        });
      },
      TEST_TIMEOUT
  ...
  describe('Data Consistency Tests', () => {
    test(
      'Should maintain data consistency across app restarts',
      async () => {
    ...
    test(
      'Should handle schema version consistency',
      async () => {
        const db = testDatabases.get('marketplace')!;

        // Check schema version
        const versionRow = await db.fetch_one(
    ...
    test(
      'Should handle concurrent modifications correctly',
      async () => {
  ...
  describe('Error Recovery Tests', () => {
    test(
      'Should recover from connection failures gracefully',
      async () => {
    ...
    test(
      'Should handle database file corruption detection',
      async () => {
