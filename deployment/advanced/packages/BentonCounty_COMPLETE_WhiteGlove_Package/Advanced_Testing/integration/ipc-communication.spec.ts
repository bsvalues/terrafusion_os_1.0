...
describe('IPC Communication Integration Tests', () => {
  ...
  describe('Basic IPC Functionality', () => {
    test(
      'All apps should register successfully',
      async () => {
    ...
    test(
      'Apps should respond to handshake requests',
      async () => {
  ...
  describe('Message Passing Tests', () => {
    test(
      'Should broadcast messages to all apps',
      async () => {
    ...
    test(
      'Should handle direct app-to-app communication',
      async () => {
    ...
    test(
      'Should handle high-priority messages faster',
      async () => {
  ...
  describe('Data Request/Response Tests', () => {
    test(
      'Should handle data requests between apps',
      async () => {
        const sourceApp = testApps[0];
        const targetApp = testApps[1];
        ...
        try {
          // Request data
          const response = await sourceApp.ipc.requestData(targetApp.id, 'test-data', {
            testParam: 'value',
          });
    ...
    test(
      'Should handle command execution between apps',
      async () => {
        const sourceApp = testApps[0];
        const targetApp = testApps[1];
        ...
        try {
          // Execute command
          const response = await sourceApp.ipc.executeCommand(targetApp.id, 'test-command', {
            testArg: 'value',
          });
  ...
  describe('Error Handling Tests', () => {
    test(
      ...
      async () => {
        ...
        try {
          // This should timeout
          await expect(sourceApp.ipc.requestData(targetApp.id, 'slow-data')).rejects.toThrow(
            'Request timeout'
          );
    ...
    test(
      ...
      async () => {
        ...
        await expect(sourceApp.ipc.requestData('non-existent-app', 'test-data')).rejects.toThrow();
  ...
  describe('Performance Tests', () => {
    test(
      'Should maintain sub-millisecond message latency',
      async () => {
        const sourceApp = testApps[0];
        const targetApp = testApps[1];
    ...
    test(
      'Should handle message bursts without dropping messages',
      async () => {
        const sourceApp = testApps[0];
        const targetApp = testApps[1];
