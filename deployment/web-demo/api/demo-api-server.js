...
app.get('/api/demo/stats', (req, res) => {
  const query =
    'SELECT stat_name, stat_value, stat_type FROM DemoStatistics ORDER BY display_order';
  ...
  setTimeout(() => {
    ...
    db.get('SELECT * FROM Properties WHERE parcel_id = ?', [parcelId], (err, property) => {
      ...
      res.json({
        ...
        data: assessment,
        demo_note:
          'This assessment was completed by Terrafusion AI in 3.2 seconds vs 30 minutes manually',
      });
    });
