...
test('Seal Gate job uses pnpm caching', () => {
  ...
  const hasPnpmCache = sealGate.includes('cache: pnpm') || sealGate.includes("cache: 'pnpm'");
...
test('Seal Gate pnpm cache uses correct dependency path', () => {
  ...
  const hasDependencyPath =
    sealGate.includes('cache-dependency-path:') && sealGate.includes('pnpm-lock.yaml');
...
test('Seal Gate job uses NuGet caching', () => {
  ...
  const hasNuGetCache =
    sealGate.includes('actions/cache@v4') &&
...
test('Seal Gate NuGet cache uses deterministic key', () => {
  ...
  const hasDotnetLockfiles =
    sealGate.includes('.csproj') ||
...
test('Seal Gate has pnpm install step with frozen lockfile', () => {
  ...
  const hasFrozenLockfile =
    sealGate.includes('pnpm install') && sealGate.includes('--frozen-lockfile');
