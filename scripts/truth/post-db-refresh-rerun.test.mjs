...
test('preserves PASS_WITH_WARNINGS from refreshed proof artifact status', () => {
      ...
      TF_POST_DB_REFRESH_COMMANDS_JSON: JSON.stringify([
        {
          ...
          args: [
            ...
            [
              ...
              "fs.writeFileSync('generated/truth/scalar-warnings.json', JSON.stringify({ warning: 'top warning', warnings: 'top warnings string', summary: { status: 'PASS_WITH_WARNINGS', warning: 'summary warning', warnings: { one: 'summary warning', two: 'summary warning' } }, receiptEvidence: { status: 'PASS_WITH_WARNINGS', warning: 'receipt warning', warnings: { one: 'receipt warning', two: 'receipt warning' }, summary: { status: 'PASS_WITH_WARNINGS', warning: 'receipt summary warning' }, rows: [{ county: 'Benton', status: 'PASS_WITH_WARNINGS', warning: 'receipt row warning', summary: { status: 'PASS_WITH_WARNINGS', warningCount: 2 } }], proofs: [{ county: 'Benton', status: 'PASS_WITH_WARNINGS', warningCount: 6, summary: { status: 'PASS_WITH_WARNINGS', warning: 'receipt proof summary warning' } }] }, rows: [{ county: 'Benton', status: 'PASS_WITH_WARNINGS', warning: 'row warning', warnings: 'row warnings string', summary: { status: 'PASS_WITH_WARNINGS' } }], proofs: [{ county: 'Benton', status: 'PASS_WITH_WARNINGS', warning: 'proof warning', warnings: { one: 'proof warning' }, summary: { status: 'PASS_WITH_WARNINGS' } }] }) + '\\n');",
            ].join(' '),
          ],
          expectedArtifacts: [
  ...
  assert.equal(report.nextAction.command, 'pnpm run readiness:june10');
  assert.equal(report.summary.artifactWarnings, 41);
  ...
  assert.equal(report.results[0].artifactOutputs[3].warningCount, 34);
  const markdown = fs.readFileSync(
    path.join(root, 'generated', 'truth', 'post-db-refresh-rerun.md'),
    'utf8'
  );
  assert.match(markdown, /Artifact warnings: 41/);
...
test('fails when a proof command writes string false proof posture fields', () => {
  const root = makeTempRepo('tf-post-db-refresh-string-false-fail-');
  const result = spawnSync('node', [scriptPath, root], {
    cwd: process.cwd(),
    encoding: 'utf8',
    timeout: 10_000,
    env: {
      ...
      TF_POST_DB_REFRESH_COMMANDS_JSON: JSON.stringify([
        {
          name: 'String false artifact writer command',
          command: process.execPath,
          args: [
            '-e',
            [
              "const fs = require('fs');",
              "fs.mkdirSync('generated/truth', { recursive: true });",
              "fs.writeFileSync('generated/truth/string-false-fail.json', JSON.stringify({ passed: 'false', summary: { passed: 'false', dbIdentityPassed: 'false' }, rows: [{ county: 'Benton', passed: 'false' }], checks: [{ propertyRowsMatchExpected: 'false' }] }) + '\\n');",
            ].join(' '),
          ],
          expectedArtifacts: ['generated/truth/string-false-fail.json'],
        },
      ]),
    },
  });
  ...
  const report = readReport(root);
  assert.equal(report.status, 'FAIL');
  assert.equal(report.results[0].artifactOutputs[0].artifactPassed, 'false');
  assert.deepEqual(report.results[0].artifactOutputs[0].artifactFailureReasons, [
...
test('fails when a proof command writes non-boolean passed proof posture fields', () => {
  const root = makeTempRepo('tf-post-db-refresh-non-boolean-passed-');
  const result = spawnSync('node', [scriptPath, root], {
    cwd: process.cwd(),
    encoding: 'utf8',
    timeout: 10_000,
    env: {
      ...
      TF_POST_DB_REFRESH_COMMANDS_JSON: JSON.stringify([
        {
          name: 'Non-boolean passed artifact writer command',
          command: process.execPath,
          args: [
            '-e',
            [
              "const fs = require('fs');",
              "fs.mkdirSync('generated/truth', { recursive: true });",
              "fs.writeFileSync('generated/truth/non-boolean-passed.json', JSON.stringify({ passed: 'true', summary: { passed: 1 }, receiptEvidence: { passed: 1 }, rows: [{ county: 'Benton', passed: 'true' }], proofs: [{ county: 'Benton', passed: 1 }] }) + '\\n');",
            ].join(' '),
          ],
          expectedArtifacts: ['generated/truth/non-boolean-passed.json'],
        },
      ]),
    },
  });
  ...
  assert.equal(report.status, 'FAIL');
  assert.deepEqual(report.results[0].artifactOutputs[0].artifactFailureReasons, [
    'artifact passed is not boolean',
    'receiptEvidence passed is not boolean',
    'Benton row passed is not boolean',
    'Benton proof passed is not boolean',
  ]);
...
test('fails when a proof command writes alternate status field casing and separators', () => {
  const root = makeTempRepo('tf-post-db-refresh-alt-status-field-fail-');
  const result = spawnSync('node', [scriptPath, root], {
    cwd: process.cwd(),
    encoding: 'utf8',
    timeout: 10_000,
    env: {
      ...
      TF_POST_DB_REFRESH_COMMANDS_JSON: JSON.stringify([
        {
          name: 'Alternate status field artifact writer command',
          command: process.execPath,
          args: [
            '-e',
            [
              "const fs = require('fs');",
              "fs.mkdirSync('generated/truth', { recursive: true });",
              "fs.writeFileSync('generated/truth/alt-status-field-fail.json', JSON.stringify({ passed: true, checks: [{ STATUS: 'fail', nested: { validation_status: 'error', pipelinestatus: 'dry_run' } }], summary: { closure_status: 'failed' } }) + '\\n');",
            ].join(' '),
          ],
          expectedArtifacts: ['generated/truth/alt-status-field-fail.json'],
        },
      ]),
    },
  });
  ...
  assert.deepEqual(report.results[0].artifactOutputs[0].artifactFailureReasons, [
    'artifact.checks.[].STATUS is fail',
    'artifact.checks.[].nested.validation_status is error',
    'artifact.checks.[].nested.pipelinestatus is dry_run',
    'artifact.summary.closure_status is failed',
  ]);
  assert.ok(report.blockers.some(item => item.includes('checks.[].STATUS is fail')));
...
test('fails when a proof command writes nested arbitrary blocker and failure collections', () => {
  ...
  const result = spawnSync('node', [scriptPath, root], {
    ...
    env: {
      ...
      TF_POST_DB_REFRESH_COMMANDS_JSON: JSON.stringify([
        {
          name: 'Nested collection failure artifact writer command',
          command: process.execPath,
          args: [
            '-e',
            [
              "const fs = require('fs');",
              "fs.mkdirSync('generated/truth', { recursive: true });",
              "fs.writeFileSync('generated/truth/nested-collection-fail.json', JSON.stringify({ passed: true, checks: [{ blockers: ['Nested readiness blocker.'], errors: ['Nested readiness error.'], metrics: { failureCount: 1 } }] }) + '\\n');",
            ].join(' '),
          ],
          expectedArtifacts: ['generated/truth/nested-collection-fail.json'],
        },
      ]),
    },
  });
  ...
  assert.equal(report.status, 'FAIL');
  assert.deepEqual(report.results[0].artifactOutputs[0].artifactFailureReasons, [
    'artifact.checks.[].blockers has 1 item(s)',
    'artifact.checks.[].errors has 1 item(s)',
    'artifact.checks.[].metrics.failureCount is 1',
  ]);
  assert.ok(report.blockers.some(item => item.includes('checks.[].blockers has 1 item(s)')));
  assert.ok(report.blockers.some(item => item.includes('metrics.failureCount is 1')));
});
