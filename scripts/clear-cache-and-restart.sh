...
echo "   Checking port \${{TF_FRONTEND_PORT:-3000}}..."
lsof -ti:${TF_FRONTEND_PORT:-3102} | xargs kill -9 2>/dev/null && echo "   ✅ Stopped process on port \${{TF_FRONTEND_PORT:-3000}}" || echo "   ℹ️  No process running on port \${{TF_FRONTEND_PORT:-3000}}"

echo "   Checking port \${{TF_FRONTEND_PORT:-3000}} (Vite)..."
lsof -ti:${TF_DEV_VITE_PORT:-3102} | xargs kill -9 2>/dev/null && echo "   ✅ Stopped process on port \${{TF_FRONTEND_PORT:-3000}}" || echo "   ℹ️  No process running on port \${{TF_FRONTEND_PORT:-3000}}"
