...
run_migrations() {
    ...
    timeout 120 bash -c 'until curl -f -s http://localhost:${TF_STATIC_PORT:-8080}/health > /dev/null; do sleep 5; done'
...
verify_deployment() {
    ...
    if curl -f -s http://localhost:${TF_STATIC_PORT:-8080}/health > /dev/null; then
...
display_summary() {
    ...
    log "• API Health: http://localhost:${TF_STATIC_PORT:-8080}/health"
    log "• API Swagger: http://localhost:${TF_STATIC_PORT:-8080}/swagger (if enabled)"
    log "• Prometheus: http://localhost:${TF_STATIC_PORT:-8080} (if enabled)"
    log "• Grafana: http://localhost:${TF_STATIC_PORT:-8080} (if enabled)"
    log ""
