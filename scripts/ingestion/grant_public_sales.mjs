...
const COUNTY='Grant',CODE='025',OFFICIAL='https://www.grantcountywa.gov';
const MODE='public_assessor_propertyaccess_sale_search_current_year';
const SEARCH='https://propertysearch.grantcountywa.gov/PropertyAccess/SaleSearch.aspx?cid=10';
const RESULTS='https://propertysearch.grantcountywa.gov/PropertyAccess/SearchResultsSales.aspx?cid=10';
const PAYLOAD='grant-propertyaccess-sale-search-2026-sanitized.json';
...
function date(v){const m=/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/.exec(String(v??'').trim());inv(m,`bad Grant date ${v}`);return`${m[3]}-${m[1].padStart(2,'0')}-${m[2].padStart(2,'0')}`}
...
async function fetchSales(){
  ...
  inv(cookie,'missing Grant session cookie');
...
async function main(){
  ...
  inv(records.length>0,'no stageable Grant sales');
