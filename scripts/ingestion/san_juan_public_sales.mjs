...
const COUNTY = 'San Juan';
const CODE = '055';
const OFFICIAL = 'https://www.sanjuanco.com';
const MODE = 'public_assessor_propertyaccess_sale_search_current_year';
const SEARCH = 'https://parcel.sanjuancountywa.gov/PropertyAccess/SaleSearch.aspx?cid=0';
const RESULTS = 'https://parcel.sanjuancountywa.gov/PropertyAccess/SearchResultsSales.aspx?cid=0';
const PAYLOAD = 'san-juan-propertyaccess-sale-search-2026-sanitized.json';
...
function date(value) {
  ...
  inv(match, `bad San Juan date ${value}`);
...
async function fetchSales() {
  ...
  inv(cookie, 'missing San Juan session cookie');
  ...
  inv(post.status === 302, `San Juan search POST expected 302 got ${post.status}`);
  ...
  inv(Number.isInteger(count) && count > 0, 'missing San Juan result count');
  ...
  for (let page = 1; page <= pages; page++) {
    ...
    inv(pageRows.length === expected, `San Juan page ${page} row count ${pageRows.length} != ${expected}`);
    rows.push(...pageRows);
  }
  inv(rows.length === count, `San Juan parsed row count ${rows.length} != advertised ${count}`);
...
function mapRecord(row, ordinal, payloadHash, generatedDate) {
  ...
  return {
    ...
    sourceMode: MODE,
    candidateSource: 'san-juan-official-propertyaccess-sale-search-current-year',
...
async function main() {
  ...
  inv(records.length === payload.count, 'San Juan record filter changed row count');
  inv(new Set(records.map(record => record.saleId)).size === records.length, 'duplicate San Juan sale IDs');
  const review = 0;
