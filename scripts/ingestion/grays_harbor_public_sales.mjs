...
const COUNTY='Grays Harbor',CODE='027',OFFICIAL='https://www.co.grays-harbor.wa.us';
const MODE='public_assessor_taxsifter_sale_search_current_year';
const BASE='https://graysharborwa-taxsifter.publicaccessnow.com';
const DISCLAIMER=`${BASE}/Disclaimer.aspx`;
const SEARCH=`${BASE}/SalesSearch/SalesSearch.aspx`;
const PAYLOAD='grays-harbor-taxsifter-sale-search-2026-sanitized.json';
...
function date(v){const m=/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/.exec(String(v??'').trim());inv(m,`bad Grays Harbor date ${v}`);return`${m[3]}-${m[1].padStart(2,'0')}-${m[2].padStart(2,'0')}`}
...
async function fetchSales(){
  ...
  inv(cookie(),'missing Grays Harbor session cookie');
  ...
  inv(Number.isInteger(count)&&count>0,'missing Grays Harbor result count');
  ...
  for(let guard=0;guard<pages+2;guard++){
    ...
    inv(Number.isInteger(idx)&&idx>0,`missing Grays Harbor page index at guard ${guard}`);
    inv(!seen.has(idx),`repeated Grays Harbor page ${idx}`);
    inv(parsed.length>0,`no Grays Harbor rows on page ${idx}`);
    seen.add(idx);
    rows.push(...parsed);
  ...
  inv(seen.size===pages,`Grays Harbor page count ${seen.size} != expected ${pages}`);
  inv(rows.length>0&&rows.length<=count,`Grays Harbor parsed rows ${rows.length} outside advertised ${count}`);
  const uniqueRows=[]; const rowIds=new Set();
  for(const row of rows){
    const rowId=[row.parcelNumber,row.saleDate,row.documentNumber,row.salePrice,row.useCode].join('|');
    if(rowIds.has(rowId))continue;
    rowIds.add(rowId);
    uniqueRows.push(row);
  }
  return{count:uniqueRows.length,advertisedCount:count,parsedRows:rows.length,duplicateParsedRows:rows.length-uniqueRows.length,pages,rows:uniqueRows,search:{url:SEARCH,start:START,end:END,pages,advertisedCount:count,parsedRows:rows.length,uniqueRows:uniqueRows.length,duplicateParsedRows:rows.length-uniqueRows.length}};
}
function map(a,i,payloadHash,genDate){
  ...
  return{saleId:`WA-${CODE}-${createHash('sha256').update(id).digest('hex').slice(0,32)}`,county:COUNTY,countyCode:CODE,parcelNumber:a.parcelNumber,saleDate:a.saleDate,saleYear:Number(a.saleDate.slice(0,4)),salePrice:a.salePrice,adjustedSalePrice:null,documentNumber:a.documentNumber,deedType:null,situsAddress:a.situsAddress,situsCity:null,situsZip:null,useCode:a.useCode,acres:a.acres,grantor:null,grantee:null,saleNote:null,neighborhoodCode:null,currentNeighborhoodCode:null,sourceMode:MODE,candidateSource:'grays-harbor-official-taxsifter-sale-search-current-year',confidenceScore:0.95,qualityScore:0.95,qualityBand:'official_assessor_sale_search_public_record',reviewStatus:'ready',grossLivingArea:null,lotSizeSqft:null,yearBuilt:null,bedrooms:null,bathrooms:null,condition:null,qualityGrade:null,provenance:{sourceUrl:SEARCH,sourceFinalUrl:SEARCH,sourcePayloadPath:PAYLOAD,sourcePayloadSha256:payloadHash,candidateIndexSource:`${PAYLOAD}#page:${a.page}:parcel:${a.parcelNumber}:document:${a.documentNumber??'unknown'}`,candidateRecordType:'official-taxsifter-sale-search-row',candidateSourceOrdinal:i,componentRows:[{sourceKey:'sale-search-row',sourceUrl:SEARCH,sourcePayloadPath:PAYLOAD,sourcePayloadSha256:payloadHash,candidateIndexSource:`${PAYLOAD}#page:${a.page}:parcel:${a.parcelNumber}:document:${a.documentNumber??'unknown'}`}]},flags:{duplicateRisk:false,needsReview:false,futureSaleDate:false,manualException:false}};
}
...
async function main(){
  ...
  inv(records.length===payload.rows.length,'Grays Harbor record filter changed row count');
  const ids=new Set();for(const record of records){inv(!ids.has(record.saleId),`duplicate ${record.saleId}`);ids.add(record.saleId)}
  const review=Math.max(0,(payload.advertisedCount??payload.count)-records.length),latest=records[0].saleDate,salesRoute=`/launch-data/washington/sales/by-county/${CODE}.json`,detailRoute=`/launch-data/washington/counties/${CODE}.json`;
  ...
  const statusEntry={county:COUNTY,countyCode:CODE,priority:'washington_assessor_launch',prometheusStatus:'public_data_ready',primarySourceMode:MODE,latestSaleDate:latest,candidateSales:(payload.advertisedCount??payload.count),stagedSales:records.length,needsReview:review,confidence:{averageQualityScore:0.95,parserStatus:'ready',rawStatus:'official_taxsifter_sale_search_verified',rawDriftDetected:false},staticRoutes:{detail:detailRoute,salesShard:salesRoute}};
  ...
  await wj(join(ROOT,'receipts','grays-harbor-source.json'),{schemaVersion:'terrafusion.washington.public-source-receipt.v1',county:COUNTY,countyCode:CODE,generatedAt:GENERATED_AT,sourceUrl:SEARCH,sourceFinalUrl:SEARCH,sourcePayloadPath:PAYLOAD,sourcePayloadRecords:(payload.advertisedCount??payload.count),sourcePayloadSha256:payloadHash,candidateSales:(payload.advertisedCount??payload.count),stagedSales:records.length,quarantinedSales:(payload.advertisedCount??payload.count)-records.length,needsReview:review,searchWindow:{start:START,end:END},pages:payload.pages,omittedFields:['owner','grantor','grantee','buyer','seller','legalDescription','geometry','map']});
  console.log(JSON.stringify({county:COUNTY,countyCode:CODE,sourcePayloadSha256:payloadHash,candidateRecords:(payload.advertisedCount??payload.count),stagedRecords:records.length,quarantinedRecords:(payload.advertisedCount??payload.count)-records.length,reviewRecords:review,latestSaleDate:latest,manifestCanonicalJsonSha256:sha(manifest)},null,2));
}
await main();
