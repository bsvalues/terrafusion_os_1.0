...
const COUNTY='Benton',CODE='005',OFFICIAL='https://co.benton.wa.us';
const MODE='public_assessor_propertyaccess_sale_search_current_year';
const SEARCH='https://propertysearch.co.benton.wa.us/PropertyAccess/SaleSearch.aspx?cid=0';
const RESULTS='https://propertysearch.co.benton.wa.us/PropertyAccess/SearchResultsSales.aspx?cid=0';
const PAYLOAD='benton-propertyaccess-sale-search-2026-sanitized.json';
...
function date(v){const m=/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/.exec(String(v??'').trim());inv(m,`bad Benton date ${v}`);return`${m[3]}-${m[1].padStart(2,'0')}-${m[2].padStart(2,'0')}`}
function money(v){const n=Number(String(v??'').replace(/[$,\s]/g,''));return Number.isFinite(n)&&n>0?n:null}
async function fetchHtml(url,cookie){const r=await fetch(url,{headers:{cookie,accept:'text/html'},redirect:'manual'});inv(r.ok,`GET ${url} HTTP ${r.status}`);return await r.text()}
function parseRows(html,page){
  ...
  for(const m of html.matchAll(/<tr[\s\S]*?<\/tr>/gi)){
    ...
    const parcelNumber=cells[4]||cells[3]||null,saleDate=date(cells[11]),salePrice=money(cells[12]),documentNumber=(cells[14]||cells[13]||cells[2]||null),situsAddress=cells[8]||null,useCode=cells[7]||null;
...
async function fetchSales(){
  ...
  inv(cookie,'missing Benton session cookie');
  ...
  const post=await fetch(SEARCH,{method:'POST',headers:{cookie,referer:SEARCH,'content-type':'application/x-www-form-urlencoded',accept:'text/html'},body,redirect:'manual'});
  inv(post.status===302,`Benton search POST expected 302 got ${post.status}`);
  ...
  inv(Number.isInteger(count)&&count>0,'missing Benton result count');
  ...
  for(let page=1;page<=pages;page++){
    ...
    inv(page===pages?pageRows.length===count-(pages-1)*25:pageRows.length===25,`Benton page ${page} row count ${pageRows.length}`);
    rows.push(...pageRows);
  }
  inv(rows.length===count,`Benton parsed row count ${rows.length} != advertised ${count}`);
...
function map(a,i,payloadHash,genDate){
  ...
  const id=[CODE,a.parcelNumber,a.documentNumber,a.saleDate,a.salePrice,a.useCode,i].map(v=>String(v??'').trim()).join('|');
  return{saleId:`WA-${CODE}-${createHash('sha256').update(id).digest('hex').slice(0,32)}`,county:COUNTY,countyCode:CODE,parcelNumber:a.parcelNumber,saleDate:a.saleDate,saleYear:Number(a.saleDate.slice(0,4)),salePrice:a.salePrice,adjustedSalePrice:null,documentNumber:a.documentNumber,deedType:null,situsAddress:a.situsAddress,situsCity:null,situsZip:null,useCode:a.useCode,acres:null,grantor:null,grantee:null,saleNote:null,neighborhoodCode:null,currentNeighborhoodCode:null,sourceMode:MODE,candidateSource:'benton-official-propertyaccess-sale-search-current-year',confidenceScore:0.95,qualityScore:0.95,qualityBand:'official_assessor_sale_search_public_record',reviewStatus:'ready',grossLivingArea:null,lotSizeSqft:null,yearBuilt:null,bedrooms:null,bathrooms:null,condition:null,qualityGrade:null,provenance:{sourceUrl:SEARCH,sourceFinalUrl:RESULTS,sourcePayloadPath:PAYLOAD,sourcePayloadSha256:payloadHash,candidateIndexSource:`${PAYLOAD}#page:${a.page}:parcel:${a.parcelNumber}:document:${a.documentNumber??'unknown'}:ordinal:${i}`,candidateRecordType:'official-propertyaccess-sale-search-row',candidateSourceOrdinal:i,componentRows:[{sourceKey:'sale-search-row',sourceUrl:SEARCH,sourcePayloadPath:PAYLOAD,sourcePayloadSha256:payloadHash,candidateIndexSource:`${PAYLOAD}#page:${a.page}:parcel:${a.parcelNumber}:document:${a.documentNumber??'unknown'}:ordinal:${i}`}]},flags:{duplicateRisk:false,needsReview:false,futureSaleDate:false,manualException:false}};
}
...
async function main(){
  ...
  inv(records.length===payload.count,'Benton record filter changed row count');
