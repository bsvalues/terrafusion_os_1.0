...
const COUNTY = 'Asotin';
const CODE = '003';
const OFFICIAL = 'https://co.asotin.wa.us';
const MODE = 'public_assessor_propertyaccess_sale_search_current_year';
const SEARCH = 'https://propertysearch.trueautomation.com/PropertyAccess/SaleSearch.aspx?cid=10';
const RESULTS = 'https://propertysearch.trueautomation.com/PropertyAccess/SearchResultsSales.aspx?cid=10';
const PAYLOAD = 'asotin-propertyaccess-sale-search-2026-sanitized.json';
...
function date(value) {
  ...
  inv(match, `bad Asotin date ${value}`);
...
async function fetchSales() {
  ...
  inv(cookie, 'missing Asotin session cookie');
  ...
  inv(post.status === 302, `Asotin search POST expected 302 got ${post.status}`);
  ...
  inv(Number.isInteger(count) && count > 0, 'missing Asotin result count');
  ...
  for (let page = 1; page <= pages; page++) {
    ...
    inv(pageRows.length === expected, `Asotin page ${page} row count ${pageRows.length} != ${expected}`);
    rows.push(...pageRows);
  }
  inv(rows.length === count, `Asotin parsed row count ${rows.length} != advertised ${count}`);
...
function mapRecord(row, ordinal, payloadHash, generatedDate) {
  ...
  return {
    ...
    sourceMode: MODE,
    candidateSource: 'asotin-official-propertyaccess-sale-search-current-year',
...
async function main() {
  ...
  inv(records.length === payload.count, 'Asotin record filter changed row count');
  inv(new Set(records.map(record => record.saleId)).size === records.length, 'duplicate Asotin sale IDs');
  const review = 0;
