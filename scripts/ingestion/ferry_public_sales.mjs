...
const COUNTY='Ferry',CODE='019',OFFICIAL='https://www.ferry-county.com';
const MODE='public_assessor_taxsifter_sale_search_current_year';
const BASE='https://ferrywa-taxsifter.publicaccessnow.com';
const DISCLAIMER=`${BASE}/Disclaimer.aspx`;
const SEARCH=`${BASE}/SalesSearch/SalesSearch.aspx`;
const PAYLOAD='ferry-taxsifter-sale-search-2026-sanitized.json';
...
function date(v){const m=/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/.exec(String(v??'').trim());inv(m,`bad Ferry date ${v}`);return`${m[3]}-${m[1].padStart(2,'0')}-${m[2].padStart(2,'0')}`}
...
async function fetchSales(){
  ...
  inv(cookie(),'missing Ferry session cookie');
  ...
  inv(Number.isInteger(count)&&count>0,'missing Ferry result count');
  ...
  for(let guard=0;guard<pages+2;guard++){
    ...
    inv(Number.isInteger(idx)&&idx>0,`missing Ferry page index at guard ${guard}`);
    inv(!seen.has(idx),`repeated Ferry page ${idx}`);
    inv(parsed.length>0,`no Ferry rows on page ${idx}`);
    seen.add(idx);
    rows.push(...parsed);
    ...
    inv(next,`missing Ferry next page target after page ${idx}`);
    p=await post(SEARCH,SEARCH,html,{'__EVENTTARGET':next.target,'__EVENTARGUMENT':next.arg});
    html=p.text;
  }
  inv(seen.size===pages,`Ferry page count ${seen.size} != expected ${pages}`);
  inv(rows.length===count,`Ferry parsed rows ${rows.length} != advertised ${count}`);
  inv(new Set(rows.map(r=>[r.parcelNumber,r.saleDate,r.documentNumber,r.salePrice,r.useCode].join('|'))).size===rows.length,'duplicate Ferry staged source row identity');
...
function map(a,i,payloadHash,genDate){
  ...
  return{saleId:`WA-${CODE}-${createHash('sha256').update(id).digest('hex').slice(0,32)}`,county:COUNTY,countyCode:CODE,parcelNumber:a.parcelNumber,saleDate:a.saleDate,saleYear:Number(a.saleDate.slice(0,4)),salePrice:a.salePrice,adjustedSalePrice:null,documentNumber:a.documentNumber,deedType:null,situsAddress:a.situsAddress,situsCity:null,situsZip:null,useCode:a.useCode,acres:a.acres,grantor:null,grantee:null,saleNote:null,neighborhoodCode:null,currentNeighborhoodCode:null,sourceMode:MODE,candidateSource:'ferry-official-taxsifter-sale-search-current-year',confidenceScore:0.95,qualityScore:0.95,qualityBand:'official_assessor_sale_search_public_record',reviewStatus:'ready',grossLivingArea:null,lotSizeSqft:null,yearBuilt:null,bedrooms:null,bathrooms:null,condition:null,qualityGrade:null,provenance:{sourceUrl:SEARCH,sourceFinalUrl:SEARCH,sourcePayloadPath:PAYLOAD,sourcePayloadSha256:payloadHash,candidateIndexSource:`${PAYLOAD}#page:${a.page}:parcel:${a.parcelNumber}:document:${a.documentNumber??'unknown'}`,candidateRecordType:'official-taxsifter-sale-search-row',candidateSourceOrdinal:i,componentRows:[{sourceKey:'sale-search-row',sourceUrl:SEARCH,sourcePayloadPath:PAYLOAD,sourcePayloadSha256:payloadHash,candidateIndexSource:`${PAYLOAD}#page:${a.page}:parcel:${a.parcelNumber}:document:${a.documentNumber??'unknown'}`}]},flags:{duplicateRisk:false,needsReview:false,futureSaleDate:false,manualException:false}};
}
...
async function main(){
  ...
  inv(records.length===payload.count,'Ferry record filter changed row count');
  ...
  await wj(join(ROOT,'receipts','ferry-source.json'),{schemaVersion:'terrafusion.washington.public-source-receipt.v1',county:COUNTY,countyCode:CODE,generatedAt:GENERATED_AT,sourceUrl:SEARCH,sourceFinalUrl:SEARCH,sourcePayloadPath:PAYLOAD,sourcePayloadRecords:payload.count,sourcePayloadSha256:payloadHash,candidateSales:payload.count,stagedSales:records.length,quarantinedSales:payload.count-records.length,needsReview:review,searchWindow:{start:START,end:END},pages:payload.pages,omittedFields:['owner','grantor','grantee','buyer','seller','legalDescription','geometry','map']});
