...
const COUNTY = 'Clallam';
const CODE = '009';
const OFFICIAL = 'https://clallamcountywa.gov';
const MODE = 'public_assessor_propertyaccess_sale_search_current_year';
const SEARCH = 'https://websrv22.clallam.net/propertyaccess/SaleSearch.aspx?cid=0';
const RESULTS = 'https://websrv22.clallam.net/PropertyAccess/SearchResultsSales.aspx?cid=0';
const PAYLOAD = 'clallam-propertyaccess-sale-search-2026-sanitized.json';
...
function date(value) {
  ...
  inv(match, `bad Clallam date ${value}`);
...
async function fetchSales() {
  ...
  inv(cookie, 'missing Clallam session cookie');
  ...
  inv(post.status === 302, `Clallam search POST expected 302 got ${post.status}`);
  ...
  inv(Number.isInteger(count) && count > 0, 'missing Clallam result count');
  ...
  for (let page = 1; page <= pages; page++) {
    ...
    inv(pageRows.length === expected, `Clallam page ${page} row count ${pageRows.length} != ${expected}`);
    rows.push(...pageRows);
  }
  inv(rows.length === count, `Clallam parsed row count ${rows.length} != advertised ${count}`);
...
function mapRecord(row, ordinal, payloadHash, generatedDate) {
  ...
  return {
    ...
    sourceMode: MODE,
    candidateSource: 'clallam-official-propertyaccess-sale-search-current-year',
...
async function main() {
  ...
  inv(records.length === payload.count, 'Clallam record filter changed row count');
  inv(new Set(records.map(record => record.saleId)).size === records.length, 'duplicate Clallam sale IDs');
  const review = 0;
