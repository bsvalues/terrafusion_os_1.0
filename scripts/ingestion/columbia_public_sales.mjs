...
const COUNTY='Columbia',CODE='013',OFFICIAL='https://columbiaco.com';
const MODE='public_assessor_propertyaccess_sale_search_current_year';
const SEARCH='http://propertysearch.co.columbia.wa.us/PropertyAccess/SaleSearch.aspx?cid=0';
const RESULTS='http://propertysearch.co.columbia.wa.us/PropertyAccess/SearchResultsSales.aspx?cid=0';
const PAYLOAD='columbia-propertyaccess-sale-search-2026-sanitized.json';
...
function date(v){const m=/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/.exec(String(v??'').trim());inv(m,`bad Columbia date ${v}`);return`${m[3]}-${m[1].padStart(2,'0')}-${m[2].padStart(2,'0')}`}
...
async function fetchSales(){
  ...
  inv(cookie,'missing Columbia session cookie');
...
function parseRows(html,page){
  ...
  for(const m of table.matchAll(/<tr[\s\S]*?<\/tr>/gi)){
    ...
    if(cells.length!==15||cells[2]==='Property ID')continue;
    const propertyId=ns(cells[2]), geoId=ns(cells[3]), propType=ns(cells[6]), situs=ns(cells[7]), saleDate=ns(cells[10]), salePrice=money(cells[11]), deed=ns(cells[12]), excise=ns(cells[13]);
...
async function main(){
  ...
  inv(records.length>0,'no stageable Columbia sales');
