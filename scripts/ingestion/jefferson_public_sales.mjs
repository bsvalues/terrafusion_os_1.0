...
const COUNTY='Jefferson',CODE='031',OFFICIAL='https://www.co.jefferson.wa.us';
const MODE='public_assessor_propertyaccess_sale_search_current_year';
const SEARCH='https://trueweb.jeffcowa.us/propertyaccess/SaleSearch.aspx?cid=0';
const RESULTS='https://trueweb.jeffcowa.us/propertyaccess/SearchResultsSales.aspx?cid=0';
const PAYLOAD='jefferson-propertyaccess-sale-search-2026-sanitized.json';
...
function date(v){const m=/^(\d{1,2})\/(\d{1,2})\/(\d{4})$/.exec(String(v??'').trim());inv(m,`bad Jefferson date ${v}`);return`${m[3]}-${m[1].padStart(2,'0')}-${m[2].padStart(2,'0')}`}
...
async function fetchSales(){
  ...
  inv(cookie,'missing Jefferson session cookie');
...
async function main(){
  ...
  inv(records.length>0,'no stageable Jefferson sales');
