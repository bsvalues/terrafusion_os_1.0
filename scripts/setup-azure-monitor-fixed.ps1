...
param(
    ...
    [string]$ResourceGroupName = "TerraFusion-RG",
    [string]$ApplicationInsightsName = "TerraFusion-AppInsights",
    [string]$LogAnalyticsWorkspaceName = "TerraFusion-LogAnalytics"
)
...
# Dashboard Template (simplified JSON structure)
$dashboardConfig = @'
{
  "dashboard": {
    "name": "TerraFusion Monitoring Dashboard",
    "description": "Comprehensive monitoring dashboard for TerraFusion application",
    "widgets": [
      {
        "type": "performance_overview",
        "title": "Application Performance",
        "query": "requests | summarize RequestCount = count(), AvgResponseTime = avg(duration) by bin(timestamp, 5m)"
      },
      {
        "type": "error_analysis",
        "title": "Error Distribution",
        "query": "exceptions | summarize ErrorCount = count() by type"
      },
      {
        "type": "system_health",
        "title": "System Resources",
        "query": "customMetrics | where name in ('System.Metrics.CpuUsage', 'System.Metrics.MemoryUsage') | summarize AvgValue = avg(value) by name"
      },
      {
        "type": "user_activity",
        "title": "User Engagement",
        "query": "pageViews | summarize UserSessions = dcount(session_Id), PageViews = count() by bin(timestamp, 10m)"
      }
    ]
  }
}
...
# Workbook Template (simplified)
$workbookConfig = @'
{
  "workbook": {
    "name": "TerraFusion Monitoring Workbook",
    "version": "1.0",
    "sections": [
      {
        ...
        "charts": [
          {
            ...
            "query": "requests | where timestamp > ago(1h) | summarize RequestCount = count(), AvgResponseTime = avg(duration) by bin(timestamp, 5m) | render timechart"
          },
          {
            "title": "Error Distribution",
            "query": "exceptions | where timestamp > ago(24h) | summarize ErrorCount = count() by type | render piechart"
          }
        ]
      },
      {
        ...
        "charts": [
          {
            ...
            "query": "customMetrics | where timestamp > ago(1h) | where name in ('System.Metrics.CpuUsage', 'System.Metrics.MemoryUsage') | summarize AvgValue = avg(value) by name, bin(timestamp, 5m) | render timechart"
          },
          {
            "title": "Dependency Performance",
            "query": "dependencies | where timestamp > ago(1h) | summarize CallCount = count(), AvgDuration = avg(duration), FailureRate = countif(success == false) * 100.0 / count() by type"
          }
        ]
      }
    ]
...
$dashboardConfig | Out-File -FilePath "$configDir\dashboard-config.json" -Encoding UTF8
$workbookConfig | Out-File -FilePath "$configDir\workbook-config.json" -Encoding UTF8
...
Write-Host "   • dashboard-config.json - Dashboard configuration" -ForegroundColor White
Write-Host "   • workbook-config.json - Workbook template" -ForegroundColor White
...
Write-Host "`n🎯 Day 7 Phase 3 Azure Monitor Integration Complete!" -ForegroundColor Green
