...
function Start-Backend {
    ...
    try {
        ...
        Write-LocalLog "✅ Backend started on https://localhost:\${{TF_API_HTTPS_PORT:-5001}}" -Level 'SUCCESS'
    }
    catch {
...
function Start-Frontend {
    ...
    try {
        ...
        Write-LocalLog "✅ Frontend started on http://localhost:\${{TF_API_HTTPS_PORT:-5001}}" -Level 'SUCCESS'
    }
    catch {
...
function Initialize-AISwarm {
    ...
    do {
        ...
        try {
            $response = Invoke-RestMethod -Uri "https://localhost:\${{TF_API_HTTPS_PORT:-5001}}/api/health" -SkipCertificateCheck -TimeoutSec 5
            break
        }
        catch {
            $retries++
    ...
    try {
        ...
        $response = Invoke-RestMethod -Uri "https://localhost:\${{TF_API_HTTPS_PORT:-5001}}/api/ai-swarm/initialize" -Method POST -Body $initPayload -ContentType "application/json" -SkipCertificateCheck
...
"@ -ForegroundColor Cyan
    ...
    Write-LocalLog "   🌐 Frontend: http://localhost:\${{TF_API_HTTPS_PORT:-5001}}" -Level 'INFO'
    Write-LocalLog "   🔧 Backend API: https://localhost:\${{TF_API_HTTPS_PORT:-5001}}" -Level 'INFO'
    Write-LocalLog "   📊 Health Check: https://localhost:\${{TF_API_HTTPS_PORT:-5001}}/api/health" -Level 'INFO'
    Write-LocalLog "   🤖 AI Swarm Status: https://localhost:\${{TF_API_HTTPS_PORT:-5001}}/api/ai-swarm/status" -Level 'INFO'
    Write-LocalLog "" -Level 'INFO'
    ...
    if ($OpenBrowser) {
        ...
        Start-Process "http://localhost:\${{TF_API_HTTPS_PORT:-5001}}"
    }
