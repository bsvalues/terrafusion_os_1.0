...
if [[ "${HARRIS_PACS_ENABLED:-false}" == "true" ]]; then
  curl -fsS http://localhost:8080/api/harris-pacs/status || { echo "Harris PACS integration test failed"; exit 1; }
fi

# Test key Benton County endpoints
curl -fsS http://localhost:8080/api/counties/benton/parcels?limit=1 || { echo "Parcels API failed"; exit 1; }
curl -fsS http://localhost:8080/api/counties/benton/assessments?limit=1 || { echo "Assessments API failed"; exit 1; }
