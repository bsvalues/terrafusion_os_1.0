...
class TerraFusionLoadTest {
  constructor() {
    this.config = {
      apiUrl: 'http://localhost:${TF_STATIC_PORT:-8080}',
      wsUrl: 'ws://localhost:${TF_STATIC_PORT:-8080}/terrafusion/core',
      maxConnections: 50,
      messageCount: 1000,
  ...
  async testApiHealth() {
    ...
    const endpoints = ['/health', '/terrafusion/core/negotiate', '/api/system/status'];
  ...
  async testPluginOperations() {
    ...
    return new Promise((resolve, reject) => {
      ...
      setTimeout(() => {
        if (operationsCompleted < totalOperations) {
          reject(
            new Error(`Plugin test timeout: ${operationsCompleted}/${totalOperations} completed`)
          );
        }
      }, 20000);
    });
        ...
        ws.on('message', data => {
          ...
          try {
            ...
            if (response.result && response.result.migrationStatus) {
              ...
              this.log(
                `✓ Migration completion: ${response.result.migrationStatus.completionPercentage}%`
              );
