...
const AI_SWARM_URL = 'http://localhost:8001';
const CLAUDE_FLOW_URL = 'http://localhost:8002';

// Test data
const PROPERTY_IDS = [
  'PROP_001', 'PROP_002', 'PROP_003', 'PROP_004', 'PROP_005',
  'PROP_006', 'PROP_007', 'PROP_008', 'PROP_009', 'PROP_010'
];
...
function testCachePerformance() {
  ...
  const cacheSuccess = check(cacheResponse, {
    ...
    'database load reduction >70%': (r) => {
      const body = JSON.parse(r.body);
      return body.databaseLoadReduction > 0.70;
    },
  });
...
function testGovernmentWorkflow() {
  ...
  const workflowSuccess = check(valuationResponse, {
...
export function handleSummary(data) {
  const summary = {
    ...
    propertiesPerMinute: (data.metrics.properties_processed.count / (data.state.testRunDurationMs / 1000)) * 60,
