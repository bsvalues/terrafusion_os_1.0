...
type OSState = {status: 'disconnected' | 'connecting' | 'authenticated' | 'error';
  sessionId?: string | null;
...
const PluginsHost: React.FC = () => {const [state, setState] = useState<OSState>({ status: 'disconnected', loadedModules: []});
  ...
  useEffect(() => {
    ...
    mods.forEach(async (name) => {
      ...
      if (!mountEl || !loader) {setStatuses((prev) => ({ ...prev, [name]: 'error'}));
        setErrors((prev) => ({ ...prev, [name]: `Mount point or loader not found for ${name}` }));
        return;
      }

      try {// Validate plugin manifest before mounting
      ...
      } catch (err: any) {setStatuses((prev) => ({ ...prev, [name]: 'error'}));
  ...
  return (<section className={styles.container} aria-label='Terrafusion Plugins Host'><header className={styles.header}><h3 className={styles.title}>Kernel Modules</h3><div className={styles.status} data-status={state.status}>Status: {state.status}</div></header>{state.lastError && (<div className={styles.error} role='status'>{state.lastError}</div>)}<div className={styles.grid}>{mods.length === 0 &&<div className={styles.placeholder}>No modules loaded yet</div>}
        {mods.map((m) => (<div key={m} className={styles.card} id={`plugin-${m}`}><div className={styles.cardHeader}><span className={styles.moduleIcon}>🧩</span><span className={styles.moduleName}>{m}</span>{statuses[m] === 'loading' &&<span title='Loading'>⏳</span>}
              {statuses[m] === 'loaded' && <span title='Loaded'>✅</span>}
              {statuses[m] === 'denied' && <span title='Denied'>❌</span>}
              {statuses[m] === 'error' && <span title='Error'>⚠️</span>}
            </div>{errors[m] && (<div className={styles.error} role='status'>{errors[m]}</div>)}<div className={styles.mount} data-module={m} /></div>))}</div></section>
  );
};
