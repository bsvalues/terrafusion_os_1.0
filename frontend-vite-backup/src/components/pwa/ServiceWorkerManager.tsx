...
export const ServiceWorkerManager: React.FC = () => {const [swState, setSwState] = useState<ServiceWorkerState>({
  ...
  const registerServiceWorker = useCallback(async () => {if (!swState.isSupported) {
  ...
  // Update app
  const updateApp = useCallback(() => {if (swState.registration?.waiting) {
  ...
  useEffect(() => {const handleMessage = (event: MessageEvent) => {
  ...
  useEffect(() => {if (navigator.storage && navigator.storage.persist) {
  ...
  const handleCloseNotification = () => {setNotifications((prev) => ({ ...prev, show: false}));
  };

  return (<div>{/* Update Available Snackbar */}<Snackbar
      ...
      ><Alert
          ...
          action={notifications.action && notifications.actionLabel ? (
              <Button
                color='inherit'
                size='small'
                onClick={() =>{
              ...
              >
                {notifications.actionLabel}</Button>) : undefined
          }
        >
          {notifications.message}</Alert></Snackbar>{/* PWA Debug Info (Development Only) */}
      {process.env.NODE_ENV === 'development' && (<Box
          ...
          zIndex={9999}
        ><div>SW Supported: {swState.isSupported ? '✓' : '✗'}</div><div>SW Registered: {swState.isRegistered ? '✓' : '✗'}</div><div>Update Available: {swState.isUpdateAvailable ? '✓' : '✗'}</div><div>Offline: {swState.isOffline ? '✓' : '✗'}</div></Box>)}</div>
  );
};

// Hook for using service worker functionality in components
export const useServiceWorker = () => {const [isOnline, setIsOnline] = useState(navigator.onLine);
  ...
  const syncInBackground = useCallback(async (data: any) => {if ('serviceWorker' in navigator && navigator.serviceWorker.controller) {
  ...
  const cacheData = useCallback(async (key: string, data: any) => {if ('caches' in window) {
      try {
        const cache = await caches.open('terrafusion-data-v1');
        const response = new Response(JSON.stringify(data));
        await cache.put(key, response);
        return true;} catch (error) {console.error('Failed to cache data:', error);
        return false;}
    }
    return false;
  }, []);

  const getCachedData = useCallback(async (key: string) => {if ('caches' in window) {
      try {
        const cache = await caches.open('terrafusion-data-v1');
        const response = await cache.match(key);
        if (response) {
          return response.data;}
      } catch (error) {console.error('Failed to get cached data:', error);}
    }
    return null;
  }, []);
