...
interface WebGLEffectsProps {effect?: number;
  onEffectChange?: (_effect: number) => void;}

const WebGLEffects: React.FC<WebGLEffectsProps> = ({effect = 1, onEffectChange}) => {const canvasRef = useRef<HTMLCanvasElement>(null);
  ...
  useEffect(() =>{if (!canvasRef.current) return;
    ...
    const handleResize = () => {if (camera && renderer) {
  ...
  useEffect(() => {if (sceneRef.current && cameraRef.current && rendererRef.current) {
  ...
  const createTranscendenceWave = (scene: THREE.Scene) => {const geometry = new THREE.PlaneGeometry(20, 20, 100, 100);
    const material = new THREE.ShaderMaterial({
      ...
      fragmentShader: `
        ...
        uniform vec3 colorB;
        varying vec2 vUv;
        void main() {float wave = sin(vUv.x * 10.0 + time) * cos(vUv.y * 10.0 + time);
  ...
  const createNeuralNetwork = (scene: THREE.Scene) => {const nodes: THREE.Mesh[] = [];
  ...
  const createDataFlow = (scene: THREE.Scene) =>{const streams: THREE.Line[] = [];
    ...
    for (let i = 0; i< 100; i++) {
      ...
      const material = new THREE.LineBasicMaterial({color: new THREE.Color().setHSL(Math.random() * 0.3 + 0.4, 0.8, 0.5),
        transparent: true,
        opacity: 0.6,});
  ...
  const createParticleGalaxy = (scene: THREE.Scene) =>{const particleCount = 5000;
  ...
  const createQuantumField = (scene: THREE.Scene) =>{// Implementation for quantum field effect
  ...
  const createDigitalCortex = (scene: THREE.Scene) => {// Implementation for digital cortex effect
  ...
  const createTranscendenceMatrix = (scene: THREE.Scene) =>{// Implementation for transcendence matrix effect
  ...
  const startAnimation = (
    ...
    renderer: THREE.WebGLRenderer
      ...
      switch (currentEffect) {case 1: // Transcendence Wave
          if ((scene as any).animateMaterial) {
            (scene as any).animateMaterial.uniforms.time.value = time * 0.001;}
          break;
        case 2: // Neural Network
          if ((scene as any).nodes) {(scene as any).nodes.forEach((node: THREE.Mesh, i: number) => {
              node.position.y += Math.sin(time * 0.001 + i) * 0.01;});
          }
          break;
        case 4: // Particle Galaxy
          if ((scene as any).particles) {(scene as any).particles.rotation.y = time * 0.0005;}
          break;
        case 5: // Quantum Field
          if ((scene as any).quantumField) {(scene as any).quantumField.rotation.x = time * 0.001;
            (scene as any).quantumField.rotation.y = time * 0.0007;}
          break;
        case 6: // Digital Cortex
          if ((scene as any).cortexGroup) {(scene as any).cortexGroup.rotation.x = time * 0.0005;
            (scene as any).cortexGroup.rotation.y = time * 0.0003;}
          break;
        case 7: // Transcendence Matrix
          if ((scene as any).matrixCubes) {(scene as any).matrixCubes.forEach((cube: THREE.Mesh, i: number) => {
  ...
  const changeEffect = (newEffect: number) => {setCurrentEffect(newEffect);
    onEffectChange?.(newEffect);};

  return (<div
      className='webgl-container'
      style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', zIndex: -1}}
    ><canvas ref={canvasRef} style={{ display: 'block'}} />{/* Effect Controls */}<div
        style={{
          position: 'fixed',
          bottom: '20px',
          left: '20px',
      ...
      >{[1, 2, 3, 4, 5, 6, 7].map((num) => (<button
            ...
            style={{
              ...
              fontWeight: 'bold',}}
          >
            {num}</button>))}</div>{/* FPS Counter */}<div
        style={{
          position: 'fixed',
          top: '20px',
          left: '20px',
