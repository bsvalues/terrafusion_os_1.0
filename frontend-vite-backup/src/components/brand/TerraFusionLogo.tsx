...
interface TerraFusionLogoProps {variant?: 'monogram' | 'embossed' | 'seal' | 'square' | 'browser';
  size?: 'small' | 'medium' | 'large';
  animated?: boolean;
  className?: string;}

const TerraFusionLogo: React.FC<TerraFusionLogoProps>= ({variant = 'monogram',
  size = 'medium',
  animated = true,
  className = '',}) => {const getSizeStyles = () => {
    switch (size) {
      case 'small':
  ...
  switch (variant) {
    ...
    case 'seal':
      return (<Box
        ...
        ><Box className='tf-seal-text'><Typography variant='caption' sx={{ fontSize: '8px', fontWeight: 700}}>TERRA</Typography><br /><Typography variant='caption' sx={{ fontSize: '8px', fontWeight: 700}}>FUSION</Typography></Box></Box>);
    ...
    case 'browser':
      return (<Box className={`tf-browser-mockup ${className}`}><Box className='tf-browser-dots'><Box className='tf-browser-dot close' /><Box className='tf-browser-dot minimize' /><Box className='tf-browser-dot maximize' /></Box><Typography
            variant='body2'
            className='tf-logo-monogram'
            sx={{ ml: 2, fontSize: '14px', fontWeight: 700}}
          >Terrafusion OS</Typography></Box>);
