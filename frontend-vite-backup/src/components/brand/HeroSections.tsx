...
interface HeroSectionsProps {currentHero?: number;
  onHeroChange?: (_hero: number) => void;}

const HeroSections: React.FC<HeroSectionsProps> = ({currentHero = 1, onHeroChange}) => {const [activeHero, setActiveHero] = useState(currentHero);
  ...
  const showHero = (num: number) =>{setActiveHero(num);
    onHeroChange?.(num);};
  ...
  return (<div className='hero-sections-container'>{/* Navigation Dots */}<div className='hero-nav'>{[1, 2, 3, 4, 5, 6].map((num) => (<div
            key={num}
            className={`nav-dot ${activeHero === num ? 'active' : ''}`}
            onClick={() =>showHero(num)}
          />
        ))}</div>{/* Hero Sections */}
      {heroes.map((hero) => (<section
          key={hero.id}
          className={`hero-section ${activeHero !== hero.id ? 'hidden' : ''}`}
          id={`hero${hero.id}`}
        ><div className='grid-bg'></div><div className='particles' id={`particles${hero.id}`}></div><div className='hero-content'><div className='transcendence-badge'>{hero.badge}</div><h1 className='hero-headline'>{hero.headline}</h1><p className='hero-subhead'>{hero.subhead}</p><div className='hero-ctas'><button className='btn-primary'>{hero.primaryCTA}</button><button className='btn-secondary'>{hero.secondaryCTA}</button></div></div></section>))}</div>
  );
};
