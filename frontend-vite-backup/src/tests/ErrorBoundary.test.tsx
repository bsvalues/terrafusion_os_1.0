...
const ThrowError: React.FC<{shouldThrow: boolean; errorMessage?: string}>= ({shouldThrow,
  errorMessage = 'Test error',}) => {if (shouldThrow) {
    throw new Error(errorMessage);}
  return<div>No error</div>;
};

// Component that uses error handler hook
const AsyncErrorComponent: React.FC<{shouldFail: boolean}>= ({shouldFail}) => {const { handleAsyncError, retryOperation} = useErrorHandler();

  const handleClick = async () => {if (shouldFail) {
      await handleAsyncError(new Error('Async operation failed'), {
        component: 'AsyncErrorComponent',});
    } else {await retryOperation(
        async () => {
...
describe('Error Boundary System', () => {
  describe('ErrorBoundary Component', () => {
    ...
    it('should reset error state when retry button is clicked', async () => {const TestComponent: React.FC = () => {
  ...
  describe('ErrorContext and ErrorProvider', () => {const ErrorContextTestComponent: React.FC = () => {
      const { reportError, clearErrors, hasErrors, state} = useErrorContext();

      const triggerError = () => {reportError({
      ...
      return (<div><button onClick={triggerError} data-testid='trigger-error'>Trigger Error</button><button onClick={clearErrors} data-testid='clear-errors'>Clear Errors</button><div data-testid='error-count'>{state.errors.length}</div><div data-testid='has-errors'>{hasErrors.toString()}</div></div>);
    };

    it('should manage error state correctly', () => {render(<ErrorProvider><ErrorContextTestComponent /></ErrorProvider>);
    ...
    it('should auto-remove non-critical errors after timeout', async () => {jest.useFakeTimers();
  ...
  describe('useErrorHandler Hook', () => {const ErrorHandlerTestComponent: React.FC = () => {
      const { handleAsyncError, handleApiError, handleValidationError, retryOperation} =
        useErrorHandler();
      const [result, setResult] = React.useState<string>('');

      const testAsyncError = async () =>{const error = new Error('Async test error');
      ...
      const testValidationError = async () => {const validationErrors = {
      ...
      const testRetryOperation = async () => {let attempts = 0;
      ...
      return (
        <div><button onClick={testAsyncError} data-testid='test-async'>Test Async Error</button><button onClick={testApiError} data-testid='test-api'>Test API Error</button><button onClick={testValidationError} data-testid='test-validation'>Test Validation Error</button><button onClick={testRetryOperation} data-testid='test-retry'>Test Retry Operation</button><div data-testid='result'>{result}</div></div>);
    };

    it('should handle different error types correctly', async () => {render(<ErrorProvider><ErrorHandlerTestComponent /></ErrorProvider>);
  ...
  describe('Error Boundary Performance', () => {it('should not impact performance when no errors occur', () => {
    ...
    it('should handle rapid error recovery without memory leaks', async () => {const TestComponent: React.FC = () => {
        ...
        const triggerErrorCycle = () => {
          ...
          setTimeout(() => {
            ...
            setErrorCount((c) => c + 1);}, 10);
        };

        // Automatically trigger error cycles
        React.useEffect(() => {if (errorCount< 5) {
  ...
  describe('Integration with Terrafusion Components', () => {it('should work with property assessment components', () => {
