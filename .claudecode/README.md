...
The `.claudecode` directory contains the comprehensive configuration for Claude
Code IDE integration with TerraFusion OS. This configuration enables advanced
development workflows, AI swarm testing, government compliance automation, and
quantum performance optimization across the entire government AI platform.
...
curl http://localhost:\${{TF_ADMIN_PORT:-8080}}/mcp/terrafusion-enhanced/health
curl http://localhost:\${{TF_ADMIN_PORT:-8080}}/mcp/playwright-server/health
curl http://localhost:\${{TF_ADMIN_PORT:-8080}}/mcp/filesystem/health
...
interface TestingCapabilities {
  ...
  aiSwarmTesting: {
    agentCount: 50000; // Full AI swarm testing (Supreme Commander + Field Generals + Operational Forces)
...
- **[.claude/settings.local.json](../.claude/settings.local.json)**: Claude AI
  permissions
- **[playwright.config.ts](../playwright.config.ts)**: Playwright testing
  configuration
...
- **Compliance**: 5 government standards (FISMA, NIST, Section 508, WCAG 2.1,
  SOC 2)
- **Performance**: Quantum optimization with 379M% target improvement
