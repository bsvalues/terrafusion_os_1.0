...
import { Info, Check, X, Leaf, DollarSign, Refresh, FileBarChart  } from '@mui/icons-material';
...
<>
            ...
            <Refresh className="h-5 w-5 text-[#3CAB36]" />
...
className="text-xs text-gray-500">Emphasize maximum cost reduction</p>
                  </div>
<>
...
className="text-xs text-gray-500">Emphasize eco-friendly alternatives</p>
                  </div>
<>
...
className="text-xs text-gray-500">Emphasize readily available materials</p>
                  </div>
<>
...
className="font-medium">{currentMaterials.length}</span>
                  </div>
                  <div className="flex justify-between mb-1">
<>
...
className="font-medium">${currentMaterials.reduce((sum, m) => sum + (m.quantity * m.unitPrice), 0).toLocaleString()}</span>
                  </div>
...
className="font-medium">{buildingType}</span>
                  </div>
                </div>
...
className="font-medium text-[#3CAB36]">
...
className="font-medium">{optimizationResults.numberOfRecommendations}</span>
                    </div>
                    <div className="flex justify-between items-center text-sm">
<>
...
value="details">Materials Detail</TabsTrigger>
              </TabsList>
...
</>>Alternative for {recommendation.originalMaterial}</CardDescription>
                            </div>
...
className="text-sm font-medium text-[#3CAB36]">${recommendation.potentialSavings.toLocaleString()}</span>
                              </div>
                              <div className="flex justify-between">
<>
...
className="text-sm font-medium">{recommendation.costReductionPercentage}%</span>
                              </div>
                            </div>
                            <div className="space-y-1">
...
</>>Detailed specification and comparison</CardDescription>
                        </CardHeader>
...
className="font-medium">{recommendation.originalMaterial}</span>
                                </div>
                                <div className="flex justify-between">
<>
...
className="font-medium">{recommendation.suggestedAlternative}</span>
                                </div>
                                <Separator />
                                <div className="flex justify-between">
<>
...
className="font-medium">{recommendation.costReductionPercentage}%</span>
                                </div>
                                <div className="flex justify-between">
<>
...
className="font-medium text-[#3CAB36]">${recommendation.potentialSavings.toLocaleString()}</span>
                                </div>
                              </div>
...
className={qualityImpactColors[recommendation.qualityImpact]}>
...
</>>{recommendation.regionalAvailability.join(', ')}</span>
                                </div>
                              </div>
                            </div>
...
className="text-sm text-gray-700">{recommendation.applicationNotes}</p>
                                </div>
                              </div>
                            </div>
                          </div>
