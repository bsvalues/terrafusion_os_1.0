...
import { ArrowUpRight, Maximize2, X, ZoomIn  } from '@mui/icons-material';
...
className="text-sm font-medium">{formatValue(dataPoint.value)}</span>
        </div>
...
className="text-sm">{formatMetadataValue(value)}</span>
          </div>
        ))}
      </div>
...
variant="outline">{dataPoint.category}</Badge>
              </div>
            )}
...
className="text-sm">{dataPoint.description}</p>
              </div>
            )}
          </div>
        </CardContent>
