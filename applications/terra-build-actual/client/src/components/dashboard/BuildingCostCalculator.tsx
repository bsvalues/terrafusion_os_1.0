...
value="result" disabled={!calculationResult || calculateCost.isPending}>
<>
...
value="export" disabled={!calculationResult || calculateCost.isPending}>
...
value="Very Poor">Very Poor</SelectItem>
                              </SelectContent>
                            </Select>
<>
...
value="Economy">Economy</SelectItem>
...
className="text-2xl font-bold">{formatCurrency(calculationResult.baseCost)}</p>
                      </div>
                      <div className="bg-background p-4 rounded-md shadow-sm">
<>
...
className="text-2xl font-bold">{formatCurrency(calculationResult.adjustedCost)}</p>
                      </div>
...
className="text-2xl font-bold">{formatCurrency(calculationResult.totalCost)}</p>
                      </div>
                    </div>
                  </div>
...
className="font-medium">{form.getValues().region}</p>
                      </div>
                      <div>
<>
...
className="font-medium">{form.getValues().buildingType}</p>
                      </div>
                      <div>
<>
...
className="font-medium">{form.getValues().squareFootage.toLocaleString()}</p>
                      </div>
                      <div>
<>
...
className="font-medium">{form.getValues().yearBuilt}</p>
                      </div>
                    </div>
                  </div>
...
className="font-medium">{calculationResult.regionalFactor.toFixed(2)}</p>
                        </div>
                        <div className="mb-3">
<>
...
className="font-medium">{calculationResult.buildingTypeFactor.toFixed(2)}</p>
                        </div>
                        <div className="mb-3">
<>
...
className="font-medium">{form.getValues().complexityFactor.toFixed(1)}</p>
                        </div>
                        <div>
<>
...
className="font-medium">{form.getValues().conditionFactor.toFixed(1)}</p>
                        </div>
                      </div>
...
className={`font-medium ${calculationResult.complexityAdjustment > 0 ? 'text-red-500' : 'text-green-500'}`}>
...
className={`font-medium ${calculationResult.conditionAdjustment > 0 ? 'text-green-500' : 'text-red-500'}`}>
...
className="font-medium">{formatPercentage(calculationResult.depreciationRate)}</p>
                        </div>
                        <div>
<>
...
className="font-medium">{formatCurrency(cost as number)}</p>
                          </div>
                        ))}
                      </div>
                    </div>
...
variant="outline" className="w-full sm:w-auto flex items-center justify-center ml-0 sm:ml-4">
                    <FileText className="h-4 w-4 mr-2" />
                    Export as CSV
                  </Button>
                </div>
