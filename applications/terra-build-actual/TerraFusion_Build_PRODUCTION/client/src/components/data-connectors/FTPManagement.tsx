...
import ... from "@/FTPFilePreview";
import ... from "@/FTPBreadcrumb";
import ... from "@/FileTypeIcon";
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
          ...
          <div className="flex space-x-2">
            <Button 
              ...
              disabled={loading}
            >
              {loading ? <Loader2 className="h-4 w-4 mr-2 animate-spin" /> : <Refresh className="h-4 w-4 mr-2" />}
              Test Connection
            </Button>
...
open={createDirOpen} onOpenChange={setCreateDirOpen}>
...
className="text-xs text-center">{uploadProgress}% Complete</div>
                    </div>
                  )}
                </div>
                <DialogFooter>
            ...
            <Button 
              ...
              disabled={loading}
            >
              {loading ? <Loader2 className="h-4 w-4 mr-1 animate-spin" /> : <Refresh className="h-4 w-4 mr-1" />}
              Refresh
            </Button>
          </div>
...
className="col-span-5 flex items-center truncate">
                        {file.type === 'directory' ? (
                          <Button 
                            variant="link" 
...
className="bg-muted/30 flex items-center justify-between px-6 py-3">
          <div className="text-sm text-muted-foreground"><>
