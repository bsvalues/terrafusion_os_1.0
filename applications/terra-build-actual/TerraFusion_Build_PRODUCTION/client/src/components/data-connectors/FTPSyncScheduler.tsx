...
value="history">Sync History</TabsTrigger>
          </TabsList>
...
className="truncate">{`${schedule.source.type}:${schedule.source.path}`}</span>
                      </div>
                      <div className="flex items-center space-x-2">
                        <Folder className="h-4 w-4 text-muted-foreground" />
<>
...
className="truncate">{`${schedule.destination.type}:${schedule.destination.path}`}</span>
                      </div>
                      {schedule.lastRun && (
                        <div className="flex items-center space-x-2">
...
</>>{formatDateTime(schedule.lastRun)}</span>
                        </div>
                      )}
                      {schedule.nextRun && (
                        <div className="flex items-center space-x-2">
...
</>>{formatDateTime(schedule.nextRun)}</span>
                        </div>
                      )}
                    </div>
                  </CardContent>
...
</>>Recent sync job executions</CardDescription>
              </CardHeader>
                    ...
                    {history.map((record: any) => {
                      const statusIcon = record.status === 'success' ? (
                        ...
                        <div className="h-8 w-8 rounded-full bg-blue-100 flex items-center justify-center">
                          <Refresh className="h-4 w-4 text-blue-600 animate-spin" />
                        </div>
                      );
...
className="text-sm font-medium capitalize">{record.status}</span>
                              </div>
                              <div className="flex items-center space-x-2">
<>
...
className="text-sm">{formatDateTime(record.startTime)}</span>
                              </div>
                              {record.endTime && (
                                <div className="flex items-center space-x-2">
<>
...
className="text-sm">{formatDateTime(record.endTime)}</span>
                                </div>
                              )}
...
className="text-sm">{record.filesTransferred} files ({Math.round(record.totalBytes / 1024)} KB)</span>
                                </div>
                              )}
...
value="local">Local Storage</SelectItem>
                          </SelectContent>
                        </Select>
                        <FormMessage />
                      </FormItem>
...
value="hourly">Hourly</SelectItem>
<>
...
type="submit" disabled={createScheduleMutation.isPending}>
