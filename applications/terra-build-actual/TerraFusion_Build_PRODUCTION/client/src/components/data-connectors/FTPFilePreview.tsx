...
import { AlertCircle, Download, File, Refresh  } from '@mui/icons-material';
...
variant="outline" className="mt-4" onClick={fetchFilePreview}>
            Retry Preview
          </Button>
        </div>
      );
...
className="text-sm text-muted-foreground mb-4">Download the file to view its contents</p>
          </div>
        );
      case 'excel':
      case 'word':
...
className="text-sm text-muted-foreground mb-4">Preview not available for this file type</p>
          </div>
        );
    }
  };
...
className="text-sm text-muted-foreground">{filename}</p>
          </div>
          <div>
<>
...
className="text-sm text-muted-foreground">{determinedFileType.toUpperCase()}</p>
          </div>
          <div>
<>
...
className="text-sm text-muted-foreground">{filePath}</p>
          </div>
          <div>
<>
...
className="text-sm text-muted-foreground">{connectionId}</p>
          </div>
        </div>
      </div>
    );
...
value="details" className="rounded-none data-[state=active]:border-b-2 data-[state=active]:border-primary data-[state=active]:shadow-none">
              Details
            </TabsTrigger>
          </TabsList>
<>
      ...
      <CardFooter className="flex justify-between pt-6 border-t">
        ...
        >
          {loading ? (
              ...
              <Refresh className="mr-2 h-4 w-4 animate-spin" /> Refreshing...
...
<>
        ...
        >
          {loading ? (
              ...
              <Refresh className="mr-2 h-4 w-4 animate-spin" /> Downloading...
