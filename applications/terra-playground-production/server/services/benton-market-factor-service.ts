...
export class BentonMarketFactorService {
  ...
  public calculateMarketFactorImpact(factors: MarketFactor[]): {
    ...
    const weights = {
      ...
      'Construction Costs': 0.1,
      'Local Development': 0.1,
      'Property Tax Rates': 0.05,
      'School District Quality': 0.1,
    ...
    factors.forEach(factor => {
      ...
      if (factor.impact === 'positive') {
        impactValue =
          factor.trend === 'rising' || factor.trend === 'improving'
            ? 1.0
            : factor.trend === 'stable'
              ? 0.5
              : 0.1;
    ...
    const marketOutlook =
      totalImpact > 1.5 ? 'positive' : totalImpact < -1.5 ? 'negative' : 'neutral';
