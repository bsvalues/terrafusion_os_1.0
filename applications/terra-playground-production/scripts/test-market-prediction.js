...
async function testMarketPredictionServices() {
  ...
  try {
    ...
    trends.slice(0, 2).forEach(trend => {
      console.log(
        `- ${trend.metric}: ${trend.value} (${trend.trend}) with ${trend.confidence * 100}% confidence`
      );
    });
  } catch (error) {
  ...
  try {
    ...
    indicators.slice(0, 2).forEach(indicator => {
      console.log(
        `- ${indicator.name}: ${indicator.value} (${indicator.impact} impact) with ${indicator.significance * 100}% significance`
      );
    });
  } catch (error) {
  ...
  try {
    ...
    console.log(
      `Retrieved property history with ${history.valueHistory.length} value changes and ${history.events.length} events.`
    );
  ...
  try {
    ...
    console.log(
      `Retrieved ${risks.risks.length} environmental risks and ${risks.environmentalHazards.length} environmental hazards.`
    );
