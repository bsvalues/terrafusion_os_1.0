...
export function VoiceCommandHelp({
  ...
  const renderEmpty = () => (
    <div className="text-center py-12 text-muted-foreground">
      ...
      <p
</> className="mt-1">No help content is available for this context.</p>
    </div>
  );
        ...
        <CollapsibleTrigger className="flex w-full items-center justify-between p-4 hover:bg-muted/50 text-left">
          <div className="flex items-center gap-2">
            ...
            <div><>
              ...
              <p
</> className="text-xs text-muted-foreground">{content.commandType}</p>
            </div>
          </div>
          ...
          <div className="mt-4 space-y-4">
              ...
              <div
                ...
                {content.examplePhrases.map((phrase, i) => (
                  <div key={i} className="flex items-center gap-2">
                    ...
                    <code
</> className="text-xs bg-muted px-2 py-1 rounded">{phrase}</code>
                  </div>
                ))}
              </div>
            </div>
                ...
                <div
                  ...
                  {Object.entries(content.parameters).map(([key, description], i) => (
                    <div key={i} className="text-xs"><>
                      ...
                      <span
</> className="ml-2">{description}</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            {content.responseExample && (
              <div><>
                ...
                <div
</> className="bg-muted p-3 rounded-md text-xs whitespace-pre-wrap">
                  {content.responseExample}
                </div>
              </div>
            )}
  ...
  return (
    <Card className={className}>
      <CardHeader><>
        ...
        <CardDescription
</>>Learn about available voice commands and how to use them</CardDescription>
