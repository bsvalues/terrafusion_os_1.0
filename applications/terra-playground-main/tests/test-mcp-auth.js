...
async function testMcpAuthentication() {
  ...
  try {
    ...
    if (!toolsResponse.ok) {
      throw new Error(
        `Failed to fetch MCP tools: ${toolsResponse.status} ${toolsResponse.statusText}`
      );
    }
    ...
    if (!toolsWithApiKeyResponse.ok) {
      throw new Error(
        `Failed to fetch MCP tools with API key: ${toolsWithApiKeyResponse.status} ${toolsWithApiKeyResponse.statusText}`
      );
    }

    const toolsWithApiKey = await toolsWithApiKeyResponse.json();
    console.log(
      `Successfully retrieved ${toolsWithApiKey.length} tools with API key authentication`
    );
    ...
    if (unauthorizedResponse.status === 403) {
      ...
      console.error(
        `Unexpected status: ${unauthorizedResponse.status} ${unauthorizedResponse.statusText}`
      );
    }
    ...
    if (!adminTokenResponse.ok) {
      throw new Error(
        `Failed to get admin token: ${adminTokenResponse.status} ${adminTokenResponse.statusText}`
      );
    }
    ...
    if (!executeResponse.ok) {
      throw new Error(
        `Failed to execute MCP tool: ${executeResponse.status} ${executeResponse.statusText}`
      );
    }
