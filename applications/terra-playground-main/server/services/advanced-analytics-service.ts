...
export class AdvancedAnalyticsService {
    ...
    try {
      ...
      const analysis = {
        ...
        riskProfile: {
          ...
          topRiskFactors: riskAssessment.riskFactors.sort((a, b) => b.score - a.score).slice(0, 3),
        },
        marketConditions: {
  ...
  private estimateRentalYield(property: any): number {
    ...
    const [minYield, maxYield] =
      rentalYieldRanges[propertyType] || rentalYieldRanges['Residential'];
    ...
    return {
      ...
      comparables: recentSales.map(property => ({
        ...
        saleDate: new Date(Date.now() - Math.random() * 180 * 24 * 60 * 60 * 1000)
          .toISOString()
          .split('T')[0], // Random date within last 180 days
  ...
  private generateStrengthsAndOpportunities(
    ...
    if (valuePrediction.annualGrowthRate > 0.03) {
      strengths.push(
        `Strong projected appreciation rate of ${(valuePrediction.annualGrowthRate * 100).toFixed(1)}% annually`
      );
    }
    ...
    if (medianPriceTrend && medianPriceTrend.trend === 'increasing' && medianPriceTrend.value > 2) {
      strengths.push(
        `Strong market growth with median prices increasing by ${medianPriceTrend.value.toFixed(1)}%`
      );
    }
    ...
    if (daysOnMarketTrend && daysOnMarketTrend.trend === 'decreasing') {
      strengths.push(
        `Improving market liquidity with days on market decreasing by ${Math.abs(daysOnMarketTrend.value).toFixed(1)}%`
      );
    }
    ...
    if (property.propertyType === 'Residential' && property.squareFootage < 1500) {
      opportunities.push(
        'Potential for expansion or renovation to increase square footage and value'
      );
    }

    if (economicIndicators.jobGrowth > 1.5) {
      opportunities.push(
        `Strong job growth in the area (${economicIndicators.jobGrowth}%) may drive increased housing demand`
      );
    }
    ...
    if (inventoryTrend && inventoryTrend.trend === 'decreasing') {
      opportunities.push(
        'Decreasing inventory levels indicate potential for price increases due to supply constraints'
      );
    }
  ...
  private generateValueScenarios(
    ...
    const pessimisticImpact = -0.1; // 10% below baseline
  ...
  private calculateMarketDownturnResistance(valuePrediction: any): number {
    ...
    const fiveYearGrowth =
      valuePrediction.predictedValues[4].value / valuePrediction.currentValue - 1;
  ...
  private generateKeyRiskPoints(
    ...
    const lowestConfidence = Math.min(
      ...valuePrediction.predictedValues.map((p: any) => p.confidence)
    );
    if (lowestConfidence < 0.7) {
      keyRisks.push(
        `Future value projections have moderate confidence level (${(lowestConfidence * 100).toFixed(0)}%)`
      );
    }
