...
export class MarketAnalysisAgent extends BaseAgent {
  ...
  constructor(
    ...
    const config: AgentConfig = {
      ...
      description:
        'Specializes in property market analysis and forecasting using authentic Benton County data',
      capabilities: [],
      permissions: [],
    };
      ...
      },
      {
        ...
        handler: async (params: any) => {
          ...
          if (scenarioAnalysis) {
            ...
            const baseValue =
              enhancedResult.predictedValues[enhancedResult.predictedValues.length - 1]
                .predictedValue;
      ...
      },
      {
        ...
        handler: async (params: any) => {
          ...
          if (!compsResult?.success || !compsResult.result || compsResult.result.length < 3) {
            return {
              success: false,
              error: 'Insufficient comparable properties for anomaly detection',
            };
          }
              ...
              const prompt = [
                {
                  role: 'system',
                  content:
                    'You are a property valuation expert for Benton County, Washington, specializing in detecting valuation anomalies.',
                },
                {
                  role: 'user',
          ...
          const result = {
            ...
            anomalyDetection: {
              ...
              possibleExplanations: this.generateAnomalyExplanations(
                property,
                valuationStats,
                comparables
              ),
            },
            recommendations: this.generateValuationRecommendations(
              property,
              valuationStats,
              isAnomaly
            ),
    ...
    try {
      ...
      if (propertyId) {
        ...
        if (finalOptions.includeSaleHistory) {
          ...
          if (
            property.result.extraFields?.lastSaleDate &&
            property.result.extraFields?.lastSalePrice
          ) {
            salesHistory.push({
      ...
      // If area is provided (like zip code or city), analyze trends for that area
        ...
        await this.logActivity('area_market_trends_analyzed', 'Area market trends analyzed', {
          area,
        });
  ...
  private async generateComparableAnalysis(params: any): Promise<any> {
    try {
      ...
      const averageValue =
        compValues.reduce((sum: number, val: number) => sum + val, 0) / compValues.length;
      ...
      const medianPricePerSqFt = this.calculateMedian(
        compPricePerSqFt.filter((p: number) => p > 0)
      );
      ...
      const percentPriceSqFtDiffFromMedian =
        medianPricePerSqFt > 0 ? (priceSqFtDiffFromMedian / medianPricePerSqFt) * 100 : 0;

      // Determine value assessment
      let valueAssessment = '';
      if (Math.abs(percentDiffFromMedian) <= 5) {
        valueAssessment =
          'The property is fairly valued compared to similar properties in the area.';
      ...
      const advantagesDisadvantages = this.generateCompAdvantages(
        property.result,
        compsResult.result
      );
      ...
      await this.logActivity('comparable_analysis_generated', 'Comparable analysis generated', {
        propertyId,
      });
      ...
      await this.logActivity(
        ...
        `Error generating comparable analysis: ${error.message}`
      );
  ...
  private async forecastPropertyValue(params: any): Promise<any> {
    try {
      ...
      if (scenarioAnalysis) {
        ...
        // Get the forecast end value
        const baseValue =
          forecastResult.result.predictedValues[forecastResult.result.predictedValues.length - 1]
            .predictedValue;
      ...
      if (this.llmService) {
        try {
          ...
          const marketFactorsText = marketFactors
            .map(f => `${f.name}: ${f.value} (${f.impact} impact, ${f.trend} trend)`)
            .join('\n');
          ...
          const narrativeResponse = await this.llmService.prompt([
            {
              role: 'system',
              content:
                'You are a real estate market analysis expert for Benton County, Washington.',
            },
            {
              role: 'user',
      ...
      await this.logActivity('value_forecast_generated', 'Property value forecast generated', {
        propertyId,
      });
      ...
      await this.logActivity(
        'forecast_error',
        `Error forecasting property value: ${error.message}`
      );
    ...
    try {
      ...
      try {
        ...
        if (compsResult?.success && compsResult.result.length > 0) {
          ...
          const areaRentalYield = comparables.some(
            (comp: any) => comp.extraFields?.rentalEstimate > 0
          )
            ? comparables.reduce((sum: number, comp: any) => {
              ...
              }, 0) / comparables.filter((comp: any) => comp.extraFields?.rentalEstimate > 0).length
            : 0;

          marketComparison = {
            comparableCount: comparables.length,
            averageValue:
              comparables.reduce((sum: number, comp: any) => sum + comp.value, 0) /
              comparables.length,
            valueRange: {
          ...
          const llmResponse = await this.llmService.prompt([
            {
              role: 'system',
              content:
                'You are an investment property analysis expert for Benton County, Washington.',
            },
            {
              role: 'user',
      ...
      await this.logActivity('investment_potential_assessed', 'Investment potential assessed', {
        propertyId,
      });
      ...
      await this.logActivity(
        'investment_assessment_error',
        `Error assessing investment potential: ${error.message}`
      );
  ...
  private calculateInvestmentMetrics(params: {
    ...
    const appreciationAnnualized =
      Math.pow(1 + appreciationPercent / 100, 1 / investmentHorizon) - 1;
    ...
    if (financingDetails) {
      ...
      totalInterestPaid =
        monthlyMortgagePayment * paymentsOverInvestmentHorizon -
    ...
    return {
      ...
      debtServiceCoverageRatio:
        monthlyMortgagePayment > 0 ? netOperatingIncome / (monthlyMortgagePayment * 12) : null,
    };
  }
  ...
  private calculateBreakEvenPoint(params: {
    ...
    const { currentValue, improvementBudget, initialInvestment, annualCashFlow, financingDetails } =
      params;
    ...
    if (annualCashFlow <= 0) {
      return {
        ...
        months: null,
        description:
          'Cannot break even on cash flow alone; property requires appreciation for positive returns.',
      };
    }
  ...
  private identifyOpportunityAreas(property: any, marketFactors: any[]): string[] {
    ...
    if (property.propertyType === 'Single Family Residential') {
      ...
      if (yearBuilt > 0 && yearBuilt < 1990) {
        opportunities.push(
          'Potential for value-add through modernization and energy efficiency upgrades'
        );
      }
    ...
    if (property.address.includes('Richland')) {
      opportunities.push(
        'Proximity to Pacific Northwest National Laboratory may provide stable rental demand'
      );
    ...
    if (populationGrowth && populationGrowth.impact === 'positive') {
      opportunities.push(
        'Positive population growth trend supports long-term appreciation potential'
      );
    }
    ...
    if (opportunities.length === 0) {
      opportunities.push(
        'Consider consulting with local property management companies for detailed rental analyses'
      );
      opportunities.push(
        'Evaluate potential for property improvements to increase value or rental income'
      );
    }
   ...
   */
  private calculateValuationStatistics(
    property: any,
    comparables: any[]
  ): {
    mean: number;
  ...
  private generateAnomalyExplanations(
    ...
    if (isOvervalued) {
      explanations.push(
        `Property value is ${Math.abs(valuationStats.deviationPercent).toFixed(1)}% higher than comparable properties in the area.`
      );

      // Check for possible improvements that might explain higher value
      if (property.extraFields?.hasRecentRenovation) {
        explanations.push(
          'Recent renovations may account for the higher valuation compared to similar properties.'
        );
      }

      if (property.extraFields?.hasView || property.address.toLowerCase().includes('view')) {
        explanations.push(
          'Property may have desirable views or features not shared by comparable properties.'
        );
      }
      ...
      if (
        property.extraFields?.lotSize &&
        avgLotSize &&
        property.extraFields.lotSize > avgLotSize * 1.25
      ) {
        explanations.push(
          `Larger lot size (${property.extraFields.lotSize.toFixed(2)} acres vs. average ${avgLotSize.toFixed(2)}) may justify higher valuation.`
        );
      }

      if (isSignificantAnomaly) {
        explanations.push(
          'The significant deviation suggests a potential valuation error or missing comparable data.'
        );
      }
    } else if (isUndervalued) {
      explanations.push(
        `Property value is ${Math.abs(valuationStats.deviationPercent).toFixed(1)}% lower than comparable properties in the area.`
      );
      ...
      if (yearBuilt > 0 && avgYearBuilt > 0 && yearBuilt < avgYearBuilt - 15) {
        explanations.push(
          `Older construction (built in ${yearBuilt} vs. average ${avgYearBuilt}) may contribute to lower valuation.`
        );
      }
      ...
      if (
        property.extraFields?.squareFootage &&
        avgSqFt &&
        property.extraFields.squareFootage < avgSqFt * 0.8
      ) {
        explanations.push(
          `Smaller living area (${property.extraFields.squareFootage.toLocaleString()} sq ft vs. average ${avgSqFt.toLocaleString()}) may explain lower valuation.`
        );
      }

      if (isSignificantAnomaly) {
        explanations.push(
          'The significant deviation suggests a potential valuation error or special circumstances affecting this property.'
        );
      }
    } else {
    ...
    if (explanations.length <= 1) {
      explanations.push(
        'Factors such as specific property condition, interior features, or recent market changes may account for valuation differences.'
      );
    }
    ...
    if (isAnomaly) {
      if (valuationStats.zScore > 0) {
        ...
        recommendations.push(
          'Review property assessment methodology to identify factors contributing to higher valuation.'
        );
        recommendations.push(
          'Consider performing a detailed on-site inspection to verify property condition and features.'
        );
        recommendations.push(
          'Evaluate if recent improvements or special characteristics justify the higher valuation.'
        );
      } else {
        // Undervalued property
        recommendations.push(
          'Verify property data for completeness and accuracy, as the property may be undervalued.'
        );
        recommendations.push(
          'Schedule property reassessment to capture potential improvements or market changes.'
        );
        recommendations.push(
          'Check for comparable sales that may have occurred after the last assessment date.'
        );
      }

      // General recommendations for anomalies
      recommendations.push(
        'Compare valuation methods with those used for comparable properties to ensure consistency.'
      );
      recommendations.push(
        'Document factors that explain the deviation from comparable properties for future reference.'
      );
    } else {
      // Normal valuation
      recommendations.push(
        'Valuation appears consistent with market comparables; routine validation is sufficient.'
      );
      recommendations.push(
        'Continue monitoring neighborhood sales for market changes that may affect future valuations.'
      );
    }
  ...
  private async generateMarketReport(params: any): Promise<any> {
    try {
      ...
      const result = {
        ...
        report: {
          summary: {
            ...
            valueAssessment: compsResult?.success
              ? compsResult.result.valueAnalysis.valueAssessment
              : null,
            investmentRating: investmentResult?.success
              ? investmentResult.result.investmentRating
              : null,
            forecastedValue: forecastResult?.success
              ? forecastResult.result.predictedValues[
                  forecastResult.result.predictedValues.length - 1
                ].predictedValue
              : null,
          },
      ...
      await this.logActivity('market_report_generated', 'Comprehensive market report generated', {
        propertyId,
      });
      ...
      await this.logActivity(
        'market_report_error',
        `Error generating market report: ${error.message}`
      );
  ...
  private async enhanceComparableProperties(targetProperty: any, comps: any[]): Promise<any[]> {
    ...
    for (const comp of comps) {
      ...
      const ageDiff =
        targetYearBuilt > 0 && compYearBuilt > 0 ? compYearBuilt - targetYearBuilt : 0;
  ...
  private calculateSimilarityScore(targetProperty: any, comp: any): number {
    ...
    const pricePercDiff =
      Math.abs((comp.value - targetProperty.value) / targetProperty.value) * 100;
   ...
   */
  private generateCompAdvantages(
    property: any,
    comps: any[]
  ): {
    advantages: string[];
    ...
    const avgSqFt =
      compsSqFt.length > 0 ? compsSqFt.reduce((sum, s) => sum + s, 0) / compsSqFt.length : 0;

    const compsYearBuilt = comps
      .map(c => c.property.extraFields?.yearBuilt || 0)
      .filter(y => y > 0);
    const avgYearBuilt =
      compsYearBuilt.length > 0
        ? compsYearBuilt.reduce((sum, y) => sum + y, 0) / compsYearBuilt.length
        : 0;
    ...
    const avgPricePerSqFt =
      compsPricePerSqFt.length > 0
        ? compsPricePerSqFt.reduce((sum, p) => sum + p, 0) / compsPricePerSqFt.length
        : 0;

    // Compare values
    if (targetValue < avgValue * 0.95) {
      advantages.push(
        `Lower price than comparable properties (${Math.round(((avgValue - targetValue) / avgValue) * 100)}% below average)`
      );
    } else if (targetValue > avgValue * 1.05) {
      disadvantages.push(
        `Higher price than comparable properties (${Math.round(((targetValue - avgValue) / avgValue) * 100)}% above average)`
      );
    }
    ...
    if (targetSqFt > avgSqFt * 1.05) {
      advantages.push(
        `Larger living area than comparable properties (${Math.round(((targetSqFt - avgSqFt) / avgSqFt) * 100)}% above average)`
      );
    } else if (targetSqFt < avgSqFt * 0.95 && targetSqFt > 0) {
      disadvantages.push(
        `Smaller living area than comparable properties (${Math.round(((avgSqFt - targetSqFt) / avgSqFt) * 100)}% below average)`
      );
    }
    ...
    if (targetYearBuilt > avgYearBuilt + 5) {
      advantages.push(
        `Newer construction than comparable properties (built ${targetYearBuilt - Math.round(avgYearBuilt)} years newer)`
      );
    } else if (targetYearBuilt < avgYearBuilt - 5 && targetYearBuilt > 0) {
      disadvantages.push(
        `Older construction than comparable properties (built ${Math.round(avgYearBuilt) - targetYearBuilt} years older)`
      );
    }
    ...
    if (targetPricePerSqFt < avgPricePerSqFt * 0.95 && targetPricePerSqFt > 0) {
      advantages.push(
        `Better value per square foot (${Math.round(((avgPricePerSqFt - targetPricePerSqFt) / avgPricePerSqFt) * 100)}% below average)`
      );
    } else if (targetPricePerSqFt > avgPricePerSqFt * 1.05 && targetPricePerSqFt > 0) {
      disadvantages.push(
        `Higher cost per square foot (${Math.round(((targetPricePerSqFt - avgPricePerSqFt) / avgPricePerSqFt) * 100)}% above average)`
      );
    }
  ...
  private calculateInvestmentRating(
    ...
    if (score >= 85) {
      rating = 'Excellent';
      explanation =
        'This property demonstrates exceptional investment potential with strong expected returns and minimal risk factors.';
    } else if (score >= 70) {
      rating = 'Good';
      explanation =
        'This property shows solid investment potential with favorable returns and manageable risk.';
    } else if (score >= 50) {
      rating = 'Moderate';
      explanation =
        'This property has average investment potential with reasonable returns but some risk factors.';
    } else if (score >= 30) {
      rating = 'Fair';
      explanation =
        'This property has below-average investment potential with limited returns or notable risk factors.';
    } else {
      rating = 'Poor';
      explanation =
        'This property demonstrates low investment potential with significant risk factors or inadequate returns.';
    }
  ...
  private identifyInvestmentRiskFactors(property: any, marketFactors: any[]): string[] {
    ...
    if (property.extraFields?.lastSalePrice && property.extraFields?.lastSaleDate) {
      ...
      const yearsSinceLastSale =
        (new Date().getTime() - lastSaleDate.getTime()) / (1000 * 60 * 60 * 24 * 365);

      if (yearsSinceLastSale > 0) {
        const annualAppreciation =
          Math.pow(currentValue / lastSalePrice, 1 / yearsSinceLastSale) - 1;

        if (annualAppreciation < 0.02) {
          riskFactors.push(
            `Low historical appreciation rate of ${(annualAppreciation * 100).toFixed(1)}% annually`
          );
        }
      }
    }
