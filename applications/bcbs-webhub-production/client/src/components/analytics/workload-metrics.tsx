...
export default function WorkloadMetrics({ className = "" }: WorkloadMetricsProps) {
  ...
  // Memoize default dates to prevent constant recalculation
  const defaultDates = useMemo(() => ({
    start: subDays(new Date(), 14),
    end: new Date()
  }), []);
  
  const startDate = dateRange?.start || defaultDates.start;
  const endDate = dateRange?.end || defaultDates.end;
  ...
  const { data: userMetrics, isLoading: isLoadingUserMetrics } = useQuery({
    queryKey: ["/api/analytics/workload", queryParams.start, queryParams.end, "users"],
    queryFn: async () => {
      ...
      const res = await apiRequest("GET", `/api/analytics/workload?${params}`);
      return res.json();
    },
  ...
  const { data: timeMetrics, isLoading: isLoadingTimeMetrics } = useQuery({
    queryKey: ["/api/analytics/workload", queryParams.start, queryParams.end, "time"],
    queryFn: async () => {
      const params = new URLSearchParams({
        start: queryParams.start,
        end: queryParams.end,
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="h-9 flex items-center">
                ...
                {dateRange ? (
                  ...
                  <span>
                    {format(defaultDates.start, "MMM d")} - {format(defaultDates.end, "MMM d, yyyy")}
                  </span>
                )}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0" align="end">
              <Calendar
                mode="range"
                selected={{
                  from: dateRange?.start || defaultDates.start,
                  to: dateRange?.end || defaultDates.end,
                }}
                onSelect={(range) => {
...
variant="outline" className="flex items-center border-emerald-200 bg-emerald-50 text-emerald-700 py-1.5"><>
...
variant="outline" className="flex items-center border-amber-200 bg-amber-50 text-amber-700 py-1.5">
