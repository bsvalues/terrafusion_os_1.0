...
export function registerEnhancedAnalyticsAPI(app: Express, ensureAuthenticated: any) {
  ...
  app.get("/api/analytics/historical", ensureAuthenticated, async (req, res) => {
    try {
      ...
      const filteredAudits = audits.filter(audit => 
        audit.submittedAt >= startDate && audit.submittedAt <= endDate
      );
      
      // Group data by time period
      const timeSeriesData = generateTimeSeriesData(filteredAudits, startDate, endDate, granularity as string);
  ...
  app.get("/api/analytics/performance", ensureAuthenticated, async (req, res) => {
    try {
      ...
      const performanceMetrics = calculatePerformanceMetrics(audits, auditEvents, startDate, endDate);
  ...
  app.post("/api/analytics/reports/generate", ensureAuthenticated, async (req, res) => {
    try {
      ...
      if (!result.success) {
        return res.status(400).json({ error: "Invalid report configuration", details: result.error });
      }
      
      const { type, dateRange, filters = {}, sections = ["summary", "charts", "tables"] } = result.data;
  ...
  app.get("/api/analytics/realtime", ensureAuthenticated, async (req, res) => {
    try {
      ...
      const yesterdayAudits = audits.filter(audit => 
        audit.submittedAt >= yesterdayStart && audit.submittedAt < todayStart
      );
      ...
      const realTimeData = {
        currentStats: {
          ...
          completedToday: todayAudits.filter(a => a.status === "approved" || a.status === "rejected").length
        },
        ...
        trends: {
          todayVsYesterday: {
            audits: ((todayAudits.length - yesterdayAudits.length) / Math.max(yesterdayAudits.length, 1) * 100).toFixed(1),
...
// Helper functions
function generateTimeSeriesData(audits: any[], startDate: Date, endDate: Date, granularity: string) {
  const data = [];
  ...
  while (current <= endDate) {
    ...
    const periodAudits = audits.filter(audit => 
      audit.submittedAt >= current && audit.submittedAt < nextPeriod
    );
...
function calculatePerformanceMetrics(audits: any[], events: any[], startDate: Date, endDate: Date) {
  ...
  return {
    ...
    completionRate: filteredAudits.length > 0 ? (completedAudits.length / filteredAudits.length * 100) : 0,
    approvalRate: completedAudits.length > 0 ? (filteredAudits.filter(a => a.status === "approved").length / completedAudits.length * 100) : 0,
    avgProcessingTime: avgProcessingTime.toFixed(2),
    throughput: completedAudits.length / Math.max(1, (endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)), // audits per day
...
function calculateCompletionTrend(todayAudits: any[], yesterdayAudits: any[]) {
  const todayCompleted = todayAudits.filter(a => a.status === "approved" || a.status === "rejected").length;
  const yesterdayCompleted = yesterdayAudits.filter(a => a.status === "approved" || a.status === "rejected").length;
