  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setMetrics(prev => ({
        ...
        aiAgentsActive: Math.max(980, Math.min(1008, prev.aiAgentsActive + Math.floor((Math.random() - 0.5) * 20))),
        quantumCoherence: Math.max(90, Math.min(99, prev.quantumCoherence + (Math.random() - 0.5) * 2)),
        networkLatency: Math.max(5, Math.min(50, prev.networkLatency + Math.floor((Math.random() - 0.5) * 10)))
      }));
    }, 2000);
