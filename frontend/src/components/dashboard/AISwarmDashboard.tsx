...
import { LineChart, Line, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, Legend, ResponsiveContainer, AreaChart, Area, PieChart, Pie, Cell, BarChart, Bar } from 'recharts';
...
export const AISwarmDashboard: React.FC = () => {
  ...
  const [selectedView, setSelectedView] = useState<'overview' | 'agents' | 'performance' | 'tasks' | 'quantum'>('overview');
  ...
  const generateMockAgentData = useCallback((): AgentMetrics[] => {
    const agentTypes = ['SUPREME_COMMANDER', 'AI_COUNCIL_MEMBER', 'QUANTUM_COMMANDER', 'DOMAIN_GENERAL', 'EXPERT_SPECIALIST'];
    const statuses: ('ACTIVE' | 'PROCESSING' | 'MAINTENANCE' | 'STANDBY')[] = ['ACTIVE', 'PROCESSING', 'MAINTENANCE', 'STANDBY'];
    ...
    if (isAutoRefresh) {
      const interval = setInterval(() => {
        ...
        setSwarmStatus(prev => ({
          ...
          averageUtilization: Math.max(0, Math.min(100, prev.averageUtilization + Math.random() * 4 - 2)),
          quantumCoherence: Math.max(90, Math.min(100, prev.quantumCoherence + Math.random() * 2 - 1)),
          consciousnessLevel: Math.max(1, Math.min(10, prev.consciousnessLevel + Math.random() * 0.4 - 0.2)),
          activeTasks: Math.max(0, prev.activeTasks + Math.floor(Math.random() * 20 - 10)),
          performanceScore: Math.max(90, Math.min(100, prev.performanceScore + Math.random() * 2 - 1))
        }));
      }, refreshInterval);
...
width="100%" height={400}>
              <LineChart data={performanceHistory}>
                ...
                <Line type="monotone" dataKey="responseTime" stroke="#2196f3" name="Response Time (ms)" />
                <Line type="monotone" dataKey="quantumCoherence" stroke="#9c27b0" name="Quantum Coherence %" />
              </LineChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
...
align="right">{task.averageTime}</TableCell>
                      <TableCell align="center">
                        <Chip
                          icon={task.successRate >= 98 ? <CheckIcon /> : task.successRate >= 95 ? <WarningIcon /> : <ErrorIcon />}
                          label={task.successRate >= 98 ? 'Excellent' : task.successRate >= 95 ? 'Good' : 'Needs Attention'}
                          color={task.successRate >= 98 ? 'success' : task.successRate >= 95 ? 'warning' : 'error'}
                          size="small"
                        />
                      </TableCell>
                    </TableRow>
          ...
          <Table>
            <TableHead>
              <TableRow>
                ...
                <TableCell align="right"><strong>Quantum</strong></TableCell>
