...
const Marketplace: React.FC = () => {
  ...
  return <div className="w-full p-8">
      ...
      <div className="flex gap-4 items-center">
        <input type="text" placeholder="Search modules, vendors, or features..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} style={{
        flex: '1',
        minWidth: '300px',
        maxWidth: '500px',
        padding: '0.8rem 1rem',
      ...
      <div className="flex gap-2">
        {categories.map(category => <button key={category.id} onClick={() => setSelectedCategory(category.id)} style={{
        background: selectedCategory === category.id ? 'linear-gradient(135deg, #0099ff, #00ffee)' : 'rgba(0,153,255,0.1)',
      ...
      {/* Featured Section */}
      {featuredItems.length > 0 && <div style={{
      marginBottom: '3rem'
    }}>
        ...
        margin: '0 auto'
            ...
            {featuredItems.map(item => <div key={item.id} style={{
          ...
          e.currentTarget.style.boxShadow = 'none';
            ...
            marginBottom: '1rem'
              ...
              color: '#00ffaa'
                ...
                <div className="flex gap-2">
                  {item.tags.map((tag, index) => <span key={index} className="font-semibold">
                      {tag}
                    </span>)}
                </div>
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => installModule(item.id)} onMouseEnter={e => {
              ...
              e.currentTarget.style.transform = 'scale(1)';
                  ...
                  <button onClick={() => viewDetails(item.id)} onMouseEnter={e => {
      ...
      marginBottom: '2rem'
        ...
        margin: '0 auto'
          ...
          {filteredItems.map(item => <div key={item.id} style={{
