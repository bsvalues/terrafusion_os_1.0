...
describe('Separator', () => {
  ...
  describe('Decorative Attribute', () => {
    ...
    it('does not apply aria-orientation when decorative', () => {
      const { container } = render(<Separator decorative={true} orientation='horizontal' />);
      const separator = container.firstChild;
  ...
  describe('Visual Styling', () => {
    ...
    it('applies custom height via className', () => {
      const { container } = render(<Separator className='h-[2px]' />);
      const separator = container.firstChild;
      expect(separator).toHaveClass('h-[2px]');
    });
    it('applies custom width via className', () => {
      const { container } = render(<Separator orientation='vertical' className='w-[2px]' />);
      const separator = container.firstChild;
      expect(separator).toHaveClass('w-[2px]');
    });
    it('applies custom color via className', () => {
      const { container } = render(<Separator className='bg-blue-500' />);
      const separator = container.firstChild;
  ...
  describe('Accessibility', () => {
    ...
    it('has no accessibility violations (with aria-label)', async () => {
      const { container } = render(<Separator decorative={false} aria-label='Content divider' />);
      const results = await axe(container);
      expect(results).toHaveNoViolations();
    });
  ...
  describe('Edge Cases', () => {
    ...
    it('applies data attributes', () => {
      const { container } = render(<Separator data-testid='my-separator' data-state='visible' />);
    ...
    it('handles undefined decorative gracefully', () => {
      const { container } = render(<Separator decorative={undefined} />);
      const separator = container.firstChild;
      expect(separator).toBeInTheDocument();
    });
    it('combines multiple classNames', () => {
      const { container } = render(<Separator className='custom-class-1 custom-class-2' />);
      const separator = container.firstChild;
