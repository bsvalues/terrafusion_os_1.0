...
export const useGovernmentCompliance = () => {
  ...
  const validateCompliance = useCallback(
    async (request: ComplianceValidationRequest): Promise<ComplianceValidationResponse> => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error ? err.message : 'Unknown compliance validation error';
  ...
  const generateComplianceReport = useCallback(
    async (projectId: string, includeAuditTrail: boolean = true) => {
      try {
  ...
  const fixComplianceViolation = useCallback(
    async (violationId: string, code: string, autoFix: boolean = true) => {
      try {
  ...
  const getComplianceTemplate = useCallback(
    async (
  ...
  const score =
    violations.length === 0
      ...
      : Math.max(
          0,
          100 -
            violations.filter((v) => v.severity === 'error').length * 20 -
