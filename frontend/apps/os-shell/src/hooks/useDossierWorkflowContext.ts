...
import { dossierWorkflowService, WorkflowRequestError, type WorkflowContext } from '../services/dossierWorkflowService';
...
export function useDossierWorkflowContext(parcelId?: string) {
  ...
  const [saveState, setSaveState] = useState<{ epoch: number; loading: boolean; error?: string }>({ epoch, loading: false });
  ...
  const saveDraft = async () => {
    ...
    try {
      const saved = await dossierWorkflowService.saveDraft(token, countyId, studyId, saveRequest.current.id);
      ...
      setSaveState({ epoch: ticket, loading: false });
    } catch (error) {
      if (generation.current.value === ticket) setSaveState({ epoch: ticket, loading: false, error: error instanceof Error ? error.message : 'Snapshot save failed.' });
    } finally {
...
type ActionState<T> = { status: 'idle' | 'loading' | 'success' | 'error'; result?: T; correlationId?: string; error?: { message: string; code: string } };
export function useWorkflowAction<T>(context: DossierWorkflowContext, extraIdentity = '') {
  ...
  const run = async (request: PilotInvokeRequest, validate?: (value: unknown) => T) => {
    ...
    setStored({ epoch: ticket, value: { status: 'loading' } });
    ...
    try {
      ...
      if (!response.success || !response.result) throw new WorkflowRequestError(response.error?.message ?? 'Workflow failed.', correlationId);
      ...
      setStored({ epoch: ticket, value: { status: 'success', result, correlationId } });
      ...
      if (current()) setStored({ epoch: ticket, value: { status: 'error', correlationId: error instanceof WorkflowRequestError ? error.correlationId ?? correlationId : correlationId,
        error: { code: 'WORKFLOW_FAILED', message: error instanceof Error ? error.message : 'Workflow failed.' } } });
    } finally {
