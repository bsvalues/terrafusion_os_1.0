...
export const ErrorProvider: React.FC<ErrorProviderProps> = ({
  ...
  const retry = useCallback(() => {
    if (state.retryCount < autoRetryAttempts) {
...
const sendErrorToMonitoring = async (error: ErrorInfo) => {
  try {
 ...
 * Phase 1: Uses correlationId-first ErrorDisplay component
 */
export const GlobalErrorDisplay: React.FC = () => {
  const { state, setGlobalError, retry } = useErrorContext();

  if (!state.globalError) {
    return null;
  }

  // Transform ErrorInfo to ErrorDisplay props format
  const errorDisplayProps = {
    message: state.globalError.message,
    errorCode: state.globalError.context?.errorCode as string | undefined,
    correlationId: state.globalError.correlationId,
    timestamp: state.globalError.timestamp,
    component: state.globalError.context?.component as string | undefined,
  };

  return (
    <div className='fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4'>
      <div className='bg-white rounded-lg shadow-xl max-w-md w-full p-6'>
        {/* Use new ErrorDisplay component with correlationId contract */}
        <ErrorDisplay error={errorDisplayProps} />
