...
function isSuiteIncomeRuntime(metadata: Record<string, unknown> | undefined): boolean {
  return (
    metadataString(metadata, 'launchContext') === 'terraforge-suite' &&
    metadataString(metadata, 'dataSource') === 'terrafusion-api' &&
    metadataString(metadata, 'runtimePath') === 'income-approach'
  );
}

export default function IncomeForge({ metadata }: IncomeForgeProps = {}) {
  ...
  const suiteRuntime = isSuiteIncomeRuntime(metadata);
    ...
    <div data-testid="income-forge" className="h-full overflow-auto bg-background p-5 text-foreground">
      <header className="mb-4 flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {suiteRuntime && <Badge variant="outline">TerraForge Suite · Benton income approach API</Badge>}
        </div>
      </header>

      {suiteRuntime && (
        <div className="mb-4 rounded border bg-muted/20 px-3 py-2 text-xs text-muted-foreground">
          Environment: Benton County. Runtime source: CostForge income-approach endpoints.
          Benton-certified until additional county income references are separately proven.
        </div>
      )}
