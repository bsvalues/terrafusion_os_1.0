...
describe('NeighborhoodRollupTable', () => {
  ...
  it('filters to selectedCity from the store', () => {
    ...
    expect(screen.getByText('Neighborhood NBHD-K1')).toBeInTheDocument();
    expect(screen.getByText('Neighborhood NBHD-K2')).toBeInTheDocument();
    expect(screen.queryByText('Neighborhood NBHD-R1')).not.toBeInTheDocument();
  });

  it('clicking a row drills to that neighborhood', () => {
    render(<NeighborhoodRollupTable />);
    fireEvent.click(screen.getByText('Neighborhood NBHD-K1'));
  ...
  it('NonCompliant filter pill hides compliant rows within the city', () => {
    ...
    expect(screen.queryByText('Neighborhood NBHD-K1')).not.toBeInTheDocument();
    expect(screen.getByText('Neighborhood NBHD-K2')).toBeInTheDocument();
  });
