...
export const getPerformanceClass = (performanceIndex: number): string => {
...
export const getMemoryClass = (memoryUsage: number): string => {
...
export const setCSSVariables = (element: HTMLElement, performance: PerformanceMetrics): void => {
  if (!element) return;
  ...
  const statusColors: Record<string, string> = {
    active: 'hsl(var(--tf-success-hs) 60%)',
    processing: 'hsl(var(--tf-network-blue-hs) 60%)',
    idle: 'hsl(var(--tf-text-secondary-hs) 60%)',
    error: 'hsl(var(--tf-error-hs) 60%)',
  };
