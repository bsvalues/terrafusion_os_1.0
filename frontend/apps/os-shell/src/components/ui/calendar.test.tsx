...
describe.skip('Calendar', () => {
  ...
  describe.skip('Date Selection', () => {
    ...
    it('renders with selected date', () => {
      ...
      const buttons = screen.getAllByRole('button');
      const selectedButton = buttons.find((btn) => btn.getAttribute('aria-selected') === 'true');
    ...
    it('supports multiple date selection mode', () => {
      ...
      const buttons = screen.getAllByRole('button');
      const selectedButtons = buttons.filter((btn) => btn.getAttribute('aria-selected') === 'true');
  ...
  describe.skip('Accessibility', () => {
    ...
    it('has no accessibility violations - with selected date', async () => {
      const { container } = render(<Calendar selected={new Date(2024, 5, 15)} />);
    ...
    it('has no accessibility violations - with disabled dates', async () => {
      const { container } = render(<Calendar disabled={new Date(2024, 5, 15)} />);
    ...
    it('day buttons have proper ARIA labels', () => {
      ...
      const dayButtons = screen.getAllByRole('button');
      const dayButton = dayButtons.find((btn) => btn.hasAttribute('aria-label'));
    ...
    it('renders birthday picker with past dates only', () => {
      ...
      render(
        <div>
          ...
          <Calendar mode='single' toDate={today} captionLayout='dropdown' />
        </div>
      );
