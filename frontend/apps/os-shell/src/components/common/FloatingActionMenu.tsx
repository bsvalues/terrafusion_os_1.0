import React, { useState } from 'react';
import { Box, SpeedDial, SpeedDialAction } from '@mui/material';
import { Add, Business, Search, Analytics, Security, CloudDownload } from '@mui/icons-material';
import { styled } from '@mui/material/styles';

const StyledSpeedDial = styled(SpeedDial)(() => ({
  position: 'fixed',
  bottom: 32,
  right: 32,
  zIndex: 1300,
  '& .MuiFab-primary': {
    background: 'linear-gradient(135deg, var(--tf-transcend-cyan), var(--tf-accent-quantum))',
    color: 'white',
    width: 64,
    height: 64,
    boxShadow: '0 8px 32px hsl(var(--tf-transcend-cyan-hs) 50% / 0.4)',
    '&:hover': {
      background: 'linear-gradient(135deg, var(--tf-network-blue), var(--tf-transcend-cyan))',
      boxShadow: '0 12px 40px hsl(var(--tf-transcend-cyan-hs) 50% / 0.6)',
      transform: 'scale(1.1)',
    },
  },
  '& .MuiSpeedDialAction-fab': {
    background: 'hsl(var(--tf-text-primary-hs) 100% / 0.1)',
    backdropFilter: 'blur(10px)',
    border: '1px solid hsl(var(--tf-transcend-cyan-hs) 50% / 0.2)',
    color: 'white',
    '&:hover': {
      background: 'hsl(var(--tf-transcend-cyan-hs) 50% / 0.2)',
      border: '1px solid hsl(var(--tf-transcend-cyan-hs) 50% / 0.4)',
      transform: 'scale(1.1)',
    },
  },
}));

interface FloatingActionMenuProps {
  onNewAssessment?: () => void;
  onSearch?: () => void;
  onAnalytics?: () => void;
  onSecurity?: () => void;
  onExport?: () => void;
}

const FloatingActionMenu: React.FC<FloatingActionMenuProps> = ({
  onNewAssessment,
  onSearch,
  onAnalytics,
  onSecurity,
  onExport,
}) => {
  const [open, setOpen] = useState(false);

  const actions = [
    {
      icon: <Business />,
      name: 'New Property Assessment',
      onClick: onNewAssessment,
      color: 'var(--tf-network-blue)',
    },
    {
      icon: <Search />,
      name: 'Search Properties',
      onClick: onSearch,
      color: 'var(--tf-transcend-cyan)',
    },
    {
      icon: <Analytics />,
      name: 'View Analytics',
      onClick: onAnalytics,
      color: 'var(--tf-accent-quantum)',
    },
    {
      icon: <Security />,
      name: 'Security Dashboard',
      onClick: onSecurity,
      color: 'var(--tf-accent-success)',
    },
    {
      icon: <CloudDownload />,
      name: 'Export Data',
      onClick: onExport,
      color: 'var(--tf-warning-amber)',
    },
  ];

  return (
    <StyledSpeedDial
      ariaLabel='Quick Actions'
      icon={<Add className='tf-rotate-glow' />}
      open={open}
      onOpen={() => setOpen(true)}
      onClose={() => setOpen(false)}
      direction='up'
    >
      {actions.map((action) => (
        <SpeedDialAction
          key={action.name}
          icon={
            <Box
              className='tf-scale-in'
              sx={{
                color: action.color,
                transition: 'all 0.3s ease',
                '&:hover': {
                  filter: `drop-shadow(0 0 10px ${action.color})`,
                },
              }}
            >
              {action.icon}
            </Box>
          }
          tooltipTitle={action.name}
          onClick={() => {
            action.onClick?.();
            setOpen(false);
          }}
          sx={{
            '& .MuiSpeedDialAction-staticTooltip': {
              background: 'hsl(var(--tf-text-primary-hs) 0% / 0.8)',
              color: 'white',
              backdropFilter: 'blur(10px)',
              border: '1px solid hsl(var(--tf-transcend-cyan-hs) 50% / 0.2)',
              borderRadius: '8px',
            },
          }}
        />
      ))}
    </StyledSpeedDial>
  );
};

export default FloatingActionMenu;
