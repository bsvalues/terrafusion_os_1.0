...
interface AssessmentSourceStatusCardProps {
  assessmentSource: FreshData<AssessmentSourceHealth>
}

export function AssessmentSourceStatusCard({ assessmentSource }: AssessmentSourceStatusCardProps) {
  if (assessmentSource.source === 'unavailable') {
    return (
      <div className="rounded-lg border border-red-900/40 bg-terra-midnight p-4 flex flex-col gap-1">
        <p className="text-xs tf-text-dim uppercase tracking-wide">Assessment Source</p>
        <span className="text-xs text-red-400 font-medium">Unavailable</span>
      </div>
    )
  }

  if (!canRender(assessmentSource)) {
    return (
  ...
  const { data } = assessmentSource

  return (
    <div className={cn(
      'rounded-lg border p-4 flex flex-col gap-1',
      assessmentSource.isStale ? 'border-amber-800/40 bg-terra-midnight' : 'border-white/10 bg-terra-midnight'
    )}>
      <div className="flex items-center justify-between">
        <p className="text-xs tf-text-dim uppercase tracking-wide">Assessment Source</p>
        {assessmentSource.source === 'fallback' && <span className="text-xs text-amber-400">Last known</span>}
        {assessmentSource.isStale && assessmentSource.source !== 'fallback' && <span className="text-xs text-amber-400">Stale</span>}
      </div>
      {data && (
        <>
