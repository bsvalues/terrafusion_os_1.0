...
const SimulatedDesktop: React.FC<{
  ...
  <DesktopErrorBoundary>
    <div data-testid='desktop-container'>
      {windows.map((w) => (
        <WindowErrorBoundary key={w.id} windowId={w.id} moduleName={`Module ${w.id}`}>
...
describe('Error Boundary Integration', () => {
  describe('Window Error Isolation', () => {
    it('failing window does not crash sibling windows', () => {
      ...
      // Window 1 shows error (appears in both error text and stack trace)
      const errorElements = screen.getAllByText(/Error in component window-1/);
    ...
    it('multiple windows can fail independently', () => {
      ...
      // Both failing windows show errors (appear in both error text and stack trace)
      const error1Elements = screen.getAllByText(/Error in component window-1/);
      const error2Elements = screen.getAllByText(/Error in component window-2/);
      expect(error1Elements.length).toBeGreaterThan(0);
      expect(error2Elements.length).toBeGreaterThan(0);
    ...
    it('desktop container remains intact when windows fail', () => {
      render(<SimulatedDesktop windows={[{ id: 'window-1', fails: true }]} />);
  ...
  describe('Desktop Error Boundary', () => {
    ...
    it('shows desktop error fallback with branding', () => {
      ...
      // TerraFusion appears multiple times (header + footer), use getAllByText
      const terraFusionElements = screen.getAllByText(/TerraFusion/i);
      expect(terraFusionElements.length).toBeGreaterThan(0);
  ...
  describe('Error Logging', () => {
    it('logs window errors with component context', () => {
      render(
        <WindowErrorBoundary windowId='logged-window' moduleName='Logged Module'>
