...
const LoadingState: React.FC<{ moduleName: string }> = ({ moduleName }) => (
  ...
  >
    ...
    <div className="mt-2 w-48 h-1 bg-slate-700 rounded-full overflow-hidden">
      <div className="h-full bg-gradient-to-r from-cyan-400 to-blue-500 animate-pulse" style={{ width: '60%' }} />
    </div>
  </div>
);
...
const NotFoundState: React.FC<{ moduleId: string }> = ({ moduleId }) => (
  ...
  >
    ...
    <p className="text-slate-400 text-sm text-center">
      The module <code className="text-amber-400 bg-slate-800 px-2 py-0.5 rounded">{moduleId || '(empty)'}</code> could not be found in the registry.
    </p>
  </div>
);
