    ...
    <div className='relative'>
      ...
      <input
        ...
        className={cn(
          ...
          'rounded-lg',
          'focus:outline-none focus:ring-2 focus:ring-[hsl(var(--tf-accent)/0.4)] focus:border-[hsl(var(--tf-accent)/0.3)]',
  ...
  return (
    <button
      ...
      className={cn(
        ...
        'focus:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--tf-accent))] focus-visible:ring-offset-1 focus-visible:ring-offset-transparent',
        'active:scale-95'
      )}
      style={{ ['--hover-bg' as string]: 'hsl(var(--tf-text)/0.06)' }}
      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = 'hsl(var(--tf-text)/0.06)'; }}
  ...
  return (
    <button
      ...
      className={cn(
        ...
        'rounded-lg',
        'transition-all duration-150',
        'focus:outline-none focus-visible:ring-2 focus-visible:ring-[hsl(var(--tf-accent))] focus-visible:ring-inset',
        'active:scale-[0.98]'
      )}
      onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = 'hsl(var(--tf-text)/0.06)'; }}
      onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = ''; }}
...
const AllAppsList: React.FC<{ onLaunch: (module: Module) => void }> = ({ onLaunch }) => {
  ...
  return (
    <div data-testid='all-apps' className='flex-1 overflow-y-auto'>
      <h2
        className='text-xs font-semibold uppercase tracking-wider mb-2 px-1 sticky top-0 py-1'
        style={{
          color: 'hsl(var(--tf-muted))',
          background: 'hsl(var(--tf-surface) / 0.97)',
        }}
      >
        {title}
      </h2>
...
const UserProfile: React.FC = () => {
  ...
  return (
    <div
      ...
      className='flex items-center justify-between p-3 mt-auto'
      style={{
    ...
    >
      <div
        ...
        className={cn(
          ...
          'focus:outline-none focus:ring-2 focus:ring-[hsl(var(--tf-accent)/0.5)]'
        )}
        onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.background = 'hsl(var(--tf-text)/0.05)'; }}
        onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.background = ''; }}
        onKeyDown={(e) => {
      ...
      >
        <div
          ...
          className={cn(
            ...
            'bg-gradient-to-br from-[hsl(var(--tf-accent))] to-[hsl(var(--tf-accent-2))]',
            'flex items-center justify-center',
            'font-bold text-sm'
          )}
          style={{ color: 'hsl(var(--tf-bg))' }}
        >
          BC
        </div>
        <div className='flex flex-col'>
          ...
          <span className='text-xs' style={{ color: 'hsl(var(--tf-muted))' }}>
            {t('startMenu.userProfile.location')}
          </span>
        </div>
      </div>
      ...
      <div className='flex items-center gap-1'>
        <button
          ...
          onMouseEnter={(e) => {
            ...
            el.style.color = 'hsl(var(--tf-text))';
            el.style.background = 'hsl(var(--tf-text)/0.07)';
          }}
          onMouseLeave={(e) => {
            const el = e.currentTarget as HTMLElement;
            el.style.color = 'hsl(var(--tf-text) / 0.6)';
            el.style.background = '';
          }}
  ...
  if (recentParcels.length === 0) {
    return (
      <div className='mt-2'>
        ...
        <p className='px-3 py-1.5 text-xs' style={{ color: 'hsl(var(--tf-muted) / 0.7)' }}>
          No recent parcels
        </p>
      </div>
    );
      ...
      <div className='space-y-0.5 max-h-[120px] overflow-y-auto'>
        {parcels.slice(0, MAX_DISPLAYED_PARCELS).map((parcelId) => (
          ...
          >
            ...
            />
            <span className='text-xs truncate' style={{ color: 'hsl(var(--tf-text) / 0.85)' }}>
              Parcel {parcelId}
            </span>
          </button>
        ))}
  ...
  return (
    ...
    >
      ...
      <div
        ...
        style={{ borderBottom: '1px solid hsl(var(--tf-border) / 0.6)' }}
      >
        ...
        <button
          onClick={close}
          aria-label='Close launcher (Escape)'
          title='Close (Escape)'
          style={{
          ...
          onMouseEnter={e => {
            (e.currentTarget as HTMLButtonElement).style.background = 'hsl(var(--tf-surface-2))';
            (e.currentTarget as HTMLButtonElement).style.color = 'hsl(var(--tf-text))';
          }}
          onMouseLeave={e => {
            (e.currentTarget as HTMLButtonElement).style.background = 'none';
            (e.currentTarget as HTMLButtonElement).style.color = 'hsl(var(--tf-muted))';
          }}
        >
          ✕
        </button>
      ...
      <div className='flex-1 p-3 flex flex-col overflow-hidden'>
        ...
        {/* Divider */}
        <div className='my-2' style={{ height: '1px', background: 'hsl(var(--tf-border))' }} />
