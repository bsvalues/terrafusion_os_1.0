...
describe('Dossier handoff trace authority', () => {
  it('does not invent prepared or appeal-accepted traces from local metadata', () => {
    expect(prepareAppealHandoff(snapshot).success).toBe(false); expect(emitTraceEvent).not.toHaveBeenCalled();
  });
  it('does not claim a completed handoff on server refusal', async () => {
    vi.stubGlobal('fetch', vi.fn(async () => response({ error: 'Nonfinalized' }, 409)));
    await expect(prepareWorkflowHandoff(context, packetId, revision, 'prepare', signal())).rejects.toThrow('Nonfinalized');
    expect(emitTraceEvent).not.toHaveBeenCalled();
  });
  it('returns a server handoff without minting a second browser audit or appeal ID', async () => {
    ...
    expect(result).not.toHaveProperty('appealId');
    expect(emitTraceEvent).not.toHaveBeenCalled();
  });
});
