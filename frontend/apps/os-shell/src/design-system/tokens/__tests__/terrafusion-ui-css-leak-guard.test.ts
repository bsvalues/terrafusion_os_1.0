/**
 * Leak-guard: terrafusion-ui.css
 * Ensures no raw hex / rgba / rgb / hsl colours creep back in.
 */
import { readFileSync } from 'fs';
import { resolve } from 'path';

const css = readFileSync(resolve(__dirname, '../../../components/ui/terrafusion-ui.css'), 'utf8');
...
test('all hsl() must be token-based (contain var(--tf-))', () => {
  ...
  const hits = findAll(HSL_RE, clean).filter((h) => !h.includes('var(--tf-'));
  expect(hits).toEqual([]);
});
