...
const TSX_PATH = resolve(
  __dirname,
  '../../../components/pilot/RiskConfirmationModal.tsx',
);
  ...
  while (i < input.length) {
    ...
    if (ch === "'") {
      ...
      while (j < input.length) {
        const c = input[j];
        s += c;
        if (c === '\\' && j + 1 < input.length) { s += input[j + 1]; j += 2; continue; }
        if (c === "'") break;
        j++;
      }
      strings.push(s);
    ...
    if (ch === '`') {
      ...
      while (j < input.length) {
        ...
        s += c;
        if (c === '\\' && j + 1 < input.length) { s += input[j + 1]; j += 2; continue; }
        if (c === '$' && input[j + 1] === '{') { depth++; s += '{'; j += 2; continue; }
...
describe('RiskConfirmationModal.tsx – TSX token leak guard', () => {
