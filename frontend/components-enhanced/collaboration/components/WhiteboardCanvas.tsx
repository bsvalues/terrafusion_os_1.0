...
import {CanvasElement,
  ...
  CollaborationUser,
  ElementStyle,} from '../types/CollaborationTypes';
import {collaborationService} from '../services/CollaborationService';

interface WhiteboardCanvasProps {sessionId: string;
...
export const WhiteboardCanvas: React.FC<WhiteboardCanvasProps> = ({sessionId,
  currentTool,
  currentUser,
  className = '',
  width = 800,
  ...
  onElementCreated,
  onElementUpdated,
  onElementDeleted,}) => {const { toast} = useToast();
  ...
  useEffect(() => {const handleWhiteboardUpdated = (data: {
      sessionId: string;
      element: CanvasElement;
      action: 'create' | 'update' | 'delete';
      userId: string;}) => {if (data.sessionId === sessionId && data.userId !== currentUser?.id) {
        switch (data.action) {
          case 'create':
    ...
    const handleCursorMove = (data: {sessionId: string;
      userId: string;
      cursor: { x: number; y: number};
    }) => {if (data.sessionId === sessionId && data.userId !== currentUser?.id) {
        setParticipants((prev) =>
          prev.map((p) => (p.user.id === data.userId ? { ...p, cursor: data.cursor} : p))
        );
      }
    };
    ...
    return () => {collaborationService.off('whiteboard-updated', handleWhiteboardUpdated);
  ...
  useEffect(() => {if (!context) return;
    ...
    // Draw all elements
    elements.forEach((element) => drawElement(context, element));}, [context, elements, backgroundColor, width, height]);

  // Draw overlay (cursors, selection, etc.)
  useEffect(() => {if (!overlayContext) return;
    ...
    // Draw selection
    if (selectedElement) {const element = elements.find((el) => el.id === selectedElement);
      if (element) {
  ...
  const getMousePos = useCallback((e: React.MouseEvent<HTMLCanvasElement>): Point => {const canvas = canvasRef.current;
  ...
  const handleMouseDown = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {if (!currentUser) return;
  ...
  const handleMouseMove = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) => {if (!currentUser) return;
      ...
      switch (currentTool) {case WhiteboardTool.PEN:
        case WhiteboardTool.HIGHLIGHTER:
  ...
  const handleMouseUp = useCallback(
    (e: React.MouseEvent<HTMLCanvasElement>) =>{if (!currentUser || !isDrawing) return;
      ...
      switch (currentTool) {
        ...
        case WhiteboardTool.SHAPE:
          if (dragStart) {createShapeElement(dragStart, pos);}
          break;
      }
  ...
  const createShapeElement = useCallback(
    (start: Point, end: Point) => {
      ...
      const element: CanvasElement = {
        ...
        data: {shape: 'rectangle', // Could be configurable
          startPoint: start,
          endPoint: end,},
  ...
  const addElement = useCallback(
    (element: CanvasElement) => {setElements((prev) => [...prev, element]);
  ...
  const deleteElement = useCallback(
    (elementId: string) => {setElements((prev) => prev.filter((el) => el.id !== elementId));
  ...
  const findElementAtPosition = useCallback(
    (pos: Point): CanvasElement | null => {// Check elements in reverse order (top to bottom)
  ...
  // Draw single element
  const drawElement = useCallback((ctx: CanvasRenderingContext2D, element: CanvasElement) =>{ctx.save();

    // Apply styles
    if (element.style.color) {
      ctx.strokeStyle = element.style.color;
      ctx.fillStyle = element.style.color;}
    if (element.style.fontSize) {ctx.lineWidth = element.style.fontSize;}
    if (element.style.opacity) {ctx.globalAlpha = element.style.opacity;}

    switch (element.type) {case CanvasElementType.LINE:
      ...
      case CanvasElementType.STICKY_NOTE:
        ...
        words.forEach((word) => {const testLine = line + word + ' ';
  ...
  const drawCursor = useCallback(
    (ctx: CanvasRenderingContext2D, participant: WhiteboardParticipant) => {ctx.save();
  ...
  // Draw selection box
  const drawSelection = useCallback((ctx: CanvasRenderingContext2D, element: CanvasElement) => {ctx.save();
    ...
    const handles = [
      ...
      {x: element.position.x + element.size.width + 5,
        y: element.position.y + element.size.height + 5,},
    ];

    ctx.fillStyle = '#0066cc';
    ctx.setLineDash([]);

    handles.forEach((handle) => {ctx.fillRect(handle.x - 3, handle.y - 3, 6, 6);});
  ...
  const drawShapePreview = useCallback(
    (ctx: CanvasRenderingContext2D, start: Point, end: Point) => {ctx.save();
  ...
  return (<div className={`relative ${className}`}><canvas
        ref={canvasRef}
        ...
        style={{ pointerEvents: 'all'}} />{/* Toolbar could be added here */}<div className='absolute top-2 right-2 flex items-center gap-2 bg-white/90 backdrop-blur-sm rounded-lg p-2 shadow-sm'><input
          type='color'
          value={currentColor}
