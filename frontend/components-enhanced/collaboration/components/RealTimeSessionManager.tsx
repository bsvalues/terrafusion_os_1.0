...
interface RealTimeSessionManagerProps extends CollaborationComponentProps {
...
export const RealTimeSessionManager: React.FC<RealTimeSessionManagerProps> = ({
  className = '',
  ...
  const startSessionMutation = useMutation({
    mutationFn: ({ projectId, type }: { projectId: string; type: SessionType }) =>
      collaborationService.startSession(projectId, type),
  ...
  useEffect(() => {
    ...
    const handleParticipantJoined = (data: { sessionId: string; participant: SessionParticipant }) => {
    ...
    const handleParticipantLeft = (data: { sessionId: string; userId: string }) => {
      if (data.sessionId === session.id) {
        setSession(prev => prev ? {
          ...prev,
    ...
    const handleScreenShareStarted = (data: { sessionId: string; userId: string }) => {
      if (data.sessionId === session.id) {
        toast({
    ...
    const handleScreenShareEnded = (data: { sessionId: string; userId: string }) => {
      if (data.sessionId === session.id) {
        toast({
  ...
  const getSessionStatusBadgeVariant = (status: SessionStatus) => {
    switch (status) {
  ...
  const getParticipantRoleIcon = (role: SessionRole) => {
    switch (role) {
