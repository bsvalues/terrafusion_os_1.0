...
export const AIAnalyticsDashboard: React.FC = () => {
  const { data: analyticsData, loading, error, refetch } = useApi<AIAnalyticsData>('aianalytics/data');
  ...
  if (error) {
    return (
      <Box display="flex" justifyContent="center" alignItems="center" height="400px">
        <Alert severity="error">Failed to load AI Analytics Data: {error}</Alert>
      </Box>
    );
  }
