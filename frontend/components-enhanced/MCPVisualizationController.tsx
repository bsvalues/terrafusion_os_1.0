...
interface MCPSchema {type: string;
  properties: Record<string, any>;
  required?: string[];}

interface MCPFunction {name: string;
  description: string;
  execute: (input: any) => Promise<any>;}

interface MCPWorkflowStep {name: string;
  execute: (input: any, state: Map<string, any>) => Promise<any>;}

interface MCPWorkflow {name: string;
  steps: MCPWorkflowStep[];
  execute: (input: any) => Promise<any>;}

// Visualization input schema
const visualizationInputSchema: MCPSchema = {type: 'object',
  properties: {
...
export interface VisualizationFilters {buildingType: string;
  regions: string[];
  counties: string[];
  states: string[];
  startYear: number;
...
const defaultFilters: VisualizationFilters = {buildingType: 'residential',
  regions: [],
  counties: [],
  states: [],
  startYear: 2020,
...
interface MCPVisualizationContextType {filters: VisualizationFilters;
...
const perceptionStep: MCPWorkflowStep = {name: 'perception',
...
const reasoningStep: MCPWorkflowStep = {name: 'reasoning',
  execute: async (input, state) => {
    ...
    if (input.buildingType) {queryKeys.push('hierarchicalCosts');
...
const actionStep: MCPWorkflowStep = {name: 'action',
  execute: async (input, state) => {
    ...
    if (input.regions?.length > 0) {input.regions.forEach((region: string) => params.append('regions', region));}

    if (input.counties?.length > 0) {input.counties.forEach((county: string) => params.append('counties', county));}

    if (input.states?.length > 0) {input.states.forEach((state: string) => params.append('states', state));}

    if (input.startYear) {params.append('startYear', input.startYear.toString());}

    if (input.endYear) {params.append('endYear', input.endYear.toString());}
...
const visualizationWorkflow: MCPWorkflow = {name: 'visualizationWorkflow',
...
export function MCPVisualizationProvider({children}: {children: ReactNode}) {const [filters, setFiltersState] = useState<VisualizationFilters>(defaultFilters);
  ...
  useEffect(() => {
    const processFilters = async () => {
      ...
      try {
        ...
        setWorkflow(result);} catch (error) {console.error('Error in visualization workflow:', error);} finally {setIsProcessing(false);}
    };
  ...
  // Update filters
  const setFilters = (newFilters: Partial<VisualizationFilters>) =>{setFiltersState((prev) => ({
      ...prev,
      ...newFilters,}));
  };

  // Queries using React Query
  const regionalCostsQuery = useQuery({queryKey: [
      'regionalCosts',
    ...
    queryFn: async () => {
      ...
      if (filters.counties.length > 0) {filters.counties.forEach((county) => params.append('counties', county));}

      if (filters.states.length > 0) {filters.states.forEach((state) => params.append('states', state));}

      const response = await fetch(`/api/analytics/regional-costs?${params.toString()}`);
      if (!response.ok) {throw new Error('Failed to fetch regional costs data');}
  ...
  const hierarchicalCostsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!response.ok) {throw new Error('Failed to fetch hierarchical costs data');}
      return response.json();
    },
  ...
  const statisticalDataQuery = useQuery({
    ...
    queryFn: async () => {
      if (!workflow?.action?.endpoints?.includes('/api/analytics/statistical-correlations'))
        return null;
      ...
      if (!response.ok) {throw new Error('Failed to fetch statistical data');}
      return response.json();
    },
  ...
  const contextValue: MCPVisualizationContextType = {filters,
    setFilters,
    regionalCostsQuery,
  ...
  return (<MCPVisualizationContext.Provider value={contextValue}>{children}</MCPVisualizationContext.Provider>);
}

// Hook for using the visualization context
export function useMCPVisualization() {const context = useContext(MCPVisualizationContext);
...
export function MCPVisualizationController({children}: {children: ReactNode}) {
  return<MCPVisualizationProvider>{children}</MCPVisualizationProvider>;
}
