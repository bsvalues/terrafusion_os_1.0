...
interface ModuleRegistryContextType {modules: TerraFusionModule[];
...
export function ModuleRegistryProvider({children}: {children: ReactNode}) {const [modules, setModules] = useState<TerraFusionModule[]>(TERRAFUSION_MODULES);

  const activeModules = modules.filter(
    (module) =>module.config.enabled && module.status === 'active'
  );
  ...
  const toggleModule = (id: string) => {setModules((prev) =>
      prev.map((module) =>
        module.id === id
          ? { ...module, config: { ...module.config, enabled: !module.config.enabled} }
          : module
      )
    );
  };

  const isModuleEnabled = (id: string) => {const module = getModule(id);
    return module?.config.enabled && module?.status === 'active';};

  const getModulePermissions = (id: string) => {const module = getModule(id);
    return module?.permissions || [];};

  return (<ModuleRegistryContext.Provider
      value={{
        modules,
        activeModules,
        getModule,
    ...
    >{children}</ModuleRegistryContext.Provider>
  );
}

export function useModuleRegistry() {const context = useContext(ModuleRegistryContext);
