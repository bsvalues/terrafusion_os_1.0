...
export function CountyGrid() {const [filter, setFilter] = useState("all")
  ...
  return (<section className="py-16 px-4"><div className="max-w-7xl mx-auto"><div className="flex flex-wrap gap-4 justify-center mb-12">{[
            ...
            >
              {label}</Button>))}</div><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">{filteredCounties.map((county /* , index */) => (<div
              key={index}
            ...
            ><div className="flex justify-between items-start mb-4"><><h3 className="text-xl font-bold text-white">{county.name}</h3><Badge
</>
className={`bg-gradient-to-r ${county.color} text-white`}>{county.priority}</Badge></div><div className="space-y-2 mb-4 text-sm"><div className="flex justify-between"><><span className="text-white/60">Population</span><span
</>
className="text-white">{county.population}</span></div><div className="flex justify-between"><><span className="text-white/60">Parcels</span><span
</>
className="text-white">{county.parcels}</span></div><div className="flex justify-between"><><span className="text-white/60">Budget</span><span
</>
className="text-white">{county.budget}</span></div></div><div className="mb-4"><div className="flex justify-between items-center mb-2"><><span className="text-sm text-white/60">Integration Score</span><span
</>
className="text-sm font-semibold text-transcend">{county.integrationScore}%</span></div><><Progress value={county.integrationScore} className="h-2" /></div><div
</>
className="bg-black/30 rounded-lg p-3 mb-4"><><div className="text-sm font-semibold text-transcend mb-1">{county.message}</div><div
</>
className="text-xs text-white/70 italic">"{county.hook}"</div></div><div className="flex justify-between text-xs text-white/60 mb-4"><><span>Savings: {county.savings}</span><span
</></>>Timeline: {county.timeline}</span></div><div className="flex gap-2"><><Button
                  size="sm"
