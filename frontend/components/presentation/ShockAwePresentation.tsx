...
interface ShockAweConfig {mode: 'jobs' | 'musk' | 'tesla' | 'ives' | 'belichick';
  audience: string[];
  objectives: string[];
  performance: {
    speed: string;
...
const SHOCK_AWE_CONFIG: ShockAweConfig = {mode: 'jobs',
  audience: [
    'Board Members',
    'Strategic Partners',
    'Government CIOs',
...
export const ShockAwePresentation: React.FC = () => {const [currentSlide, setCurrentSlide] = useState(0);
  ...
  const modeStyles = {jobs: {
      background: 'linear-gradient(135deg, #000000 0%, #1a1a1a 100%)',
      accent: '#007AFF',
      font: 'SF Pro Display, -apple-system, sans-serif',},
    musk: {background: 'linear-gradient(135deg, #0a0a0a 0%, #1a0a0a 100%)',
      accent: '#FF6B35',
      font: 'Inter, sans-serif',},
    tesla: {background: 'linear-gradient(135deg, #000000 0%, #CC0000 100%)',
      accent: '#FFFFFF',
      font: 'Tesla, sans-serif',},
    ives: {background: 'linear-gradient(135deg, #F5F5F7 0%, #FFFFFF 100%)',
      accent: '#1D1D1F',
      font: 'SF Pro Display, sans-serif',},
    belichick: {background: 'linear-gradient(135deg, #002244 0%, #C60C30 100%)',
      accent: '#FFFFFF',
  ...
  useEffect(() =>{const timer = setInterval(() => {
      setCurrentSlide((prev) => (prev + 1) % slides.length);}, 4000);
  ...
  return (<div
    ...
    >{/* Mode Selector */}<div className='absolute top-4 right-4 flex gap-2 z-10'>{Object.keys(modeStyles).map((modeKey) => (<button
            key={modeKey}
          ...
          >
            {modeKey.toUpperCase()}</button>))}</div>{/* Main Content */}<AnimatePresence mode='wait'><motion.div
          key={currentSlide}
        ...
        ><motion.h1
          ...
          >{slides[currentSlide].title}</motion.h1><motion.h2
            className='text-4xl font-light mb-8 opacity-90'
            initial={{ opacity: 0}}
            animate={{ opacity: 0.9}}
            transition={{ duration: 0.6, delay: 0.4}}
          >{slides[currentSlide].subtitle}</motion.h2><motion.p
            className='text-2xl font-medium opacity-80 max-w-2xl mx-auto leading-relaxed'
            initial={{ opacity: 0}}
            animate={{ opacity: 0.8}}
            transition={{ duration: 0.6, delay: 0.6}}
          >{slides[currentSlide].content}</motion.p></motion.div></AnimatePresence>{/* Progress Indicators */}<div className='absolute bottom-8 flex gap-3'>{slides.map((_ /* , index */) => (<button
            key={index}
            onClick={() =>setCurrentSlide(index)}
            className={`w-3 h-3 rounded-full transition-all ${
              index === currentSlide ? 'bg-white' : 'bg-white/30 hover:bg-white/50'}`}
          />
        ))}</div>{/* Floating Metrics */}<div className='absolute bottom-20 left-8 text-left'><div className='text-sm opacity-60 mb-1'>PERFORMANCE MULTIPLIER</div><div className='text-2xl font-bold' style={{ color: currentStyle.accent}}>379,000,000×</div></div><div className='absolute bottom-20 right-8 text-right'><div className='text-sm opacity-60 mb-1'>MARKET POTENTIAL</div><div className='text-2xl font-bold' style={{ color: currentStyle.accent}}>$1B ARR</div></div></div>
  );
};
