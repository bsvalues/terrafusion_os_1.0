...
try {
  ...
  const { ProjectStatus, UserRole, hasPermission, formatCurrency, TERRAFUSION_SHARED_VERSION } =
    shared;
  ...
  console.log(
    ...
    ProjectStatus ? Object.keys(ProjectStatus).slice(0, 3) : 'undefined'
  );
  console.log(
    '   UserRole:',
    typeof UserRole,
    UserRole ? Object.keys(UserRole).slice(0, 3) : 'undefined'
  );
