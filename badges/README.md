...
The badges directory contains SVG quality indicators that provide immediate visual feedback on code coverage, quality metrics, performance budgets, and government compliance status for Terrafusion OS.
...
- **coverage-branches.svg** - Branch coverage percentage with government thresholds
- **coverage-functions.svg** - Function coverage tracking for critical systems
- **coverage-lines.svg** - Line-by-line coverage analysis
- **coverage-statements.svg** - Statement execution coverage for audit compliance
...
![Coverage](./badges/coverage-summary.svg)
...
```bash
# Badge system health
curl -s http://localhost:5000/health/badges
