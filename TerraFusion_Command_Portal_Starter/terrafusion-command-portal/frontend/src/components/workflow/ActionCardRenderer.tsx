...
const Badge = ({ children, variant = 'default', className = '' }: BadgeProps) => {
  ...
  return (
    <div className={`inline-flex items-center rounded-md px-2 py-1 text-xs font-medium ${variants[variant]} ${className}`}>
      {children}
    </div>
  )
}
...
const Input = ({ className = '', ...props }: React.InputHTMLAttributes<HTMLInputElement>) => (
  <input 
    className={`flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-900 disabled:cursor-not-allowed disabled:opacity-50 ${className}`}
    {...props}
  />
)

const Label = ({ children, className = '', ...props }: React.LabelHTMLAttributes<HTMLLabelElement> & { children: React.ReactNode }) => (
...
const Textarea = ({ className = '', ...props }: React.TextareaHTMLAttributes<HTMLTextAreaElement>) => (
  <textarea 
    className={`flex min-h-20 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-900 disabled:cursor-not-allowed disabled:opacity-50 ${className}`}
    {...props}
  />
)
...
const Select = ({ children, onValueChange, value }: SelectProps) => {
  ...
  return (
    <div className="relative">
      {React.isValidElement(children) ? React.cloneElement(children as any, { isOpen, setIsOpen, onValueChange, value }) : children}
    </div>
  )
}

const SelectTrigger = ({ children, isOpen, setIsOpen }: { children: React.ReactNode; isOpen?: boolean; setIsOpen?: (open: boolean) => void }) => (
  <button 
    className="flex h-10 w-full items-center justify-between rounded-md border border-slate-200 bg-white px-3 py-2 text-sm placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-900 disabled:cursor-not-allowed disabled:opacity-50"
...
const Checkbox = ({ checked, onCheckedChange, className = '' }: { 
  ...
  <input
    ...
    className={`h-4 w-4 rounded border-slate-200 ${className}`}
  />
)
...
const ActionCardRenderer: React.FC<ActionCardRendererProps> = ({
  ...
  // Type-safe value conversion for form inputs
  const getStringValue = (value: string | boolean | string[]): string => {
