...
class AgentAuthenticator extends EventEmitter {
  ...
  async authenticateAgent(agentId, signature, challenge, metadata = {}) {
    ...
    try {
      ...
      if (anomalyScore > this.config.anomalyThreshold) {
        ...
        return this.createAuthResult(false, 'anomaly_detected', authId, startTime, {
          anomalyScore,
        });
      }
  ...
  async checkRateLimit(agentId) {
    ...
    if (attemptsLastMinute >= this.config.maxAttemptsPerMinute) {
      return {
        ...
        retryAfter:
          minute - (now - Math.max(...agentData.attempts.slice(-this.config.maxAttemptsPerMinute))),
      };
    }
  ...
  async verifyAgentCertificate(agentId) {
    ...
    const now = Date.now();
    return (
      certificate.validFrom <= now && certificate.validTo >= now && certificate.status === 'active'
    );
  }
