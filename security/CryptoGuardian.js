...
class CryptoGuardian extends EventEmitter {
    ...
    async validateSignature(message, signature, publicKeyPem, agentId, metadata = {}) {
        ...
        try {
            ...
            if (validations.anomalyScore > 0.8) {
                await this.triggerSecurityAlert('high_anomaly_score', { agentId, score: validations.anomalyScore });
            ...
            if (this.config.consensusEnabled) {
                validations.consensus = await this.getConsensusVerification(message, signature, publicKeyPem);
            }

            validations.executionTime = Date.now() - startTime;
            validations.success = validations.verification && (this.config.consensusEnabled ? validations.consensus : true);
    ...
    async detectAnomalies(agentId, signature, message) {
        ...
        if (recentValidations.length > 100) { // Suspiciously high rate
            anomalyScore += 0.4;
        }
    ...
    handleCryptoFailure(error, agentId) {
        ...
        let state = this.circuitBreakerState.get(agentId) || { failures: 0, lastFailure: 0, isOpen: false };
    ...
    logValidation(validation, status) {
        const logEntry = {
            ...validation,
            status,
            messageHash: validation.message ? crypto.createHash('sha256').update(validation.message).digest('hex') : null,
            signatureHash: validation.signature ? crypto.createHash('sha256').update(validation.signature).digest('hex') : null
        };
    ...
    getHealthMetrics() {
        ...
        return {
            ...
            circuitBreakersOpen: Array.from(this.circuitBreakerState.values()).filter(s => s.isOpen).length,
            avgResponseTime: recentLogs.reduce((sum, log) => sum + (log.executionTime || 0), 0) / totalCount,
            timestamp: now
        };
    }
}
