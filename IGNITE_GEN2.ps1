...
if (Test-Path "backend/api/deno.json") {
    Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd backend/api; deno task dev"
} else {
    Write-Host "Warning: backend/api/deno.json not found. Skipping API start."
}
