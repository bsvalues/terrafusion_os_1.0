...
/**
 * 🧪 GOVERNMENT-CORE API Test Suite
...
describe('Government Core API Tests', () => {
  let authToken;
  let testClient;
  ...
  beforeAll(async () => {
    ...
    authToken = await generateTestToken({
      ...
      scopes: ['government-core:read', 'government-core:write'],
  ...
  describe('🏥 Health & Monitoring', () => {
    ...
    test('should provide metrics endpoint', async () => {
      const response = await testClient
        .get('/api/v1/government-core/metrics')
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200);
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /health', async () => {
      const response = await testClient
        .get('/api/v1/government-core/health')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'government-core');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /data', async () => {
      const response = await testClient
        .get('/api/v1/government-core/data')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'government-core');
    ...
    test('should handle POST /operations', async () => {
      const response = await testClient
        .post('/api/v1/government-core/operations')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'government-core');
    ...
    test('should handle GET /status', async () => {
      const response = await testClient
        .get('/api/v1/government-core/status')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'government-core');
  ...
  describe('🛡️ Security Tests', () => {
    ...
    test('should reject malformed requests', async () => {
      await testClient
        .post('/api/v1/government-core/data')
        .set('Authorization', `Bearer ${authToken}`)
        .send('{invalid json}')
        .expect(400);
    });
  ...
  describe('♿ Accessibility & Compliance', () => {
    test('should include accessibility metadata', async () => {
      const response = await testClient
        .get('/api/v1/government-core/docs')
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200);
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/government-core/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'government-core-api-tests',
  security_level: 'MEDIUM',
  endpoints_tested: 6,
