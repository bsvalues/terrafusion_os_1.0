...
describe('Terra Bank API Tests', () => {
  ...
  beforeAll(async () => {
    ...
    authToken = await generateTestToken({
      ...
      scopes: ['terra-bank:read', 'terra-bank:write'],
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /health', async () => {
      const response = await testClient
        .get('/api/v1/terra-bank/health')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'terra-bank');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /data', async () => {
      const response = await testClient
        .get('/api/v1/terra-bank/data')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'terra-bank');
    ...
    test('should handle POST /operations', async () => {
      const response = await testClient
        .post('/api/v1/terra-bank/operations')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'terra-bank');
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/terra-bank/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'terra-bank-api-tests',
  security_level: 'MEDIUM',
  endpoints_tested: 6,
