...
describe('Shock And Awe API Tests', () => {
  let authToken;
  let testClient;
  ...
  beforeAll(async () => {
    ...
    authToken = await generateTestToken({
      ...
      scopes: ['shock-and-awe:read', 'shock-and-awe:write'],
  ...
  describe('🏥 Health & Monitoring', () => {
    ...
    test('should provide metrics endpoint', async () => {
      const response = await testClient
        .get('/api/v1/shock-and-awe/metrics')
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200);
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /health', async () => {
      const response = await testClient
        .get('/api/v1/shock-and-awe/health')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'shock-and-awe');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /data', async () => {
      const response = await testClient
        .get('/api/v1/shock-and-awe/data')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'shock-and-awe');
    ...
    test('should handle POST /operations', async () => {
      const response = await testClient
        .post('/api/v1/shock-and-awe/operations')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'shock-and-awe');
    ...
    test('should handle GET /status', async () => {
      const response = await testClient
        .get('/api/v1/shock-and-awe/status')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'shock-and-awe');
  ...
  describe('🛡️ Security Tests', () => {
    ...
    test('should reject malformed requests', async () => {
      await testClient
        .post('/api/v1/shock-and-awe/data')
        .set('Authorization', `Bearer ${authToken}`)
        .send('{invalid json}')
        .expect(400);
    });
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/shock-and-awe/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'shock-and-awe-api-tests',
  security_level: 'MEDIUM',
  endpoints_tested: 6,
