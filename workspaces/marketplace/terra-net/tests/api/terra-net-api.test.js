...
describe('Terra Net API Tests', () => {
  ...
  beforeAll(async () => {
    ...
    authToken = await generateTestToken({
      workspace: 'terra-net',
      scopes: ['terra-net:read', 'terra-net:write'],
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /health', async () => {
      const response = await testClient
        .get('/api/v1/terra-net/health')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'terra-net');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /data', async () => {
      const response = await testClient
        .get('/api/v1/terra-net/data')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'terra-net');
    ...
    test('should handle POST /operations', async () => {
      const response = await testClient
        .post('/api/v1/terra-net/operations')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'terra-net');
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/terra-net/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'terra-net-api-tests',
  security_level: 'MEDIUM',
  endpoints_tested: 6,
