CREATE VIEW [CO\FGP].ParcelUpdates_SmartGov AS select distinct
a.first_name 											as 'owner_first name',
a.last_name 											as 'owner_last name',
pv.prop_id 												as 'prop_id',
p.geo_id 												as 'geo_id',
pv.legal_desc 											as 'legal_desc',
concat(rtrim(ltrim(ad.addr_line1))+
' ',rtrim(ltrim(ad.addr_line2))+' ',
rtrim(ltrim(ad.addr_line3))) as 'owner address',
rtrim(ltrim(ad.addr_city)) as 'owner city',
rtrim(ltrim(ad.addr_state)) as 'owner state',
rtrim(ltrim(ad.addr_zip)) as 'owner zip',
concat(rtrim(ltrim(s.situs_num))+' ',
rtrim(ltrim(s.situs_street_prefx))+' ',
rtrim(ltrim(s.situs_street))) as 'situs address',
rtrim(ltrim(s.situs_city)) as 'situs city',
rtrim(ltrim(s.situs_state)) as 'situs state',
rtrim(ltrim(s.situs_zip)) as 'situs zip', 								
