...
create view property_use_val as 
...
sp.CENTROID_X,sp.CENTROID_Y,sp.XCoord, sp.YCoord, shape, Geometry
...
left join
(select * from Benton_spatial_data.dbo.all_spatial_coords)as sp on sp.Prop_ID=property_val.prop_id
where prop_val_yr = (select appr_yr from pacs_oltp.dbo.pacs_system) - 1
--and property_val.prop_id=309698
--ORDER BY prop_id
