...
by the FeatureclassConversion.py, TableConversion.py, and RasterConversion.py
modules.
...
def GenerateOutputName(inName, outWorkspace, beforeVSafter="before", uniqueName=True):
    try:
        ...
        if hasattr(dsc, "namestring"):
            # use os.path.split to account for layer within group layers
            outName = os.path.split(dsc.namestring)[-1]
        ...
        if inContainer:
            ...
            inContainerName = os.path.basename(
                inContainer)  # Full path excluding the featureclass. D:\Data\redlands.mdb\city
            ...
            # If the input is a shapefile, txt or dbf,
            # do not include the extention (".shp", ".dbf", ".txt") as part of the output name
            # Do this every time since the output could may or may not be a geodatabase feature class or table
            elif dsc.DataType in ["DbaseTable", "ShapeFile", "TextFile"]:
                outName = os.path.splitext(outName)[0]

        # The next 3 steps (get rid of invalid chars, add extention
        # (if the output will have one), and generate a unique name)
...
def GenerateRasterName(inName, outWorkspace, ext):
    try:
        ...
        """ This part determines the output dataset base name based on the input"""
        ...
        if inContainer:
            ...
            # Check if input raster is raster catalog item
            if des.DataType == "RasterCatalog":  # rastercatalog
                outName = os.path.basename(inContainer)  # use raster catalog name as basename
            # Check if input raster is mosaic dataset item
            elif des.DataType == "MosaicDataset":
                outName = os.path.basename(inContainer)  # use mosaic dataset name as basename
            elif des.WorkspaceType == "FileSystem":  # file with extension
                ids = outName.rfind(".")
                if (ids > -1):
                    outName = outName[:ids]
            elif des.WorkspaceType == "RemoteDatabase":
                # for ArcSDE
                outName.replace(":", ".")
                ids = outName.rfind(".")
                if ids > -1:
                    outName = outName[(ids + 1):]
            # All remaining cases uses the base name of the input raster
            # Including: FGDB/MDB/SQLite raster dataset/mosaic dataset, layers
            else:
                outName = os.path.basename(inName)
        else:
            outName = os.path.basename(inName)

        """ This part determines the output dataset name based on output workspace"""
        ...
        raise Exception("%s (%s)" % (msgErrorGenOutputRaster, err))
...
def GenerateUniqueName(name, workspace):
    ...
    extlist = (
        '.shp', '.dbf', '.img', '.tif', '.bmp', '.jpg', '.jp2', '.gif', '.png', '.crf', '.mrf', '.bil', '.bsq', '.bip',
        '.dat')
    if Exists(workspace, name):
        extension = ''
        if os.path.splitext(name)[1].lower() in extlist:
...
def CopyRasters(inRasters, outRasters, keyword, format=""):
    try:
        gp.CopyRaster_management(inRasters, outRasters, keyword, format=format)
    except:
        raise Exception(gp.GetMessages(2).replace("\n", " "))
