...
def checkNumberPolygons(numObs, fromAGOL=False):
    if numObs < minNumFeatures:
        LOGGER.error(1535, extra={"message_ID": 1535, "add_argument1": str(minNumFeatures)})
        raise SystemExit()
...
def adjustCopyFeatureFieldName_AGOL(fc, fieldName):
    ...
    for f in ARCPY.Describe(fc).fields:
        if f.name.upper() == fnu:
            return fieldName
        elif f.name.upper() == fnMap[fnu]:
            return fnMap[fnu]

    LOGGER.debug("Field name lost while copying features, error found!")
    return fieldName

def runOOA(inputLayer, outputFC, varName, aggMethod,
           boundaryFC, polygonFC, permutationsOption,
           cellSize, distanceBand, normField,
           geoenrichMethod):
    """
    Call the Optimized Outlier Analysis from AGOL
    :param inputLayer:
    :param outputFC:
    :param varName:
    :param aggMethod:
    :param boundaryFC:
    :param polygonFC:
    :param permutationsOption:
    :param cellSize:
    :param distanceBand:
    :param normField:
    :param geoenrichMethod:
    :return:
    """
    ...
    if cellSize:
        csp = cellSize.split()
        userCellSize = float(csp[0])
        userCellUnit = csp[1].upper()
    ...
    if distanceBand:
        csp = distanceBand.split()
        userBandSize = float(csp[0])
        userBandUnit = csp[1].upper()
    ...
    oa = OptimizedOutlier(ssdo, outputFC, varName=varName,
                          ...
                          normField=normField, geoenrichMethod=geoenrichMethod, fromAGOL=True)

    DM.Delete(featureLayer)
    return oa.agol_messages
...
class OptimizedOutlier(object):
    ...
    def printOOALocationalOutliers(self, outliers, aggType=1):
        ...
        if self.FLAG_AGOL:
            if numOutliers:
                ...
                self.printOOABullet_AGOL(84437)
        else:
            self.printOOASubject(84438, addNewLine=False)
            if numOutliers:
                if numOutliers == 1:
    ...
    def printOOASubject(self, messID, addNewLine=True):
    ...
    def printOOAAnswer(self, messStr, addNewLine=True, useIndentation = True):
        """Prints Subject Message Header to Results Window."""
        if UTILS.couldExportHTMLMessage():
    ...
    def knnDecision(self, ssdo):
        ...
        if not self.FLAG_AGOL:
            self.printOOASubject(84463, addNewLine=False)
    ...
    def checkBoundary(self, projection = None):
        ...
        if not self.FLAG_AGOL:
            self.printOOASubject(84486, addNewLine = False)
    ...
    def doPoint2Poly(self):
        ...
        if self.FLAG_AGOL:
            self.printOOATitle_AGOL(84428)
        else:
            self.printOOASection(84428, prependNewLine=True)
            self.printOOASubject(84431, addNewLine=False)
        ...
        if not self.FLAG_AGOL:
            self.printOOASubject(84430, addNewLine=False)
    ...
    def doIntegrate(self):
        ...
        self.printOOALocationalOutliers(outliers, aggType = self.aggType)

        #### Agg Header ####
        if self.FLAG_AGOL:
            self.printOOATitle_AGOL(84444)
        else:
            self.printOOASection(84444)
        ...
        #### Snap Subject ####
        if not self.FLAG_AGOL:
            self.printOOASubject(84442, addNewLine = False)
    ...
    def doGrid(self):
        ...
        #### Fish Subject ####
        if self.FLAG_AGOL:
            self.printOOASubject(creatingMSG, addNewLine = False)
        ...
        if not self.FLAG_AGOL:
            ...
            self.printOOASubject(countMSG, addNewLine = False)
        ...
        if not self.FLAG_AGOL:
            self.printOOAAnswer(ARCPY.GetIDMessage(countMSGNumber))
    ...
    def createAnalysisSSDO(self, tempFC, varName):
        ...
        if self.aggType == 2:
            ...
            if self.FLAG_AGOL:
                ...
                self.printOOABullet_AGOL(84489)

            else:
                self.printOOASection(84444)
                #### Do Spatial Join ####
                msg = ARCPY.GetIDMessage(84458)
                self.printOOASubject(84458, addNewLine = False)
        ...
        ARCPY.SetProgressor("default", msg)
        if not self.FLAG_AGOL:
            self.printOOASubject(msgID, addNewLine = False)
        ...
        if self.FLAG_AGOL:
            ...
            self.printOOAAnswer(msg, addNewLine=False)
    ...
    def setAnalysisSSDO(self, tempFC, varName):
        ...
        if not self.FLAG_AGOL:
            self.printOOASubject(84429, addNewLine = False)
        if self.sensitivity:
            sensField = []
        ...
        if self.FLAG_AGOL:
            ...
            self.printOOAAnswer(msg)
            #### Errors and Warnings ####
            self.printOOASubject(84432, addNewLine=False)
        ...
        lo = UTILS.LocationInfo(self.analysisSSDO)
        cellSize, threshold, meanDist, outliers = lo.getNearestNeighborInfo()
        self.printOOALocationalOutliers(outliers, aggType = 2)

    def calculateClusterOutlier(self, yData = None):
        #### modify yData for sensitivity analysis ####
        if yData is not None:
            self.analysisSSDO.fields[self.varName].data = yData
        #### Scale Header ####
        if self.FLAG_AGOL:
            self.printOOATitle_AGOL(84459)
        else:
            self.printOOASection(84459)
        ...
        if not self.bandSize2Use:
            ...
            ARCPY.SetProgressor("default", msg)
            if not self.FLAG_AGOL:
                self.printOOASubject(84460, addNewLine = False)
            ...
            if not peakFound:
                ...
                if not self.FLAG_AGOL:
                    ...
                    self.printOOAAnswer(msg)
                distanceBand = self.knnDecision(self.analysisSSDO)
                distanceStr = self.ssdo.distanceInfo.printDistance(distanceBand)
                wType = 1
                numNeighs = 0
        ...
        #### Outlier  Header ####
        if self.FLAG_AGOL:
            self.printOOATitle_AGOL(84704)
        else:
            self.printOOASection(84704)
        ...
        li = LM.LocalI(self.analysisSSDO, self.varName, self.outputFC, wType,
                       ...
                       numNeighs = 1, thresholdOrigin = self.bandSizeOrigin,
                       normField=self.normField,
                       fromAGOL=self.FLAG_AGOL, sensitivity=self.sensitivity)
        
        if self.sensitivity:
            return li

        #### Report and Set Parameters ####
        liField, ziField, pvField, coField = li.outputResults(includeSSRateField=self.FLAG_AGOL)
        ...
        if self.varName == 'JOIN_COUNT' or self.varName == 'ICOUNT':
            ...
            if variable in self.ssdo.fields:
                varAlias = self.ssdo.fields[variable].alias
            else:
                varAlias = variable

        if yData is None:
            self.printResults(li, varAlias)

        #### Outlier  Header ####
        if self.FLAG_AGOL:
            self.printOOATitle_AGOL(84471)
        else:
            self.printOOASection(84471)

        if self.renderResults(self.analysisSSDO):
            self.printResultInfo(varAlias)

        #### Set Chart Output ####
        if UTILS.isPRO() and not self.FLAG_AGOL:
    ...
    def printResultInfo(self, varAlias):
        if self.FLAG_AGOL:
            self.returnJSON_FromMessageMatch_AGOL(84712, ["Variable"], [varAlias],
                                        rowStyle='<ul><li></li></ul>')
            self.returnJSON_FromMessageMatch_AGOL(84713, ["Variable"], [varAlias],
                                        rowStyle='<ul><li></li></ul>')
            self.returnJSON_FromMessageMatch_AGOL(84714, ["Variable"], [varAlias],
                                        rowStyle='<ul><li></li></ul>')
            self.returnJSON_FromMessageMatch_AGOL(84715, ["Variable"], [varAlias],
                                        rowStyle='<ul><li></li></ul><br/>')
        else:
            msg = ARCPY.GetIDMessage(84712).format(varAlias)
            self.printOOAAnswer(msg, addNewLine=False)
            msg = ARCPY.GetIDMessage(84713).format(varAlias)
            self.printOOAAnswer(msg, addNewLine=False)
            msg = ARCPY.GetIDMessage(84714).format(varAlias)
            self.printOOAAnswer(msg, addNewLine=False)
            msg = ARCPY.GetIDMessage(84715).format(varAlias)
            self.printOOAAnswer(msg, addNewLine = False)
    ...
    def printResults(self, LIObject, variable):
        ...
        if self.FLAG_AGOL:
            self.returnJSON_FromMessageMatch_AGOL(84705, ["NumPermutation"], [str(self.permutations)],
                                    rowStyle='<ul><li></li></ul>')
        else:
            msg = ARCPY.GetIDMessage(84705).format(str(self.permutations))
            self.printOOAAnswer(msg, addNewLine = False, useIndentation = False)
        ...
        #### FDR Significance ###
        if self.FLAG_AGOL:
            self.returnJSON_FromMessageMatch_AGOL(84470, ["NumSignificant"], [str(numFDR)],
                                    rowStyle='<ul><li></li></ul>')
        else:
            FDRmsg = ARCPY.GetIDMessage(84707).format(numFDR)
            self.printOOAAnswer(FDRmsg, addNewLine = False)
        ...
        if self.FLAG_AGOL:
            self.returnJSON_FromMessageMatch_AGOL(84709, ["NumHighOutliers"], [str(hl)],
                                             rowStyle='<ul><li></li></ul>')
            self.returnJSON_FromMessageMatch_AGOL(84708, ["NumLowOutliers"], [str(lh)],
                                             rowStyle='<ul><li></li></ul>')
            self.returnJSON_FromMessageMatch_AGOL(84710, ["NumLowClusters"], [str(ll)],
                                             rowStyle='<ul><li></li></ul>')
            self.returnJSON_FromMessageMatch_AGOL(84711, ["NumHighClusters"], [str(hh)],
                                             rowStyle='<ul><li></li></ul><br/>')
        else:
...
class OptOutlierSensitivity():
    def __init__(self, Input_Features, Output_Features, Analysis_Field,
                ...
                Performance_Adjustment, Cell_Size, Distance_Band,
        ...
        self.aggType = "COUNT_INCIDENTS_WITHIN_FISHNET_POLYGONS"
        ...
        ### Avoid Parsing ###
        if Performance_Adjustment not in [None, "", "#"]:
            if "199" in Performance_Adjustment:
                self.permutations = 199
            if "999" in Performance_Adjustment:
                self.permutations = 999
            if "499" in Performance_Adjustment:
                self.permutations = 499
        self.resultSSDO = resultSSDO
    ...
    def execute(self):
        ...
        if seed == 0:
            seed = int(NUM.random.randint(50000))
        ...
        ooa = OptimizedOutlier(ssdo, self.outputFC, self.varName, aggType=self.aggType,
                               sensitivity=sensitivity,parameters=self.parameters , permutations=self.permutations,
                               bandSize2Use = processingBandSize, bandSizeOrigin = bandSizeOrigin)

        #### Compare SSDO and Result SSDO ####
        if resultSSDO is not None:
            if resultSSDO.numObs == ooa.analysisSSDO.numObs:
                sourceIDsSSDO = list(ooa.analysisSSDO.master2Order.keys())
        ...
        yMeanNeighbors = None
        if sensitivity["simulation_method"] == "NEIGHBORHOOD":
            li = ooa.calculateClusterOutlier()
            li.initialize()
        ...
        meanValue = 0
        yDataOriginal = ssdo.fields[self.varName].data.copy()
        ...
        for sim in NUM.arange(numSimulations):
            ...
            yData = self.getDataRealization(ssdo, self.varName, sensitivity, seeds[sim], yMeanNeighbors, zFactor)
            ...
            li = ooa.calculateClusterOutlier(yData)
            ...
            if pathWS is not None:
                ...
                candidateFieldSim = SSDO.CandidateField(varNameOutput, "DOUBLE", yData, alias = ssdo.fields[self.varName.upper()].alias)
        ...
        if resultSSDO is not None:
            ...
            for masterID in ooa.analysisSSDO.master2Order:
                orderID = ooa.analysisSSDO.master2Order[masterID]
        ...
        li.renderResults(self.parameters,  simulationInformation = {"field": "COType", "heading": ARCPY.GetIDMessage(220973), "popup": SSU.createPopup(fr"//{breaksString}")})

        # #### Add Group Layer ####
        self.addGroupLayer(li, self.outputFC)
