...
def execute():
    ...
    try:
        ...
        if dscInput.DatasetType == "Table":
            ...
            must_have_list = [dscOutput.OIDFieldName.upper()]
        elif dscInput.DatasetType == "FeatureClass":
            # create a feature class
            # Message "Creating output feature class..."
            gp.AddMessage(gp.GetIDMessage(86181))
            gp.CreateFeatureClass_management(outPath, outName, dscInput.ShapeType, inTable, "SAME_AS_TEMPLATE",
                                             "SAME_AS_TEMPLATE", dscInput.SpatialReference)
            dscOutput = gp.describe(outTable)
            must_have_list = [dscOutput.OIDFieldName.upper(), dscOutput.ShapeFieldName.upper()]
        else:
