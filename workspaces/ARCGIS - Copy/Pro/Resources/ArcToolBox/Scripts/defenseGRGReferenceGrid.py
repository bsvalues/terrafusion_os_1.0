'''
 ...
 ==================================================
 description:
 Gridded Reference Graphic (GRG) Reference Grid Creation Utilities
 ==================================================
'''
...
class ReferenceGrid(object):
    ...
    def Build(self, out_features):
        ...
        def _buildHundredGrid(out_features, sq):
            ''' Build Hundred Meters Grid
            '''
            features = _createFC(out_features, "POLYGON", srWGS84)
        ...
        def checkPolarRegion(inputFeature):
            ...
            if(not outsideNorthPolar or not outsideSouthPolar):
                arcpy.AddIDMessage("WARNING", 200506) # The GRG extent is within a polar region. Cells that fall within the polar region will not be created.
        ...
        try:
            ...
            arcpy.AddMessage(arcpy.GetIDMessage(200502).format(str(self.gridSize))) # Grid Size: {}
            
            extent_polygon = arcpy.Describe(self.inputArea).extent.polygon
            
            # If input geographic convert to a SR with meters units 
            if extent_polygon.spatialReference.type == 'Geographic' or \
                extent_polygon.spatialReference.linearUnitName != 'Meter':
                extent_polygon = extent_polygon.projectAs(arcpy.SpatialReference(3857))

            extent_area = int(extent_polygon.area)

            arcpy.AddMessage(arcpy.GetIDMessage(200501).format(str(extent_area))) # Extent Area: {}

            # A large grid area would be 45 rows * 45 columns * (Size ^ 2)
            # Ex: for 100m grid - 45 * 45 * 100 * 100 = 20,250,000m 
            largeGridDetected = False
            gridSizeMeters = self.GRID_SIZE_LOOKUP[self.gridSize]
            maxGridArea = gridSizeMeters * gridSizeMeters * 45 * 45
            largeGridDetected = extent_area > maxGridArea
            
            if largeGridDetected:
                if self.allowLargeGrids is False:
                    arcpy.AddIDMessage("ERROR", 200508, str(extent_area), str(self.gridSize)) # Area {0} exceeds large grid value for {1}. Use a smaller Input Area or choose a larger Grid Size.
                    return None
                else:
                    arcpy.AddIDMessage("WARNING", 200507, str(extent_area), str(self.gridSize)) # Area {0} exceeds large grid value for {1}. Proceeding with grid construction. 

            resetXYResolution = False
            if arcpy.env.XYResolution is None:
                # WORKAROUND: this tool has a small amount of error, that in turn is creating small gaps
                # between the output polygons, setting this resolution minimizes the creation of these gaps 
                # TODO: we should expose XYResolution in the doc as an enviroment variable used by tool
                resetXYResolution = True
                arcpy.env.XYResolution = '0.1 meters'

            # Get the coordinates of the first polygon (a little more accurate than feature class extent)
            extent = None
            with arcpy.da.SearchCursor(self.inputArea, ['SHAPE@']) as rows:
                for row in rows:
                    shape = row[0]
                    poly = shape.projectAs(srWGS84)
                    extent = poly.extent
                    break

            AOIPoly = extent.polygon

            checkPolarRegion(AOIPoly)

            # create a feature class for the grid zones
            gridZonesName = arcpy.CreateUniqueName("gridZones", self.scratch)
            deleteme.append(gridZonesName)

            arcpy.AddMessage(arcpy.GetIDMessage(200503)) # Creating Grid zones/latitude bands...
            gridZones = _createFC(gridZonesName, "POLYGON", srWGS84)
            ...
            # create a utm zone feature class by dissolving the grid zones by their utm zone number
            utmZonesName = arcpy.CreateUniqueName("dissolve", self.scratch)
            deleteme.append(utmZonesName)

            utmZones = arcpy.Dissolve_management(gridZones, utmZonesName, ["utmZone"])

            # select the grid zones that intersect with the input extent
            gridZonesLayer = arcpy.MakeFeatureLayer_management(gridZones, "gridZones_lyr")
            Selection = arcpy.SelectLayerByLocation_management(gridZonesLayer, "INTERSECT", 
                                                               self.inputArea)
            ...
            arcpy.AddMessage(arcpy.GetIDMessage(200504)) # Creating 100k grid squares...
            sq = _processZonePolygons(Selection, AOIPoly)

            if self.gridSize == '100000M_GRID':
                return _buildHundredGrid(out_features, sq)
            else:
                sq100kName = arcpy.CreateUniqueName("sq100k", self.scratch)
                deleteme.append(sq100kName)
                sq100k = _buildHundredGrid(sq100kName, sq)

            # only if not 100K
            arcpy.AddMessage(arcpy.GetIDMessage(200505)) # Creating sub 100K grid squares...
            ...
            while currentValue >= testValue:
                ...
                sq = []
                sq = sq + polys
                if (testValue < 100) & (currentValue == 100): # Special case if grid size < 100 selected
                    currentValue = testValue
                else:
                    currentValue = currentValue / 10

            gridsTempFC = arcpy.CreateUniqueName("gridsOut", self.scratch)
            deleteme.append(gridsTempFC)
            
            _createFC(gridsTempFC, "POLYGON", srWGS84)
            arcpy.AddField_management(gridsTempFC, GRID_FIELD_NAME, "text")
            with arcpy.da.InsertCursor(gridsTempFC, ['SHAPE@', GRID_FIELD_NAME]) as cursor:
                for i in range(0,len(polys)):
                    cursor.insertRow([polys[i]['clippedPolygon'],
                                    polys[i]['text']])

            polysCount = len(polys)
            if (polysCount == 0):
                # This should not happen, but just in case
                arcpy.AddIDMessage("ERROR", 10151) # No features created by operation
            elif (polysCount <= 2):
                # If we have only 1 or 2 features, then it is probably because
                # the input extent is smaller than the grid size
                arcpy.AddIDMessage("WARNING", 964) # Specified extent is invalid
                selectionLayer = gridsTempFC
            else: # polysCount > 1
                # WORKAROUND: some of the generated grids are showing up outside the AOI, 
                # so we need to reselect from "within" the AOI
                gridsLayerName = "grids_lyr"
                gridsLayer = arcpy.MakeFeatureLayer_management(gridsTempFC, gridsLayerName)
                selectionLayer = arcpy.SelectLayerByLocation_management(in_layer=gridsLayer, 
                        overlap_type="WITHIN", select_features=self.inputArea)

            arcpy.CopyFeatures_management(selectionLayer, out_features)

            # Reset: arcpy.env.XYResolution
            if resetXYResolution:
                arcpy.env.XYResolution = None
...
def _NonPolarGridZone(args):
    ...
    polygon = arcpy.Polygon(arcpy.Array([arcpy.Point(*coords) for coords in feature_info]), srWGS84)
    
    npgz = {"id": gid, "utmZone": utmZone, "latitudeZone": latitudeZone, "polygon": polygon}
    ...
    for n in range(int(math.floor(minN / 100000) * 100000), int(math.ceil(maxN / 100000) * 100000), 100000):
        ...
        for e in range(int(math.floor(minE / 100000) * 100000), int(math.ceil(maxE / 100000) * 100000), 100000):
            ...
            for i in range(n, n + 100000, 25000):
                ...
                if i == n:
                    BL = arcpy.PointGeometry(arcpy.Point(pt['lon'], pt['lat']), srWGS84)
            ...
            BR = arcpy.PointGeometry(arcpy.Point(pt['lon'], pt['lat']), srWGS84)
            ...
            polygon = arcpy.Polygon(arcpy.Array([arcpy.Point(*coords) for coords in ring]), srWGS84)
...
def _handleGridSquares(poly, interval, AOI):
    ...
    for n in range(int(math.floor(minN / interval) * interval), int(maxN), int(interval)):        
        for e in range(int(math.floor(minE / interval) * interval), int(maxE), int(interval)):    
            ...
            clippedPolygon = None
            if not polygon.disjoint(clippedPoly): # Two geometries intersect if disjoint returns False.
            ...
            if not AOI.disjoint(polygon):
                ...
                # Based on interval, set the number of digits to keep/drop 
                # Ex. 1000, drop trailing '000'; 100, drop '00'; 50,25 don't drop any, TRICKY: 10 drop '0'
                if interval == 10:
                    digitsToKeep = 4
                else:
                    powersOfTen = int(math.log10(interval))
                    digitsToKeep = 5 - powersOfTen if powersOfTen >= 2 else 5

                # Get MGRS coordinate
                ptGeometry = arcpy.PointGeometry(clippedPolygon.centroid, srWGS84)
                mgrsCoord = ptGeometry.toCoordString('mgrs')
                gZDand100KSq = mgrsCoord[:5]
                eastingInt = int(mgrsCoord[5:10])
                northingInt = int(mgrsCoord[13:18])
                # Round to the nearest interval (ex. 1000, 100, 50, 25)
                eastingRounded = int(math.trunc(eastingInt / interval) * interval)
                northingRounded = int(math.trunc(northingInt / interval) * interval)
                labelEasting = str(eastingRounded).rjust(5, '0')[:digitsToKeep]
                labelNorthing = str(northingRounded).rjust(5, '0')[:digitsToKeep]
                separator = '' # debug if you want a separator between coords
                text = gZDand100KSq + separator + labelEasting + separator + labelNorthing
                ...
                # For debug
                COMPARE_TO_OLD_METHOD = False
                if COMPARE_TO_OLD_METHOD:
                    # Original Method of computing X, Y labels
                    xLabel = _padZero(e % 100000 / interval, 5 - math.log10(interval))
                    yLabel = _padZero(((10000000 + n) if minN < 0 else n) % 100000 / interval,5 - math.log10(interval))
                    textAltMethod = GZD + separator + xLabel + separator + yLabel
                    if ((interval != 50) and (interval != 25)) and (text != textAltMethod):
                        arcpy.AddWarning("Grid label coords don't match: " + text + " vs. " + textAltMethod)
...
def _lettersHelper(setId, row, col):
    '''
