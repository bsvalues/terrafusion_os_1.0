...
class ProgressCallback(Callback):
    ...
    def on_epoch_end(self, **kwargs):
        ...
        avg_precision = float(last_metrics[1])
    ...
    with open(trainParams['messageFile'], "r+b") as f:
        ...
        try:
            ...
            if Assign_GPU(trainParams['processorType'], trainParams['gpu_id']):
                ...
                transformArgs  = arcgis.learn.Transform3d(rotation=[2.5, 2.5, 180], scaling=5, jitter=0)
                prepareArgDict = {'dataset_type'   : 'PointCloudOD',
                ...
                if modelArgs['pretrained_model']:
                    pcod_model = arcgis.learn.MMDetection3D.from_model(modelArgs['pretrained_model'], 
                                                                       trainingData)
                else:
                    learnArgDict = {"model": modelArgs['model'],
                                    "voxel_parms": modelArgs['voxel_parms']}
                    pcod_model = arcgis.learn.MMDetection3D(trainingData, **learnArgDict)
                # Get fit arguments
                fitArgs = trainParams['fitArgs']
                learningRate = fitArgs['lr']
                if not learningRate:
                    write_to_memory_1b(mm, 8, 2)
                    learningRate = pcod_model.lr_find(allow_plot = False)
                    write_to_memory_real(mm, 192, learningRate)
                    write_to_memory_1b(mm, 200, 1)

                pcod_model.fit(epochs          = fitArgs['epochs'],
                               lr              = learningRate,
                               early_stopping  = fitArgs['early_stopping'],
                               checkpoint      = fitArgs['checkpoint'],
                               monitor         = fitArgs['monitor'],
                               one_cycle       = fitArgs['one_cycle'],
                               callbacks       = [ProgressCallback(pcod_model, fitArgs['epochs'], totalTrainingIterations, totalValidationIterations, epoch_stats_dict = epochStatsDict)]
                              )

                write_to_memory_32_safe(mm, 32, 0xFFFFFFFFFFFFFFFF)

                pcod_model.save(trainParams['outModel'], compute_metrics = False)
                ...
                with open(trainParams['epochStatsFile'], 'w', newline = '') as csvfile:
                    ...
                    stats_writer.writerow(['EPOCH', 'TRAINING_LOSS', 'VALIDATION_LOSS', 
                                            'AVERAGE_PRECISION', 'ELAPSED_TIME'])
                    # Write data rows
                    for epoch in sorted(epochStatsDict.keys()):
                        stats_writer.writerow([epoch,
                                               ...
                                               epochStatsDict[epoch]['AVERAGE_PRECISION'],
