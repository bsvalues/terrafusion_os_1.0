...
def returnJSON_FromMessageMatch_AGOL(agolMessages, msgID, listOfStringKeys, listOfStringVals,
                                     rowStyle='<span></span>',
                                     msg2Use=None):
...
def numClustersMessage(labels, agol_process_info=None):
    """
    ...
    if n:
        ...
        if agol_process_info is not None:
            returnJSON_FromMessageMatch_AGOL(agol_process_info, 84764, ["number"], [str(n)],
                                             rowStyle='<ul><li></li></ul>',
                                             msg2Use=f"{ARCPY.GetIDMessage(84764)}" + "{0}")
            returnJSON_FromMessageMatch_AGOL(agol_process_info, 84766, ["number"], [str(nNoise)],
                                             rowStyle='<ul><li></li></ul>',
                                             msg2Use=f"{ARCPY.GetIDMessage(84766)}" + "{0}")

    else:
        ARCPY.AddIDMessage("WARNING", 110142)
        returnJSON_FromMessageMatch_AGOL(agol_process_info, 110142, [], [],
                                         rowStyle='<ul><li></li></ul>')
...
def output(ssdo , outputFC, classification, colorData = None, layer = True, nCounts = None, checkNull = [], timeEnabled = None):
    ...
    if layer :
        ...
        return getLayer(uniques, nCounts, colorData, ssdo.hasZ, timeEnabled = timeEnabled)
...
class TimeEnabler(object):
    def __init__(self, ssdo, timeField, timeInterval, outputFC):
        ...
        #### Warn if time field is high precision ####
        if self.timeField is not None:
            self.ssdo.warnNotUsingHighPrecisionDates([self.timeField])
...
class Colors:
    ...
    def getColors(self):
        ...
        if len(uniqueCases) <= 8:
            ...
            ###@ Set Groups without color using the least common color ####
            noSet = groupsList[self.colorId == 0]
            if len(noSet)>0:
                for groupId in noSet:
                    cl = self.colorNeighbors(groupId)
                    options = self.distributionColors[NUM.argsort(self.distributionColors.T[1])][:,0]
                    nextC = 1
                    if len(cl):
                        if cl[0] != 0:
                            nextC = cl[0]
...
class DBSCAN(TimeEnabler):
    ...
    def __init__(self, inputFC, outputFC, minPoints, eps = None, seed = 0, leaf_size = 40, parallel = -1,
        ...
        self.ssdo = SSDO.SSDataObject(inputFC, ignoreDateHighPrecision=True)
        self.agol_process_info = []
        returnJSON_FromMessageMatch_AGOL(self.agol_process_info, 220647, ["minFeatures"],
                                         [str(minPoints)], rowStyle='<ul><li></li></ul>')
    ...
    def run(self):
        ...
        returnJSON_FromMessageMatch_AGOL(self.agol_process_info, 110140,
                                         ["searchDistance"], [self.ssdo.distanceInfo.printDistance(self.eps)],
                                         rowStyle='<ul><li></li></ul>',
                                         msg2Use=ARCPY.GetIDMessage(110140).replace("%1", "{0}"))
        ...
        data = ARC._ss.dbscan_kdtree(self.ssdo, 
                                        ...
                                        self.breakTimeSize)
        if data is None:
            LOGGER.error(426, extra={"message_ID": 426})
            raise SystemExit() 
    ...
    def output(self):
        ...
        numClustersMessage(self.out, agol_process_info=self.agol_process_info)
        sourceId = NUM.array(list(self.ssdo.master2Order.keys()))
        fieldDict = []
        if self.ssdo.hasOID64:
            fieldDict.append(["SOURCE_ID" , (NUM.asarray(sourceId, dtype = NUM.int64), "BIGINTEGER", ARCPY.GetIDMessage(220125))])
        else:
...
class OPTICS(TimeEnabler):
    ...
    def run(self):
        ...
        if calculateEps:
            ...
            agol_eps = self.ssdo.distanceInfo.printDistance(optic.eps)
        else:
            agol_eps = self.ssdo.distanceInfo.printDistance(self.eps)

        returnJSON_FromMessageMatch_AGOL(self.agol_process_info, 110140,
                                         ["searchDistance"], [agol_eps],
                                         rowStyle='<ul><li></li></ul>',
                                         msg2Use=ARCPY.GetIDMessage(110140).replace("%1", "{0}"))
        ...
        self.idClusters, self.colors =  zones.getClusters(self.orderedReach,
                                                          ...
                                                          showPlot = usePLT,
                                                          agol_process_info=self.agol_process_info)
    ...
    def output(self):
        ...
        #### Messages ####
        numClustersMessage(labels, agol_process_info=self.agol_process_info)
...
class DetectZones(object):
    ...
    def getIdClustersByZonesClass(self, reach, clusters, recOrder = None, pltMat = False):
        ...
        for i in NUM.arange(numClusters, dtype = NUM.int32):
    ...
    def getClusters(self, reach, orderValues, minNumFeatures , thresholdB = None, 
                    showReachInMapPlotLib = False, showPlot = False, agol_process_info = None):
        ...
        #### Return All Noise ####
        if not found:
            if agol_process_info is not None and thresholdB is not None:
                returnJSON_FromMessageMatch_AGOL(agol_process_info, 84786, ["sensitivity"],
                                                 [str(thresholdB)],
                                                 rowStyle='<ul><li></li></ul>',
                                                 msg2Use=f"{ARCPY.GetIDMessage(84786)} " + "{0}")
        ...
        if not detectThreshold:
            ...
            if agol_process_info is not None:
                returnJSON_FromMessageMatch_AGOL(agol_process_info, 84786, ["sensitivity"],
                                                 [str(maxValueThreshold - thresholdB)],
                                                 rowStyle='<ul><li></li></ul>',
                                                 msg2Use=f"{ARCPY.GetIDMessage(84786)} " + "{0}")
            ...
            if agol_process_info is not None:
                returnJSON_FromMessageMatch_AGOL(agol_process_info, 84786, ["sensitivity"], [str(maxValueThreshold - maxDer[0] - 1)],
                                                 rowStyle='<ul><li></li></ul>',
                                                 msg2Use=f"{ARCPY.GetIDMessage(84786)} " + "{0}")
                returnJSON_FromMessageMatch_AGOL(agol_process_info, 84787, ["sensitivity"], [str(maxValueThreshold - maxDer[1] - 1)],
                                                 rowStyle='<ul><li></li></ul>',
                                                 msg2Use=f"{ARCPY.GetIDMessage(84787)} " + "{0}")
...
class Outlier(object):
    ...
    def __searchGrid(self, n, percentile = None):
        ...
        if percentile is None:
            ...
            neighPer = n*percentile
            if neighPer < 1 :
                neighPer = 1/n
                ARCPY.AddIDMessage("Warning", 110410,  UTILS.formatValue(neighPer*100, "%0.3f") )
                neighPer = 1.05/n
            else:
                neighPer= percentile
        
            percs = NUM.array([neighPer])
    ...
    def run(self):
        ...
        if self.outlierType == "LOCAL":
            ...
            if self.minPoints> 0 and self.percentile is not None:
                neighPer = len(xyz)*self.percentile/100.0

                if neighPer < 1 :
                    neighPer = 100/len(xyz)
                    ARCPY.AddIDMessage("Warning", 110410,  UTILS.formatValue(neighPer, "%0.3f") )
                    neighPer = 105/len(xyz) ## Remove 
                else:
                    neighPer= self.percentile
                self.percentile = neighPer


            ### Use a Grid to find the best K, C ####
            if self.minPoints == -1 or self.percentile is None :
                minForAutoLOF = 2
                ...
                k, c = self.autoLOF(len(xyz), percs, ks, minForAutoLOF)
                self.minPoints = k
    ...
    def autoLOF(self, nFeatures, rangeC, rangeK, minC = 2):
        """ Find best K, C 
        INPUT:
        ...
        rangeCUpdate = rangeC[rangeC_cn>=minC]
    ...
    def createFinalOutput(self, multipleFLTAreCreated, workspaceOutput, listRasterZones,
        ...
        if multipleFLTAreCreated :
            ...
            #### Merging Rasters ####
            msg = ARCPY.GetIDMessage(84497)
            ARCPY.SetProgressor("default", msg.format(outName))
            try:
            ...
            msg = ARCPY.GetIDMessage(84497)
            ARCPY.SetProgressor("default", msg.format(outputRasterPath))
    ...
    def runDetectionRaster(self, outputRaster):
        ...
        if ARCPY.env.cellSize not in  ["MAXOF", "MINOF"]:
            cellSizeBase = -1
            try:
                cellSizeBase = UTILS.strToFloat(ARCPY.env.cellSize)
            except:
                pass
            try:
        ...
        if outputRaster is not None: 
            ...
            ARCPY.SetProgressor("default", msg.format("",""))
            spatialRef2SetBack = ARCPY.env.outputCoordinateSystem
    ...
    def output(self):
        ...
        if not self.keepJustOutlier:
            ...
            if not self.ssdo.hasOID64:
                ...
                fieldDict.append(["SOURCE_ID" , (NUM.asarray(sourceId[self.justOutliers], dtype = NUM.int64), "BIGINTEGER",  ARCPY.GetIDMessage(220125))])
    ...
    def __applyJSONLayer(self,parameterIndex=4, editClasses = True):
        ...
        if not ARCPY.Exists(self.outputRaster):
            ARCPY.AddIDMessage("WARNING", 10029, self.outputRaster)
            return
...
def getReachability(inputFC, minPoints = None, threshold = None, 
    ...
    if functionE is None:
        zones = DetectZones()
        idClusters, _ =  zones.getClusters(reachValues,
                                            orderValues,
                                            minPoints, 
                                            threshold,
                                            showReachInMapPlotLib = viewReach)
    else:
        idClusters, _ =  functionE.getClusters(reachValues,
                                            orderValues,
                                            minPoints, 
                                            threshold,
                                            showReachInMapPlotLib = viewReach)
