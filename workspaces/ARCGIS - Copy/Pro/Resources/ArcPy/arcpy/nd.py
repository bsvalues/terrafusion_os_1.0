...
def AppendToDiagram(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AppendToDiagram_nd(*gp_fixargs((in_network_diagram_layer, map), True))
        )
        return retval
    except Exception as e:
        raise e
...
def ChangeDiagramsOwner(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ChangeDiagramsOwner_nd(*gp_fixargs((in_diagrams, target_owner, source_owner, diagram_names), True))
        )
        return retval
    except Exception as e:
        raise e
...
def CreateDiagram(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.CreateDiagram_nd(*gp_fixargs((in_utility_network, template_name, features), True))
        )
        return retval
    except Exception as e:
        raise e
...
def DeleteDiagram(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.DeleteDiagram_nd(*gp_fixargs((in_diagrams, template_names, diagram_names), True))
        )
        return retval
    except Exception as e:
        raise e
...
def ExtendDiagram(
    ...
    INPUTS:
     ...
     ignore_traversability {Boolean}:
         ...
         parameter is not specified.IGNORE_TRAVERSABILITY-The traversability of
         the network will be
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ExtendDiagram_nd(*gp_fixargs((in_network_diagram_layer, ignore_traversability, extension_type), True))
        )
        return retval
    except Exception as e:
        raise e
...
def MakeDiagramLayer(
    ...
    """MakeDiagramLayer_nd(in_utility_network, network_diagram_name, out_layer, {sublayers_option})
    ...
    INPUTS:
     ...
     sublayers_option {Boolean}:
         Specifies the type of output layer that will be
         created.DO_NOT_ADD_SUBLAYERS-The output will be a diagram layer
         without a
         sublayer. Diagram layers without a sublayer can be used as input to
         geoprocessing tools such as the Store Diagram, Update Diagram, and
         Apply Smart Tree Layout tools. This is the default.ADD_SUBLAYERS-The
         output will be a group layer with sublayers. The
         feature sublayers can be exported to geodatabase feature classes using
...
def OverwriteDiagram(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.OverwriteDiagram_nd(*gp_fixargs((in_network_diagram_layer, map), True))
        )
        return retval
    except Exception as e:
        raise e
...
def PurgeTemporaryDiagrams(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.PurgeTemporaryDiagrams_nd(*gp_fixargs((in_utility_network, created_before), True))
        )
        return retval
    except Exception as e:
        raise e
...
def StoreDiagram(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.StoreDiagram_nd(*gp_fixargs((in_network_diagram_layer, out_name, access_right_type, tags), True))
        )
        return retval
    except Exception as e:
        raise e
...
def AddCollapseContainerByAttributeRule(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddCollapseContainerByAttributeRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
@gptooldoc("AddCollapseContainerByCategoryRule_nd", None)
def AddCollapseContainerByCategoryRule(
    in_utility_network=None,
    template_name=None,
    is_active: Literal["ACTIVE", "INACTIVE"] | None = None,
    container_type: Literal["JUNCTIONS", "EDGES", "BOTH"] | None = None,
    inverse_category_selection: Literal["EXCLUDE_CATEGORIES", "INCLUDE_CATEGORIES"] | None = None,
    category=None,
    reconnected_edges_option: Literal["AGGREGATE_RECONNECTED_EDGES", "DONT_AGGREGATE_RECONNECTED_EDGES"] | None = None,
    description=None,
) -> Result2[str, str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddCollapseContainerByCategoryRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddCollapseContainerRule(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddCollapseContainerRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddConnectivityAssociationsRule(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddConnectivityAssociationsRule_nd(
...
def AddDiagramFeatureCapabilityByAttributeRule(
    ...
    """AddDiagramFeatureCapabilityByAttributeRule_nd(in_utility_network, template_name, is_active, network_source, where_clause, capability, {description}, {valence_0}, {valence_1}, {valence_2}, {valence_3_plus})

       Adds a diagram rule to the rule sequence specified on a diagram
       template to assign a particular capability on diagram features
       currently represented in the diagram during the diagram building
       process. This capability is used by other rules that run later in the
       rule sequence. The diagram features that will be processed are queried
       from a network source class or object table by attributes.
    ...
    INPUTS:
     ...
     capability (String):
         ...
         features at the end of the rule operation. The specified capability
         will be used by other rules that run later in the rule
         ...
         Container rules that run later in the rule sequence. This is the
         ...
         rules that run later in the rule sequence.PREVENT_TO_REDUCE_JUNCTION-
         All queried junctions will be flagged to
         prevent reduction by Reduce Junction rules that run later in the rule
         sequence.ALLOW_TO_REDUCE_JUNCTION-All queried junctions will be
         flagged to
         allow reduction by Reduce Junction rules that run later in the rule
         sequence.SAVE_EMPTY_CONTAINER_AS_POLYGON_IF_UNCONNECTED-All queried
         unconnected
         container junctions will be saved with polygon geometry instead of
         point geometry if the container is
         empty.IGNORED_STARTING_POINT_FOR_UPSTREAM_TRACE-All queried features
         will be
         ignored as starting points by any Trace rules configured later in the
         rule sequence to trace
         upstream.IGNORED_STARTING_POINT_FOR_DOWNSTREAM_TRACE-All queried
         features will
         be ignored as starting points by any Trace rules configured later in
         the rule sequence to trace downstream.
     description {String}:
         The description of the rule.
     valence_0 {Boolean}:
         Specifies whether the selected capability will apply to unconnected
         network diagram junctions.APPLY-The selected capability will apply to
         unconnected network
         diagram junctions. This is the default.DO_NOT_APPLY-The selected
         capability will not apply to unconnected
         network diagram junctions.
     valence_1 {Boolean}:
         Specifies whether the selected capability will apply to network
         diagram junctions connected to a single junction.APPLY-The selected
         capability will apply to network diagram junctions
         connected to a single junction. This is the default.DO_NOT_APPLY-The
         selected capability will not apply to network diagram
         junctions connected to a single junction.
     valence_2 {Boolean}:
         Specifies whether the selected capability will apply to network
         diagram junctions connected to two different junctions.APPLY-The
         selected capability will apply to network diagram junctions
         connected to two different junctions. This is the
         default.DO_NOT_APPLY-The selected capability will not apply to network
         diagram
         junctions connected to two different junctions.
     valence_3_plus {Boolean}:
         Specifies whether the selected capability will apply to network
         diagram junctions connected to three or more different
         junctions.APPLY-The selected capability will apply to network diagram
         junctions
         connected to three or more different junctions. This is the
         default.DO_NOT_APPLY-The selected capability will not apply to network
         diagram
         junctions connected to three or more different junctions."""
    from arcpy.geoprocessing._base import gp, gp_fixargs
    from arcpy.arcobjects.arcobjectconversion import convertArcObjectToPythonObject

    try:
        retval = convertArcObjectToPythonObject(
            gp.AddDiagramFeatureCapabilityByAttributeRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
@gptooldoc("AddDiagramFeatureCapabilityByCategoryRule_nd", None)
def AddDiagramFeatureCapabilityByCategoryRule(
    in_utility_network=None,
    template_name=None,
    is_active: Literal["ACTIVE", "INACTIVE"] | None = None,
    inverse_category_selection: Literal["EXCLUDE_CATEGORIES", "INCLUDE_CATEGORIES"] | None = None,
    ...
    """AddDiagramFeatureCapabilityByCategoryRule_nd(in_utility_network, template_name, is_active, inverse_category_selection, category;category..., capability, {description}, {valence_0}, {valence_1}, {valence_2}, {valence_3_plus})
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddDiagramFeatureCapabilityByCategoryRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddDiagramTemplate(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddDiagramTemplate_nd(*gp_fixargs((in_utility_network, template_name, ndbd_file, ndld_file), True))
        )
        return retval
    except Exception as e:
        raise e
...
def AddExpandContainerByAttributeRule(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddExpandContainerByAttributeRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
@gptooldoc("AddExpandContainerByCategoryRule_nd", None)
def AddExpandContainerByCategoryRule(
    in_utility_network=None,
    template_name=None,
    is_active: Literal["ACTIVE", "INACTIVE"] | None = None,
    containers_visibility: Literal["KEEP_VISIBLE", "HIDE"] | None = None,
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddExpandContainerByCategoryRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddMainlineTreeLayout(
    ...
    INPUTS:
     ...
     are_edges_orthogonal {Boolean}:
         ...
         specified.ORTHOGONAL_EDGES-All diagram edges related to the tree
         branches will
...
def AddPartialOverlappingEdgesLayout(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddPartialOverlappingEdgesLayout_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddRadialTreeLayout(
    ...
    INPUTS:
     ...
     disjoined_graph_absolute {Linear Unit}:
         The minimum spacing that will separate features belonging to disjoined
         graphs when the diagram contains such graphs. This parameter is used
         with absolute units. The default is 10 in the units of the diagram's
         coordinate system.
     disjoined_graph_proportional {Double}:
         The minimum spacing that will separate features belonging to disjoined
         graphs when the diagram contains such graphs. This parameter is used
         with proportional units. The default is 10.
     radius_factor {Double}:
         The multiplicative factor that will be used to increase or decrease
         the radius of each concentric circle. It is also the distance that
         separates each concentric circle related to a hierarchical level. When
         using a radius factor less than 1, the distance that separates the
         diagram junctions belonging to the (n) hierarchical level and the
         (n+1) hierarchical level progressively decreases. With a factor
         greater than 1, the distance between the hierarchical levels increases
         progressively. The default is 1."""
    from arcpy.geoprocessing._base import gp, gp_fixargs
    from arcpy.arcobjects.arcobjectconversion import convertArcObjectToPythonObject

    try:
        retval = convertArcObjectToPythonObject(
            gp.AddRadialTreeLayout_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddReduceEdgeByAttributeRule(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddReduceEdgeByAttributeRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddReduceJunctionByAttributeRule(
    ...
    """AddReduceJunctionByAttributeRule_nd(in_utility_network, template_name, is_active, junction_source, {where_clause}, {connectivity_options}, {unconnected_junctions}, {one_connected_junction}, {two_connected_junctions}, {edges_attributes;edges_attributes...}, {description}, {use_digitized_direction})
    ...
    INPUTS:
     ...
     connectivity_options {String}:
         ...
         more connections
         will be considered. In a network with a hierarchical tier definition,
         such a junction will be reduced to any other junction it is connected
         to, except when it is connected to a single junction. In a network
         with a partitioned tier definition, upstream and downstream traces
         will be run to determine the single upstream or downstream junction to
     ...
     unconnected_junctions {Boolean}:
         ...
         will be reduced. This parameter is only enabled when the
         connectivity_options parameter is set to
     ...
     one_connected_junction {Boolean}:
         ...
         is only enabled when the connectivity_options parameter is set to
         MAX_2_CONNECTED_JUNCTIONS.REDUCE_JCT_TO_1JCT-Network diagram junction
     ...
     two_connected_junctions {Boolean}:
         ...
         parameter is only enabled when the connectivity_options parameter is
         set to MAX_2_CONNECTED_JUNCTIONS.REDUCE_JCT_TO_2JCTS-Network diagram
         junction reduction candidates that
         connect to two other junctions will be reduced. Each junction and its
         ...
         reduction candidates that
         connect to two other junctions will not be reduced; they will be kept.
     ...
     use_digitized_direction {Boolean}:
         Specifies whether the digitized direction will be used when reducing
         junctions with three or more connections.This parameter is only
         enabled when the connectivity_options parameter
         is set to MIN_3_CONNECTED_JUNCTIONS.USE_DIGITIZED_DIRECTION-The
         reduction process will consider flow
         direction based on the digitized direction to identify the single
         upstream or single downstream junction to which the junction will be
         reduced.IGNORE_DIGITIZED_DIRECTION-The reduction process will consider
         flow
         direction based on the location of subnetwork controllers to identify
         the single upstream or single downstream junction to which the
         junction will be reduced. This is the default."""
    from arcpy.geoprocessing._base import gp, gp_fixargs
    from arcpy.arcobjects.arcobjectconversion import convertArcObjectToPythonObject

    try:
        retval = convertArcObjectToPythonObject(
            gp.AddReduceJunctionByAttributeRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
@gptooldoc("AddReduceJunctionByCategoryRule_nd", None)
def AddReduceJunctionByCategoryRule(
    in_utility_network=None,
    template_name=None,
    is_active: Literal["ACTIVE", "INACTIVE"] | None = None,
    inverse_category_selection: Literal["EXCLUDE_CATEGORIES", "INCLUDE_CATEGORIES"] | None = None,
    ...
    """AddReduceJunctionByCategoryRule_nd(in_utility_network, template_name, is_active, inverse_category_selection, category;category..., {connectivity_options}, {unconnected_junctions}, {one_connected_junction}, {two_connected_junctions}, {edges_attributes;edges_attributes...}, {description}, {use_digitized_direction})
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddReduceJunctionByCategoryRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddReduceJunctionRule(
    ...
    """AddReduceJunctionRule_nd(in_utility_network, template_name, is_active, inverse_source_selection, {junction_source;junction_source...}, {connectivity_options}, {unconnected_junctions}, {one_connected_junction}, {two_connected_junctions}, {edges_attributes;edges_attributes...}, {description}, {use_digitized_direction})

       Adds a diagram rule to the rule sequence specified on a diagram
       template to automatically reduce diagram junctions during the diagram
       building process. This tool reduces junctions based on several network
    ...
    INPUTS:
     ...
     junction_source {Table / Feature Class}:
         ...
         junctions and system junction objects. When the
         inverse_source_selection parameter is set to
         INCLUDE_SOURCE_CLASSES, one or more network junction source classes or
         object tables will be processed. This is the default. All diagram
         junctions related to network junctions that belong to those source
         classes and object tables are reduction candidates. The tool
         will process the junction source classes and object tables in
         ...
         list.When the inverse_source_selection parameter is set to
         EXCLUDE_SOURCE_CLASSES, no particular junction source class or object
         table must be specified. In this case, all junctions in the generated
         diagrams, regardless of their source class or object table, will be
         reduced.
     connectivity_options {String}:
         Specifies the number of junction connections that will be considered
         for reduction.MAX_2_CONNECTED_JUNCTIONS-Junctions with two or fewer
...
def AddRelativeMainlineLayout(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddRelativeMainlineLayout_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddRemoveFeatureByAttributeRule(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddRemoveFeatureByAttributeRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
@gptooldoc("AddRemoveFeatureByCategoryRule_nd", None)
def AddRemoveFeatureByCategoryRule(
    in_utility_network=None,
    template_name=None,
    is_active: Literal["ACTIVE", "INACTIVE"] | None = None,
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddRemoveFeatureByCategoryRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddReshapeDiagramEdgesLayout(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddReshapeDiagramEdgesLayout_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddSetRootJunctionByAttributeRule(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddSetRootJunctionByAttributeRule_nd(
                *gp_fixargs(
...
def AddSetStartingPointByAttributeRule(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddSetStartingPointByAttributeRule_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddSpatialDispatchLayout(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddSpatialDispatchLayout_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def AddStartIterationRule(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddStartIterationRule_nd(*gp_fixargs((in_utility_network, template_name, is_active, description), True))
        )
        return retval
    except Exception as e:
        raise e
...
def AddStopIterationRule(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddStopIterationRule_nd(*gp_fixargs((in_utility_network, template_name, is_active, description), True))
        )
        return retval
    except Exception as e:
        raise e
...
def AddStructuralAttachmentsRule(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.AddStructuralAttachmentsRule_nd(
                *gp_fixargs((in_utility_network, template_name, is_active, description), True)
            )
        )
        return retval
...
def CreateDiagramLayerDefinition(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.CreateDiagramLayerDefinition_nd(
                *gp_fixargs(
                    (
                        in_utility_network,
                        template_name,
...
def DeleteDiagramTemplate(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.DeleteDiagramTemplate_nd(*gp_fixargs((in_utility_network, template_name), True))
        )
        return retval
    except Exception as e:
        raise e
...
def ExportDiagramLayerDefinition(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ExportDiagramLayerDefinition_nd(*gp_fixargs((in_network_diagram_layer, out_ndld_file), True))
        )
        return retval
    except Exception as e:
        raise e
...
def ExportDiagramTemplateDefinitions(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ExportDiagramTemplateDefinitions_nd(
...
def GetDiagramTemplateNames(
    ...
    try:
        retval = convertArcObjectToPythonObject(gp.GetDiagramTemplateNames_nd(*gp_fixargs((in_utility_network,), True)))
        return retval
    except Exception as e:
        raise e
...
def ImportDiagramTemplateDefinitions(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ImportDiagramTemplateDefinitions_nd(
...
def ApplyAngleDirectedLayout(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ApplyAngleDirectedLayout_nd(
                *gp_fixargs(
                    (
...
def ApplyForceDirectedLayout(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ApplyForceDirectedLayout_nd(
                *gp_fixargs(
                    (
                        in_network_diagram_layer,
                        are_containers_preserved,
...
def ApplyLinearDispatchLayout(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ApplyLinearDispatchLayout_nd(
                *gp_fixargs(
                    (
                        in_network_diagram_layer,
                        junction_placement_type,
...
def ApplyPartialOverlappingEdgesLayout(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ApplyPartialOverlappingEdgesLayout_nd(
                *gp_fixargs(
...
def ApplyRelativeMainlineLayout(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ApplyRelativeMainlineLayout_nd(
                *gp_fixargs(
                    (
                        in_network_diagram_layer,
                        line_attribute,
...
def ApplySpatialDispatchLayout(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ApplySpatialDispatchLayout_nd(
                *gp_fixargs(
                    (
                        in_network_diagram_layer,
                        are_containers_preserved,
...
def ApplyTemplateLayouts(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ApplyTemplateLayouts_nd(*gp_fixargs((in_network_diagram_layer,), True))
        )
        return retval
    except Exception as e:
        raise e
...
def ReshapeDiagramEdgesLayout(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ReshapeDiagramEdgesLayout_nd(
                *gp_fixargs(
                    (
                        in_network_diagram_layer,
                        are_containers_preserved,
