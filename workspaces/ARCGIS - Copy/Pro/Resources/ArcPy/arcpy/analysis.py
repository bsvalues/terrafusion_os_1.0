...
__all__ = [
    ...
    "Frequency",
    "GenerateNearTable",
    "GenerateOriginDestinationLinks",
...
def GeographicallyWeightedRegression(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.GeographicallyWeightedRegression_analysis(
                *gp_fixargs(
                    (
                        in_features,
                        dependent_field,
...
def Clip(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.Clip_analysis(*gp_fixargs((in_features, clip_features, out_feature_class, cluster_tolerance), True))
        )
        return retval
    except Exception as e:
        raise e
...
def Select(
    ...
    """Select_analysis(in_features, out_feature_class, {where_clause})
       ...
       typically using a select or SQL expression, and stores them in an
       output feature class.
    ...
    INPUTS:
     ...
     where_clause {SQL Expression}:
         The SQL expression that will be used to select a subset of features.
         For more information about SQL syntax, see SQL reference for query
         expressions used in ArcGIS.

    OUTPUTS:
     out_feature_class (Feature Class):
         The output feature class that will be created. If no expression is
         used, the output will contain all the input features."""
    from arcpy.geoprocessing._base import gp, gp_fixargs
    from arcpy.arcobjects.arcobjectconversion import convertArcObjectToPythonObject

    try:
        retval = convertArcObjectToPythonObject(
            gp.Select_analysis(*gp_fixargs((in_features, out_feature_class, where_clause), True))
        )
        return retval
    except Exception as e:
        raise e
...
def Split(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.Split_analysis(
                *gp_fixargs((in_features, split_features, split_field, out_workspace, cluster_tolerance), True)
            )
        )
        return retval
    except Exception as e:
...
def SplitByAttributes(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.SplitByAttributes_analysis(*gp_fixargs((Input_Table, Target_Workspace, Split_Fields), True))
        )
        return retval
    except Exception as e:
        raise e
...
def TableSelect(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.TableSelect_analysis(*gp_fixargs((in_table, out_table, where_clause), True))
        )
        return retval
    except Exception as e:
        raise e
...
def ApportionPolygon(
    in_features=None,
    apportion_fields=None,
    target_features=None,
    out_features=None,
    method: Literal["AREA", "LENGTH", "POINTS"] | None = None,
    estimation_features=None,
    weight_field=None,
    maintain_geometries: Literal["MAINTAIN_GEOMETRIES", "INTERSECT_GEOMETRIES"] | None = None,
) -> Result1[str]:
    ...
    INPUTS:
     ...
     apportion_fields (Value Table):
         The numeric fields from the input polygons that will be summarized by
         each target polygon and recorded in the output feature class.The
         optional statistic types are as follows:SUM-The values for the
         specified field will be added together. This is
         the default.MEAN-The average for the specified field will be
         calculated. This
         option is only valid when the method parameter is set to
         AREA.MEDIAN-The median of the specified field will be calculated. This
         option is only valid when the method parameter is set to AREA.
     ...
     maintain_geometries {Boolean}:
         ...
         geometries of the target polygon layer.MAINTAIN_GEOMETRIES-The output
         feature class will maintain the
...
def CountOverlappingFeatures(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.CountOverlappingFeatures_analysis(
                *gp_fixargs((in_features, out_feature_class, min_overlap_count, out_overlap_table), True)
            )
        )
        return retval
    except Exception as e:
...
def Erase(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.Erase_analysis(*gp_fixargs((in_features, erase_features, out_feature_class, cluster_tolerance), True))
        )
        return retval
    except Exception as e:
        raise e
...
def Identity(
    ...
    cluster_tolerance=None,
    relationship: Literal["KEEP_RELATIONSHIPS", "NO_RELATIONSHIPS"] | None = None,
) -> Result1[str]:
...
def Intersect(
    ...
    join_attributes: Literal["NO_FID", "ONLY_FID", "ALL"] | None = None,
    cluster_tolerance=None,
    output_type: Literal["INPUT", "LINE", "POINT"] | None = None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.Intersect_analysis(
                *gp_fixargs((in_features, out_feature_class, join_attributes, cluster_tolerance, output_type), True)
            )
        )
        return retval
    except Exception as e:
...
def RemoveOverlapMultiple(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.RemoveOverlapMultiple_analysis(
                *gp_fixargs((in_features, out_feature_class, method, join_attributes), True)
            )
        )
        return retval
    except Exception as e:
...
def SpatialJoin(
    ...
    join_features=None,
    out_feature_class=None,
    join_operation: Literal["JOIN_ONE_TO_ONE", "JOIN_ONE_TO_MANY"] | None = None,
    join_type: Literal["KEEP_ALL", "KEEP_COMMON"] | None = None,
    field_mapping=None,
    match_option: Literal[
        "INTERSECT",
        "INTERSECT_3D",
        "LARGEST_OVERLAP",
...
def SymDiff(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.SymDiff_analysis(
                *gp_fixargs((in_features, update_features, out_feature_class, join_attributes, cluster_tolerance), True)
            )
        )
        return retval
    except Exception as e:
...
def Union(
    ...
    join_attributes: Literal["NO_FID", "ONLY_FID", "ALL"] | None = None,
    cluster_tolerance=None,
    gaps: Literal["GAPS", "NO_GAPS"] | None = None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.Union_analysis(
                *gp_fixargs((in_features, out_feature_class, join_attributes, cluster_tolerance, gaps), True)
            )
        )
        return retval
    except Exception as e:
...
def Update(
    ...
    update_features=None,
    out_feature_class=None,
    keep_borders: Literal["BORDERS", "NO_BORDERS"] | None = None,
    cluster_tolerance=None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.Update_analysis(
                *gp_fixargs((in_features, update_features, out_feature_class, keep_borders, cluster_tolerance), True)
            )
        )
        return retval
    except Exception as e:
...
def PairwiseBuffer(
    ...
    dissolve_option: Literal["NONE", "ALL", "LIST"] | None = None,
    dissolve_field=None,
    method: Literal["GEODESIC", "PLANAR"] | None = None,
    max_deviation=None,
) -> Result1[str]:
    ...
    INPUTS:
     ...
     buffer_distance_or_field (Linear Unit / Field):
         ...
         to buffer each feature.See the Usage section for more information
         about how the tool handles
         linear units.When specifying a distance, if the linear unit has two
         words, such as
     ...
     max_deviation {Linear Unit}:
         The maximum distance the output buffer boundary will deviate from the
         true buffer boundary.While the true boundary of the buffer is a curve,
         the output boundary
         will be densified. Use this parameter to control how the output
         polygon boundary approximates the true buffer boundary.If this
         parameter is not set or is set to 0, the tool will identify
         the maximum deviation. It is recommended that you use the default
         value. Performance degradation in the tool and in subsequent analyses
         may result from using a maximum offset deviation that is too small.
...
def PairwiseClip(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.PairwiseClip_analysis(
                *gp_fixargs((in_features, clip_features, out_feature_class, cluster_tolerance), True)
            )
        )
        return retval
    except Exception as e:
...
def PairwiseDissolve(
    in_features=None,
    out_feature_class=None,
    dissolve_field=None,
    statistics_fields=None,
    multi_part: Literal["MULTI_PART", "SINGLE_PART"] | None = None,
    concatenation_separator=None,
) -> Result1[str]:
    ...
    INPUTS:
     ...
     statistics_fields {Value Table}:
         ...
         statistic. Text
         attribute fields can be summarized using minimum, maximum, count,
         first, last, unique, concatenate, and mode statistics.SUM-The values
         for the specified field will be added together.MEAN-The
         average for the specified field will be calculated.MIN-The smallest
         value of the specified field will be identified.MAX-The largest value
         of the specified field will be identified.RANGE-The range of values
         (maximum minus minimum) for the specified
         field will be calculated.STD-The standard deviation of values for the
         specified field will be
         calculated.COUNT-The number of values in the specified field will be
         identified.FIRST-The specified field value of the first record in the
         input will
         be used.LAST-The specified field value of the last record in the input
         will be
         used.MEDIAN-The median of the specified field will be
         calculated.VARIANCE-The variance of the specified field will be
         ...
         parameter.MODE-The mode (the most common value) for the specified
         field will be
         identified. If more than one value is equally common, the lowest value
         will be returned.
     multi_part {Boolean}:
...
def PairwiseErase(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.PairwiseErase_analysis(
                *gp_fixargs((in_features, erase_features, out_feature_class, cluster_tolerance), True)
            )
        )
        return retval
    except Exception as e:
...
def PairwiseIntegrate(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.PairwiseIntegrate_analysis(*gp_fixargs((in_features, cluster_tolerance), True))
        )
        return retval
    except Exception as e:
        raise e
...
def Buffer(
    ...
    out_feature_class=None,
    buffer_distance_or_field=None,
    line_side: Literal["FULL", "LEFT", "RIGHT", "OUTSIDE_ONLY"] | None = None,
    line_end_type: Literal["ROUND", "FLAT"] | None = None,
    dissolve_option: Literal["NONE", "ALL", "LIST"] | None = None,
    dissolve_field=None,
    method: Literal["GEODESIC", "PLANAR"] | None = None,
) -> Result1[str]:
...
def CreateThiessenPolygons(
    in_features=None,
    out_feature_class=None,
    fields_to_copy: Literal["ONLY_FID", "ALL"] | None = None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.CreateThiessenPolygons_analysis(*gp_fixargs((in_features, out_feature_class, fields_to_copy), True))
        )
        return retval
    except Exception as e:
        raise e
...
def GenerateNearTable(
    ...
    closest: Literal["CLOSEST", "ALL"] | None = None,
    closest_count=None,
...
def GenerateOriginDestinationLinks(
    origin_features=None,
    destination_features=None,
    out_feature_class=None,
    origin_group_field=None,
    destination_group_field=None,
    line_type: Literal["PLANAR", "GEODESIC"] | None = None,
    num_nearest=None,
    search_distance=None,
    distance_unit: Literal["KILOMETERS", "METERS", "MILES", "NAUTICALMILES", "YARDS", "FEET"] | None = None,
    aggregate_links: Literal["AGGREGATE_OVERLAPPING", "NO_AGGREGATE"] | None = None,
    sum_fields=None,
) -> Result1[str]:
    ...
    INPUTS:
     ...
     sum_fields {Value Table}:
         ...
         statistic. Text
         attribute fields can be summarized using minimum, maximum, count,
         first, last, unique, and concatenate statistics.SUM-The values for the
         specified field will be added together.MEAN-The
         average for the specified field will be calculated.MIN-The smallest
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.GenerateOriginDestinationLinks_analysis(
                *gp_fixargs(
                    (
                        origin_features,
...
def GraphicBuffer(
    ...
    out_feature_class=None,
    buffer_distance_or_field=None,
    line_caps: Literal["SQUARE", "BUTT", "ROUND"] | None = None,
    line_joins: Literal["MITER", "BEVEL", "ROUND"] | None = None,
    miter_limit=None,
    max_deviation=None,
) -> Result1[str]:
...
def MultipleRingBuffer(
    ...
    | None = None,
    Field_Name=None,
    Dissolve_Option: Literal["ALL", "NONE"] | None = None,
    Outside_Polygons_Only: Literal["OUTSIDE_ONLY", "FULL"] | None = None,
    Method: Literal["PLANAR", "GEODESIC"] | None = None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.MultipleRingBuffer_analysis(
                *gp_fixargs(
                    (
                        Input_Features,
...
def Near(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.Near_analysis(
                *gp_fixargs(
                    (in_features, near_features, search_radius, location, angle, method, field_names, distance_unit),
                    True,
                )
            )
        )
...
def PointDistance(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.PointDistance_analysis(*gp_fixargs((in_features, near_features, out_table, search_radius), True))
        )
        return retval
    except Exception as e:
        raise e
...
def PolygonNeighbors(
    in_features=None,
    out_table=None,
    in_fields=None,
    area_overlap: Literal["AREA_OVERLAP", "NO_AREA_OVERLAP"] | None = None,
    both_sides: Literal["BOTH_SIDES", "NO_BOTH_SIDES"] | None = None,
    cluster_tolerance=None,
    out_linear_units: Literal[
        "UNKNOWN",
        "INCHES",
        "POINTS",
        "FEET",
...
def EnrichLayer(
    ...
    | None = None,
    distance=None,
    unit: Literal["MILES", "YARDS", "FEET", "KILOMETERS", "METERS", "HOURS", "MINUTES", "SECONDS"] | None = None,
) -> Result2[str, str]:
    ...
    INPUTS:
     ...
     buffer_type {String}:
         ...
         are lines, STRAIGHT_LINE is the only valid
         option.STRAIGHT_LINE-Straight-line or Euclidean distance is used as
         the
         distance measure.DRIVE_TIME-Driving time is used as the distance
         measure. Current
         posted speed limits, one-way streets, and turn restrictions affect
         driving time.DRIVING_DISTANCE-Driving distance is used as the distance
         measure.
         One-way streets and turn restrictions affect driving
         distance.TRUCKING_TIME-Trucking time is used as the distance measure.
         Current
         posted speed limits, one-way streets, and turn restrictions affect
         trucking time. This option is similar to Driving time, but travel can
         only occur on roads that are suitable for
         trucks.TRUCKING_DISTANCE-Trucking distance is used as the distance
         measure.
         ...
         roads that are suitable for trucks.WALKING_TIME-Walking time is used
         ...
         limited-access highways.WALKING_DISTANCE-Walking distance is used as
     ...
     unit {String}:
         Specifies the unit for distance parameter value.MILES-MilesYARDS-Yards
         FEET-FeetKILOMETERS-KilometersMETERS-MetersHOURS
         -HoursMINUTES-MinutesSECONDS-Seconds
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.EnrichLayer_analysis(
                *gp_fixargs(
                    (in_features, out_feature_class, country, data_collection, variables, buffer_type, distance, unit),
                    True,
                )
            )
        )
...
def Frequency(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.Frequency_analysis(*gp_fixargs((in_table, out_table, frequency_fields, summary_fields), True))
        )
        return retval
    except Exception as e:
        raise e
...
def Statistics(
    ...
    INPUTS:
     ...
     statistics_fields (Value Table):
         ...
         will be used to calculate the specified statistic. Multiple statistic
         and field combinations can be specified. Null values are excluded from
         all calculations.Numeric attribute fields can be summarized using any
         statistic. Text
         attribute fields can be summarized using minimum, maximum, count,
         first, last, unique, concatenate, and mode statistics. Date, Date
         only, and Timestamp offset attribute fields can be summarized only
         with the mean, minimum, maximum, count, first, last, unique, and
         concatenate statistics.The DBMS statistic types are only supported for
         input geodatabase data
         sources, including enterprise geodatabases, feature services, cloud
         data warehouses, and SQLite geodatabasesSUM-The values for the
         specified field will be added together.MEAN-The
         average for the specified field will be calculated.MIN-The smallest
         value of the specified field will be identified.MAX-The largest value
         of the specified field will be identified.RANGE-The range of values
         ...
         field will be
         identified. If more than one value is equally common, the lowest value
         will be returned.DBMS_SUM-The values for the specified field will be
         added together.DBMS_MEAN-The average for the specified field will be
         calculated.DBMS_MIN-The smallest value of the specified field will be
         identified.DBMS_MAX-The largest value of the specified field will be
         identified.DBMS_STD-The standard deviation of values for the specified
         field will
         be calculated.DBMS_COUNT-The number of values in the specified field
         will be
         identified.DBMS_MEDIAN-The median of the specified field will be
         calculated.DBMS_VARIANCE-The variance of the specified field will be
         calculated.
     case_field {Field}:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.Statistics_analysis(
                *gp_fixargs((in_table, out_table, statistics_fields, case_field, concatenation_separator), True)
            )
        )
        return retval
    except Exception as e:
...
def SummarizeNearby(
    ...
    | None = None,
    distances=None,
    distance_units: Literal["METERS", "KILOMETERS", "FEET", "YARDS", "MILES", "SECONDS", "MINUTES", "HOURS"]
    | None = None,
    time_of_day=None,
    time_zone: Literal["UTC", "GEOLOCAL"] | None = None,
    keep_all_polygons: Literal["KEEP_ALL", "ONLY_INTERSECTING"] | None = None,
    sum_fields=None,
    sum_shape: Literal["ADD_SHAPE_SUM", "NO_SHAPE_SUM"] | None = None,
    shape_unit: Literal[
        "METERS",
        "KILOMETERS",
        "FEET",
    ...
    | None = None,
    group_field=None,
    add_min_maj: Literal["ADD_MIN_MAJ", "NO_MIN_MAJ"] | None = None,
    add_group_percent: Literal["ADD_PERCENT", "NO_PERCENT"] | None = None,
    Output_Grouped_Table=None,
) -> Result2[str, str]:
...
def TabulateIntersection(
    in_zone_features=None,
    zone_fields=None,
    in_class_features=None,
    out_table=None,
    class_fields=None,
    sum_fields=None,
    xy_tolerance=None,
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.TabulateIntersection_analysis(
                *gp_fixargs(
                    (
                        in_zone_features,
                        zone_fields,
