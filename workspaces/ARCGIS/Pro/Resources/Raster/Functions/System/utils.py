...
class ZonalAttributesTable():
    def __init__(self, tableUri, idField=None, attribList=None):
        ...
        self.idField, self.idFI = (idField, 0) if idField else (None, None)
        ...
        if s.startswith('http://') or s.startswith('https://'):
            ...
            import importlib
            self.urllib_request = importlib.import_module("urllib.request")
            self.urllib_parse = importlib.import_module("urllib.parse")
    ...
    def _queryFeatureService(self, where=None, extent=None, sr=None):
        ...
        T = {}
        r = self.urllib_request.urlopen(self.queryUrl, self.urllib_parse.urlencode(p).encode("utf-8")).read()
