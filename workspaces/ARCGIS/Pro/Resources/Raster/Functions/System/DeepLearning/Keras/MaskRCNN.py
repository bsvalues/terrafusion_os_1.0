...
class MatterMaskRCNN:
    def initialize(self, model, model_as_file):
        if not HAS_KERAS:
            raise Exception(
                'Keras(2.3.1 or above), Tensorflow(version 2.1.0 or above) and scikit-image(version 0.15.0 or above) '
                'libraries are not installed. Install them using "conda install tensorflow-gpu=2.1.0 scikit-image=0.15.0 keras-gpu=2.3.1 ".')
