...
r"""The AllSource toolbox contains tools that convert and analyze
intelligence data sources. Use these tools as part of intelligence
workflows to import data from many sources at once, analyze point
track data to determine patterns, and assess physical environment data
for suitable mobility or landing zones."""
...
def FindOverlaps(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.FindOverlaps_intelligence(*gp_fixargs((in_features, out_intersection, out_centroid, group_field), True))
        )
        return retval
...
def GenerateBlindSpotAreas(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.GenerateBlindSpotAreas_intelligence(
                *gp_fixargs((in_features, out_feature_class, clip_features, start_time_field, end_time_field), True)
            )
        )
        return retval
    except Exception as e:
...
def GenerateCoverageAreas(
    ...
    out_feature_class=None,
    buffer_type=None,
    range_unit: Literal["Meters", "Kilometers", "Feet", "Miles", "NauticalMiles"] | None = None,
    start_time_field=None,
    end_time_field=None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.GenerateCoverageAreas_intelligence(
                *gp_fixargs(
                    (in_features, out_feature_class, buffer_type, range_unit, start_time_field, end_time_field), True
                )
            )
        )
        return retval
...
# Analysis\Suitability toolset
@gptooldoc("DOFToObstacleFeatures_intelligence", None)
def DOFToObstacleFeatures(
    in_table=None,
    out_obstacle_features=None,
    out_obstacle_buffers=None,
    clip_features=None,
) -> Result2[str, str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.DOFToObstacleFeatures_intelligence(
                *gp_fixargs((in_table, out_obstacle_features, out_obstacle_buffers, clip_features), True)
            )
        )
        return retval
    except Exception as e:
...
def DropZones(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.DropZones_intelligence(
                *gp_fixargs((in_slope_raster, in_vegetation_features, clip_features, out_feature_class), True)
            )
        )
        return retval
    except Exception as e:
...
def GenerateHLZSuitability(
    in_slope_raster=None,
    in_land_cover_raster=None,
    in_obstacle_buffer_features=None,
    out_raster=None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.GenerateHLZSuitability_intelligence(
                *gp_fixargs((in_slope_raster, in_land_cover_raster, in_obstacle_buffer_features, out_raster), True)
            )
        )
        return retval
    except Exception as e:
...
def GenerateObstacleFeatures(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.GenerateObstacleFeatures_intelligence(
                *gp_fixargs(
                    (in_features, height_field, out_obstacle_features, out_obstacle_buffers, clip_features), True
                )
            )
        )
        return retval
...
def LeastCostPath(
    in_cost_surface=None,
    in_start_point=None,
    in_end_point=None,
    out_path_feature_class=None,
    handle_zeros: Literal["SMALL_POSITIVE", "NO_DATA"] | None = None,
) -> Result3[str, str, str]:
    ...
    INPUTS:
     in_cost_surface (Raster Layer):
         The input raster that will be used to determine the cost to travel
         from starting point to ending point. No Data values cannot be crossed.
     ...
     in_end_point (Feature Set):
         ...
         separate paths.
     handle_zeros {String}:
         Specifies how zero values in the in_cost_surface parameter value will
         be handled.SMALL_POSITIVE-All zeros will be changed to a small
         positive value.
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.LeastCostPath_intelligence(
                *gp_fixargs((in_cost_surface, in_start_point, in_end_point, out_path_feature_class, handle_zeros), True)
            )
        )
        return retval
    except Exception as e:
...
def BatchImportData(
    ...
    target_gdb=None,
    filter=None,
    include_sub_folders: Literal["SUBFOLDERS", "NO_SUBFOLDERS"] | None = None,
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.BatchImportData_intelligence(
                *gp_fixargs((in_data, target_gdb, filter, include_sub_folders, include_groundoverlay), True)
            )
        )
        return retval
    except Exception as e:
...
def CreateLocationFileFromTextFile(
    in_placenames_file=None,
    data_source: Literal["GEONAMES", "NGA_GNS", "USGS_GNIS", "USGS_ANTARCTIC_NAMES"] | None = None,
    out_location_file=None,
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.CreateLocationFileFromTextFile_intelligence(
                *gp_fixargs((in_placenames_file, data_source, out_location_file, include_features, in_rois), True)
            )
        )
        return retval
    except Exception as e:
...
@gptooldoc("MilitaryRasterToMosaicDataset_intelligence", None)
def MilitaryRasterToMosaicDataset(
    in_folder=None,
    target_geodatabase=None,
    output_mosaic_basename=None,
    import_format: list[Literal["CADRG", "CIB", "DTED", "HRE"]]
    | Literal["CADRG", "CIB", "DTED", "HRE"]
    | str
    | None = None,
    allow_duplicate_datasets: Literal["EXCLUDE_DUPLICATES", "ALLOW_DUPLICATES", "OVERWRITE_DUPLICATES"] | None = None,
) -> Result:
    """MilitaryRasterToMosaicDataset_intelligence(in_folder, target_geodatabase, output_mosaic_basename, import_format;import_format..., allow_duplicate_datasets)

       Imports Compressed ARC Digitized Raster Graphic (CADRG), Enhance
       Compressed Raster Graphic (ECRG), Controlled Image Base (CIB), Digital
       Terrain Elevation Dataset (DTED), and High Resolution Elevation (HRE)
       rasters into one or more mosaic datasets. It combines a multistep
       process into a single tool that can handle most use cases for working
       with Raster Product Format (RPF) and RPF-like rasters.

    INPUTS:
     in_folder (Folder):
         The folder containing the CADRG/ECRG, CIB, DTED, and HRE raster
         formats that will be imported. Folders will be searched recursively.
     target_geodatabase (Workspace):
         The enterprise or file geodatabase where the output mosaics will be
         created.
     output_mosaic_basename (String):
         The name of the output mosaic dataset. If multiple import_format
         parameter options are specified, a suffix will be added to the output
         mosaic dataset name. For example, if the output mosaic dataset name is
         mymosaic and the CADRG, CIB, and DTED options are specified, the
         resulting mosaics will be named mymosaic_CADRG, mymosaic_CIB, and
         mymosaic_DTED.
     import_format (String):
         Specifies the raster formats that will be imported from the in_folder
         parameter value as a single string or list of strings.CADRG-Files of
         CADRG/ECRG format will be imported.CIB-Files of CIB
         format will be imported.DTED-Files of DTED format will be
         imported.HRE-Files of HRE format will be imported.When specifying more
         than one format, use a semicolon-separated list,
         for example, "CADRG; CIB; DTED; HRE".
     allow_duplicate_datasets (String):
         Specifies whether duplicate rasters will be
         added.EXCLUDE_DUPLICATES-Duplicate rasters will not be added. This is
         the
         default.ALLOW_DUPLICATES-All rasters will be added even if they
         already exist
         in the mosaic dataset.OVERWRITE_DUPLICATES-Duplicate rasters will
         overwrite existing rasters
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.MilitaryRasterToMosaicDataset_intelligence(
                *gp_fixargs(
                    (in_folder, target_geodatabase, output_mosaic_basename, import_format, allow_duplicate_datasets),
                    True,
                )
            )
        )
...
def PointsToTrackSegments(
    ...
    out_feature_class=None,
    group_field=None,
    include_velocity: Literal["INCLUDE_VELOCITY", "EXCLUDE_VELOCITY"] | None = None,
    out_point_feature_class=None,
    error_on_duplicate_timestamps: Literal["ERROR_DUPLICATE_TIMESTAMPS", "ALLOW_DUPLICATE_TIMESTAMPS"] | None = None,
    keep_input_fields: Literal["KEEP_INPUT_FIELDS", "DISCARD_INPUT_FIELDS"] | None = None,
) -> Result2[str, str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.PointsToTrackSegments_intelligence(
                *gp_fixargs(
                    (
                        in_features,
                        date_field,
...
def ClassifyMovementEvents(
    ...
    roi_id_field=None,
    include_turn_ids: Literal["INCLUDE_TURN_IDS", "NO_TURN_IDS"] | None = None,
    exclude_non_turn_events: Literal["ONLY_TURN_EVENTS", "ALL_FEATURES"] | None = None,
    turn_events_representation: Literal["ALL_FEATURES", "TURN_MIDPOINT"] | None = None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ClassifyMovementEvents_intelligence(
                *gp_fixargs(
                    (
                        in_features,
                        id_field,
...
def CompareAreas(
    ...
    in_area_features=None,
    out_featureclass=None,
    point_id_field=None,
    area_id_field=None,
    relationship: Literal["LOCATION_ONLY", "LOCATION_TIME"] | None = None,
    time_difference=None,
    time_relationship: Literal["NEAR", "NEAR_BEFORE", "NEAR_AFTER"] | None = None,
    include_time_statistics: Literal["TIME_STATISTICS", "NO_TIME_STATISTICS"] | None = None,
) -> Result1[str | Layer]:
...
def FindCotravelers(
    ...
    time_difference=None,
    input_type: Literal["ONE_FEATURECLASS", "TWO_FEATURECLASSES"] | None = None,
    secondary_features=None,
    secondary_id_field=None,
    create_summary_table: Literal["CREATE_SUMMARY_TABLE", "NO_SUMMARY_TABLE"] | None = None,
    out_summary_table=None,
    include_min_cotraveling_duration: Literal["MIN_COTRAVELING_DURATION", "NO_MIN_COTRAVELING_DURATION"] | None = None,
    min_cotraveling_duration=None,
) -> Result2[str, str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.FindCotravelers_intelligence(
                *gp_fixargs(
                    (
                        input_features,
                        out_featureclass,
...
def FindFrequentedLocations(
    ...
    expression=None,
    search_distance=None,
    minimum_loiter_time=None,
    time_boundary=None,
    minimum_dwells=None,
    normalize_daily_distribution: Literal["NORMALIZED", "REAL"] | None = None,
    summary_fields=None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.FindFrequentedLocations_intelligence(
                *gp_fixargs(
                    (
                        in_features,
                        track_id_field,
...
def FindMeetingLocations(
    in_features=None,
    out_area_features=None,
    out_point_features=None,
    unique_name_field=None,
    search_distance=None,
    minimum_loiter_time=None,
    temporal_relationship: Literal["OVERLAPS", "INTERSECTS"] | None = None,
    min_meeting_duration=None,
    max_meeting_duration=None,
) -> Result2[str, str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.FindMeetingLocations_intelligence(
                *gp_fixargs(
                    (
                        in_features,
                        out_area_features,
...
def SelectMovementTracks(
    ...
    time_relationship: Literal["BEFORE_AFTER", "BEFORE", "AFTER", "NONE"] | None = None,
    selection_time=None,
) -> Result1[str | Layer]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.SelectMovementTracks_intelligence(
                *gp_fixargs((in_features, track_id_field, area_of_interest, time_relationship, selection_time), True)
            )
        )
        return retval
    except Exception as e:
