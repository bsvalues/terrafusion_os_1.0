...
def ClearJobReplicationInfo(
    Input_Repository_URL=None,
    Input_DatabasePath=None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ClearJobReplicationInfo_wmx(*gp_fixargs((Input_Repository_URL, Input_DatabasePath), True))
        )
        return retval
...
def CreateJobDataReplica(
    Input_Parent_Repository_URL=None,
    Input_Parent_Repository_Name=None,
    Input_Multi_Name=None,
) -> Result2[str, str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.CreateJobDataReplica_wmx(
                *gp_fixargs((Input_Parent_Repository_URL, Input_Parent_Repository_Name, Input_Multi_Name), True)
            )
        )
        return retval
    except Exception as e:
...
def CreateWorkflowDatabase(
    Input_Database_Connection=None,
    AOI_Spatial_Reference=None,
    Import_Configuration: Literal["Minimum Configuration", "Quick Configuration", "Custom Configuration"] | None = None,
    Input_Custom_Configuration=None,
    User_Store: Literal["TRADITIONAL", "PORTAL"] | None = None,
) -> Result1[str]:
...
def ExportJobData(
    Input_Folder=None,
    Input_DatabasePath=None,
    Input_Repository_Name=None,
    Input_Export_Since=None,
    Input_Export_Until=None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ExportJobData_wmx(
                *gp_fixargs(
                    (Input_Folder, Input_DatabasePath, Input_Repository_Name, Input_Export_Since, Input_Export_Until),
                    True,
                )
            )
        )
...
def ImportJobData(
    Input_File=None,
    Input_Merge: Literal["COMBINE", "REPLACE"] | None = None,
    Input_DatabasePath=None,
    Input_Repository_Name=None,
) -> Result1[str]:
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.ImportJobData_wmx(
                *gp_fixargs((Input_File, Input_Merge, Input_DatabasePath, Input_Repository_Name), True)
            )
        )
        return retval
    except Exception as e:
...
def PublishWorkflowService(
    service_name=None,
    aoi_service_name=None,
    server=None,
    out_service_draft_location=None,
    input_database_path=None,
    server_folder=None,
    description=None,
    overwrite: Literal["OVERWRITE", "NO_OVERWRITE"] | None = None,
) -> Result2[str, str]:
...
def UpgradeWorkflowDatabase(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.UpgradeWorkflowDatabase_wmx(*gp_fixargs((Input_Database_Connection, User_Store), True))
        )
        return retval
    except Exception as e:
        raise e
...
def CreateJobs(
    ...
    Number_of_Jobs=None,
    Users=None,
    Priority_of_Jobs=None,
    Feature_Layer_AOI=None,
    Union_option: Literal["UNION", "NO_UNION"] | None = None,
) -> Result1[str]:
    ...
    INPUTS:
     ...
     Union_option {Boolean}:
         ...
         polygons.UNION-One union polygon will be generated from the AOI
         polygons and
         one job will be created regardless of the input number of
         jobs.NO_UNION-Each AOI polygon will be used to generate one job. The
         total
         number of jobs created is equal to the total number of polygons in the
         feature layer. This is the default."""
    from arcpy.geoprocessing._base import gp, gp_fixargs
    from arcpy.arcobjects.arcobjectconversion import convertArcObjectToPythonObject
...
def CreateNewJobs(
    ...
    Assign_To=None,
    Priority=None,
    Feature_Layer_LOI=None,
    Union: Literal["UNION", "NO_UNION"] | None = None,
) -> Result1[str]:
...
def GetJobAOI(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.GetJobAOI_wmx(*gp_fixargs((Input_JobID, aoi_Layer, Input_DatabasePath), True))
        )
        return retval
    except Exception as e:
        raise e
...
def GetJobDataWorkspace(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.GetJobDataWorkspace_wmx(*gp_fixargs((Input_JobID, Input_DatabasePath, Input_SDEFileLocation), True))
        )
        return retval
    except Exception as e:
        raise e
...
def GetJobParentVersion(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.GetJobParentVersion_wmx(*gp_fixargs((Input_JobID, Input_DatabasePath), True))
        )
        return retval
    except Exception as e:
        raise e
...
def GetJobVersion(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.GetJobVersion_wmx(*gp_fixargs((Input_JobID, Input_DatabasePath), True))
        )
        return retval
    except Exception as e:
        raise e
...
def PostJobVersion(
    ...
    try:
        retval = convertArcObjectToPythonObject(
            gp.PostJobVersion_wmx(*gp_fixargs((Input_JobID, Input_DatabasePath), True))
        )
        return retval
    except Exception as e:
        raise e
