...
USER_AGENT = "Mozilla/4.0 (_server_admin)"
...
class RestURL:
    ...
    def __init__(self, url, file_data=None):
        ...
        for k, v in list(compat.parse_qs(urllist[3]).items()):
            query_dict[k] = v[0]
            if k.lower() == 'token':
                self.__token__ = v[0]
    ...
    def _get_subfolder(self, foldername, returntype,
        ...
        """Return an object of the requested type with the path relative
           ...
           may be set."""
        newurl = compat.urljoin(self.url, compat.quote(compat.ensure_bytes(foldername)), False)
        ...
        if params:
            ...
            query_dict = dict((k, v[0]) for k, v in
                               list(compat.parse_qs(urllist[3]).items()))
    ...
    def _contents(self):
        ...
        if self.__urldata__ is Ellipsis or self.__cache_request__ is False:
            if self._file_data:
                ...
                self.__post__ = True
                boundary = compat.ensure_bytes("-"*12+str(uuid.uuid4())+"$")
                multipart_data = compat.ensure_bytes('')
                for k, v in list(compat.parse_qs(self.query).items()):
                    if not isinstance(v, list):
                        v = [v]
                    for val in v:
                        multipart_data += boundary + compat.ensure_bytes("\r\n")
                        multipart_data += compat.ensure_bytes('Content-Disposition: form-data; '
                                           'name="%s"\r\n\r\n' % compat.ensure_string(k))
                        multipart_data += val + compat.ensure_bytes("\r\n")
                for k, v in list(self._file_data.items()):
                    ...
                    multipart_data += compat.ensure_bytes('Content-Disposition: form-data; '
                                       ...
                                       'Content-Type:%s\r\n\r\n' %
                                            (k, fn, ct))
                    multipart_data += v.read() + compat.ensure_bytes("\r\n")
                multipart_data += boundary + compat.ensure_bytes("--\r\n\r\n")
                req_dict = {'User-Agent' : USER_AGENT,
                            'Content-Type':
                            'multipart/form-data; boundary='+compat.ensure_string(boundary[2:]),
...
class GPJobStatus(RestURL):
    ...
    def results(self):
        ...
        if self._results is None:
            def item_iterator():
                for resref in list(self._json_struct['results'].values()):
    ...
    def messages(self):
        ...
        return list(map(GPMessage, self._json_struct['messages']))
...
class GPTask(RestURL):
    ...
    def __expandparamstodict(self, params, kw):
        ...
        parametervalues = dict(list(zip((p['name'] for p in self_parameters),
                                    params)))
        for kw, kwval in list(kw.items()):
...
class LengthsResult(JsonResult):
    ...
    @property
    def lengths(self):
        return list(map(float(length) for length in self._json_struct['lengths']))
...
class AreasAndLengthsResult(LengthsResult):
    ...
    @property
    def areas(self):
        return list(map(float(area) for area in self._json_struct['areas']))
...
class FeatureLayer(MapLayer):
    ...
    def DeleteFeatures(self, objectIds=None, where=None, geometry=None,
        ...
        """This operation deletes features in a feature layer or table (POST
           ...
           includes an error code and an error description."""
        # Removed unused variable and added geometry.Geometry to address
        # Coverity CID 278240
        gt = geometry.__geometry_type__
        geo_json = json.dumps(geometry.Geometry._json_struct_without_sr)
