...
def execute(parameters, messages):
    ...
    if masterField == dependentVar:
        ARCPY.AddIDMessage("ERROR", 945, masterField, ARCPY.GetIDMessage(84112))
        raise SystemExit()
    ...
    expReg = ExploratoryRegression(ssdo, dependentVar, independentVars,
                                   weightsFile = weightsFile, outputReportFile = outputReportFile,
                                   outputTable = outputTable, maxIndVars = maxIndVars,
                                   minIndVars = minIndVars, minR2 = minR2, maxCoef = maxCoef,
                                   maxVIF = maxVIF, minJB = minJB, minMI = minMI)

    #### Assure Table is Added to TOC ####
    if outputTable:
        if expReg.dbf:
            UTILS.setParameterAsText(5, expReg.outputTable, parameters)
...
class ResultHandler(object):
    ...
    def __init__(self, allVarNames, numChoose, ssdo,
        ...
        if numChoose <= 2:
            ...
            self.eachAppears = COMB(self.numVars - 2, numChoose - 2)
    ...
    def report(self):
        ...
        r2Report = UTILS.outputTextTable(r2Info, header = header,
                                         justify = masterJustify, returnHTMLMsg=True, force2Txt=False)
        r2ReportTxt = UTILS.outputTextTable(r2Info, header = header,
                                         justify = masterJustify, returnHTMLMsg=True, force2Txt=True)
        ...
        passingReport = UTILS.outputTextTable(passList, header=header, returnHTMLMsg=True, emphasizeHeadRow=False, force2Txt=False)
        passingReportTxt = UTILS.outputTextTable(passList, header=header, returnHTMLMsg=True, emphasizeHeadRow=False, force2Txt=True)
        ...
        if UTILS.couldExportHTMLMessage():
            title = UTILS.outputSectionalTitle(title, returnHTMLMsg=True)
        else:
            finalReport.append(starMess)
        returnReport.append(starMess)

        finalReport += [title, r2Report, passingReport]
        returnReport += [self.label, r2ReportTxt, passingReportTxt]
        for msg in finalReport:
...
class OLSResult(object):
    ...
    def report(self, orderType = 0, formatStr = "%0.6f", addModel = True):
        ...
        if self.maxVIFValue >= 1000:
            resultListVals[vifInd] = ">" + LOCALE.format_string("%0.2f", 1000.)
        if addModel:
...
class ExploratoryRegression(object):
    ...
    def runModels(self):
        ...
        self.x = NUM.ones((self.n, k), dtype = NUM.float64)
        for column, variable in enumerate(self.independentVars):
            self.x[:,column + 1] = ssdo.fields[variable].returnDouble()
                ...
                if (not runModel) or nearPerfectBool:
                    ...
                    if self.outputTable:
                        ...
                        if lesserKeepModel:
                            ...
                            giPVal = res.miPVal
                            if giPVal is None or self.allMIPass:
    ...
    def endSummary(self):
        ...
        if UTILS.couldExportHTMLMessage():
            UTILS.outputSectionalTitle(globalHeader)
        ...
        perfectInterStr += " (%s)" % LOCALE.format_string("%0.2f", 100)
        ...
        for ind, category in enumerate(categories):
            outValue = LOCALE.format_string("%0.2f", boolPerc[ind])
        ...
        self.passReport = UTILS.outputTextTable(passResults,
                                                header=passHeader, pad = 1,
                                                justify = ["left", "right", "right", "right", "right"], force2Txt=False)
        self.passReportTxt = UTILS.outputTextTable(passResults,
                                                   header = passHeader,
                                                   pad = 1, justify = ["left", "right", "right", "right", "right"],
                                                   force2Txt=True)
        ...
        for ind, varName in enumerate(self.independentVars):
            ...
            if successfulRun:
                ...
                rowRes = [varName, LOCALE.format_string("%0.2f", percentSign),
                          LOCALE.format_string("%0.2f", percentNeg),
                          LOCALE.format_string("%0.2f", percentPos)]
            else:
                percentSign = -1.0
            ...
            if successfulRun:
                if self.printVIF:
                    ...
                    if abs(globalVIF) >= 1000:
                        globalVIFOut = "> " + LOCALE.format_string("%0.2f", 1000.)
                    else:
                        globalVIFOut = LOCALE.format_string("%0.2f", globalVIF)
                else:
                ...
                for covariate, totalTimes in sortedCovariates:
                    ...
                    tRatio = LOCALE.format_string("%0.2f", tRatio)
        ...
        self.signReport = UTILS.outputTextTable(signResults,
                                                ...
                                                justify = ["left", "right",
                                                           "right", "right"],
                                                pad = 1, force2Txt=False)
        self.signReportTxt = UTILS.outputTextTable(signResults,
                                                header = signHeader,
                                                justify = ["left", "right",
                                                           "right", "right"],
                                                pad = 1, force2Txt=True)
        ...
        self.vifReport = UTILS.outputTextTable(vifResults, header = vifHeader,
                                               justify = ["left", "right",
                                                          "right" if UTILS.couldExportHTMLMessage() else "center",
                                                          "left"],
                                               pad = 1, force2Txt=False)
        self.vifReportTxt = UTILS.outputTextTable(vifResults, header = vifHeader,
                                               justify = ["left", "right",
                                                          "right" if UTILS.couldExportHTMLMessage() else "center",
                                                          "left"],
                                               pad = 1, force2Txt=True)
        ...
        self.jbReport = UTILS.outputTextTable(jbResults, header = jbHeader,
                                          pad = 1, justify = masterJustify, force2Txt=False)
        self.jbReportTxt = UTILS.outputTextTable(jbResults, header = jbHeader,
                                              pad = 1, justify = masterJustify, force2Txt=True)
        ...
        if not self.allMIPass:
            ...
            self.miReport = UTILS.outputTextTable(miResults, header = miHeader,
                                              pad = 1, justify = masterJustify, force2Txt=False)
            self.miReportTxt = UTILS.outputTextTable(miResults, header = miHeader,
                                                  pad = 1, justify = masterJustify, force2Txt=True)
        else:
            self.miReport = "\n" + ARCPY.GetIDMessage(84311)
            self.miReport += " (" + ARCPY.GetIDMessage(84500) + ")\n"
            self.miReportTxt = self.miReport

        #### Significance Locale String ####
        modelString2 = ARCPY.GetIDMessage(84314)
        numSep = ";"
        modelString2 = modelString2.format(LOCALE.format_string("%0.2f", .1), numSep,
                                           LOCALE.format_string("%0.2f", .05),
                                           LOCALE.format_string("%0.2f", .01))
        ...
        self.abbReport = UTILS.outputTextTable(abbResults, header=abbHeader, emphasizeHeadRow=False, force2Txt=False)
        self.abbReportTxt = UTILS.outputTextTable(abbResults, header=abbHeader, emphasizeHeadRow=False, force2Txt=True)
        ...
        globalHeader = globalHeader.format(self.dependentVar)
        globalHeader = globalHeader.center(78, "*")
        self.fullReport = [starMess, globalHeader, self.passReportTxt,
                           dashMess, self.signReportTxt,
                           dashMess, self.vifReportTxt,
                           dashMess, self.jbReportTxt,
                           dashMess, self.miReportTxt,
                           dashMess, self.abbReportTxt,
                           dashMess]
