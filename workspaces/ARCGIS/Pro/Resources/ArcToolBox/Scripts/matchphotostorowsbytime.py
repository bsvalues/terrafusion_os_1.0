            ...
            with arcpy.da.Editor(os.path.dirname(outtable)) as edit_session:
                ...
                for file in photolist:
                    ...
                    if photo.m:
                        ...
                        if isinstance(photo.m, bytes):
                            photo.m = datetime.datetime.fromtimestamp(time.mktime(time.strptime(photo.m.decode("utf-8"),
                                                                                                '%Y:%m:%d %H:%M:%S')))
                        else:
                            photo.m = datetime.datetime.fromtimestamp(time.mktime(time.strptime(photo.m,
                                                                                                '%Y:%m:%d %H:%M:%S')))
