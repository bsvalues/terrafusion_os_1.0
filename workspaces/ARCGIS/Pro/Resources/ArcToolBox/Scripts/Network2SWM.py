...
def network2SWM(inputFC, masterField, swmFile, inputNetwork, 
                ...
                travelMode = None, timeOfDay = None):
    ...
    if travelMode is not None:
        if travelMode not in ["CUSTOM", "Custom"]:
            travelModes = NET.GetTravelModes(inputNetwork)
            ...
            hierarchy = travelModeInfo.useHierarchy == "USE_HIERARCHY"
    ...
    odCostMatrixLayer = NET.MakeODCostMatrixLayer(inputNetwork, ODCostMatrix, impedance, cutoff,
                                                  ...
                                                  "NO_LINES", timeOfDay).getOutput(0)
    ...
    #### Assure Locations on Network ####
    statusString = ARCPY.AddFieldDelimiters(destinationLayer, 'STATUS')
    selectNoExtent = UTILS.clearExtent(DM.SelectLayerByAttribute)
    sqlString = statusString + " = 0 OR " + statusString + " = 7"
    selectNoExtent(destinationLayer, "NEW_SELECTION", sqlString)
    cnt = UTILS.getCount(destinationLayer)
    if cnt < 2:
    ...
    sqlStrings = UTILS.sqlChunkStrings(destinationLayer, masterField, chunkedIDs)
