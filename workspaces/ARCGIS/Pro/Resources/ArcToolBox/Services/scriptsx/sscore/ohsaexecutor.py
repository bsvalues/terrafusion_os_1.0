"""FindHotSpots core logic executor."""
...
from OptimizedHotSpotAnalysis import runOHSA
...
from .utilities import duplicate_reserved_fields, densify_features, DISTANCE_UNIT_INFO, SUPPORTED_NUMERIC_TYPES

LOGGER = LogUtils.setup_logger(__name__)
ERROR_ID = [192, 366, 401, 906, 929, 1533, 1534, 1572, 1573, 1575, 1607, 100007, 84426, 110243]


class OHSAExecutor(PAExecutor):
    """ Core logic for OptimizedHotSpotAnalysis tool. """

    def __init__(self, input_layer: PAFeatureLayer, output_layer: PAOutputFeatureLayer,
                 analysis_field: str, bounding_polygon_layer: Optional[PAFeatureLayer],
                 aggregation_polygon_layer: Optional[PAFeatureLayer], 
                 aggregate_method: str = "", cell_size: str = "", distance_band: str = "",
                 divided_by_field: str = "", service_url: str = "", entoken: str = "", referer: str = "",
                 output_wkspc: str = "",
    ...
    def validate_parameters(self) -> bool:
        ...
        if self.cell_size and self.distance_band:
            try:
                ...
                unit_cell, factor_cell = DISTANCE_UNIT_INFO[user_cell_unit]
                unit_band, factor_band = DISTANCE_UNIT_INFO[user_band_unit]
    ...
    def execute(self):
        ...
        if spatial_ref.PCSCode == 102100 or spatial_ref.PCSCode == 3857:
            output_path = AOLUtils.create_unique_name("temp_ohsa_output", self.output_wkspc)
        else:
            output_path = self.output_layer.data
        try:
            arcpy.env.extent = None  # type: ignore
            if self.input_layer.shapeType == "Polygon":
                agol_messages = runOHSA(self.input_layer, output_path, self.analysis_field,
                                        "", "", "",
                ...
                agol_messages = runOHSA(self.input_layer, output_path, self.analysis_field,
                                        self.aggregate_method, bounding_polygons, aggregated_polygons,
                                        self.cell_size, self.distance_band, self.divided_by_field,
                                        self.enrich_layer)
            ...
            for i in info:
                if i[1] == 728:
                    ...
                    LOGGER.error(100052, extra={"message_ID": 100052, 
                                 "fieldName": field_name, "paramName": param_name})
                elif i[1] == 897:
        ...
        if spatial_ref.PCSCode == 102100 or spatial_ref.PCSCode == 3857:
            ...
            # arcpy.Project_management(tempOutput, self.output_layer.data, spatialRef.PCSCode)
        ...
        self.out_layer_popupInfo = self.getPopupContent()
        # LOGGER.debug(json.dumps(self.out_layer_popupInfo.get_popup_info()))

    def getPopupContent(self):
        '''Creates appropriate popup content'''
        # create popup content for count always
        ohsa_popup_info = PopupInfo("Find Hot Spots Summary")
        uid = self.output_layer.oidFieldName
        if uid.upper() == "OID":
            uid = "objectid"
        ohsa_popup_info.add_field_info(uid, "ID")
        ...
        if self.has_analysis_field:
            ohsa_popup_info.add_field_info(duplicate_map[self.analysis_field.upper()], self.analysis_field_alias, True)
            divided_alias = u"{} per {} ".format(self.analysis_field_alias, self.divided_by_field_alias)
        else:
            ohsa_popup_info.add_field_info("JOIN_COUNT", "Number of Points")
        ...
        if self.divided_by_field:
            if self.divided_by_field.upper() == "ESRIPOPULATION":
                ...
                if tp_field:
                    ohsa_popup_info.add_field_info(tp_field, "Esri Population", True, places=0)
            else:
                ohsa_popup_info.add_field_info(duplicate_map[self.divided_by_field.upper()], self.divided_by_field_alias, self.has_analysis_field)
            ohsa_popup_info.add_field_info("ss_rate", divided_alias)
            
        ohsa_popup_info.add_field_info("gi_text", "Statistical Significance")
        return ohsa_popup_info
    ...
    def enrich_layer(self, input_layer: Union[PAFeatureLayer, str]):
        ...
        tp_field = [tp for tp in af_field if not tp in bf_field][0]
