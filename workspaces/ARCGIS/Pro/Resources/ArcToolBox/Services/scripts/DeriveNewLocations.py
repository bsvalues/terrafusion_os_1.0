...
# Constant variables
TASK_NAME = u"DeriveNewLocations"
REQD_TOOLBOXES = "Workflows.tbx"
ERROR_CODES = [100053, 100054, 100055, 100056, 100057,
               100058, 100059, 100060, 100049, 100024,
               100262]
    ...
    if totalCnt == 0:
        ...
        for index, cnt in enumerate(inputLayersCount):
            if cnt == 0:
                ...
                errMsg = "No features in the processing extent for {} ".format(lyrName)
...
def updateChangedFields(expressionList, changedFieldsList):
    """updates change Fields in expression"""
...
def deriveNewLocations(startTime, shapeType):
    """Find existing locations."""
    LocationUtils(inputLayers, json.dumps(expressions, ensure_ascii=False), resultLayer).derive_new_locations()

    startTime = aolutils.AddTimerMessage(startTime, "DeriveNewLocations tool")
    descResultLayer = arcpy.Describe(resultLayer)
    shapeType = descResultLayer.shapeType
    if shapeType == "Polygon":
        units = aolutils.getUnits(hostedgp)
        aolutils.createShapeAreaField(resultLayer, units, descResultLayer)
    elif shapeType == "Polyline":
        units = aolutils.getUnits(hostedgp, shapeUnitsPolygon=False)
        aolutils.createShapeLengthField(resultLayer, units, descResultLayer)
...
if __name__ == '__main__':
    ...
    try:
        ...
        # process parameters
        startTime = deriveNewLocations(startTime, shapeType)
