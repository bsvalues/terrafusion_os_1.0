...
class MinMaxBin(object):
    ...
    def updateMinMax(self, groupByFieldText, shapeValue, shapePercent=None):
        ...
        if not (isinstance(groupByFieldText, str) or isinstance(groupByFieldText, unicode)):             
            groupByFieldText = unicode(groupByFieldText)
...
def addMinorityMajority(fidFieldName,  groupByFieldName, groupByTable,
    ...
    with arcpy.da.UpdateCursor(outputPolygon, polygonCursorFields, ) as polygonCursor:   
        ...
        #arcpy.AddMessage([fidFieldName, groupByFieldName, shapeFieldName])
                ...
                if currentTableRowID != tblRow[0]:                                        
                    while True:
                        ...
                        if tblRow[0] == lyrRow[0]:
                            if isPolygon:
                                shpGeom = lyrRow[2]
                                totalShape = shpGeom.getArea("PRESERVE_SHAPE", sumUnits)
                            else:
                                totalShape = lyrRow[1]
                            break
                    currentTableRowID = tblRow[0]
                if totalShape:
                    tblRow[2] = (locale.atof(str(tblRow[1]))/locale.atof(str(totalShape)))*100.0
                else:
                    tblRow[2] = 0.0
...
if __name__ == '__main__':
    ...
    outputPolygon = r"D:\MinorityMajority\AshOutput.gdb\SummaryOutput"
    groupByTable = r"D:\MinorityMajority\AshOutput.gdb\ColorSummary"
    groupByFieldName = "Color"
    shapeFieldName = "Point_Count"
    fidFieldName = "Join_ID"
    
    addMinorityMajority(fidFieldName, groupByFieldName, groupByTable,
                        outputPolygon, shapeFieldName,
                        minMajorityPercent=False)
