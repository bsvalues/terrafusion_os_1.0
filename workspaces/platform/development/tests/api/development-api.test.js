...
describe('Development API Tests', () => {
  let authToken;
  let testClient;
  ...
  beforeAll(async () => {
    ...
    authToken = await generateTestToken({
      ...
      scopes: ['development:read', 'development:write'],
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /health', async () => {
      const response = await testClient
        .get('/api/v1/development/health')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'development');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /data', async () => {
      const response = await testClient
        .get('/api/v1/development/data')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'development');
    ...
    test('should handle POST /operations', async () => {
      const response = await testClient
        .post('/api/v1/development/operations')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'development');
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/development/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'development-api-tests',
  security_level: 'MEDIUM',
  endpoints_tested: 6,
