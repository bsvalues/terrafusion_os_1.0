...
describe('Engines API Tests', () => {
  ...
  beforeAll(async () => {
    ...
    authToken = await generateTestToken({
      workspace: 'engines',
      scopes: ['engines:read', 'engines:write'],
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /health', async () => {
      const response = await testClient
        .get('/api/v1/engines/health')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'engines');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /data', async () => {
      const response = await testClient
        .get('/api/v1/engines/data')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'engines');
    ...
    test('should handle POST /operations', async () => {
      const response = await testClient
        .post('/api/v1/engines/operations')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'engines');
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/engines/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'engines-api-tests',
  security_level: 'MEDIUM',
  endpoints_tested: 6,
