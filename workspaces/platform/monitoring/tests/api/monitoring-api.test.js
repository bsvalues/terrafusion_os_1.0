...
describe('Monitoring API Tests', () => {
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /metrics', async () => {
      const response = await testClient
        .get('/api/v1/monitoring/metrics')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'monitoring');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /logs', async () => {
      const response = await testClient
    ...
    test('should handle POST /alerts', async () => {
      const response = await testClient
    ...
    test('should handle GET /dashboards', async () => {
      const response = await testClient
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/monitoring/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'monitoring-api-tests',
  security_level: 'HIGH',
  endpoints_tested: 6,
  compliance_verified: ['System Visibility', 'Performance Tracking', 'Alert Accuracy']
};