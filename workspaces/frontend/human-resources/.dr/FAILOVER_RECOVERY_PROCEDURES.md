# Failover & Recovery Procedures for human-resources
...
1. **Local Storage**
   - Path: `/var/backups/human-resources`
   - Retention: 30 days
   - Purpose: Quick recovery

2. **AWS Cloud**
   - Bucket: `terrafusion-backups-human-resources`
   - Region: us-east-1
   - Retention: 180 days
   - Purpose: Geographic redundancy

3. **Azure Cloud**
   - Container: `terrafusion-backups-human-resources`
   - Region: eastus
   - Retention: 180 days
   - Purpose: Geographic redundancy
...
|--------|--------|--------|
| RTO (Recovery Time Objective) | 15 minutes | Monitored |
...
|----------|--------|-----------|-------------------|
| Primary database down | Automatic failover | 15 min | Automated system |
| Data corruption detected | Stop writes, restore from backup | 30 min | DBA + Team lead |
| Network partition | Manual failover assessment | 45 min | Operations manager |
| Multi-region failure | Initiate full disaster recovery | 75 min | Disaster recovery team |
