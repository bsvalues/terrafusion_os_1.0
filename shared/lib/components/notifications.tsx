...
function getTypeColors(type: NotificationType, darkMode = false): {
  ...
  if (darkMode) {
    return {
      success: { background: '#065f46', border: '#10b981', text: '#d1fae5', icon: '#10b981' },
      error: { background: '#7f1d1d', border: '#ef4444', text: '#fee2e2', icon: '#ef4444' },
      warning: { background: '#78350f', border: '#f59e0b', text: '#fef3c7', icon: '#f59e0b' },
      info: { background: '#1e3a8a', border: '#3b82f6', text: '#dbeafe', icon: '#3b82f6' },
    }[type];
  }

  return {
    success: { background: '#d1fae5', border: '#10b981', text: '#065f46', icon: '#10b981' },
    error: { background: '#fee2e2', border: '#ef4444', text: '#7f1d1d', icon: '#ef4444' },
    warning: { background: '#fef3c7', border: '#f59e0b', text: '#78350f', icon: '#f59e0b' },
    info: { background: '#dbeafe', border: '#3b82f6', text: '#1e3a8a', icon: '#3b82f6' },
  }[type];
}
...
function Toast({
  ...
  return (
    <div style={toastStyle} role="alert" aria-live="polite" aria-atomic="true">
...
export function Banner({
  ...
  return (
    <div className={className} style={bannerStyle} role="alert">
...
export function NotificationProvider({
  ...
  animation = 'slide',
  darkMode = false,
}: NotificationProviderProps & { darkMode?: boolean }) {
  ...
  const success = useCallback(
    (message: string, options?: Omit<ToastProps, 'type' | 'message'>) => {
...
export function useAsyncToast() {
  const { toast, success, error } = useNotification();
  ...
  const asyncToast = useCallback(
    async <T,>(
      ...
      const loadingId = options.loading
        ? toast({ type: 'info', message: options.loading, duration: 0, dismissible: false })
        : '';
