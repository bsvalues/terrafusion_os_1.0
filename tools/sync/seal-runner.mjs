#!/usr/bin/env node
// ============================================================================
// TerraFusion Sync — Seal-Check Runner wrapper
// ----------------------------------------------------------------------------
// Runs seal-check-runner.sql via psql and writes the raw pipe-delimited
// output to stdout.  Used by the Workbench server to serve
// POST /api/seal-check/run.
//
// Output (two SQL statements, both with -t -A):
//   Statement 1 — per-gate rows:
//     lane|check_name|measured|expected|verdict
//     (verdict is one of PASS | WARN-CHANGED | WARN-REGRESSED | FAIL)
//   Statement 2 — overall verdict:
//     OVERALL: PASS — ...|0|
//     OVERALL: FAIL — N gate(s) failed|N|reason1 | reason2 | ...
//
// Exit codes:
...
async function main() {
  ...
  const { stdout } = await runFile(
    psql,
    [CONN, '-f', resolve(TOOLS, 'seal-check-runner.sql'), '-t', '-A'],
    {
      env:     { ...process.env, PGPASSWORD: PG_PASSWORD },
      timeout: 60_000,
    }
