...
import { EvidenceItem, Finding, ScanContext, Scanner, WaterfallKind } from './types.js';
 ...
 */
function calculatePriorityScore(filePath: string, group: AwaitSite[], kind: WaterfallKind): number {
...
function buildFinding(
  ...
  switch (classification.kind) {
    ...
    case 'dependent':
      suggestedFix =
        'Review data dependencies. If independent, extract to separate functions and parallelize.';
      break;
    case 'loop-seq':
      suggestedFix =
        'Consider: const results = await Promise.all(items.map(item => fetchItem(item)));';
      break;
    case 'retry-seq':
      suggestedFix =
        'Sequential retry is usually intentional. Add // perf-skill:ignore-waterfall to suppress.';
      break;
    default:
