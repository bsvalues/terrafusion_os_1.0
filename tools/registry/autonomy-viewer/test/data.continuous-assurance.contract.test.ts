...
function createMockVerificationPolicy(overrides: Partial<VerificationPolicy> = {}): VerificationPolicy {
  return {
    cadence: 'daily',
...
function createMockContinuousAssuranceStore(): DataAccessContinuousAssuranceStore {
  ...
  return {
    ...
    async getCadenceMetrics(policy) {
      ...
      const cadenceMs = policy.cadence === 'hourly' ? 3600000 :
                        policy.cadence === 'daily' ? dayMs :
                        policy.cadence === 'weekly' ? dayMs * 7 : dayMs * 30;
    ...
    async isVerificationOverdue(policy) {
      ...
      const now = Date.now();
      const cadenceMs = policy.cadence === 'hourly' ? 3600000 :
                        policy.cadence === 'daily' ? 86400000 :
                        policy.cadence === 'weekly' ? 604800000 : 2592000000;
    ...
    async getPostureTrend(periodDays) {
      ...
      const avgAnomalies = relevantSnapshots.length > 0
        ? relevantSnapshots.reduce((sum, s) => sum + s.anomaly_count, 0) / relevantSnapshots.length
        : 0;
      const avgDrifts = relevantSnapshots.length > 0
        ? relevantSnapshots.reduce((sum, s) => sum + s.policy_drift_count, 0) / relevantSnapshots.length
        : 0;
      return {
...
describe('Data Access Continuous Assurance Contracts', () => {
  ...
  describe('CONTRACT: assurance_scheduled_verification', () => {
    ...
    it('respects max duration policy', async () => {
      ...
      assert.ok(run.duration_ms <= policy.max_duration_ms,
  ...
  describe('CONTRACT: assurance_evidence_pack_generation', () => {
    ...
    it('includes summary without raw query text', async () => {
      ...
      assert.ok(['healthy', 'degraded', 'critical', 'unknown'].includes(pack.summary.posture_state));
    });
  ...
  describe('CONTRACT: assurance_posture_history', () => {
    it('captures posture snapshots with checksum chain', async () => {
      ...
      assert.strictEqual(snapshot2.previous_snapshot_id, snapshot1.snapshot_id,
