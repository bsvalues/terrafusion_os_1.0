...
> Project: AI Lab
> Feature: RAG Query Citations Contract
> Mode: feature | Risk: med | SpecLock: strict
> Created: 2025-12-17T07:53:19Z
...
5. **Commit discipline**: Commit after each significant increment, run gate.
   - Commit message format: `feat(ai-lab): <slice> [SESSION:20251217_075319Z_ai-lab_rag-query-citations-contract]`
   - Run: `tf ai status && tf gate`
...
### Git
- **Base branch**: main
- **Work branch**: feat/ai-lab/rag-query-citations-contract

### Gate command
```bash
tf ai status && tf gate
```
...
2. Paste diff into `PATCHLOG.md` with:
   ...
   - Fenced `diff` block
3. Run: `tf ai status && tf gate`
4. Commit: `git commit -m "feat(ai-lab): <slice> [SESSION:20251217_075319Z_ai-lab_rag-query-citations-contract]"`
5. Repeat
