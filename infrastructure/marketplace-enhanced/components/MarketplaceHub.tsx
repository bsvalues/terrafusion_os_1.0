...
import { Search, Grid, List, Filter, TrendingUp, Shield, 
  ...
  Download, Activity, BarChart3, Zap, Refresh
...
export const MarketplaceHub: React.FC<MarketplaceHubProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'discover' | 'dashboard' | 'installed' | 'security'>('discover');
  ...
  const loadMarketplaceData = async () => {
    ...
    try {
      ...
      if (countyProfile) {
        await analytics.addCounty(countyProfile);
      }
  ...
  const handlePluginViewDetails = (pluginId: string) => {
    // Track view event
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        ...
        <span className="ml-2 text-gray-600">Loading Terrafusion Marketplace...</span>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {activeTab === 'security' && (userRole === 'admin' || userRole === 'developer') && (
          <SecurityOverview
            plugins={plugins}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="mt-8 pt-8 border-t border-gray-200">
            <p className="text-sm text-gray-500 text-center">
              © 2025 Terrafusion County OS. All rights reserved. Powered by quantum-enhanced AI.
            </p>
          </div>
        </div>
      </footer>
...
const InstalledPluginsView: React.FC<{
  ...
  onSecurityScan?: (pluginId: string) => void;
}> = ({ plugins, onUninstall, onSecurityScan }) => (
  <div className="space-y-6">
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      {plugins.map(plugin => (
        <div key={plugin.id} className="bg-white rounded-lg border border-gray-200 p-6">
...
const SecurityOverview: React.FC<{
  ...
  securityScanner: SecurityScanner;
  userRole: string;
}> = ({ plugins, securityScanner, userRole }) => (
  <div className="space-y-6">
    <div className="flex items-center justify-between"><>
