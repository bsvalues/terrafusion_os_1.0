...
export interface GeniusNotificationProps {id: string;
...
export const GeniusNotification: React.FC<GeniusNotificationProps> = ({id,
  type,
  title,
  message,
  duration = 5000,
  ...
  animate = true,
  sound = true,
  haptic = true}) => {const [isVisible, setIsVisible] = useState(false);
  ...
  useEffect(() => {if (persistent || duration<= 0) return;
    ...
    const playNotificationSound = () => {
      try {
        ...
        notes.forEach((freq /* , index */) => {setTimeout(() => {
            ...
            osc.stop(audioContext.currentTime + 0.3);}, index * 100);
        });
      } catch (error) {// Graceful fallback - no sound
  ...
  useEffect(() => {if (!haptic || !('vibrate' in navigator)) return;
  ...
  const handleDismiss = () => {if (isExiting) return;
  ...
  const handleMouseEnter = () => {// Pause auto-dismiss on hover
    if (progressInterval.current) clearInterval(progressInterval.current);
    if (dismissTimeout.current) clearTimeout(dismissTimeout.current);};

  const handleMouseLeave = () => {// Resume auto-dismiss after hover
  ...
  const handleKeyDown = (event: React.KeyboardEvent) =>{if (event.key === 'Escape') {
      handleDismiss();}
  };

  const getIcon = () => {if (icon) return icon;
  ...
  return (<div
    ...
    >{/* Progress indicator */}
      {!persistent && duration > 0 && (<div className="genius-notification__progress"><div 
            className="genius-notification__progress-bar"
            style={{ width: `${progress}%` }} /></div>)}

      {/* Main content */}<div className="genius-notification__content">{/* Icon */}<div className="genius-notification__icon">{getIcon()}</div>{/* Text content */}<div className="genius-notification__text"><><h4 
          ...
          >
            {message}</p></div>{/* Actions */}
        {actions.length > 0 && (<div className="genius-notification__actions">{actions.map((action /* , index */) => (<button
                key={index}
        ...
        {/* Dismiss button */}<button
          ...
          title="Dismiss"
        >×</button></div>{/* Celebration particles */}
      {type === 'celebration' && (<div className="genius-notification__particles">{Array.from({length: 12}).map((_, i) => (<div
              key={i}
