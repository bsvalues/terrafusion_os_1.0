...
interface HarrisPacsJurisdiction {
  id: string;
  name: string;
  state: string;
  county: string;
...
interface HarrisPacsSyncStatus {
  jurisdiction: string;
  lastSync: string;
...
interface HarrisPacsSystemStatus {
  isOnline: boolean;
  version: string;
...
const HarrisPacsIntegrationDashboard: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      setError('Failed to load Harris PACS integration data');
      console.error('Error fetching Harris PACS data:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSyncJurisdiction = async (jurisdictionId: string) => {
    try {
      const response = await fetch(`/api/harris-pacs/jurisdictions/${jurisdictionId}/sync`, {
        method: 'POST'
      });
      
      if (response.ok) {
        // Refresh sync status after initiating sync
        setTimeout(() => fetchJurisdictions(), 2000);
        setSyncDialogOpen(false);
      }
    } catch (err) {
      console.error('Error syncing jurisdiction:', err);
    }
  };
            ...
            <Card>
              <CardContent>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                  <Storage sx={{ mr: 1, color: systemStatus.isOnline ? '#4caf50' : '#f44336' }} />
                  <Typography variant="h6">System Status</Typography>
                </Box><>

                <Typography variant="h4" sx={{ color: systemStatus.isOnline ? '#4caf50' : '#f44336' }}>
...
variant="body2" color="text.secondary">
          ...
          <Grid item xs={12} md={3}>
            <Card>
              <CardContent>
                <Box sx={{ display: 'flex', alignItems: 'center', mb: 2 }}>
                  <AccountBalance sx={{ mr: 1, color: '#f57c00' }} />
                  ...
                  <Table stickyHeader>
                    ...
                    <TableBody>
                      {properties.slice(0, 20).map((property) => (
                        <TableRow key={property.parcelId}><>
      ...
      <Dialog open={syncDialogOpen} onClose={() => setSyncDialogOpen(false)} maxWidth="sm" fullWidth><>
        ...
        <DialogContent
          ...
          <FormControl fullWidth sx={{ mt: 1 }}><>
            ...
            >
              {jurisdictions.filter(j => j.isActive).map((jurisdiction) => (
                <MenuItem key={jurisdiction.id} value={jurisdiction.id}>
                  {jurisdiction.name} ({jurisdiction.county}, {jurisdiction.state})
                </MenuItem>
              ))}
            </Select>
          </FormControl>
