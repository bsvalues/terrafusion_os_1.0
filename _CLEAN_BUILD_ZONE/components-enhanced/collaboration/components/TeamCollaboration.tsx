...
interface TeamCollaborationProps extends CollaborationComponentProps {
  teams: Team[];
  projects: Project[];
  showMetrics?: boolean;
}
...
export const TeamCollaboration: React.FC<TeamCollaborationProps> = ({
  ...
  const teamProjects = useMemo(() => {
    return projects.filter(p => p.team.id === selectedTeamId);
  ...
  const teamMetrics = useMemo(() => {
    ...
    return {
      totalMembers: selectedTeam.members.length,
      activeProjects,
      completedProjects,
  ...
  const addMemberMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(['collaboration-teams']);
      toast({
        title: 'Member Added',
  ...
  const getPerformanceBadgeVariant = (completionRate: number) => {
  ...
  const MemberCard: React.FC<{ member: CollaborationUser; stats?: any }> = ({ member, stats }) => (
...
className="h-4 w-4 text-muted-foreground" />
                  ...
                  <CardContent>
                    <div className="text-2xl font-bold">
                      {teamMetrics?.totalTasks - teamMetrics?.completedTasks || 0}
                    </div>
                  </CardContent>
                </Card>
              ...
              <Card>
                ...
                <CardContent>
                  {teamProjects.length === 0 ? (
                    <p className="text-muted-foreground text-sm">No projects assigned to this team</p>
                  ) : (
...
className="text-xs text-muted-foreground">{project.description}</p>
                      ...
                      {teamProjects.length > 3 && (
                        <p className="text-xs text-muted-foreground text-center pt-2">
                          And {teamProjects.length - 3} more projects...
                        </p>
                      )}
                    </div>
                  )}
...
className="flex items-center gap-2">
                          <div className="w-32 bg-secondary rounded-full h-2"><>
                            ...
                            <div
                              ...
                              style={{ width: `${teamMetrics.productivity}%` }}
                            />
                          </div>
                          <span
</>
...
className="text-sm font-medium">{teamMetrics.completionRate}%</span>
                ...
                <Card>
                  <CardHeader>
                    <CardTitle className="text-sm">Member Performance</CardTitle>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      {teamMetrics.memberStats.map((stats) => (
                        <div key={stats.member.id} className="flex items-center gap-4 p-3 border rounded">
                          <Avatar className="h-8 w-8">
                            <AvatarImage src={stats.member.avatar} />
...
className="flex items-center gap-4 text-xs text-muted-foreground"><>
                          ...
                          <Badge variant={getPerformanceBadgeVariant(stats.completionRate)}>
                            {stats.completionRate}%
                          </Badge>
                        </div>
                      ))}
