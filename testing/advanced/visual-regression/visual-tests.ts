...
test.describe('Visual Regression - Government UI Standards', () => {
  ...
  test('property assessment form visual consistency', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="assessment-form"]')).toHaveScreenshot(
      'assessment-form.png'
    );
  });
  ...
  test('AI swarm dashboard visual stability', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="ai-swarm-dashboard"]')).toHaveScreenshot(
      'ai-swarm-dashboard.png'
    );
  });
