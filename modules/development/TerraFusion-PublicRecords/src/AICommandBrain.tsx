...
className="text-sm text-slate-400">Active Models</div>
          ...
          <div className="bg-slate-800/50 rounded-lg p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-blue-400">{aiStatus.accuracyRate.toFixed(1)}%</span>
            </div><>
            ...
            <div key={capability.id} className="bg-slate-800/50 rounded-xl border border-slate-700 p-4">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className={`p-2 bg-slate-900 rounded-lg ${capability.color}`}><>
...
className="text-sm text-slate-400 mb-2">{prediction.impact}</p>
                ...
                <button className="ml-4 px-3 py-1.5 bg-purple-600 hover:bg-purple-700 rounded-lg text-sm transition-colors">
                  Apply
                </button>
              </div>
            </div>
      ...
      <div>
        <h3 className="text-xl font-bold mb-4 flex items-center gap-2"><>
          ...
          <Command className="text-cyan-400" />
          Active Automations
        </h3>
        <div
</>
...
className="px-4 py-3 text-left text-xs font-medium text-slate-400 uppercase">Time Saved</th>
            ...
            <tbody className="divide-y divide-slate-700">
              {automations.map(automation => (
                <tr key={automation.id} className="hover:bg-slate-800/50 transition-colors">
...
className="px-4 py-3">
                  ...
                  <td className="px-4 py-3">
                    <span className="text-sm text-green-400">{automation.savedTime}</span>
                  </td>
                  <td className="px-4 py-3">
                    <button className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                      automation.status === 'active' 
                        ? 'bg-green-900/30 text-green-400' 
                        : 'bg-yellow-900/30 text-yellow-400'
                    }`}>
                      {automation.status === 'active' ? <Unlock size={12} /> : <Lock size={12} />}
                      {automation.status}
                    </button>
                  </td>
                </tr>
