...
import { 
  parallelRealityEngine, 
  RealityDimension, 
  ParallelComparison,
  QuantumDecisionAnalysis
} from '../engines/ParallelRealityEngine';

interface RealityVisualizationProps {
 ...
 */
const ParallelRealityVisualization3D: React.FC<RealityVisualizationProps> = ({
  dimensions,
  selectedDimensions,
  quantumCoherence,
  onDimensionSelect,
  ...
  const getRealityPosition = (index: number, total: number) => {
  ...
  return (
    <group ref={groupRef}>
      ...
      {dimensions.map((dimension, index) => {
        ...
        return (
          <group key={dimension.id} position={position}>
            {/* Main reality sphere */}
            <mesh
            ...
            >
              <sphereGeometry args={[
                isBaseline ? 1.2 : 0.8 + dimension.probability * 0.4,
                16, 
                16
              ]} />
              <meshStandardMaterial
                ...
                transparent
                opacity={isBaseline ? 1.0 : 0.7 + dimension.quantumState.coherence * 0.3}
              />
            </mesh>
            ...
            {dimension.timeline[dimension.timeline.length - 1]?.quantumProperties?.superposition && (
              <group>
            ...
            {/* Outcome performance bars */}
            <group position={[0, -2, 0]}>
              <mesh position={[-0.6, 0, 0]}>
                <cylinderGeometry args={[0.05, 0.05, dimension.outcomes.citizenSatisfaction / 50, 8]} />
                <meshStandardMaterial color="#00ff00" transparent opacity={0.7} />
              </mesh>
              <mesh position={[-0.2, 0, 0]}>
                <cylinderGeometry args={[0.05, 0.05, dimension.outcomes.governmentEfficiency / 50, 8]} />
                <meshStandardMaterial color="#0099ff" transparent opacity={0.7} />
              </mesh>
              <mesh position={[0.2, 0, 0]}>
                <cylinderGeometry args={[0.05, 0.05, dimension.outcomes.budgetPerformance / 50, 8]} />
                <meshStandardMaterial color="#ff9900" transparent opacity={0.7} />
              </mesh>
              <mesh position={[0.6, 0, 0]}>
                <cylinderGeometry args={[0.05, 0.05, dimension.outcomes.technologicalAdvancement / 50, 8]} />
                <meshStandardMaterial color="#ff00ff" transparent opacity={0.7} />
              </mesh>
            </group>
            ...
            <Billboard position={[0, -3, 0]}>
              <Text
                ...
                anchorY="middle"
              >
                P = {(dimension.probability * 100).toFixed(1)}%
              </Text>
            </Billboard>
          </group>
        );
      ...
      {Array.from({ length: 8 }, (_, i) => (
        ...
        >
          <torusGeometry args={[8 + i * 1.5, 0.02, 8, 32]} />
          <meshBasicMaterial
            color="#00ffee"
            transparent
      ...
      <mesh position={[0, 8, 0]} rotation={[0, time * 0.5, 0]}>
 ...
 */
const RealityDetailsPanel: React.FC<{
  dimensions: RealityDimension[];
  selectedDimensions: string[];
  onDimensionCreate: (name: string, decision: string) => void;
  onQuantumCollapse: (dimensionId: string) => void;
}> = ({ dimensions, selectedDimensions, onDimensionCreate, onQuantumCollapse }) => {
      ...
      <CardContent>
            ...
            <Grid container spacing={2}>
              <Grid item xs={12}>
                <FormControl fullWidth size="small">
                  <InputLabel sx={{ color: 'white' }}>Key Decision</InputLabel>
                  <Select
        ...
        {selectedDimension && (
          <Box>
            ...
            {/* Quantum state */}
            <Box sx={{ mb: 2, p: 2, backgroundColor: 'rgba(0,255,238,0.1)', borderRadius: 1 }}>
              <Typography variant="subtitle2" gutterBottom>
                ⚛️ Quantum State
              </Typography>
              <Grid container spacing={2}>
                <Grid item xs={6}>
                  <Typography variant="caption" color="grey.400">Coherence</Typography>
                  <LinearProgress
                    variant="determinate"
                    value={selectedDimension.quantumState.coherence * 100}
                    sx={{ '& .MuiLinearProgress-bar': { backgroundColor: '#00ffee' } }}
                  />
                  <Typography variant="body2">{(selectedDimension.quantumState.coherence * 100).toFixed(1)}%</Typography>
                </Grid>
                <Grid item xs={6}>
                  <Typography variant="caption" color="grey.400">Stability</Typography>
                  <LinearProgress
                    variant="determinate"
                    value={selectedDimension.quantumState.stability}
                    sx={{ '& .MuiLinearProgress-bar': { backgroundColor: '#00ff00' } }}
                  />
                  <Typography variant="body2">{selectedDimension.quantumState.stability.toFixed(1)}%</Typography>
                </Grid>
              </Grid>
            </Box>
            ...
            <Box sx={{ mb: 2 }}>
              ...
              <Grid container spacing={1}>
                {Object.entries(selectedDimension.outcomes).map(([metric, value]) => (
                  <Grid item xs={6} key={metric}>
                    <Typography variant="caption" color="grey.400">
                      {metric.replace(/([A-Z])/g, ' $1').replace(/^./, str => str.toUpperCase())}
                    </Typography>
                    <Typography variant="body2" color={
                      value > 80 ? '#00ff00' : value > 60 ? '#ffff00' : value > 40 ? '#ff8800' : '#ff4400'
                    }>
                      {value.toFixed(1)}%
                    </Typography>
                  </Grid>
            ...
            {selectedDimension.quantumState.entanglement.length > 0 && (
              <Box sx={{ mb: 2 }}>
                <Typography variant="subtitle2" gutterBottom>
                  🔗 Quantum Entanglements
                </Typography>
                <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 1 }}>
                  {selectedDimension.quantumState.entanglement.map(entanglement => (
                    <Chip
                      key={entanglement}
            ...
            {selectedDimension.timeline[selectedDimension.timeline.length - 1]?.quantumProperties?.superposition && (
              <Box sx={{ mb: 2, p: 1, backgroundColor: 'rgba(255,0,255,0.1)', borderRadius: 1 }}>
                <Typography variant="subtitle2" color="#ff00ff">
                  ⚡ Quantum Superposition Active
                </Typography>
                <Typography variant="body2" color="grey.400">
                  This reality exists in multiple simultaneous states until observed
                </Typography>
                <Button
                  variant="outlined"
                  size="small"
      ...
      <Card sx={{ background: 'rgba(0, 0, 0, 0.8)', color: 'white' }}>
        <CardContent>
          <Typography variant="h6">
            Select multiple dimensions to compare
          </Typography>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card sx={{ background: 'rgba(0, 0, 0, 0.8)', color: 'white' }}>
      <CardContent>
        ...
        <Box sx={{ mb: 3, p: 2, backgroundColor: 'rgba(0,255,0,0.1)', borderRadius: 1 }}>
          <Typography variant="subtitle1" color="#00ff00" gutterBottom>
            🎯 Optimal Reality Path
          </Typography>
          <Typography variant="h6">
            {dimensions.find(d => d.id === comparison.optimalPath.dimensionId)?.name || 'Unknown'}
          </Typography>
          <Typography variant="body2" color="grey.400" sx={{ mb: 1 }}>
            Confidence: {(comparison.optimalPath.confidence * 100).toFixed(1)}%
          </Typography>
          <List dense>
            {comparison.optimalPath.reasoning.map((reason, index) => (
        ...
        {Object.entries(comparison.metrics).map(([dimensionId, metrics]) => {
              ...
              <AccordionSummary expandIcon={<ExpandMoreIcon sx={{ color: 'white' }} />}>
                <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                  ...
                  <Chip
                    ...
                    size="small"
                    color={
                      metrics.performance > 80 ? 'success' :
                      metrics.performance > 60 ? 'warning' :
                      'error'
                    }
                  />
                </Box>
 ...
 */
export const ParallelRealityVisualization: React.FC = () => {
  const [dimensions, setDimensions] = useState<RealityDimension[]>([]);
  ...
  useEffect(() => {
    const initializeEngine = async () => {
      ...
      const handleDimensionCreated = (dimension: RealityDimension) => {
        setDimensions(prev => [...prev, dimension]);
      };

      const handleRealityUpdate = (dimension: RealityDimension) => {
        setDimensions(prev => prev.map(d => d.id === dimension.id ? dimension : d));
      };
      ...
      return () => {
        parallelRealityEngine.off('baselineInitialized', handleBaselineInitialized);
  ...
  const handleQuantumCollapse = async (dimensionId: string) => {
    await parallelRealityEngine.collapseQuantumState(dimensionId, 'OBSERVED');
    // Refresh dimensions
    const updatedDimensions = parallelRealityEngine.getActiveDimensions();
    setDimensions(updatedDimensions);
  };

  const handleCompareRealities = async () => {
    if (selectedDimensions.length >= 2) {
      const comparisonResult = await parallelRealityEngine.compareParallelDimensions(selectedDimensions);
  ...
  const toggleSimulation = () => {
    if (isSimulationActive) {
      parallelRealityEngine.stopParallelSimulation();
    } else {
      parallelRealityEngine.startParallelSimulation();
    }
    ...
    <Box sx={{ width: '100%', height: '100vh', display: 'flex', backgroundColor: 'black' }}>
      ...
      <Box sx={{ flex: 1, position: 'relative' }}>
        <Canvas camera={{ position: [0, 8, 20], fov: 60 }}>
          ...
          <pointLight position={[-15, -15, -15]} />
          <spotLight position={[0, 25, 0]} angle={0.3} penumbra={1} castShadow />
          
          <ParallelRealityVisualization3D
            dimensions={dimensions}
            selectedDimensions={selectedDimensions}
            quantumCoherence={quantumCoherence}
            onDimensionSelect={handleDimensionSelect}
            onDimensionCompare={handleCompareRealities}
          />
        ...
        <Box sx={{
          ...
          <Typography variant="subtitle1" color="grey.400">
            Explore infinite government possibilities across quantum dimensions
          </Typography>
          <Typography variant="body2" color="grey.500" sx={{ mt: 1 }}>
            Active Realities: {dimensions.length} | Selected: {selectedDimensions.length}
          </Typography>
        </Box>
        ...
        <Box sx={{
          ...
          >
            {isSimulationActive ? '⏸️ Pause' : '▶️ Start'} Quantum Simulation
          </Button>
