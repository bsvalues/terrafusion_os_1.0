...
function updateServicesGrid(services) {
  ...
  Object.entries(services).forEach(([serviceId, service]) => {
    ...
    const cardClass =
      service.status === 'healthy' ? '' : service.status === 'unhealthy' ? 'warning' : 'critical';
...
function viewLogs() {
  ...
  const logsPromises = Object.keys(servicesData).map(serviceId => $.get(`/api/logs/${serviceId}`));
...
function showSystemInfo() {
  ...
  $('#system-info-content').html(
    '<div class="text-center"><i class="fas fa-spinner fa-spin"></i> Preparing transcendence...</div>'
  );
