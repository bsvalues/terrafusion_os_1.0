...
if lsof -i:9999 > /dev/null 2>&1; then
    echo -e "${RED}Dashboard API is already running on port 9999${NC}"
...
if curl -s http://localhost:9999/api/metrics/current > /dev/null; then
    ...
    echo ""
    echo "API Endpoints:"
    echo "  http://localhost:9999/api/metrics/current"
    echo "  http://localhost:9999/api/services/health"
    echo "  http://localhost:9999/api/activities/recent"
    echo ""
