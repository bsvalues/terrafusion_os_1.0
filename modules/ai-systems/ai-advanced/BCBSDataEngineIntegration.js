...
exports.bcbsDataEngine = exports.BCBSDataEngine = void 0;
const events_1 = require("events");
const MCPIntegrationHub_1 = require("./MCPIntegrationHub");
class BCBSDataEngine extends events_1.EventEmitter {
    dataSources = new Map();
    ...
    async initialize() {
        ...
        await MCPIntegrationHub_1.mcpIntegrationHub.initialize();
    ...
    async executePipeline(pipelineId) {
        ...
        try {
            ...
            const orchestrationResult = await MCPIntegrationHub_1.mcpIntegrationHub.orchestrateAnalysis(aiTask);
            ...
            pipeline.nextRun = pipeline.schedule
                ? this.calculateNextRun(pipeline.schedule.cron)
                : undefined;
     ...
     * Extract insights from AI analysis
     */
    extractInsightsFromAnalysis(orchestrationResult, pipelineId) {
        const insights = [];
     ...
     */
    discoverRevenueOpportunities(orchestrationResult, pipelineId) {
        const opportunities = [];
     ...
     */
    generateInsightRecommendations(type) {
     ...
     */
    async discoverAutomaticInsights() {
        // Skip if no connected sources
        const connectedSources = Array.from(this.dataSources.values()).filter(source => source.status === 'connected');
        ...
        try {
            const result = await MCPIntegrationHub_1.mcpIntegrationHub.orchestrateAnalysis(aiTask);
    ...
    getStatus() {
        const connectedSources = Array.from(this.dataSources.values()).filter(source => source.status === 'connected');
        const activePipelines = Array.from(this.dataPipelines.values()).filter(pipeline => pipeline.status === 'running');
        return {
            dataSources: {
            ...
            insights: {
                ...
                recent: this.insights.filter(i => Date.now() - i.metadata.timestamp.getTime() < 24 * 60 * 60 * 1000).length,
            },
            opportunities: {
...
exports.BCBSDataEngine = BCBSDataEngine;
// Export singleton instance
exports.bcbsDataEngine = new BCBSDataEngine();
//# sourceMappingURL=BCBSDataEngineIntegration.js.map