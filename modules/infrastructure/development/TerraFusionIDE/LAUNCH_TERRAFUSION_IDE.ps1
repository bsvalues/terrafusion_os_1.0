...
if ($processes) {
    ...
    if ($netstatOutput) {
        $osRunning = $true
        Write-Host "✅ TerraFusion OS detected running on port 5000" -ForegroundColor Green
    }
}
...
Write-Host "  🖥️  TerraFusion OS:     http://localhost:3003" -ForegroundColor White
Write-Host "  🏛️  TerraFusion API:    http://localhost:5000" -ForegroundColor White  
Write-Host "  💻  TerraFusion IDE:    http://localhost:5173" -ForegroundColor White
Write-Host ""
