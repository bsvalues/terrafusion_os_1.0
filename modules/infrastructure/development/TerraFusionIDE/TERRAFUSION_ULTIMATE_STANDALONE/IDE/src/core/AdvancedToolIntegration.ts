...
export class AdvancedToolIntegration extends EventEmitter {
  ...
  private async databaseSetupTool(parameters: Record<string, any>): Promise<any> {
    ...
    return {
      ...
      connectionString: `${parameters.databaseType}://localhost:\${{TF_POSTGRES_PORT:-5432}}/terrafusion`,
      environmentVariables: {
        DATABASE_URL: `${parameters.databaseType}://localhost:\${{TF_POSTGRES_PORT:-5432}}/terrafusion`,
        DB_HOST: 'localhost',
        DB_PORT: '5432',
        DB_NAME: 'terrafusion',
      },
