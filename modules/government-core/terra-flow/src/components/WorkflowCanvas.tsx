...
export const WorkflowCanvas: React.FC<WorkflowCanvasProps> = ({ className }) => {
  ...
  const completeConnection = useCallback((toNodeId: string, toInputId: string) => {
    ...
    return (
      ...
      >
        ...
        <div className={`p-3 rounded-t-lg ${
          ...
          nodeTypeInfo?.color || 'bg-gray-500'
        } text-white`}>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              {nodeTypeInfo?.icon && <nodeTypeInfo.icon className="w-4 h-4" />}
  ...
  return (
    <div className={`workflow-canvas relative h-full bg-gray-50 overflow-hidden ${className || ''}`}>
      {/* Canvas */}
      <div 
        ref={canvasRef}
      ...
      {/* Toolbar */}
      <div className="absolute top-4 left-4 bg-white rounded-lg shadow-lg border border-gray-200 p-2 z-40">
      ...
      {/* Instructions */}
      {nodes.length === 1 && (
        <div className="absolute bottom-4 left-4 bg-blue-50 border border-blue-200 rounded-lg p-4 max-w-md">
          <h4 className="font-medium text-blue-900 mb-2">Getting Started</h4>
          <ul className="text-sm text-blue-700 space-y-1">
            <li>• Double-click to add nodes</li>
            <li>• Drag nodes to reposition</li>
            <li>• Click output dots to start connections</li>
            <li>• Click input dots to complete connections</li>
          </ul>
        </div>
      )}
    </div>
