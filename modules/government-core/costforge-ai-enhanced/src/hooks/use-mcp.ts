...
export function useMCP() {
  ...
  const predictCost = useCallback(async (request: CostPredictionRequest): Promise<CostPredictionResponse> => {
    setIsLoading(true);
    ...
    try {
      ...
      const adjustedCostPerSqFt = baseCostPerSqFt * regionMultiplier * conditionMultiplier * complexityMultiplier;
...
function getEstimatedTimeline(squareFootage: number, buildingType: string): string {
  ...
  const typeMultiplier = buildingType === 'Healthcare' ? 1.5 : 
                        buildingType === 'Educational' ? 1.3 : 1.0;
