...
export default function LegalDescriptionAgentPage() {
  const [descriptions, setDescriptions] = useState<LegalDescription[]>([]);
  ...
  const generateLegalDescription = async () => {
    if (!generationRequest.propertyId) {
    ...
    try {
      ...
      alert('Failed to generate legal description. Please try again.');
    } finally {
  ...
  const simulateAIGeneration = async (request: GenerationRequest): Promise<LegalDescription> => {
    ...
    const newDescription: LegalDescription = {
      ...
      method: request.generationMethod === 'ai-assisted' ? 'ai-generated' : 
               request.generationMethod === 'template-based' ? 'manual' : 'hybrid',
      status: 'draft',
      metadata: {
        parcelNumber: request.parcelNumber || `${Math.floor(Math.random() * 999) + 100}-${Math.floor(Math.random() * 999) + 100}-${Math.floor(Math.random() * 999) + 100}`,
  ...
  const validateLegalDescription = async (description: LegalDescription): Promise<ValidationResult[]> => {
    ...
    if (!description.description.includes('County')) {
      results.push({
        id: 'format-2',
        type: 'format',
        status: 'error',
    ...
    if (!description.metadata.parcelNumber) {
      results.push({
        id: 'completeness-1',
        type: 'completeness',
        status: 'warning',
    ...
    // Legal validation
    if (!description.description.includes('more particularly described')) {
      results.push({
        id: 'legal-1',
        type: 'legal',
        status: 'warning',
  ...
  // Update statistics
  const updateStats = () => {
    const completed = descriptions.filter(d => d.status !== 'draft');
  ...
  const filteredDescriptions = descriptions.filter(desc => {
    if (filterStatus && desc.status !== filterStatus) return false;
    if (searchTerm && !desc.description.toLowerCase().includes(searchTerm.toLowerCase()) &&
        !desc.propertyId.toLowerCase().includes(searchTerm.toLowerCase())) return false;
    return true;
  });
  ...
  const updateDescriptionStatus = (descriptionId: string, newStatus: LegalDescription['status']) => {
  ...
  // Get status color
  const getStatusColor = (status: LegalDescription['status']) => {
    switch (status) {
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">AI-powered legal description generation and validation</p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="outline">{descriptions.length} Descriptions</Badge>
          <Badge variant="outline">{Math.round(stats.accuracyRate)}% Accuracy</Badge>
        </div>
      </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{Math.round(stats.validationSuccessRate)}%</p>
              </div>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setGenerationRequest(prev => ({ ...prev, propertyId: e.target.value }))}
                  />
                </div>
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setGenerationRequest(prev => ({ ...prev, parcelNumber: e.target.value }))}
                  />
                </div>
                ...
                <div>
                  ...
                  <Select value={generationRequest.generationMethod} onValueChange={(value: any) => 
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="coordinate-based">Coordinate Based</SelectItem>
                      <SelectItem value="parcel-based">Parcel Based</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
                ...
                <div>
                  ...
                  <Select value={generationRequest.priority} onValueChange={(value: any) => 
                ...
                <div className="flex items-center space-x-2">
                  <input
                    ...
                    onChange={(e) => setGenerationRequest(prev => ({ ...prev, includeValidation: e.target.checked }))}
                  />
                  <Label htmlFor="includeValidation">Include automatic validation</Label>
                </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    Legal descriptions are legally binding documents. Always have them reviewed by a qualified professional before use in official transactions.
                  </AlertDescription>
                </Alert>
              </CardContent>
            </Card>
                ...
                {filteredDescriptions.map(description => (
                  <div
                    ...
                    className={`p-4 border rounded-lg cursor-pointer transition-colors ${
                      selectedDescription?.id === description.id ? 'border-blue-500 bg-blue-50' : 'hover:border-gray-300'
                    }`}
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-2">
                          <h3 className="font-medium">Property: {description.propertyId}</h3>
                          <Badge variant="outline" className={getStatusColor(description.status)}>
                            {description.status}
                          </Badge>
                          <Badge variant="secondary">{description.method}</Badge>
                        </div>
                        <p className="text-sm text-muted-foreground line-clamp-2 mb-2">
                          {description.description}
                        </p>
                        <div className="flex items-center gap-4 text-xs text-muted-foreground">
                          <span>Generated: {description.generatedAt.toLocaleDateString()}</span>
                          <span>Accuracy: {description.accuracy}%</span>
                          <span>Confidence: {description.confidence}%</span>
                          {description.metadata.acreage && (
                            <span>Acreage: {description.metadata.acreage}</span>
                          )}
                        </div>
                      </div>
                      <div className="flex items-center gap-2 ml-4">
                        ...
                        <Select value={description.status} onValueChange={(value: any) => {
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground">
                    {descriptions.length === 0 
                      ...
                      : "No descriptions match your current filters."
                    }
                  </p>
                </div>
              )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <div className="mt-2 space-y-2 text-sm">
                      <div>Parcel: {selectedDescription.metadata.parcelNumber || 'N/A'}</div>
                      <div>County: {selectedDescription.metadata.county}, {selectedDescription.metadata.state}</div>
                      <div>Township: {selectedDescription.metadata.township || 'N/A'}</div>
                      <div>Range: {selectedDescription.metadata.range || 'N/A'}</div>
                      <div>Section: {selectedDescription.metadata.section || 'N/A'}</div>
                      {selectedDescription.metadata.acreage && (
                        <div>Acreage: {selectedDescription.metadata.acreage}</div>
                      )}
                      {selectedDescription.metadata.zoning && (
                        <div>Zoning: {selectedDescription.metadata.zoning}</div>
                      )}
                    </div>
                  </div>

                  <div>
                    <Label>Generation Statistics</Label>
                    <div className="mt-2 space-y-2 text-sm">
                      <div>Generated: {selectedDescription.generatedAt.toLocaleString()}</div>
                      <div>Method: {selectedDescription.method}</div>
                      <div>Status: {selectedDescription.status}</div>
                      <div>
                        Accuracy: 
                        <Progress value={selectedDescription.accuracy} className="inline-block w-20 ml-2 mr-2" />
                        {selectedDescription.accuracy}%
                      </div>
                      <div>
                        Confidence: 
                        <Progress value={selectedDescription.confidence} className="inline-block w-20 ml-2 mr-2" />
                        {selectedDescription.confidence}%
                      </div>
                    </div>
                  </div>
                </div>

                {selectedDescription.metadata.coordinates && (
                  <div>
                    <Label>Coordinates</Label>
                    <div className="flex items-center gap-4 mt-2 text-sm">
                      <span>Lat: {selectedDescription.metadata.coordinates.lat.toFixed(6)}</span>
                      <span>Lng: {selectedDescription.metadata.coordinates.lng.toFixed(6)}</span>
            ...
            <CardContent>
              {selectedDescription ? (
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <h3 className="text-lg font-medium">
                      Validation for Property: {selectedDescription.propertyId}
                    </h3>
                    <Button onClick={() => validateLegalDescription(selectedDescription).then(setValidationResults)}>
                  ...
                  {validationResults.length > 0 ? (
                    <div className="space-y-3">
                      {validationResults.map(result => (
                        <Alert key={result.id} className={`border-l-4 ${
                    ...
                    <div className="text-center py-8">
                      ...
                      <p className="text-muted-foreground">
                        Run validation to check the quality and accuracy of this legal description.
                      </p>
                      <Button 
                        className="mt-4"
                ...
                <div className="text-center py-8">
                  ...
                  <h3 className="text-lg font-medium mb-2">No Description Selected</h3>
                  <p className="text-muted-foreground">
                    Select a legal description from the Descriptions tab to view validation results.
                  </p>
                </div>
              )}
            </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{Math.round(stats.accuracyRate)}%</span>
                </div>
                <div className="flex justify-between">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{Math.round(stats.validationSuccessRate)}%</span>
                </div>
              </CardContent>
            </Card>
          </div>
            ...
            <CardContent>
              <div className="space-y-3">
                {descriptions.slice(0, 10).map(desc => (
                  <div key={desc.id} className="flex items-center gap-3 py-2 border-b last:border-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium">Property: {desc.propertyId}</p>
                      <p className="text-xs text-muted-foreground">
                        {desc.method} • {desc.accuracy}% accuracy • {desc.generatedAt.toLocaleDateString()}
                      </p>
                    </div>
                    <Badge variant="outline" className={getStatusColor(desc.status)}>
                      {desc.status}
                    </Badge>
                  </div>
                ))}
