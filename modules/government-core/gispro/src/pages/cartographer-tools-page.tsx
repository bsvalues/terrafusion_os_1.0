...
export default function CartographerToolsPage() {
  ...
  const getCategoryTools = (category: CartographyTool['category']) => {
    return cartographyTools.filter(tool => tool.category === category);
  };

  const getCategoryIcon = (category: CartographyTool['category']) => {
    switch (category) {
      case 'drawing':
    ...
    >
      ...
      >
        <h1 className="text-4xl font-bold text-foreground">
          Professional Cartographer Tools
        </h1>
        <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
          Advanced mapping and surveying tools for precision cartography, 
          boundary analysis, and professional geospatial data creation.
        </p>
      ...
      <motion.div 
        ...
        variants={cardVariants}
      >
        {['drawing', 'measurement', 'editing', 'analysis'].map((category) => (
          <Card key={category}>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-lg">
                {getCategoryIcon(category as CartographyTool['category'])}
                {category.charAt(0).toUpperCase() + category.slice(1)} Tools
              </CardTitle>
              <CardDescription>
                {category === 'drawing' && 'Create geometric features and annotations'}
                {category === 'measurement' && 'Precise distance, area, and angle calculations'}
                {category === 'editing' && 'Modify and transform existing features'}
                {category === 'analysis' && 'Advanced spatial analysis and calculations'}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <motion.div 
                ...
                animate="visible"
              >
                {getCategoryTools(category as CartographyTool['category']).map((tool) => (
                  <motion.div
                    key={tool.id}
      ...
      >
        ...
        <div className="xl:col-span-3">
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <CardDescription>
                    Interactive cartography workspace with precision tools
                  </CardDescription>
                </div>
            ...
            <CardContent>
              <div className="relative bg-gradient-to-br from-gray-50 to-blue-50 rounded-lg h-96 border overflow-hidden">
                {/* Grid background */}
                {workspaceSettings.snapToGrid && (
                  <div 
                ...
                {/* Coordinates display */}
                {workspaceSettings.showCoordinates && (
                  <div className="absolute top-2 left-2 bg-black/75 text-white text-xs px-2 py-1 rounded">
                    Current Tool: {cartographyTools.find(t => t.id === selectedTool)?.name}
                  </div>
                )}
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-lg">Tool Properties</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <label className="text-sm font-medium mb-2 block">Selected Tool</label>
                <div className="p-3 border rounded-lg bg-muted/50">
                  <div className="flex items-center gap-2 mb-1">
                    {cartographyTools.find(t => t.id === selectedTool)?.icon}
                    <span className="font-medium">
                      {cartographyTools.find(t => t.id === selectedTool)?.name}
                    </span>
                  </div>
                  <p className="text-xs text-muted-foreground">
                    {cartographyTools.find(t => t.id === selectedTool)?.description}
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-3">
                <div>
                  <label className="text-sm font-medium mb-1 block">Precision</label>
                  <Input 
                    type="number" 
                ...
                <div className="space-y-2">
                  <Button 
                    ...
                    size="sm"
                    className="w-full"
                    onClick={() => setWorkspaceSettings(prev => ({ 
                      ...prev, 
            ...
            <CardContent>
              <div className="space-y-2">
                {mapFeatures.length === 0 ? (
                  ...
                  mapFeatures.map((feature) => (
                    <div key={feature.id} className="flex items-center justify-between p-2 border rounded text-xs">
                      <div>
                        <div className="font-medium">{feature.properties.name || feature.type}</div>
                        <div className="text-muted-foreground">{feature.type}</div>
                      </div>
      ...
      <Dialog open={jsonDialogOpen} onOpenChange={setJsonDialogOpen}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              Export your cartography work or import existing GeoJSON data
            </DialogDescription>
          </DialogHeader>

          <Tabs defaultValue="json" value={exportTab} onValueChange={setExportTab}>
            <TabsList className="grid w-full grid-cols-2">
              <TabsTrigger value="json">Export GeoJSON</TabsTrigger>
              <TabsTrigger value="import">Import</TabsTrigger>
            </TabsList>

            <TabsContent value="json" className="space-y-4">
              <div>
                <label className="text-sm font-medium mb-2 block">GeoJSON Output</label>
                <Textarea
            ...
            <TabsContent value="import" className="space-y-4">
              <div>
                <label className="text-sm font-medium mb-2 block">Import GeoJSON</label>
                <Textarea
                  value={importData}
      ...
      <Dialog open={showTutorial} onOpenChange={setShowTutorial}>
        <DialogContent className="max-w-4xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>Cartographer Tools Tutorial</DialogTitle>
            <DialogDescription>
              Learn how to use professional cartography tools for precision mapping
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="space-y-4 mt-4">
            <div className="border-l-4 border-blue-500 pl-4 py-2 mb-6 bg-blue-50 rounded">
              ...
              <p className="text-sm text-blue-800 mt-1">
                Select tools from the categories above and start creating precise geospatial features. 
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-2">
                Create geometric features with precision
              </p>
              <ul className="text-sm space-y-1 ml-4">
                <li><strong>Point Tool:</strong> Click to place precise markers</li>
                <li><strong>Line Tool:</strong> Draw lines with precise measurements</li>
                <li><strong>Polygon Tool:</strong> Create closed areas and boundaries</li>
                <li><strong>Circle Tool:</strong> Draw perfect circles and ellipses</li>
                <li><strong>Freehand Tool:</strong> Draw organic shapes and annotations</li>
              </ul>
            </div>
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-2">
                Accurate distance, area, and angle calculations
              </p>
              <ul className="text-sm space-y-1 ml-4">
                <li><strong>Measure Tool:</strong> Calculate distances between points</li>
                <li><strong>Bearing Tool:</strong> Determine directional angles</li>
                <li><strong>Area Tool:</strong> Calculate area measurements</li>
              </ul>
            </div>

            <div>
              <h4 className="font-semibold mb-2">Editing Tools</h4>
              <p className="text-sm text-muted-foreground mb-2">
                Modify and transform existing features
              </p>
              <ul className="text-sm space-y-1 ml-4">
                <li><strong>Select Tool:</strong> Choose and manipulate features</li>
                <li><strong>Move Tool:</strong> Reposition features precisely</li>
                <li><strong>Rotate Tool:</strong> Rotate features around pivot points</li>
            ...
            <div>
              ...
              <ul className="text-sm space-y-1 ml-4">
                <li><strong>Coordinate Display:</strong> Real-time coordinate tracking</li>
                <li><strong>Grid System:</strong> Snap-to-grid for precision alignment</li>
                <li><strong>Layer Management:</strong> Organize features in layers</li>
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-2">
                Import, export, and share your cartographic work
              </p>
              <ul className="text-sm space-y-1 ml-4">
                <li><strong>GeoJSON Export:</strong> Industry-standard format support</li>
                <li><strong>Import Features:</strong> Load existing geospatial data</li>
                <li><strong>Auto-save:</strong> Automatic work preservation</li>
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground mb-2">
                Enterprise-grade capabilities for professional cartographers
              </p>
              <ul className="text-sm space-y-1 ml-4">
                <li><strong>Survey Integration:</strong> Import survey data directly</li>
                <li><strong>Legal Descriptions:</strong> Generate legal property descriptions</li>
                <li><strong>Coordinate Systems:</strong> Support for multiple projections</li>
