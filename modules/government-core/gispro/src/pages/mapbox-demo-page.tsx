...
export default function MapboxDemoPage() {
  ...
  // Add markers to map
  const addMarkersToMap = (mapInstance: mapboxgl.Map, markerData: MapMarker[]) => {
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Mapbox Integration Demo</h1>
          <p className="text-muted-foreground">Interactive map with Mapbox GL JS featuring various styles and controls</p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant={isMapLoaded ? 'default' : 'secondary'}>
            {isMapLoaded ? 'Map Loaded' : 'Loading...'}
          </Badge>
              ...
              {!isMapLoaded && (
                <div className="absolute inset-0 bg-white bg-opacity-90 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <div className="text-sm text-muted-foreground">Loading Mapbox...</div>
                  </div>
                </div>
              )}
            </CardContent>
              ...
              <Tabs value={selectedTab} onValueChange={setSelectedTab}>
                ...
                <TabsContent value="controls" className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-2">
                      <Button size="sm" onClick={zoomIn} disabled={!isMapLoaded}>
                        <ZoomIn className="h-4 w-4" />
                      </Button>
                      <Button size="sm" onClick={zoomOut} disabled={!isMapLoaded}>
                        <ZoomOut className="h-4 w-4" />
                      </Button>
                      <Button size="sm" onClick={resetBearing} disabled={!isMapLoaded}>
                        <RotateCcw className="h-4 w-4" />
                      </Button>
                      <Button size="sm" onClick={resetPitch} disabled={!isMapLoaded}>
                  ...
                  {/* Zoom Control */}
                  <div>
                    <Label className="text-sm font-medium">Zoom Level: {mapZoom.toFixed(1)}</Label>
                    <Slider
                      value={[mapZoom]}
                  ...
                  {/* Pitch Control */}
                  <div>
                    <Label className="text-sm font-medium">Pitch: {mapPitch.toFixed(0)}°</Label>
                    <Slider
                      value={[mapPitch]}
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground mt-1 font-mono">
                      Lat: {mapCenter[1].toFixed(4)}<br />
                      Lng: {mapCenter[0].toFixed(4)}
                    </div>
                  </div>
                </TabsContent>
                ...
                <TabsContent value="styles" className="space-y-4">
                  <div>
                    ...
                    <Select value={currentStyle} onValueChange={changeMapStyle}>
                    ...
                    {mapStyles.map(style => (
                      <div
                        ...
                        className={`p-3 border rounded-lg cursor-pointer transition-colors ${
                          currentStyle === style.id ? 'border-blue-500 bg-blue-50' : 'hover:border-gray-300'
                        }`}
                ...
                <TabsContent value="locations" className="space-y-4">
                  ...
                  <Alert>
                    ...
                    <AlertDescription>
                      Click on any location to fly to that area on the map.
                    </AlertDescription>
                  </Alert>
                </TabsContent>
              </Tabs>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Choose from 6 different map styles</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">Zoom, rotate, and navigate smoothly</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-semibold">Local Points</h3>
                <p className="text-sm text-muted-foreground">Explore Benton County landmarks</p>
              </div>
            </div>
          </CardContent>
        </Card>
