...
import { Plus, Search, Filter, Download, FileText, MapPin, Settings, RefreshCw } from '@mui/icons-material';
...
export default function ParcelGeneratorPage() {
  ...
  useEffect(() => {
    const savedParcels = localStorage.getItem('generated-parcels');
    if (savedParcels) {
  ...
  const generateParcels = async () => {
    ...
    const streets = ['Main St', 'Oak Ave', 'Pine Rd', 'Cedar Ln', 'Elm Dr', 'Maple Way', 'Park Blvd'];
    const owners = ['Smith, John', 'Johnson, Mary', 'Williams, David', 'Brown, Susan', 'Davis, Michael'];
  ...
  const exportParcels = (format: 'csv' | 'json' | 'geojson') => {
    ...
    if (format === 'csv') {
      const headers = ['Parcel Number', 'Address', 'Owner', 'Acreage', 'Zoning', 'Tax Value', 'Status'];
      const csvContent = [
        headers.join(','),
      ...
      downloadFile(JSON.stringify(selectedParcelData, null, 2), 'parcels.json', 'application/json');
  ...
  const deleteSelectedParcels = () => {
    const updatedParcels = parcels.filter(p => !selectedParcels.includes(p.id));
    setParcels(updatedParcels);
    setFilteredParcels(updatedParcels);
    localStorage.setItem('generated-parcels', JSON.stringify(updatedParcels));
    setSelectedParcels([]);
  ...
  return (
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Parcel Generator</h1>
          <p className="text-muted-foreground">Generate and manage synthetic parcel data for testing and development</p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="outline">{parcels.length} Total Parcels</Badge>
          {selectedParcels.length > 0 && (
            <Badge variant="default">{selectedParcels.length} Selected</Badge>
          )}
        </div>
      </div>
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab}>
        ...
        <TabsContent value="generator" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                <div>
                  <Label htmlFor="count">Number of Parcels</Label>
                  <Input
                    id="count"
                    type="number"
                    min="1"
                    max="1000"
                    value={config.count}
                    onChange={(e) => setConfig(prev => ({ ...prev, count: parseInt(e.target.value) || 50 }))}
                  />
                </div>
                ...
                <div>
                  ...
                  <Select value={config.parcelType} onValueChange={(value) => setConfig(prev => ({ ...prev, parcelType: value }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  ...
                  <Select value={config.addressFormat} onValueChange={(value) => setConfig(prev => ({ ...prev, addressFormat: value }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="standard">Standard (123 Main St)</SelectItem>
                      <SelectItem value="rural">Rural (County Road 45)</SelectItem>
                      <SelectItem value="apartment">Apartment (123 Main St #4B)</SelectItem>
                    </SelectContent>
                  </Select>
                </div>

                <div>
                  <Label htmlFor="coordinates">Coordinate System</Label>
                  <Select value={config.coordinateSystem} onValueChange={(value) => setConfig(prev => ({ ...prev, coordinateSystem: value }))}>
                    <SelectTrigger>
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="wgs84">WGS84 (Lat/Lon)</SelectItem>
                      <SelectItem value="utm">UTM</SelectItem>
                      <SelectItem value="state-plane">State Plane</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              {isGenerating && (
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Generating {config.count} parcels... {Math.round(generationProgress)}% complete
                  </p>
                </div>
              )}
            </CardContent>
          ...
          {parcels.length === 0 && (
            <Alert>
              <AlertDescription>
                No parcels have been generated yet. Configure your settings above and click "Generate Parcels" to start.
              </AlertDescription>
            </Alert>
          )}
        </TabsContent>
                      ...
                      <div className="flex items-start justify-between">
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 flex-1">
                          <div>
                            ...
                            <div className="text-sm text-muted-foreground">{parcel.address}</div>
                          </div>
                          <div>
                            <div className="text-sm">Owner: {parcel.owner}</div>
                            <div className="text-sm">Acreage: {parcel.acreage}</div>
                          </div>
                          <div>
                            <Badge variant="outline" className="mr-2">{parcel.zoning}</Badge>
                            <Badge variant={parcel.status === 'approved' ? 'default' : 'secondary'}>
                              {parcel.status}
                            </Badge>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-sm text-muted-foreground">
                            ${parcel.taxValue.toLocaleString()}
                          </span>
                        </div>
                      </div>
                    </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {selectedParcels.length === 0 && (
                <Alert>
                  <AlertDescription>
                    Select one or more parcels from the Management tab to enable export options.
                  </AlertDescription>
                </Alert>
              )}
              ...
              {selectedParcels.length > 0 && (
                <Alert>
                  <AlertDescription>
                    Ready to export {selectedParcels.length} selected parcel{selectedParcels.length !== 1 ? 's' : ''}.
                  </AlertDescription>
                </Alert>
              )}
            </CardContent>
