...
export default function DataMigrationPage() {
  ...
  // Helper functions
  const getStatusIcon = (status: MigrationJob['status']) => {
    switch (status) {
      case 'completed':
  ...
  const getTypeIcon = (type: MigrationJob['type']) => {
    switch (type) {
      case 'ftp':
    ...
    >
      ...
      >
        ...
        <p className="text-xl text-muted-foreground max-w-3xl mx-auto">
          Transfer spatial data files, documents, and other information between BentonGeoPro 
          and external systems. Connect to FTP servers, databases, and APIs to upload and download 
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {migrationJobs.filter(j => j.status === 'running').length}
                  </div>
              ...
              <Card>
                ...
                <CardContent>
                  <div className="text-2xl font-bold">
                    {migrationJobs.filter(j => j.status === 'completed').length}
                  </div>
                  <p className="text-xs text-muted-foreground">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Latest data migration jobs and their status
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="space-y-4">
                  {migrationJobs.slice(0, 3).map((job) => (
          ...
          <TabsContent value="jobs" className="space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-medium">Migration Jobs</h3>
                      ...
                      <TableRow key={job.id}>
                        ...
                        <TableCell>
                          <div className="text-sm">
                            ...
                            <div className="text-muted-foreground">→ {job.destination}</div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="space-y-1">
                            ...
                            <div className="text-sm text-muted-foreground">
                              {job.progress}% ({formatFileSize(job.size)})
                            </div>
                          </div>
                        </TableCell>
                        <TableCell>
                          <div className="text-sm">
                            <div>{formatNumber(job.recordsTransferred)} / {formatNumber(job.totalRecords)}</div>
                            {job.errorCount > 0 && (
          ...
          <TabsContent value="connections" className="space-y-6">
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-medium">Connection Configurations</h3>
              <Button className="flex items-center gap-2">
                <Network className="h-4 w-4" />
                Add Connection
              </Button>
              ...
              {connections.map((connection) => (
                <Card key={connection.id}>
                  <CardHeader>
                    ...
                    <CardDescription>
                      {connection.type.toUpperCase()} connection to {connection.host}
                    </CardDescription>
                  </CardHeader>
                  ...
                  <TableBody>
                    {dataSources.map((source) => (
                      <TableRow key={source.id}>
                        ...
                        <TableCell>{formatNumber(source.recordCount)}</TableCell>
                        <TableCell>
                          <div className="flex items-center gap-1">
                            <MapPin className="h-3 w-3" />
                            <span className="capitalize">{source.geometry}</span>
                          </div>
                        </TableCell>
                        <TableCell>
                          ...
                          <div className="flex items-center gap-1">
                            <TooltipProvider>
                              <Tooltip>
                                ...
                                <TooltipContent>Preview data</TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                            ...
                            <TooltipProvider>
                              <Tooltip>
                                ...
                                <TooltipContent>Import to system</TooltipContent>
                              </Tooltip>
                            </TooltipProvider>
                          </div>
                        </TableCell>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Configure default migration behavior and performance settings
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="batch-size">Batch Size</Label>
                    <Input id="batch-size" placeholder="1000" />
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <div className="font-medium">Show Passwords</div>
                      <div className="text-sm text-muted-foreground">
                        Display connection passwords in plain text
                      </div>
                    </div>
                    <Button
                      variant="outline"
      ...
      <motion.div variants={cardVariants}>
        <Alert className="border-green-200 bg-green-50">
          ...
          <AlertDescription>
            Data Migration system is online and ready. All connections are secure and monitored.
          </AlertDescription>
        </Alert>
      </motion.div>
    </motion.div>
