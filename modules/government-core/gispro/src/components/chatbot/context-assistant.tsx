...
const ContextAssistant: React.FC<ContextAssistantProps> = ({
  ...
  useEffect(() => {
    if (context && !analysisRef.current) {
  ...
  const generateContextInsights = (contextData: ContextData): Insight[] => {
    ...
    insights.push({
      ...
      timestamp: now,
      actions: ['Explore elevation relationship', 'Control for topography', 'Add terrain variables'],
  ...
  const generateContextualResponse = (query: string, contextData: ContextData, availableInsights: Insight[]): string => {
    ...
    if (lowerQuery.includes('anomal') || lowerQuery.includes('outlier')) {
      return "I found 7 statistical outliers in your dataset, primarily concentrated in the northwest region. These points deviate significantly from the expected pattern and may indicate: data collection errors, unique local conditions, or genuinely exceptional cases that warrant investigation.";
    }
    
    if (lowerQuery.includes('trend')) {
      return "Your data shows a clear upward trend with a 12% annual increase. This trend is statistically significant (p < 0.01) and appears consistent across most spatial regions. The trend strength suggests systematic change rather than random variation.";
    }
    
    if (lowerQuery.includes('recommend') || lowerQuery.includes('next')) {
      return "Based on my analysis, I recommend: 1) Investigate the 7 outliers in the northwest region, 2) Add 15-20 sample points in the southern area to improve coverage, 3) Explore the elevation correlation further, and 4) Consider temporal controls in your model. These steps could improve analysis reliability by ~23%.";
    }
  ...
  const handleActionClick = (action: string, insight?: Insight) => {
    if (onActionRequest) {
                ...
                {insights.length === 0 && !isAnalyzing && (
                  <div className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">
                      Provide context data to generate AI-powered insights.
                    </p>
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2">
                  ...
                  >
                    ...
                    <span className="text-xs">Export Insights</span>
                  </Button>
                  ...
                  >
                    ...
                    <span className="text-xs">Share Analysis</span>
                  </Button>
