...
interface IllustratedTooltipProps {
  title: string;
  content: ReactNode;
...
export function IllustratedTooltip({
  ...
  const defaultIllustration = (
    ...
    >
      ...
      <circle cx="80" cy="40" r="20" fill="hsl(var(--primary))" fillOpacity="0.3" />
      <circle cx="240" cy="80" r="15" fill="hsl(var(--primary))" fillOpacity="0.4" />
      <rect x="120" y="30" width="80" height="8" fill="hsl(var(--muted-foreground))" fillOpacity="0.3" rx="4" />
      <rect x="120" y="50" width="120" height="6" fill="hsl(var(--muted-foreground))" fillOpacity="0.2" rx="3" />
      <rect x="120" y="70" width="60" height="6" fill="hsl(var(--muted-foreground))" fillOpacity="0.2" rx="3" />
    </svg>
  );
    ...
    <TooltipProvider delayDuration={delay}>
      <Tooltip>
          ...
          <span className={cn(
            ...
            {trigger || (
              showIcon && (
                <Icon 
                  size={iconSize} 
                  className={cn("transition-colors", config.iconClass)}
                />
              )
            )}
          </span>
        ...
        <TooltipContent 
          side={position} 
          className={cn(
            "p-0 overflow-hidden shadow-lg border",
            config.contentClass
          )}
        ...
        >
          <div className="flex flex-col">
            ...
            <div className="w-full bg-gradient-to-br from-background/50 to-muted/50">
              {renderIllustration()}
            </div>
            ...
            <div className="p-4 space-y-2">
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <h4 className="font-semibold text-sm leading-tight text-foreground mb-1">
                    {title}
                  </h4>
...
export const InfoTooltip = (props: Omit<IllustratedTooltipProps, 'variant'>) => (
  <IllustratedTooltip {...props} variant="info" />
);

export const WarningTooltip = (props: Omit<IllustratedTooltipProps, 'variant'>) => (
  <IllustratedTooltip {...props} variant="warning" />
);

export const SuccessTooltip = (props: Omit<IllustratedTooltipProps, 'variant'>) => (
  <IllustratedTooltip {...props} variant="success" />
);

export const ErrorTooltip = (props: Omit<IllustratedTooltipProps, 'variant'>) => (
  <IllustratedTooltip {...props} variant="error" />
);

// Simple help tooltip with just text
export const HelpTooltip = ({ text, ...props }: { text: string } & Partial<IllustratedTooltipProps>) => (
  <IllustratedTooltip
    title="Help"
    content={text}
    {...props}
...
export const FeatureTooltip = ({ 
  ...
  description: string; 
  isNew?: boolean;
} & Partial<IllustratedTooltipProps>) => (
  <IllustratedTooltip
