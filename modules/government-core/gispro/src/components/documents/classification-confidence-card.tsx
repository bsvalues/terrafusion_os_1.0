...
interface ClassificationResult {
  ...
  evidence?: Array<{
    type: 'keyword' | 'phrase' | 'pattern' | 'semantic' | 'statistical';
    value: string;
    weight: number;
...
interface ClassificationConfidenceCardProps {
  ...
  onValidate?: (result: ClassificationResult, feedback: 'correct' | 'incorrect' | 'partial', notes?: string) => void;
...
const ClassificationConfidenceCard: React.FC<ClassificationConfidenceCardProps> = ({
  result,
  metrics,
  stats,
  showDetails = true,
  ...
  const getConfidenceLevel = (confidence: number) => {
    ...
    } else if (confidence >= confidenceThreshold.low) {
      return {
        level: 'low',
  ...
  // Get flag styling
  const getFlagStyling = (type: string, severity: string) => {
    const typeStyles = {
  ...
  const handleFeedbackSubmit = (feedback: 'correct' | 'incorrect' | 'partial') => {
    if (onValidate) {
  ...
  return (
    <motion.div
      ...
      style={style}
    >
      <Card className={`${confidenceDisplay.border} border-2`}>
        <CardHeader>
          <div className="flex items-start justify-between">
            <div className="flex-1">
              <CardTitle className="flex items-center gap-3 mb-2">
                <div className={`p-2 rounded-lg ${confidenceDisplay.bg}`}>
                  <BrainIcon className={`h-6 w-6 ${confidenceDisplay.color}`} />
                </div>
                <div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <Badge className={`${confidenceDisplay.bg} ${confidenceDisplay.color} border-current`}>
                    {formatPercentage(result.confidence)} - {confidenceDisplay.level.toUpperCase()}
                  </Badge>
                </div>
                
                <div className="relative h-3 bg-gray-200 rounded-full overflow-hidden" ref={progressRef}>
                  <motion.div
                    variants={progressVariants}
                    initial="hidden"
                    animate={animateProgress ? "visible" : "hidden"}
                    className={`h-full ${confidenceDisplay.progressColor} rounded-full`}
                  />
                  ...
                  <div className="absolute inset-0 flex items-center">
                    <div
                      ...
                      style={{ left: `${confidenceThreshold.low * 100}%` }}
                    />
                    <div
                      className="absolute w-0.5 h-full bg-gray-400 opacity-50"
                      style={{ left: `${confidenceThreshold.medium * 100}%` }}
                    />
                    <div
                      className="absolute w-0.5 h-full bg-gray-400 opacity-50"
                      style={{ left: `${confidenceThreshold.high * 100}%` }}
                    />
                  </div>
                </div>

                <div className="flex justify-between text-xs text-muted-foreground">
                  <span>Low ({formatPercentage(confidenceThreshold.low)})</span>
                  <span>Medium ({formatPercentage(confidenceThreshold.medium)})</span>
                  <span>High ({formatPercentage(confidenceThreshold.high)})</span>
                </div>
              </div>
            </div>
          ...
          {result.flags && result.flags.length > 0 && (
            <div className="mt-3 space-y-2">
              {result.flags.map((flag, index) => {
                ...
                return (
                  <Alert key={index} className={`${flagStyle.bg} ${flagStyle.severity} ${flagStyle.color}`}>
          ...
          {showDetails && (
            <Tabs value={activeTab} onValueChange={setActiveTab}>
              ...
              <TabsContent value="classification" className="space-y-4">
                  ...
                  <div>
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span className="font-medium">{formatPercentage(result.probability)}</span>
                      </div>
                      <div className="flex justify-between">
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{result.metadata.modelName}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Version:</span>
                          <span className="font-medium">{result.metadata.modelVersion}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Processing Time:</span>
                          <span className="font-medium">{formatProcessingTime(result.metadata.processingTime)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Threshold:</span>
                          <span className="font-medium">{formatPercentage(result.metadata.threshold)}</span>
                        </div>
                      </div>
                    </div>
                  )}
                ...
                {showValidation && result.validation && (
                  <div className="p-3 border rounded-lg bg-gray-50">
                    ...
                    <div className="space-y-2 text-sm">
                      <div className="flex items-center gap-2">
                        {result.validation.humanVerified ? (
                          <CheckCircleIcon className="h-4 w-4 text-green-600" />
                        ) : (
                          <AlertCircleIcon className="h-4 w-4 text-yellow-600" />
                        )}
                        <span>
                          {result.validation.humanVerified ? 'Human verified' : 'Pending verification'}
                        </span>
                      </div>
                      
                      {result.validation.verifiedBy && (
                        <div className="flex justify-between">
                          <span className="text-muted-foreground">Verified by:</span>
                          <span className="font-medium">{result.validation.verifiedBy}</span>
                        </div>
                      )}
                      ...
                      {result.validation.feedback && (
                        <div className="flex justify-between">
                          ...
                          <Badge 
                            className={
                              result.validation.feedback === 'correct' ? 'bg-green-100 text-green-800' :
                              result.validation.feedback === 'incorrect' ? 'bg-red-100 text-red-800' :
              ...
              <TabsContent value="evidence" className="space-y-4">
                {showEvidence && result.evidence && result.evidence.length > 0 ? (
                    ...
                    {result.evidence
                      ...
                      .map((evidence, index) => (
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4 text-xs text-muted-foreground">
                                <span>Weight: {(evidence.weight * 100).toFixed(1)}%</span>
                                <span>Relevance: {(evidence.relevance * 100).toFixed(1)}%</span>
                                {evidence.position && (
                                  <span>Position: {evidence.position.start}-{evidence.position.end}</span>
                                )}
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-2">
                              <div className="w-16 h-2 bg-gray-200 rounded-full overflow-hidden">
                                <div
                                  ...
                                  style={{ width: `${evidence.weight * 100}%` }}
                                />
                              </div>
                              <ChevronDownIcon 
                          ...
                          <AnimatePresence>
                            {expandedEvidence.includes(`evidence-${index}`) && evidence.context && (
                              <motion.div
                                initial={{ opacity: 0, height: 0 }}
                                animate={{ opacity: 1, height: 'auto' }}
                                exit={{ opacity: 0, height: 0 }}
              ...
              <TabsContent value="alternatives" className="space-y-4">
                {showAlternatives && result.alternatives && result.alternatives.length > 0 ? (
                  <div className="space-y-3">
                    <h4 className="text-sm font-medium">Alternative Classifications</h4>
                    {result.alternatives
                        ...
                        <div key={index} className="p-3 border rounded-lg">
                          <div className="flex items-center justify-between mb-2">
                            ...
                            <Badge variant="outline">
                              {formatPercentage(alternative.confidence)}
                            </Badge>
                          </div>
                  ...
                  <div className="text-center py-6">
                    <TargetIcon className="h-8 w-8 text-muted-foreground mx-auto mb-2" />
                    <p className="text-sm text-muted-foreground">No alternative classifications</p>
                  </div>
                )}
              </TabsContent>
              ...
              <TabsContent value="metrics" className="space-y-4">
                {metrics ? (
                  <div className="grid grid-cols-2 gap-4">
                    <div className="space-y-3">
                      <h4 className="text-sm font-medium">Confidence Metrics</h4>
                      <div className="space-y-2">
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-2">
                          <div className="flex justify-between text-sm">
                            ...
                            <span className="font-medium">
                              {formatPercentage(stats.successfulClassifications / stats.totalDocuments)}
                            </span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span className="text-muted-foreground">Avg Confidence:</span>
                            <span className="font-medium">{formatPercentage(stats.averageConfidence)}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span className="text-muted-foreground">High Confidence:</span>
                            <span className="font-medium">
                              {formatPercentage(stats.highConfidenceCount / stats.totalDocuments)}
                            </span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span className="text-muted-foreground">Avg Processing:</span>
                            <span className="font-medium">
                              {formatProcessingTime(stats.processingTimeStats.average)}
                            </span>
                          </div>
                        </div>
                      </div>
