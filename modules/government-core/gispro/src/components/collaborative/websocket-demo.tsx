...
const WebSocketDemo: React.FC<WebSocketDemoProps> = ({
  ...
  const handleConnect = useCallback(() => {
    ...
    try {
      const protocols = config.protocols?.length ? config.protocols : undefined;
      ...
      websocketRef.current.onopen = (event) => {
        ...
        if (config.heartbeatEnabled && config.pingInterval > 0) {
      ...
      websocketRef.current.onmessage = (event) => {
        ...
        try {
          ...
          messageData = { content: event.data, type: 'data' };
        }

        // Handle ping/pong for latency measurement
        if (messageData.type === 'pong' && messageData.originalTimestamp) {
          const latency = receiveTime - messageData.originalTimestamp;
          setMetrics(prev => {
      ...
      websocketRef.current.onclose = (event) => {
        ...
        if (config.autoReconnect && !event.wasClean && reconnectCount.current < config.maxReconnectAttempts) {
  ...
  const getConnectionStatus = () => {
    switch (connectionState) {
      case 'connected':
        return { icon: CheckCircleIcon, color: 'text-green-600', bg: 'bg-green-100', text: 'Connected' };
      case 'connecting':
        return { icon: RefreshCwIcon, color: 'text-blue-600', bg: 'bg-blue-100', text: 'Connecting...' };
      case 'reconnecting':
        return { icon: RefreshCwIcon, color: 'text-yellow-600', bg: 'bg-yellow-100', text: 'Reconnecting...' };
      case 'error':
        return { icon: AlertCircleIcon, color: 'text-red-600', bg: 'bg-red-100', text: 'Error' };
      default:
        return { icon: WifiOffIcon, color: 'text-gray-600', bg: 'bg-gray-100', text: 'Disconnected' };
    }
  };

  // Get message type display
  const getMessageTypeDisplay = (type: WebSocketMessage['type']) => {
    switch (type) {
      case 'system':
      ...
      <Card className="flex-1 flex flex-col">
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              ...
              WebSocket Demo
              <Badge variant="outline" className={`${connectionStatus.bg} ${connectionStatus.color} border-current`}>
          ...
          <div className="grid grid-cols-4 gap-4 mt-4">
            ...
            <div className="text-center">
              <div className="text-2xl font-bold text-green-600">{metrics.messagesSent}</div>
              <div className="text-xs text-muted-foreground">Sent</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">{metrics.messagesReceived}</div>
        ...
        <CardContent className="flex-1 overflow-hidden">
          <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
            ...
            <TabsContent value="connection" className="flex-1 flex flex-col min-h-0">
              <div className="grid grid-cols-2 gap-6 h-full">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-sm font-medium mb-1 block">Auto Reconnect</label>
                        <Switch
                      ...
                      <div>
                        <label className="text-sm font-medium mb-1 block">Heartbeat</label>
                        <Switch
                          checked={config.heartbeatEnabled}
                          onCheckedChange={(checked) => setConfig(prev => ({ ...prev, heartbeatEnabled: checked }))}
                        />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-sm font-medium mb-1 block">Reconnect Interval (ms)</label>
                        <Input
                          type="number"
                          value={config.reconnectInterval}
                          onChange={(e) => setConfig(prev => ({ ...prev, reconnectInterval: Number(e.target.value) }))}
                          min="1000"
                          max="60000"
                          step="1000"
                          className="text-sm"
                      ...
                      <div>
                        <label className="text-sm font-medium mb-1 block">Max Reconnect Attempts</label>
                        <Input
                          type="number"
                          value={config.maxReconnectAttempts}
                          onChange={(e) => setConfig(prev => ({ ...prev, maxReconnectAttempts: Number(e.target.value) }))}
                          min="1"
                          max="20"
                          className="text-sm"
                        />
                    ...
                    <div>
                      <label className="text-sm font-medium mb-1 block">Protocols (optional)</label>
                      <Input
                        placeholder="wamp, chat, etc. (comma-separated)"
                        value={config.protocols?.join(', ') || ''}
                        onChange={(e) => {
                          const protocols = e.target.value.split(',').map(p => p.trim()).filter(Boolean);
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div className={`p-4 rounded-lg ${connectionStatus.bg}`}>
                      ...
                      {connectionState === 'connected' && metrics.connectTime && (
                        <div className="text-sm text-muted-foreground">
                          Connected at {metrics.connectTime.toLocaleTimeString()}
                        </div>
                      )}
                      {connectionState === 'reconnecting' && (
                        <div className="text-sm text-muted-foreground">
                          Attempt {reconnectCount.current} of {config.maxReconnectAttempts}
                        </div>
                      )}
                    </div>
                    ...
                    {connectionState === 'connected' && (
                      <div className="space-y-2">
                        <div className="flex justify-between text-sm">
                          <span>Uptime:</span>
                          <span>{Math.floor(metrics.connectionUptime / 1000)}s</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span>Protocol:</span>
                          <span>{websocketRef.current?.protocol || 'Default'}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span>Ready State:</span>
                          <span>{websocketRef.current?.readyState}</span>
                        </div>
                        <div className="flex justify-between text-sm">
                          <span>Buffer Size:</span>
                          <span>{messageQueue.current.length} messages</span>
                        </div>
                      </div>
                    )}
                          ...
                          <div className="space-y-1 text-xs">
                            {connectionHistory.slice(-10).reverse().map((event, index) => (
                              <div key={index} className="flex items-start gap-2">
                                ...
                                <span className={
                                  event.event === 'connect' ? 'text-green-600' :
                                  event.event === 'disconnect' ? 'text-gray-600' :
                  ...
                  {filteredMessages.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-muted-foreground">
                        {messages.length === 0 ? 'Connect and start sending messages.' : 'No messages match your filters.'}
                      </p>
                    </div>
                  ) : (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-start gap-3">
                            <TypeIcon className={`h-4 w-4 mt-0.5 ${typeDisplay.color}`} />
              ...
              <div className="grid grid-cols-2 gap-6 h-full">
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-4 border rounded-lg">
                      <div className="text-2xl font-bold text-red-600">{metrics.failedConnections}</div>
                      <div className="text-sm text-muted-foreground">Failed Connections</div>
                    </div>
                    <div className="p-4 border rounded-lg">
                      <div className="text-2xl font-bold text-green-600">{metrics.reconnectAttempts}</div>
                      <div className="text-sm text-muted-foreground">Reconnect Attempts</div>
                    </div>
                    <div className="p-4 border rounded-lg">
                      <div className="text-2xl font-bold text-purple-600">{metrics.errorCount}</div>
                      <div className="text-sm text-muted-foreground">Errors</div>
                    </div>
                  </div>

                  <div className="space-y-2">
                    <h4 className="font-medium">Latency Statistics</h4>
                    <div className="grid grid-cols-3 gap-2 text-sm">
                      <div>
                        <div className="text-muted-foreground">Average</div>
                        <div className="font-medium">{Math.round(metrics.averageLatency)}ms</div>
                      </div>
                      <div>
                        <div className="text-muted-foreground">Min</div>
                        <div className="font-medium">{metrics.minLatency}ms</div>
                      </div>
                      <div>
                        <div className="text-muted-foreground">Max</div>
                        <div className="font-medium">{metrics.maxLatency}ms</div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="text-sm">
                      <div className="flex justify-between">
                        ...
                        <span>{(metrics.bytesTransferred / 1024).toFixed(2)} KB</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Message Rate:</span>
                        <span>
                          {metrics.connectionUptime > 0 ? 
                            (metrics.totalMessages / (metrics.connectionUptime / 1000 / 60)).toFixed(2) : 
                            '0'
                          } msg/min
                        </span>
                      </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center py-8">
                    ...
                    <h4 className="text-lg font-medium mb-2">Performance Charts</h4>
                    <p className="text-muted-foreground">
                      Real-time performance visualization would be displayed here.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <TabsContent value="config" className="flex-1">
              <div className="grid grid-cols-2 gap-6 h-full">
                <div className="space-y-4">
                  ...
                  <div className="space-y-3">
                    <div>
                      <label className="text-sm font-medium mb-1 block">Ping Interval (ms)</label>
                      <Input
                        type="number"
                        value={config.pingInterval}
                        onChange={(e) => setConfig(prev => ({ ...prev, pingInterval: Number(e.target.value) }))}
                        min="5000"
                        max="120000"
                        step="5000"
                        className="text-sm"
                    ...
                    <div>
                      <label className="text-sm font-medium mb-1 block">Max Buffer Size</label>
                      <Input
                        type="number"
                        value={config.maxBufferSize}
                        onChange={(e) => setConfig(prev => ({ ...prev, maxBufferSize: Number(e.target.value) }))}
                        min="10"
                        max="10000"
                        className="text-sm"
                      />
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="text-sm font-medium mb-1 block">Buffer Messages</label>
                        <Switch
                          checked={config.bufferMessages}
                          onCheckedChange={(checked) => setConfig(prev => ({ ...prev, bufferMessages: checked }))}
                        />
                      </div>
                      <div>
                        <label className="text-sm font-medium mb-1 block">Message Retry</label>
                        <Switch
                          checked={config.messageRetryEnabled}
                          onCheckedChange={(checked) => setConfig(prev => ({ ...prev, messageRetryEnabled: checked }))}
                        />
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => setConfig(prev => ({ ...prev, logLevel: e.target.value as typeof config.logLevel }))}
            ...
            <TabsContent value="logs" className="flex-1">
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground">
                  Detailed system logs and debug information would be displayed here.
                </p>
              </div>
            </TabsContent>
          </Tabs>
