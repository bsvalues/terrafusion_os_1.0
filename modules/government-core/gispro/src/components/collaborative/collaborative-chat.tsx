...
const CollaborativeChat: React.FC<CollaborativeChatProps> = ({
  currentUser,
  channels = [],
  initialChannel,
  onChannelChange,
  ...
  const [selectedChannel, setSelectedChannel] = useState<string>(initialChannel || channels[0]?.id || 'general');
  ...
  const sampleMessages: Message[] = [
    {
      id: 'msg-1',
      content: 'Good morning team! The field survey results from yesterday are now available in the shared drive.',
    ...
    {
      id: 'msg-2',
      content: 'Great work on the data collection! I\'ve started the preliminary analysis. Initial patterns look promising.',
    ...
    {
      id: 'msg-3',
      content: 'I\'ve uploaded the updated GIS layers with the new coordinate system. Please validate the spatial accuracy.',
    ...
    {
      id: 'msg-4',
      content: 'Meeting scheduled for 2 PM to review the analysis results. Location: Conference Room B.',
      author: sampleUsers[0],
    ...
    {
      id: 'msg-5',
      content: 'The correlation analysis shows significant patterns in the northern region. Should we increase sampling density there?',
    ...
    <div className={`h-full flex ${className}`}>
      ...
      <div className="w-64 border-r bg-gray-50 flex flex-col">
        ...
        <div className="p-4 border-b bg-white">
          <h2 className="font-semibold text-lg">TerraFusion Project</h2>
          <p className="text-sm text-muted-foreground">GIS Collaboration Hub</p>
        </div>
            ...
            <div className="p-2">
              <div className="mb-4">
                <div className="flex items-center justify-between mb-2 px-2">
                  <h3 className="text-xs font-medium text-muted-foreground uppercase">Channels</h3>
                  ...
                  {currentChannels.map(channel => (
                    ...
                    >
                      <div className="flex items-center gap-2 flex-1 min-w-0">
                        ...
                        {channel.isPrivate && <Eye className="h-3 w-3" />}
                      </div>
                ...
                <div className="mb-4">
                  ...
                  <div className="space-y-1">
                    {onlineUsers.map(user => (
                      <div key={user.id} className="flex items-center gap-2 px-2 py-1 rounded hover:bg-white">
      ...
      <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="space-y-4">
            {messages.map(message => (
              <div key={message.id} className={`flex gap-3 ${
                ...
                <div className={`flex-1 min-w-0 ${message.type === 'system' ? 'max-w-md' : ''}`}>
                  {message.type === 'system' ? (
                    <Alert>
                      ...
                      {message.attachments && message.attachments.length > 0 && (
                        <div className="space-y-2 mb-2">
                          {message.attachments.map(attachment => (
                            <div key={attachment.id} className="flex items-center gap-2 p-2 bg-gray-100 rounded">
        ...
        <div className="p-4 border-t bg-white">
          ...
          {/* Typing Indicators */}
          {isTyping[selectedChannel] && isTyping[selectedChannel].length > 0 && (
            <div className="mt-2 text-xs text-muted-foreground">
              {isTyping[selectedChannel].map(user => user.name).join(', ')} 
              {isTyping[selectedChannel].length === 1 ? ' is' : ' are'} typing...
            </div>
          )}
        </div>
      </div>
