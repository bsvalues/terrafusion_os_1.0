...
export function CollaborativeMapbox({
  roomId,
    ...
    switch (message.type) {
      ...
      case MessageTypeEnum.MAP_ANNOTATION:
        if (message.data.annotation) {
          addAnnotation(message.data.annotation);
        }
        break;
      
      case MessageTypeEnum.MAP_VIEW_CHANGE:
        if (message.data.userId !== currentUser.id && map.current) {
          map.current.flyTo({
    ...
    const initializeMap = async () => {
      ...
      try {
        ...
        mapInstance.on('load', () => {
          ...
          mapInstance.addSource('annotations', {
            type: 'geojson',
            data: { type: 'FeatureCollection', features: [] }
          });
  ...
  const updateUserCursor = useCallback((cursorData: UserCursor) => {
    setUserCursors(prev => {
  ...
  const handleDrawing = (lngLat: mapboxgl.LngLat) => {
    if (!connected || drawingMode === 'none') return;
    ...
    send({
      type: MessageTypeEnum.MAP_ANNOTATION,
      data: {
        annotation,
        userId: currentUser.id
      }
    ...
    const annotationFeatures = annotations.map(annotation => ({
      ...
      geometry: {
        type: annotation.type === 'point' ? 'Point' : 
              annotation.type === 'line' ? 'LineString' : 'Polygon',
    ...
    <div className="relative w-full h-full">
        ...
        <Card className="w-64">
          ...
          <CardContent className="pt-0">
            <div className="text-xs text-muted-foreground">
              {participants.length} users in room
            </div>
      ...
      <div className="absolute top-4 right-4">
        <div className="flex flex-col gap-2">
          {!isLoaded && (
            <Badge variant="secondary">Loading map...</Badge>
          )}
          {userCursors.length > 0 && (
            <Badge variant="outline">
              <Cursor className="h-3 w-3 mr-1" />
              {userCursors.length} active cursors
            </Badge>
          )}
      ...
      {!isLoaded && (
        <div className="absolute inset-0 bg-white bg-opacity-90 flex items-center justify-center">
          <div className="text-center">
            ...
            <div className="text-sm text-muted-foreground">Loading collaborative map...</div>
          </div>
        </div>
      )}
    </div>
