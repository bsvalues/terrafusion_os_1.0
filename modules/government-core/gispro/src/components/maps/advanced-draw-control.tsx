...
export const AdvancedDrawControl: React.FC<AdvancedDrawControlProps> = ({
  ...
  useEffect(() => {
    if (map && featureGroupRef.current && !historyManagerRef.current) {
  ...
  useEffect(() => {
    if (map && !annotationManagerRef.current) {
  ...
  useEffect(() => {
    if (map && featureGroupRef.current && !snapManagerRef.current) {
  ...
  useEffect(() => {
    ...
    const onDrawCreated = (e: any) => {
      ...
      if (measurementManagerRef.current && showMeasurements) {
        measurementManagerRef.current.addMeasurement(layer);
      }
  ...
  const handleClearAll = useCallback(() => {
    if (featureGroupRef.current) {
      ...
      if (annotationManagerRef.current) {
        annotationManagerRef.current.clearAll();
      }
  ...
  const handleAddAnnotation = useCallback(() => {
    if (annotationManagerRef.current && selectedFeature && annotationText) {
  ...
  return (
    <React.Fragment>
      ...
      <div className="leaflet-bar leaflet-control">
        <div className="bg-white border border-gray-300 rounded shadow-md p-2 space-y-1">
          {toolConfigs.map(({ tool, icon: Icon, label, description }) => (
          ...
          <div className="border-t pt-2 space-y-1">
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent side="right">
                  <p>Save current drawing state</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
                ...
                <Tabs defaultValue="drawing" className="w-full">
                  ...
                  <TabsContent value="drawing" className="space-y-4">
                    <div className="space-y-4">
                      ...
                      <div className="flex flex-col gap-1">
                        <Label htmlFor="snap-mode">Snap Mode</Label>
                        <Select 
                          value={snapMode} 
                        ...
                        >
                          ...
                          <SelectContent>
                            ...
                            <SelectItem value={SnapMode.BOTH}>Vertex and Edge</SelectItem>
                          </SelectContent>
                        </Select>
                      </div>
                      ...
                      <div className="flex flex-col gap-1">
                        <Label htmlFor="annotation-text">Add Annotation</Label>
                        <Input
                          id="annotation-text"
                  ...
                  <TabsContent value="history" className="space-y-4">
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <Label>Drawing History</Label>
                        <div className="text-sm text-muted-foreground">
                          {currentHistoryIndex + 1} / {historySnapshots.length}
                        </div>
                      </div>
                            ...
                            {historySnapshots.map((snapshot, index) => (
                              ...
                              >
                                ...
                                <div className="text-xs mt-1 opacity-70">
                                  {snapshot.featureCount || 0} features
                                </div>
                              </div>
                            ))}
                          </div>
      ...
      {/* Measurement display */}
      {measurementText && (
        <div className="leaflet-control-custom-measurement">
          <div className="bg-white border border-gray-300 rounded shadow-md p-2">
            <div className="text-sm font-medium">{measurementText}</div>
          </div>
        </div>
      )}
      ...
      {annotations.length > 0 && (
        <div className="leaflet-control-custom-annotations">
          <div className="bg-white border border-gray-300 rounded shadow-md p-2 max-w-xs">
            ...
            <ScrollArea className="max-h-32">
              {annotations.map((annotation, index) => (
                <div key={annotation.id || index} className="text-xs p-1 border-b">
                  <div className="flex items-center gap-1">
                    <User className="h-3 w-3" />
                    <span className="font-medium">{annotation.author}</span>
                  </div>
                  <div className="text-muted-foreground">{annotation.text}</div>
