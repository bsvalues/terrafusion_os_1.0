...
export function WorkflowDashboard() {
  ...
  const filteredWorkflows = useMemo(() => {
    ...
    let filtered = workflows.filter((workflow: Workflow) => {
      ...
      // Filter by priority
      if (selectedPriority && workflow.priority !== selectedPriority) {
        return false;
      }
      ...
      if (startDate || endDate) {
        const workflowDate = parseISO(workflow.createdAt);
        
        if (startDate && isBefore(workflowDate, startDate)) {
          return false;
        }
        
        if (endDate && isAfter(workflowDate, endDate)) {
          return false;
        }
      }
    ...
    // Sort workflows
    if (sortField) {
      filtered = filtered.sort((a: Workflow, b: Workflow) => {
  ...
  return (
    <div className="container mx-auto p-4">
              ...
              <Dialog open={showDateFilterDialog} onOpenChange={setShowDateFilterDialog}>
                <DialogTrigger asChild>
                  <Button variant="outline" className="flex items-center gap-1">
                    <Calendar size={16} />
                    Date Filter
                    {(startDate || endDate) && <Badge variant="secondary" className="ml-1 px-1 text-xs">Active</Badge>}
                  </Button>
                </DialogTrigger>
                <DialogContent className="sm:max-w-[425px]">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Filter workflows by their creation date
                    </DialogDescription>
                  </DialogHeader>
      ...
      <Dialog open={showPriorityChangeDialog} onOpenChange={setShowPriorityChangeDialog}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader>
            <DialogTitle>Change Workflow Priority</DialogTitle>
            <DialogDescription>
              Update the priority level for this workflow
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-4 py-4">
            <div className="grid grid-cols-4 items-center gap-4">
              ...
              <Select defaultValue={selectedWorkflow?.priority}>
                ...
                {workflowEvents && workflowEvents.length > 0 ? (
                  ...
                  <div className="space-y-4">
                    <div className="flex">
                      ...
                      <div className="flex-1">
                        <h4 className="text-sm font-medium">Workflow Created</h4>
                        <p className="text-sm text-muted-foreground">
                          This workflow was created and is ready for processing.
                        </p>
                        <p className="mt-1 flex items-center text-xs text-muted-foreground">
                          <CalendarDays className="mr-1 h-3 w-3" />
                          {selectedWorkflow && formatDate(selectedWorkflow.createdAt)}
                        </p>
                      </div>
                    </div>
                  </div>
