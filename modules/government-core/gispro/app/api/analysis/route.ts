...
class GAMAAnalysisEngine {
  ...
  private calculateBaseValue(data: AnalysisRequest): number {
    ...
    return Math.max(100000, data.size_sqft * pricePerSqft + bedroomValue + bathroomValue + ageAdjustment)
  }
  ...
  private analyzeMarketConditions(data: AnalysisRequest): number {
    ...
    const baseFactor = locationFactors[data.location_type.toLowerCase() as keyof typeof locationFactors] || 1.0
  ...
  private calculateConfidence(data: AnalysisRequest): number {
    const dataCompleteness =
      Object.values(data).filter((v) => v !== null && v !== undefined).length / Object.keys(data).length
    ...
    return Math.min(0.99, Math.max(0.5, dataCompleteness * 0.4 + comparableFactor * 0.3 + marketStability * 0.3))
  }

  private assessRisk(data: AnalysisRequest, confidence: number) {
    const ageRisk = Math.max(0, (2024 - data.year_built - 20) * 2)
    const locationRisk = { urban: 15, suburban: 10, rural: 25, waterfront: 20 }[data.location_type.toLowerCase()] || 15
  ...
  private generateRecommendations(data: AnalysisRequest, confidence: number, geometryFactor: number): string[] {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const requiredFields = ["address", "size_sqft", "bedrooms", "bathrooms", "year_built", "location_type"]
