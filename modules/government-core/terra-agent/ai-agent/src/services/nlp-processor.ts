...
export class NLPProcessor {
  ...
  private detectIntent(text: string): { name: string; confidence: number; alternatives: any[] } {
    ...
    if (lowerText.includes('analyze') || lowerText.includes('analysis') || lowerText.includes('value')) {
      return { name: 'property_analysis', confidence: 0.8, alternatives: [] };
    }
    if (lowerText.includes('market') || lowerText.includes('trends') || lowerText.includes('prices')) {
      return { name: 'market_analysis', confidence: 0.8, alternatives: [] };
    }
    if (lowerText.includes('invest') || lowerText.includes('investment') || lowerText.includes('roi')) {
  ...
  private extractEntities(text: string): any[] {
    ...
    const addressPattern = /\d+\s+[A-Za-z\s]+(?:street|st|avenue|ave|road|rd|drive|dr|lane|ln|boulevard|blvd)/gi;
  ...
  private analyzeSentiment(text: string): { polarity: number; subjectivity: number; emotion: string } {
    ...
    const polarity = positiveCount > negativeCount ? 0.5 : 
  ...
  private extractTopic(text: string): string {
    ...
    if (lowerText.includes('property') || lowerText.includes('house') || lowerText.includes('home')) {
      return 'property';
    }
