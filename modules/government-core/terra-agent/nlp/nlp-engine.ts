...
export interface AnalyticsIntent {
  ...
  analyticsType: 'market_intelligence' | 'investment_scoring' | 'cma_automation' | 'visualization' | 'general';
  confidence: number;
...
export class NLPEngine {
  ...
  async parseQuery(query: string, userId?: string): Promise<AnalyticsIntent> {
    try {
      ...
      console.log(`✅ Parsed intent: ${primaryAction} (${analyticsType}) - Confidence: ${confidence}`);
      return intent;
  ...
  async extractEntities(query: string): Promise<EntityExtractions> {
    try {
      ...
      console.log(`📍 Extracted entities: ${entities.addresses.length} addresses, ${entities.locations.length} locations`);
      return entities;
  ...
  async determineContext(query: string, history: any[], userId: string): Promise<QueryContext> {
    try {
      const existingContext = this.conversationHistory.get(userId) || this.createDefaultContext(userId);
      ...
      console.log(`💭 Context determined for user ${userId}: ${existingContext.conversationState.currentTopic}`);
  ...
  private determineAnalyticsType(query: string, entities: EntityExtractions): AnalyticsIntent['analyticsType'] {
  ...
  private extractQueryParameters(query: string, entities: EntityExtractions): QueryParameters {
    ...
    const propertyTypeMatches = query.match(/\b(house|home|condo|townhouse|apartment|duplex|commercial)\b/g);
  ...
  private async extractAddresses(query: string): Promise<AddressEntity[]> {
    ...
    // Address patterns
    const fullAddressPattern = /\b\d+\s+[\w\s]+(?:st|street|ave|avenue|rd|road|dr|drive|ln|lane|way|blvd|boulevard)[,\s]+[\w\s]+[,\s]+[A-Z]{2}\b/gi;
    const streetOnlyPattern = /\b\d+\s+[\w\s]+(?:st|street|ave|avenue|rd|road|dr|drive|ln|lane|way|blvd|boulevard)\b/gi;
  ...
  private determineOutputFormat(query: string): AnalyticsIntent['outputFormat'] {
    ...
    if (query.includes('chart') || query.includes('graph') || query.includes('visual')) return 'visualization';
  ...
  private calculateParsingConfidence(entities: EntityExtractions, parameters: QueryParameters): number {
  ...
  private initializeRealEstateTerms(): Map<string, string[]> {
    ...
    terms.set('property_types', [
      'house', 'home', 'single family', 'condo', 'condominium', 'townhouse', 
      'duplex', 'apartment', 'commercial', 'land', 'lot'
    ]);
    
    terms.set('features', [
      'bedroom', 'bed', 'br', 'bathroom', 'bath', 'ba', 'square feet', 'sqft',
      'lot size', 'garage', 'parking', 'pool', 'deck', 'patio'
    ]);
    
    terms.set('analytics_actions', [
      'analyze', 'compare', 'evaluate', 'score', 'predict', 'forecast',
      'estimate', 'value', 'appraise', 'report', 'chart', 'graph'
    ]);
  ...
  private initializeEntityPatterns(): Map<string, RegExp> {
    ...
    patterns.set('address', /\b\d+\s+[\w\s]+(?:st|street|ave|avenue|rd|road|dr|drive|ln|lane|way|blvd|boulevard)\b/gi);
