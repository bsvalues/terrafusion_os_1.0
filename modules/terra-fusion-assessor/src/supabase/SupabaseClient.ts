...
export class TerraFusionSupabaseClient {
  ...
  private handleValuationChange(payload: any): void {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('valuation-change', {
        detail: { type: payload.eventType, data: payload.new || payload.old }
      }));
    }
  }
  ...
  private handleDataQualityAlert(payload: any): void {
    ...
    if (typeof window !== 'undefined') {
      window.dispatchEvent(new CustomEvent('data-quality-alert', {
        detail: payload.new
      }));
    }
  }
  ...
  async getPropertyById(propertyId: string): Promise<{
    ...
    const { data: comparables, error: comparablesError } = await this.client
   ...
   */
  async createProperty(property: Omit<PropertyRecord, 'id' | 'created_at' | 'updated_at'>): Promise<{
   ...
   */
  async updateProperty(propertyId: string, updates: Partial<PropertyRecord>): Promise<{
    data: PropertyRecord | null;
    error: PostgrestError | null;
  }> {
    const { data, error } = await this.client
  ...
  async getDataQualityMetrics(): Promise<{
    ...
    const { data, error } = await this.client
      .rpc('calculate_data_quality_metrics');
   ...
   */
  async getDataQualityIssues(filters: {
    severity?: string;
    tableName?: string;
    unresolved?: boolean;
    limit?: number;
