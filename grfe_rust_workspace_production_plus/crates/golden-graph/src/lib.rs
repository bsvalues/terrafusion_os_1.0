...
#[cfg(feature = "blas")]
pub fn eigh_blas(m: &Array2<f64>) -> Result<Spectrum> {
    use ndarray_linalg::eigh::Eigh;
    let (eigs, evecs) = m.clone().eigh(ndarray_linalg::UPLO::Lower)?;
    let ev: Vec<f64> = eigs.to_vec();
    Ok(Spectrum { eigenvalues: ev, eigenvectors: evecs })
}
