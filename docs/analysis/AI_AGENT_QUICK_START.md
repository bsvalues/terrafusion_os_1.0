...
This is the **MASTER REFERENCE** for any AI agent working on Terrafusion OS.
**IMPLEMENTATION STATUS: COMPLETE** - All major objectives achieved and system
is production-ready.
...
2. **Don't assume mock tests are real** - Real tests are in
   `/modules/testing-suite/` (716 tests)
3. **Don't ignore championship files** - Root directory has critical test
   orchestrators
...
4. **Check `/modules/testing-suite/`** - Contains 716 real tests with 91.9% pass
   rate
5. **Use discovery scripts** - Run `./scripts/discover-all-tests.sh` to find
   everything
...
**A:** Tests are distributed! Check `TEST_REGISTRY.md` - found 361 tests across
10+ locations.

### **Q: How do I run all tests?**

**A:** Run `./scripts/discover-all-tests.sh` then use the generated execution
script.

### **Q: Are the performance tests real?**

**A:** Mock performance tests are in `/tests/mock_tests/`. Real performance
validation is in `/modules/testing-suite/`, `/backend/quantum-performance/`, and
`/scripts/`.
...
**A:** Read `CLAUDE.md`, then `TEST_REGISTRY.md`, then explore the actual
directories.
