...
|---|------------------|-------------|-------|--------|------|
| 2.1 | [x] `routes_data_quality.py` (44.8 KB) | `LevyDataQualityController.cs` + `ILevyDataQualityService` | Copilot | DONE | Ported 7 JSON endpoints → `api/levy/v1/data-quality/{analyze,ai-recommendations,monitoring-status,monitoring/toggle,realtime-metrics,trends,audit}`. Service+controller+DI wired; `TerraFusion.API` build 0/0. Deterministic placeholder logic with 5 canned AI recommendations (filterable by focus area). HTML dashboard routes (`/`, `/rules`, `/errors`, `/activities`, `/rules/create`) skipped per precedent. Runtime bind still blocked upstream. |
| 2.2 | [x] `routes_property_assessment.py` (16.4 KB) | New `LevyPropertyAssessmentController.cs` (distinct from top-level PropertyAssessment — this is the levy-scoped projection) | Copilot | DONE | Ported 4 POSTs → `api/levy/v1/property-assessment/{validate,calculate-value,verify-compliance,execute-workflow}`. Service+controller+DI wired; full `TerraFusion.API` build 0/0. Deterministic placeholder logic (agent wiring deferred to later phase). UI routes skipped per precedent. Also removed stale `ILevyCertificationService` DI line (non-existent types — real cert service lives at `TerraFusion.Core.Services.ICertificationService`, already registered). Runtime bind still blocked upstream. |
...
| 2.5 | [x] `routes_db_fix.py` (7.0 KB) | ~~CLI mode in `TerraFusion.Levy.CLI`~~ | Copilot | CANCELLED | Target tables (`import_log`, `export_log`) and enums (`ImportType`, `ExportType`) are **not present** in `TerraFusion.Levy` or `TerraFusionDbContext`. Utility is Flask-legacy-schema-scoped — no valid .NET port target exists. Re-open if/when Import/Export logging is introduced to the .NET schema. |
