...
`babc2456`; docs shipped in merged PR #1556; V1a code merged in PR #1557).
...
(`babc2456`, PR #1557). Historical gaps are retained for traceability, while
the current delivery state is called out explicitly below.
...
| # | Gap | Evidence |
|---|---|---|
| G1 | **Resolved on current main.** Before PR #1557, ratio stats were computed regardless of n without additive advisory metadata. | PR #1557 now adds `sampleSizeAdequacy` to the existing `GetRatioStudy` response; V1b remains open. |
...
Current state: the documentation slice is merged (#1556) and the corrected V1a
implementation is merged (#1557). V1b study-readiness remains the next bounded
sales-intelligence follow-up; no production mutation or 39-county claim is
implied.
