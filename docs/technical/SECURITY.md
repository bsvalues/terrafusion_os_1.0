# Security Policy
 ...
 - Report vulnerabilities: [security@terrafusion.local](mailto:security@terrafusion.local) (do not open public issues)
 - Do not include sensitive data in reports. Share PoCs privately.

## Key and Secrets Handling

 - Never commit production secrets or private keys. The `keys/` directory must only contain placeholders or dev materials.
 - Rotate keys per-county. Each county (sovereign model) must use distinct credentials.
 - Store secrets in your secure vault (see `frontend/electron/security/secure-vault.js`).
