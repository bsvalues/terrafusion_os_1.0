# Day 10 Evidence Bundle — Wave 0 (EXAMPLE)

> ⚠️ **DEMO ONLY — NOT A PRODUCTION PILOT**  
> This document demonstrates the evidence bundle format. Replace all `EXAMPLE_sha256:` IDs with real identifiers before production use.

> **Pilot:** Wave 0 (Example)  
> **Day:** 10 of 14 (Week 2 Midpoint)  
> **Date:** 2026-02-13  
> **Bundle ID:** `EXAMPLE_sha256:bundle_day10_3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2c3d`

---

## War Room Agenda (≤15 min)

### 1. Attendance Confirmation

| Role | ID | Present |
|------|----|---------|
| Incident Commander | `EXAMPLE_sha256:operator_1a2b...` | ☑ |
| On-Call Engineer | `EXAMPLE_sha256:operator_2b3c...` | ☑ |
| Observability Lead | `EXAMPLE_sha256:operator_3c4d...` | ☑ |

---

### 2. KPI Snapshot

| Metric | Value | Trend | Threshold | Status |
|--------|-------|-------|-----------|--------|
| MTTR | 12m | → | ≤30m | ✔ |
| Rollback Success | 99% | → | ≥95% | ✔ |
| Availability | 99.95% | ↑0.05% | ≥99.5% | ✔ |
| DR Freshness | 34d | +1 | ≤90d | ✔ |

**All KPIs within threshold.** Week 2 midpoint — excellent stability.

---

### 3. Stop-Condition Check

| Condition | Status | Notes |
|-----------|--------|-------|
| MTTR_REGRESSION | ☐ Clear | — |
| ROLLBACK_FAILURE | ☐ Clear | — |
| DR_DRILL_FAILURE | ☐ Clear | — |
| AUDIT_INTEGRITY_ALERT | ☐ Clear | — |

**Stop Triggers Today:** 0  
**Auto-Pause Events:** 0

---

### 4. Exception Status

| Exception ID | Days Remaining | Action |
|--------------|----------------|--------|
| `EXAMPLE_sha256:exc_8b9c...` | 10d | On track |

**Expired Exceptions:** 0

---

### 5. Incident Review

| Incident ID | Severity | Status | MTTR | Notes |
|-------------|----------|--------|------|-------|
| — | — | — | — | No incidents |

---

### 6. Decisions Required

| Decision | Type | Approvers | Status |
|----------|------|-----------|--------|
| — | — | — | No decisions required |

**Week 2 Midpoint Note:** KPIs stable, no stop triggers, war room compliance improving (10/14 = 71%). On track for Day 14 exit.

---

### 7. Actions & Blockers

| Action | Owner | Status |
|--------|-------|--------|
| Prepare exit evaluation pack | IC | Assigned (due Day 14) |

---

## Evidence Artifacts

| Artifact | Hash | Verified |
|----------|------|----------|
| KPI Export | `EXAMPLE_sha256:kpi_day10_...` | ☑ |
| Audit Log | `EXAMPLE_sha256:audit_day10_...` | ☑ |
| Exception Report | `EXAMPLE_sha256:exc_day10_...` | ☑ |

---

## Sign-Off

| Role | ID | Signature | Timestamp |
|------|----|-----------|-----------|
| Incident Commander | `EXAMPLE_sha256:operator_1a2b...` | ☑ | 2026-02-13 09:10 UTC |

---

> ⚠️ **REMINDER:** This is an EXAMPLE instance. Replace all `EXAMPLE_sha256:` identifiers with real values before production use.

---

*Government. Transcended.*
