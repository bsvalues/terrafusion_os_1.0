...
**Contents:** KERNEL_ARCHITECTURE.md **Analysis:** THIS IS THE OS KERNEL
DOCUMENTATION **Decision:** **KEEP AS-IS - THIS IS THE CORE**
...
**Contents:** api_server.py, boot_sequence.py, kernel/, services/, substrate/,
rust-performance-engine/ **Analysis:** THIS IS THE ACTUAL RUNNING OS
...
- This IS TerraFusion OS itself **Critical:** User said "Python cOS on 8090 both
  healthy" - this is working, don't touch

#### **rust-performance-engine/** (Root level)

**Contents:** Cargo.lock, crates/, target/ **Analysis:** Rust performance layer
**Decision:** **EVALUATE - Might be duplicate of
...
**Analysis:** Consciousness layer with universal translation **Decision:**
**MOVE TO os-platform/consciousness/**

- **USER SAID:** "consciousness engine and all the other Non department
  specific"
...
**Contents:** .keep file (likely models stored here) **Decision:** **MOVE TO
os-platform/models/**
...
**Contents:** benton_analysis.json, clark_extraction.json, etc. (County
valuations) **Analysis:** AI intelligence extraction results for counties
...
**Contents:** API service for explanation mode **Decision:** **MOVE TO
os-platform/services/explain-mode/**
...
**Contents:** ai-consciousness/, cybersecurity-command/, emergency-management/,
federal-compliance/, geospatial-intelligence/, operations-tools/,
public-health/, public-records-portal/, public-safety/, research-engine/
**Analysis:** Mix of OS services and applications **Decision:** **SPLIT
REQUIRED**
...
**Contents:** (Need to check) **Decision:** **MOVE TO os-platform/shared/ OR
terrafusion-shared/**
...
**Contents:** (Already exists - need to verify contents) **Decision:** **KEEP -
SHARED LIBRARIES**
...
**Contents:** (Need to check - UI components?) **Decision:** **EVALUATE THEN
SPLIT**
...
**Contents:** government-core/, government-edition/, commercial/,
property-workbench/, terra-levy/, terra-justice/, etc. **Analysis:** THESE ARE
APPLICATIONS **Decision:** **MOVE TO marketplace/modules/**

- **EXCEPT:** terra-sync/, terra-flow/, costforge-ai/ (USER SAID ALREADY MOVED
  TO OS PLATFORM)
...
- terra-bank/, terra-collections/, terra-levy/, terra-justice/ (finance/legal
  apps)
...
**Contents:** (Need to check what's here) **Decision:** **KEEP AND CONSOLIDATE**
...
**Contents:** government-edition/, commercial/, shared/, shock-and-awe/,
tf-audio/, tf-visual/ **Analysis:** Build packages or published packages
**Decision:** **SPLIT**
...
**Contents:** React 18 + TypeScript dashboard **Analysis:** The UI that runs IN
the OS (the dashboard) **Decision:** **KEEP IN ROOT - OS SHELL UI**
...
**Contents:** Terrafusion.Shell.exe, WebView2 host **Analysis:** The native
shell that hosts the UI **Decision:** **KEEP IN ROOT - OS SHELL**
...
**Contents:** demo/, desktop-electron/, elite-showcase/, gui/, ui/ **Analysis:**
Various app shells **Decision:** **EVALUATE EACH**
...
**Contents:** TerraFusion.API, port 5000, serving OS APIs **Analysis:** The OS
API server **Decision:** **KEEP IN ROOT - OS API SERVER**
...
**However:** backend/ might have marketplace-specific code mixed in **TODO:**
Review backend/Controllers/ and backend/Services/

- OS-level controllers/services: Keep
- Marketplace-specific controllers: Move to marketplace/backend/ or marketplace
  modules
...
1. **rust-performance-engine/** - Check if duplicate of
   terrafusion-cos/rust-performance-engine/
...
- kubernetes/, terraform/, docker/, deployment/, infrastructure/, compose/,
  ops/, monitoring/

#### Development Tools

- development/, workspace-optimization/, workspace-explorer/,
  terrafusion-ops-tools/, ai-workspace-companion/, terrafusion-ide-electron/,
  terrafusion-repo-mapper/, tools/ (most), testing/, tests/,
  COMPLETE_TEST_SUITE/, testing-coordination/

#### Documentation

- docs/, PLATFORM_EMPIRE_PLANNING/, plans/, architecture-diagrams/, reports/,
  AUDIT_REPORTS/, VALIDATION_REPORTS/, FORENSIC_REPORTS/, CURRENT_STATUS/
...
- data/, .data/, database/, backups/, archive/, LEGACY_CODE_ARCHIVE/,
  RECOVERY_OPERATION/
...
- sales/, partners/, partner-deliverables/, grants/, hostinger/,
  gov_deploy_packages/, governance/, expansion/, research/

#### Hidden/Config

- .github/, .devcontainer/, .vscode/, .vs/, .claude/, .claudecode/,
  .playwright-mcp/, .githooks/, .husky/, .venv/
...
**The Vision:** Boot TERRAFUSION_OS_CORE → Start terrafusion-cos → Load
os-platform/ services → Launch native-shell → User can then install marketplace/
apps

Just like Windows boots → Starts services → Loads Explorer → User installs
Microsoft Office
