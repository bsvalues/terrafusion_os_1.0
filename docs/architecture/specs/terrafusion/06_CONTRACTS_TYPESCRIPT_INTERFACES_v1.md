...
## 3) TerraTrace Event Contracts
```ts
export type TraceClassification = "PUBLIC"|"CONFIDENTIAL"|"RESTRICTED";
export type TraceEventType =
  | "tool_invoked" | "tool_succeeded" | "tool_failed"
  | "mode_switched" | "permission_denied" | "policy_blocked"
  | "workflow_state_changed" | "artifact_created" | "artifact_published"
  | "redaction_applied" | "retention_archived" | "retention_deleted";
...
export interface TraceEvent {
  ...
  type: TraceEventType;
  ts: string;                 // ISO 8601
  correlationId: string;      // links invoked -> result
  suite: "os"|"forge"|"atlas"|"dais"|"dossier"|"pilot"|"gpt";
  ...
  // Store large/sensitive payloads by reference
  payloadRef?: string;
