...
**Correction of record:** an earlier framing of this finding as "20 sci-fi scaffold modules" was itself marketing-driven (judging by names). The defensible findings are: (a) the *zone* is frozen by the repo's own governance, (b) it is unwired to the live system, (c) stratum 1 is fantasy by its implementation, and (d) strata 2–3 contain substantive code that must not be dismissed by name.
