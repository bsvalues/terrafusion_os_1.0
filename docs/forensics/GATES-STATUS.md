...
| **C — Branch Trustworthiness** | Branch dispositions evidence-backed; **lineage_class + mergeability_class required before branch trust accepted** (tightened) | **FULL (decision layer)** | Lineage + mergeability for all 741 (`branch-census.csv`); risk fields (ci_trust/owner-sensitive/red-flags/ai-reality/product-intent) populated; **per-branch scoring complete** in `R11-GATE-C-SCORING.md` (U/F/O at initiative granularity) with a **scored, fenced, ordered needle set**. Explicit residual *by design*: per-needle commit-level + "already-landed-via-recut?" content check is an **R12-entry** step, not a Gate C blocker. Class-level + needle dispositions are now trustable. |
