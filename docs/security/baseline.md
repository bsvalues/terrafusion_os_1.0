...
| 11 | **SC-12 / SI-7** Secrets in Config | `appsettings.{Env}.local.json` files commit-able in worktree (gitignored, but discovered by audit); historic JWT/DB secrets in older configs | **OPEN** | PR-2 + repo-hygiene sweep; PR-7 replaced `appsettings.BentonCounty.json` `Security.JwtSecret` / `Security.EncryptionKey` literals with `${VAR}` placeholders. Git-history-burned secret inventory and per-deployment verified-rotation steps tracked in [`credential-rotation.md`](credential-rotation.md) (AC-3 / SC-12). |
