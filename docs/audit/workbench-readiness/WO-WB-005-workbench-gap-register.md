...
| **G3** | **Honesty-contract coverage is 4/9 — an *instrumentation* gap** (amended by WO-WB-C1-001/003). Summary/Forge/Atlas/Dais have `*.honesty.contract` tests; the 5 uncovered tabs cannot get them **tests-only** because none renders an **idle `unavailable` source badge** (Clerk/Treasury/Audit/Pilot have no badge; Dossier's are post-invocation `source='live'` only) | **Instrumentation** (was: Test coverage) | **S2** | present: `__tests__/workbench/{PropertyAtlas,PropertyDais,PropertyForge,PropertySummary}.honesty.contract.test.tsx` + `Reconciliation.honesty.contract.test.ts`; badge idle-absent for Clerk/Treasury/Audit/Pilot (none) + Dossier (`PropertyDossier.tsx:755,793,800`, post-invocation only) | WB-002 §4 / WB-003 §3 / **WB-C1-001/003** | *instrument-then-test* — add idle source-badge instrumentation to the 5 tabs first (see `WO-WB-INSTRUMENTATION-001-candidate-packet.md`); **not** tests-only |
...
| Instrumentation | G3 (S2) | honesty idle source-badge instrumentation (amended by WO-WB-C1-001/003 — not tests-only) |
| Test coverage | G4 (S2) | the skipped launch test |
