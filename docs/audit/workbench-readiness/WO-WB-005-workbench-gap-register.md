...
| **G3** | **Honesty-contract coverage is 4/9** — Summary/Forge/Atlas/Dais have `*.honesty.contract` tests; Clerk/Treasury/Audit/Dossier/Pilot do not, despite being tool-output surfaces | Test coverage | **S2** | `__tests__/workbench/*.honesty.contract.*` (present for 4) | WB-002 §4 / WB-003 §3 | *fix* — add honesty contracts for the 5 uncovered tabs |
| **G4** | **Skipped launch-surface contract test** — Phase-16 `launchSurfaceContractParcelWorkbench.contract.test.tsx` skipped since 2026-04-25 (crashed the vitest worker via `SuiteModuleGrid`); re-author recommended in-file | Test coverage | **S2** | `__tests__/shell/launchSurfaceContractParcelWorkbench.contract.test.tsx` | WB-001 §5 | *fix* — re-author with shallow mocks |
...
| **G8** | **Forge sub-tabs lack a path segment** — 5 state-based sub-tabs are query-param deep-linkable (`?tab=`) but have no route path each | UX/routing | **S3** | `PropertyForge.tsx:77-83,110` | WB-002 §2 | *defer* — enhancement, not a defect |
