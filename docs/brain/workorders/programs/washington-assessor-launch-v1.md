...
| `WO-WAL-001F` | Report public-source connector health only from an observed bounded probe; refuse connect and refuse an unimplemented read instead of an empty success | bounded child of open 001 |
...
| `WO-WAL-001F` | R3 | `wal.public-source.connector-health-truth.v1` | `offline-connector-health-proof-with-closed-loopback-sink-only` | No acquisition authorization, DI registration, credential or secret access, county/PACS/CAMA/GIS/Sync or protected data, external write, other connector file, schema or capability claim, runtime population, deployment or production |
