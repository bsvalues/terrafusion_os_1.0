...
**Status:** Active; WO-SR-008A audit complete pending merge; WO-SR-008D admitted behind merge interlock
...
| WO-SR-008D | Restore bounded synthetic single-parcel journey proof | Ready R2; dependency-interlocked until WO-SR-008A merges; two test files only plus governance/evidence |
...
fallback, and the Pilot evidence rail is not durable. `WO-SR-008D` is the smallest same-risk
successor: it restores two stale/skipped contracts without product-source changes or live claims.
