...
|---------|------|------|--------|------------|---------|--------------|------------|
| [Five-Suite Federated Repository Buildout](programs/five-suite-federated-repository-buildout.md) | `GOAL-FIVE-SUITE-FEDERATED-REPOSITORIES` | `LOOP-FIVE-SUITE-FEDERATED-REPOSITORIES` | Active; direct extraction rejected and built-fresh foundation admitted | `WO-SR-005B-F1` standalone spatial projection foundation | Portfolio reconciliation after F1; runtime integration remains gated | Continue exact R3 destination implementation | Stop on scope expansion, sovereign copying, runtime wiring, providers, packages, workflows, county/PACS/SQL, secrets, deployment, or cutover |
