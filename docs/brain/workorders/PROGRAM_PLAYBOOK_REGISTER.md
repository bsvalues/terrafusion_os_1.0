...
*(Updated 2026-07-17 - WO-ATLAS-003 map renderer contract decision)*
...
| [Portfolio Operator](programs/portfolio-operator.md) | `GOAL-PORTFOLIO-OPERATOR-001` | `LOOP-PORTFOLIO-OPERATOR-001` | Active - Mapbox retained as current renderer baseline | `WO-ATLAS-003` complete on protected merge | `WO-ATLAS-004` GeoForge Popup Content Safety Audit | Auto-select, admit, deliver, merge, and continue dependency-cleared authorized nodes | Stop only on scope expansion, failed required proof, destructive action, or protected boundaries |
