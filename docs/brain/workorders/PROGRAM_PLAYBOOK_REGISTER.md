...
|---------|------|------|--------|------------|---------|--------------|------------|
| [Codex Operator Autonomy](programs/codex-operator-autonomy.md) | `GOAL-TF-CODEX-OPERATOR-AUTONOMY-001` | `LOOP-TF-CODEX-OPERATOR-AUTONOMY-001` | Active until merged, then operator autonomy baseline | `WO-OP-AUTO-000` through `WO-OP-AUTO-012` | Return to active owner-selected lane after merge | Owner-authorized docs/governance scope under `docs/brain/workorders/**`; auto within operator autonomy only while validation passes | Stop on merge authority unless pre-granted, branch/merge conflict, scope expansion, runtime/CI/deployment/protected-resource changes, local hook bypass without active owner authorization, destructive operations, or conflicting canon |
