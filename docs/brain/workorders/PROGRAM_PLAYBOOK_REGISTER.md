...
|---------|------|------|--------|------------|---------|--------------|------------|
| [Five-Suite Federated Repository Buildout](programs/five-suite-federated-repository-buildout.md) | `GOAL-FIVE-SUITE-FEDERATED-REPOSITORIES` | `LOOP-FIVE-SUITE-FEDERATED-REPOSITORIES` | Active between cohorts; R2 queue exhausted | `WO-SR-005D-A4` complete; Dossier and GPT F1 incidents open | Exact bounded F1 disposition authority | Reconcile automatically when new bounded authority lands | Stop on retain/revert, extraction, runtime/provider/persistence adoption, F1, packages/publication, unrelated workflows, county/PACS/SQL, credentials, secrets, deployment, cutover, or new scope |
