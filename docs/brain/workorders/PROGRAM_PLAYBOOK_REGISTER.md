...
|---------|------|------|--------|------------|---------|--------------|------------|
| [Five-Suite Federated Repository Buildout](programs/five-suite-federated-repository-buildout.md) | `GOAL-FIVE-SUITE-FEDERATED-REPOSITORIES` | `LOOP-FIVE-SUITE-FEDERATED-REPOSITORIES` | Active; Atlas standalone foundation complete and Dais preparation admitted | `WO-SR-005C-P` Dais domain-contract and county-isolation preparation | Bounded Dais contract decomposition only if evidence is implementation-ready | Continue exact R2 read-only evidence | Stop on source copying, contract/runtime implementation, packages, workflows, county/PACS/SQL, secrets, deployment, or cutover |
