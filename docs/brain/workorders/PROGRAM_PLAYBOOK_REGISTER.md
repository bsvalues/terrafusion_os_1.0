...
| [Local OMEN Runtime Repair](programs/ACTIVE_PROGRAM_PLAYBOOK.md#program-5---local-omen-runtime-repair) | `GOAL-LOCAL-OMEN-RUNTIME-REPAIR` | `LOOP-LOCAL-OMEN-RUNTIME-REPAIR` | Diagnosed - repair authority gated | `WO-LOCAL-093` complete | Proposed `WO-LOCAL-094` disk-path repair | No auto mutation | Docker WSL storage-path repair, engine start, and container action remain separately blocked |
...
| Local OMEN Docker WSL disk target is unavailable | Proposed WO-LOCAL-094 | SW-09 bounded local Docker storage/engine mutation |
