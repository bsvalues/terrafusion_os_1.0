...
| [Portfolio Operator](programs/portfolio-operator.md) | `GOAL-PORTFOLIO-OPERATOR-001` | `LOOP-PORTFOLIO-OPERATOR-001` | Active - GIS token disposition complete; protected-path planner integration next | `WO-ATLAS-008` complete on protected merge | `WO-PORTFOLIO-009` Protected-Path Authority Planner Integration; `WO-ATLAS-009` blocked pending it | Auto-select, admit, deliver, merge, and continue dependency-cleared authorized nodes | Stop only on scope expansion, failed required proof, destructive action, or protected boundaries |
