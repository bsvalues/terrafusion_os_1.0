...
*(Updated 2026-07-18 - WO-PORTFOLIO-012 decomposed every remaining boundary and removed stale P8 routing)*
...
| [Local OMEN Runtime Repair](programs/ACTIVE_PROGRAM_PLAYBOOK.md#program-5---local-omen-runtime-repair) | `GOAL-LOCAL-OMEN-RUNTIME-REPAIR` | `LOOP-LOCAL-OMEN-RUNTIME-REPAIR` | Preservation blocked - historical volume required | `WO-LOCAL-094` complete | Proposed `WO-LOCAL-095` volume recovery | No auto mutation | Reconnect and GUID-match historical storage before drive-letter, junction, or Docker start action |
...
| [Portfolio Operator](programs/portfolio-operator.md) | `GOAL-PORTFOLIO-OPERATOR-001` | `LOOP-PORTFOLIO-OPERATOR-001` | Reconciled - follow-on protected boundary | `WO-LOCAL-094` complete | Proposed `WO-LOCAL-095` historical-volume recovery | Auto-select, admit, deliver, merge, and continue dependency-cleared authorized nodes | Physical historical-volume availability is required; other successors retain their recorded boundaries |
...
| Local OMEN historical Docker data volume is disconnected | Proposed WO-LOCAL-095 | Physical volume recovery and exact GUID match before SW-09 mutation |
