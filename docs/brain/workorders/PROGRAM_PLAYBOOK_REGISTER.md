...
*(Updated 2026-07-17 - WO-ATLAS-007 GeoForge Mapbox token alias cleanup)*
...
| [Portfolio Operator](programs/portfolio-operator.md) | `GOAL-PORTFOLIO-OPERATOR-001` | `LOOP-PORTFOLIO-OPERATOR-001` | Active - GeoForge browser token alias cleanup complete on merge | `WO-ATLAS-007` complete on protected merge | `WO-ATLAS-008` GIS Package Mapbox Token Metadata Disposition Audit | Auto-select, admit, deliver, merge, and continue dependency-cleared authorized nodes | Stop only on scope expansion, failed required proof, destructive action, or protected boundaries |
