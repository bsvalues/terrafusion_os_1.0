...
| [Portfolio Operator](programs/portfolio-operator.md) | `GOAL-PORTFOLIO-OPERATOR-001` | `LOOP-PORTFOLIO-OPERATOR-001` | Active - protected-path authority integrated; GIS metadata alignment next | `WO-PORTFOLIO-009` complete on protected merge | `WO-ATLAS-009` GIS Package Mapbox Token Metadata Alignment | Auto-select, admit, deliver, merge, and continue dependency-cleared authorized nodes | Stop only on scope expansion, failed required proof, destructive action, or protected boundaries |
