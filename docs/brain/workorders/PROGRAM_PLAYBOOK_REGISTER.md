...
| [Portfolio Operator](programs/portfolio-operator.md) | `GOAL-PORTFOLIO-OPERATOR-001` | `LOOP-PORTFOLIO-OPERATOR-001` | Active - GIS package token metadata disposition complete on merge | `WO-ATLAS-008` complete on protected merge | `WO-ATLAS-009` GIS Package Mapbox Token Metadata Alignment | Auto-select, admit, deliver, merge, and continue dependency-cleared authorized nodes | Stop only on scope expansion, failed required proof, destructive action, or protected boundaries |
