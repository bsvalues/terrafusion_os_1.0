...
| [Portfolio Operator](programs/portfolio-operator.md) | `GOAL-PORTFOLIO-OPERATOR-001` | `LOOP-PORTFOLIO-OPERATOR-001` | Reconciled - follow-on protected boundary | `WO-LOCAL-093` complete | Proposed `WO-LOCAL-094` disk-path repair | Auto-select, admit, deliver, merge, and continue dependency-cleared authorized nodes | Local Docker storage/engine mutation needs an exact envelope; other successors retain their recorded boundaries |
