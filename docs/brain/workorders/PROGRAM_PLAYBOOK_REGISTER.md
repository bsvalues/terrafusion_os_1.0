...
|---------|------|------|--------|------------|---------|--------------|------------|
| [Five-Suite Federated Repository Buildout](programs/five-suite-federated-repository-buildout.md) | `GOAL-FIVE-SUITE-FEDERATED-REPOSITORIES` | `LOOP-FIVE-SUITE-FEDERATED-REPOSITORIES` | Active; Atlas extraction blocked on missing domain contract | `WO-SR-005B-C` Atlas read-contract decomposition | Contract implementation packet only if decomposition passes | Continue same-risk discovery and contract design | No cutover; no Atlas extraction before a hash-frozen contract and parity gate exist |
