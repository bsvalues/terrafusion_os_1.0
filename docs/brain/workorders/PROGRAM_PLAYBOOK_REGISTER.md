...
| [Portfolio Operator](programs/portfolio-operator.md) | `GOAL-PORTFOLIO-OPERATOR-001` | `LOOP-PORTFOLIO-OPERATOR-001` | Active - Atlas popup text-safety repair complete on merge | `WO-ATLAS-002` complete on protected merge | `WO-ATLAS-003` Map Renderer Contract Decision | Auto-select, admit, deliver, merge, and continue dependency-cleared authorized nodes | Stop only on scope expansion, failed required proof, destructive action, or protected boundaries |
