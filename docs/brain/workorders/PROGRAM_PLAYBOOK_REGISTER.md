...
|---------|------|------|--------|------------|---------|--------------|------------|
| [Five-Suite Federated Repository Buildout](programs/five-suite-federated-repository-buildout.md) | `GOAL-FIVE-SUITE-FEDERATED-REPOSITORIES` | `LOOP-FIVE-SUITE-FEDERATED-REPOSITORIES` | Active; Forge valuation-kernel parity complete | `WO-SR-005B-P` Atlas contract/parity preparation | `WO-SR-005B` if its contract and parity gates are proven | Continue same-risk discovery and bounded copy-first work | No cutover; no Atlas extraction before the preparation gate passes |
