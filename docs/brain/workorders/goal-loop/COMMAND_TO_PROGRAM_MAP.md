...
| `program-next` | Five-Suite Federated Repository Buildout | WO-SR-002 complete on merge; WO-SR-003A-E physical creation credential-blocked | NO - continue in-repo work; isolate the external credential boundary | `program`, `evidence`, `discovery` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | WO-SR-002 shared contract freeze | NO - active ratified program | `program`, `evidence`, `discovery` |
