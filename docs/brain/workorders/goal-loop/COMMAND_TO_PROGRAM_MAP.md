...
| `program-next` | Five-Suite Federated Repository Buildout | `WO-SR-008G` Forge cost fact/schedule semantics audit after WO-SR-008F merge | YES - WO-SR-008F merge and post-merge verification required | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-008F` closing; `WO-SR-008G` next behind merge interlock | NO - active; valuation-policy invention and product/runtime/test/host mutation remain blocked | `program`, `once`, `evidence` |
