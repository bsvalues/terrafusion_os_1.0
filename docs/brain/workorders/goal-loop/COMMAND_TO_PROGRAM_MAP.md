...
| `program-next` | Portfolio Operator | `WO-SR-006` Forge canonical runtime ownership cutover | NO - exact Forge-only R4 authority is active | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-006` active under sequential Mode B | NO - continue only inside the exact Forge cutover envelope | `program`, `once`, `evidence` |
