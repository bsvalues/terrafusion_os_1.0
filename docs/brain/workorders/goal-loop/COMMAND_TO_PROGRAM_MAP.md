...
| `program-next` | Five-Suite Federated Repository Buildout | `WO-SR-008H` Forge cost schedule version/modifier projection contract after WO-SR-008G merge | YES - WO-SR-008G merge and post-merge verification required | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-008G` closing; `WO-SR-008H` next behind merge interlock | NO - active; valuation-policy invention and product/runtime/test/host mutation remain blocked | `program`, `once`, `evidence` |
