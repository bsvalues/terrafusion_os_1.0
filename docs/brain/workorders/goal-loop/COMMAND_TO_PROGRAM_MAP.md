...
| `program-next` | Five-Suite Federated Repository Buildout | `WO-SR-008D` synthetic single-parcel journey proof restoration after WO-SR-008A merge | NO - same-risk R2 evidence restoration | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-008A` closing; `WO-SR-008D` next behind merge interlock | NO - active; product/runtime mutation remains blocked | `program`, `once`, `evidence` |
