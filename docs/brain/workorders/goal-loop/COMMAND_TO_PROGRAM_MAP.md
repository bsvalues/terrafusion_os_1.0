...
| `program-next` | Five-Suite Federated Repository Buildout | Portfolio reconciliation after completed `WO-SR-009B` | NO - operator selects the next bounded node from live evidence | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | Active between cohorts; `WO-SR-009B` complete | NO - portfolio reconciliation is current operator work | `program`, `once`, `evidence` |
