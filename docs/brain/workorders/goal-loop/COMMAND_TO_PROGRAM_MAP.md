...
| `program-next` | Portfolio Operator | Reconcile after completed `WO-SR-006A` | NO - operator recomputes the graph | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | Active between cohorts; local Forge shadow proof complete | NO - no automatically admitted cutover | `program`, `once`, `evidence` |
