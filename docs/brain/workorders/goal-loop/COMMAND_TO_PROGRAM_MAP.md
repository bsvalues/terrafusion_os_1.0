...
| `program-next` | Portfolio Operator | `WO-SR-007A` Atlas local sovereign shadow projection proof | NO - exact bounded R3 authority is active | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-007A` Phase 0 activation, then implementation and closeout | NO - exact local proof authority active; runtime adoption remains blocked | `program`, `once`, `evidence` |
