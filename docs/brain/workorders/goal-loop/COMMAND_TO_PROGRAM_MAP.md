...
| `program-next` | Portfolio Operator | TRUE_PORTFOLIO_BOUNDARY after WO-PORTFOLIO-012; recommend bounded read-only WO-LOCAL-093 grant | YES - every useful successor has an exact protected or strategic activation boundary | `once`, `evidence`, `discovery` |
...
| `local-omen-status` | Local OMEN Runtime Repair | WO-LOCAL-093 read-only diagnosis | YES - exact local diagnosis envelope absent; repair remains separately blocked | `evidence`, `discovery` |
