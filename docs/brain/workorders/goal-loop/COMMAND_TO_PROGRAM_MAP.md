...
| `program-next` | Portfolio Operator | Reconcile after completed `WO-SR-006B` | NO - operator recomputes the graph | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | Forge process-local selection and rollback proven on merge | NO - no persistent adoption or cutover implied | `program`, `once`, `evidence` |
