...
| `program-next` | Portfolio Operator | `WO-SR-007A` implementation proof, then terminal closeout | NO - exact bounded R3 authority is active | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-007A` implementation merge, then terminal closeout | NO - exact local proof authority active; runtime adoption remains blocked | `program`, `once`, `evidence` |
