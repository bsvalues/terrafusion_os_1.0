...
| `program-next` | Portfolio Operator | Portfolio reconciliation; separately authorized Dais F1 only | YES - Dossier and GPT F1 are retained and closed; remaining successor F1 work requires exact R3 authority | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | Portfolio reconciliation; separately authorized Dais F1 only | YES - Dossier and GPT F1 remediation is complete; extraction/runtime/provider/protected scope remains gated | `program`, `once`, `evidence` |
