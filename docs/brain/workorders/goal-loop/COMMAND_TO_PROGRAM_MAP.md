...
| `program-next` | Portfolio Operator | WO-SR-005D-A4 Dossier path-canon registration | NO for R2 registration; YES for extraction, runtime/provider adoption, F1, successor R3, or protected scope | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | WO-SR-005D-A4 after GPT E3 found no executable direct-copy slice | NO for R2 registration; YES for extraction, runtime/provider adoption, F1, successor R3, or protected scope | `program`, `once`, `evidence` |
