...
| `program-next` | Five-Suite Federated Repository Buildout | `WO-SR-008H-E1` pure cost schedule projection foundation after WO-SR-008H merge and exact R3 authority | YES - bounded product/test authority required | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-008H` closing; `WO-SR-008H-E1` proposed behind R3 authority | YES - exact E1 authority required; broader valuation/runtime scope remains denied | `program`, `once`, `evidence` |
