...
| `program-next` | Portfolio Operator | `WO-SR-005E-A2` GPT source-identity projection design | YES only if the design proves a new protected or R3 implementation boundary | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-005E-A2` ready; GPT source-identity decomposition | YES for adapter implementation, extraction, runtime/provider adoption, F1, or any new R3/protected scope | `program`, `once`, `evidence` |
