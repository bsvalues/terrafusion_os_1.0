...
| `terrafusion-owner-usable-product-v1` | TerraFusion Owner-Usable Product V1 | 000 + 001 (matrix v1) on protected main (43ef0238e); Step Zero UI lineage SETTLED (os-shell -> native-shell/ui/dist -> API canonical; electron superseded); native-host release authority OPEN (WPF = surviving candidate, no CI packages it); children: 002 auth honesty + sign-in, 003 lineage drift fix (ready), 004 canonical release assembly + host release-proof (blocked on 003), 005 launcher reconciliation (ready) | NO - owner mission authority recorded (Issue #1589); exact reservations still required | `once`, `program`, `merge-watch`, `evidence`, `recovery`, `discovery` |
