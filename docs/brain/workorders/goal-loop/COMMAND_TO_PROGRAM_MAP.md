...
| `program-next` | Five-Suite Federated Repository Buildout | `WO-SR-008A` single-parcel assessor journey runtime-readiness audit | NO - dependency-cleared R2 evidence node | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-008A` single-parcel assessor journey runtime-readiness audit | NO - active; product/runtime mutation remains blocked | `program`, `once`, `evidence` |
