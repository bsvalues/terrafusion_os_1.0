...
| `program-next` | Portfolio Operator | `WO-SR-006` Forge sovereign implementation ready for PR | NO - exact Forge-only R4 authority is active | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-006` Phase 2 under sequential Mode B | NO - continue only inside the exact Forge cutover envelope | `program`, `once`, `evidence` |
