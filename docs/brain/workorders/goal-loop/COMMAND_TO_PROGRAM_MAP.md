...
| `program-next` | Portfolio Operator | `WO-SR-007B` exact Atlas process-host foundation boundary | NO - new `backend/src/**` and runtime-capable process boundary require exact R3 authority | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-007B-P` complete; `WO-SR-007B` protected successor | NO - process-host implementation, runtime adoption, and cutover remain blocked | `program`, `once`, `evidence` |
