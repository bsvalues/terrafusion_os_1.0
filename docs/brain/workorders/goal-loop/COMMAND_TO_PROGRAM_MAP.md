...
| `program-next` | Portfolio Operator | Dais E1/E2 complete; reconcile the next bounded cohort | YES if every candidate crosses a new protected boundary | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | Dais E1/E2 complete; portfolio reconciliation current | YES for F1 or any other ungranted R3/protected scope | `program`, `once`, `evidence` |
