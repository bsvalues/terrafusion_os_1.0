...
| `program-next` | Five-Suite Federated Repository Buildout | Consolidated Forge consumer completion envelope decision after completed `WO-SR-008H-E1` | YES - new exact R3 consumer scope required | `program`, `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | Portfolio reconciliation; E1 complete and no implementation successor admitted | YES - broader valuation/runtime scope remains denied | `program`, `once`, `evidence` |
