...
| `program-next` | Five-Suite Federated Repository Buildout | `WO-SR-009C` Point-mapping amendment | YES - frozen adapter/test scope expansion required | `once`, `evidence` |
| `suite-repositories` | Five-Suite Federated Repository Buildout | `WO-SR-009C` approved; blocked at exact adapter scope wall | YES - decide exact Point-mapping amendment | `once`, `evidence` |
