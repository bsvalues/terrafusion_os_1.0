...
| Automatic startup migration policy is not decided | Major | CurrentUse calls `MigrateAsync()` for non-InMemory/non-SQLite providers. | Release policy must decide whether startup migration is permitted in shared/prod environments. |
