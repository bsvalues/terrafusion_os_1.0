...
| F13 | `brain/packs/README.md:37-50` | Cross-repo allocation could rely on repo name or assumption without exact path canon. | NARROW | Cross-repo dispatch fails closed until committed path-canon exists; in-repo pack map remains usable. |
...
| F15 | `docs/brain/workorders/STOP_WALL_REGISTER.md:13-16,76-78` | Routine merge conflict and in-scope recovery could be misclassified as authority walls. | CLARIFY | Walls require unresolved new authority; routine conflict, review, validation, and approved repair are non-walls. |
