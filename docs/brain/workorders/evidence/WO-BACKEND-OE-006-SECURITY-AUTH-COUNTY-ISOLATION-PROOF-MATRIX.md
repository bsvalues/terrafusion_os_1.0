...
| Audit middleware | `AuditLoggingMiddleware` logs API calls, 403 authorization denials, and 4xx/5xx response details through `IAuditLogger`, while skipping health/static/event paths. | Request/authorization/error audit surfaces exist, with explicit skip paths. |
