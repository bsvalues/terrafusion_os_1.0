...
WO-LOCAL-093 completed the authorized read-only Local OMEN diagnosis. Docker Desktop 4.79.0 cannot
initialize its Linux/WSL engine because the configured disk path is a junction to unavailable
`E:\DockerData`. The named proof containers are not observable until that storage-path dependency is
repaired. No runtime mutation occurred.
...
| local-omen-runtime-repair | proposed WO-LOCAL-094 | SW-09 / local Docker storage mutation | WO-LOCAL-093 diagnosed an unavailable junction target; repair and engine restart remain separately blocked | `evidence/WO-LOCAL-093-LOCAL-OMEN-READ-ONLY-DIAGNOSIS.md` |
...
**Portfolio result after WO-LOCAL-093:** `FOLLOW_ON_PROTECTED_BOUNDARY`. The read-only diagnosis is
complete. The exact next candidate is `WO-LOCAL-094 - Docker WSL Disk Path Repair and OMEN
Re-observation`; it requires a bounded local runtime-mutation envelope. The registry and wave planner
return no executable node inside current authority.
