...
| Multi-county catalog set | `C54-MULTI-*` | **C54-MULTI-A landed** — policy locked; C54-MULTI-B implementation deferred. See `docs/sync/pacs-schema-multi-county-catalog-policy.md`. | Catalogs per county (so different counties' PACS schemas live side-by-side). Identity is `(CountyId, SourceConnectionId)`; manifests are per-catalog; no implicit default; four isolation rules (ISOL-1..ISOL-4) prevent cross-county manifest / dictionary / fallback / report-diff bleed. |
