...
> (agency-backed). **Stage 3A** (`1ce148235`) discovered the payment doctrine: `bill.amount_paid`
> reconciles exactly to the collection ledger (`coll_transaction.base_amount_pd`), so the bill-grain
> **net paid / balance** truth is already carried by the two sealed bill models. The "Revenue Spine
> deferred" disposition below is superseded for those stages; the full cash ledger
> (receipt/tender/void history), fund/distribution, delinquency, and prior-year history remain
> deferred. See `benton-revenue-spine-stage1-addendum.md`, `…-stage2b-addendum.md`,
> `…-stage3a-addendum.md`.
...
| Revenue Spine — bill-grain **net paid / balance** (L and A) | **✅ Covered** by Stage 1 + 2B; Stage 3A (`1ce148235`) proved `bill.amount_paid` ≡ `SUM(coll_transaction.base_amount_pd)` exactly. See addendum #3. |
| Revenue Spine — receipt/tender/void ledger · fund / distribution / delinquency / history | **Deferred** — treasurer-grade cash ledger. `fund_id` deliberately excluded from canonical revenue/jurisdiction entities. |
