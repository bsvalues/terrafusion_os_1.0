...
- **Windows vs. POSIX scripts**: Prefer `.ps1` on Windows PowerShell; `.sh`
  requires WSL or Git Bash. Example: `deploy-benton-fixed.ps1`.
- **Multiple tsconfig files**: Use the project-specific `tsconfig.json` (e.g.,
  `frontend/tsconfig.json`) when running editors/TS tooling.
- **Playwright MCP config**: If MCP tests fail, verify
  `playwright.mcp.config.ts` exists at repo root and credentials in
  `.env.local`.
- **Electron context isolation**: Ensure any Node access is proxied via
  `frontend/electron/preload.js` and `frontend/electron/os-bridge.js`.

---

**💡 Pro Tip**: Always start with `README.md` and `START_HERE.md` for
