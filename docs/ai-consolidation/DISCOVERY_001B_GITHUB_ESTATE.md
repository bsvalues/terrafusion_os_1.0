...
> **Authorization (governance trail):** TF-AI-OPS-001 directive, per the linkage recorded in
> [`DISCOVERY_001A_REPO_QUARANTINE.md`](DISCOVERY_001A_REPO_QUARANTINE.md) §Authorization.
> **Provenance — two passes:** Pass 1 (merged via #955) ran in a cloud session whose GitHub tooling
> was scoped to `terrafusion_os_1.0` only; it enumerated all 130 repos but recorded every per-repo
> verdict as **metadata-grade, NOT file-verified**, and explicitly requested file-level verification
> from a broader-scoped session. This document is **Pass 2: the file-verified sweep** (local session,
> account-wide `gh` raw-content reads, ~30 AI-relevant repos inspected at tree+file level). It
> supersedes Pass 1's verdicts; Pass 1's estate-shape buckets remain accurate.
> **Pass-1 open questions, now answered:** `terrafusion-ai-platform` family = **duplicate of the main
> tree** (verified — compiled extraction of `os-platform/ai-systems`); `PACS-DataBridge` = **real,
> local classical-ML** (sklearn/transformers, no LLM API; early, 39 files); `TerraAgent` = **Ollama-capable**
> via `utils/llm_providers/` (OpenAI default); `terragroq` = **unmodified Vercel×Groq template fork** (vapor).
