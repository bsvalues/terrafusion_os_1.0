...
| Stale local-dev network or cache volumes | Local Docker state from prior toolbox/profile runs remains | `docker network ls` and `docker volume ls`, limiting attention to TerraFusion/Compose-project resources and not inspecting or removing unrelated assets | Run `docker compose -f docker/dev/compose.yaml --env-file docker/dev/.env.example down` to remove containers/networks, or add `--volumes` to remove local cache volumes | Cleanup would delete unrelated Docker assets |
