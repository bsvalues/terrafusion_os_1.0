...
try {
    if (process && process.stdout && typeof process.stdout.on === 'function') {
        process.stdout.on('error', (e) => { try { appendMainLog('STDOUT_ERROR', String(e)); } catch (_) { } });
    }
    if (process && process.stderr && typeof process.stderr.on === 'function') {
        process.stderr.on('error', (e) => { try { appendMainLog('STDERR_ERROR', String(e)); } catch (_) { } });
    }
} catch (e) {
    // ignore
}
...
class TerraFusionCOSDesktop {
    ...
    createMainWindow() {
        ...
        const useEmbeddedBrand = process.env.TF_USE_EMBEDDED_BRAND === '1' || process.env.TF_USE_EMBEDDED_BRAND === 'true';
        ...
        const startEmbeddedServer = () => {
            ...
            return new Promise((resolve, reject) => {
                ...
                const serverInstance = appServer.listen(brandPort, '127.0.0.1', () => {
                    ...
                    appendMainLog('Embedded brand server started', `port=${brandPort}`, `assets=${assetsPath}`);
                    resolve();
                });
            });
        };
                    ...
                    (async () => {
                        ...
                        for (let attempt = 1; attempt <= 3; attempt++) {
                            try {
                                ...
                                if (img && !img.isEmpty && typeof img.toPNG === 'function') {
                                    try { fs.writeFileSync(pngPath, img.toPNG()); appendMainLog('Saved renderer screenshot', pngPath); captured = true; break; } catch (e) { appendMainLog('Failed to write screenshot', e && e.message ? e.message : String(e)); }
                                }
                            } catch (err) {
                                lastErr = err;
                                appendMainLog(`capturePage attempt ${attempt} failed`, err && err.message ? err.message : String(err));
                            }
                        ...
                        if (!captured) {
                            ...
                            try {
                                ...
                                const data = await this.mainWindow.webContents.printToPDF({ printBackground: true });
                                if (data && data.length) {
                                    try { fs.writeFileSync(pdfPath, data); appendMainLog('Saved renderer PDF fallback', pdfPath); captured = true; }
                                    catch (e) { appendMainLog('Failed to write PDF fallback', e && e.message ? e.message : String(e)); }
                                }
                            } catch (err) {
                        ...
                        if (!captured) {
                            ...
                            try {
                                ...
                                const html = await this.mainWindow.webContents.executeJavaScript('document.documentElement.outerHTML', true);
                                if (html) {
                                    try { fs.writeFileSync(htmlPath, html, { encoding: 'utf8' }); appendMainLog('Saved renderer HTML snapshot', htmlPath); captured = true; }
                                    catch (e) { appendMainLog('Failed to write HTML snapshot', e && e.message ? e.message : String(e)); }
                                }
                            } catch (err) {
                                appendMainLog('HTML snapshot failed', err && err.message ? err.message : String(err));
                            }
                        }

                        if (!captured) {
                            appendMainLog('All capture strategies failed', lastErr && lastErr.message ? lastErr.message : String(lastErr));
                        }
                    })();
                });
            } catch (err) {
                appendMainLog('Brand URL unreachable after embedded server start', err && err.message ? err.message : String(err));
