...
class VendorModuleMarketplace:
    ...
    def __init__(self):
        # Feature flag: only enable marketplace behavior when explicitly allowed
        import os
        self._marketplace_enabled = os.getenv("TF_ENABLE_MARKETPLACE", "false").lower() in ("1", "true", "yes")

        if not self._marketplace_enabled:
            logging.info("Vendor module marketplace is disabled by TF_ENABLE_MARKETPLACE (default: false).")
    ...
    def search_modules(self, query: str, category: str = None, 
        ...
        # If marketplace is disabled, return an empty list to avoid exposing marketplace modules
        if not getattr(self, "_marketplace_enabled", False):
            logging.debug("Marketplace search requested but TF_ENABLE_MARKETPLACE is false; returning empty results.")
            return []
    ...
    def get_featured_modules(self) -> List[VendorModule]:
        ...
        # If marketplace is disabled, return empty featured list
        if not getattr(self, "_marketplace_enabled", False):
            logging.debug("Requested featured modules but marketplace is disabled.")
            return []
    ...
    def get_category_modules(self, category: str) -> List[VendorModule]:
        ...
        if not getattr(self, "_marketplace_enabled", False):
            logging.debug("Requested category modules but marketplace is disabled.")
            return []
