...
class TerraFusionErrorLogger {
      ...
      const observer = new PerformanceObserver(list => {
        for (const entry of list.getEntries()) {
          if (entry.duration > 3000) {
            // Operations taking more than 3 seconds
            this.logError(
              {
  ...
  logError(errorData, severity = 'error') {
    ...
    if (this.config.enableConsoleLog) {
      ...
      console.log(
        `${color}[Terrafusion ${severity.toUpperCase()}]:\x1b[0m`,
        errorData.message
      );
    }
...
if (typeof window !== 'undefined') {
  ...
  window.logError = (message, data) =>
    window.tfErrorLogger.error(message, data);
  window.logWarning = (message, data) =>
    window.tfErrorLogger.warning(message, data);
