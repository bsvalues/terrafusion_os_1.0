...
class SecurityHardener {
  ...
  async runPenetrationTest(): Promise<VulnerabilityReport[]> {
    console.log('Running comprehensive penetration testing...');
  ...
  private async testSQLInjection(): Promise<VulnerabilityReport[]> {
    ...
    for (const endpoint of testEndpoints) {
      for (const payload of sqlPayloads) {
        try {
          const response = await axios.get(`${this.baseUrl}${endpoint}?q=${encodeURIComponent(payload)}`, {
    ...
    for (const endpoint of inputEndpoints) {
      for (const input of maliciousInputs) {
        try {
          ...
          if (response.data && typeof response.data === 'string') {
            if (response.data.includes('49') || // 7*7 result
...
describe('Security Hardening Tests', () => {
  ...
  beforeAll(async () => {
    hardener = new SecurityHardener(process.env.TEST_API_URL || 'http://localhost:5000');
