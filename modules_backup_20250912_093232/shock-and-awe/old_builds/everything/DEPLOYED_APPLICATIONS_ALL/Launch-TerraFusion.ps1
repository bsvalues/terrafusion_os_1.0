...
Write-Host "   1. Start with BCBSGISPRO (port \${{TF_FRONTEND_PORT:-3000}}) - Full property search & GIS" -ForegroundColor White
Write-Host "   2. Use TerraFlow (port \${{TF_FRONTEND_PORT:-3000}}) - Workflow engine for assessments" -ForegroundColor White
Write-Host "   3. Access TerraFusionPro (port \${{TF_FRONTEND_PORT:-3000}}) - Complete professional suite" -ForegroundColor White
Write-Host "   4. View analytics in Dashboard (port \${{TF_FRONTEND_PORT:-3000}}) - Advanced reporting" -ForegroundColor White
Write-Host "   5. Run valuations with BSIncomeValuation (port \${{TF_FRONTEND_PORT:-3000}}) - Valuation engine" -ForegroundColor White
Write-Host ""
...
try {
    ...
    Write-Host "⚠️ Could not open browser automatically. Please navigate to http://localhost:\${{TF_FRONTEND_PORT:-3000}}" -ForegroundColor Yellow
}
