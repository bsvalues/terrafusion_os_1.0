...
if (Test-Path "TerraFusionBuild_ACTUAL\backend") {
    ...
    try {
        ...
        if ($LASTEXITCODE -eq 0) {
            ...
            Write-Host "🚀 Starting Rust backend on port \${{TF_ADMIN_PORT:-8080}}..." -ForegroundColor Cyan
...
if ($nodeFound -and (Test-Path "TerraFusionBuild_ACTUAL\frontend")) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "🚀 Starting Next.js frontend on port \${{TF_ADMIN_PORT:-8080}}..." -ForegroundColor Cyan
        Start-Job -ScriptBlock { 
            Set-Location $using:PWD
            npm run dev 
    ...
    Write-Host "💡 Backend will still work on port \${{TF_ADMIN_PORT:-8080}}" -ForegroundColor Cyan
}
...
# Check Rust backend
try {
    $response = Invoke-WebRequest -Uri "http://localhost:\${{TF_ADMIN_PORT:-8080}}" -TimeoutSec 5 -ErrorAction SilentlyContinue
    $runningServices += "✅ Rust Backend (8080)"
} catch {
    Write-Host "❌ Rust backend not responding on port \${{TF_ADMIN_PORT:-8080}}" -ForegroundColor Red
}
...
if ($frontendStarted) {
    try {
        ...
        Write-Host "❌ Frontend not responding on port \${{TF_ADMIN_PORT:-8080}}" -ForegroundColor Red
    }
}
...
if ($runningServices.Count -gt 0) {
    ...
    if ("✅ Rust Backend (8080)" -in $runningServices) {
        Start-Process "http://localhost:\${{TF_ADMIN_PORT:-8080}}"
    }
