...
def main():
    ...
    print("\n🔥 Starting TerraFlow on port \${{TF_API_HTTPS_PORT:-5001}}...")
    print("   URL: http://localhost:\${{TF_API_HTTPS_PORT:-5001}}")
    ...
    try:
        ...
        process = subprocess.Popen([
            sys.executable, "-c",
            "from app import app; app.run(host='0.0.0.0', port=\${{TF_API_HTTPS_PORT:-5001}}, debug=False)"
        ], env=env)
        ...
        try:
            response = requests.get("http://localhost:\${{TF_API_HTTPS_PORT:-5001}}", timeout=5)
            if response.status_code == 200:
                print("✅ TerraFlow is LIVE and responding!")
                print("\n🎯 Access Points:")
                print("   - Main App: http://localhost:\${{TF_API_HTTPS_PORT:-5001}}")
                print("   - File Manager: http://localhost:\${{TF_API_HTTPS_PORT:-5001}}/files")
                print("   - Dashboard: http://localhost:\${{TF_API_HTTPS_PORT:-5001}}/mcp-dashboard")
                print("   - API Tester: http://localhost:\${{TF_API_HTTPS_PORT:-5001}}/api-tester")
            else:
