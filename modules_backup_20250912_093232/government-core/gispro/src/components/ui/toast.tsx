...
export interface ToastProps {title?: string;
  description?: string;
...
const Toast = React.forwardRef<HTMLDivElement, ToastProps>(
  ({className, variant = 'default', title, description, action, onClose, ...props}, ref) =>{
    return (<div
        ...
        {...props}
      ><div className="grid gap-1 flex-1">{title &&<h4 className="font-medium leading-none">{title}</h4>}
          ...
          ><X className="h-4 w-4" /><span className="sr-only">Close</span></button>)}</div>
    );
  }
);
...
const ToastClose = React.forwardRef<
  HTMLButtonElement,
  React.ButtonHTMLAttributes<HTMLButtonElement>>(({className, ...props}, ref) => (<button
    ref={ref}
    className={cn(
    ...
    toast-close=""
    {...props}
  ><X className="h-4 w-4" /><span className="sr-only">Close</span></button>
));
...
const ToastDescription = React.forwardRef<
  ...
  React.HTMLAttributes<HTMLParagraphElement>>(({className, ...props}, ref) => (<p ref={ref} className={cn('text-sm opacity-90', className)} {...props} />));
