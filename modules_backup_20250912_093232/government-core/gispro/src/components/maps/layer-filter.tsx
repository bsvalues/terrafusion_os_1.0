...
import {Map, 
  Control, 
  DomUtil, 
  DomEvent} from 'leaflet';
import {MapLayerType,
  MapLayer} from '@/lib/map-utils';
import {Checkbox} from "@/components/ui/checkbox";
import {Input} from "@/components/ui/input";
import {Button} from "@/components/ui/button";
import {X, Filter, Plus} from '@mui/icons-material';

interface PropertyFilter {name: string;
  value: string;
  operator?: 'equals' | 'contains' | 'greaterThan' | 'lessThan';}

interface LayerFilterProps {map: Map;
 ...
 */
const LayerFilter: React.FC<LayerFilterProps> = ({map,
  position,
  layers,
  onFilterChange}) => {const [isExpanded, setIsExpanded] = useState(false);
  ...
  const layerTypes = React.useMemo(() => {
    ...
    return Array.from(types);}, [layers]);

  // Toggle layer type filter
  const toggleLayerType = (type: MapLayerType) => {let newTypes: string[];
    
    if (selectedTypes.includes(type)) {
      newTypes = selectedTypes.filter(t => t !== type);} else {newTypes = [...selectedTypes, type];}
    
    setSelectedTypes(newTypes);
    
    // Apply filters when a layer type is toggled
    if (map) {applyLayerFilters(map, newTypes);}
  ...
  const addPropertyFilter = () => {if (!propertyName || !propertyValue) return;
  ...
  const removePropertyFilter = (index: number) => {// In future iterations, we'll handle property filters};

  // Apply filters - directly apply current selectedTypes
  const applyFilters = () => {// In our simplified version, we'll just use the layer type filters
    if (map) {
  ...
  // Clear all filters
  const clearFilters = () => {const emptyTypes: string[] = [];
  ...
  const formatOperator = (operator?: PropertyFilter['operator']) => {switch(operator) {
      case 'equals': return '=';
  ...
  const LayerFilterControl = Control.extend({
    onAdd: function() {
      ...
      createPortal(
        <div className="layer-filter-control"><div className="flex flex-col space-y-2"><div className="flex items-center justify-between"><h3 className="font-medium flex items-center"><><Filter className="w-4 h-4 mr-1" />Layer Filter</h3><button
</>onClick={() => setIsExpanded(!isExpanded)}
                className="px-2 py-1 text-xs bg-gray-200 rounded"
              >
                {isExpanded ? 'Collapse' : 'Expand'}</button></div>{isExpanded && (
                {/* Layer type filters */}<div className="border-t pt-2"><><h4 className="text-xs font-medium mb-1">Layer Type</h4><div
                    ...
                    {layerTypes.map(type => (<label 
                      ...
                      ><Checkbox 
                        ...
                        /><span>{type.charAt(0).toUpperCase() + type.slice(1)}</span></label>))}</div></div>{/* Property filters */}<div className="border-t pt-2"><h4 className="text-xs font-medium mb-1">Properties</h4>{/* Property filter inputs */}<div className="flex flex-col space-y-1"><Input
                      placeholder="Enter property name"
                      value={propertyName}
                      onChange={(e) => setPropertyName(e.target.value)}
                      className="h-7 text-xs"
                    /><div className="flex space-x-1"><select
                      ...
                      ><><option value="equals">=</option><option
</>
value="contains">contains</option><><option value="greaterThan">&gt;</option><option
</>
value="lessThan">&lt;</option></select><Input
                      ...
                      ><Plus className="h-3 w-3" /></Button></div></div>{/* Property filters section is simplified in this version */}<div className="mt-2 text-xs text-muted-foreground">Property filtering will be available in the next update.</div></div>{/* Filter actions */}<div className="flex justify-between pt-2"><><Button
                    variant="outline"
                    size="sm"
                  ...
                  >Clear Filters</Button><Button
                    ...
                    className="h-7 text-xs"
                  >
                    Apply Filters</Button></div>)}</div></div>,
        mountPoint
      );
...
function applyLayerFilters(
...
): void {// Apply filters to each layer in the map
  ...
  map.eachLayer(layer => {
    ...
    if ('feature' in layer) {// Extract the layer type from feature properties
...
): boolean {if (!properties || !(filter.name in properties)) {
    return false;}
  
  const value = properties[filter.name];
  
  switch (filter.operator) {case 'contains':
