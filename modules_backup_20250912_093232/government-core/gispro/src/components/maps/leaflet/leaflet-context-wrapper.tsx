...
interface LeafletContextWrapperProps {children: React.ReactNode;
 ...
 */
export function LeafletContextWrapper({children,
  ...
  visible = false,
  mapboxInstance = null,
  onLeafletMapReady,}: LeafletContextWrapperProps) {const mapRef = useRef<L.Map | null>(null);
  ...
  useEffect(() =>{
    ...
    return () => {mapboxInstance.off('move', syncMapPosition);
  ...
  return (<div style={wrapperStyle} className="leaflet-context-wrapper"><MapContainer
        center={position}
        zoom={zoomLevel}
