...
import {animateBoundary, 
  animateZoomToBoundary, 
  createPulsatingBoundary} from '@/lib/animation-utils';
import {Button} from '@/components/ui/button';
import {cn} from '@/lib/utils';

type BoundaryState = 'county' | 'township' | 'section' | 'parcel';

interface AnimatedCountyBoundariesProps {countyData: GeoJSONFeature;
 ...
 */
export function AnimatedCountyBoundaries({countyData,
  townshipData = [],
  sectionData = [],
  parcelData = [],
  onBoundaryChange,
  ...
  position = 'topleft'}: AnimatedCountyBoundariesProps) {const map = useMap();
  ...
  useEffect(() =>{
    ...
    // Clean up on unmount
    return () => {if (boundaryLayerRef.current) {
  ...
  const transitionTo = (targetState: BoundaryState) => {if (!map || !boundaryLayerRef.current || isAnimating) return;
    ...
    if (!targetData) {targetData = countyData;}
    ...
    boundaryLayerRef.current.eachLayer((layer: any) => {if (layer.feature) {
        currentFeatures.push(layer.feature);}
    });
    
    // If no current features, just add the target data
    if (currentFeatures.length === 0) {boundaryLayerRef.current.addData(singleTargetData);
  ...
  return (<div className={controlClasses}><><Button
        size="sm"
