...
interface CollaborativeMapContainerProps {roomId: string;
...
export function CollaborativeMapContainer({roomId, 
  height = '500px',
  onMapUpdated,
  defaultProvider = 'mapbox',
  onParcelSelected}: CollaborativeMapContainerProps) {const [map, setMap] = useState<mapboxgl.Map | any>(null);
  ...
  const handleArcGISMapCreated = useCallback((view: any) => {setMap(view);
  ...
  const handleConnectionStatusChange = useCallback((status: ConnectionStatusEnum) => {setConnectionStatus(status);
    ...
    if (status === ConnectionStatusEnum.CONNECTED) {
      toast({
        ...
        variant: "default",});
    } else if (status === ConnectionStatusEnum.DISCONNECTED) {toast({
  ...
  const handleFeaturesUpdate = useCallback((updatedFeatures: CollaborativeFeature[]) => {setFeatures(updatedFeatures);
  ...
  const handleAnnotationsUpdate = useCallback((updatedAnnotations: any[]) => {setAnnotations(updatedAnnotations);}, []);
  ...
  const handleUserActivityUpdate = useCallback((userId: string, activityType: "drawing" | "editing" | "viewing" | "idle", data?: any) => {
    ...
    setUserActivities(prev => {// Find existing activity for this user
  ...
  const getStatusIcon = () => {switch (connectionStatus) {
  ...
  const getStatusVariant = () =>{switch (connectionStatus) {
      case ConnectionStatusEnum.CONNECTED:
        return "outline";
      case ConnectionStatusEnum.CONNECTING:
      case ConnectionStatusEnum.RECONNECTING:
  ...
  const getStatusText = () => {switch (connectionStatus) {
      case ConnectionStatusEnum.CONNECTED:
        return "Connected";
      case ConnectionStatusEnum.CONNECTING:
        return "Connecting";
  ...
  const handleSessionSave = useCallback((name: string, description?: string) => {const sessionData: SessionData = {
      name,
      description,
      features,
      annotations,
  ...
  return (<div className="space-y-3">{/* Status bar */}<div className="flex items-center justify-between"><div className="flex items-center space-x-2"><Badge variant={getStatusVariant()} className="flex items-center space-x-1">{getStatusIcon()}<span>{getStatusText()}</span></Badge>{collaborators.length > 0 && (<Badge variant="outline" className="flex items-center"><span>{collaborators.length} collaborator{collaborators.length !== 1 ? 's' : ''}</span></Badge>)}</div><Badge variant="outline" className="flex items-center"><span>Room: {roomId}</span></Badge></div>{/* Session manager */}<div className="flex items-start justify-between gap-2"><CollaborativeSessionManager 
          onSave={handleSessionSave}
          onLoad={handleSessionLoad}
          roomId={roomId} />{/* User activity indicator */}<CollaborativeUserIndicator 
          activities={userActivities} 
          collaborators={collaborators} /></div>{/* Map container */}<div style={containerStyle}><MapProviderSelector
        ...
        >{map && mapProvider === 'mapbox' && (<CollaborativeMap
              roomId={roomId} 
          ...
          {map && mapProvider === 'arcgis' && (<div className="absolute inset-0 flex items-center justify-center bg-black/10"><Card className="w-auto mx-4"><CardContent className="p-4 text-center"><><p>Collaborative features not yet implemented for ArcGIS maps.</p><p
</>
className="text-xs mt-1 text-muted-foreground">Switch to Mapbox for collaborative editing.</p></CardContent></Card></div>)}</MapProviderSelector></div></div>
  );
}