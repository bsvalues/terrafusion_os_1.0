...
interface ArcGISMapProps {initialCenter?: [number, number];
  initialZoom?: number;
 ...
 */
export function ArcGISMap({initialCenter = [-123.3617, 44.5646], // Benton County, Oregon
  initialZoom = 10,
  onMapCreated,
  width = '100%',
  height = '500px',
  children,
  interactive = true,}: ArcGISMapProps) {const [mapReady, setMapReady] = useState(false);
  ...
  return (<ArcGISProvider
    ...
    >{mapReady && children}</ArcGISProvider>
  );
}
