...
interface CursorPosition {userId: string;
  position: [number, number];
  timestamp: Date;
  color: string;}

interface CollaborativeCursorProps {map: mapboxgl.Map;
  roomId: string;
  userId: string;
  enabled?: boolean;}

// Random color generator for user cursors
function getRandomColor(seed: string): string {// Create a simple hash from the string
  let hash = 0;
...
export function CollaborativeCursor({map,
  roomId,
  userId,
  enabled = true,}: CollaborativeCursorProps) {
  ...
  useEffect(() =>{if (!map || !enabled) return;
    ...
    // Cleanup on unmount
    return () => {// Remove all cursor elements
  ...
  useEffect(() => {if (!map || !enabled || status !== ConnectionStatusEnum.CONNECTED) return;
    ...
    const handleMouseMove = (e: mapboxgl.MapMouseEvent) => {
      ...
      send({type: MessageTypeEnum.CURSOR_MOVE,
        roomId,
        userId,
        payload: {
  ...
  useEffect(() => {if (!lastMessage || !map || !enabled) return;
  ...
  useEffect(() => {if (!map || !enabled) return;
    ...
    Object.entries(updatedPositions).forEach(([uid, cursor]) => {const timeSinceUpdate = now.getTime() - cursor.timestamp.getTime();
    ...
    if (hasChanges) {setCursorPositions(updatedPositions);}
