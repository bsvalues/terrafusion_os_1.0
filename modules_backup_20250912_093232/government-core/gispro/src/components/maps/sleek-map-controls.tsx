...
import {Tooltip,
  ...
  TooltipProvider,
  TooltipTrigger,
  SimpleTooltip,} from '@/components/ui/tooltip';
 ...
 */
export function SleekMapControls({activeTool,
  onToolChange,
  controlGroups = [],
  position = 'top-left',
  direction = 'horizontal',
  className,}: SleekMapControlsProps) {// Position-specific classes
  ...
  const directionClasses = {horizontal: 'flex-row',
    vertical: 'flex-col',};

  return (<div className={cn('absolute z-10', positionClasses[position], className)}><div className={cn('flex gap-2', directionClasses[direction])}>{controlGroups.map(group => (<div
            key={group.id}
            className={cn(
          ...
          >{group.controls.map(control => (<SimpleTooltip
                key={control.id}
              ...
              ><Button
                ...
                >{control.icon}</Button></SimpleTooltip>))}</div>))}</div></div>
  );
}
