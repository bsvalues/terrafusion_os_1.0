...
export interface EnhancedMapViewerRef {getMap: () => any;
...
export const EnhancedMapViewer = forwardRef<EnhancedMapViewerRef, EnhancedMapViewerProps>(
  (
    ...
    return (<div
      ...
      >{/* Actual Leaflet Map with MapContainer */}<MapContainer
          center={[center[0], center[1]]}
          zoom={zoom}
          style={{ height: '100%', width: '100%'}}
          ref={mapRef}
        >{/* Base tile layer */}<TileLayer
            attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a>contributors'
            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
          />

          {/* Child components (including ParcelOverlay) can now use Leaflet context */}
          {children}</MapContainer>{/* Debug Info */}<div
          style={{
            position: 'absolute',
            bottom: 10,
            left: 10,
            ...
            borderRadius: '4px',
            fontSize: '12px',}}
        ><div>Tool: {activeTool}</div>{activeTool === MapTool.MEASURE && measurementType && (<div>Measuring: {measurementType} in {measurementUnit}</div>)}</div></div>
    );
  }
);
