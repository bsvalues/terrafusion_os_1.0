...
declare module 'leaflet' {namespace Control {
  ...
  interface MeasureOptions {position?: L.ControlPosition;
...
interface MeasurementControlProps {measureType: 'distance' | 'area' | null;
  position?: 'topleft' | 'topright' | 'bottomleft' | 'bottomright';}

export function MeasurementControl({measureType,
  position = 'bottomright',}: MeasurementControlProps) {const map = useMap();
  ...
  useEffect(() => {
    ...
    // Handle cleanup
    return () => {if (measureControlRef.current) {
        try {
          measureControlRef.current.disable();
          map.removeControl(measureControlRef.current);} catch (e) {console.error('Error removing measure control:', e);}
      }
    };
  }, [map, position]);

  // Handle measurement tool activation/deactivation
  useEffect(() => {if (!measureControlRef.current) return;
    ...
    try {
      if (measureType === 'distance' || measureType === 'area') {
        if (!isMeasuring) {
          ...
          setIsMeasuring(true);}
      } else if (isMeasuring) {measureControlRef.current.disable();
        setIsMeasuring(false);}
    } catch (e) {console.error('Error toggling measure control:', e);}
