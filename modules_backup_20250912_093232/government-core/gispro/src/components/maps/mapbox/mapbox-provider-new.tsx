...
export const MapboxProvider: React.FC<MapboxProviderProps> = ({initialViewState = { longitude: -121.3153, latitude: 44.0582, zoom: 13},
  ...
  const fetchTokenDirectly = useCallback(async () => {
    if (error && !fetchingDirectly && !token && !directToken) {
      ...
      try {
        ...
        if (data && typeof data.token === 'string') {console.log('Successfully retrieved Mapbox token directly');
  ...
  useEffect(() => {fetchTokenDirectly();}, [fetchTokenDirectly]);
...
</></>>{error}</AlertDescription></Alert>)}<div ref={mapContainerRef} style={{ width: '100%', height: '100%'}} />{mapInitialized && children && (<MapboxContext.Provider value={contextValue}>{children}</MapboxContext.Provider>)}</div>
  );
};
