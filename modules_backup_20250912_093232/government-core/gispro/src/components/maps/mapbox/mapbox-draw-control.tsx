...
export interface DrawEvent {type: string;
  features: GeoJSON.Feature[];}

interface MapboxDrawStyles {[key: string]: any;}

// Proper type definitions for MapboxDraw options
interface MapboxDrawOptions {displayControlsDefault?: boolean;
  controls?: {
    point?: boolean;
    line_string?: boolean;
    polygon?: boolean;
...
export interface MapboxDrawControlProps {position?: 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right';
  controls?: {
    point?: boolean;
    line?: boolean;
    polygon?: boolean;
 ...
 */
export function MapboxDrawControl({position = 'top-left',
  controls = {
    point: true,
    line: true,
    polygon: true,
  ...
  onDrawRender,
  onDrawActionable,
}: MapboxDrawControlProps) {const { map, isLoaded} = useMapbox();
  const drawRef = useRef<MapboxDraw | null>(null);
  const [currentMode, setCurrentMode] = useState<DrawMode>(defaultMode);

  // Initialize the draw control
  useEffect(() => {if (!isLoaded || !map) return;
    ...
    try {
      ...
      if (!drawRef.current) {
        ...
        const drawOptions: MapboxDrawOptions = {displayControlsDefault: false,
        ...
        const handleCreate = (e: any) => {if (onDrawCreate) onDrawCreate(e);};

        const handleUpdate = (e: any) => {if (onDrawUpdate) onDrawUpdate(e);};

        const handleDelete = (e: any) => {if (onDrawDelete) onDrawDelete(e);};

        const handleSelectionChange = (e: any) => {if (onDrawSelectionChange) onDrawSelectionChange(e);};

        const handleModeChange = (e: any) => {const mode = e.mode as DrawMode;
          setCurrentMode(mode);
          if (onDrawModeChange) onDrawModeChange(mode);};

        const handleRender = (e: any) => {if (onDrawRender) onDrawRender(e);};

        const handleActionable = (e: any) => {if (onDrawActionable) onDrawActionable(e);};
    ...
    } catch (error) {console.error('Error setting up draw control:', error);
      toast({
  ...
  const getAllFeatures = (): GeoJSON.Feature[] => {if (drawRef.current) {
      return drawRef.current.getAll().features;}
    return [];
  };

  // Method to get selected features
  const getSelectedFeatures = (): GeoJSON.Feature[] => {if (drawRef.current) {
      return drawRef.current.getSelected().features;}
    return [];
  };

  // Method to add features
  const addFeatures = (featureCollection: GeoJSON.FeatureCollection) => {if (drawRef.current) {
      try {
        drawRef.current.add(featureCollection);} catch (error) {console.error('Error adding features:', error);
        toast({
  ...
  const deleteFeatures = (featureIds: string[]) => {if (drawRef.current) {
      try {
        drawRef.current.delete(featureIds);} catch (error) {console.error('Error deleting features:', error);
        toast({
  ...
  const deleteAllFeatures = () => {if (drawRef.current) {
      try {
        drawRef.current.deleteAll();} catch (error) {console.error('Error deleting all features:', error);
        toast({
