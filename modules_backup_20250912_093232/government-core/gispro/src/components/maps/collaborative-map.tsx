...
interface CollaborativeMapProps {roomId?: string;
  username?: string;
...
export function CollaborativeMap({roomId = 'map-room',
  username,
  ...
  initialZoom = 11,
  height = '600px',
  className}: CollaborativeMapProps) {
  ...
  const {status,
    ...
    leaveRoom,
    sendCursorPosition,
    currentRoomData} = useEnhancedWebSocket({roomId,
    username,
    autoJoin: true});

  // Initialize Mapbox map
  const {map, mapContainer} = useMapbox({center: initialCenter,
    zoom: initialZoom,
  ...
  useEffect(() =>{if (!map) return;
    ...
    const onMapLoad = () => {
      ...
      map.on('mousemove', (e: mapboxgl.MapMouseEvent) => {
        ...
        if (status === 'connected' && currentRoomData) {sendCursorPosition(e.lngLat.lng, e.lngLat.lat);}
      });
    };
  ...
  const toggleUsers = () => {setShowingUsers(!showingUsers);};
  
  return (<div className={cn("relative", className)}><div 
        ref={mapContainer} 
        className="w-full rounded-md overflow-hidden"
        style={{ height}} />{/* Map Controls */}
      {showControls && (<div className="absolute top-4 left-4 z-10 flex flex-col gap-2"><Card className="shadow-md"><CardContent className="p-2"><div className="flex flex-col gap-2"><Button
                  variant={activeMode === 'view' ? 'default' : 'outline'}
                  size="sm"
                  onClick={() => handleModeChange('view')}
                  title="Pan & Zoom"
                ><><Hand className="h-4 w-4" /></Button><Button
</>
                ...
                ><MousePointer className="h-4 w-4" /></Button>{allowDrawing && (<Button
                    variant={activeMode === 'draw' ? 'default' : 'outline'}
                    size="sm"
                    onClick={() => handleModeChange('draw')}
                    title="Draw Features"
                  ><Edit3 className="h-4 w-4" /></Button>)}<Button
                ...
                ><PenTool className="h-4 w-4" /></Button></div></CardContent></Card></div>)}
      
      {/* Layer Controls */}
      {showLayerControls && (<div className="absolute top-4 right-4 z-10"><Card className="shadow-md"><CardContent className="p-2"><Button
                variant="outline"
                size="sm"
                title="Layer Controls"
              ><Layers className="h-4 w-4" /></Button></CardContent></Card></div>)}
      
      {/* Collaborator Controls */}
      {showCollaborators && (<div className="absolute bottom-4 left-4 z-10"><Card className="shadow-md"><CardContent className="p-2"><div className="flex items-center gap-2"><Button
                  variant="outline"
                  size="sm"
                  onClick={toggleUsers}
                  className="relative"
                  title="Collaborators"
                ><Users className="h-4 w-4" />{roomUsers.length > 0 && (<Badge 
                      className="absolute -top-2 -right-2 h-5 w-5 p-0 flex items-center justify-center text-[10px]"
                      variant="default"
                    >{roomUsers.length}</Badge>)}</Button>{showingUsers && (<div className="bg-background border rounded-md p-2 shadow-md ml-2"><h4 className="text-xs font-medium mb-1">Collaborators</h4>{roomUsers.length > 0 ? (<ul className="space-y-1">{roomUsers.map((user) => (<li key={user.id} className="flex items-center gap-1 text-xs"><div className="h-2 w-2 rounded-full bg-green-500"></div>{user.username || `User ${user.id.substring(0, 4)}`}</li>))}</ul>) : (<p className="text-xs text-muted-foreground">No other users</p>)}</div>)}</div></CardContent></Card></div>)}
      
      {/* Add user cursors from other collaborators here */}
      {showCollaborators && roomUsers.map((user) => (
        user.cursor && (<CollaborativeCursor 
            key={user.id}
            user={user}
            mapLoaded={mapLoaded}
            map={map} />)
...
interface CollaborativeCursorProps {user: CollaborativeUser;
  mapLoaded: boolean;
...
function CollaborativeCursor({user, mapLoaded, map}: CollaborativeCursorProps) {
  ...
  useEffect(() =>{
    ...
    return () => {map.off('move', updateCursorPosition);
  ...
  return (<div 
    ...
    ><div className="flex flex-col items-center"><div className="text-primary"><><MousePointer className="h-4 w-4" /></div><div
</>className="px-1 py-0.5 bg-primary text-primary-foreground text-[10px] rounded">
          {user.username || `User ${user.id.substring(0, 4)}`}</div></div></div>
  );
}