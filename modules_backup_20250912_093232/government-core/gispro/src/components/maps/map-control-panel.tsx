 ...
 */
export function MapControlPanel({title,
  children,
  position = 'top-right',
  collapsible = true,
  ...
  onDismiss,
  offset = 16,
  className,}: MapControlPanelProps) {
  ...
  return (<Card
    ...
    >{/* Panel header with title and controls */}<div className="flex items-center justify-between px-4 py-2 border-b bg-muted/30"><><h3 className="text-sm font-medium truncate">{title}</h3><div
          ...
          {collapsible && (<Button
            ...
            >{isCollapsed ? (<ChevronDown className="h-4 w-4" />) : (<ChevronUp className="h-4 w-4" />)}</Button>)}
          ...
          {dismissible && (<Button
            ...
            ><X className="h-4 w-4" /></Button>)}</div></div>{/* Panel content (hidden when collapsed) */}<div
        className={cn(
          'overflow-hidden transition-all duration-300 ease-in-out',
          isCollapsed ? 'max-h-0' : 'max-h-[70vh]'
        )}
      ><CardContent className="overflow-y-auto p-3">{children}</CardContent></div></Card>
  );
}