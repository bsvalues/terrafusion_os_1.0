...
type ParcelWithGeoJson = Parcel & {geometry?: any;}

type ParcelOverlayProps = {workflowId?: number;
  parcelId?: number;
 ...
 */
export function ParcelOverlay({workflowId,
  parcelId,
  filter,
  style,
  onParcelSelect,
  onParcelHover,
  showPopups = true,}: ParcelOverlayProps) {const map = useMap();
  ...
  const { data: parcels, isLoading} = useQuery<Parcel[]>({queryKey: workflowId 
      ? ['/api/workflows', workflowId, 'parcels'] 
      : ['/api/parcels', parcelId],});
  
  // Convert parcels to GeoJSON
  const createGeoJson = (): GeoJSONFeatureCollection => {if (!parcels || parcels.length === 0) {
      return {
    ...
    const features = filteredParcels.map(parcel => {// Access geometry from parcel.geometry
      ...
      } else {// Create a placeholder if no geometry exists (would be populated from DB in real app)
  ...
  const defaultStyle = (feature: Feature<Geometry, GeoJsonProperties> | undefined): L.PathOptions => {if (!feature || !feature.properties) return {
      color: '#6B7280',
      weight: 1,
      fillColor: '#E5E7EB',
      fillOpacity: 0.2,};
    ...
    return {color: isSelected ? '#FF4500' : (isHovered ? '#2563EB' : '#6B7280'),
  ...
  const getStyle = (feature: Feature<Geometry, GeoJsonProperties>| undefined): L.PathOptions => {const baseStyle = defaultStyle(feature);
    
    if (!style) {
      return baseStyle;}
    
    if (typeof style === 'function' && feature) {return { ...baseStyle, ...style(feature)};
    }
    
    if (typeof style === 'object') {return { ...baseStyle, ...style};
    }
  ...
  const onEachFeature = (feature: GeoJSONFeature, layer: L.Layer) => {if (feature.properties) {
      ...
      layer.on({
        ...
        mouseout: (e) => {setHoveredParcel(null);
          if (onParcelHover) onParcelHover(null);
          
          // Prevent propagation to avoid map panning issues
          e.originalEvent.stopPropagation();},
        click: (e) => {setSelectedParcel(selectedParcel?.id === parcel.id ? null : parcel);
  ...
  useEffect(() => {if (parcels && parcels.length > 0 && parcelLayerRef.current) {
      try {
  ...
  return (<GeoJSON
      ...
      ref={parcelLayerRef}
    >{showPopups && hoveredParcel && !selectedParcel && (<Tooltip 
          permanent={false} 
          direction="top" 
          offset={[0, -10]}
        ><div><><div className="font-semibold">{hoveredParcel.parcelNumber}</div><div
</></>>{hoveredParcel.owner}</div></div></Tooltip>)}
      
      {showPopups && selectedParcel && (<Tooltip 
          permanent 
          direction="top" 
          offset={[0, -10]}
