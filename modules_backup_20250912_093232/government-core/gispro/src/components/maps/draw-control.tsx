...
interface DrawControlProps {position?: L.ControlPosition;
...
declare module 'leaflet' {interface LeafletEvent {
    layer?: L.Layer;
    layers?: {
      getLayers(): L.Layer[];};
    layerType?: string;
 ...
 */
export function DrawControl({position = 'topleft',
  onCreate,
  onEdit,
  onDelete,
  onMounted,
  draw,
  edit,}: DrawControlProps) {const map = useMap();
  ...
  useEffect(() => {
    ...
    const defaultOptions = {polyline: {
      ...
      polygon: {allowIntersection: false,
        ...
        metric: true,
        repeatMode: true,
        tooltip: {start: 'Click to start drawing a parcel',
      ...
      circle: {shapeOptions: {
        ...
        metric: true,
        repeatMode: true,
        tooltip: {start: 'Click and drag to draw a circle',
    ...
    if (draw) {// Start with a clean slate for all draw tools
      ...
      Object.entries(draw).forEach(([tool, option]) => {
        ...
        if (defaultOptions.hasOwnProperty(tool)) {const defaultForTool = defaultOptions[tool as keyof typeof defaultOptions];
          ...
          if (typeof option === 'object') {
            ...
            processedDrawOptions[tool] = {
              ...defaultForTool,
              ...option,};
          } else {// For boolean true or undefined, use defaults
            processedDrawOptions[tool] = { ...defaultForTool};
          }
        }
      });
    } else {// If no draw options provided, disable all tools
      processedDrawOptions.polyline = false;
      processedDrawOptions.polygon = false;
      processedDrawOptions.rectangle = false;
      processedDrawOptions.circle = false;
    ...
    } else {editOptions.edit = false;}

    const drawOptions = {position,
    ...
    if (onMounted && drawControlRef.current) {onMounted(drawControlRef.current);}

    // Event handler for draw:created
    const handleCreated = (e: L.LeafletEvent) => {featureGroup.addLayer(e.layer);
    ...
    const handleEdited = (e: L.LeafletEvent) => {if (onEdit && e.layers) {
    ...
    const handleDeleted = (e: L.LeafletEvent) => {if (onDelete && e.layers) {
    ...
    return () => {map.off(CREATED, handleCreated);
