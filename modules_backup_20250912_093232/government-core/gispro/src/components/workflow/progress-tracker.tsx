...
type ProgressTrackerProps = {steps: WorkflowStep[];
  currentStep: number;
  onStepClick?: (stepId: number) =>void;};

export function ProgressTracker({steps, currentStep, onStepClick}: ProgressTrackerProps) {
  return (<div className="bg-white rounded-md shadow-sm border border-neutral-200 p-4 mb-6"><div className="flex items-center overflow-x-auto">{steps.map((step /* , index */) => {
          const isActive = step.id === currentStep;
          const isCompleted = step.id< currentStep;
          const isLast = index === steps.length - 1;
          
          return (
            <div key={step.id} className="flex items-center"><div className="flex flex-col items-center"><><button
                  className={cn(
