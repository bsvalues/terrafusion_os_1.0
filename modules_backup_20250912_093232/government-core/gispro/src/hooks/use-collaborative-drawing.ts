...
export function useCollaborativeDrawing(roomId: string = 'default') {
  ...
  const { send, lastMessage, status, userId } = useWebSocket({ roomId: roomIdRef.current });
  ...
  useEffect(() => {
    ...
    if (
      lastMessage.type !== MessageTypeEnum.FEATURE_ADD &&
      ...
      lastMessage.type !== MessageTypeEnum.FEATURE_DELETED
    )
      return;
  ...
  const updateFeature = useCallback((id: string, feature: Feature) => {
    ...
    setFeatureCollection(prev => ({
      ...prev,
      features: prev.features.map(f => (f.id === id ? updatedFeature : f)),
    }));
