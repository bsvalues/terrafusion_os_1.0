...
export function useCollaborativeAnnotations(roomId: string = 'default') {
    ...
    try {
      ...
      switch (change.action) {
        ...
        case AnnotationActionType.UPDATE:
          setAnnotations(prev =>
            prev.map(ann => (ann.id === change.annotation.id ? change.annotation : ann))
          );
          break;
  ...
  const updateAnnotation = useCallback(
    (id: string, updates: Partial<Omit<Annotation, 'id' | 'createdBy' | 'createdAt'>>) => {
