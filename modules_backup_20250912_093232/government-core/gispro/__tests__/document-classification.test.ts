...
describe('Document Classification Service', () => {
  const baseUrl = 'http://localhost:${TF_STATIC_PORT:-8080}';
  ...
  test('POST /api/documents/classify should classify documents based on text content', async () => {
    ...
    const sampleTexts = {
      [DocumentType.PLAT_MAP]:
        'PLAT MAP OF BENTON COUNTY SUBDIVISION LOT 7 BLOCK 3 RECORDED DOCUMENT',
      [DocumentType.DEED]: 'WARRANTY DEED TRANSFER OF PROPERTY GRANTOR GRANTEE LEGAL DESCRIPTION',
      [DocumentType.SURVEY]:
        'BOUNDARY SURVEY PREPARED BY LICENSED SURVEYOR COORDINATES BEARINGS DISTANCES',
      [DocumentType.LEGAL_DESCRIPTION]:
        'LEGAL DESCRIPTION: THE EAST 50 FEET OF LOT 12, BLOCK 7, VISTA HEIGHTS ADDITION',
      [DocumentType.BOUNDARY_LINE_ADJUSTMENT]:
        'BOUNDARY LINE ADJUSTMENT APPLICATION ADJACENT PARCELS REVISED LEGAL DESCRIPTION',
  ...
  test('POST /api/documents/classify should handle documents that cannot be clearly classified', async () => {
    ...
    const ambiguousText =
      'This document contains generic text that does not clearly indicate the document type.';
