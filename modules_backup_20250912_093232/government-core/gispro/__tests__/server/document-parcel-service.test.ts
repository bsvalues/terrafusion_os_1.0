...
describe('DocumentParcelService', () => {
  ...
  describe('associateDocumentWithParcels', () => {
    ...
    test('should throw error if document not found', async () => {
      ...
      // Execute and assert
      await expect(documentParcelService.associateDocumentWithParcels(1, [101])).rejects.toThrow(
  ...
  describe('getDocumentRelationships', () => {
    ...
    test('should throw error if document not found', async () => {
      ...
      // Execute and assert
      await expect(documentParcelService.getDocumentRelationships(1)).rejects.toThrow(
        'Document with ID 1 not found'
      );
    });
  });
  ...
  describe('getParcelRelationships', () => {
    ...
    test('should throw error if parcel not found', async () => {
      ...
      // Execute and assert
      await expect(documentParcelService.getParcelRelationships(101)).rejects.toThrow(
        'Parcel with ID 101 not found'
      );
    });
  });
  ...
  describe('updateDocumentParcelLink', () => {
    ...
    test('should throw error if link not found', async () => {
      ...
      // Execute and assert
      await expect(
        documentParcelService.updateDocumentParcelLink(1001, 'ownership')
  ...
  describe('getDocumentsForParcelNumber', () => {
    test('should return documents for the matching parcel number', async () => {
      // Setup mocks
      const mockParcels = [{ id: 101, parcelNumber: '12345-67-89' }];
      const mockDocuments = [
        { id: 1, name: 'Deed.pdf', type: 'deed' },
        { id: 2, name: 'Survey.pdf', type: 'survey' },
      ];
