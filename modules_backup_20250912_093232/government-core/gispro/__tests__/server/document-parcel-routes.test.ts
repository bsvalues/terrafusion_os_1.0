...
describe('Document-Parcel Relationship API Routes', () => {
  ...
  describe('POST /api/documents/:id/parcels', () => {
    test('should associate document with parcels', async () => {
      ...
      (documentParcelService.associateDocumentWithParcels as jest.Mock).mockResolvedValue(
        mockLinks
      );

      const response = await request.post('/api/documents/1/parcels').send({
        parcelIds: [101],
        linkType: 'ownership',
        notes: 'Test notes',
      });
  ...
  describe('DELETE /api/documents/:id/parcels', () => {
    test('should remove document-parcel associations', async () => {
      ...
      const response = await request.delete('/api/documents/1/parcels').send({
        parcelIds: [101, 102],
      });
  ...
  describe('PATCH /api/document-parcel-links/:id', () => {
    test('should update document-parcel link', async () => {
      ...
      const response = await request.patch('/api/document-parcel-links/1001').send({
        linkType: 'legal_description',
        notes: 'Updated notes',
      });
