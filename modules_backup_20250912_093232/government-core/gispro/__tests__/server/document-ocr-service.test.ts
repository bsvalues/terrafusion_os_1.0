import {
  documentOcrService,
  OcrExtractionResult,
} from '../../server/services/document-ocr-service';
...
describe('Document OCR Service', () => {
  ...
  it('should suggest document links based on content similarity', async () => {
    ...
    const suggestions = await documentOcrService.suggestRelatedDocuments(
      documents[0],
      documents.slice(1)
    );
  ...
  describe('Document Service Integration', () => {
    ...
    it('should find related documents based on content similarity', async () => {
      ...
      const mockDocuments = [
        mockDocument,
        { id: 2, name: 'Related Doc.pdf' },
        { id: 3, name: 'Unrelated Doc.pdf' },
      ];
      (storage.getDocuments as jest.Mock).mockResolvedValue(mockDocuments);
      (documentOcrService.calculateContentSimilarity as jest.Mock).mockImplementation(
        (text1, text2) => {
      ...
      (documentOcrService.extractText as jest.Mock).mockImplementation(content => {
