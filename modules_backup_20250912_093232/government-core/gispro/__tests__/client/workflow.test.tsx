...
jest.spyOn(queryClient, 'apiRequest').mockImplementation(async (method, url, data) => {return {
    ok: true,
    json: async () => {
      if (url === '/api/workflows' && method === 'POST') {
        return { id: 1, title: data.title, type: data.type, status: 'in_progress'};
      }
      if (url === '/api/workflows/1/state' && method === 'PATCH') {return { id: 1, workflowId: 1, currentStep: data.currentStep};
      }
      if (url === '/api/workflows' && method === 'GET') {return [{ id: 1, title: 'Test Workflow', type: 'long_plat', status: 'in_progress'}];
      }
      return {};
    },
  } as Response;
...
describe('Workflow Functionality', () => {const queryClient = new QueryClient({
    defaultOptions: {
      queries: {
        retry: false,},
    },
  ...
  test('Creating a new workflow', async () => {// This is a more integration-style test that verifies the workflow creation process
    const mockCreateWorkflow = jest.fn().mockImplementation(async data => {
      return { id: 1, ...data};
    });

    // Assert the workflow was created with correct data
    const workflowData = {title: 'New Test Workflow',
      type: 'long_plat',
  ...
  test('Workflow state transitions', async () => {const mockUpdateWorkflowState = jest.fn().mockImplementation(async (workflowId, stateData) => {
      return {
        id: 1,
        workflowId,
  ...
  test('Checklist item completion', async () => {const mockUpdateChecklistItem = jest.fn().mockImplementation(async (itemId, completed) => {
