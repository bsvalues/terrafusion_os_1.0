...
jest.mock('leaflet', () => ({
  ...
  Control: {extend: jest.fn().mockReturnValue(function () {
      this.onAdd = jest.fn().mockReturnValue(document.createElement('div'));
      this.onRemove = jest.fn();
      return this;}),
  },
  TileLayer: jest.fn().mockImplementation(() => ({addTo: jest.fn(),
    remove: jest.fn(),})),
  LatLng: jest.fn().mockImplementation((lat, lng) => ({lat, lng})),
  Polyline: jest.fn().mockImplementation(() => ({addTo: jest.fn(),
    remove: jest.fn(),})),
  Polygon: jest.fn().mockImplementation(() => ({addTo: jest.fn(),
    remove: jest.fn(),})),
  LayerGroup: jest.fn().mockImplementation(() => ({addTo: jest.fn(),
    addLayer: jest.fn(),
    clearLayers: jest.fn(),
    remove: jest.fn(),})),
}));
...
// Mock turf.js module
jest.mock('@turf/turf', () => ({point: jest.fn(coords => ({ type: 'Point', coordinates: coords})),
  ...
  featureCollection: jest.fn(features => ({type: 'FeatureCollection',
    features,})),
}));
  ...
  {id: 1,
    ...
    data: {
      ...
      features: [
        {
          type: 'Feature',
          properties: { owner: 'Smith', acres: 5},
...
describe('Map Visualization Tools Integration', () => {beforeEach(() => {
    jest.clearAllMocks();});

  it('should be able to switch between measurement types', () => {
    const onMeasure = jest.fn();
    render(<MeasurementTool map={mockMap as any} position="topright" onMeasure={onMeasure} />);
  ...
  it('should be able to filter map layers by type', () =>{
    ...
    expect(onFilterChange).toHaveBeenCalledWith(
      expect.objectContaining({types: expect.arrayContaining([MapLayerType.PARCEL]),})
    );
  });

  it('should be able to change the base map', () => {
    const onBaseMapChange = jest.fn();
    render(<BaseMapSelector map={mockMap as any} position="topright" onBaseMapChange={onBaseMapChange} />);
  ...
  it('should correctly integrate measurement tool with map interactions', () => {const onMeasure = jest.fn();
    const { rerender} = render(<MeasurementTool map={mockMap as any} position="topright" onMeasure={onMeasure} />);
  ...
  it('should filter layers correctly based on property values', () => {// Test the filterLayersByProperty utility function
    const filters = {
