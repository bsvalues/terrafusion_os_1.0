...
// Mock dependencies
jest.mock('../../client/src/lib/map-utils', () => ({getDummyParcelData: jest.fn(() => ({
    type: 'Feature',
    geometry: {
      type: 'Polygon',
      coordinates: [
    ...
    properties: {parcelId: 'test-parcel',
      address: '123 Test St',
      acres: 1.5,},
  })),
  DEFAULT_MAP_LAYERS: [
...
describe('MapPreview Component', () => {
  ...
  beforeEach(() => {
    ...
    render(<QueryClientProvider client={queryClient}><MapPreview parcelId="12345678" /></QueryClientProvider>);
  });

  test('Renders map container', () => {expect(screen.getByTestId('map-container')).toBeInTheDocument();});

  test('Renders parcel information', () => {expect(screen.getByText(/Parcel Information/i)).toBeInTheDocument();
    expect(screen.getByText(/123 Test St/i)).toBeInTheDocument();
    expect(screen.getByText(/1.5/i)).toBeInTheDocument();});

  test('Toggles map layers', () => {const layerToggle = screen.getByLabelText(/Parcels/i);
  ...
  test('Triggers full map view when button clicked', () => {
    ...
    render(<QueryClientProvider client={queryClient}><MapPreview parcelId="12345678" enableFullMap={true} onOpenFullMap={onOpenFullMap} /></QueryClientProvider>
    );
