...
jest.mock('@/hooks/use-document-classifier', () => ({useDocumentClassifier: jest.fn(),}));
...
describe('BatchDocumentProcessor Component', () => {
  ...
  const createMockFiles = (count: number) => {
    ...
    for (let i = 0; i< count; i++) {
      files.push(new File(['test content'], `document-${i + 1}.pdf`, {type: 'application/pdf'}));
    }
    return files;
  };
  ...
  test('should allow selecting multiple files for batch processing', async () => {
    render(<BatchDocumentProcessor workflowId={mockWorkflowId} onComplete={mockOnComplete} />);
    ...
    await waitFor(() => {expect(screen.getByText(/document-1.pdf/i)).toBeInTheDocument();
      expect(screen.getByText(/document-2.pdf/i)).toBeInTheDocument();
      expect(screen.getByText(/document-3.pdf/i)).toBeInTheDocument();});
  });

  test('should process files in batch and show progress', async () => {const uploadWithClassificationMock = jest
      .fn()
    ...
    (useDocumentClassifier as jest.Mock).mockReturnValue({uploadWithClassification: uploadWithClassificationMock,
      isUploading: false,});
    ...
    await waitFor(() => {expect(uploadWithClassificationMock).toHaveBeenCalledTimes(2);
      expect(mockOnComplete).toHaveBeenCalled();});
  });

  test('should handle file processing errors', async () => {// Mock one file failing
    (useDocumentClassifier as jest.Mock).mockReturnValue({
      uploadWithClassification: jest
        .fn()
        .mockResolvedValueOnce({
          id: 1,
          name: 'document-1.pdf',
    ...
    // Verify error state
    await waitFor(() => {expect(screen.getByText(/document-2.pdf/i)).toBeInTheDocument();
  ...
  test('should allow batch tagging of processed documents', async () => {
    ...
    (useMutation as jest.Mock).mockReturnValue({mutateAsync: batchTagMutationMock,
      isPending: false,});

    // Mock successful uploads
    (useDocumentClassifier as jest.Mock).mockReturnValue({uploadWithClassification: jest.fn().mockImplementation(file =>
        Promise.resolve({
          id: Math.floor(Math.random() * 1000),
          name: file.name,
          documentType: 'plat_map',
    ...
    await waitFor(() => {expect(screen.getByText(/Apply Tags/i)).toBeInTheDocument();});

    // Apply batch tags
    fireEvent.click(screen.getByText(/Apply Tags/i));

    await waitFor(() => {expect(batchTagMutationMock).toHaveBeenCalled();
