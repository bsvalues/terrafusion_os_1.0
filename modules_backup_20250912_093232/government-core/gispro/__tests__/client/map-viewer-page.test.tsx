...
jest.mock('@/components/maps/enhanced-map-viewer', () => ({__esModule: true,
  default: jest.fn(({ children, mapLayers}) => (<div data-testid="mock-map-viewer"><div data-testid="map-layers">{JSON.stringify(mapLayers)}</div>{children}</div>
  )),
}));

jest.mock('@/components/maps/enhanced-layer-control', () => ({__esModule: true,
  default: jest.fn(() =><div data-testid="mock-layer-control" />),}));

jest.mock('@/components/layout/sidebar', () => ({__esModule: true,
  default: jest.fn(() =><div data-testid="mock-sidebar" />),}));

jest.mock('@/components/layout/header', () => ({__esModule: true,
  default: jest.fn(() =><div data-testid="mock-header" />),}));

describe('MapViewerPage', () => {beforeEach(() => {
  ...
  test('renders map viewer with layers from API', async () => {await act(async () => {
    ...
    await waitFor(() => {const mapLayersEl = screen.getByTestId('map-layers');
  ...
  test('handles API errors gracefully', async () => {// Override the fetch mock to simulate an error
  ...
  test('correctly formats map layer styles', async () => {await act(async () => {
      render(<MapViewerPage />);});
