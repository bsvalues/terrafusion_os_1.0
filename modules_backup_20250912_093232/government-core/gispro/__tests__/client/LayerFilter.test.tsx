...
describe('LayerFilter Component', () => {beforeEach(() => {
    jest.clearAllMocks();});

  it('should render layer filter controls correctly', () => {
    render(<LayerFilter map={mockMap as any} position="topright" layers={mockLayers} />);
  ...
  it('should filter layers by property values correctly', () => {
    ...
    // Verify filter change callback was called with correct filters
    expect(onFilterChange).toHaveBeenCalledWith(
      expect.objectContaining({properties: expect.arrayContaining([
  ...
  it('should reset filters when clear button is clicked', () => {
    ...
    expect(onFilterChange).toHaveBeenCalledWith(
      expect.objectContaining({types: expect.arrayContaining([]),
