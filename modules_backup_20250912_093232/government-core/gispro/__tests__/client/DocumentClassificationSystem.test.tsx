...
jest.mock('@/hooks/use-document-classifier', () => ({useDocumentClassifier: jest.fn().mockReturnValue({
    uploadWithClassification: jest.fn().mockResolvedValue({
      document: {
        id: 3,
        name: 'new_document.pdf',
        type: 'deed',},
      classification: {documentType: DocumentType.DEED,
        confidence: 0.85,
...
describe('Document Classification System Integration', () => {const mockWorkflow = {
    id: 1,
    title: 'Test Workflow',
    type: 'bla',
    status: 'in_progress',};
  ...
  beforeEach(() => {// Setup default mock implementations
    (useQuery as jest.Mock).mockImplementation(({ queryKey}) => {
      ...
      if (queryKey[0] === `/api/workflows/${mockWorkflow.id}/documents`) {return {
          data: mockDocuments,
          isLoading: false,
          error: null,};
      }
      ...
      if (queryKey[0]?.startsWith('/api/documents/') && queryKey[0]?.endsWith('/versions')) {const docId = parseInt(queryKey[0].split('/')[3]);
        return {
      ...
      if (queryKey[0]?.startsWith('/api/documents/') && queryKey[0]?.endsWith('/parcels')) {const docId = parseInt(queryKey[0].split('/')[3]);
        return {
  ...
  test('should complete end-to-end document management workflow', async () => {
    ...
    await waitFor(() => {expect(screen.getByText(/Survey\.pdf/i)).toBeInTheDocument();
    ...
    await waitFor(() => {expect(screen.getByText(/new_document\.pdf/i)).toBeInTheDocument();});

    // View document details
    fireEvent.click(screen.getByText(/Survey\.pdf/i));

    // Check if document details are displayed
    await waitFor(() => {expect(screen.getByText(/Document Details/i)).toBeInTheDocument();
  ...
  test('should maintain data integrity across operations', async () => {// Mock version update mutation
    ...
    (useMutation as jest.Mock).mockImplementationOnce(() => ({mutateAsync: mockCreateVersionMutation,
      isPending: false,}));
    ...
    // Add version notes
    fireEvent.change(screen.getByLabelText(/Version notes/i), {target: { value: 'Updated survey information'},
    });
    ...
    await waitFor(() => {expect(mockCreateVersionMutation).toHaveBeenCalledWith(
