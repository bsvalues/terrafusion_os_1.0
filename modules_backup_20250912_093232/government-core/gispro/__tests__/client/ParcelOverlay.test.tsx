...
jest.mock('react-leaflet', () =>{const originalModule = jest.requireActual('react-leaflet');
...
jest.mock('react-leaflet', () => {const reactLeaflet = jest.requireActual('react-leaflet');
...
// Mock useQuery hook
jest.mock('@tanstack/react-query', () =>{return {
    ...jest.requireActual('@tanstack/react-query'),
    useQuery: ({ queryKey}) => {// Return mock parcel data based on the query key
      ...
      if (queryKey[0] === '/api/workflows' && queryKey[2] === 'parcels') {return {
...
const TestWrapper = ({children}: {children: React.ReactNode}) => (<MapContainer center={[0, 0]} zoom={10} style={{ height: '100vh'}}>{children}</MapContainer>);

describe('ParcelOverlay', () => {
  test('renders parcels from API data', () => {
    render(<TestWrapper><ParcelOverlay /></TestWrapper>);
  ...
  test('renders workflow-specific parcels when workflowId is provided', () => {
    render(<TestWrapper><ParcelOverlay workflowId={1} /></TestWrapper>);
  ...
  test('renders specific parcel when parcelId is provided', () => {
    render(<TestWrapper><ParcelOverlay parcelId={1} /></TestWrapper>);
  ...
  test('applies filter function to parcels', () => {
    ...
    render(<TestWrapper><ParcelOverlay filter={filter} /></TestWrapper>
    );
