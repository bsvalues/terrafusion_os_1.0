...
jest.mock('@tanstack/react-query', () => ({...jest.requireActual('@tanstack/react-query'),
  useQuery: jest.fn().mockImplementation(({ queryKey}) => {if (queryKey[0] === '/api/workflows') {
      return {
        data: mockWorkflows,
        isLoading: false,
        error: null,};
    }
    if (queryKey[0] === '/api/workflow-events' && queryKey[1]) {return {
  ...
  useMutation: jest.fn().mockImplementation(() => ({mutate: jest.fn(),
    isPending: false,
    isError: false,
    isSuccess: false,})),
}));
...
describe('WorkflowDashboard Component', () => {beforeEach(() => {
    jest.clearAllMocks();});

  test('should render workflow dashboard with all workflows', async () => {renderWithProviders(<WorkflowDashboard />);
  ...
  test('should filter workflows by status', async () => {renderWithProviders(<WorkflowDashboard />);
    ...
    await waitFor(() => {expect(screen.getByText('Test Long Plat 1')).toBeInTheDocument();
  ...
  test('should filter workflows by priority', async () => {renderWithProviders(<WorkflowDashboard />);
  ...
  test('should highlight high priority workflows', async () => {renderWithProviders(<WorkflowDashboard />);
  ...
  test('should display workflow timeline', async () => {renderWithProviders(<WorkflowDashboard />);
  ...
  test('should display detailed workflow events in timeline', async () => {renderWithProviders(<WorkflowDashboard />);
  ...
  test('should expand event details when clicked', async () => {renderWithProviders(<WorkflowDashboard />);
    ...
    await waitFor(() => {expect(screen.getByText(/Old Priority: medium/i)).toBeInTheDocument();
      expect(screen.getByText(/New Priority: high/i)).toBeInTheDocument();});
  });

  test('should filter workflows by date range', async () => {renderWithProviders(<WorkflowDashboard />);
    ...
    await waitFor(() => {expect(screen.queryByText('Test Long Plat 1')).not.toBeInTheDocument();
      expect(screen.getByText('Test BLA 1')).toBeInTheDocument();
      expect(screen.queryByText('Test Merge Split')).not.toBeInTheDocument();});
  });

  test('should update workflow priority', async () => {renderWithProviders(<WorkflowDashboard />);
    ...
    await waitFor(() => {const updatedWorkflowItem = screen.getByText('Test BLA 1').closest('.workflow-item');
      expect(updatedWorkflowItem).toHaveClass('priority-high');});
  });

  test('should handle sorting of workflows', async () => {renderWithProviders(<WorkflowDashboard />);
    ...
    await waitFor(() => {const firstWorkflow = screen.getAllByTestId('workflow-row')[0];
  ...
  test('should search workflows by title', async () => {renderWithProviders(<WorkflowDashboard />);
  ...
  test('should handle empty filter results gracefully', async () => {renderWithProviders(<WorkflowDashboard />);
  ...
  test('should combine multiple filters', async () => {renderWithProviders(<WorkflowDashboard />);
  ...
  test('should reset all filters', async () => {renderWithProviders(<WorkflowDashboard />);
