...
describe('DocumentVersionControl Component', () => {const mockDocument = {
    id: 1,
  ...
  beforeEach(() => {// Setup default mock implementations
    (useQuery as jest.Mock).mockImplementation(({ queryKey}) => {
      if (queryKey[0] === `/api/documents/${mockDocument.id}/versions`) {return {
          data: mockVersions,
          isLoading: false,
          error: null,};
      }
  ...
  test('should allow viewing version details', async () => {
    ...
    await waitFor(() => {fireEvent.click(screen.getByText(/Version 2/i));});
  ...
  test('should support version comparison', async () => {
    ...
    await waitFor(() => {fireEvent.click(screen.getByLabelText(/Compare Version 2/i));
  ...
  test('should allow reverting to previous version', async () => {const mockRevertMutation = jest.fn().mockResolvedValue({
      id: 3,
      documentId: 1,
      versionNumber: 3,
      contentHash: 'ghi789',
    ...
    (useMutation as jest.Mock).mockReturnValue({mutateAsync: mockRevertMutation,
      isPending: false,});

    render(<DocumentVersionControl document={mockDocument} />);

    // Open version details
    await waitFor(() => {fireEvent.click(screen.getByText(/Version 1/i));});
    ...
    await waitFor(() => {expect(mockRevertMutation).toHaveBeenCalled();});
  });

  test('should capture metadata for version changes', async () => {const mockCreateVersionMutation = jest.fn().mockResolvedValue({
      id: 3,
      documentId: 1,
      versionNumber: 3,
      contentHash: 'jkl012',
    ...
    (useMutation as jest.Mock).mockReturnValue({mutateAsync: mockCreateVersionMutation,
      isPending: false,});
    ...
    // Add version notes
    fireEvent.change(screen.getByLabelText(/Version notes/i), {target: { value: 'Added new survey information'},
    });
