...
const NotificationTestHarness = () => {const { 
    notifications, 
    markAsRead, 
    dismissNotification,
    dismissAll,
  ...
  return (<div><><div data-testid="unread-count">{unreadCount}</div><button
</>
onClick={() => markAsRead(1)}>Mark #1 as Read</button><><button onClick={() => dismissNotification(1)}>Dismiss #1</button><button
</>
onClick={dismissAll}>Dismiss All</button><div>{notifications.map(notification => (<div key={notification.id} data-testid={`notification-${notification.id}`}>{notification.title}</div>))}</div></div>);
};

describe('Notification System', () => {// Sample notifications for testing
  ...
  beforeEach(() => {jest.clearAllMocks();
    ...
    // Mock query response
    (useQuery as jest.Mock).mockImplementation(({ queryKey}) => {if (queryKey[0] === '/api/notifications') {
        return {
  ...
  test('should render notification center with correct notifications', async () => {render(<NotificationProvider><NotificationCenter /></NotificationProvider>);
  ...
  test('should allow marking notifications as read', async () => {
    ...
    render(<NotificationProvider><NotificationTestHarness /></NotificationProvider>);
    
    // Wait for notifications to load
    await waitFor(() => {expect(screen.getByTestId('notification-1')).toBeInTheDocument();});
    
    // Mark notification as read
    fireEvent.click(screen.getByText('Mark #1 as Read'));
    
    // Check that API was called
    await waitFor(() => {expect((window as any).apiRequest).toHaveBeenCalledWith(
        'PATCH',
  ...
  test('should allow dismissing notifications', async () => {
    ...
    await waitFor(() => {expect(screen.queryByTestId('notification-1')).not.toBeInTheDocument();
      expect(screen.getByTestId('notification-2')).toBeInTheDocument();
      expect(screen.getByTestId('notification-3')).toBeInTheDocument();});
  });
  
  test('should group notifications by type and entity', async () => {// Add more mock notifications of the same type/entity
    ...
    render(<NotificationProvider><NotificationCenter groupNotifications={true} /></NotificationProvider>);
    
    // Check for grouped notification indicator
    await waitFor(() => {const groupedNotification = screen.getByText(/3 updates for Boundary Survey.pdf/i);
      expect(groupedNotification).toBeInTheDocument();});
  });
  
  test('should filter notifications by type', async () => {render(<NotificationProvider><NotificationCenter /></NotificationProvider>);
    ...
    await waitFor(() => {expect(screen.queryByText('Document Classified')).not.toBeInTheDocument();
      expect(screen.queryByText('Parcel Linked')).not.toBeInTheDocument();
      expect(screen.getByText('Low Confidence Classification')).toBeInTheDocument();});
  });
  
  test('should show empty state when no notifications are available', async () => {// Update mock to return empty array
    (useQuery as jest.Mock).mockImplementation(({ queryKey}) => {if (queryKey[0] === '/api/notifications') {
        return {
          data: [],
          isLoading: false,
    ...
    render(<NotificationProvider><NotificationCenter /></NotificationProvider>
    );
