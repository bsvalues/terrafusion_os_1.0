...
describe('ReportGenerator', () =>{const queryClient = new QueryClient();
  const mockTemplates = [
  ...
  test('renders report generator interface', async () => {
    render(<QueryClientProvider client={queryClient}><ReportGenerator /></QueryClientProvider>);
  ...
  test('loads available report templates', async () => {
    ...
    await waitFor(() => {expect(screen.getByText('Property Assessment Summary')).toBeInTheDocument();
  ...
  test('successfully submits report generation request', async () => {const mockGenerate = jest
      .fn()
      .mockImplementation(() => Promise.resolve({ id: 123, status: 'pending'}));
    global.fetch = jest.fn().mockImplementation((url, options) => {if (url === '/api/reports' && options.method === 'POST') {
        return Promise.resolve({
          ok: true,
          json: () => mockGenerate(),});
      }
