...
function renderWithProviders(ui: React.ReactElement) {const queryClient = new QueryClient({
    defaultOptions: {
      queries: {
        retry: false,},
    },
...
const mockExportStatus = {available: true,
...
describe('ReportExporter Component', () => {beforeEach(() => {
    jest.clearAllMocks();
  ...
  test('renders export format options correctly', async () => {
    ...
    await waitFor(() => {expect(screen.queryByText('Checking export availability...')).not.toBeInTheDocument();});
  ...
  test('shows appropriate UI when exports are not available', async () => {// Override fetch mock for this test
...
describe('ReportScheduler Component', () => {beforeEach(() => {
    jest.clearAllMocks();
    // Mock successful API responses
    global.fetch = jest.fn().mockImplementation(url => {
  ...
  test('renders schedules list correctly', async () => {renderWithProviders(<ReportScheduler />);
  ...
  test('displays new schedule form when button is clicked', async () => {renderWithProviders(<ReportScheduler />);
    ...
    await waitFor(() => {expect(screen.getByText('Create Schedule')).toBeInTheDocument();});
...
describe('Core GIS Components Integration', () => {// This will test that components work together properly
