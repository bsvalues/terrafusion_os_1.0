...
describe('ReportViewer Component', () => {const mockReport = {
    id: 123,
  ...
  beforeEach(() => {// Setup default mock implementations
    (useQuery as jest.Mock).mockImplementation(({ queryKey}) => {
      // Mock report data
      if (queryKey[0] === `/api/reports/${mockReport.id}/data`) {return {
          data: mockReportData,
          isLoading: false,
          error: null,};
      }
      ...
      if (queryKey[0] === `/api/reports/${mockReport.id}`) {return {
          data: mockReport,
          isLoading: false,
          error: null,};
      }
  ...
  test('renders report header with metadata', async () => {
    ...
    await waitFor(() => {expect(screen.getByText('SM00 Report - 2023-01-01 to 2023-01-31')).toBeInTheDocument();
  ...
  test('renders summary cards with report metrics', async () => {
    ...
    await waitFor(() => {expect(screen.getByText('Total Workflows')).toBeInTheDocument();
  ...
  test('renders chart visualization', async () => {
    ...
    await waitFor(() => {expect(screen.getByText('Workflow Activity')).toBeInTheDocument();});
  ...
  test('handles table sorting', async () => {
    ...
    await waitFor(() => {expect(screen.getByText('Workflow Title')).toBeInTheDocument();});

    // Click on column header to sort
    fireEvent.click(screen.getByText('Workflow Title'));

    // Verify sort indicator appears
    await waitFor(() => {expect(screen.getByLabelText('sorted ascending')).toBeInTheDocument();});

    // Click again to reverse sort
    fireEvent.click(screen.getByText('Workflow Title'));

    // Verify sort indicator changes
    await waitFor(() => {expect(screen.getByLabelText('sorted descending')).toBeInTheDocument();});
  });

  test('handles pagination', async () => {// Mock report with multiple pages
  ...
  test('displays loading state while data is loading', async () => {// First return loading state
    (useQuery as jest.Mock).mockImplementationOnce(({ queryKey}) => {return {
        data: null,
        isLoading: true,
        error: null,};
    });
    ...
    await waitFor(() => {expect(screen.getByText('BLA-2023-001')).toBeInTheDocument();});
  });

  test('handles error state gracefully', async () => {// Mock error in report data fetch
    (useQuery as jest.Mock).mockImplementation(({ queryKey}) => {
      if (queryKey[0] === `/api/reports/${mockReport.id}/data`) {return {
          data: null,
          isLoading: false,
    ...
    await waitFor(() => {expect(screen.getByText('Error loading report data')).toBeInTheDocument();
