...
jest.mock('@tanstack/react-query', () => ({...jest.requireActual('@tanstack/react-query'),
  useQuery: jest.fn().mockImplementation(({ queryKey}) => {if (queryKey[0] === '/api/workflows' && queryKey[2] === 'state') {
      return {
...
jest.mock('@/lib/workflow-map-integration', () => {return {
...
jest.mock('@/components/maps/geo-data-io', () => ({GeoDataIO: ({ onImport, onExport, importDialogOpen, exportDialogOpen}) => (<div data-testid="geo-data-io">{importDialogOpen &&<div data-testid="import-dialog">Import Dialog</div>}
...
const mockWorkflow: Workflow = {id: 1,
...
describe('WorkflowMapControls', () => {const handleToolChange = jest.fn();
  ...
  test('tools are disabled when workflow is completed', () => {// Mock the WorkflowMapIntegration to return only viewing tools
