...
describe('ReportsDashboard Integration', () => {const mockReports = [
    {
      id: 1001,
  ...
  const mockTemplates = [
    ...
    {id: 2,
      name: 'Parcel Changes Report',
      description: 'Tracks boundary adjustments and ownership changes',},
    {id: 3,
      name: 'Document Classification Report',
      description: 'Statistics on document types processed',},
  ];
  ...
  beforeEach(() => {// Setup default mock implementations
    (useQuery as jest.Mock).mockImplementation(({ queryKey}) => {// Mock reports data
      ...
      if (queryKey[0] === '/api/reports/templates') {return {
          data: mockTemplates,
          isLoading: false,
          error: null,};
      }
      ...
      if (queryKey[0] === '/api/reports/schedules') {return {
          data: mockSchedules,
          isLoading: false,
          error: null,};
      }
    ...
    // Mock mutations
    (useMutation as jest.Mock).mockReturnValue({mutateAsync: jest.fn().mockResolvedValue({ success: true}),
      isPending: false,
    });
  });

  test('renders all dashboard tabs correctly', async () => {render(<ReportsDashboard />);
  ...
  test('displays recent reports in the reports list', async () => {render(<ReportsDashboard />);
  ...
  test('allows viewing a report', async () => {render(<ReportsDashboard />);
  ...
  test('navigates to generate report tab and shows form', async () => {render(<ReportsDashboard />);
  ...
  test('navigates to scheduled reports tab and shows schedules', async () => {render(<ReportsDashboard />);
  ...
  test('end-to-end flow: generate and view a report', async () => {const mockGenerateReport = jest.fn().mockResolvedValue({
      id: 1004,
    ...
    (useMutation as jest.Mock).mockReturnValueOnce({mutateAsync: mockGenerateReport,
      isPending: false,});
    ...
    await waitFor(() => {expect(screen.getByText('Create New Report')).toBeInTheDocument();});
    ...
    await waitFor(() => {expect(screen.getByText('Report Generated Successfully')).toBeInTheDocument();});
  ...
  test('allows exporting a report in different formats', async () => {// Create spy for window.open for export download
  ...
  test('handles failed reports appropriately', async () => {render(<ReportsDashboard />);
    ...
    await waitFor(() => {expect(screen.getByText('Error Details')).toBeInTheDocument();
  ...
  test('allows filtering reports by date range and status', async () => {render(<ReportsDashboard />);
    ...
    fireEvent.change(screen.getByLabelText('To'), {target: { value: '2023-02-01'},
    });
    ...
    await waitFor(() => {// In a real component, this would verify the filtered list
