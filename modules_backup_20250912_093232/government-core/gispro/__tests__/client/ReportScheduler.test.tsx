...
describe('ReportScheduler', () =>{const queryClient = new QueryClient();
  const mockTemplates = [
  ...
  beforeEach(() => {global.fetch = jest.fn().mockImplementation(url => {
      if (url === '/api/reports/templates') {
        return Promise.resolve({
          ok: true,
          json: () => Promise.resolve(mockTemplates),});
      }
      if (url === '/api/reports/schedules') {return Promise.resolve({
          ok: true,
  ...
  test('renders report scheduler interface', async () => {
    render(<QueryClientProvider client={queryClient}><ReportScheduler /></QueryClientProvider>);
  ...
  test('displays existing scheduled reports', async () => {
    ...
    await waitFor(() => {expect(screen.getByText('Monthly Assessment Report')).toBeInTheDocument();
  ...
  test('successfully creates a new scheduled report', async () => {const mockCreate = jest
      .fn()
      .mockImplementation(() => Promise.resolve({ id: 2, name: 'New Weekly Report'}));
    global.fetch = jest.fn().mockImplementation((url, options) => {if (url === '/api/reports/schedules' && options.method === 'POST') {
        return Promise.resolve({
          ok: true,
          json: () => mockCreate(),});
      }
      if (url === '/api/reports/templates') {return Promise.resolve({
          ok: true,
          json: () => Promise.resolve(mockTemplates),});
      }
      if (url === '/api/reports/schedules') {return Promise.resolve({
