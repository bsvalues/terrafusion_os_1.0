...
describe('Document-Parcel Relationship Management', () => {const mockDocument = {
    id: 1,
  ...
  beforeEach(() => {jest.clearAllMocks();
    ...
    (useQuery as jest.Mock).mockImplementation(({ queryKey}) => {
      if (queryKey[0] === `/api/documents/${mockDocument.id}/parcels`) {return {
          data: mockParcels.slice(0, 1), // Document is linked to the first parcel
          isLoading: false,
          error: null,};
      } else if (queryKey[0] === `/api/documents/${mockDocument.id}/parcel-links`) {return {
          data: mockLinks,
          isLoading: false,
          error: null,};
      } else if (queryKey[0] === '/api/parcels') {return {
          data: mockParcels,
          isLoading: false,
          error: null,};
      }
  ...
  test('should display currently linked parcels', async () => {
    ...
    await waitFor(() => {expect(screen.getByText(/Linked Parcels/i)).toBeInTheDocument();
      expect(screen.getByText('12345-67-89')).toBeInTheDocument();
      expect(screen.getByText('123 Main St')).toBeInTheDocument();});
  });

  test('should allow searching for parcels', async () => {// Mock search mutation
    ...
    await waitFor(() => {expect(screen.getByText('789 Pine Ln')).toBeInTheDocument();
  ...
  test('should allow linking and unlinking parcels', async () => {
    ...
    (useMutation as jest.Mock).mockImplementation(({mutationFn}) => {// Different mocks based on the mutation type
    ...
    await waitFor(() => {expect(screen.getByText('12345-67-89')).toBeInTheDocument();});

    // Unlink parcel
    fireEvent.click(screen.getByText(/Unlink/i));

    // Confirm unlink
    await waitFor(() => {expect(unlinkMutationMock).toHaveBeenCalled();
    ...
    (useMutation as jest.Mock).mockReturnValue({mutateAsync: searchMock,
      isPending: false,});
    ...
    (useMutation as jest.Mock).mockReturnValue({mutateAsync: linkMutationMock,
      isPending: false,});
    ...
    await waitFor(() => {expect(linkMutationMock).toHaveBeenCalled();
      expect(queryClient.invalidateQueries).toHaveBeenCalled();});
  });
  ...
  test('should handle bulk linking operations', async () => {
    ...
    (useMutation as jest.Mock).mockReturnValue({mutateAsync: bulkLinkMutationMock,
      isPending: false,});
    ...
    (useQuery as jest.Mock).mockImplementation(({queryKey}) => {
      if (queryKey[0] === `/api/documents/${mockDocument.id}/parcels`) {return {
          ...
          isLoading: false,
          error: null,};
      } else if (queryKey[0] === '/api/parcels/search') {return {
          data: mockParcels.slice(1), // Return parcels 2 and 3
          isLoading: false,
          error: null,};
      }
      return {data: mockParcels,
        isLoading: false,
        error: null,};
    });
    ...
    await waitFor(() => {expect(bulkLinkMutationMock).toHaveBeenCalledWith({
  ...
  test('should create a new document-parcel link with specific link type', async () => {const createLinkMutationMock = jest.fn().mockResolvedValue({
      id: 2001,
      documentId: 1,
      parcelId: 102,
    ...
    (useMutation as jest.Mock).mockReturnValue({mutateAsync: createLinkMutationMock,
      isPending: false,});
    ...
    // Mock search results
    (useQuery as jest.Mock).mockImplementation(({queryKey}) => {if (queryKey.includes('search')) {
        return {
          data: [mockParcels[1]],
          isLoading: false,
          error: null,};
    ...
    await waitFor(() => {expect(createLinkMutationMock).toHaveBeenCalledWith({
        documentId: mockDocument.id,
        parcelId: 102,
        linkType: 'legal_description',
        notes: 'Contains legal description for this parcel',});
  ...
  test('should update an existing document-parcel link type', async () => {const updateLinkMutationMock = jest.fn().mockResolvedValue({
      id: 1001,
      documentId: 1,
      parcelId: 101,
      linkType: 'ownership',
    ...
    (useMutation as jest.Mock).mockReturnValue({mutateAsync: updateLinkMutationMock,
      isPending: false,});
    ...
    await waitFor(() => {expect(updateLinkMutationMock).toHaveBeenCalledWith({
        id: 1001,
        linkType: 'ownership',
        notes: 'Updated relationship',});
      expect(queryClient.invalidateQueries).toHaveBeenCalled();
...
describe('DocumentParcelRelationshipVisualization Component', () => {const mockDocumentWithParcels = {
    id: 1,
  ...
  const mockParcelWithDocuments = {id: 101,
    parcelNumber: '12345-67-89',
    address: '123 Main St',
    owner: 'John Doe',
    linkedDocuments: [
  ...
  beforeEach(() => {jest.clearAllMocks();
    ...
    // Mock queries
    (useQuery as jest.Mock).mockImplementation(({ queryKey}) => {
      if (queryKey[0] === `/api/documents/${mockDocumentWithParcels.id}/relationships`) {return {
          data: mockDocumentWithParcels,
          isLoading: false,
          error: null,};
      } else if (queryKey[0] === `/api/parcels/${mockParcelWithDocuments.id}/relationships`) {return {
  ...
  test('should display all linked parcels for a document with relationship type', async () => {
    ...
    await waitFor(() => {expect(screen.getByText(mockDocumentWithParcels.name)).toBeInTheDocument();
      expect(
        screen.getByText(mockDocumentWithParcels.linkedParcels[0].parcelNumber)
      ).toBeInTheDocument();
      expect(
        screen.getByText(mockDocumentWithParcels.linkedParcels[1].parcelNumber)
      ).toBeInTheDocument();
  ...
  test('should display all linked documents for a parcel with relationship type', async () => {
    ...
    await waitFor(() => {expect(screen.getByText(mockParcelWithDocuments.parcelNumber)).toBeInTheDocument();
  ...
  test('should filter linked documents by type', async () => {
    ...
    await waitFor(() => {expect(screen.getByText('Survey Map.pdf')).toBeInTheDocument();
      expect(screen.queryByText('Deed of Trust.pdf')).not.toBeInTheDocument();});
  });

  test('should show appropriate message when no relationships exist', async () => {// Mock empty relationship data
    ...
    await waitFor(() => {expect(screen.getByText(/No linked parcels found/i)).toBeInTheDocument();
