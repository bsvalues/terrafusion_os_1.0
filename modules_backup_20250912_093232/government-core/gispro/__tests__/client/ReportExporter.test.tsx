...
describe('ReportExporter', () => {const mockReport = {
    id: 1,
  ...
  test('does not render for non-completed reports', () => {const pendingReport = { ...mockReport, status: 'pending', completedAt: undefined};
  ...
  test('handles export submission', async () => {const mockExport = jest
      .fn()
      .mockImplementation(() =>
        Promise.resolve({ downloadUrl: '/api/reports/exports/123', filename: 'report.pdf'})
      );
    global.fetch = jest.fn().mockImplementation((url, options) => {if (url === '/api/reports/1/export' && options.method === 'POST') {
  ...
  test('shows error message when export fails', async () => {global.fetch = jest.fn().mockImplementation(() => {
      return Promise.resolve({
        ok: false,
        status: 500,
