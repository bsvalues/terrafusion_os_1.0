...
describe('DocumentParcelManager Component', () => {const mockDocument = {
    id: 1,
    name: 'Test Deed.pdf',
    type: 'deed',
  ...
  const mockLinkedParcels = [mockParcels[0]];
  ...
  beforeEach(() => {// Setup default mock implementations
    (useQuery as jest.Mock).mockImplementation(({ queryKey}) => {
      ...
      if (queryKey[0] === '/api/parcels/search') {return {
  ...
  test('should allow searching for parcels to link', async () => {
    ...
    // Enter search query
    fireEvent.change(screen.getByPlaceholderText(/Search by parcel number or address/i), {target: { value: 'Main St'},
    });
  ...
  test('should allow linking document to parcel', async () => {const mockLinkMutation = jest.fn().mockResolvedValue({ success: true});

    (useMutation as jest.Mock).mockReturnValue({mutateAsync: mockLinkMutation,
      isPending: false,});
    ...
    await waitFor(() => {expect(mockLinkMutation).toHaveBeenCalledWith(
  ...
  test('should allow removing parcel links', async () => {const mockUnlinkMutation = jest.fn().mockResolvedValue({ count: 1});

    (useMutation as jest.Mock).mockReturnValue({mutateAsync: mockUnlinkMutation,
      isPending: false,});

    render(<DocumentParcelManager document={mockDocument} />);

    // Wait for linked parcels to load
    await waitFor(() => {expect(screen.getByText(/123456-0001/i)).toBeInTheDocument();});
    ...
    await waitFor(() => {expect(mockUnlinkMutation).toHaveBeenCalledWith(
        expect.objectContaining({
          documentId: mockDocument.id,
  ...
  test('should handle documents with multiple parcel links', async () => {// Mock both parcels as linked
    (useQuery as jest.Mock).mockImplementation(({ queryKey}) => {
      if (queryKey[0] === `/api/documents/${mockDocument.id}/parcels`) {return {
