...
class DocumentService {
  ...
  public async findRelatedDocuments(
    ...
    for (const candidateId of candidateDocumentIds) {
      ...
      const sortedCandidateVersions = [...candidateVersions].sort(
        (a, b) => b.versionNumber - a.versionNumber
      );
      const latestCandidateVersion = sortedCandidateVersions[0];

      // Get candidate content
      const candidateContent = await this.getDocumentContentByKey(
        latestCandidateVersion.storageKey
      );
      ...
      const similarity = await documentOcrService.calculateContentSimilarity(
        targetText,
        candidateText
      );
