...
export class FtpService {
  ...
  async checkConnection(): Promise<boolean> {
    ...
    try {
      ...
      logger.error(
        `FTP connection check failed: ${error instanceof Error ? error.message : String(error)}`
      );
      this.isConnected = false;
      return false;
    }
  ...
  async reconnect(): Promise<boolean> {
    ...
    try {
      ...
      logger.error(
        `Failed to reconnect to FTP server: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
  ...
  async disconnect(): Promise<void> {
    try {
      ...
      logger.error(
        `Error disconnecting from FTP server: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
  ...
  getActiveTransfers(): FileTransferStatus[] {
    return Array.from(this.transfersInProgress.values()).filter(
  ...
  async fileExists(remotePath: string): Promise<boolean> {
    try {
      ...
      logger.error(
        `Failed to check if file exists: ${error instanceof Error ? error.message : String(error)}`
      );
      return false;
    }
  }
