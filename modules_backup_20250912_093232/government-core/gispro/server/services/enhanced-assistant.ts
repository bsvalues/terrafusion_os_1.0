...
const responseTemplates: Record<string, string> = {
  ...
  next_step_long_plat:
    "For your Long Plat workflow '{workflowTitle}', your next step is '{nextStepName}': {nextStepDescription}. Make sure to complete the following items: {pendingChecklistItems}",
  next_step_bla:
    "For your BLA workflow '{workflowTitle}', your next step is '{nextStepName}': {nextStepDescription}. Make sure you've uploaded the necessary legal documents and verified property lines.",
  next_step_merge_split:
    "For your Merge/Split workflow '{workflowTitle}', your next step is '{nextStepName}': {nextStepDescription}. Be sure to confirm all parcel boundaries are properly defined.",
  next_step_sm00_report:
    "For your SM00 Report '{workflowTitle}', your next step is '{nextStepName}': {nextStepDescription}. Ensure all data parameters are set correctly for the reporting period.",

  // Document handling responses
  missing_documents_long_plat:
    'I notice your Long Plat workflow is missing these critical documents: {missingDocuments}. Please upload them to proceed with the approval process.',
  document_requirements_bla:
    'For a Boundary Line Adjustment, you need: 1) Legal description of existing parcels, 2) Survey of proposed boundary changes, 3) Owner consent forms, 4) Title reports for all affected parcels.',
  document_classification:
    "I analyzed your document '{documentName}' and identified it as a {documentType} with {confidence}% confidence. This document contains information about {documentKeywords}.",

  // Parcel management responses
  parcel_generation_help:
    'To generate new parcel numbers: 1) Enter parent parcel ID ({parentParcelExample}), 2) Specify number of new parcels needed ({totalLots}), 3) The system will create sequential IDs following the Ab/Sub code system used in Benton County.',
  parcel_requirements:
    "Each new parcel should have: 1) A unique ID, 2) A complete legal description, 3) Defined boundaries, 4) Recorded ownership information. Your workflow '{workflowTitle}' needs {pendingChecklistItems}.",

  // GIS mapping responses
  map_layer_advice:
    'For best visualization when working with {workflowType} workflows, I recommend enabling these layers: {recommendedLayers}. This will help you better analyze the spatial relationships.',

  // Step-specific guidance
  step_guidance:
    "In the current step '{currentStepName}', focus on: {stepGuidance}. Based on similar workflows, users typically spend {avgTimeOnStep} completing this step.",
};
...
function categorizeQuery(query: string): QuestionCategory {
  ...
  const categoryKeywords: Record<QuestionCategory, string[]> = {
    [QuestionCategory.WORKFLOW_GENERAL]: [
      'workflow',
      'process',
      'steps',
      'complete',
    ...
    [QuestionCategory.DOCUMENT_HANDLING]: [
      'document',
      'upload',
      'file',
      'scan',
    ...
    [QuestionCategory.PARCEL_MANAGEMENT]: [
      'parcel',
      'lot',
      'number',
      'generate',
    ...
    [QuestionCategory.WORKFLOW_SPECIFIC]: [
      'long plat',
      'short plat',
      'bla',
...
function analyzeWorkflowContext(context: AssistantContext): {
  ...
  switch (workflow.type) {
    case 'long_plat':
      ...
      result.missingDocuments = expectedLongPlatDocs.filter(
        doc => !documents?.some(d => d.name.toLowerCase().includes(doc.toLowerCase()))
      );
    ...
    case 'bla':
      ...
      result.missingDocuments = expectedBLADocs.filter(
        doc => !documents?.some(d => d.name.toLowerCase().includes(doc.toLowerCase()))
      );
    ...
    case 'merge_split':
      ...
      result.missingDocuments = expectedMergeSplitDocs.filter(
        doc => !documents?.some(d => d.name.toLowerCase().includes(doc.toLowerCase()))
      );
    ...
    case 'sm00_report':
      ...
      result.missingDocuments = expectedSM00Docs.filter(
        doc => !documents?.some(d => d.name.toLowerCase().includes(doc.toLowerCase()))
      );
...
export function getEnhancedResponse(query: string, context: AssistantContext): string {
  ...
  if (
    ...
    query.toLowerCase().includes('what should i do')
  ) {
    if (context.workflow) {
    ...
    (query.toLowerCase().includes('generate') || query.toLowerCase().includes('create'))
  ) {
  ...
  const templateVariables = {
    ...
    totalLots:
      context.workflowState?.formData && 'totalLots' in (context.workflowState.formData as any)
        ? (context.workflowState.formData as any).totalLots
        : '4',
 ...
 */
export function generateWorkflowRecommendations(
    ...
    switch (context.workflow.type) {
      case 'long_plat':
        recommendations.push({
          ...
          description:
            'Ensure all parcel legal descriptions are accurate and conform to county standards',
        });
        break;
      case 'bla':
        recommendations.push({
          ...
          description:
            'Confirm all property owners have provided signed consent forms for the boundary adjustment',
        });
        break;
      case 'merge_split':
        recommendations.push({
          ...
          description:
            'Verify that the new parcel configuration complies with current zoning requirements',
        });
        break;
      case 'sm00_report':
        recommendations.push({
...
export function generateDynamicChecklist(
  ...
  if (specialConsiderations && specialConsiderations.length > 0) {
    for (const consideration of specialConsiderations) {
      ...
      if (
        consideration.toLowerCase().includes('historical') ||
        consideration.toLowerCase().includes('heritage')
      ) {
        checklist.push({
      ...
      if (
        consideration.toLowerCase().includes('environmental') ||
        consideration.toLowerCase().includes('wetland')
      ) {
        checklist.push({
