...
export function computeUnion(
  ...
  try {
    ...
    throw new Error(
      `Failed to compute union: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export function computeDifference(
  ...
  try {
    ...
    throw new Error(
      `Failed to compute difference: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export function calculateArea(
  ...
  try {
    ...
    throw new Error(
      `Failed to calculate area: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export function findCentroid(feature: Feature): Feature<Point> {
  try {
    ...
    throw new Error(
      `Failed to find centroid: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export function calculateDistance(
  ...
  try {
    ...
    throw new Error(
      `Failed to calculate distance: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export function simplifyFeature(
  ...
  try {
    ...
    throw new Error(
      `Failed to simplify feature: ${error instanceof Error ? error.message : String(error)}`
    );
  }
}
...
export function splitParcel(
  ...
  try {
    ...
    const parcel1Id =
      newParcelIds && newParcelIds.length > 0
        ? newParcelIds[0]
        : `${parcel.properties?.parcelId}_1`;
    const parcel2Id =
      newParcelIds && newParcelIds.length > 1
        ? newParcelIds[1]
        : `${parcel.properties?.parcelId}_2`;
  ...
  try {
    switch (operation) {
        ...
        return {
          ...
          metadata: {
            area: intersectionResult
              ? calculateArea(intersectionResult as Feature<Polygon | MultiPolygon>)
              : 0,
            unit: MeasurementUnit.ACRES,
          },
        };
      ...
      case GeospatialOperationType.UNION:
        ...
        const unionResult = computeUnion(inputFeatures as Feature<Polygon | MultiPolygon>[]);
