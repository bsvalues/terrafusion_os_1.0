...
const EXAMPLE_DESCRIPTIONS = [
  {
    ...
    description:
      "Commencing at the Northeast corner of Section 14, Township 4 North, Range 3 West; thence South 89°42'30\" West 210 feet; thence South 0°15' East 185 feet; thence North 89°42'30\" East 210 feet; thence North 0°15' West 185 feet to the point of beginning.",
    type: LegalDescriptionType.METES_AND_BOUNDS,
  },
  {
    name: 'Complex Metes and Bounds',
    description:
      'Beginning at a point which is 330 feet South of the Northwest corner of the Northeast Quarter of Section 22, Township 7 North, Range 2 East of the Salt Lake Base and Meridian; thence East 330 feet; thence South 165 feet; thence West 330 feet; thence North 165 feet to the point of beginning.',
    type: LegalDescriptionType.METES_AND_BOUNDS,
  },
  {
    name: 'Section Township Range',
    description:
      'The Northeast Quarter of the Southwest Quarter (NE¼ SW¼) of Section 32, Township 8 North, Range 4 West, Boise Meridian, Benton County, Washington, containing 40 acres, more or less.',
    type: LegalDescriptionType.SECTION_TOWNSHIP_RANGE,
  },
  {
    name: 'Lot and Block',
    description:
      'Lot 7, Block 12, WOODLAND HILLS SUBDIVISION, according to the official plat thereof, filed in Book 8 of Plats at Pages 10-12, records of Benton County, Washington.',
 ...
 */
export function getExampleDescriptions(): {
  description: string;
  type: LegalDescriptionType;
  name: string;
}[] {
...
function extractKeywords(text: string): string[] {
  ...
  const sectionTerms =
    text.match(/section|township|range|quarter|NE¼|NW¼|SE¼|SW¼|meridian/gi) || [];
  ...
  return keywordsArray.filter((value, index, self) => self.indexOf(value) === index);
}
...
function identifyPatterns(text: string, type: LegalDescriptionType): string[] {
  ...
  const bearingPatterns =
    text.match(/([NSEW])\s*(\d+)[°\s]*(\d*)['\s]*(\d*)["\s]*([NSEW])?/gi) || [];
