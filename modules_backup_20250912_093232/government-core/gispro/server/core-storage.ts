...
export class PostgresStorage implements ICoreStorage {
  counties = {
    ...
    async list(): Promise<County[]> {
      return await db
        .select()
        .from(counties)
        .where(eq(counties.isActive, true))
    ...
    async search(query: string, countyId?: string): Promise<Parcel[]> {
      ...
      if (countyId) {
        return await db
          .select()
          .from(parcels)
          .where(and(eq(parcels.countyId, countyId), conditions));
      }
  ...
  workflows = {
    ...
    async execute(id: string): Promise<any> {
      ...
      await db.update(workflows).set({ lastRun: new Date() }).where(eq(workflows.id, id));
