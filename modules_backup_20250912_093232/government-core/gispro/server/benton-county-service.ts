...
export class BentonCountyService {
  ...
  async fetchParcels(
    ...
    try {
      const response = await fetch(
        `${BENTON_COUNTY_CONFIG.gisServices.parcelService}/query?${queryParams}`
      );
  ...
  async fetchTaxingDistricts(): Promise<
    Array<{
      name: string;
      code: string;
      taxRate: number;
  ...
  async fetchCityBoundaries(): Promise<any> {
    try {
      const response = await fetch(
        `${this.baseUrl}/CityLimits/FeatureServer/0/query?f=json&where=1=1&outFields=*&returnGeometry=true`
      );
  ...
  async fetchZoningData(extent?: number[]): Promise<any> {
    ...
    try {
      const response = await fetch(
        `${BENTON_COUNTY_CONFIG.gisServices.zoningService}/query?${queryParams}`
      );
  ...
  async getPropertyStatistics(): Promise<{
    ...
    try {
      const stats = await fetch(
        `${BENTON_COUNTY_CONFIG.gisServices.parcelService}/query?f=json&where=1=1&returnCountOnly=true`
      );
      ...
      return {
        ...
        cityBreakdown: BENTON_COUNTY_CONFIG.cities.reduce(
          (acc, city) => {
