...
export function calculateEndPoint(
  ...
  try {
    ...
    throw new Error(
      'Failed to calculate coordinate. Please check the bearing and distance values.'
    );
  }
}
...
export function parseMetesBounds(text: string, referencePoint?: [number, number]): ParsingResult {
  try {
    ...
    while ((match = regex.exec(text)) !== null) {
      ...
      const nextPoint = calculateEndPoint(currentPoint, bearing, distance.distance, distance.unit);
