...
export class MarketIntelligenceEngine {
  ...
  private async initializeModels(): Promise<void> {
    try {
      ...
      try {
        const marketModel = await tf.loadLayersModel(
          `file://${modelPath}/market_prediction/model.json`
        );
  ...
  private calculateOverallSentiment(sources: any): any {
    const weighted =
      sources.news.score * 0.4 + sources.social.score * 0.3 + sources.analyst.score * 0.3;
