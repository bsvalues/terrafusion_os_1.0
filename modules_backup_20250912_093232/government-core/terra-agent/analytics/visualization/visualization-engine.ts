...
export class VisualizationEngine {
  ...
  async generateMarketMap(region: string, filters: MapFilters): Promise<InteractiveMap> {
    try {
    ...
    return [
      {
        ...
        tooltip: {
          template:
            '<div><strong>{{address}}</strong><br/>Price: ${{price}}<br/>{{bedrooms}} bed, {{bathrooms}} bath</div>',
