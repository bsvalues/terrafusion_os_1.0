...
import {
  ...
  IntelligentResponse,
} from '../../response/response-generator';
import {
  VoiceInterface,
  MobileOptimizer,
  AccessibilityManager,
} from '../../interaction/advanced-patterns';
...
export class TerraAgent {
  ...
  async generateDashboard(userId: string): Promise<AgentResponse> {
    try {
      ...
      const response: AgentResponse = {
        message:
          "I've generated your personalized real estate market dashboard with the latest analytics and insights.",
        type: 'visualization',
        data: dashboard,
        suggestions: [
   ...
   */
  async scoreInvestment(address: string, criteria: any, userId: string): Promise<AgentResponse> {
    try {
  ...
  private async analyzeMessageIntent(message: string, context: AgentContext): Promise<any> {
    ...
    const addressPattern =
      /\d+\s+[\w\s]+(?:st|street|ave|avenue|rd|road|dr|drive|ln|lane|way|blvd|boulevard)/i;
    const cityPattern = /(?:in|near|around)\s+[\w\s]+,?\s*[A-Z]{2}/i;

    const requiresAnalytics = analyticsKeywords.some(keyword => lowerMessage.includes(keyword));
  ...
  private async processAnalyticsRequest(
    ...
    try {
      ...
      if (intent.keywords.includes('investment') || intent.keywords.includes('roi')) {
        ...
        intent.keywords.includes('visualization')
      ) {
      ...
      return {
        message:
          'I apologize, but I encountered an issue with your analytics request. Please try rephrasing your question.',
        type: 'text',
        confidence: 0,
        processingTime: 0,
      };
  ...
  private async handleInvestmentQuery(
    message: string,
    intent: any,
    context: AgentContext
  ): Promise<AgentResponse> {
    if (intent.extractedAddress) {
      return await this.scoreInvestment(
        intent.extractedAddress,
        context.preferences || {},
        context.userId
      );
    } else {
      return {
        message:
          'I can help you analyze investment opportunities. Please provide a specific property address for investment scoring.',
        type: 'text',
        suggestions: [
  ...
  private async handleCMAQuery(
    ...
    if (intent.extractedAddress) {
      ...
      return {
        message:
          'I can generate a Comparative Market Analysis (CMA) for any property. Please provide the property address.',
        type: 'text',
  ...
  private async handlePropertyQuery(
    message: string,
    intent: any,
    context: AgentContext
  ): Promise<AgentResponse> {
  ...
  private async handleGeneralAnalyticsQuery(
    message: string,
    intent: any,
    context: AgentContext
  ): Promise<AgentResponse> {
    return {
      message:
        'I have comprehensive analytics capabilities including market intelligence, investment scoring, CMA automation, and advanced visualization. What specific analysis would you like?',
      type: 'text',
      suggestions: [
        'Analyze a property',
        'Show market trends',
  ...
  private formatInvestmentScore(address: string, data: any): string {
    return (
      `💰 **Investment Analysis for ${address}**\n\n` +
