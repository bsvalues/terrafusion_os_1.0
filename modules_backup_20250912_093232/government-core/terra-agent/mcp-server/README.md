...
The TerraAgent MCP Server is a production-grade Model Context Protocol server
that provides comprehensive real estate assessment and property analysis tools
...
| `TERRA_AGENT_BACKEND_URL` | TerraAgent backend service URL               | http://localhost:\${{TF_API_PORT:-5000}} |
...
2. **Register Tool** Add to `initializeTools()` in `index.ts`

3. **Add Validation** Add tool-specific validation to `ValidationService`
...
- GitHub Issues:
  [Repository Issues](https://github.com/terrafusion/terrafusion-os/issues)
