...
export class ValidationService {
  ...
  private validatePropertySearch(input: any): string[] {
    ...
    if (!hasCriteria) {
      errors.push(
        'At least one search criterion must be provided (address, parcelId, location, or filters)'
      );
    }
    ...
    if (input.location?.boundingBox) {
      ...
      if (northEast.latitude <= southWest.latitude) {
        errors.push(
          'Invalid bounding box: northEast latitude must be greater than southWest latitude'
        );
      }
      if (northEast.longitude <= southWest.longitude) {
        errors.push(
          'Invalid bounding box: northEast longitude must be greater than southWest longitude'
        );
      }
    }
