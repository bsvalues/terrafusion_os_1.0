...
export class PropertySearchTool implements MCPTool {
  ...
  public readonly description =
    'Search for properties using various criteria including address, location, characteristics, and market filters';
  ...
  private async processSearchResults(
    ...
    for (const item of results.items) {
      ...
      if (
        !item.lastSale?.pricePerSqFt &&
        item.lastSale?.salePrice &&
        item.characteristics?.squareFootage
      ) {
  ...
  private async getDetailedPropertyInfo(
    propertyId: string,
    context: ToolExecutionContext
  ): Promise<any> {
