...
export class PropertyAnalysisTool implements MCPTool {
  ...
  public readonly description =
    'Perform comprehensive analysis of a property including valuation, market trends, comparables, and risk assessment';
  ...
  public readonly inputSchema: any = {
    ...
    anyOf: [{ required: ['propertyId'] }, { required: ['address'] }, { required: ['parcelId'] }],
  };
  ...
  public async execute(args: any, context: ToolExecutionContext): Promise<any> {
    ...
    try {
      ...
      for (const analysisType of analysisResults.analysisTypes) {
        switch (analysisType) {
          ...
          case 'market':
            ...
            break;
          case 'comparables':
            analysisResults.comparables = await this.findComparableProperties(
              property,
              args.maxComparables || 5,
              context
            );
            break;
          case 'risk':
            ...
            break;
          case 'neighborhood':
            analysisResults.neighborhoodAnalysis = await this.performNeighborhoodAnalysis(
              property,
              context
            );
            break;
      ...
      throw new Error(
        `Property analysis failed: ${error instanceof Error ? error.message : 'Unknown error'}`
      );
    }
  }
  ...
  private async performValuationAnalysis(
    ...
    const salesComparisonValue =
      property.lastSale?.salePrice || property.assessment?.marketValue || 0;
  ...
  private async findComparableProperties(
    ...
    context.logger.debug(
      `Finding comparable properties for ${property.id}, max: ${maxComparables}`
    );
  ...
  private async performNeighborhoodAnalysis(
    property: any,
    context: ToolExecutionContext
  ): Promise<any> {
