...
export class IntelligentResponseGenerator {
  ...
  async generateResponse(
    ...
    try {
      ...
      const confidence = this.calculateResponseConfidence(analyticsResult, conversation, intent);
      ...
      console.log(
        `✅ Intelligent response generated: ${naturalLanguageResponse.length} chars, ${visualComponents.length} visuals, ${actionableInsights.length} insights`
      );
      return response;
    } catch (error) {
  ...
  private async generateVisualComponents(
    ...
    try {
      ...
      const userPrefs =
        conversation.userProfile.visualPreferences || this.getDefaultVisualPreferences();
      ...
      switch (intent.analyticsType) {
        ...
        case 'visualization':
          components.push(
            ...(await this.generateCustomVisuals(analyticsResult, userPrefs, intent))
          );
          break;
        default:
  ...
  private initializeResponseTemplates(): void {
    ...
    this.responseTemplates.set('investment_scoring', [
      {
        pattern:
          'Based on my analysis of {property}, this property scores {score}/100 for investment potential. {keyFactors}',
        ...
        examples: [
          'Based on my analysis of 123 Main St, this property scores 85/100 for investment potential. The strong rental demand and appreciation trends are key factors.',
        ],
      },
      {
        pattern:
          'Looking at the investment metrics for {property}, I found {positiveAspects}. However, {concerns}. Overall, {recommendation}.',
        ...
        examples: [
          'Looking at the investment metrics for 456 Oak Ave, I found strong cash flow potential and good location fundamentals. However, the property needs some maintenance work. Overall, this could be a solid long-term investment.',
        ],
      },
    ]);
    ...
    this.responseTemplates.set('market_intelligence', [
      {
        ...
        examples: [
          'The Seattle market is currently showing strong buyer activity. Inventory is down 15% year-over-year while demand remains high. This suggests continued price appreciation in the near term.',
        ],
      },
    ]);
    ...
    this.responseTemplates.set('cma_automation', [
      {
        pattern:
          'The comparative market analysis for {property} suggests a value range of {valueRange}. {comparableDetails} {marketPosition}',
        ...
        examples: [
          'The comparative market analysis for 789 Pine St suggests a value range of $450,000 - $480,000. Three recent sales within 0.3 miles support this range. The property is positioned competitively in the current market.',
        ],
      },
    ]);
  ...
  private initializeVisualGenerators(): void {
    ...
    this.visualComponentGenerators.set(
      'investment_scorecard',
      (data: any, prefs: VisualPreferences) => ({
        type: 'card',
  ...
  private extractResponseData(analyticsResult: any, intent: AnalyticsIntent): any {
    return {
      ...
      valueRange:
        analyticsResult.valueRange || analyticsResult.estimatedValue || 'To be determined',
    };
  }
  ...
  private async generateMarketVisuals(
    data: any,
  ...
  private async generateInvestmentVisuals(
    data: any,
    prefs: VisualPreferences
  ): Promise<VisualComponent[]> {
    const components: VisualComponent[] = [];
  ...
  private async generateCMAVisuals(
    data: any,
    prefs: VisualPreferences
  ): Promise<VisualComponent[]> {
    const components: VisualComponent[] = [];
  ...
  private async generateCustomVisuals(
    data: any,
    prefs: VisualPreferences,
    intent: AnalyticsIntent
  ): Promise<VisualComponent[]> {
  ...
  private async generateGeneralVisuals(
    data: any,
    prefs: VisualPreferences
  ): Promise<VisualComponent[]> {
  ...
  private async extractInvestmentInsights(
    data: any,
    userProfile: any
  ): Promise<ActionableInsight[]> {
  ...
  private async extractConversationInsights(
    ...
    if (
      conversation.conversationFlow.currentPhase === 'analysis' &&
