...
export async function registerRoutes(app: Express, server: Server): Promise<Server> {
    ...
    try {
      ...
      if (transferType === 'ftp' && ftpConfig) {
        ...
        try {
          ...
          await ftpClient.uploadFile(req.file.path, path.basename(req.file.path));
        ...
        try {
          ...
          (async () => {
            try {
              ...
              logger.info(
                `RAG processing completed for file ${savedFile.id} with ${embeddings.length} embeddings`
              );
            } catch (ragError) {
          ...
          logger.error(
            `Error starting RAG processing for file ${savedFile.id}:`,
            ragError instanceof Error ? ragError : 'Unknown error'
          );
  ...
  app.delete('/api/data-sources/:id', isAuthenticated, async (req, res) => {
    try {
      ...
      if (source.userId !== req.session.userId) {
        return res
          .status(403)
          .json({ message: "You don't have permission to delete this data source" });
      }
  ...
  app.post('/api/suggest-transformations', isAuthenticated, async (req, res) => {
    try {
      ...
      console.log(
        'Requesting AI suggestions for data:',
  ...
  app.get('/api/pipelines/:id', isAuthenticated, async (req, res) => {
    try {
      ...
      if (pipeline.userId !== req.session.userId) {
        return res
          .status(403)
          .json({ message: "You don't have permission to access this pipeline" });
      }
  ...
  app.patch('/api/pipelines/:id', isAuthenticated, async (req, res) => {
    try {
      ...
      if (pipeline.userId !== req.session.userId) {
        return res
          .status(403)
          .json({ message: "You don't have permission to modify this pipeline" });
      }
  ...
  app.delete('/api/pipelines/:id', isAuthenticated, async (req, res) => {
    try {
      ...
      if (pipeline.userId !== req.session.userId) {
        return res
          .status(403)
          .json({ message: "You don't have permission to delete this pipeline" });
      }
