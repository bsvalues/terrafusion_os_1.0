...
const renderForm = () => {
  return render(<QueryClientProvider client={queryClient}><DataSourceForm /></QueryClientProvider>
  );
};

describe('DataSourceForm Validation', () => {beforeEach(() => {
    vi.clearAllMocks();
    queryClient.clear();});

  it('shows validation errors for empty required fields', async () => {const user = userEvent.setup();
    renderForm();
  ...
  it('validates SQL credentials when not using trusted connection', async () => {const user = userEvent.setup();
    renderForm();
  ...
  it('validates port number range', async () => {const user = userEvent.setup();
    renderForm();
  ...
  it('accepts valid form submission', async () => {const mockApiResponse = { ok: true, json: () => Promise.resolve({ id: 1}) };
