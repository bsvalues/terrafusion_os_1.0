...
vi.mock('@shared/schema', () => ({// No need to return anything for our test case}));

// Create a type that matches the component's expected props
type MockFile = {id: string;
  name: string;
  path: string;
  type: string;
  size: number;
...
describe('FilePreview Component', () => {// Set up mock data that matches the File type
  const mockFile = {
    id: '1',
  ...
  it('renders different file types with correct labels', () => {const textFile: MockFile = {
      ...mockFile,
      name: 'notes.txt',
      type: 'text/plain',};
  ...
  it('displays file status information', () => {const processingFile: MockFile = {
    ...
    const pendingFile: MockFile = {...mockFile,
    ...
    const errorFile: MockFile = {...mockFile,
  ...
  it('displays file metadata when available', () => {const fileWithMetadata: MockFile = {
      ...mockFile,
      metadata: {
        pages: 15,
        author: 'John Doe',
