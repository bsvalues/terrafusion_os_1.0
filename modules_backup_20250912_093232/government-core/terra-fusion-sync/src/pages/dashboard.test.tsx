...
vi.mock('../components/analytics/metrics-cards', () => ({MetricsCards: () =><div data-testid="metrics-cards">Metrics Cards</div>,}));

vi.mock('../components/analytics/usage-chart', () => ({UsageChart: () =><div data-testid="usage-chart">Usage Chart</div>,}));
...
describe('Dashboard Page', () => {const queryClient = new QueryClient({
    defaultOptions: {
      queries: {
        retry: false,},
    },
  ...
  it('renders the dashboard with title and components', () => {renderDashboard();
  ...
  it('displays recent activity when data is available', async () => {// Mock successful activity fetch
  ...
  it('handles empty recent activity', async () => {// Mock empty activity response
    (global.fetch as any).mockResolvedValueOnce({
      ok: true,
  ...
  it('displays error message when activity fetch fails', async () => {// Mock failed activity fetch
    (global.fetch as any).mockResolvedValueOnce({
      ok: false,
      status: 500,
    ...
    await waitFor(() => {expect(screen.getByText(/Error loading recent activity/i)).toBeInTheDocument();});
  });

  it('shows correct formatting for timestamps', async () => {const now = new Date();
    ...
    await waitFor(() => {// Check for relative time formats (exact text may vary in testing environment)
