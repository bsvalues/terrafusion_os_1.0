...
vi.mock('../components/file-upload', () => ({FileUpload: () =><div data-testid="file-upload">File Upload Component</div>,}));

vi.mock('../components/file-preview', () => ({FilePreview: ({ file}: {file: any}) => (<div data-testid="file-preview">File Preview for: {file.name}</div>),
}));
...
describe('FileExplorer Page', () => {const queryClient = new QueryClient({
    defaultOptions: {
      queries: {
        retry: false,},
    },
  ...
  const renderFileExplorer = () => {
    return render(<QueryClientProvider client={queryClient}><FileExplorer /></QueryClientProvider>
    );
  };
  ...
  it('renders the file explorer with title and upload component', () => {renderFileExplorer();
  ...
  it('displays files when data is available', async () => {
    ...
    await waitFor(() => {expect(screen.getByText(/document.pdf/i)).toBeInTheDocument();
      expect(screen.getByText(/data.csv/i)).toBeInTheDocument();});
  });

  it('handles empty files list', async () => {// Mock empty files response
    (global.fetch as any).mockResolvedValueOnce({
      ok: true,
  ...
  it('displays error message when files fetch fails', async () => {// Mock failed files fetch
    (global.fetch as any).mockResolvedValueOnce({
      ok: false,
      status: 500,
    ...
    await waitFor(() => {expect(screen.getByText(/Error loading files/i)).toBeInTheDocument();});
  });
  ...
  it('allows file deletion', async () => {
    ...
    await waitFor(() => {// Verify the delete API was called
