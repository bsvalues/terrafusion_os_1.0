...
This guide will help you quickly deploy the BCBSLevyMaster Flask backend
integrated into the TerraLevy module.
...
```bash
DATABASE_URL=postgresql://postgres:password@localhost:\${{TF_PORT_5433:-5433}}/levy_db
...
```bash
# Health check
curl http://localhost:\${{TF_PORT_5433:-5433}}/health

# Test API
curl http://localhost:\${{TF_PORT_5433:-5433}}/api/dashboard
```
...
# Production mode
gunicorn --bind 0.0.0.0:\${{TF_API_HTTPS_PORT:-5001}} main:app
```
...
curl -X POST http://localhost:\${{TF_PORT_5433:-5433}}/start-backend
...
### Backend Service (Port \${{TF_API_HTTPS_PORT:-5001}})
...
### Integration Service (Port \${{TF_API_HTTPS_PORT:-5001}})
...
### Database (Port \${{TF_API_HTTPS_PORT:-5001}})

- **PostgreSQL**: `postgresql://postgres:password@localhost:\${{TF_PORT_5433:-5433}}/levy_db`
...
# Manual API testing
curl -X POST http://localhost:\${{TF_PORT_5433:-5433}}/api/test-endpoint \
