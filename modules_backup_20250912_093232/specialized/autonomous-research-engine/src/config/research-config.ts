...
export class ResearchConfig {
  ...
  private configHistory: Array<{
    timestamp: Date;
    config: Partial<AutonomousResearchEngineConfig>;
  }> = [];
  ...
  private validateAndMergeConfig(
    config: AutonomousResearchEngineConfig
  ): AutonomousResearchEngineConfig {
  ...
  private validateConfig(config: AutonomousResearchEngineConfig): void {
    ...
    if (
      config.ethicsConfig.explainabilityLevel < 0 ||
      config.ethicsConfig.explainabilityLevel > 1
    ) {
