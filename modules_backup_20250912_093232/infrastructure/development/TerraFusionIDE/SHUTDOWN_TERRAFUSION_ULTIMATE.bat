...
if %errorlevel% equ 0 (
    echo 🔌 Backend API detected on port \${{TF_API_PORT:-5000}}
...
if %errorlevel% equ 0 (
    echo 🌐 Frontend IDE detected on port \${{TF_API_PORT:-5000}}
...
if "%FRONTEND_RUNNING%"=="true" (
    ...
    echo.
    echo Terminating React development server on port \${{TF_API_PORT:-5000}}...
...
if "%BACKEND_RUNNING%"=="true" (
    ...
    echo.
    echo Terminating .NET API on port \${{TF_API_PORT:-5000}}...
...
netstat -an | findstr ":5000" >nul 2>&1
if %errorlevel% equ 0 (
    echo ❌ Backend API still running on port \${{TF_API_PORT:-5000}}
...
netstat -an | findstr ":5173" >nul 2>&1
if %errorlevel% equ 0 (
    echo ❌ Frontend IDE still running on port \${{TF_API_PORT:-5000}}
