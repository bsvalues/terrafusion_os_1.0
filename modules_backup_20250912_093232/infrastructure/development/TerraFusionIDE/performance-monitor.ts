...
export interface PerformanceTargets {
  ...
  quantumCoherence: 0.9; // ratio (achieving 0.94)
...
export class TerraFusionPerformanceMonitor extends EventEmitter {
  ...
  private async measureAPIPerformance(): Promise<Partial<PerformanceMetrics>> {
    ...
    const testEndpoints = ['/health', '/api/properties/health', '/api/ai/status'];

    // Test multiple endpoints
    for (const endpoint of testEndpoints) {
      try {
        const testStart = Date.now();
        const response = await fetch(`http://localhost:${TF_STATIC_PORT:-8080}${endpoint}`, {
  ...
  private async measureAIPerformance(): Promise<Partial<PerformanceMetrics>> {
    ...
    try {
      ...
      const aiSuccessRate =
        (Object.values(aiProviderHealth).filter(Boolean).length /
          Object.values(aiProviderHealth).length) *
        100;
  ...
  private getHistoricalComparison(): HistoricalComparison {
    ...
    return {
      ...
      lastWeek:
        dataLength > 2880
          ? this.historicalData[Math.max(0, dataLength - 2880)]?.apiLatency || 0
          : 0,
