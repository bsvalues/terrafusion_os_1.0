...
REM Check port \${{TF_API_PORT:-5000}} (Backend API)
netstat -an | findstr ":5000" >nul 2>&1
if %errorlevel% equ 0 (
    echo    ✅ Port \${{TF_API_PORT:-5000}}: Backend API is running
) else (
    echo    ❌ Port \${{TF_API_PORT:-5000}}: Backend API not running
)

REM Check port \${{TF_API_PORT:-5000}} (Frontend IDE)
netstat -an | findstr ":5173" >nul 2>&1
if %errorlevel% equ 0 (
    echo    ✅ Port \${{TF_API_PORT:-5000}}: Frontend IDE is running
) else (
    echo    ❌ Port \${{TF_API_PORT:-5000}}: Frontend IDE not running
)
echo.
...
if "%backend_status%"=="true" (
    echo    ✅ Backend API: RUNNING (Port \${{TF_API_PORT:-5000}})
) else (
    echo    ❌ Backend API: STOPPED
)

if "%frontend_status%"=="true" (
    echo    ✅ Frontend IDE: RUNNING (Port \${{TF_API_PORT:-5000}})
) else (
