...
export class BSArmyAgentManager extends EventEmitter {
  ...
  private checkAgentHealth(agent: BSAgent): void {
    ...
    agent.resources.networkUsage = Math.max(
      0,
      Math.min(100, agent.resources.networkUsage + (Math.random() - 0.5) * 15)
    );
  ...
  private collectPerformanceMetrics(): void {
    const metrics = {
      ...
      runningTasks: Array.from(this.taskQueue.values()).filter(
  ...
  private reassignAgentTasks(agentId: string): void {
    const tasksToReassign = Array.from(this.taskQueue.values()).filter(
