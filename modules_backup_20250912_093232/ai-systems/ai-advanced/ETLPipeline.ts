...
export class ETLPipeline extends EventEmitter {
  ...
  private async initializeDataSources(): Promise<void> {
    ...
    console.log(
      `   ✅ ${Array.from(this.dataSources.values()).filter(s => s.enabled).length} data sources ready`
    );
  }
  ...
  private collectMetrics(): void {
    const metrics = {
      ...
      bufferedRecords: Array.from(this.recordBuffer.values()).reduce(
  ...
  async runETLJob(jobId: string): Promise<void> {
    ...
    try {
      ...
      console.log(
        `   📊 ${job.statistics.recordsExtracted} extracted, ${job.statistics.recordsLoaded} loaded`
      );
  ...
  private async transformData(
    ...
    for (const record of records) {
      ...
      for (const transformation of transformations) {
        try {
          record.transformedData = await this.applyTransformation(
