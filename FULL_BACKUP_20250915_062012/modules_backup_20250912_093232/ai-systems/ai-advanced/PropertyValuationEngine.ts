...
export class PropertyValuationEngine extends EventEmitter {
  ...
  async valuateProperty(
    ...
    console.log(
      `✅ Valuation complete: $${prediction.toLocaleString()} (${(confidence * 100).toFixed(1)}% confidence)`
    );
  ...
  private getGISFeatures(property: PropertyData): number[] {
    ...
    const distanceToDowntown =
      Math.abs((property.latitude || 46.27) - 46.27) +
  ...
  private calculateConfidence(
    ...
    const comparableCount = this.trainingData.filter(
      p =>
  ...
  private generateMarketInsights(
    ...
    if (comparables.length > 0) {
      const avgComparablePrice =
        comparables.reduce((sum, c) => sum + (c.last_sale_price || 0), 0) / comparables.length;
