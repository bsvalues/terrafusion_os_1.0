...
export class SwarmStrategicCoordinator extends EventEmitter {
  ...
  private setupCommunicationNetwork(): void {
    ...
    console.log('🔗 Swarm communication network established on port \${{TF_ADMIN_PORT:-8080}}');
  }
  ...
  private createModuleOptimizationTask(
    module: string,
  ...
  public getSwarmStatus(): any {
    ...
    const workingAgents = Array.from(this.agents.values()).filter(
      a => a.status === 'working'
    ).length;
