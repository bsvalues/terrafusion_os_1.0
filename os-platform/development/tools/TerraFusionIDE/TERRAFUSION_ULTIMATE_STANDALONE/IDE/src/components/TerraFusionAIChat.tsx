...
import { Send, Bot, User, Brain, Zap, Shield, Database, Code, Settings  } from '@mui/icons-material';
import { terraFusionAI, AIResponse, AISwarmStatus, ClaudeFlowStatus } from '../services/TerraFusionAIService';
...
export const TerraFusionAIChat: React.FC = () => {
  ...
  useEffect(() => {
    // Listen for AI system status updates
    const handleStatusUpdate = (status: { swarm: AISwarmStatus; claudeFlow: ClaudeFlowStatus }) => {
      setSwarmStatus(status.swarm);
      setClaudeFlowStatus(status.claudeFlow);
    };

    terraFusionAI.on('statusUpdate', handleStatusUpdate);
    terraFusionAI.on('systemsReady', () => {
      console.log('🎯 Terrafusion AI Systems Ready!');
    });
    ...
    <div className="flex flex-col h-full bg-gray-900 text-white">
      ...
      <div className="flex items-center justify-between p-4 border-b border-gray-700 bg-gray-800">
        <div className="flex items-center space-x-3">
          ...
          <h2 className="text-lg font-semibold">Terrafusion AI Chat</h2>
        </div>
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="flex items-center space-x-2 text-sm">
            ...
            <span>AI Swarm: {swarmStatus.activeAgents.toLocaleString()}</span>
          </div>
          
          {/* Claude Flow Status */}
          <div className="flex items-center space-x-2 text-sm">
            <Bot className="w-4 h-4 text-blue-400" />
            <span>Claude Flow: {claudeFlowStatus.isActive ? 'Active' : 'Inactive'}</span>
          </div>
          
          {/* Quantum Coherence */}
          <div className="flex items-center space-x-2 text-sm">
            <Shield className="w-4 h-4 text-purple-400" />
            <span>Quantum: {(swarmStatus.quantumCoherence * 100).toFixed(0)}%</span>
          </div>
        </div>
      </div>
            ...
            <div className={`max-w-3xl rounded-lg p-4 ${
              ...
              {message.tools && message.tools.length > 0 && (
                <div className="mt-2 pt-2 border-t border-gray-600">
                  <div className="text-xs opacity-75">Tools used: {message.tools.join(', ')}</div>
                </div>
              )}
              
              {message.metadata && (
                <div className="mt-2 pt-2 border-t border-gray-600 text-xs opacity-75">
                  <div>Agents: {message.metadata.swarmAgentsUsed} • Time: {message.metadata.processingTime}ms • Quantum: {(message.metadata.quantumCoherence * 100).toFixed(0)}%</div>
                </div>
              )}
            </div>
          </div>
...
className="w-2 h-2 bg-blue-400 rounded-full animate-bounce" style={{animationDelay: '0.1s'}}></div>
      ...
      <div className="p-4 border-t border-gray-700 bg-gray-800">
        ...
        <div className="mt-2 text-xs text-gray-400">
          💡 Try: "Show me the AI swarm status", "How is Claude Flow performing?", "Help me with Terrafusion development"
        </div>
      </div>
    </div>
  );
