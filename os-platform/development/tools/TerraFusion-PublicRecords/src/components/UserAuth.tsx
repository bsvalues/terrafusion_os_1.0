...
export const UserAuth: React.FC<UserAuthProps> = ({ onLogin, onClose }) => {
  const [mode, setMode] = useState<'login' | 'register' | 'gov'>('login');
...
className="text-gray-600">
        ...
        <div className="flex gap-2 mb-6">
          {['login', 'register', 'gov'].map((m) => (
            ...
            >
              {m === 'login' ? 'Sign In' : m === 'register' ? 'Register' : 'Gov Employee'}
            </button>
          ))}
        </div>
