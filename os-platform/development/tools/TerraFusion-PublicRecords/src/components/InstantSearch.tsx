...
export const InstantSearch: React.FC<InstantSearchProps> = ({ onSearch, recordCount }) => {
  ...
  useEffect(() => {
    if (query.length > 2) {
      const timer = setTimeout(() => {
        performSearch(query);
      }, 100); // Near-instant, but with tiny debounce for UX
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    setTimeout(() => {
      ...
      if (searchQuery.includes('contract')) {
        setAiSuggestion('AI found 23 contracts with similar suspicious patterns. Want to see them all?');
      } else if (searchQuery.includes('permit')) {
        setAiSuggestion('AI prediction: 14 more permits will be delayed this week without intervention.');
      } else {
  ...
  return (
    <div className="relative">
      ...
      >
        ...
        <div className="relative bg-white/10 backdrop-blur-xl rounded-2xl p-2">
          <div className="flex items-center gap-4">
            <Search className="w-8 h-8 text-white/60 ml-4" />
            <input
              ref={searchRef}
              type="text"
              value={query}
            ...
            {isSearching && (
              ...
              >
                <Sparkles className="w-6 h-6 text-purple-400 mr-4" />
              </motion.div>
            )}
          </div>
        </div>
        ...
        {aiSuggestion && (
          ...
          >
            <div className="flex items-start gap-3">
              ...
              <p className="text-purple-200">{aiSuggestion}</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
                ...
                <div className="flex items-start justify-between">
                  <div className="flex-1">
                    <div className="flex items-center gap-3 mb-2">
                      <span className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-sm font-semibold ${getTypeColor(result.type)}`}>
                        {getIcon(result.type)}
                    ...
                    {result.aiInsight && (
                      <div className="mt-4 p-3 bg-purple-900/30 rounded-lg border border-purple-500/30">
                        <div className="flex items-start gap-2">
                          ...
                          <p className="text-sm text-purple-200">{result.aiInsight}</p>
                        </div>
                      </div>
                    )}
                  </div>
