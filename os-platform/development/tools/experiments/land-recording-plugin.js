...
app.post('/api/land-records/recordings', upload.single('document'), async (req, res) => {
  ...
  try {
    ...
    const { rows: [recording] } = await client.query(`
      ...
      RETURNING *
    `, [
      recordingNumber, req.body.documentTypeId, grantors, grantees,
      parcelIds, legalDesc, req.body.propertyAddress, req.body.pageCount,
      req.body.considerationAmount, fees.recordingFee, fees.total,
      req.file.originalname, fileHash, req.file.size, ocrText,
      'pending', req.user.email
    ]);
    
    // Store document in S3/local storage
    const storagePath = await storeDocument(req.file, recordingNumber);
    await client.query(
      'UPDATE land_records.recordings SET storage_path = $1 WHERE id = $2',
      [storagePath, recording.id]
    );
...
app.post('/api/land-records/erecording/submit', express.text({ type: 'application/xml' }), async (req, res) => {
  const xml = req.body;
...
function calculateRecordingFees(docType, pageCount, consideration = 0) {
  const fees = {
    ...
    transferTax: 0,
    techFund: 5.00, // State technology fund
    archiveFee: 2.00,
    total: 0
  };
...
function parseDocument(text) {
  ...
  const grantorMatch = text.match(/GRANTOR[S]?:(.*?)GRANTEE/si);
  if (grantorMatch) {
    const names = grantorMatch[1].split(/,|AND/i).map(n => n.trim());
    grantors.push(...names.filter(n => n.length > 2));
  }
  
  const granteeMatch = text.match(/GRANTEE[S]?:(.*?)(?:WITNESSETH|PROPERTY|CONSIDERATION)/si);
  if (granteeMatch) {
  ...
  const legalMatch = text.match(/LEGAL DESCRIPTION:(.*?)(?:SUBJECT TO|TOGETHER WITH|$)/si);
  if (legalMatch) {
