...
interface AuditItemProps {audit: Audit;
  onSelect: (audit: Audit) =>void;}

export default function AuditItem({audit, onSelect}: AuditItemProps) {// Function to get priority badge style
  ...
  const getStatusBadge = () =>{switch (audit.status) {
      case "pending":
  ...
  const getPriorityIcon = () =>{if (audit.priority === "urgent" || audit.priority === "high") {
  ...
  return (<div 
    ...
    ><div className="flex justify-between items-start"><div><div className="flex items-center">{getPriorityIcon()}<h4 className="font-medium">{audit.auditNumber}</h4>{getPriorityBadge()}</div><><p className="mt-1 text-sm text-neutral-600">{audit.description}</p><div
</>
className="mt-2 flex items-center text-xs text-neutral-500"><><span className="material-icons text-xs mr-1">calendar_today</span><span
</></>>Submitted: {formatDate(audit.submittedAt)}</span><><span className="mx-2">•</span><span
</>
className="material-icons text-xs mr-1">person</span><span>By: {audit.submittedById}</span></div></div><div className="flex flex-col items-end">{getStatusBadge()}<span className="mt-2 text-xs text-neutral-500">Due: {formatDate(audit.dueDate)}</span></div></div></div>
  );
}
