...
interface CommentFormProps {auditId: number;}

export default function CommentForm({auditId}: CommentFormProps) {const [comment, setComment] = useState("");
  const { toast} = useToast();
  const queryClient = useQueryClient();

  // Comment mutation
  const commentMutation = useMutation({mutationFn: async ({ auditId, comment}: {auditId: number, comment: string}) =>{
  ...
  const handleSubmit = (e: React.FormEvent) => {e.preventDefault();
    ...
    commentMutation.mutate({auditId,
      comment: comment.trim()});
  };

  return (<form onSubmit={handleSubmit} className="mt-4"><div className="mb-3"><><label htmlFor="comment" className="block text-sm font-medium text-neutral-700 mb-1">Add Comment</label><textarea
</>
        ...
        ></textarea></div><div className="flex justify-end"><button
          type="submit"
