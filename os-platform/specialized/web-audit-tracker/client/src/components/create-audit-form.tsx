...
const createAuditSchema = insertAuditSchema.extend({title: z.string().min(5, "Title must be at least 5 characters"),
  ...
  dueDate: z.date({
    ...
    invalid_type_error: "Due date must be a valid date"}).refine((date) => date > new Date(), {message: "Due date must be in the future"})
});

type CreateAuditFormValues = z.infer<typeof createAuditSchema>;

interface CreateAuditFormProps {onSuccess?: () => void;
  onCancel?: () => void;}

export default function CreateAuditForm({onSuccess, onCancel}: CreateAuditFormProps) {const { user} = useAuth();
  ...
  const defaultValues: Partial<CreateAuditFormValues> = {title: "",
    description: "",
    propertyId: "",
    address: "",
    currentAssessment: 0,
  ...
  const {register,
    ...
    reset,
    formState: { errors}
  } = useForm<CreateAuditFormValues>({resolver: zodResolver(createAuditSchema),
    defaultValues});
  ...
  const onSubmit = (data: CreateAuditFormValues) => {// Calculate the tax impact if it's not provided
    if (!data.taxImpact) {
  ...
  return (<div className="bg-white p-6 rounded-lg shadow-md"><><h2 className="text-xl font-semibold mb-6">Create New Audit</h2><form
</>onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        {/* Basic Audit Information */}<div className="space-y-4"><><h3 className="text-md font-medium border-b pb-2 mb-2">Basic Information</h3><div
</>
className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><><label className="block text-sm font-medium text-neutral-700 mb-1">Title</label><input
</>type="text"
              ...
              />
              {errors.title && (<p className="text-red-500 text-xs mt-1">{errors.title.message}</p>)}</div><div><><label className="block text-sm font-medium text-neutral-700 mb-1">Priority</label><select
</>
...
value="normal">Normal</option><><option value="high">High</option><option
</>
value="urgent">Urgent</option></select></div></div><div><><label className="block text-sm font-medium text-neutral-700 mb-1">Description</label><textarea
</>
              ...
              disabled={createAuditMutation.isPending}
            ></textarea>{errors.description && (<p className="text-red-500 text-xs mt-1">{errors.description.message}</p>)}</div><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><><label className="block text-sm font-medium text-neutral-700 mb-1">Property ID</label><input
</>type="text"
                ...
                disabled={createAuditMutation.isPending}
              />
              {errors.propertyId && (<p className="text-red-500 text-xs mt-1">{errors.propertyId.message}</p>)}</div><div><><label className="block text-sm font-medium text-neutral-700 mb-1">Due Date</label><input
</>type="date"
                ...
                disabled={createAuditMutation.isPending}
              />
              {errors.dueDate && (<p className="text-red-500 text-xs mt-1">{errors.dueDate.message}</p>)}</div></div><div><><label className="block text-sm font-medium text-neutral-700 mb-1">Property Address</label><input
</>type="text"
              ...
              disabled={createAuditMutation.isPending}
            />
            {errors.address && (<p className="text-red-500 text-xs mt-1">{errors.address.message}</p>)}</div></div>{/* Assessment Information */}<div className="space-y-4"><><h3 className="text-md font-medium border-b pb-2 mb-2">Assessment Information</h3><div
</>
className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><><label className="block text-sm font-medium text-neutral-700 mb-1">Current Assessment ($)</label><input
</>type="number"
                min="0"
                step="1000"
                ...
                disabled={createAuditMutation.isPending}
              />
              {errors.currentAssessment && (<p className="text-red-500 text-xs mt-1">{errors.currentAssessment.message}</p>)}</div><div><><label className="block text-sm font-medium text-neutral-700 mb-1">Proposed Assessment ($)</label><input
</>type="number"
                min="0"
                step="1000"
                ...
                disabled={createAuditMutation.isPending}
              />
              {errors.proposedAssessment && (<p className="text-red-500 text-xs mt-1">{errors.proposedAssessment.message}</p>)}</div></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><div><label className="block text-sm font-medium text-neutral-700 mb-1">Estimated Tax Impact ($) <span className="text-neutral-500 text-xs">(Optional)</span></label><><input
                type="number"
                className="w-full border border-neutral-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Will be calculated if empty"
                {...register("taxImpact", {valueAsNumber: true})}
                disabled={createAuditMutation.isPending} /></div><div
</></>><label className="block text-sm font-medium text-neutral-700 mb-1">Reason for Amendment <span className="text-neutral-500 text-xs">(Optional)</span></label><input
                type="text"
                className="w-full border border-neutral-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                placeholder="Brief explanation for the assessment change"
                {...register("reason")}
                disabled={createAuditMutation.isPending} /></div></div></div>{/* Form Actions */}<div className="flex justify-end space-x-4 pt-4">{onCancel && (<button
              type="button"
