...
type AuthContextType = {user: SelectUser | null;
  isLoading: boolean;
  error: Error | null;
...
export function AuthProvider({children}: {children: ReactNode}) {const { toast} = useToast();
  ...
  const {data: user,
    error,
    isLoading,
    refetch,} = useQuery<SelectUser | null, Error>({queryKey: ['/api/user'],
  ...
  // Login mutation
  const loginMutation = useMutation({mutationFn: async (credentials: LoginData) =>{
    ...
    onSuccess: (user: SelectUser) => {
      ...
      .then(validatedUser => {console.log("Session validated successfully:", validatedUser);
        // Force full page reload to ensure proper session handling
        window.location.href = "/";})
      .catch(err => {console.error("Error validating session after login:", err);
  ...
  const registerMutation = useMutation({mutationFn: async (credentials: InsertUser) => {
    ...
    onSuccess: (user: SelectUser) => {
      ...
      .then(validatedUser => {console.log("Session validated successfully:", validatedUser);
        // Force full page reload to ensure proper session handling
        window.location.href = "/";})
      .catch(err => {console.error("Error validating session after registration:", err);
        // Try one more time with different approach - hard reload
        window.location.replace("/");});
      */
    ...
    onError: (error: Error) => {console.error('Registration error:', error.message);
      toast({
  ...
  // Logout mutation
  const logoutMutation = useMutation({mutationFn: async () => {
    ...
    onError: (error: Error) => {console.error('Logout error:', error.message);
      toast({
        title: 'Logout failed',
  ...
  const updateEmailMutation = useMutation({mutationFn: async (data: { email: string}) => {console.log('Email update attempt:', data.email);
      try {
        const res = await apiRequest('POST', '/api/update-email', data);
        return await res.json();} catch (error) {console.error('Email update error:', error);
        throw error instanceof Error ? error : new Error('Failed to update email');}
    },
    onSuccess: data => {console.log('Email update successful:', data.email);
    ...
    onError: (error: Error) => {console.error('Email update error:', error.message);
      toast({
  ...
  const resetPasswordMutation = useMutation({mutationFn: async (data: { username: string}) => {console.log('Password reset attempt for:', data.username);
      try {
        const res = await apiRequest('POST', '/api/reset-password', data);
        return await res.json();} catch (error) {console.error('Password reset error:', error);
        throw error instanceof Error ? error : new Error('Failed to reset password');}
    },
    onSuccess: data => {console.log('Password reset request successful');
      toast({
        title: 'Password reset initiated',
        description:
          data.message || 'If your account exists, a password reset email has been sent.',});
    },
    onError: (error: Error) => {console.error('Password reset error:', error.message);
      toast({
