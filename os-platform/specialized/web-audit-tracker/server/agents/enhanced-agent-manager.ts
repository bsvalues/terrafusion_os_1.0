...
export class EnhancedAgentManager {
  ...
  private async checkAgentHealth(agentId: string): Promise<void> {
    ...
    try {
      ...
      if (isHealthy) {
        ...
        log(
          `Agent ${agentId} health check failed, consecutive failures: ${health.healthCheck.consecutiveFailures}`,
          'enhanced-manager'
        );
  ...
  private handleAgentFailure(agentId: string): void {
    ...
    if (health.healthCheck.consecutiveFailures > (config.maxRetries || 3) * 2) {
      log(
        `Agent ${agentId} has failed too many times, not attempting to restart`,
        'enhanced-manager'
      );
      return;
    }
  ...
  private updateAgentHealth(agentId: string, update: Partial<AgentHealth>): void {
    ...
    if (statusChanged) {
      ...
      log(
        `Agent ${agentId} status changed from ${currentHealth.status} to ${update.status}`,
        'enhanced-manager'
      );
    }
