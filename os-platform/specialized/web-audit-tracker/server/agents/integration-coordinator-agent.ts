...
export class IntegrationCoordinatorAgent extends BaseAgent {
  ...
  private runIntegrationCheckpoint(): void {
    ...
    const message = createMessage(this.id, 'all', MessageEventType.BROADCAST, {
  ...
  registerApiEndpoint(endpoint: Omit<ApiEndpoint, 'consumers'>): string {
    ...
    const message = createMessage(this.id, 'all', MessageEventType.EVENT, {
  ...
  private handleApiQuery(message: any): void {
    ...
    // Send response
    const response = createMessage(this.id, senderId, MessageEventType.RESPONSE, responseData, {
  ...
  getStatus(): AgentStatus {
    return {
      ...
      issueCount: Array.from(this.integrations.values()).reduce(
        (count, integration) => count + (integration.issues?.length || 0),
        0
      ),
    };
  }
