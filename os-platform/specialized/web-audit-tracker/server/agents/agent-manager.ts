...
export class AgentManager {
  ...
  constructor(
    ...
    config: AgentManagerConfig = { agents: {} }
  ) {
  ...
  public async initialize(): Promise<void> {
    ...
    try {
      ...
      const startupPromises = Object.entries(this.config.agents).map(([agentId, agentConfig]) =>
        this.startAgent(agentId, agentConfig.type, agentConfig.settings)
      );
  ...
  private async startMasterControlProgram(): Promise<BaseAgent> {
    ...
    const mcp = new MasterControlProgram(mcpId, this.communicationBus, {
      priorityQueue: true,
  ...
  private async startDataValidationAgent(
    ...
    const agent = new DataValidationAgent(agentId, this.communicationBus, settings);
  ...
  private async startComplianceAgent(
    ...
    const agent = new ComplianceAgent(agentId, this.communicationBus, settings);
  ...
  private async startArchitectPrimeAgent(
    ...
    const agent = new ArchitectPrimeAgent(agentId, this.communicationBus, settings);
  ...
  private async startIntegrationCoordinatorAgent(
    ...
    const agent = new IntegrationCoordinatorAgent(agentId, this.communicationBus, settings);
  ...
  public async startAgent(
    ...
    try {
      ...
      if (agent && agentId !== 'mcp') {
        ...
        if (mcp) {
          const message = createMessage('agent-manager', 'mcp', MessageEventType.EVENT, {
  ...
  public async shutdown(): Promise<void> {
    ...
    try {
      ...
      this.communicationBus.unsubscribeFromAllMessages(
        this.handleMessageForReplayBuffer.bind(this)
      );
    ...
    if (
      ...
      const priorityLevel =
        message.metadata?.priority === MessagePriority.HIGH
          ? 0
          : message.metadata?.priority === MessagePriority.MEDIUM
            ? 1
            : 2;
