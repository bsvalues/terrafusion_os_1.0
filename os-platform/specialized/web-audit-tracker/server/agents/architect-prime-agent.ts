...
export class ArchitectPrimeAgent extends BaseAgent {
  ...
  private sendWelcomeMessage(agentId: string, agentType: string): void {
    const message = createMessage(this.id, agentId, MessageEventType.COMMAND, {
      command: 'welcome',
  ...
  private handleArchitectureQuery(message: any): void {
    ...
    // Send response
    const response = createMessage(this.id, requesterId, MessageEventType.RESPONSE, responseData, {
