...
export class MasterControlProgram extends BaseAgent {
  ...
  private async setupAgents(): Promise<void> {
    ...
    const architectPrimeAgent = new ArchitectPrimeAgent('architect-prime', communicationBus, {
      strategicVision:
        'Ensure accurate property assessments and compliance with Washington State regulations',
      systemGoals: [
        'Reduce assessment errors by 85%',
    ...
    const integrationCoordinatorAgent = new IntegrationCoordinatorAgent(
    ...
    this.logger(
      `Agent registry setup complete. ${this.agents.size} agents registered in hierarchical structure`
    );
  }
  ...
  private async handleGetAgentStatus(params: any): Promise<any> {
    ...
    const agentTasks = Array.from(this.taskInfoMap.values()).filter(
      task => task.agentType === agentType
    );

    const pendingTasks = agentTasks.filter(task => task.status === 'pending').length;
    const processingTasks = agentTasks.filter(
      task => task.status === 'assigned' || task.status === 'processing'
    ).length;
  ...
  private async handleGetSystemStatus(params: any): Promise<any> {
    ...
    const pendingTasks = Array.from(this.taskInfoMap.values()).filter(
      task => task.status === 'pending'
    ).length;
    const assignedTasks = Array.from(this.taskInfoMap.values()).filter(
      task => task.status === 'assigned'
    ).length;
    const processingTasks = Array.from(this.taskInfoMap.values()).filter(
      task => task.status === 'processing'
    ).length;
    const completedTasks = Array.from(this.taskInfoMap.values()).filter(
      task => task.status === 'completed'
    ).length;
    const failedTasks = Array.from(this.taskInfoMap.values()).filter(
      task => task.status === 'failed'
    ).length;
    const cancelledTasks = Array.from(this.taskInfoMap.values()).filter(
    ...
    const completedTasksArray = Array.from(this.taskInfoMap.values()).filter(
