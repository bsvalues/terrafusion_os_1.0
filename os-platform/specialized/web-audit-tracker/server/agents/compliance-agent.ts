  ...
  PROPERTY_CLASSIFICATION: [
    {
      ...
      description:
        'Properties must be classified according to Washington Administrative Code land use codes',
    ...
    },
    {
      ...
      description:
        'Properties must be correctly classified as residential, commercial, or other approved categories',
...
export class ComplianceAgent extends BaseAgent {
  ...
  private async checkPropertyCompliance(params: any): Promise<any> {
    try {
      ...
      const categoriesToCheck =
        categories && categories.length > 0 ? categories : Object.keys(this.regulationsCache);
            ...
            for (const regulation of this.regulationsCache[category]) {
              ...
              if (typeof this[checkFunction] === 'function') {
                try {
                  ...
                  this.logger(
                    `Error running compliance check ${regulation.id} for property ${propertyId}: ${error.message}`
                  );
                  propertyResults.push({
                    regulationId: regulation.id,
                    regulationName: regulation.name,
  ...
  private async generateComplianceReport(params: any): Promise<any> {
    try {
      const { taxingDistrict, propertyType, assessmentYear, propertyIds, categories, sampleSize } =
        params;
      ...
      const overallComplianceScore =
        totalChecks > 0 ? Math.round((totalCompliantChecks / totalChecks) * 100) : 0;
   ...
   */
  private async validateLandUseCode(
    property: any,
    assessmentYear?: number
  ): Promise<Omit<ComplianceCheckResult, 'regulationId' | 'regulationName'>> {
   ...
   */
  private async validateMarketValueAlignment(
    property: any,
    assessmentYear?: number
  ): Promise<Omit<ComplianceCheckResult, 'regulationId' | 'regulationName'>> {
   ...
   */
  private async validateValueConsistency(
    property: any,
    assessmentYear?: number
  ): Promise<Omit<ComplianceCheckResult, 'regulationId' | 'regulationName'>> {
    ...
    if (!isConsistent) {
      return {
        ...
        severity: 'critical',
        remediation:
          'Recalculate and correct the total value to match the sum of land and improvement values',
      };
    }
   ...
   */
  private async validateSeniorExemption(
    property: any,
    exemptionData?: any
  ): Promise<Omit<ComplianceCheckResult, 'regulationId' | 'regulationName'>> {
    ...
    if (
      (!exemptionData || Object.keys(exemptionData).length === 0) &&
   ...
   */
  private async validateAssessmentCycle(
    property: any,
    assessmentYear?: number
  ): Promise<Omit<ComplianceCheckResult, 'regulationId' | 'regulationName'>> {
   ...
   */
  private async validateListingDeadlines(
    property: any,
    assessmentYear?: number
  ): Promise<Omit<ComplianceCheckResult, 'regulationId' | 'regulationName'>> {
   ...
   */
  private async validateUniformity(
    property: any,
    assessmentYear?: number
  ): Promise<Omit<ComplianceCheckResult, 'regulationId' | 'regulationName'>> {
