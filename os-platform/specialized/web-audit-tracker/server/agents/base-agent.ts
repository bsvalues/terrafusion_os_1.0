...
export abstract class BaseAgent {
  ...
  protected registerMessageHandlers(): void {
    ...
    this.messageHandlers.set(
      MessageEventType.ASSISTANCE_REQUESTED,
      this.handleAssistanceRequest.bind(this)
    );
  }
  ...
  public async shutdown(): Promise<void> {
    ...
    try {
      ...
      if (typeof this.communicationBus.unsubscribeFromAgent === 'function') {
        ...
        logger.warn(
          `Agent ${this.agentId}: unsubscribeFromAgent not available on communication bus`
        );
      }
  ...
  protected async handleMessage(message: AgentMessage): Promise<void> {
    logger.debug(
      `Agent ${this.agentId} received message: ${message.messageId} of type ${message.eventType}`
    );
  ...
  protected async sendMessage(
    ...
    const message = createMessage(this.agentId, targetAgentId, eventType, payload, { priority });
    ...
    try {
      if (typeof this.communicationBus.sendMessageWithResponse === 'function') {
        ...
        logger.warn(
          `Agent ${this.agentId}: sendMessageWithResponse not available on communication bus`
        );
  ...
  protected broadcastStatus(): void {
    const statusMessage = createMessage(this.agentId, 'broadcast', MessageEventType.STATUS_UPDATE, {
      status: this.status,
