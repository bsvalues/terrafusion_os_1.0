...
export class ValuationAgent extends BaseAgent {
  ...
  protected async onInitialize(): Promise<void> {
    ...
    await this.sendMessage(AgentType.MCP, MessageEventType.COMMAND, {
  ...
  protected async onShutdown(): Promise<void> {
    ...
    try {
      await this.sendMessage(AgentType.MCP, MessageEventType.STATUS_UPDATE, {
  ...
  private async handleValuationRequest(message: ValuationRequestMessage): Promise<void> {
    ...
    try {
      ...
      if (!property) {
        throw new Error(
          `Property not found with ID ${propertyId} or parcel number ${parcelNumber}`
        );
      }
  ...
  private async handleAnomalyDetectionRequest(message: AgentMessage): Promise<void> {
    ...
    try {
      ...
      this.communicationBus.publish(AgentCommunicationBus.createErrorResponse(message, error));
    }
  }
  ...
  private async calculatePropertyValue(params: {
    ...
    if (useComparables) {
      try {
        ...
        if (
          comparableAnalysis &&
          Math.abs(comparableAnalysis.medianValue - totalValue) > totalValue * 0.1
        ) {
  ...
  private async findComparableProperties(params: any): Promise<any> {
    ...
    const comparableResult = await this.comparableService.findComparableProperties({
      propertyId,
      maxResults: params.maxResults || params.count || 10,
    ...
    const referenceMetrics = this.comparableService.calculateValueMetrics(
  ...
  private async detectValuationAnomalies(options: any): Promise<any> {
    ...
    return {
      ...
      summary: {
        ...
        averageDeviation:
          anomalies.reduce((sum, a) => sum + Math.abs(a.deviationPercentage), 0) /
          (anomalies.length || 1),
      },
      timestamp: new Date(),
    };
