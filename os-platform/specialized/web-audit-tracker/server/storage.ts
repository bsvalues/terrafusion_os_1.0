...
export interface IStorage {
  ...
  updateWorkflowDefinition(
    id: number,
    update: Partial<WorkflowDefinition>
...
export class DatabaseStorage implements IStorage {
  ...
  async updateUserEmail(userId: number, email: string): Promise<User | undefined> {
    ...
    const results = await db.update(users).set({ email }).where(eq(users.id, userId)).returning();
  ...
  async updateAudit(id: number, update: Partial<Audit>): Promise<Audit | undefined> {
    ...
    const results = await db.update(audits).set(updatedValues).where(eq(audits.id, id)).returning();
  ...
  async getWorkflowInstance(auditId: number): Promise<WorkflowInstance | undefined> {
    const results = await db
      ...
      .where(eq(workflowInstances.auditId, auditId));
    return results.length > 0 ? results[0] : undefined;
  }
  ...
  async advanceWorkflow(
    ...
    const completedSteps = [
      ...(instance.completedStepIndexes as number[]),
