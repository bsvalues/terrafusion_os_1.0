 ...
 */
function handleSelect(query, values, tables) {
 ...
 */
function handleInsert(query, values, tables) {
 ...
 */
function handleUpdate(query, values, tables) {
    ...
    rows.forEach(row => {
        ...
        if (parcelIdMatch && countyIdMatch) {
            ...
            if (values[0] !== undefined)
                row.assessed_value = values[0];
            if (values[1] !== undefined)
                row.tax_year = values[1];
            if (values[2] !== undefined)
                row.updated_by = values[2];
 ...
 */
function handleDelete(query, values, tables) {
    const tableMatch = query.match(/FROM\s+(\w+)/i);
    const tableName = tableMatch ? tableMatch[1] : 'properties';
    const rows = tables.get(tableName) || [];
