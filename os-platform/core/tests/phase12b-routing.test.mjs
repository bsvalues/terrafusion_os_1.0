...
describe('Phase 12B - MCP PostGIS Routing (Valid DSN Resolution)', () => {
  it('resolves Benton development DSN correctly', async () => {
    ...
    assert.strictEqual(
      ...
      'terrafusion_benton_dev',
      'Database must match fixture'
    );
  });
...
describe('Phase 12B - MCP PostGIS Routing (Manifest Generation)', () => {
  it('generates routing manifest with required fields', async () => {
    ...
    assert.strictEqual(
      ...
      undefined,
      'Manifest must NOT include raw connection string'
    );
  });
});
...
describe('Phase 12B - MCP PostGIS Routing Regression Guards', () => {
  ...
  it('postgis-routing.ts exports required functions', async () => {
    ...
    assert.strictEqual(
      typeof postgisRouting.resolveDataSource,
      'function',
      'Must export resolveDataSource'
    );
    assert.strictEqual(
      typeof postgisRouting.generateRoutingManifest,
      'function',
      'Must export generateRoutingManifest'
    );
    assert.strictEqual(
      typeof postgisRouting.calculateDSNHash,
      'function',
      'Must export calculateDSNHash'
    );
  });
});
