 ...
 */
export function calculateDSNHash(dataSource) {
 ...
 */
function extractCountyFromDatabase(database) {
 ...
 */
function extractEnvironmentFromDatabase(database) {
    const match = database.match(/^terrafusion_[a-z]+_(dev|development|staging|prod|production)$/i);
    if (!match)
        return null;
 ...
 */
export async function resolveDataSource(requestedCounty, requestedEnvironment, dataSource) {
    ...
    if (!actualCounty) {
        throw new Error(`Invalid database name format: ${dataSource.database}. ` +
            `Expected: terrafusion_{county}_{environment}`);
    }
    if (actualCounty !== requestedCounty.toLowerCase()) {
        throw new Error(`County mismatch: requested ${requestedCounty} but DSN resolved to ${actualCounty}. ` +
    ...
    if (actualEnvironment !== requestedEnvironment.toLowerCase()) {
        throw new Error(`Environment mismatch: requested ${requestedEnvironment} but DSN resolved to ${actualEnvironment}. ` +
 ...
 */
export async function generateRoutingManifest(resolved) {
    const manifest = {
