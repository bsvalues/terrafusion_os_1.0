...
/**
 * Query validation modes
 */
export type QueryMode = 'read' | 'write' | 'ddl';

/**
 * Query risk levels
 */
export type QueryRisk = 'read' | 'write' | 'ddl' | 'injection' | 'filesystem-access';
...
/**
 * PII-safe trace payload (no raw SQL or params)
 */
export interface TracePayload {
  ...
  risk: QueryRisk;
  timestamp: string;
  // Explicitly no rawQuery or rawParams
}
 ...
 */
function detectStatementType(normalized: string): 'select' | 'write' | 'ddl' | 'unknown' {
 ...
 */
function containsDangerousKeyword(sql: string): string | null {
 ...
 */
function isParameterized(sql: string, params: unknown[]): boolean {
  const upper = sql.toUpperCase();
 ...
 */
function extractRowLimit(sql: string): number | null {
