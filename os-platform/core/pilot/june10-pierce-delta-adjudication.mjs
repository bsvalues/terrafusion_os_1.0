...
const PIERCE = {
  county: "Pierce",
  fips: "53053",
  countyId: "d4f5c5a1-8c6d-d91e-932e-de7f6b4f83e8"
};

const DEFAULT_ACQUISITION = path.join(repoRoot, "os-platform", "core", "pilot", "evidence", "june10-wave2-source-acquisition.latest.json");
...
const DEFAULT_OUT_ROOT = path.join(repoRoot, "os-platform", "core", "pilot", "evidence", "june10-pierce-delta-adjudication");
const DEFAULT_OUT_JSON = path.join(repoRoot, "os-platform", "core", "pilot", "evidence", "june10-pierce-delta-adjudication.latest.json");
const DEFAULT_OUT_MD = path.join(repoRoot, "os-platform", "core", "pilot", "evidence", "june10-pierce-delta-adjudication.latest.md");
...
function fetchPierceCanonicalIdsAfterPrefixRepair() {
  const sql = `select regexp_replace("ParcelNumber", '^053-', '')
from canonical_tf.tf_parcel
where "CountyId"='${PIERCE.countyId}'::uuid
...
export function classifyPierceDecision({
  sourceOnlyCount,
  canonicalOnlyCount,
  sourceCoverageRatio,
  sourceArtifactKind,
...
function decideProbe(acquisitionCounty) {
  ...
  const sourceArtifactKind = /sale\.txt|sale|sales/i.test(sourceText) ? "sales_history_datamart" : "unknown_partial_payload";
  return {
    attempted: false,
    sourceArtifactKind,
    reason:
      sourceArtifactKind === "sales_history_datamart"
        ? "Pierce evidence is a sale.txt sales-history datamart export, not a governed full parcel inventory or current parcel query endpoint."
        : "No governed current-source parcel lookup/query endpoint is recorded for Pierce in this slice."
  };
}

export function adjudicatePierceDelta({ sourceIds, canonicalIds, acquisitionCounty, repairCounty }) {
  ...
  const decision = classifyPierceDecision({
  ...
  if (decision === "blocked_source_semantics") {
    blockers.push("Pierce source artifact is not a governed full parcel inventory, so canonical-only rows cannot be adjudicated.");
  }
...
function renderMarkdown(report) {
  ...
  return `# Pierce Delta Adjudication
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const acquisitionCounty = acquisition.counties.find((county) => county.county === "Pierce");
  const repairCounty = repair.counties.find((county) => county.county === "Pierce");
  const report = adjudicatePierceDelta({
    sourceIds: parseSourceIdsJsonl(paths.sourceIds),
    canonicalIds: paths.canonicalIds ? parseCanonicalIdsText(paths.canonicalIds) : fetchPierceCanonicalIdsAfterPrefixRepair(),
    acquisitionCounty,
    repairCounty
  });

  writeLines(path.join(paths.outRoot, "pierce-source-only-parcels.txt"), report._deltaLists.sourceOnly);
  writeLines(path.join(paths.outRoot, "pierce-canonical-only-parcels.txt"), report._deltaLists.canonicalOnly);
  writeLines(path.join(paths.outRoot, "pierce-overlap-parcels.txt"), report._deltaLists.overlap);
  delete report._deltaLists;
  writeJson(paths.outJson, report);
  writeText(paths.outMd, renderMarkdown(report));
  console.log(`Pierce delta adjudication written: ${repoRelative(paths.outJson)}`);
