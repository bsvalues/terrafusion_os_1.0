...
const KLICKITAT = {
  county: "Klickitat",
  fips: "53039",
  countyId: "9d619518-23ca-f6e9-03c9-6219db494501"
};
...
const DEFAULT_OUT_ROOT = path.join(
  ...
  "pilot",
  "evidence",
  "june10-klickitat-delta-adjudication"
);
const DEFAULT_OUT_JSON = path.join(
  ...
  "pilot",
  "evidence",
  "june10-klickitat-delta-adjudication.latest.json"
);
const DEFAULT_OUT_MD = path.join(
  ...
  "pilot",
  "evidence",
  "june10-klickitat-delta-adjudication.latest.md"
);
...
function fetchKlickitatCanonicalIdsAfterPrefixRepair() {
  const sql = `select regexp_replace("ParcelNumber", '^039-', '')
from canonical_tf.tf_parcel
where "CountyId"='${KLICKITAT.countyId}'::uuid
...
export function classifyKlickitatDecision({
  sourceOnlyCount,
  canonicalOnlyCount,
  sourceCoverageRatio,
  sourceArtifactKind,
...
function decideProbe(acquisitionCounty) {
  ...
  const sourceArtifactKind = /sales|sale/i.test(sourceText) ? "sales_report" : "unknown_partial_payload";
  return {
    attempted: false,
    sourceArtifactKind,
    reason:
      sourceArtifactKind === "sales_report"
        ? "Klickitat evidence is static sales-report workbook data, not a governed parcel inventory or query endpoint."
        : "No governed current-source parcel lookup/query endpoint is recorded for Klickitat in this slice."
  };
}

export function adjudicateKlickitatDelta({ sourceIds, canonicalIds, acquisitionCounty, repairCounty }) {
  ...
  const decision = classifyKlickitatDecision({
  ...
  if (decision === "blocked_source_semantics") {
    blockers.push("Klickitat source artifact is not a governed full parcel inventory, so canonical-only rows cannot be adjudicated.");
  }
...
function renderMarkdown(report) {
  ...
  return `# Klickitat Delta Adjudication
...
export async function main(argv = process.argv.slice(2)) {
  ...
  const acquisitionCounty = acquisition.counties.find((county) => county.county === "Klickitat");
  const repairCounty = repair.counties.find((county) => county.county === "Klickitat");
  const report = adjudicateKlickitatDelta({
    sourceIds: parseSourceIdsJsonl(paths.sourceIds),
    canonicalIds: paths.canonicalIds ? parseCanonicalIdsText(paths.canonicalIds) : fetchKlickitatCanonicalIdsAfterPrefixRepair(),
    acquisitionCounty,
    repairCounty
  });

  writeLines(path.join(paths.outRoot, "klickitat-source-only-parcels.txt"), report._deltaLists.sourceOnly);
  writeLines(path.join(paths.outRoot, "klickitat-canonical-only-parcels.txt"), report._deltaLists.canonicalOnly);
  writeLines(path.join(paths.outRoot, "klickitat-overlap-parcels.txt"), report._deltaLists.overlap);
  delete report._deltaLists;
  writeJson(paths.outJson, report);
  writeText(paths.outMd, renderMarkdown(report));
  console.log(`Klickitat delta adjudication written: ${repoRelative(paths.outJson)}`);
