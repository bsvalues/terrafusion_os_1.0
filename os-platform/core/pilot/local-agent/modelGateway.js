...
exports.LocalAgentModelGateway = void 0;
const eventLog_js_1 = require("./eventLog.js");
class LocalAgentModelGateway {
    constructor(options = {}) {
        this.options = options;
    }
    async draftPlan(task, context) {
        ...
        try {
            ...
            return {
                ...
                status: error instanceof Error && error.name === 'AbortError'
                    ...
                    : `Local model endpoint unavailable: ${error.message}`,
                httpStatus: null,
                latencyMs: Date.now() - startedAt,
                payload: {},
            };
    ...
    failedChatResult(model, endpoint, status, httpStatus = null, latencyMs = null, available = false) {
        return {
            ok: false,
            available,
            status,
    ...
    logEvent(type, payload) {
        ...
        (0, eventLog_js_1.appendLocalAgentEvent)(this.options.repoRoot, type, payload);
    }
    logChatEvent(result, messages) {
...
exports.LocalAgentModelGateway = LocalAgentModelGateway;
function normalizeLoopbackEndpoint(rawEndpoint) {
...
function extractPlanDraft(payload) {
...
function toPlanShape(candidate) {
...
function hasUnsafeAuthorityFields(candidate) {
...
function extractChatResult(payload, model, endpoint, httpStatus, latencyMs) {
    const choice = Array.isArray(payload.choices) ? payload.choices[0] : null;
    ...
    const toolNames = toolCalls
        ...
        return fn && typeof fn === 'object' ? asNullableTrimmedText(fn.name) : null;
    })
...
function safeEndpointText(rawEndpoint) {
    try {
...
function asNullableTrimmedText(value) {
