...
exports.LocalAgentReleaseNotesBuilder = void 0;
exports.renderLocalAgentReleaseNotes = renderLocalAgentReleaseNotes;
const node_fs_1 = require("node:fs");
const eventLog_js_1 = require("./eventLog.js");
class LocalAgentReleaseNotesBuilder {
    ...
    build() {
        ...
        this.write(notes);
        (0, eventLog_js_1.appendLocalAgentEvent)(this.repoRoot, 'release_notes_written', {
    ...
    write(notes) {
        ...
        (0, node_fs_1.writeFileSync)((0, eventLog_js_1.terrafusionPath)(this.repoRoot, 'release-notes-0.1.0.json'), JSON.stringify(jsonPayload, null, 2), 'utf8');
        const markdown = renderLocalAgentReleaseNotes(notes);
        (0, node_fs_1.writeFileSync)((0, eventLog_js_1.terrafusionPath)(this.repoRoot, 'release-notes-0.1.0.md'), markdown, 'utf8');
        (0, node_fs_1.writeFileSync)(resolvePath(this.repoRoot, 'CHANGELOG.md'), ['# Changelog', '', '## 0.1.0 - TerraFusion Local Agent Runtime MVP', '', markdown, ''].join('\n'), 'utf8');
    }
}
exports.LocalAgentReleaseNotesBuilder = LocalAgentReleaseNotesBuilder;
function renderLocalAgentReleaseNotes(notes) {
    return [
...
function renderCommand(commandDefinition) {
    return [
...
function command(title, commandText, purpose) {
...
function resolvePath(repoRoot, path) {
    if (path.startsWith('.terrafusion/')) {
        return (0, eventLog_js_1.terrafusionPath)(repoRoot, path.slice('.terrafusion/'.length));
    }
    if (path.includes('/')) {
