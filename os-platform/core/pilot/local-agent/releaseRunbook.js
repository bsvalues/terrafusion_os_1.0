...
exports.LocalAgentReleaseRunbookBuilder = exports.LocalAgentReleaseRunbookError = void 0;
exports.renderLocalAgentReleaseRunbook = renderLocalAgentReleaseRunbook;
const node_fs_1 = require("node:fs");
const eventLog_js_1 = require("./eventLog.js");
class LocalAgentReleaseRunbookError extends Error {
}
exports.LocalAgentReleaseRunbookError = LocalAgentReleaseRunbookError;
class LocalAgentReleaseRunbookBuilder {
    ...
    build(version) {
        ...
        const tagCommandPayload = readJson((0, eventLog_js_1.terrafusionPath)(this.repoRoot, 'tag-command-report.json'));
        ...
        const verificationCommands = Array.isArray(tagCommandPayload?.verificationCommands) && tagCommandPayload.verificationCommands.length > 0
            ? tagCommandPayload.verificationCommands.map((command) => String(command))
        ...
        (0, node_fs_1.writeFileSync)((0, eventLog_js_1.terrafusionPath)(this.repoRoot, `release-runbook-${cleanVersion}.json`), JSON.stringify(runbook, null, 2), 'utf8');
        (0, node_fs_1.writeFileSync)((0, eventLog_js_1.terrafusionPath)(this.repoRoot, `release-runbook-${cleanVersion}.md`), renderLocalAgentReleaseRunbook(runbook), 'utf8');
        (0, eventLog_js_1.appendLocalAgentEvent)(this.repoRoot, 'release_runbook_written', {
    ...
    artifact(name, path, required, version) {
        const fullPath = resolvePath(this.repoRoot, path);
        const exists = (0, node_fs_1.existsSync)(fullPath);
        if (!exists) {
...
exports.LocalAgentReleaseRunbookBuilder = LocalAgentReleaseRunbookBuilder;
function renderLocalAgentReleaseRunbook(runbook) {
    return [
