...
export type RedactValue =
  ...
  | null
  | RedactValue[]
  | { [key: string]: RedactValue };

interface Pattern {
  // Replacement label. Empty string means strip the match entirely.
  kind: string;
  // Matcher; must be /g so .replace iterates.
  re: RegExp;
  // Optional transform — receives match and returns replacement.
...
const PATTERNS: Pattern[] = [
  ...
  // POSIX home directories.
  {
    ...
    transform: (match) => {
      ...
      return match.slice(0, second === -1 ? match.length : second).replace(
        /[^\/]+$/,
        '[redacted-user]',
      );
    },
...
export interface RedactResult<T extends RedactValue> {
  value: T;
  stats: RedactStats;
}

function redactString(input: string, stats: RedactStats): string {
  let out = input;
...
function redactRec(value: unknown, stats: RedactStats): RedactValue {
  if (typeof value === 'string') {
    return redactString(value, stats);
  }
  if (typeof value === 'number' || typeof value === 'boolean' || value === null) {
    return value as RedactValue;
  }
  if (Array.isArray(value)) {
    const out: RedactValue[] = new Array(value.length);
  ...
  if (isPlainObject(value)) {
    ...
    for (const key of Object.keys(value)) {
      ...
      out[key] = redactRec((value as Record<string, unknown>)[key], stats);
    }
    return out;
  }
 ...
 */
export function redactPayload<T extends Record<string, RedactValue>>(
  payload: T,
): RedactResult<Record<string, RedactValue>> {
  const stats: RedactStats = { replacements: 0 };
  ...
  return { value: value as Record<string, RedactValue>, stats };
}

/** Convenience: redact a single string value (used by tests). */
export function redactStringValue(input: string): string {
  const stats: RedactStats = { replacements: 0 };
