import { existsSync, readdirSync } from 'node:fs';

export interface LocalAgentNextRecommendation {
  command: string;
  reason: string;
  confidence: 'high' | 'medium' | 'low';
  detectedState: Record<string, boolean>;
}

function getDefaultModelEndpointExample(): string {
  const port = process.env.TF_LOCAL_MODEL_PORT?.trim() || '11434';
  return `http://127.0.0.1:${port}/v1`;
}

export class LocalAgentHelpSystem {
  constructor(private readonly repoRoot: string) {}

  helpMe(): string {
    return [
      'TerraFusion Local Agent Help',
      '',
      'Prometheus is the internal codename for the TerraFusion Local Agent Runtime.',
      'Prometheus is the local-first harness, not a model, not OpenMythos, and not a GUI.',
      'Use --repo-root <path> when a scripted run should write .terrafusion state into another repo root.',
      '',
      'Founder quick-start (Windows):',
      '  .\\os-platform\\core\\pilot\\tf.ps1 init',
      '  .\\os-platform\\core\\pilot\\tf.ps1 doctor',
      '  .\\os-platform\\core\\pilot\\tf.ps1 start',
      '  .\\os-platform\\core\\pilot\\tf.ps1 events',
      '  .\\os-platform\\core\\pilot\\tf.ps1 release',
      '',
      '  Or add os-platform/core/pilot to PATH, then: tf init  |  tf doctor  |  tf start',
      '',
      'pnpm equivalents:',
      '  pnpm run tf:la -- init',
      '  pnpm run tf:la -- doctor',
      '  pnpm run tf:la -- start',
      '  pnpm run tf:la -- --repo-root C:/temp/tf-repro start',
      '',
      'Bound a task before patching:',
      '  pnpm run tf:la -- plan "Describe your task here"',
      '  pnpm run tf:la -- lock-card "Describe your task here"',
      '',
      'Daily founder flow:',
      '  pnpm run tf:la -- status',
      '  pnpm run tf:la -- current-card',
      '  pnpm run tf:la -- proof',
      '  pnpm run tf:la -- save-state "Summarize what happened" --next-step "Write the next exact action"',
      '  pnpm run tf:la -- finalize',
      '',
      'State for future UI consumers:',
      `  pnpm run tf:la -- doctor --model-endpoint ${getDefaultModelEndpointExample()} --model-name local-coder`,
      `  pnpm run tf:la -- model-health --model-endpoint ${getDefaultModelEndpointExample()} --model-name local-coder`,
      `  pnpm run tf:la -- list-models --model-endpoint ${getDefaultModelEndpointExample()}`,
      '  pnpm run tf:la -- command-registry',
      '  pnpm run tf:la -- control-center-state',
      '  pnpm run tf:la -- control-center-preview',
      '',
      'Read-only advisory review:',
      '  pnpm run tf:la -- explain --include-proof --include-save-state',
      '  pnpm run tf:la -- review --include-events --include-pending-patches',
      `  pnpm run tf:la -- model-chat --model-endpoint ${getDefaultModelEndpointExample()} --message "Summarize current blockers"`,
      '',
      'Release evidence flow:',
      '  pnpm run tf:la -- release-notes',
      '  pnpm run tf:la -- docs-index',
      '  pnpm run tf:la -- product-manifest',
      '  pnpm run tf:la -- release-check',
      '  pnpm run tf:la -- release-freeze',
      '  pnpm run tf:la -- ship-mvp release --overwrite',
      '  pnpm run tf:la -- tag-gate 0.1.0',
      '  pnpm run tf:la -- release-approve 0.1.0 --name "Founder"',
      '  pnpm run tf:la -- tag-command 0.1.0',
      '  pnpm run tf:la -- release-runbook 0.1.0',
      '',
      'When unsure:',
      '  pnpm run tf:la -- next',
      '  pnpm run tf:la -- explain-commands',
      '',
      'Safety rules:',
      '  - No locked card, no patch.',
      '  - No preview, no write.',
      '  - No proof, no success claim.',
      '  - No finalize, no done.',
      '',
    ].join('\n');
  }

  explainCommands(): string {
    return [
      'TerraFusion Local Agent Command Map',
      '',
      'Guidance:',
      '  Global option: --repo-root <path>',
      '    Run the CLI against another repo root without changing the shell working directory.',
      '  start',
      '    Open the founder cockpit for guided flows.',
      '  init',
      '    First-run preflight; writes a starter plan-mode work card if none exists.',
      '  status',
      '    Read-only daily-glance synthesis from local artifacts (no model probe, no writes).',
      '  events',
      '    Read-only audit-log tail (default 20 newest). Use --tail N or --type T to narrow.',
      '  release',
      '    Read-only release plan: surveys release artifacts and prints the next exact command.',
      '  doc-truth',
      '    Verify founder-facing docs reference real CLI verbs (gate; non-zero on violation).',
      '  help-me',
      '    Show common workflows and reminders.',
      '  next',
      '    Recommend one safe next command from local state.',
      '  explain-commands',
      '    Print this command map.',
      '  command-registry',
      '    Write the machine-readable command registry.',
      '  doctor',
      '    Write local doctor and model runtime status artifacts.',
      '  model-health',
      '    Check loopback model gateway health without granting authority.',
      '  list-models',
      '    List available models from the loopback gateway.',
      '  model-chat',
      '    Send a zero-authority advisory prompt to the loopback gateway.',
      '  control-center-state',
      '    Write the future desktop Control Center state contract.',
      '  control-center-preview',
      '    Render the read-only terminal preview from control-center state.',
      '',
      'Planning:',
      '  plan',
      '    Create a bounded work card without locking it.',
      '  lock-card',
      '    Lock a work card before patching.',
      '  current-card',
      '    Show the locked work card.',
      '  clear-card',
      '    Remove the locked work card.',
      '',
      'Patch Control:',
      '  preview-patch',
      '    Create a diff preview under locked-card scope.',
      '  show-patch',
      '    Re-open a stored preview.',
      '  apply-patch',
      '    Apply a stored preview only with --approve.',
      '',
      'Validation and Handoff:',
      '  proof',
      '    Run the proof gates from the locked card.',
      '  explain',
      '    Summarize local-agent state in a read-only explanation report.',
      '  review',
      '    Review risks, patch state, and finalize blockers without mutating state.',
      '  save-state',
      '    Write the founder handoff state.',
      '  finalize',
      '    Declare done only after proof and save state exist.',
      '',
      'Release:',
      '  release-notes',
      '    Write CHANGELOG.md and the 0.1.0 release notes.',
      '  docs-index',
      '    Write the release artifact reading path.',
      '  product-manifest',
      '    Write the runtime shipping contract and release governance posture.',
      '  release-check',
      '    Validate release evidence artifacts.',
      '  release-freeze',
      '    Snapshot the founder launch freeze posture, fingerprint release artifacts, and record rerun gates.',
      '  ship-mvp',
      '    Write the release evidence bundle without approving, tagging, or pushing.',
      '  tag-gate',
      '    Validate tag readiness without creating the tag.',
      '  release-approve',
      '    Record human release owner approval after Tag Gate passes.',
      '  tag-command',
      '    Print the final manual tag command and verification commands.',
      '  release-runbook',
      '    Write the final human release runbook.',
      '',
      'Advanced:',
      '  tool',
      '    Run advanced governed tool commands under founder policy.',
      '',
      'Help may guide. Help does not mutate.',
      '',
    ].join('\n');
  }

  recommendNext(): LocalAgentNextRecommendation {
    const detectedState = this.detectState();

    if (!detectedState.lockedCard) {
      return {
        command: 'pnpm run tf:local-agent -- start',
        reason: 'No locked work card exists. The founder cockpit is the safest way to pick up or start a bounded slice.',
        confidence: 'high',
        detectedState,
      };
    }

    if (!detectedState.proofResults) {
      return {
        command: 'pnpm run tf:local-agent -- proof',
        reason: 'A locked work card exists, but proof has not run yet.',
        confidence: 'high',
        detectedState,
      };
    }

    if (!detectedState.saveState) {
      return {
        command: 'pnpm run tf:local-agent -- save-state "Summarize what happened" --next-step "Write the next exact action"',
        reason: 'Proof exists, but founder handoff state has not been written.',
        confidence: 'high',
        detectedState,
      };
    }

    if (!detectedState.finalReport) {
      return {
        command: 'pnpm run tf:local-agent -- finalize',
        reason: 'Locked card, proof, and save state exist. Finalize is the remaining completion gate.',
        confidence: 'high',
        detectedState,
      };
    }

    return {
      command: 'pnpm run tf:local-agent -- start',
      reason: 'The current run appears finalized. Re-open the founder cockpit for the next bounded slice.',
      confidence: 'medium',
      detectedState,
    };
  }

  private detectState(): Record<string, boolean> {
    const patchDir = `${this.repoRoot}/.terrafusion/patches`;
    return {
      commandRegistry: existsSync(`${this.repoRoot}/.terrafusion/command-registry.json`),
      controlCenterState: existsSync(`${this.repoRoot}/.terrafusion/control-center-state.json`),
      lockedCard: existsSync(`${this.repoRoot}/.terrafusion/current-work-card.json`),
      patchPreview: existsSync(patchDir) && readdirSync(patchDir).some(entry => entry.endsWith('.json')),
      proofResults: existsSync(`${this.repoRoot}/.terrafusion/proof-results.json`),
      saveState: existsSync(`${this.repoRoot}/.terrafusion/save-state.md`),
      finalReport: existsSync(`${this.repoRoot}/.terrafusion/final-report.json`),
      activePolicy: existsSync(`${this.repoRoot}/.terrafusion/active-policy.json`),
      doctorReport: existsSync(`${this.repoRoot}/.terrafusion/doctor-report.json`),
      modelRuntimeStatus: existsSync(`${this.repoRoot}/.terrafusion/model-runtime-status.json`),
      releaseNotes: existsSync(`${this.repoRoot}/.terrafusion/release-notes-0.1.0.json`),
      docsIndex: existsSync(`${this.repoRoot}/.terrafusion/docs-index.json`),
      productManifest: existsSync(`${this.repoRoot}/.terrafusion/product-manifest.json`),
      releaseCheck: existsSync(`${this.repoRoot}/.terrafusion/release-check-report.json`),
      releaseFreeze: existsSync(`${this.repoRoot}/.terrafusion/release-freeze-card.json`),
      shipReport: existsSync(`${this.repoRoot}/.terrafusion/ship-report.json`),
      tagGate: existsSync(`${this.repoRoot}/.terrafusion/tag-gate-report.json`),
      releaseApproval: existsSync(`${this.repoRoot}/.terrafusion/release-approval.json`),
      tagCommand: existsSync(`${this.repoRoot}/.terrafusion/tag-command-report.json`),
      releaseRunbook: existsSync(`${this.repoRoot}/.terrafusion/release-runbook-0.1.0.json`),
    };
  }
}

export function renderLocalAgentNextRecommendation(recommendation: LocalAgentNextRecommendation): string {
  return [
    'Recommended Next Command',
    '',
    `  ${recommendation.command}`,
    '',
    'Reason:',
    `  ${recommendation.reason}`,
    '',
    'Confidence:',
    `  ${recommendation.confidence}`,
    '',
    'Detected State:',
    ...Object.entries(recommendation.detectedState).map(([key, value]) => `  - ${key}: ${value}`),
    '',
  ].join('\n');
}