...
exports.LocalAgentWorkCardFactory = void 0;
exports.renderLocalAgentWorkCard = renderLocalAgentWorkCard;
...
class LocalAgentWorkCardFactory {
    constructor(repoRoot) {
        this.repoRoot = repoRoot;
    }
    build(task, assistanceInput) {
    ...
    detectProofGates(profileId) {
        const gates = ['git diff --check'];
        if ((0, node_fs_1.existsSync)((0, node_path_1.resolve)(this.repoRoot, 'package.json'))) {
            gates.push('pnpm run type-check');
        }
        if (profileId === 'local-agent-runtime' &&
            (0, node_fs_1.existsSync)((0, node_path_1.resolve)(this.repoRoot, 'os-platform/core/tests/local-agent-policy.test.mjs')) &&
            (0, node_fs_1.existsSync)((0, node_path_1.resolve)(this.repoRoot, 'os-platform/core/tests/local-agent-runtime.test.mjs'))) {
            gates.push('pnpm run test:local-agent');
        }
        return gates;
    }
    readGitStatus() {
        const completed = (0, node_child_process_1.spawnSync)('git', ['status', '--short'], {
            cwd: this.repoRoot,
            encoding: 'utf8',
            windowsHide: true,
        });
...
exports.LocalAgentWorkCardFactory = LocalAgentWorkCardFactory;
function renderLocalAgentWorkCard(card) {
...
function applyModelAssistance(card, assistanceInput) {
    ...
    const candidateFileDecision = validateCandidateFiles(assistanceInput.candidateFiles ?? [], card.allowedFiles, card.forbiddenFiles);
...
function defaultPlanAssistance() {
    return {
        requested: false,
        used: false,
...
function sanitizeRiskNotes(values) {
...
function validateCandidateFiles(values, allowedFiles, forbiddenFiles) {
    ...
    for (const candidate of values) {
        ...
        const forbidden = forbiddenFiles.some(pattern => (0, policy_js_1.matchesGlob)(normalized, pattern));
        const allowed = allowedFiles.some(pattern => (0, policy_js_1.matchesGlob)(normalized, pattern));
...
function normalizeCandidateFile(value) {
