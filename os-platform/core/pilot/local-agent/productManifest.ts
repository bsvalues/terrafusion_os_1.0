...
export interface LocalAgentReleaseGovernanceSummary {
  requiresTagGate: boolean;
  requiresReleaseApproval: boolean;
  printsTagCommandOnly: boolean;
  createsGitTag: boolean;
  pushesGitTag: boolean;
  runbookArtifact: string;
}

export interface LocalAgentProductManifest {
  createdAt: number;
  version: string;
  productId: string;
  productName: string;
  ...
  releaseGovernance: LocalAgentReleaseGovernanceSummary;
}

export class LocalAgentProductManifestBuilder {
  constructor(private readonly repoRoot: string) {}

  build(): LocalAgentProductManifest {
    const manifest: LocalAgentProductManifest = {
    ...
    writeFileSync(terrafusionPath(this.repoRoot, 'product-manifest.json'), JSON.stringify(manifest, null, 2), 'utf8');
    writeFileSync(terrafusionPath(this.repoRoot, 'product-manifest.md'), renderLocalAgentProductManifest(manifest), 'utf8');

    appendLocalAgentEvent(this.repoRoot, 'product_manifest_written', {
...
export function renderLocalAgentProductManifest(manifest: LocalAgentProductManifest): string {
  return [
