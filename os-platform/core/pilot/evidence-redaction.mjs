...
const SENSITIVE_ASSIGNMENT_PATTERN =
  /(\b(?:access[_-]?token|refresh[_-]?token|id[_-]?token|token|password|authorization|cookie|set-cookie|api[_-]?key|client[_-]?secret|secret)\b["']?\s*[:=]\s*["']?)([^"',\s;}]+)/gi;
...
export function redactEvidenceText(value) {
  return String(value)
    ...
    .replace(SENSITIVE_ASSIGNMENT_PATTERN, `$1${REDACTION_MARKER}`);
}
