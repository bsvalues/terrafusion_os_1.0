...
const defaultExec: Exec = (cmd, args) =>
  execFileSync(cmd, args, { encoding: 'utf8', stdio: ['ignore', 'pipe', 'pipe'] }).trimEnd();
...
/**
 * Compute report and optionally enforce simple hygiene policies:
 * - If `pact/` appears untracked, we flag it as "expected to be ignored" (Phase 206A).
 * - If staged set contains files outside an allowlist, we warn (prevents accidental commits).
 *
...
export function computeOpsHealthReport(opts?: {
  ...
  const branch = safe(() => exec('git', ['rev-parse', '--abbrev-ref', 'HEAD']));

  const porcelainZ = safe(() => exec('git', ['status', '--porcelain=v1', '-z'])) ?? '';
  ...
  const disallowedStaged = staged.filter(e => !isAllowedPath(e.path, allowedStagePrefixes));
...
function isAllowedPath(filePath: string, prefixes: string[]): boolean {
  for (const p of prefixes) {
    if (p.endsWith('/')) {
      if (filePath.startsWith(p)) return true;
    } else if (filePath === p || filePath.startsWith(`${p}/`)) {
      return true;
    }
  }
  return false;
