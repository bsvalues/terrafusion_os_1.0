...
The `.claude` directory contains Claude AI development environment
configuration, specifically permission settings that enable comprehensive
development operations for TerraFusion OS. This configuration optimizes the
development workflow by pre-approving essential commands and operations.
...
```json
{
  "permissions": {
    "allow": [
      "Bash(export VITE_API_URL=\"http://localhost:\${{TF_API_HTTPS_PORT:-5001}}\")", // Environment variables
...
- **Command Authorization**: Pre-approved patterns eliminate authorization
  delays
- **Script Execution**: Government operations scripts run without additional
  prompts
- **Build Operations**: Complete build pipeline permissions enable seamless
  CI/CD
...
curl http://localhost:\${{TF_API_HTTPS_PORT:-5001}}/health     # API health checking
...
# System connectivity
curl http://localhost:\${{TF_API_HTTPS_PORT:-5001}}/health     # API connectivity
