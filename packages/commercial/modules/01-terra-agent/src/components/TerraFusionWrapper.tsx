...
interface TerraFusionWrapperProps {
...
export const TerraFusionWrapper: React.FC<TerraFusionWrapperProps> = ({
  children,
  appName,
  appIcon = '🌍',
  appVersion = '1.0.0',
  ...
  useEffect(() => {
    ...
    return () => {
      document.body.classList.remove('terrafusion-branded');
    };
  }, [appName]);
  ...
  if (standalone) {
    return (
      <div className="terrafusion-standalone-app">
        <header className="terrafusion-app-header">
          <div className="terrafusion-app-header-content">
            <div className="terrafusion-app-brand"><>
...
className="status-badge championship">Championship Edition</span>
        ...
        <main className="terrafusion-app-content">
          {children}
        </main>
        <footer
</>
...
className="separator">•</span>
  ...
  return (
    <div className="terrafusion-module-content">
      {children}
    </div>
  );
};
...
.terrafusion-app-header {
  background: var(--terra-void);
  border-bottom: 1px solid rgba(0, 212, 255, 0.1);
  padding: 1rem 1.5rem;
}

.terrafusion-app-header-content {
  max-width: 1440px;
  margin: 0 auto;
  display: flex;
...
.status-badge.championship {
  background: var(--terra-gradient-empire);
  color: var(--terra-white);
  border: none;
...
.terrafusion-branded * {
  font-family: var(--terra-font-body) !important;
}
...
.terrafusion-branded h6 {
  font-family: var(--terra-font-display) !important;
  color: var(--terra-void);
}

.terrafusion-branded button {
  font-family: var(--terra-font-body) !important;
...
.terrafusion-branded textarea:focus {
  border-color: var(--terra-fusion) !important;
  box-shadow: 0 0 0 3px rgba(0, 212, 255, 0.1) !important;
}
`;

// Inject styles if not already present
if (!document.getElementById('terrafusion-standalone-styles')) {
