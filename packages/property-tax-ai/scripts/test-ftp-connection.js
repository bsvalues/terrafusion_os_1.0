...
const mockStorage = {
  getProperty: () => Promise.resolve({ propertyId: 'BC001', address: '123 Test St', parcelNumber: '12345-123-123', propertyType: 'residential', status: 'active', acres: '0.25', value: '150000' }),
  getAllProperties: () => Promise.resolve([
    { propertyId: 'BC001', address: '123 Test St', parcelNumber: '12345-123-123', propertyType: 'residential', status: 'active', acres: '0.25', value: '150000' },
    { propertyId: 'BC002', address: '456 Sample Ave', parcelNumber: '12345-123-124', propertyType: 'commercial', status: 'active', acres: '1.5', value: '750000' }
  ])
};
...
async function runFtpTests() {
  try {
    ...
    try {
      ...
      files.forEach(file => {
        console.log(`- ${file.name} (${file.type === 1 ? 'File' : 'Directory'}, Size: ${file.size} bytes)`);
      });
