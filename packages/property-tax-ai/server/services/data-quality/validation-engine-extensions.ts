...
import { ValidationIssue, IssueStatus, EntityType, RuleLevel, ValidationContext } from '../../../shared/schema';
...
export class WashingtonValidationEngine extends PropertyValidationEngine {
  ...
  public registerCustomValidator(
    name: string, 
    validatorFn: (entity: any) => boolean
  ): void {
  ...
  protected async evaluateRule(
    ...
    try {
      ...
      const implementation = typeof rule.implementation === 'string'
        ? JSON.parse(rule.implementation)
        : rule.implementation;
      
      // Check for custom validator
      if (implementation.conditions.customValidator) {
        return this.runCustomValidator(
          implementation.conditions.customValidator,
          entity,
          rule
        );
      }
