            ...
            <TableBody>
              {filteredAgents.map((agent) => (
                <TableRow key={agent.id}>
                  ...
                  <TableCell>{formatTimestamp(agent.lastActive)}</TableCell>
                  <TableCell>
          ...
          <DialogContent className="max-w-2xl">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <div className="mt-2 space-y-2">
                  <div>
                    ...
                    <span className="text-sm ml-2">{selectedAgent.id}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium">Type:</span>
                    <span className="text-sm ml-2">{selectedAgent.type}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium">Provider:</span>
                    <span className="text-sm ml-2">{selectedAgent.provider}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium">Version:</span>
                    <span className="text-sm ml-2">{selectedAgent.version}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium">Status:</span>
                    <span className="text-sm ml-2">{getStatusBadge(selectedAgent.status)}</span>
                  </div>
                  <div>
                    <span className="text-sm font-medium">Last Active:</span>
                    <span className="text-sm ml-2">{formatTimestamp(selectedAgent.lastActive)}</span>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="mt-2 space-y-2">
                  <div>
                    ...
                    <span className="text-sm ml-2">{selectedAgent.config.contextWindow} tokens</span>
                  </div>
                  {selectedAgent.config.outputTokenLimit && (
                    <div>
                      <span className="text-sm font-medium">Output Token Limit:</span>
                      <span className="text-sm ml-2">{selectedAgent.config.outputTokenLimit} tokens</span>
                    </div>
                  )}
                  {selectedAgent.config.temperature && (
                    <div>
                      <span className="text-sm font-medium">Temperature:</span>
                      <span className="text-sm ml-2">{selectedAgent.config.temperature}</span>
                    </div>
                  )}
                  {selectedAgent.config.baseUrl && (
                    <div>
                      <span className="text-sm font-medium">Base URL:</span>
                      <span className="text-sm ml-2">{selectedAgent.config.baseUrl}</span>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="col-span-2">
                ...
                <p className="mt-2 text-sm">{selectedAgent.description}</p>
              </div>
