...
import { BarChart4, Landmark, Loader2, MapPin, AlertCircle, TrendingUp, Users, Building, FilePlus2 } from 'lucide-react';
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="px-6 pt-2">
          <TabsList className="grid grid-cols-5 w-full">
            ...
            <TabsTrigger value="zoning" className="text-xs">Zoning</TabsTrigger>
        ...
        <ScrollArea className="h-[400px] px-1">
          <TabsContent value="overview" className="p-6 pt-4 space-y-4 m-0">
            <div>
              ...
              <p className="text-sm">{analysis.overview.summary}</p>
              
              <div className="grid grid-cols-3 gap-3 mt-4">
                <div className="bg-primary/10 rounded-lg p-3 text-center">
                  <div className="text-xs text-muted-foreground">Permit Count</div>
                  <div className="text-2xl font-bold">{analysis.overview.permitCount}</div>
                </div>
                <div className="bg-primary/10 rounded-lg p-3 text-center">
                  <div className="text-xs text-muted-foreground">Average Value</div>
                  <div className="text-2xl font-bold">{currencyFormat.format(analysis.overview.averageValue || 0)}</div>
                </div>
                <div className="bg-primary/10 rounded-lg p-3 text-center">
                ...
                {analysis.trends.keyIndicators.map((indicator, index) => (
                  <div key={index} className="border rounded-md p-2">
                    ...
                    <div className="flex justify-between items-center mt-1">
                      ...
                      <Badge variant={indicator.change > 0 ? "default" : "outline"} className="text-[10px]">
          ...
          <TabsContent value="demographics" className="p-6 pt-4 space-y-4 m-0">
            <div>
              ...
              <p className="text-sm">{analysis.demographics.summary}</p>
              
              <div className="bg-muted/50 p-3 rounded-lg mt-4">
                <h4 className="font-medium text-xs mb-1">Population Impact</h4>
                <p className="text-sm">{analysis.demographics.populationImpact}</p>
              </div>
              ...
              <div className="space-y-3">
                {analysis.demographics.keyDemographics.map((demo, index) => (
                  <div key={index} className="border rounded-md p-3">
                    ...
                    <div className="text-xs text-muted-foreground mt-1">{demo.needs}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <TabsContent value="zoning" className="p-6 pt-4 space-y-4 m-0">
            <div>
              ...
              <p className="text-sm">{analysis.zoning.summary}</p>
