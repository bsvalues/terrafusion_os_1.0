...
export const CostMatrixManager: React.FC = () => {
  ...
  const {
    ...
    importExcel: importExcelMatrix,
  } = useFileUploads();

  const matrices = (getAllMatrices.data as any[]) || [];
  const fileUploads = (getAllFileUploads.data as any[]) || [];
          ...
          <TabsContent value="upload" className="pt-4">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Upload government cost matrix Excel files to import into the system
                </CardDescription>
              </CardHeader>
              <CardContent>
                <div className="flex flex-col gap-4">
                  ...
                  >
                    ...
                    <input
                      ...
                      aria-label="Upload cost matrix file"
                      title="Upload cost matrix file (.xlsx or .xls)"
                    />
                  </div>
                          ...
                          <TableCell>
                            <div className="w-full max-w-[120px]">
                              {file.status === 'processing' && file.totalItems ? (
                                <div>
                                  <Progress
                                    value={((Number(file.processedItems) || 0) / (Number(file.totalItems) || 1)) * 100}
                                    className="h-2"
                                  />
