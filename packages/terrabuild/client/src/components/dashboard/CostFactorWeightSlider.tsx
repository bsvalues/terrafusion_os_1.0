  ...
  const applyPreset = async (presetId: string) => {
    ...
    if (!isNaN(presetIdNumber)) {
      try {
        ...
        let serverPreset = (defaultServerPresets.data as any[])?.find((p: any) => p.id === presetIdNumber);

        // If not in default presets, check user presets
        if (!serverPreset && userServerPresets?.data) {
          serverPreset = (userServerPresets.data as any[]).find((p: any) => p.id === presetIdNumber);
        }
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Customize how different factors influence building cost calculations for government assessments
          </p>
        </div>
      ...
      <TabsContent value="presets" className="mt-0 space-y-4">
        ...
        <Card className="mb-4">
          <CardHeader className="pb-2">
            ...
            <CardDescription>
              Official TerraFusion assessment presets for different building types
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            ...
            ) : defaultServerPresets.data && Array.isArray(defaultServerPresets.data) && defaultServerPresets.data.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(defaultServerPresets.data as any[]).map((preset: any) => (
                  <div
                    key={preset.id}
                    className={`p-4 border rounded-lg cursor-pointer transition-all ${
        ...
        <Card className="mb-4">
          ...
          <CardContent className="space-y-4">
            {user ? (
              ...
              ) : userServerPresets?.data && Array.isArray(userServerPresets.data) && userServerPresets.data.length > 0 ? (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {(userServerPresets.data as any[]).map((preset: any) => (
                    <div
                      key={preset.id}
          ...
          <CardContent className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                placeholder="e.g., East Region Commercial"
              />
            </div>
