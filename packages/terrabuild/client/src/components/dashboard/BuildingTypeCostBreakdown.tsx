...
const DEFAULT_MATERIAL_BREAKDOWNS: Record<string, Record<string, number>> = {
  "RESIDENTIAL": {
    "Structure": 0.25,
    "Foundation": 0.12,
    "Electrical": 0.10,
...
export function BuildingTypeCostBreakdown({
  ...
  const processBuildingTypeData = () => {
    ...
    const totalValue = typeData.reduce((sum: number, item: any) => sum + item.value, 0);
  ...
  const getMostCommonBuildingType = (): string => {
    ...
    Object.entries(typeCounts).forEach(([type, count]) => {
      if ((count as number) > highestCount) {
        mostCommon = type;
