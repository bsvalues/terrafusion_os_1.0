...
const calculatorSchema = z.object({
  ...
  businessPropertyValue: z.coerce
    .number()
...
const BCBSCostCalculator = () => {
  ...
  // Default form values for CostForge AI assessment
  const defaultValues: Partial<CalculatorFormValues> = {
    ...
    conditionFactor: 1.0,
    region: 'CENTRAL_DISTRICT', // Default to Central District
    buildingAge: 0,
  };
  ...
  // Building types and quality levels for CostForge AI government assessment
  const buildingTypes = [
  ...
  const calculateAgeDepreciation = (buildingAge: number, buildingType: string): number => {
    ...
    const annualDepreciationRates: Record<string, number> = {
      ...
      BOAT: 0.1, // 10% per year for boats
      BUSINESS_PROPERTY: 0.1, // 10% per year for business personal property
    };
    ...
    const annualRate =
      annualDepreciationRates[buildingType] || annualDepreciationRates['RESIDENTIAL'];
    ...
    // Apply minimum value
    const minimumValue =
      minimumDepreciationValues[buildingType] || minimumDepreciationValues['RESIDENTIAL'];
  ...
  const prepareTreemapData = () => {
    ...
    const ageDepreciationName =
      buildingAge > 0 ? `Age Depreciation (${deprecationPercentage}% Loss)` : 'Age Depreciation';
    ...
    if (
      ...
      buildingType === 'BOAT' ||
      buildingType === 'BUSINESS_PROPERTY'
    ) {
      ...
      const conditionAdjustment =
        costBreakdown.find(c => c.category === 'Condition Adjustment')?.cost || 0;
      const regionalAdjustment =
        costBreakdown.find(c => c.category === 'Regional Adjustment')?.cost || 0;
      ...
      const complexityAdjustment =
        costBreakdown.find(c => c.category === 'Complexity Adjustment')?.cost || 0;
      const conditionAdjustment =
        costBreakdown.find(c => c.category === 'Condition Adjustment')?.cost || 0;
      const regionalAdjustment =
        costBreakdown.find(c => c.category === 'Regional Adjustment')?.cost || 0;
    ...
    <div className="container mx-auto p-4">
      <Card className="w-full shadow-md border-[#29B7D3]/20">
            ...
            <TabsContent value="calculator">
              <div className="bg-[#e6eef2] p-4 rounded-lg mb-6 flex items-center text-sm">
                ...
                <p className="text-[#243E4D]">
                  Enter your building specifications to get an accurate cost estimate. All fields
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>
                                <InteractiveCostTooltip
                                  title={
                                    field.value && field.value.toLowerCase().includes('residential')
                                      ? costTooltipContent.buildingType.residential.title
                                      : field.value &&
                                          field.value.toLowerCase().includes('commercial')
                                        ? costTooltipContent.buildingType.commercial.title
                                        : field.value &&
                                            field.value.toLowerCase().includes('industrial')
                                          ? costTooltipContent.buildingType.industrial.title
                                          : field.value &&
                                              field.value.toLowerCase().includes('agricultural')
                              ...
                              <FormDescription>Select the type of building</FormDescription>
                              <FormMessage />
                            </FormItem>
                          )}
                        />
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>
                                <InteractiveCostTooltip
                                  title={
                                    field.value && field.value.toLowerCase().includes('economy')
                                      ? costTooltipContent.quality.economy.title
                                      : field.value &&
                                          field.value.toLowerCase().includes('standard')
                                        ? costTooltipContent.quality.standard.title
                                        : field.value &&
                                            field.value.toLowerCase().includes('custom')
                                          ? costTooltipContent.quality.custom.title
                                          : field.value &&
                                              field.value.toLowerCase().includes('luxury')
                          ...
                          render={({ field }) => (
                            <FormItem>
                              <FormLabel>
                                <InteractiveCostTooltip
                                  title={
                                    field.value && field.value.toLowerCase().includes('richland')
                                      ? costTooltipContent.region.richland.title
                                      : field.value &&
                                          field.value.toLowerCase().includes('kennewick')
                                        ? costTooltipContent.region.kennewick.title
                                        : field.value &&
                                            field.value.toLowerCase().includes('prosser')
                          ...
                          render={({ field }) => (
                            <FormItem className="bg-[#e6eef2] p-3 rounded-md">
                              <div className="flex justify-between items-center">
                                ...
                                <Badge
                                  variant="outline"
                                  className="bg-white text-[#243E4D] border-[#29B7D3]/30"
                                >
                                  {field.value}
                                </Badge>
                              </div>
                                      ...
                                      <TooltipContent>
                                        <div className="w-80">
                                          <p className="text-xs mb-2">
                                            ...
                                            Age affects depreciation due to wear and tear, outdated
                                            systems, and reduced remaining useful life.
                                          </p>
                                          <div className="bg-gray-100 p-2 rounded-md mb-2">
                                            <h5 className="text-xs font-medium mb-1">
                                              Depreciation Rates by Building Type:
                                            </h5>
                                            <ul className="text-xs space-y-1">
                                              <li className="flex items-center">
                                                <div className="w-2 h-2 rounded-full bg-red-500 mr-1"></div>
                                                <span>
                                                  Residential: 1.333% per year (min. value: 30%)
                                                </span>
                                              </li>
                                              <li className="flex items-center">
                                                <div className="w-2 h-2 rounded-full bg-amber-500 mr-1"></div>
                                                <span>
                                                  Commercial: 1% per year (min. value: 25%)
                                                </span>
                                              </li>
                                              <li className="flex items-center">
                                                <div className="w-2 h-2 rounded-full bg-green-500 mr-1"></div>
                                                <span>
                                                  Industrial: 0.889% per year (min. value: 20%)
                                                </span>
                                              </li>
                                            </ul>
                                          </div>
                                          <p className="text-xs italic">
                                            Higher depreciation means lower property value
                                            assessment and potentially lower taxes or insurance
                                            costs.
                                          </p>
                                        </div>
                                      </TooltipContent>
                              ...
                              {field.value > 0 ? (
                                <div className="mt-2">
                                  ...
                                  <FormDescription className="mt-1">
                                    Building has lost {deprecationPercentage}% of its value due to
                                    age
                                  </FormDescription>
                                </div>
                              ) : (
            ...
            <TabsContent value="materials">
              <div className="space-y-4">
                    ...
                    <UITooltip>
                      <TooltipTrigger asChild>
                        <Button
                          onClick={addMaterial}
                          size="sm"
                          className="flex items-center gap-1 bg-[#29B7D3] hover:bg-[#29B7D3]/90 text-white"
                        >
                ...
                <div className="bg-[#e8f8fb] p-4 rounded-lg mb-4 flex items-center text-sm">
                  ...
                  <p className="text-[#243E4D]">
                    Adding specific materials will provide a more accurate cost estimate. All
                    materials will be included in the final calculation.
                  </p>
                </div>
                  ...
                  <div className="text-center py-10 border rounded-md bg-[#e6eef2]">
                    <div className="flex flex-col items-center justify-center space-y-3">
                      ...
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        className="mt-2 border-[#29B7D3]/30 hover:bg-[#e8f8fb] hover:text-[#29B7D3]"
                      >
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gradient-to-r from-[#e6eef2] to-[#e8f8fb] p-6 rounded-lg border border-[#29B7D3]/20">
                  ...
                  <p className="text-center text-sm text-[#243E4D]/70">
                    Based on {form.getValues().squareFootage.toLocaleString()} sq ft{' '}
                    {regions.find(r => r.value === form.getValues().region)?.label}{' '}
                    {form.getValues().buildingType.toLowerCase()} building
                  </p>
                </div>
                    ...
                    <div className="bg-white border border-dashed p-4 rounded-lg">
                      <div className="flex justify-between items-center mb-3">
                        ...
                        <Badge
                          ...
                          style={{
                            borderColor: getDepreciationColor(
                              getDepreciationPercentage(
                                form.getValues().buildingAge,
                                form.getValues().buildingType
                              )
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
                        <div className="bg-gray-50 p-3 rounded border">
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            {form.getValues().buildingType === 'RESIDENTIAL'
                              ? '1.333% per year'
                              : form.getValues().buildingType === 'COMMERCIAL'
                                ? '1% per year'
                                : '0.889% per year'}
                          </div>
                        </div>
                        ...
                        <div className="bg-gray-50 p-3 rounded border">
                          ...
                          <div className="text-xl font-medium">
                            -$
                            {(
                              costBreakdown.find(c => c.category === 'Age Depreciation')?.cost || 0
                            ).toLocaleString()}
                          </div>
                          <div className="text-xs text-gray-500 mt-1">
                            {(
                              ((costBreakdown.find(c => c.category === 'Age Depreciation')?.cost ||
                                0) /
                                totalCost) *
                              100
                            ).toFixed(1)}
                      ...
                      <div className="mt-4 p-3 bg-[#f8f9fa] rounded-md border">
                        ...
                        <div className="text-sm space-y-2">
                          <div className="flex justify-between items-center border-b border-gray-200 pb-1">
                            ...
                            <span className="font-medium">
                              {
                                buildingTypes.find(t => t.value === form.getValues().buildingType)
                                  ?.label
                              }
                            </span>
                          </div>
                          <div className="flex justify-between items-center border-b border-gray-200 pb-1">
                            ...
                            <span className="font-medium">
                              {form.getValues().buildingType === 'RESIDENTIAL'
                                ...
                                : form.getValues().buildingType === 'COMMERCIAL'
                                  ? '1.000'
                                  : form.getValues().buildingType === 'INDUSTRIAL'
                                    ? '0.889'
                                    : '1.000'}
                              % per year
                            </span>
                          ...
                          <div className="flex justify-between items-center">
                            ...
                            <span className="font-medium">
                              Max(Minimum Value, 1.0 - (Age × Annual Rate))
                            </span>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 p-3 bg-[#e6eef2] rounded-md text-xs text-[#243E4D] flex items-start">
                        ...
                        <p>
                          Different building types depreciate at different rates based on Benton
                          County Building Cost Assessment standards. Residential buildings
                          depreciate faster (1.333% per year) than commercial (1.000% per year) and
                          industrial buildings (0.889% per year).
                        </p>
                      </div>
                      ...
                      <div className="mt-4 p-3 bg-gray-50 rounded-lg border">
                        <div className="flex justify-between items-center mb-2">
                          ...
                          <div className="text-xs text-gray-500">
                            Impact of building age on total cost
                          </div>
                        </div>
                      ...
                      <div className="flex items-start">
                        ...
                        <div>
                          <p className="text-[#243E4D] mb-1">
                            <span className="font-medium">Understanding Age Depreciation:</span>{' '}
                            Building age impacts value due to wear and tear, outdated systems, and
                            reduced remaining useful life.
                          </p>
                          <p className="text-[#243E4D]">
                            Different building types depreciate at different rates based on
                            construction materials, typical usage patterns, and industry standards.
                            The calculator applies appropriate depreciation rates for{' '}
                            {form.getValues().buildingType.toLowerCase()} buildings.
                          </p>
                        </div>
                      </div>
                    </div>
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie
                            ...
                            activeIndex={
                              hoveredCostItem
                                ? costBreakdown.findIndex(
                                    item => item.category === hoveredCostItem
                                  ) >= 0
                                  ? [
                                      costBreakdown.findIndex(
                                        item => item.category === hoveredCostItem
                          ...
                          >
                            {costBreakdown.map((entry, index) => (
                              <Cell
                                ...
                                style={{
                                  opacity:
                                    hoveredCostItem === entry.category
                                      ? 1
                                      : hoveredCostItem
                                        ? 0.5
                                        : 1,
                                  filter:
                                    hoveredCostItem === entry.category
                                      ? 'brightness(1.1) drop-shadow(0px 0px 5px rgba(0,0,0,0.2))'
                                      : 'none',
                                  transform:
                                    hoveredCostItem === entry.category ? 'scale(1.05)' : 'scale(1)',
                                }}
                                cursor="pointer"
                          ...
                          {costBreakdown.map((entry, index) => (
                            <Cell
                              ...
                              style={{
                                ...
                                filter:
                                  hoveredCostItem === entry.category
                                    ? 'brightness(1.2) drop-shadow(0px 0px 4px rgba(0,0,0,0.2))'
                                    : 'none',
                              }}
                              cursor="pointer"
                              onMouseEnter={() => setHoveredCostItem(entry.category)}
                ...
                <div className="mt-6 space-y-6">
                  <div className="bg-white p-6 border rounded-lg shadow-sm">
                    <div className="flex items-center mb-4">
                      ...
                      <h4 className="text-lg font-medium text-[#243E4D]">
                    ...
                    <div className="flex flex-col space-y-4">
                      {costBreakdown.map((item, index) => {
                        ...
                        const bgColor =
                          index % 3 === 0 ? '#e6eef2' : index % 3 === 1 ? '#e8f7e8' : '#e8f8fb';
                        ...
                        return (
                          <div key={item.category} className="group relative">
                              ...
                              <div className="flex items-center">
                                ...
                                <span className="font-medium text-sm">
                                  ...
                                  {isDepreciation && buildingAge > 0 && (
                                    <span
                                      className="ml-2 px-1.5 py-0.5 text-xs rounded text-white"
                                      style={{ backgroundColor: depreciationColor }}
                                    >
                            ...
                            <div
                              ...
                              style={{
                                borderColor: isDepreciation
                                  ? `${depreciationColor}50`
                                  : 'transparent',
                              }}
                              onMouseEnter={() => {
                            ...
                            >
                              ...
                              <div
                                ...
                                style={{
                                  width: width,
                                  backgroundColor:
                                    isDepreciation && buildingAge > 0 ? 'transparent' : barColor,
                                  backgroundImage:
                                    isDepreciation && buildingAge > 0
                                      ? `linear-gradient(to right, ${barColor}, ${barColor}99)`
                                      : 'none',
                                  boxShadow:
                                    hoveredCostItem === item.category
                                      ? '0 6px 12px rgba(0,0,0,0.2)'
                                      : '0 4px 6px rgba(0,0,0,0.1)',
                                  transform:
                                    hoveredCostItem === item.category ? 'scaleY(1.1)' : 'scaleY(1)',
                ...
                {timelineData.length > 0 && (
                  <div className="mt-8 bg-white p-6 border rounded-lg shadow-sm">
                    ...
                    <div className="bg-[#e8f8fb] p-4 rounded-lg mb-4 flex items-center text-sm">
                      <Info className="text-[#29B7D3] mr-2 h-4 w-4" />
                      <p className="text-[#243E4D]">
                        This chart shows how costs might be distributed over a 12-month project
                        timeline. The teal line shows projected costs, while the green bars show
                        actual costs with typical project variations.
                      </p>
                    </div>
                ...
                {treemapData.length > 0 && (
                  <div className="mt-8">
                    ...
                    <div className="bg-[#e8f8fb] p-4 rounded-lg mb-4 flex items-center text-sm">
                      <Info className="text-[#29B7D3] mr-2 h-4 w-4" />
                      <p className="text-[#243E4D]">
                        This interactive treemap visualization shows the hierarchical breakdown of
                        costs. Larger blocks represent higher costs. Hover over blocks to see
                        details.
                      </p>
                    </div>
                  ...
                  <div className="p-2 bg-white border rounded-lg mb-6">
                    <div className="flex items-center mb-2">
                      <p className="text-sm text-gray-600">
                        Watch how different factors impact your building cost estimate:
                      </p>
                    </div>
                    <CostImpactAnimation
                      ...
                      baseCost={
                        ...
                        getBaseCostPerSqFt(form.getValues().buildingType, form.getValues().quality)
                      }
                      ...
                      ageDepreciation={getDepreciationPercentage(
                        form.getValues().buildingAge,
                        form.getValues().buildingType
                      )}
                    ...
                    <p className="text-xs text-gray-500 mt-2 italic">
                      Click the play button to see how costs accumulate from base cost through each
                      adjustment factor.
                    </p>
                  </div>
            ...
            <TabsContent value="scenarios">
              <div className="space-y-6">
                <div className="bg-[#e6eef2] p-4 rounded-lg mb-6 flex items-center text-sm">
                  ...
                  <p className="text-[#243E4D]">
                    What-If Scenario Analysis allows you to create and compare different building
                    scenarios to make better decisions.
                  </p>
                </div>
                ...
                {scenarios.length === 0 ? (
                  ...
                  <div className="space-y-6">
                            ...
                            <TableBody>
                              {scenarios.map(scenario => (
                                <TableRow
                                  key={scenario.id}
                                  className={
                                    currentScenario?.id === scenario.id ? 'bg-[#e8f8fb]/30' : ''
                                  }
                                >
                                  <TableCell className="font-medium">
                                    {scenario.name}
                                    {currentScenario?.id === scenario.id && (
                                      <Badge className="ml-2 bg-[#29B7D3] text-white">
                                        Current
                                      </Badge>
                                    )}
                                  </TableCell>
                                  ...
                                  <TableCell>
                                    {
                                      regions.find(r => r.value === scenario.formValues.region)
                                        ?.label
                                    }
                                  </TableCell>
                                  <TableCell>
                          ...
                          {scenarioComparison.baseline && scenarioComparison.comparison ? (
                            <div>
                              <div className="flex justify-between p-3 bg-[#e6eef2] rounded-md mb-3">
                                <div>
                                  <h4 className="font-medium">
                                    {scenarioComparison.baseline.name}
                                  </h4>
                                  <p className="text-sm text-gray-600">
                                    ${scenarioComparison.baseline.totalCost.toLocaleString()}
                                  </p>
                                </div>
                                <div>
                                  <h4 className="font-medium text-right">
                                    {scenarioComparison.comparison.name}
                                  </h4>
                                  <p className="text-sm text-gray-600 text-right">
                                    ${scenarioComparison.comparison.totalCost.toLocaleString()}
                                  </p>
                                </div>
                              </div>
                              ...
                              {(() => {
                                ...
                                return (
                                  <div
                                    className={`p-3 rounded-md ${diff.isIncrease ? 'bg-red-50' : 'bg-green-50'}`}
                                  >
                                    <p className="font-medium flex items-center justify-center">
                                      <ArrowRightLeft
                                        className={`mr-2 h-4 w-4 ${diff.isIncrease ? 'text-red-500' : 'text-green-500'}`}
                                      />
                                      <span
                                        className={
                                          diff.isIncrease ? 'text-red-500' : 'text-green-500'
                                        }
                                      >
                                        {diff.isIncrease ? 'Increase' : 'Saving'} of $
                                        {Math.abs(diff.costDifference).toLocaleString()} (
                                        {Math.abs(diff.percentDifference).toFixed(1)}%)
                                      </span>
                                    </p>
                                  </div>
                                );
                              ...
                              <div className="mt-4">
                                ...
                                <div className="space-y-2 text-sm">
                                  {scenarioComparison.baseline.formValues.buildingType !==
                                    scenarioComparison.comparison.formValues.buildingType && (
                                        ...
                                        <span>
                                          {
                                            buildingTypes.find(
                                              t =>
                                                t.value ===
                                                scenarioComparison.comparison?.formValues
                                                  .buildingType
                                            )?.label
                                          }
                                        </span>
                                  ...
                                  {scenarioComparison.baseline.formValues.quality !==
                                    scenarioComparison.comparison.formValues.quality && (
                                    <div className="flex justify-between border-b pb-1">
                                      <span>Quality:</span>
                                      <div className="flex items-center">
                                        <span>
                                  ...
                                  {scenarioComparison.baseline.formValues.region !==
                                    scenarioComparison.comparison.formValues.region && (
                                    <div className="flex justify-between border-b pb-1">
                                      <span>Region:</span>
                                      <div className="flex items-center">
                                        <span>
                                  ...
                                  {scenarioComparison.baseline.formValues.squareFootage !==
                                    scenarioComparison.comparison.formValues.squareFootage && (
                                    <div className="flex justify-between border-b pb-1">
                                      <span>Square Footage:</span>
                                      <div className="flex items-center">
                                        <span>
                                          {scenarioComparison.baseline.formValues.squareFootage.toLocaleString()}
                                        </span>
                                        <ArrowRightLeft className="mx-2 h-3 w-3" />
                                        <span>
                                          {scenarioComparison.comparison.formValues.squareFootage.toLocaleString()}
                                        </span>
                                      </div>
                                    </div>
                                  )}
                                  {scenarioComparison.baseline.materials.length !==
                                    scenarioComparison.comparison.materials.length && (
                                    <div className="flex justify-between border-b pb-1">
                                      <span>Materials:</span>
                                      <div className="flex items-center">
                                        <span>
                                          {scenarioComparison.baseline.materials.length} items
                                        </span>
                                        <ArrowRightLeft className="mx-2 h-3 w-3" />
                                        <span>
                                          {scenarioComparison.comparison.materials.length} items
                                        </span>
                                      </div>
                                    </div>
                                  )}
                                        ...
                                        data={[
                                          {
                                            name: 'Base Cost',
                                            [scenarioComparison.baseline.name]:
                                              scenarioComparison.baseline.costBreakdown.find(
                                                c => c.category === 'Base Cost'
                                              )?.cost || 0,
                                            [scenarioComparison.comparison.name]:
                                              scenarioComparison.comparison.costBreakdown.find(
                                                c => c.category === 'Base Cost'
                                              )?.cost || 0,
                                          },
                                          {
                                            ...
                                            [scenarioComparison.baseline.name]:
                                              scenarioComparison.baseline.costBreakdown.find(
                                                c => c.category === 'Complexity Adjustment'
                                              )?.cost || 0,
                                            [scenarioComparison.comparison.name]:
                                              scenarioComparison.comparison.costBreakdown.find(
                                                c => c.category === 'Complexity Adjustment'
                                          ...
                                          },
                                          {
                                            ...
                                            [scenarioComparison.baseline.name]:
                                              scenarioComparison.baseline.costBreakdown.find(
                                                c => c.category === 'Condition Adjustment'
                                              )?.cost || 0,
                                            [scenarioComparison.comparison.name]:
                                              scenarioComparison.comparison.costBreakdown.find(
                                                c => c.category === 'Condition Adjustment'
                                          ...
                                          },
                                          {
                                            ...
                                            [scenarioComparison.baseline.name]:
                                              scenarioComparison.baseline.costBreakdown.find(
                                                c => c.category === 'Regional Adjustment'
                                              )?.cost || 0,
                                            [scenarioComparison.comparison.name]:
                                              scenarioComparison.comparison.costBreakdown.find(
                                                c => c.category === 'Regional Adjustment'
                                          ...
                                          },
                                          {
                                            name: 'Total',
                                            [scenarioComparison.baseline.name]:
                                              scenarioComparison.baseline.totalCost,
                                            [scenarioComparison.comparison.name]:
                                              scenarioComparison.comparison.totalCost,
                                          },
                                        ]}
                                        margin={{ top: 20, right: 30, left: 20, bottom: 5 }}
                                      >
                    ...
                    <div className="mt-8 p-4 border rounded-lg bg-white shadow-sm">
                      <h3 className="text-lg font-medium text-[#243E4D] mb-4">
                        Scenario Analytics Dashboard
                      </h3>

                      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                        <div className="bg-[#e6eef2] p-3 rounded-lg">
                          <h4 className="text-sm font-medium text-[#243E4D] mb-1">
                            Total Scenarios
                          </h4>
                        ...
                        <div className="bg-[#e8f8fb] p-3 rounded-lg">
                          ...
                          <p className="text-2xl font-bold text-[#29B7D3]">
                            $
                            {Math.round(
                              scenarios.reduce((sum, scenario) => sum + scenario.totalCost, 0) /
        ...
        <CardFooter className="flex justify-between border-t pt-4 bg-[#e6eef2]/40">
          <div>
            <p className="text-sm text-[#243E4D]/80">
              Regional Multiplier:{' '}
              <span className="font-medium">{regionalMultiplier.toFixed(2)}</span>
            </p>
          </div>
          <div className="text-right">
            <p className="text-sm font-medium text-[#243E4D]">
              Total Estimated Cost: <span className="font-bold">${totalCost.toLocaleString()}</span>
            </p>
          </div>
        </CardFooter>
      </Card>
