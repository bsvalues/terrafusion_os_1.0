...
import { Eye, FilePlus2, FileSpreadsheet, FileText, Printer } from 'lucide-react';
  ...
  const exportToPDF = async () => {
    animateExport('PDF', async () => {
      ...
      try {
        ...
        // Add TerraFusion branding header
        ...
        if (currentCalculation) {
          ...
          if (currentCalculation.materialCosts && currentCalculation.materialCosts.length > 0) {
            ...
            // Table rows
            pdf.setTextColor(0, 0, 0);
            currentCalculation.materialCosts.forEach((material: any, index: number) => {
              const isEvenRow = index % 2 === 0;
              if (isEvenRow) {
                pdf.setFillColor(245, 245, 245);
                pdf.rect(margin, yPosition - 4, pdfWidth - (margin * 2), 6, 'F');
        ...
        pdf.text(`© TerraFusion Government OS`, pdfWidth - 60, pdfHeight - 5);
  ...
  const exportToExcel = () => {
    animateExport('Excel', async () => {
      ...
      if (!exportData && currentCalculation) {
        ...
        const materialData = currentCalculation.materialCosts.map((mat: any) => ({
          ...
          <div class="calculation-summary">
            ...
            ${currentCalculation.materialCosts && currentCalculation.materialCosts.length > 0 ? `
              ...
              <table>
                ...
                <tbody>
                  ${currentCalculation.materialCosts.map((mat: any) => `
                    <tr>
