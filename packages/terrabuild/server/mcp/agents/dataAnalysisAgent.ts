...
// Local type definition for agent events
...
export class DataAnalysisAgent extends CustomAgentBase {
  ...
  public async shutdown(): Promise<boolean> {
    // Base class doesn't have shutdown method, so we handle it ourselves
    ...
    optimizations.forEach(optimization => {
      if (optimization.type === 'index') {
        ...
        if (match) {
          ...
          implementation += `    ${indexName}Idx: index('${tableName}_${indexName}_idx').on(${columns.map((c: string) => `table.${c.trim()}`).join(', ')}),
`;
        }
      }
    });
