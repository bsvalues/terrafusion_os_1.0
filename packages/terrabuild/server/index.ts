...
(async () => {
  ...
  try {
    ...
    // Use Replit Auth only when running on Replit (REPLIT_DOMAINS is set).
    // In local TerraFusion OS dev (packages/terrabuild), skip Replit Auth entirely.
    if (process.env.REPLIT_DOMAINS) {
      await setupAuth(app);
    } else {
      log('Local OS dev mode: Using county auth only (no Replit Auth)');
    }
    ...
    log('Auth error: Falling back to county auth only');
    setupCountyNetworkAuth(app);
  }
