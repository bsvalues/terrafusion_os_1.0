...
features.forEach(feature => {
  ...
  if (fs.existsSync(filePath)) {
    ...
    const hasClass =
      content.includes('class ' + feature.className) ||
      content.includes('class TerraFusionGIS') ||
      (feature.className === 'GISViewer' && content.includes('class TerraFusionGIS'));
    const hasWindowExport =
      content.includes('window.' + feature.className) || content.includes('window.launch');
    const hasShowMethod =
      content.includes('.show()') || content.includes('show:') || content.includes('function show');
    ...
    console.log(
      '   ' +
        (hasWindowExport ? '✅' : '❌') +
        ' Export: ' +
        (hasWindowExport ? 'Found' : 'Missing')
    );
    console.log(
      '   ' + (hasShowMethod ? '✅' : '❌') + ' Show: ' + (hasShowMethod ? 'Found' : 'Missing')
    );
