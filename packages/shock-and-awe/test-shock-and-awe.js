...
class ShockAndAweTestSuite {
  ...
  async testDemoCards() {
    ...
    const expectedDemoCards = ['costforge', 'ai-swarm', 'gis-transcended', 'hybrid-intelligence'];
  ...
  async testJavaScriptIntegration() {
    ...
    if (!htmlContent.includes('launchCostForgeWizard') || !htmlContent.includes('showAISwarmViz')) {
  ...
  async testCostForgeWizard() {
    const costforgeContent = fs.readFileSync(
      path.join(this.baseDir, 'js/costforge-wizard.js'),
      'utf8'
    );
  ...
  async testAISwarmVisualization() {
    const swarmContent = fs.readFileSync(path.join(this.baseDir, 'js/ai-swarm.js'), 'utf8');
  ...
  async testModuleManifest() {
    ...
    const requiredFields = ['name', 'version', 'description', 'capabilities', 'status'];
