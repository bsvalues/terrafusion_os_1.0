class VisualizationManager {
  ...
  createNetworkVisualization(visualization) {
    ...
    const simulation = d3
      .forceSimulation(visualization.data.nodes)
      .force(
        'link',
        d3
          .forceLink(visualization.data.links)
          .id((d) => d.id)
          .distance(visualization.options.linkDistance)
      )
  ...
  createHeatmapVisualization(visualization) {
    ...
    const g = svg
      ...
      .attr(
        'transform',
        `translate(${visualization.options.margin},${visualization.options.margin})`
      );
  ...
  createTimelineVisualization(visualization) {
    ...
    svg.append('g').attr('transform', `translate(${visualization.options.margin},0)`).call(yAxis);
