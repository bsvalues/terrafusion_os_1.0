...
The launcher-v3 at `/mnt/e/TerraFusion_Master_Workspace/launcher-v3/` is
supposed to be the central hub that:
...
But currently it has hardcoded URLs pointing to web apps (localhost:\${{TF_CONSCIOUSNESS_PORT:-3002}}, etc.)
instead of launching the Tauri apps!
...
Update the existing launcher-v3 to properly launch the Tauri desktop
applications:

```typescript
// Instead of:
url: 'http://localhost:\${{TF_CONSCIOUSNESS_PORT:-3002}}';
...
This way users have ONE launcher that opens all 14 TerraFusion desktop
applications!
