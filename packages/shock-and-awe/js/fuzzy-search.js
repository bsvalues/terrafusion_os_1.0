...
class FuzzyAddressSearch {
  ...
  search(input, maxResults = 8) {
    ...
    const results = this.propertyDatabase
      .map(property => ({
  ...
  getPropertyByAddress(address) {
    return this.propertyDatabase.find(
      property => property.address.toLowerCase() === address.toLowerCase()
    );
  }
...
class AddressAutocomplete {
  ...
  highlightMatch(text, query) {
    ...
    queryTerms.forEach(term => {
      ...
      highlighted = highlighted.replace(
        regex,
        '<span style="background: rgba(0, 255, 238, 0.3); color: #00ffee; font-weight: 600;">$1</span>'
      );
    });
  ...
  autoFillPropertyData(property) {
    ...
    // Map property type
    const propertyTypeSelect = form.querySelector(
      'select[name*="type" i], select[id*="type" i], #assessment-type, #property-type'
    );
        ...
        if (
          typeOptions.some(
            type =>
              ...
              option.value.toLowerCase().includes(type.toLowerCase())
          )
        ) {
    ...
    // Fill square footage
    const squareFootageInput = form.querySelector(
      'input[placeholder*="square" i], input[name*="square" i], input[id*="square" i], input[placeholder*="footage" i]'
    );
    ...
    const countySelect = form.querySelector(
      'select[name*="county" i], select[id*="county" i], #county-select'
    );
    ...
    const resultsArea =
      document.getElementById('demo-results') ||
      form.querySelector('.demo-results, .property-results, .results');
    if (resultsArea) {
