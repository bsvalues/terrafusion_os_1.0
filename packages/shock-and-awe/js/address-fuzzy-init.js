  ...
  addressInputs.forEach(input => {
    if (typeof AddressAutocomplete !== 'undefined') {
      try {
        ...
        addSearchIcon(input);
        console.log(
          `🔍 Fuzzy search enabled for dynamic input: ${input.id || input.name || input.placeholder}`
        );
      } catch (error) {
...
window.getFuzzySearchStats = function () {
  ...
  return {
    ...
    totalAddresses: window.FuzzyAddressSearch
      ? new window.FuzzyAddressSearch().addressDatabase.length
      : 0,
