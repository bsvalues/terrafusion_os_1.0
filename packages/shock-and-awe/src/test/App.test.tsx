...
vi.mock('@mui/material/Drawer', () => ({__esModule: true,
...
describe('App Component', () => {beforeEach(() => {
  ...
  it('displays system status correctly', async () => {render(<App />);

    await waitFor(() => {
      expect(screen.getByText('System Status: OPERATIONAL')).toBeInTheDocument();});
  });

  it('opens navigation drawer when menu is clicked', async () => {const user = userEvent.setup();
    render(<App />);
  ...
  it('displays all demo modules in grid', async () => {render(<App />);
  ...
  it('handles component loading states', async () => {render(<App />);

    // Should show loading initially
    expect(screen.getByText(/Loading revolutionary AI demonstrations/)).toBeInTheDocument();});

  it('displays correct difficulty levels', async () => {render(<App />);
  ...
  it('shows notification system', async () => {const user = userEvent.setup();
    render(<App />);
  ...
  it('handles error boundary gracefully', () => {// This test would check error boundary functionality
    // For now, we verify the app renders without throwing
    expect(() => render(<App />)).not.toThrow();});

  it('maintains correct theme configuration', async () => {render(<App />);

    // Check that dark theme is applied (background should be dark)
    const main = document.querySelector('body');
    expect(main).toBeTruthy();});

  it('displays correct agent count and metrics', async () => {render(<App />);
