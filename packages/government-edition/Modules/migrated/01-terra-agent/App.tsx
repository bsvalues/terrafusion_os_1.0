...
function App() {const [agents, setAgents] = useState<Agent[]>([]);
  ...
  const sendMessage = async () => {
    ...
    try {const response = await invoke<string>("process_query", { 
        query: inputValue});
  ...
  return (<div className="container"><><h1>TerraAgent Desktop</h1><p
</></>>Your AI-powered property analysis assistant</p>{/* Agent Status Section */}<div className="agent-list">{agents.map((agent) => (<div key={agent.id} className="agent-card"><><h3>{agent.name}</h3><span
</>className={`agent-status ${agent.status}`}>
              {agent.status}</span><><p>{agent.description}</p><small
</></>>Type: {agent.type}</small></div>))}</div>{/* Chat Interface */}<div className="chat-container"><div className="chat-messages">{messages.map((message) => (<div key={message.id} className={`message ${message.role}`}><><div>{message.content}</div><small
</></>>{message.timestamp.toLocaleTimeString()}</small></div>))}
