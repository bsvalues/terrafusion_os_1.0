...
class TerraFusionInsightEngine {
  ...
  async generateInsights(analysisType, data = {}) {
    ...
    console.log(`✅ Insight generated: ${insightId} (${insight.processingTime}ms, ${(insight.confidence * 100).toFixed(1)}% confidence)`);
  ...
  generateVisualizationConfig(analysisType) {
    const visualizations = {
      ...
      property_valuation_ai: ['valuation_scatter_plot', 'market_trend_line', 'comparable_properties_map'],
      government_performance: ['efficiency_radar_chart', 'department_performance_bars', 'citizen_satisfaction_gauge'],
      market_intelligence: ['economic_indicators_dashboard', 'competitive_matrix', 'market_forecast_timeline'],
      compliance_analytics: ['compliance_scorecard', 'risk_assessment_matrix', 'audit_readiness_gauge']
    };
  ...
  async generatePredictiveAnalysis(modelId, historicalData, forecastPeriods = 12) {
    ...
    console.log(`✅ Predictive analysis completed: ${predictionId} (${prediction.processingTime}ms, ${(prediction.accuracy * 100).toFixed(1)}% accuracy)`);
  ...
  getInsightStats() {
    return {
      ...
      averageAccuracy: this.insights.length > 0 ? 
        this.insights.reduce((sum, insight) => sum + insight.confidence, 0) / this.insights.length : 0,
...
async function testInsightEngine() {
  ...
  models.forEach(model => {
    console.log(`   ${model.name} - ${(model.accuracy * 100).toFixed(1)}% accuracy (${model.processingTime}s processing)`);
  });
  ...
  const testAnalyses = [
    ...
    { type: 'government_performance', data: { quarter: 'Q3', departments: ['planning', 'finance'] } },
    { type: 'market_intelligence', data: { sector: 'real_estate', timeframe: '12_months' } },
    { type: 'compliance_analytics', data: { scope: 'full_audit', regulations: ['data_privacy', 'financial'] } }
  ];
