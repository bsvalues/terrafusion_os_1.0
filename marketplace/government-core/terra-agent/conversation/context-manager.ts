...
export class ConversationContextManager {
  ...
  async trackContext(
    ...
    try {
      ...
      console.log(`✅ Context tracked: Topic=${conversation.currentTopic}, Phase=${conversation.conversationFlow.currentPhase}`);
      return conversation;
  ...
  async resolveReferences(
    ...
    try {
      ...
      resolvedQuery.resolvedReferences.push(...await this.resolvePronounalReferences(query, context));

      // Resolve contextual references
      resolvedQuery.resolvedReferences.push(...await this.resolveContextualReferences(query, context));

      // Resolve implicit references
      resolvedQuery.resolvedReferences.push(...await this.resolveImplicitReferences(query, context));
      ...
      resolvedQuery.confidence = this.calculateResolutionConfidence(resolvedQuery.resolvedReferences);

      // Determine if confirmation needed
      if (resolvedQuery.confidence < 0.8 || resolvedQuery.resolvedReferences.some(ref => ref.confidence < 0.7)) {
        resolvedQuery.requiresConfirmation = true;
        resolvedQuery.confirmationMessage = this.generateConfirmationMessage(resolvedQuery);
      }

      console.log(`✅ References resolved: ${resolvedQuery.resolvedReferences.length} refs, confidence: ${resolvedQuery.confidence}`);
  ...
  private async createNewConversation(userId: string, sessionId?: string): Promise<ConversationState> {
    const userProfile = this.globalUserProfiles.get(userId) || await this.createDefaultUserProfile(userId);
  ...
  private calculateFrameImportance(intent: AnalyticsIntent, entities: EntityExtractions): ContextFrame['importance'] {
    let score = 0;
  ...
  private async updateConversationFlow(conversation: ConversationState, intent: AnalyticsIntent): Promise<void> {
  ...
  private async resolveImplicitReferences(
    ...
    if (!query.match(/\b[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*[,\s]+[A-Z]{2}\b/) && 
  ...
  private findMostRecentReference(context: ConversationState, pronoun: string): any {
    ...
    const recentProperty = context.activeReferences.properties.sort((a, b) => 
    ...
    const recentLocation = context.activeReferences.locations.sort((a, b) => 
      b.lastMentioned.getTime() - a.lastMentioned.getTime()
    )[0];
  ...
  private generateConfirmationMessage(resolvedQuery: ResolvedQuery): string {
    ...
    if (uncertainRefs.length > 0) {
      const refList = uncertainRefs.map(ref => `"${ref.originalText}" → ${ref.resolvedTo}`).join(', ');
