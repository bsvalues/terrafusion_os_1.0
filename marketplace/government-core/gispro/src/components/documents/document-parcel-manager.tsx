...
export function DocumentParcelManager({ document, showLinkTypeOptions = false }: DocumentParcelManagerProps) {
  ...
  const linkParcelMutation = useMutation({
    mutationFn: async ({ parcelId, linkType, notes }: { parcelId: number, linkType?: string, notes?: string }) => {
      const response = await apiRequest(`/documents/${document.id}/parcels`, {
        method: 'POST',
        body: JSON.stringify({
          parcelId,
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['document-parcels', document.id] });
  ...
  const unlinkParcelMutation = useMutation({
    mutationFn: async (parcelId: number) => {
      const response = await apiRequest(`/documents/${document.id}/parcels/${parcelId}`, {
        method: 'DELETE'
      });
      return response.data;
    },
  ...
  const handleLinkParcel = (parcel: Parcel) => {
    if (showLinkTypeOptions) {
      setSelectedParcelToLink(parcel);
      setIsLinkOptionsDialogOpen(true);
      setIsSearchDialogOpen(false);
    } else {
      linkParcelMutation.mutate({ parcelId: parcel.id });
    }
  };

  const handleConfirmLink = () => {
    if (selectedParcelToLink) {
  ...
  const handleUnlinkParcel = () => {
    if (selectedParcelToUnlink) {
      unlinkParcelMutation.mutate(selectedParcelToUnlink.id);
    }
  };

  const handleEditLink = (parcel: ParcelWithLinkInfo) => {
  ...
  const getBadgeVariant = (linkType?: string) => {
    switch (linkType) {
      case 'ownership': return 'default';
      case 'legal_description': return 'secondary';
      case 'subdivision': return 'outline';
      case 'transaction': return 'destructive';
  ...
  const formatAddress = (parcel: Parcel) => {
    const parts = [];
  ...
  const availableParcels = searchResults.filter(
    (searchParcel: Parcel) => !linkedParcels.some((linkedParcel: ParcelWithLinkInfo) => linkedParcel.id === searchParcel.id)
  );
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              ...
              {linkedParcels.length > 0 && (
                <Badge variant="secondary">{linkedParcels.length}</Badge>
              )}
            </CardTitle>
            <Button 
          ...
          <CardDescription>
            Parcels that are associated with this document
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoadingLinkedParcels ? (
            <div className="flex items-center justify-center py-8">
              <div className="text-center">
                <div className="animate-spin h-8 w-8 border-4 border-primary border-t-transparent rounded-full mx-auto mb-2"></div>
                <p className="text-sm text-muted-foreground">Loading linked parcels...</p>
              </div>
            </div>
          ) : linkedParcels.length === 0 ? (
            <div className="text-center py-8">
              <Map className="h-12 w-12 mx-auto text-slate-300 dark:text-slate-600 mb-3" />
              <h3 className="text-lg font-medium mb-2">No parcels linked</h3>
              <p className="text-muted-foreground text-sm mb-4">
                This document is not currently linked to any parcels.
              </p>
              <Button 
                onClick={() => setIsSearchDialogOpen(true)}
                variant="outline"
            ...
            <Table>
              ...
              <TableBody>
                {linkedParcels.map((parcel: ParcelWithLinkInfo) => (
                  <TableRow key={parcel.id}>
                    <TableCell className="font-medium">
                      {formatParcelId(parcel)}
                    </TableCell>
                    <TableCell>
                    ...
                    {showLinkTypeOptions && (
                      <TableCell>
                        <Badge variant={getBadgeVariant(parcel.linkType)}>
                          {documentLinkTypes.find(type => type.value === parcel.linkType)?.label || 'Reference'}
                        </Badge>
                      </TableCell>
                    )}
                    <TableCell>
                      <div className="flex items-center gap-1">
                        ...
                        <AlertDialog>
                          ...
                          <AlertDialogContent>
                            <AlertDialogHeader>
                              <AlertDialogTitle>Unlink Parcel</AlertDialogTitle>
                              <AlertDialogDescription>
                                Are you sure you want to unlink {formatParcelId(parcel)} from this document? 
      ...
      <Dialog open={isSearchDialogOpen} onOpenChange={setIsSearchDialogOpen}>
        <DialogContent className="max-w-4xl max-h-[80vh] flex flex-col">
          <DialogHeader>
            <DialogTitle>Search and Link Parcels</DialogTitle>
            <DialogDescription>
              Search for parcels to link to this document
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="border rounded-lg flex-1 min-h-0 flex flex-col">
              {isSearching ? (
                <div className="flex items-center justify-center py-8">
                  <div className="text-center">
                    <div className="animate-spin h-8 w-8 border-4 border-primary border-t-transparent rounded-full mx-auto mb-2"></div>
                    <p className="text-sm text-muted-foreground">Searching parcels...</p>
                  </div>
                </div>
              ) : searchQuery && availableParcels.length === 0 ? (
                <div className="text-center py-8">
                ...
                <div className="flex-1 min-h-0">
                  <Table>
                    ...
                    <TableBody>
                      {availableParcels.map((parcel: Parcel) => (
                        <TableRow key={parcel.id}>
                          <TableCell className="font-medium">
                            {formatParcelId(parcel)}
                          </TableCell>
                          ...
                          <TableCell>
                            ...
                            >
                              {linkParcelMutation.isPending ? 'Linking...' : 'Link'}
                            </Button>
                          </TableCell>
                        </TableRow>
                      ))}
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-muted-foreground text-sm">
                    Enter a search term above to find parcels to link to this document.
                  </p>
                </div>
              )}
            </div>
        ...
        <Dialog open={isLinkOptionsDialogOpen} onOpenChange={setIsLinkOptionsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Configure how this parcel relates to the document
              </DialogDescription>
            </DialogHeader>
            
            {selectedParcelToLink && (
              <div className="space-y-4">
                <div>
                  <h4 className="font-medium mb-2">Selected Parcel</h4>
                  <div className="p-3 bg-muted rounded-lg">
                    <p className="font-medium">{formatParcelId(selectedParcelToLink)}</p>
                    <p className="text-sm text-muted-foreground">{formatAddress(selectedParcelToLink)}</p>
                  </div>
                </div>
            ...
            <DialogFooter>
              ...
              <Button 
                ...
                disabled={linkParcelMutation.isPending}
              >
                {linkParcelMutation.isPending ? 'Linking...' : 'Link Parcel'}
              </Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
      ...
      {showLinkTypeOptions && (
        <Dialog open={isEditLinkDialogOpen} onOpenChange={setIsEditLinkDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Edit Parcel Link</DialogTitle>
              <DialogDescription>
                Update the relationship details for this parcel link
              </DialogDescription>
            </DialogHeader>
            ...
            {selectedParcelToEdit && (
              <div className="space-y-4">
                <div>
                  <h4 className="font-medium mb-2">Parcel</h4>
                  <div className="p-3 bg-muted rounded-lg">
                    <p className="font-medium">{formatParcelId(selectedParcelToEdit)}</p>
                    <p className="text-sm text-muted-foreground">{formatAddress(selectedParcelToEdit)}</p>
                  </div>
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="edit-link-type">Relationship Type</Label>
                  <Select value={linkType} onValueChange={setLinkType}>
