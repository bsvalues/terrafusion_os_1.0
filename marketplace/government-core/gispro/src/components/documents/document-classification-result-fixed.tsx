...
const DocumentClassificationResult: React.FC<DocumentClassificationResultProps> = ({
  ...
  useEffect(() => {
    const timer = setTimeout(() => {
      setAnimatedConfidence(classification.confidence);
    }, 300);
  ...
  const getConfidenceLevel = (confidence: number) => {
    if (confidence >= 0.9) return { level: 'very-high', label: 'Very High', color: 'bg-emerald-500' };
  ...
  const getValidationStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'needs_review':
        return <Badge className="bg-yellow-100 text-yellow-800 border-yellow-200">Needs Review</Badge>;
      default:
  ...
  const renderMetrics = () => (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <div className="bg-green-50 rounded-lg p-3 border border-green-200">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-sm font-medium text-green-900">Reliability</p>
            <p className="text-xl font-bold text-green-700">
              {((classification.confidence + 0.1) * 0.9 * 100).toFixed(1)}%
            </p>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xl font-bold text-purple-700">
              {((classification.confidence + 0.05) * 0.95 * 100).toFixed(1)}%
            </p>
          </div>
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xl font-bold text-orange-700">
              {classification.processingTime ? formatProcessingTime(classification.processingTime) : 'N/A'}
            </p>
          </div>
  ...
  const renderEvidenceAnalysis = () => (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold text-gray-900">Evidence Analysis</h4>
        <Button
          variant="outline"
          size="sm"
        ...
        >
          ...
          {showAllEvidence ? <ChevronUp className="w-3 h-3 ml-1" /> : <ChevronDown className="w-3 h-3 ml-1" />}
        </Button>
      </div>
          ...
          .map((evidence, index) => (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <h5 className="text-sm font-medium text-gray-900">{evidence.feature}</h5>
                <div className="flex items-center gap-2">
                  <span className="text-xs text-gray-600">
                    Weight: {(evidence.importance * 100).toFixed(0)}%
                  </span>
                  <div 
                  ...
                  >
                    <div 
                      ...
                      style={{ width: `${evidence.importance * 100}%` }}
                    />
                  </div>
                </div>
              </div>
  ...
  const renderAlternativePredictions = () => (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold text-gray-900">Alternative Classifications</h4>
        <Button
          variant="outline"
          size="sm"
          ...
          className="text-xs"
        >
          {showAllAlternatives ? 'Show Less' : 'Show All'}
        </Button>
      </div>
  ...
  const renderValidationWorkflow = () => (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h4 className="text-sm font-semibold text-gray-900">Validation Workflow</h4>
        {classification.validationStatus && getValidationStatusBadge(classification.validationStatus)}
      </div>
      ...
      {classification.humanFeedback && (
        <div className="mt-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
          <h5 className="text-sm font-medium text-blue-900 mb-2">Human Feedback</h5>
          <div className="space-y-1 text-xs">
            <p><span className="font-medium">Correct:</span> {classification.humanFeedback.isCorrect ? 'Yes' : 'No'}</p>
            {classification.humanFeedback.suggestedType && (
              <p><span className="font-medium">Suggested Type:</span> {classification.humanFeedback.suggestedType}</p>
            )}
            {classification.humanFeedback.comments && (
              <p><span className="font-medium">Comments:</span> {classification.humanFeedback.comments}</p>
            )}
          </div>
        </div>
      )}
  ...
  const renderInteractiveFeedback = () => (
    <div className="space-y-4">
      <h4 className="text-sm font-semibold text-gray-900">Provide Feedback</h4>
  ...
  const renderConfidenceThresholds = () => (
    <div className="space-y-4">
      <h4 className="text-sm font-semibold text-gray-900">Confidence Thresholds</h4>
      
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-600">Current Threshold</span>
          <Badge className={classification.confidence >= confidenceThreshold ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}>
        ...
        <div className="flex items-center gap-2">
          <div className={`w-3 h-3 rounded-full ${classification.confidence >= confidenceThreshold ? 'bg-green-500' : 'bg-red-500'}`} />
    ...
    >
      <div className="p-6">
        <div className="flex items-start justify-between mb-6">
          <div>
            <h2 className="text-xl font-semibold text-gray-900 mb-2">Classification Results</h2>
            {document && (
              <div className="flex items-center gap-2 text-sm text-gray-600">
                <FileText className="w-4 h-4" />
                <span>{document.name}</span>
                {document.size && <span>• {(document.size / 1024).toFixed(1)} KB</span>}
              </div>
            )}
          </div>
          ...
          {showActions && (
            <div className="flex items-center gap-2">
              ...
              {onExportResult && (
                <Button variant="outline" size="sm" onClick={() => onExportResult(classification)}>
              ...
              {onReprocessDocument && (
                <Button variant="outline" size="sm" onClick={onReprocessDocument}>
        ...
        <div className="space-y-6">
          ...
          >
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">{classification.documentTypeLabel}</h3>
                  <p className="text-sm text-gray-600">Document Classification</p>
                </div>
              </div>
              
              <div className="text-right">
                <div className="flex items-center gap-2">
                  <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium ${confidenceInfo.color} text-white`}>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-sm">
                ...
                <span className="font-medium text-gray-900">{confidenceInfo.label}</span>
              </div>
              ...
              <div className="flex items-center gap-4 text-xs text-gray-500">
                ...
                {classification.modelVersion && (
                  <span className="flex items-center gap-1">
                    ...
                    Model v{classification.modelVersion}
                  </span>
                )}
              </div>
            </div>
          ...
          {variant === 'detailed' && (
            <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <TabsContent value="evidence" className="space-y-6">
                {classification.evidence && renderEvidenceAnalysis()}
              </TabsContent>
              
              <TabsContent value="alternatives" className="space-y-6">
                {classification.alternativeTypes && renderAlternativePredictions()}
              </TabsContent>
              
              <TabsContent value="validation" className="space-y-6">
                {renderValidationWorkflow()}
              </TabsContent>
              
              <TabsContent value="metadata" className="space-y-6">
                {showMetadata && classification.metadata && (
                  <div className="space-y-4">
                    <h4 className="text-sm font-semibold text-gray-900">Document Metadata</h4>
                    
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      {classification.metadata.pageCount && (
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          <span className="text-sm text-gray-600">Pages</span>
                          <span className="font-medium">{classification.metadata.pageCount}</span>
                        </div>
                      )}
                      
                      {classification.metadata.language && (
                        <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                          <span className="text-sm text-gray-600">Language</span>
                          <span className="font-medium">{classification.metadata.language}</span>
                        </div>
                      )}
                      ...
                      {classification.metadata.keywords && (
                        <div className="col-span-full">
                          ...
                          <div className="flex flex-wrap gap-2">
                            {classification.metadata.keywords.map((keyword, index) => (
          ...
          {variant === 'default' && (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {showEvidence && classification.evidence && (
                <div className="space-y-4">
                  <button
              ...
              {showAlternatives && classification.alternativeTypes && (
                <div className="space-y-4">
                  <button
          ...
          {classification.flags && classification.flags.length > 0 && (
            <motion.div
              ...
              className="space-y-3"
            >
              <h4 className="text-sm font-semibold text-gray-900">Flags & Warnings</h4>
              {classification.flags.map((flag, index) => (
                <div
                  key={index}
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-sm font-medium ${
                        flag.type === 'error' ? 'text-red-900' :
                        flag.type === 'warning' ? 'text-yellow-900' :
                        'text-blue-900'
                      }`}>
                    ...
                    <p className={`text-sm ${
                      flag.type === 'error' ? 'text-red-700' :
                      flag.type === 'warning' ? 'text-yellow-700' :
                      'text-blue-700'
                    }`}>
                      {flag.message}
                    </p>
