...
export function DocumentClassificationResult({
  ...
  const validationSummary = useMemo(() => {
    if (!classification.validationFlags) return null;
      ...
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="font-medium">{classification.documentTypeLabel}</span>
      ...
      >
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
              ...
              <Card>
                ...
                <CardContent className="space-y-6">
                  ...
                  {classification.tags && classification.tags.length > 0 && (
                    <div>
                      <h4 className="text-sm font-medium mb-2">Document Tags</h4>
                  ...
                  <div className="flex gap-2 pt-4 border-t">
                    {onAcceptClassification && (
                      <Button onClick={onAcceptClassification} size="sm">
                        <CircleCheck className="h-4 w-4 mr-1" />
                        Accept Classification
                      </Button>
                    )}
                    {onReclassify && (
                      <Button onClick={() => onReclassify(classification.documentTypeId)} variant="outline" size="sm">
                        <Edit3 className="h-4 w-4 mr-1" />
                        Reclassify
                      </Button>
                    )}
                    {onRejectClassification && (
                      <Button onClick={onRejectClassification} variant="outline" size="sm">
                        Reject
                      </Button>
                    )}
                  </div>
          ...
          <TabsContent value="alternatives" className="space-y-4 mt-6">
            {showAlternatives && classification.alternatives && classification.alternatives.length > 0 ? (
              <motion.div variants={itemVariants}>
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                    ...
                    <CardDescription>
                      Other possible document types based on analysis
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
              ...
              <Card>
                <CardContent className="flex items-center justify-center h-32 text-muted-foreground">
                  <div className="text-center">
                    ...
                    <p>No alternative classifications available</p>
                  </div>
                </CardContent>
              </Card>
            )}
              ...
              <motion.div variants={itemVariants}>
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>
                      Detailed analysis results and extracted information
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="space-y-6">
                    {/* Document Statistics */}
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {classification.metadata.pageCount && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          <div className="text-2xl font-bold text-primary">{classification.metadata.pageCount}</div>
                          <div className="text-xs text-muted-foreground">Pages</div>
                        </div>
                      )}
                      {classification.metadata.wordCount && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          <div className="text-2xl font-bold text-primary">{classification.metadata.wordCount.toLocaleString()}</div>
                          <div className="text-xs text-muted-foreground">Words</div>
                        </div>
                      )}
                      {classification.metadata.processingTime && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          <div className="text-2xl font-bold text-primary">{classification.metadata.processingTime}ms</div>
                          <div className="text-xs text-muted-foreground">Processing Time</div>
                        </div>
                      )}
                      {classification.metadata.readabilityScore && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          <div className="text-2xl font-bold text-primary">{classification.metadata.readabilityScore}</div>
                    ...
                    {/* Key Terms */}
                    {classification.metadata.keyTerms && classification.metadata.keyTerms.length > 0 && (
                      <div>
                        <h4 className="text-sm font-medium mb-2">Key Terms Identified</h4>
                        <div className="flex flex-wrap gap-1">
                          {classification.metadata.keyTerms.map((term, index) => (
                    ...
                    {classification.metadata.suggestedCategories && classification.metadata.suggestedCategories.length > 0 && (
                      <div>
                        <h4 className="text-sm font-medium mb-2">Suggested Categories</h4>
                        <div className="flex flex-wrap gap-1">
                          {classification.metadata.suggestedCategories.map((category, index) => (
                            <Badge key={index} variant="outline" className="text-xs">
                              {category}
                            </Badge>
                          ))}
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                      {classification.metadata.documentLanguage && (
                        <div>
                          <span className="font-medium">Language:</span> {classification.metadata.documentLanguage}
                        </div>
                      )}
                      {classification.metadata.modelVersion && (
                        <div>
                          <span className="font-medium">Model Version:</span> {classification.metadata.modelVersion}
                        </div>
                      )}
                    </div>
                  </CardContent>
          ...
          <TabsContent value="validation" className="space-y-4 mt-6">
            {showValidation && classification.validationFlags && classification.validationFlags.length > 0 ? (
              <motion.div variants={itemVariants}>
                <Card>
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2">
                    ...
                    <CardDescription>
                      Quality checks and validation feedback
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-3">
                      {classification.validationFlags.map((flag, index) => (
                        <motion.div
                          key={index}
                          className={cn(
                        ...
                        >
                          <div className="flex items-start gap-2">
                            ...
                            {flag.type === 'success' && <CircleCheck className="w-5 h-5 text-green-500 mt-0.5" />}
                            
                            <div className="flex-1">
                              <p className={cn(
                                "font-medium text-sm",
                                flag.type === 'error' ? 'text-red-900' :
                                flag.type === 'warning' ? 'text-amber-900' :
                                flag.type === 'info' ? 'text-blue-900' :
                                'text-green-900'
                              )}>
                                {flag.message}
                              </p>
        ...
        {showHistory && classification.classificationHistory && classification.classificationHistory.length > 0 && (
          <motion.div variants={itemVariants}>
            <Collapsible>
              ...
              <CollapsibleContent>
                <Card className="mt-2">
                  <CardContent className="pt-6">
                    <div className="space-y-3">
                      {classification.classificationHistory.map((entry, index) => (
