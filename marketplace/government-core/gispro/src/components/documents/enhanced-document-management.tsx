...
export const EnhancedDocumentManagement = ({ className }: EnhancedDocumentManagementProps) => {
  ...
  const { data: documents = [], isLoading } = useQuery({
    queryKey: ['documents', searchTerm, selectedType, selectedStatus],
    queryFn: async () => {
      // Simulate API call
  ...
  return (
    <div className={`space-y-6 ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage and organize your legal documents, surveys, and permits.
          </p>
        </div>
        ...
        <div className="flex gap-2">
          <IllustratedTooltip
            content={
              <div className="space-y-2">
                ...
                <p className="text-sm">Process multiple documents simultaneously with AI classification.</p>
              </div>
            }
            illustration={illustrations.analytics}
          >
            <Button onClick={() => setShowBatchProcessor(true)}>
          ...
          <IllustratedTooltip
            content={
              <div className="space-y-2">
                <p className="font-medium">Parcel Management</p>
                <p className="text-sm">Link documents to specific parcels for better organization.</p>
              </div>
            }
            illustration={illustrations.features}
          >
            <Button variant="outline" onClick={() => setShowParcelManager(true)}>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{documentStats.processing}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{documentStats.errors}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <div className="flex flex-col md:flex-row gap-4 items-center">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                <option value="legal_description">Legal Descriptions</option>
                    ...
                    <TableRow key={doc.id} className="cursor-pointer hover:bg-gray-50">
                      ...
                      <TableCell>
                        <div className="flex gap-1">
                          {doc.parcelIds?.slice(0, 2).map(parcelId => (
                            <Badge key={parcelId} variant="secondary" className="text-xs">
                              {parcelId}
                            </Badge>
                          ))}
                          {doc.parcelIds && doc.parcelIds.length > 2 && (
                            <Badge variant="secondary" className="text-xs">
                              +{doc.parcelIds.length - 2}
                            </Badge>
                          )}
                        </div>
                      </TableCell>
      ...
      <Dialog open={!!selectedDocument} onOpenChange={handleCloseDocumentDialog}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>{selectedDocument?.title}</DialogTitle>
            <DialogDescription>
              Document details and classification information
            </DialogDescription>
          </DialogHeader>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-3">
                    <div>
                      ...
                      <p className="text-sm">{getDocumentTypeLabel(selectedDocument.type)}</p>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        {formatDistanceToNow(new Date(selectedDocument.uploadedAt), { addSuffix: true })}
                      </p>
                    </div>
                    
                    <div>
                      <label className="text-sm font-medium text-gray-600">Last Modified By</label>
                      <p className="text-sm">{selectedDocument.lastModifiedBy}</p>
                    </div>
                  </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      ...
                      <div className="flex items-center gap-2">
                        {React.createElement(getStatusIcon(selectedDocument.status), { 
                          className: "h-4 w-4" 
                        })}
                    ...
                    <div>
                      <label className="text-sm font-medium text-gray-600">Associated Parcels</label>
                      <div className="flex gap-1 flex-wrap">
                        {selectedDocument.parcelIds?.map(parcelId => (
                          <Badge key={parcelId} variant="secondary">
                            {parcelId}
                          </Badge>
                        ))}
                      </div>
              ...
              <TabsContent value="classification">
                {selectedDocument.classification ? (
                  ...
                  <div className="text-center py-8 text-gray-500">
                    ...
                    <p>No classification data available for this document.</p>
                  </div>
                )}
              </TabsContent>
              
              <TabsContent value="version">
                <DocumentVersionControl documentId={selectedDocument.id} />
              </TabsContent>
            </Tabs>
          )}
        </DialogContent>
      ...
      <Dialog open={showBatchProcessor} onOpenChange={setShowBatchProcessor}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>Batch Document Processing</DialogTitle>
            <DialogDescription>
              Upload and process multiple documents with AI classification
            </DialogDescription>
          </DialogHeader>
          <BatchDocumentProcessor onClose={() => setShowBatchProcessor(false)} />
        </DialogContent>
      </Dialog>

      {/* Parcel Manager Dialog */}
      <Dialog open={showParcelManager} onOpenChange={setShowParcelManager}>
        <DialogContent className="max-w-4xl">
          <DialogHeader>
            <DialogTitle>Document-Parcel Management</DialogTitle>
            <DialogDescription>
              Link documents to specific parcels for better organization
            </DialogDescription>
          </DialogHeader>
          <DocumentParcelManager onClose={() => setShowParcelManager(false)} />
        </DialogContent>
      </Dialog>
    </div>
  );
