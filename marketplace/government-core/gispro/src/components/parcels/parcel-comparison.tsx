...
interface ParcelComparisonProps {
  referenceParcels: string[]; // IDs of parcels to compare against
  className?: string;
...
export function ParcelComparison({
  ...
  const [sortBy, setSortBy] = useState<'score' | 'price' | 'acreage' | 'distance' | 'year'>('score');
  ...
  const fetchComparisonData = async () => {
    ...
    try {
      ...
      const comparisonResponse = await fetch('/api/parcels/compare', {
        method: 'POST',
  ...
  const handleCriteriaChange = (key: keyof ComparisonCriteria, value: boolean) => {
    ...
    return (
      <Card className={className}>
        <CardContent className="text-center py-8">
          ...
          <h3 className="text-lg font-semibold mb-2">No Reference Parcels Selected</h3>
          <p className="text-muted-foreground">
            Select one or more parcels to begin comparison analysis
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <div className={cn('space-y-6', className)}>
      ...
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-bold">Parcel Comparison</h2>
          <p className="text-muted-foreground">
            Comparing against {referenceParcels.length} reference parcel{referenceParcels.length > 1 ? 's' : ''}
          </p>
        </div>
              ...
              <TooltipContent>
                <div className="space-y-1">
                  ...
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    onClick={() => onExportComparison?.('pdf')}
                  >
                  ...
                  <Button 
                    variant="ghost" 
                    size="sm" 
                    onClick={() => onExportComparison?.('excel')}
                  >
                    <Download className="h-3 w-3 mr-1" />
                    Export Excel
                  </Button>
              ...
              <CardContent className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <div className="text-sm text-muted-foreground">
                      ±{filters.priceVariance}%
                    </div>
                  </div>

                  {/* Acreage variance */}
                  <div className="space-y-2">
                    <Label>Acreage Variance (%)</Label>
                    <Slider
                    ...
                    />
                    <div className="text-sm text-muted-foreground">
                      ±{filters.acreageVariance}%
                    </div>
                  </div>

                  {/* Year built range */}
                  <div className="space-y-2">
                    <Label>Year Built Range (years)</Label>
                    <Slider
                  ...
                  <div className="space-y-2">
                    ...
                    />
                    <div className="text-sm text-muted-foreground">
                      {(filters.minConfidenceScore * 100).toFixed(0)}%
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center space-x-2">
                      <Switch
                        ...
                        onCheckedChange={(checked) => handleFilterChange('includeUnverified', checked)}
                      />
                      <Label htmlFor="include-unverified">Include Unverified Data</Label>
                    </div>
                  </div>
                </div>
                  ...
                  {Object.entries(criteria).map(([key, value]) => (
                    <div key={key} className="flex items-center space-x-2">
                      <Switch
                        id={key}
                        checked={value}
                        onCheckedChange={(checked) => handleCriteriaChange(key as keyof ComparisonCriteria, checked)}
                      />
      ...
      {referenceData.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-lg">Reference Parcels</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {referenceData.map((parcel) => (
                <div key={parcel.id} className="p-3 border rounded-lg bg-muted/30">
                  ...
                  <div className="grid grid-cols-2 gap-2 mt-2 text-xs">
                    <div>
                      ...
                      <p className="font-medium truncate">{parcel.owner}</p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Value:</span>
                      <p className="font-medium">{formatCompactCurrency(parcel.taxValue)}</p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Acreage:</span>
                      <p className="font-medium">{parcel.acreage.toFixed(2)} ac</p>
                    </div>
                    <div>
                      <span className="text-muted-foreground">Type:</span>
                      <p className="font-medium">{parcel.propertyType}</p>
                    </div>
                  </div>
                </div>
              ))}
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">
            Comparison Results ({sortedData.length} parcels)
          </CardTitle>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8">
              <Search className="h-12 w-12 mx-auto text-muted-foreground mb-4" />
              <h3 className="text-lg font-semibold mb-2">No Comparison Results</h3>
              <p className="text-muted-foreground">
                No parcels match the current criteria and filters
              </p>
              <Button variant="outline" className="mt-4" onClick={() => setShowFilters(true)}>
                Adjust Filters
              </Button>
            </div>
          ) : (
            <div className="space-y-4">
                      ...
                      <CardHeader className="pb-3">
                        <div className="flex items-start justify-between">
                          <div className="flex-1 min-w-0">
                            <CardTitle className="text-base truncate">{parcel.parcelNumber}</CardTitle>
                            {parcel.address && (
                              <CardDescription className="truncate">{parcel.address}</CardDescription>
                            )}
                          </div>
                          {parcel.score && (
                            <Badge variant="outline" className={getScoreColor(parcel.score)}>
                              {(parcel.score * 100).toFixed(0)}% match
                            </Badge>
                          )}
                        </div>
                      </CardHeader>
                        ...
                        <div className="grid grid-cols-2 gap-2 text-sm">
                          ...
                          {parcel.distance && (
                            <div className="col-span-2">
                              ...
                              <p className="font-medium">{parcel.distance.toFixed(1)} miles</p>
                            </div>
                          )}
                        </div>
              ...
              {currentView === 'list' && (
                <div className="space-y-2">
                  {sortedData.map((parcel) => (
                    <div key={parcel.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/50 transition-colors">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3">
                          <div className="flex-1 min-w-0">
                            <h4 className="font-semibold truncate">{parcel.parcelNumber}</h4>
                            {parcel.address && (
                              <p className="text-sm text-muted-foreground truncate">{parcel.address}</p>
                            )}
                          </div>
                          {parcel.score && (
                            <Badge variant="outline" className={getScoreColor(parcel.score)}>
                              {(parcel.score * 100).toFixed(0)}%
                            </Badge>
                          )}
                        </div>
                        <div className="flex items-center gap-6 mt-2 text-sm">
                          <span>
                            <span className="text-muted-foreground">Owner:</span> {parcel.owner}
                          </span>
                          <span>
                            <span className="text-muted-foreground">Value:</span> {formatCompactCurrency(parcel.taxValue)}
                          </span>
                          <span>
                            <span className="text-muted-foreground">Acreage:</span> {parcel.acreage.toFixed(2)} ac
                          </span>
                          {parcel.distance && (
                            <span>
                              <span className="text-muted-foreground">Distance:</span> {parcel.distance.toFixed(1)} mi
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  <table className="w-full border-collapse">
                    <thead>
                      <tr className="border-b">
                        <th className="text-left p-2">Parcel #</th>
                        <th className="text-left p-2">Owner</th>
                        <th className="text-left p-2">Type</th>
                        <th className="text-right p-2">Tax Value</th>
                        <th className="text-right p-2">Acreage</th>
                    ...
                    <tbody>
                      {sortedData.map((parcel) => (
                        <tr key={parcel.id} className="border-b hover:bg-muted/50">
                          ...
                          <td className="p-2 truncate max-w-32">{parcel.owner}</td>
                          <td className="p-2">{parcel.propertyType}</td>
                          <td className="p-2 text-right font-medium text-green-600">
                            {formatCompactCurrency(parcel.taxValue)}
                          </td>
                          <td className="p-2 text-right">{parcel.acreage.toFixed(2)} ac</td>
                          <td className="p-2 text-right">
                            {parcel.distance ? `${parcel.distance.toFixed(1)} mi` : '-'}
                          </td>
