...
export default function ReportViewer() {
  ...
  const sampleReport: ReportData = {
    ...
    description: 'Comprehensive financial performance analysis for Q4 2024 including revenue trends, expense analysis, and strategic recommendations.',
    type: 'pdf',
      ...
      sections: [
        {
          ...
          type: 'text',
          content: 'This report provides a comprehensive analysis of our financial performance during Q4 2024. Key highlights include revenue growth of 15%, improved profit margins, and strategic market expansion initiatives. The analysis covers revenue streams, operational efficiency metrics, and future growth projections.',
          pageNumber: 1,
        ...
        },
        {
          ...
          content: {
            ...
            caption: 'Key financial performance indicators comparison'
          } as TableData,
          pageNumber: 3,
  ...
  const handleSearch = (term: string) => {
    ...
    currentReport.content.sections.forEach(section => {
      ...
      if (matches > 0) {
        ...
        if (existingPage) {
          existingPage.matches += matches;
        } else {
          results.push({ page: section.pageNumber, matches });
        }
      }
    });
  ...
  const handleAddAnnotation = (type: Annotation['type'], content: string, position: { x: number; y: number }) => {
  ...
  const navigateToPage = (page: number) => {
    if (currentReport && page >= 1 && page <= currentReport.pages) {
      setCurrentPage(page);
    }
  };
  ...
  return (
    ...
    >
      ...
      {!isFullscreen && (
        ...
        >
          <div className="text-center space-y-2">
            ...
            <p className="text-muted-foreground">
              Interactive document viewer with annotations and collaboration tools
            </p>
          ...
          <CardContent className="py-3">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center space-x-2">
                <h2 className="font-semibold text-lg truncate max-w-xs">
                  {currentReport.title}
                </h2>
                <Badge variant="outline">{currentReport.type.toUpperCase()}</Badge>
                <Badge variant="outline" className="flex items-center gap-1">
                  <FileText className="h-3 w-3" />
                  {currentReport.pages} pages
                </Badge>
              </div>
      ...
      {/* Main Content */}
      <div className={`${isFullscreen ? 'container mx-auto' : 'container mx-auto'} pb-8`}>
        <div className="grid grid-cols-12 gap-6">
          ...
          >
            <Tabs defaultValue="toc" className="w-full">
              ...
              <TabsContent value="toc" className="space-y-2">
                <Card>
              ...
              <TabsContent value="search" className="space-y-2">
                <Card>
                  <CardHeader className="pb-3">
                    <CardTitle className="text-sm">Search Report</CardTitle>
                  </CardHeader>
                  <CardContent>
                        ...
                        {annotations.map((annotation) => (
                          ...
                          >
                            ...
                            <div className="text-xs text-muted-foreground mb-1">
                              Page {annotation.pageNumber} • {annotation.createdBy}
                            </div>
              ...
              <CardContent className="p-6">
                <div
                  ...
                  style={{
                    ...
                    width: viewerSettings.fitToWidth ? '100%' : `${800 * (viewerSettings.zoom / 100)}px`
                  }}
                >
                  {/* Document Content */}
                  <div className="p-8 min-h-full bg-white relative">
                    {/* Page Header */}
                    <div className="text-center mb-8 border-b pb-4">
                      <h1 className="text-2xl font-bold mb-2">
                        {currentReport.content.metadata.title}
                      </h1>
                      <p className="text-muted-foreground">
                        Page {currentPage} of {currentReport.pages}
                      </p>
                    </div>

                    {/* Content Sections for Current Page */}
                        ...
                        >
                          ...
                          {section.type === 'table' && (
                            <div className="overflow-auto">
                              <table className="w-full border-collapse border border-gray-300">
                                <thead>
                                  <tr className="bg-gray-50">
                                    {(section.content as TableData).headers.map((header, index) => (
                                      <th
                                        key={index}
                                ...
                                <tbody>
                                  {(section.content as TableData).rows.map((row, rowIndex) => (
                                    <tr key={rowIndex}>
                              ...
                              {(section.content as TableData).caption && (
                          ...
                          {section.type === 'chart' && (
                            <div className="bg-gray-50 p-4 rounded border">
                              <h3 className="font-medium mb-3">
                                {(section.content as ChartData).title}
                              </h3>
                              <div className="space-y-2">
                                {(section.content as ChartData).data.map((item, index) => (
                    ...
                    {/* Annotations Overlay */}
                    {viewerSettings.showAnnotations && getCurrentPageAnnotations().map((annotation) => (
                      <div
                        key={annotation.id}
                      ...
                      >
                        {annotation.type !== 'highlight' && (
                          <div className="text-xs font-medium">
                            {getAnnotationIcon(annotation.type)} {annotation.content}
                          </div>
                        )}
                      </div>
                    ))}
      ...
      {!isFullscreen && (
        ...
        >
          <Alert className="border-purple-200 bg-purple-50">
            ...
            <AlertDescription>
              <strong>Advanced Document Viewer:</strong> Interactive report viewing with zoom controls, 
              rotation, full-screen mode, search functionality, annotation system, table of contents 
              navigation, and collaborative commenting features.
            </AlertDescription>
          </Alert>
        </motion.div>
      )}
