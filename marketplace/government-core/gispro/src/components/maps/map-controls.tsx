...
export function MapControls({ position = 'topleft', className }: MapControlsProps) {
    ...
    const handleMapMouseMove = (e: L.LeafletMouseEvent) => {
      ...
      switch (activeTool) {
        ...
        case 'rectangle':
          ...
          const tempRect = L.rectangle(rectBounds, {
            color: '#ff0000',
            weight: 2,
            opacity: 0.7,
            fillOpacity: 0.2,
        ...
        case 'circle':
          ...
          const tempCircle = L.circle(measureStartPointRef.current, {
            radius: radius,
            color: '#ff0000',
            weight: 2,
            opacity: 0.7,
  ...
  const handleDistanceMeasurement = (point: L.LatLng) => {
    ...
    if (!measureStartPointRef.current) {
      ...
      // Subsequent clicks - add to measurement
      ...
      for (let i = 1; i < measurePointsRef.current.length; i++) {
        totalDistance += measurePointsRef.current[i - 1].distanceTo(measurePointsRef.current[i]);
      }
      ...
      const distanceText = segmentDistance > 1000 
  ...
  const handleRectangleMeasurement = (point: L.LatLng) => {
    if (!measureLayerRef.current) return;
    ...
    if (!measureStartPointRef.current) {
      ...
      const areaText = area > 10000 
        ? `${squareMetersToAcres(area).toFixed(2)} acres`
        : `${area.toFixed(1)} m²`;
  ...
  const handleCircleMeasurement = (point: L.LatLng) => {
    if (!measureLayerRef.current) return;
  ...
  // Control component
  useEffect(() => {
    // Add CSS for measurement labels
  ...
  return (
    <div className={cn(
      "flex flex-col gap-1 bg-white rounded-lg shadow-lg border p-1",
      className
    )}>
      <Button
      ...
      {measurementInfo && (
        <div className="mt-2 p-2 bg-gray-50 rounded text-xs space-y-1 min-w-32">
          {measurementInfo.distance && (
            <div>
              <strong>Distance:</strong><br />
              {measurementInfo.distance > 1000 
                ? `${(measurementInfo.distance / 1000).toFixed(2)} km`
                : `${measurementInfo.distance.toFixed(1)} m`}
            </div>
          )}
          {measurementInfo.area && (
            <div>
              <strong>Area:</strong><br />
              {measurementInfo.area > 10000 
                ? `${squareMetersToAcres(measurementInfo.area).toFixed(2)} acres`
                : `${measurementInfo.area.toFixed(1)} m²`}
              <br />
              <span className="text-gray-600">
                {squareMetersToSquareFeet(measurementInfo.area).toLocaleString()} ft²
              </span>
            </div>
          )}
        </div>
      ...
      {activeTool && (
        <div className="mt-1 p-2 bg-blue-50 rounded text-xs text-blue-700">
          ...
          {activeTool === 'distance' && 'Click to add points'}
          {activeTool === 'rectangle' && 'Click two corners'}
          {activeTool === 'circle' && 'Click center, then radius'}
        </div>
      )}
    </div>
  );
