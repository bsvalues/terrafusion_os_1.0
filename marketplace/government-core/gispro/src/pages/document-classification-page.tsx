...
export default function DocumentClassificationPage() {
  ...
  const generateMockClassification = (document: DocumentFile): DocumentClassification => {
    const categories = documentCategories;
    const category = categories[Math.floor(Math.random() * categories.length)];
    const confidence = Math.floor(Math.random() * 30) + 70; // 70-100%
    
    const subcategories: { [key: string]: string[] } = {
      'Legal Documents': ['Contract', 'Agreement', 'License', 'Permit'],
...
The actual document processing would extract real text content, identify key information, and provide accurate classification results based on the document type and content.`;
  ...
  const filteredDocuments = documents.filter(doc => {
    ...
    if (filters.searchTerm && !doc.name.toLowerCase().includes(filters.searchTerm.toLowerCase())) return false;
    return true;
  });
  ...
  // Reprocess document
  const reprocessDocument = (document: DocumentFile) => {
    const resetDoc = {
      ...document,
    ...
    setDocuments(prev => 
      prev.map(doc => doc.id === document.id ? resetDoc : doc)
    );
  ...
  const getStatusIcon = (status: DocumentFile['status']) => {
    switch (status) {
      ...
      default: return <FileText className="h-4 w-4 text-yellow-600" />;
    }
  };
    ...
    <div className="container mx-auto p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">AI-powered document analysis and categorization</p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="outline">{documents.length} Documents</Badge>
          <Badge variant="outline">{stats.processedDocuments} Processed</Badge>
        </div>
      </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-2xl font-bold">{stats.processingTime.toFixed(1)}s</p>
              </div>
      ...
      <Tabs value={selectedTab} onValueChange={setSelectedTab}>
            ...
            <CardContent>
              ...
              >
                ...
                <div className="text-sm text-muted-foreground">
                  <p>Supported formats: PDF, DOC, DOCX, TXT, JPG, PNG, TIFF</p>
              ...
              {documents.length > 0 && (
                <div className="mt-6">
                  <div className="flex items-center justify-between mb-4">
                    <h4 className="text-lg font-medium">Recent Uploads</h4>
                    <Button onClick={clearAllDocuments} variant="outline" size="sm">
                ...
                {filteredDocuments.map(doc => (
                  <div
                    ...
                    className={`p-4 border rounded-lg cursor-pointer transition-colors ${
                      selectedDocument?.id === doc.id ? 'border-blue-500 bg-blue-50' : 'hover:border-gray-300'
                    }`}
                    onClick={() => {
                          ...
                          <div className="flex items-center gap-4 mt-1 text-sm text-muted-foreground">
                            ...
                            {doc.classification && (
                              <>
                                ...
                                <span>{doc.classification.confidence}% confidence</span>
                              </>
                            )}
                          </div>
                        </div>
        ...
        <TabsContent value="details" className="space-y-6">
          {selectedDocument ? (
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm">{selectedDocument.uploadedAt.toLocaleString()}</p>
                  </div>
                  <div>
                    <Label>Status</Label>
                    <div className="flex items-center gap-2">
                  ...
                  {selectedDocument.status === 'processing' && (
                    <div>
                      ...
                      <Progress value={selectedDocument.progress} className="mt-2" />
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div>
                      ...
                      <p className="text-sm font-medium">{selectedDocument.classification.category}</p>
                    </div>
                    <div>
                      <Label>Subcategory</Label>
                      <p className="text-sm">{selectedDocument.classification.subcategory}</p>
                    </div>
                    <div>
                      <Label>Confidence Score</Label>
                      <div className="flex items-center gap-2">
                        <Progress value={selectedDocument.classification.confidence} className="flex-1" />
                        <span className="text-sm font-medium">{selectedDocument.classification.confidence}%</span>
                      </div>
                    </div>
                    <div>
                      <Label>Tags</Label>
                    ...
                    <div>
                      ...
                      <div className="text-sm space-y-1">
                        {selectedDocument.classification.metadata.pageCount && (
                          <p>Pages: {selectedDocument.classification.metadata.pageCount}</p>
                        )}
                        {selectedDocument.classification.metadata.wordCount && (
                          <p>Words: {selectedDocument.classification.metadata.wordCount}</p>
                        )}
                        {selectedDocument.classification.metadata.language && (
                          <p>Language: {selectedDocument.classification.metadata.language}</p>
                        )}
                      </div>
                    </div>
                  </CardContent>
              ...
              {selectedDocument.classification?.suggestedActions && (
                <Card className="lg:col-span-2">
                  <CardHeader>
                  ...
                  <CardContent>
                    <ul className="space-y-2">
                      {selectedDocument.classification.suggestedActions.map((action, index) => (
            ...
            <Card>
              <CardContent className="p-8 text-center">
                ...
                <p className="text-muted-foreground">
                  Select a document from the Documents tab to view its details and classification results.
                </p>
              </CardContent>
            </Card>
          )}
        ...
        <TabsContent value="analytics" className="space-y-6">
                      ...
                      <div className="flex items-center gap-2">
                        <div className="w-20 bg-gray-200 rounded-full h-2">
                          <div 
                            ...
                            style={{ width: `${(count / stats.processedDocuments) * 100}%` }}
                          />
                        </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">{stats.processedDocuments}</span>
                </div>
                <div className="flex justify-between">
                  <span>Average Confidence</span>
                  <span className="font-medium">{Math.round(stats.averageConfidence)}%</span>
                </div>
                <div className="flex justify-between">
                  <span>Total Processing Time</span>
                  <span className="font-medium">{stats.processingTime.toFixed(1)}s</span>
                </div>
                <div className="flex justify-between">
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-3">
                {documents.slice(-10).reverse().map(doc => (
                  <div key={doc.id} className="flex items-center gap-3 py-2 border-b last:border-0">
                    {getStatusIcon(doc.status)}
                    <div className="flex-1 min-w-0">
                      <p className="text-sm font-medium truncate">{doc.name}</p>
                      <p className="text-xs text-muted-foreground">
                        {doc.status === 'completed' ? 'Classified as ' + doc.classification?.category : 
                         doc.status === 'processing' ? 'Processing...' :
                         doc.status === 'failed' ? 'Failed to process' : 'Waiting to process'}
                      </p>
                    </div>
