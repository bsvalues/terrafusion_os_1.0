...
namespace TerraFusion.API.Controllers
{
    ...
    public class AISwarmController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetSwarmStatus()
        {
            try
            {
                ...
                var claudePort = Environment.GetEnvironmentVariable("TF_CLAUDE_PORT") ?? "8002";
                var claudeResponse = await httpClient.GetAsync($"http://localhost:{claudePort}/api/claude/status");
                var gaugeResponse = await httpClient.GetAsync($"http://localhost:{claudePort}/api/swarm/status");
        ...
        public async Task<IActionResult> OptimizeCounty([FromBody] CountyOptimizationRequest request)
        {
            try
            {
                ...
                var claudePort = Environment.GetEnvironmentVariable("TF_CLAUDE_PORT") ?? "8002";
                var response = await httpClient.PostAsync($"http://localhost:{claudePort}/api/swarm/optimize", content);
        ...
        public async Task<IActionResult> ExecuteWorkflow([FromBody] AISwarmWorkflowExecutionRequest request)
        {
            try
            {
                ...
                var claudePort = Environment.GetEnvironmentVariable("TF_CLAUDE_PORT") ?? "8002";
                var response = await httpClient.PostAsync($"http://localhost:{claudePort}/api/claude/execute", content);
        ...
        public async Task<IActionResult> GetPerformanceMetrics()
        {
            try
            {
                ...
                using var httpClient = new HttpClient();
                var claudePort = Environment.GetEnvironmentVariable("TF_CLAUDE_PORT") ?? "8002";
                var gaugePerformanceTask = httpClient.GetAsync($"http://localhost:{claudePort}/api/swarm/performance");
                var claudeAgentsTask = httpClient.GetAsync($"http://localhost:{claudePort}/api/claude/agents");
        ...
        public async Task<IActionResult> GetAvailableWorkflows()
        {
            try
            {
                ...
                using var httpClient = new HttpClient();
                var claudePort = Environment.GetEnvironmentVariable("TF_CLAUDE_PORT") ?? "8002";
                var response = await httpClient.GetAsync($"http://localhost:{claudePort}/api/claude/workflows");
    ...
    /// </summary>
    public class AISwarmWorkflowExecutionRequest
    {
        [Required]
