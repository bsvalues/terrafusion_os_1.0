...
public class AIModuleOrchestrator : IAIModuleOrchestrator
{
    ...
    {
        {
            ...
            new AIModuleConfig
            {
                ...
                BaseUrl = $"http://localhost:{Environment.GetEnvironmentVariable("TF_API_PORT") ?? 
                    throw new InvalidOperationException("❌ ANTI-HARDCODING: TF_API_PORT environment variable must be set")}/api/ai-command-brain",
                Status = "active",
                Agents = int.Parse(Environment.GetEnvironmentVariable("TF_AI_AGENT_COUNT") ?? 
                    throw new InvalidOperationException("❌ ANTI-HARDCODING: TF_AI_AGENT_COUNT environment variable must be set")),
                MCPTools = int.Parse(Environment.GetEnvironmentVariable("TF_MCP_TOOLS_COUNT") ?? 
                    throw new InvalidOperationException("❌ ANTI-HARDCODING: TF_MCP_TOOLS_COUNT environment variable must be set")),
                Description = "Supreme Commander + Field Generals + operational agents coordination",
                Priority = 1
            }
        },
        {
            ...
            new AIModuleConfig
            {
                ...
                BaseUrl = $"http://localhost:{Environment.GetEnvironmentVariable("TF_API_PORT") ?? 
                    throw new InvalidOperationException("❌ ANTI-HARDCODING: TF_API_PORT environment variable must be set")}/api/ai-swarm",
                Status = "active",
                Agents = int.Parse(Environment.GetEnvironmentVariable("TF_AI_AGENT_COUNT") ?? 
                    throw new InvalidOperationException("❌ ANTI-HARDCODING: TF_AI_AGENT_COUNT environment variable must be set")),
                Description = "Swarm coordination managing concurrent agents",
                Priority = 2
            }
        },
        {
            ...
            new AIModuleConfig
            {
                ...
                BaseUrl = $"http://localhost:{Environment.GetEnvironmentVariable("TF_API_PORT") ?? 
                    throw new InvalidOperationException("❌ ANTI-HARDCODING: TF_API_PORT environment variable must be set")}/api/ai-advanced",
                Status = "active",
                ROI = Environment.GetEnvironmentVariable("TF_AI_ROI_PERCENTAGE") ?? 
                    throw new InvalidOperationException("❌ ANTI-HARDCODING: TF_AI_ROI_PERCENTAGE environment variable must be set"),
    ...
    public Task<bool> StopAIModuleAsync(string moduleName)
    {
        try
        {
