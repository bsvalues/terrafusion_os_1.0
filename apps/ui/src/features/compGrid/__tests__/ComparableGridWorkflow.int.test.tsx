...
describe('Comparable Grid Workflow', () => {
  it('happy path: add comparable, edit row, see score summary update', async () => {
  ...
  it('optimistic update rolls back on 409', async () => {
    server.use(
      http.post('/api/comp-grid/rows', async () => HttpResponse.json({ error: 'Conflict' }, { status: 409 }))
    );
