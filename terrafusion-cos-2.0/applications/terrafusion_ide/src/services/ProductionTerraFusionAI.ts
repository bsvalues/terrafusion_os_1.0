...
export class ProductionTerraFusionAI extends TerraFusionAIService {
  ...
  private initializeProductionProviders(): void {
    ...
    const ollamaPort = process.env.TF_OLLAMA_PORT || '11434';
    const ollamaEndpoint = process.env.OLLAMA_ENDPOINT || `http://localhost:${ollamaPort}`;
  ...
  private selectOptimalProvider(request: AIRequest): AIProvider {
    ...
    const availableProviders = Array.from(this.providers.values()).filter(
      provider => provider.isOperational
    );
