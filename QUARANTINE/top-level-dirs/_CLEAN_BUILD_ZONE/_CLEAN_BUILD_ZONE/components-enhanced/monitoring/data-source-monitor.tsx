...
export function DataSourceMonitor() {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'DEGRADED': return <Warning className="w-5 h-5 text-tf-warning" />
...
className="text-sm text-tf-light/70">Online</div>
          ...
          <Card className="bg-tf-dark-lighter/50 border-tf-warning/30">
            <CardContent className="p-4 text-center">
              <Warning className="w-6 h-6 text-tf-warning mx-auto mb-2" /><>
...
className="text-sm text-tf-light/70">Avg Success Rate</div>
          ...
          <div className="mt-6 text-center">
            <p className="text-lg text-tf-light italic">
              "Real-time monitoring ensures Terrafusion delivers on 379,000,000× performance promise"
            </p>
            {isMonitoring && (
