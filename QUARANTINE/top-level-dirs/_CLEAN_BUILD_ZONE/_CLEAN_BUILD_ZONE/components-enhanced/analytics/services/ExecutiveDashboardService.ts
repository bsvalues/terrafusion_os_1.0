class ExecutiveDashboardService {
  ...
  async getDashboardData(jurisdiction: string, timeRange: string = '30d') {
    const response = await fetch(`${this.baseUrl}/data?jurisdiction=${jurisdiction}&timeRange=${timeRange}`, {
  ...
  async getKPIData(jurisdiction: string, timeRange: string = '30d') {
    const response = await fetch(`${this.baseUrl}/kpis?jurisdiction=${jurisdiction}&timeRange=${timeRange}`, {
      headers: this.getAuthHeaders()
    });
  ...
  async exportDashboard(jurisdiction: string, format: 'pdf' | 'excel'): Promise<string> {
    const response = await fetch(`${this.baseUrl}/export?jurisdiction=${jurisdiction}&format=${format}`, {
      method: 'POST',
      headers: this.getAuthHeaders()
    });
