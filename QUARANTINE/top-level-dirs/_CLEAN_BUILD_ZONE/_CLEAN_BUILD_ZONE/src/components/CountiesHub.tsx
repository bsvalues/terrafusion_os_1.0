...
const CountiesHub: React.FC = () => {
  ...
  return <div className="w-full p-8">
      ...
      }].map(filterOption => <button key={filterOption.key} onClick={() => setFilter(filterOption.key)} style={{
        background: filter === filterOption.key ? 'linear-gradient(135deg, #0099ff, #00ffee)' : 'rgba(0,153,255,0.1)',
      ...
      margin: '0 auto'
        ...
        {filteredCounties.map(county => <div key={county.id} style={{
        ...
        e.currentTarget.style.borderColor = 'rgba(0,255,238,0.2)';
          ...
          background: `linear-gradient(90deg, ${getCountyColor(county.type)}, transparent)`
            ...
            color: '#fff'
              ...
              <div style={{
            background: county.status === 'ready' ? 'rgba(0,255,136,0.1)' : 'rgba(255,170,0,0.1)',
            color: county.status === 'ready' ? '#00ff88' : '#ffaa00',
            border: county.status === 'ready' ? '1px solid rgba(0,255,136,0.3)' : '1px solid rgba(255,170,0,0.3)',
            animation: county.status === 'analyzing' ? 'pulse 2s ease-in-out infinite' : 'none'
          }} className="font-semibold">
          ...
          marginBottom: '1rem'
            ...
            label: 'Annual Budget',
            value: county.budget
          }].map((info, index) => <div key={index} className="flex justify-between">
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => initiateMigration(county.id)} onMouseEnter={e => {
            ...
            e.currentTarget.style.boxShadow = 'none';
              ...
              <button onClick={() => viewDetails(county.id)} onMouseEnter={e => {
      ...
      <div className="p-8 text-center">
        ...
        }].map((item, index) => <div key={index} className="p-4">
