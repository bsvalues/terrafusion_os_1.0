...
const ChampionshipDeployment: React.FC = () => {
  ...
  const initializeSwarmVisualization = () => {
    ...
    nodes.push(createSwarmNode('supreme', container.offsetWidth / 2 - 15, container.offsetHeight / 2 - 15));
  ...
  return <div style={{
    ...
    overflowX: 'hidden'
      ...
      minHeight: '400px'
        ...
        <div ref={swarmContainerRef} style={{
        position: 'relative',
        height: '350px'
      }}>
          {swarmNodes.current.map((node, _index) => <div key={node.id} style={{
          position: 'absolute',
          width: node.type === 'supreme' ? '30px' : node.type === 'brady' ? '20px' : node.type === 'squad' ? '15px' : '8px',
          height: node.type === 'supreme' ? '30px' : node.type === 'brady' ? '20px' : node.type === 'squad' ? '15px' : '8px',
          borderRadius: '50%',
          left: `${node.x}px`,
          top: `${node.y}px`,
          background: node.type === 'supreme' ? 'linear-gradient(135deg, #ffd700, #ffed4e)' : node.type === 'brady' ? 'linear-gradient(135deg, #00e676, #69f0ae)' : node.type === 'squad' ? 'linear-gradient(135deg, #ff6b6b, #ff8787)' : '#4fc3f7',
          boxShadow: node.type === 'supreme' ? '0 0 30px #ffd700' : node.type === 'brady' ? '0 0 20px #00e676' : node.type === 'squad' ? '0 0 15px #ff6b6b' : '0 0 10px #4fc3f7',
      ...
      }].map((button, index) => <button key={index} onClick={button.action} style={{
      ...
      overflowY: 'auto'
        ...
        {logEntries.map((entry, index) => <div key={index} style={{
        padding: '8px',
        margin: '5px 0',
