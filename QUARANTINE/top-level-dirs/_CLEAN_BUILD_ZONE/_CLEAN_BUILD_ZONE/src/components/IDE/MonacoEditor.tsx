...
export const MonacoEditor: React.FC<MonacoEditorProps> = ({
  ...
  useEffect(() => {
    monaco.editor.defineTheme('terrafusion-government', {
      base: 'vs-dark',
      inherit: true,
      rules: [
  ...
  useEffect(() => {
    if (editorRef.current) {
      ...
      if (complianceMode) {
        setupComplianceValidation(editorInstance);
      }
  ...
  const registerTerraFusionLanguages = () => {
    // Register Government Module DSL
    monaco.languages.register({ id: 'government-module' });
    monaco.languages.setMonarchTokensProvider('government-module', {
      tokenizer: {
        root: [
    ...
    monaco.languages.setMonarchTokensProvider('ai-agent-config', {
      tokenizer: {
        root: [
  ...
  const setupAICodeCompletion = (editorInstance: monaco.editor.IStandaloneCodeEditor) => {
    // Custom completion provider with AI integration
    monaco.languages.registerCompletionItemProvider(language, {
      provideCompletionItems: async (model, position) => {
        ...
        try {
          ...
          return {
            suggestions: [...suggestions, ...terraFusionCompletions]
          };
        } catch (error) {
          console.error('AI completion error:', error);
          return { suggestions: getTerraFusionCompletions(textUntilPosition, language) };
        }
      }
    });
  };
  ...
  const setupComplianceValidation = (editorInstance: monaco.editor.IStandaloneCodeEditor) => {
  ...
  const setupKeyboardShortcuts = (editorInstance: monaco.editor.IStandaloneCodeEditor) => {
  ...
  const executeCode = async (code: string, lang: string, type: string) => {
    ...
    <Paper elevation={2} sx={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <Toolbar variant="dense" sx={{ borderBottom: 1, borderColor: 'divider' }}>
        <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
          TerraFusion IDE - {language.toUpperCase()} ({projectType})
        </Typography>
        
        <IconButton onClick={handleSave} color="primary" title="Save (Ctrl+S)">
          <Save />
        </IconButton>
        ...
        <IconButton onClick={handleAIAssist} color="secondary" title="AI Assist (Ctrl+J)">
          {aiLoading ? <CircularProgress size={20} /> : <BugReport />}
        </IconButton>
      ...
      {complianceMode && complianceStatus && (
        <Box sx={{ p: 1, bgcolor: complianceStatus.isCompliant ? 'success.light' : 'warning.light' }}>
          <Typography variant="caption">
            Compliance: {complianceStatus.isCompliant ? '✓ Compliant' : '⚠ Issues Found'} 
            ({complianceStatus.score}% score)
          </Typography>
        </Box>
      )}
    </Paper>
