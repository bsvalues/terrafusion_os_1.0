...
export const WorkflowDesigner: React.FC<WorkflowDesignerProps> = ({
  ...
  const loadAIAgents = async () => {
    try {
      const response = await fetch('/api/ai-swarm/agents');
      const agentData = await response.json();
      
      setAiAgents(agentData.agents || []);
      setSwarmStatus(agentData.swarmStatus || 'IDLE');
    } catch (error) {
      ...
      const newNode: WorkflowNode = {
        ...
        data: {
          ...
          configuration: {
            ...
            securityClearance: template.type === 'SUPREME_COMMANDER' ? 'RED' : 
                              template.type === 'FIELD_GENERAL' ? 'YELLOW' : 'GREEN',
  ...
  const getNodeTypeForTemplate = (template: any): string => {
    if (template.type === 'SERVICE') return 'governmentService';
  ...
  const handleWorkflowExecute = async () => {
    ...
    try {
      ...
      if (!complianceValidation.isCompliant) {
        throw new Error(`Workflow compliance validation failed: ${complianceValidation.violations.join(', ')}`);
      }
  ...
  const validateWorkflowCompliance = async () => {
    ...
    for (const node of nodes) {
      ...
      if (node.data.agentType === 'SUPREME_COMMANDER' && node.data.complianceLevel !== 'RED') {
        violations.push(`Supreme Commander must have RED clearance`);
      }
  ...
  const executeWorkflowPlan = async (plan: any) => {
    const response = await fetch('/api/ai-swarm/execute-workflow', {
      method: 'POST',
      headers: {
  ...
  return (
    <Box sx={{ height: '100vh', display: 'flex' }}>
      ...
      >
          ...
          <Card>
            <CardContent>
              ...
              <Typography variant="caption" color="text.secondary">
                Supreme Commander: 1 | Field Generals: 7 | Operational: {aiAgents.length - 8}
              </Typography>
            </CardContent>
          </Card>
        </Box>
            ...
            return (
              ...
              >
                ...
                {template.type !== 'SERVICE' && (
                  <Chip 
                    label={template.type === 'SUPREME_COMMANDER' ? 'RED' : 
                           template.type === 'FIELD_GENERAL' ? 'YELLOW' : 'GREEN'}
                    size="small"
                    sx={{ 
                      backgroundColor: getClearanceColor(
                        template.type === 'SUPREME_COMMANDER' ? 'RED' : 
                        template.type === 'FIELD_GENERAL' ? 'YELLOW' : 'GREEN'
                      ),
                      color: 'white',
                      fontSize: '10px'
        ...
        {/* Active Agents */}
        <Typography variant="subtitle1" sx={{ px: 2, py: 1 }}>
          Active Agents ({aiAgents.filter(a => a.status === 'ACTIVE').length})
        </Typography>
        <List dense>
          {aiAgents.slice(0, 10).map((agent) => (
            <ListItem key={agent.id}>
              <ListItemIcon>
                <Badge 
                  badgeContent={agent.type === 'SUPREME_COMMANDER' ? 'SC' : 
                               agent.type === 'FIELD_GENERAL' ? 'FG' : 'OA'}
                  color={getAgentStatusColor(agent.status)}
                >
                  <Psychology sx={{ color: getClearanceColor(agent.securityClearance) }} />
                </Badge>
              </ListItemIcon>
              <ListItemText 
                primary={agent.name}
      ...
      <Box sx={{ flexGrow: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <Toolbar variant="dense" sx={{ borderBottom: 1, borderColor: 'divider' }}>
          <IconButton onClick={() => setIsToolboxOpen(!isToolboxOpen)}>
            {isToolboxOpen ? <VisibilityOff /> : <Visibility />}
          </IconButton>
          
          <Typography variant="h6" component="div" sx={{ flexGrow: 1 }}>
            TerraFusion AI Workflow Designer
          </Typography>
          ...
          <IconButton onClick={handleWorkflowSave} color="primary">
            <Save />
          </IconButton>
          ...
          {selectedNode && (
            <IconButton onClick={() => setConfigDialog(true)} color="secondary">
              <Settings />
            </IconButton>
          )}
          ...
          {selectedNode && (
            <Grid container spacing={2} sx={{ mt: 1 }}>
                ...
                <FormControl fullWidth>
                  ...
                  <Select
                    ...
                    onChange={(e) => {
                      const updatedNode = {
                        ...selectedNode,
                        data: { ...selectedNode.data, complianceLevel: e.target.value as 'RED' | 'YELLOW' | 'GREEN' }
                      };
                      setSelectedNode(updatedNode);
                      setNodes((nds) => nds.map((n) => n.id === selectedNode.id ? updatedNode : n));
                    }}
                ...
                <Grid item xs={12}>
                  ...
                  <Box sx={{ pl: 2 }}>
                    <Typography variant="body2">
                      Type: {selectedNode.data.agentType}
                    </Typography>
                    <Typography variant="body2">
                      Specialization: {selectedNode.data.configuration?.specialization?.join(', ') || 'General'}
                    </Typography>
                    <Typography variant="body2">
                      Security Clearance: {selectedNode.data.configuration?.securityClearance}
                    </Typography>
                  </Box>
                </Grid>
              )}
              ...
              <Grid item xs={12}>
                ...
                <Box sx={{ pl: 2 }}>
                  {(selectedNode.data.governmentStandards || []).map((standard) => (
                    <Chip
                      key={standard}
                      label={standard}
                      size="small"
