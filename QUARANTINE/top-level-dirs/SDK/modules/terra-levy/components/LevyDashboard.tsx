...
import { useLevyHealth, useLevyMeasures, useAllLevyScenarios } from '../hooks/useLevyData';
...
const MetricCard: React.FC<MetricCardProps> = ({ title, value, subtitle, status = 'success', className = '' }) => {
  const statusColors = {
  ...
  return (
    <div className={`terra-glass rounded-lg p-6 border-2 ${statusColors[status]} hover-quantum transition-all ${className}`}>
...
export const LevyDashboard: React.FC = () => {
  ...
  const { data: measures, isLoading: measuresLoading, error: measuresError } = useLevyMeasures(undefined, 10, 0);
  const { data: scenarios, isLoading: scenariosLoading, error: scenariosError } = useAllLevyScenarios(10, 0);

  // Calculate dashboard metrics
  const activeMeasures = measures?.items?.filter((m) => (m.status || '').toUpperCase() === 'ACTIVE').length || 0;
  ...
  return (
    <div className="min-h-screen bg-[#0A0E1A] text-white p-8">
      ...
      <div className="mb-8">
        ...
        <p className="text-xl text-[#00ffee]/70">
      ...
      {health && (
        <div className="mb-8 terra-glass rounded-lg p-4 border border-[#00ffee]/30">
          <div className="flex items-center gap-3">
            <div className={`w-3 h-3 rounded-full ${health.status === 'healthy' ? 'bg-[#00ffaa] quantum-pulse' : health.status === 'degraded' ? 'bg-[#ffaa00]' : 'bg-[#ff0055]'}`} />
            <span className="text-sm uppercase tracking-wide">System Status: {health.status}</span>
            <span className="text-xs text-[#00ffee]/50 ml-auto">{new Date(health.timestamp).toLocaleString()}</span>
          </div>
        </div>
      )}
      {(measuresError || scenariosError) && (
        <div className="mb-6">
          <Notice kind="error">{String((measuresError as any)?.message || (scenariosError as any)?.message || 'Failed to load dashboard data.')}</Notice>
        </div>
      )}
      ...
      <div className="mb-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <button
            ...
            className="terra-glass rounded-lg p-6 border-2 border-[#0099ff] hover:border-[#00ffee] transition-all hover:transform hover:-translate-y-1 text-left"
          >
            <div className="text-lg font-semibold mb-2">Calculate Optimal Rate</div>
            <div className="text-sm text-[#00ffee]/70">
              AI-powered levy rate optimization with quantum accuracy
            </div>
          </button>
          <button
            ...
            className="terra-glass rounded-lg p-6 border-2 border-[#0099ff] hover:border-[#00ffee] transition-all hover:transform hover:-translate-y-1 text-left"
          >
            <div className="text-lg font-semibold mb-2">Analyze Scenarios</div>
            <div className="text-sm text-[#00ffee]/70">
              Compare multiple levy scenarios with championship-level insights
            </div>
          </button>
          <button
            ...
            className="terra-glass rounded-lg p-6 border-2 border-[#0099ff] hover:border-[#00ffee] transition-all hover:transform hover:-translate-y-1 text-left"
          >
            <div className="text-lg font-semibold mb-2">Revenue Projections</div>
            <div className="text-sm text-[#00ffee]/70">
              Multi-year quantum-enhanced revenue forecasting
            </div>
          </button>
        </div>
      </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-[#00ffee]/20">
                  <th className="text-left p-4 text-sm uppercase tracking-wide text-[#00ffee]/70">Measure</th>
                  <th className="text-left p-4 text-sm uppercase tracking-wide text-[#00ffee]/70">Levy Year</th>
                  <th className="text-left p-4 text-sm uppercase tracking-wide text-[#00ffee]/70">Type</th>
                  <th className="text-left p-4 text-sm uppercase tracking-wide text-[#00ffee]/70">Status</th>
                  <th className="text-right p-4 text-sm uppercase tracking-wide text-[#00ffee]/70">Target Revenue</th>
                  <th className="text-right p-4 text-sm uppercase tracking-wide text-[#00ffee]/70">Actions</th>
                </tr>
              </thead>
              <tbody>
            ...
            {scenarios.items.slice(0, 3).map((scenario) => (
              ...
              >
                <div className="flex items-start justify-between mb-3">
                  ...
                  <span className={`text-xs px-2 py-1 rounded uppercase ${scenario.isActive ? 'bg-[#00ffaa]/20 text-[#00ffaa]' : 'bg-[#00ffee]/20 text-[#00ffee]'}`}>
