...
const BatchImportHandler: React.FC<BatchImportHandlerProps> = ({
  ...
  const handleFileUploadComplete = (fileId: number) => {
    ...
    const newFile = fileUploads.find((file: FileUpload) => file.id === fileId);
    if (newFile) {
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  {(parseInt(selectedFile.fileSize) / 1024).toFixed(1)} KB • Uploaded {new Date(selectedFile.uploadedAt).toLocaleDateString()}
                </p>
              </div>
            </div>
            ...
            {selectedFile.status === 'processing' && (
              <div className="space-y-2">
                ...
                <Progress
                  value={selectedFile.totalItems ? ((Number(selectedFile.processedItems) || 0) / Number(selectedFile.totalItems)) * 100 : 50}
                  className="h-2"
                />
              </div>
            )}
