...
import { AlertCircle, Building, InfoIcon, MapPin, TrendingUp } from "lucide-react";
  ...
  const generateAIInsights = async (forceRefresh = false) => {
    ...
    try {
      ...
      const insightText = `
        ...
        ${result.factors ? result.factors.map((factor: any) => `- ${factor.name} (${factor.impact} impact): ${factor.description}`).join('\n') : '- No specific factors provided'}
