...
export default function UserManagement() {
      ...
      { id: user.id, isActive: !user.isActive },
      {
        onSuccess: () => {
          toast({
            ...
            description: `User ${user.username} has been ${!user.isActive ? "activated" : "deactivated"}.` as string,
          });
        },
        onError: (error: any) => {
          toast({
    ...
    updateUserMutation.mutate(
      { id: user.id, role: newRole },
      {
        onSuccess: () => {
          toast({
            title: "Role updated",
            description: `${user.username}'s role has been updated to ${newRole}.` as string,
          });
        },
        onError: (error: any) => {
          toast({
  ...
  const handleDeleteUser = (user: User) => {
    if (window.confirm(`Are you sure you want to delete user ${user.username}? This action cannot be undone.`)) {
      deleteUserMutation.mutate(parseInt(user.id), {
        onSuccess: () => {
          toast({
            title: "User deleted",
                ...
                <TableRow key={user.id}>
                  <TableCell className="font-medium">
                    <div>
                      ...
                      {(user.firstName || user.lastName) && <div className="text-sm text-neutral-500">{user.firstName} {user.lastName}</div>}
                    </div>
                  </TableCell>
                  <TableCell>
                    <Badge variant={
