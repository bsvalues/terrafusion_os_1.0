...
class BackupSystem {
    ...
    async createBackupManifest(backupPath, backupName) {
        const manifest = {
            ...
            contents: {
                ...
                log_files: fs.existsSync(path.join(backupPath, 'logs')) ? 
                    fs.readdirSync(path.join(backupPath, 'logs')).length : 0
            },
            integrity: {
        ...
        console.log(`[BACKUP] Manifest created: ${manifest.integrity.total_files} files, ${manifest.integrity.backup_size_mb}MB`);
    }
