...
curl -w "@curl-format.txt" -s -o /dev/null http://localhost:3000/api/demo/overview

# Monitor real-time metrics
watch -n 1 'curl -s http://localhost:3000/api/monitoring/performance | jq .'
```
...
#### Symptoms
- Grafana dashboard not loading (port 3001)
- Prometheus metrics not available (port 9090)
...
# Enable debug port
NODE_OPTIONS="--inspect=0.0.0.0:9229" node demo-server.js
...
# API health check
curl -s http://localhost:3000/api/demo/health > health-check.txt
curl -s http://localhost:3000/api/monitoring/performance > performance-metrics.txt
...
echo -e "\n📈 Performance Metrics:"
curl -s http://localhost:3000/api/monitoring/performance | jq '.'

echo -e "\n💾 Backup Status:"
curl -s http://localhost:3000/api/backup/list | jq '.'
...
### Monitoring Tools
- **Application Dashboard**: http://localhost:3000/analytics-dashboard.html
...
*Built with championship precision for reliable troubleshooting*  
*Terrafusion Troubleshooting Guide v3.0.0 - Solving Problems with Excellence*