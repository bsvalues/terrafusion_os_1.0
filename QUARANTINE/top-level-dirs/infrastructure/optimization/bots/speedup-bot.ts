...
export class SpeedupBot extends EventEmitter {
  ...
  private calculateAverageSpeedup(): number {
    ...
    const total = this.advantageCalculations.reduce(
      (sum, calc) => sum + calc.speedup_factor,
      0,
    );
  ...
  private groupSpeedupByAlgorithm(): any {
    ...
    for (const alg in grouped) {
      ...
      grouped[alg] =
        speedups.reduce((a: number, b: number) => a + b, 0) / speedups.length;
    }
  ...
  async getStatus(): Promise<any> {
    return {
      ...
      average_speedup: this.calculateAverageSpeedup(),
      quantum_advantage_instances: this.advantageCalculations.filter(
        (c) => c.practical_advantage,
      ).length,
    };
  }
