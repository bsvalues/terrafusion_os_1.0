...
localizableUnit = {
    ...
    "us_feet": ARCPY.GetIDMessage(220216),
...
def checkForDuplicateOutput(outputName, workSpace, count):
    if count == 0:
        outputFile = OS.path.join(workSpace, outputName)
    else:
        outputFile = OS.path.join(workSpace, outputName+ str(count))
    if ARCPY.Exists(outputFile):
        return checkForDuplicateOutput(outputName, workSpace, count + 1)
    else:
        return outputFile
...
def getNumericParameter(paramNum, parameters = None, defualt = None):
...
def getTextParameter(paramNum, parameters = None, fieldName = False):
...
def getLinearUnitParameter(paramNum, parameters = None):
...
def getMultiFieldParameter(paramNum, parameters = None):
    """Obtains "optional" Field Parameters.
...
def checkOutputPath(path, typeOutput, listExtensions = [".ASC", ".IMG",".TIFF", \
                                                        ".JPG", ".TIF", ".JP2", ".PNG", \
                                                        ".GIF", ".BIP", ".DAT", ".CRF", \
                                                        ".MRF", ".BMP", ".BIL", ".BSQ"]):
    """
    ...
    INPUT:
        ...
        typeOutput (str): RASTER, FC, TABLE
    ...
    if path not in ["", None]:
        if path.lower().startswith("memory\\") or path.lower().startswith("in_memory\\"):
            if typeOutput == "RASTER":
                endData = [True for e in listExtensions  if path.upper().endswith(e)]
                if len(endData):
                    ARCPY.AddIDMessage("ERROR", 210, path )
                    raise SystemExit
            if typeOutput == "FC":
        ...
        if typeOutput == "RASTER":
            if isGDB(path):
                if path[4:].upper() in listExtensions or len([i for i in outName if i in noAllowCharacters]) > 0:
                    ARCPY.AddIDMessage("ERROR", 210, path )
                    raise SystemExit
    ...
    if not inside[0]:
        ...
        if not xInside:
            ...
            setXYDomainProjection(min(minValues[0] - extend, xMin), yMin, 
                                  max(maxValues[0] + extemd, xMax), yMax, 
                                  spatialRef)
...
def getBaseWorkspaceType(dirName, workType = None,
    ...
    if dirName.upper().count(".SDE") > 0 or dirName.upper().startswith('HTTP') or \
       dirName.upper().startswith('GIS SERVERS'):
...
def returnTableName(tableName, extension = ".dbf"):
    ...
    ### Check In Memory ####
    if tableName.lower().startswith("in_memory"):
        return tableName, 0
...
def buildTableCell(data, align=None, bold=False, rowSpan=1, colSpan=1):
    ...
    align       : {"left", "right", None}, default=None
    ...
    if align:
        if align.lower() not in ["right", "left"]:
            align = "left"
...
class DistanceInfo(object):
    ...
    def printDistance(self, distance, formatStr = "%0.4f"):
        return LOCALE.format_string(formatStr, distance) + " " + self.outputString
...
class DataContainer(object):
    ...
    def generateOutput(self, outputPath, listFields, names = None, alias = None):
...
class GenericReader(object):
    ...
    def __init__(self, inputData, fields, generateBlockOfData = True,
        ...
        self.ssdo = SSDO.SSDataObject(inputData, displayProjectionWarning = displayProjectionWarning)
    ...
    def output(self, outputFC, outputData, fieldNames, aliasFieldNames, fieldOrder = [],
                parameters = None, indexOutput = None, indexInput = None, allowOverwriteFields = False):
        ...
        if self.outputOption == "NEW_DATASET_JUST_NEW_FIELDS":
            ...
            if self.isTable:
                ...
                container.generateOutput(outputFC,outputData,fieldNames,aliasFieldNames)
            else:
                if self.getGeometry:
                    container = DataContainer(spatialRef = self.ssdo.spatialRef, shapes = list(self.ssdo.shapes))
                    container.generateOutput(outputFC,outputData,fieldNames,aliasFieldNames)
                else:
                    zValues = None
                    if self.ssdo.hasZ:
                    ...
                    container = DataContainer(spatialRef = self.ssdo.spatialRef, xy = self.ssdo.xyCoords,z = zValues)
                    container.generateOutput(outputFC, outputData,fieldNames,aliasFieldNames)
            ...
            if not allowOverwriteFields:
                candFields = self.checkDuplicated(candFields)
            else:
    ...
    def __onlyOverwriteDoubleFields(self, candidateFields):
        ...
        if len(checkDuplicatesList) > 0 :
            listFieldsOutput = self.checkDuplicated(checkDuplicatesList)
    ...
    def __output2NewDataset(self, outputFC, candidateFields,
                            appendFields = [], fieldOrder = []):
        ...
        self.ssdo.output2NewFC(outputFC, cf,
                               listBase, listBase)
    ...
    # check if the filed name already exists
    def checkDuplicated(self, newCandidateFields):
...
class BaseVariable(object):
    ...
    def maskData(self, info):
        if info.dtype == NUM.int32:
            nullInt = NUM.iinfo(NUM.int32).min
            nulls = info != nullInt
            if len(nulls) > 0:
    ...
    def minmax(self, data):
        ...
        if minValueV == maxValueV:
            return data, None, 110384
...
def buildLocaleCIMLayer(templatePath, paramID, data=None):
    """
