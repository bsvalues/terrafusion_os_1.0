...
from common import (PATool, PAFeatureLayer, FeatureServiceLayerPublisher,
                    ...
                    ModelBuilderMixin, RemoteToolboxUtils,
                    PAErrorProcessor, PAEnvironment,
                    COST_KEY, ParameterUnpackMixin,
                    AOLUtils)
from common import FHS_PARAM_NAMES as PARAM_NAMES
...
ERROR_CODES = [192, 366, 401, 641, 897, 906, 929, 1533, 1534, 1535, 1536, 
...
class OHSATool(ParameterUnpackMixin, ModelBuilderMixin, PATool):
    """Implement the Optimized Hot Spot Analysis tool."""

    def get_parameters(self):
        """Overwrite the abstractmethod of get_parameters."""
        # Tool signature matches the descriptions here:
        # https://developers.arcgis.com/rest/analysis/api-reference/find-hot-spots.htm
        ...
        if self.get_param_as_text(PARAM_NAMES["boundingPolygonLayer"]):
            bounding_polygon_layer = PAFeatureLayer(
                PARAM_NAMES["boundingPolygonLayer"], 
        ...
        if self.get_param_as_text(PARAM_NAMES["aggregationPolygonLayer"]):
            aggregation_polygon_layer = PAFeatureLayer(
                PARAM_NAMES["aggregationPolygonLayer"], 
                metadata={"parameterDataType": "Feature Set",
        ...
        if has_analysis_field:
            aggregate_method = ""
            self.num_objects = input_layer.count
        elif aggregation_polygon_layer:
            aggregate_method = "COUNT_INCIDENTS_WITHIN_AGGREGATION_POLYGONS"
            self.num_objects = input_layer.count + aggregation_polygon_layer.count
        else:
            ...
            if bounding_polygon_layer is not None:
                self.num_objects += bounding_polygon_layer.count
        ...
        if self.is_running_in(PAEnvironment.MODELBUILDER):
            result_layer = AnalysisUtils.initialize_output_layer(
                specified_out_path=self.get_param_as_text(PARAM_NAMES["hotSpotsResultLayer"]))
        else:
            result_layer = AnalysisUtils.initialize_output_layer(
                input_layer.count, "HotSpotsOutput")
        output_wkspc = AOLUtils.get_output_wkspc(input_layer.count)
        self.executor: OHSAExecutor = OHSAExecutor(input_layer, result_layer,
                                                   analysis_field,
    ...
    def set_visualization(self):
        ...
        renderer = SpatialStatsRenderer.get_hotspot_rendering_info(self.executor.output_layer)
    ...
    def publish_outputs(self):
        ...
        publisher.add_layer_to_publish(
            ...
            PARAM_NAMES["hotSpotsResultLayer"],
            "HotSpotsOutput", layer_index=0)
        publisher.publish()
    ...
    def log_usage_metering(self):
        ...
        cost = self.num_objects * 0.001
        LogUtils.log_usage(self.task_name, self.executor.output_layer.count, cost, values)


def execute_tool(version: float):
    """Entry of FindHotSpots tool."""
    try:
        ohsa_tool = OHSATool(TASK_NAME, output_name_index=10, context_index=11,
                             version=version)
        ohsa_tool.run()
