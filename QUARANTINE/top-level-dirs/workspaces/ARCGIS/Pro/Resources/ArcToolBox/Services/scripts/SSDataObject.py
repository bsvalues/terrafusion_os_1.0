...
################## Methods #########################
def table2RecArray(inputTable, fieldNames, types = [0,1,2,3,4,5,6],
    ...
    if not includeNulls:
        ...
        for row in rows:
            ...
            if badValues:
                ...
                for fieldInd, fieldName in enumerate(fieldNames):
                    fieldValue = restFields[fieldInd]
                    if fieldValue not in [None, 'nan']:
                        fields[fieldName].data[numObs] = fieldValue
                    else:
                        typeField = fields[fieldName].data.dtype
                        if typeField in [NUM.float, NUM.float32]:
                            fields[fieldName].data[numObs] = NUM.nan
                        elif typeField.kind in ['U', 'S']:
                            fields[fieldName].data[numObs] = 'nan'
                        elif typeField.kind == "M":
                            fields[fieldName].data[numObs] = 'nat'
                        else:
                            fields[fieldName].data[numObs] = NUM.iinfo(typeField).min
                numObs += 1
                oids.append(oid)
                master2Order[oid] = c
                order2Master[c] = oid
        ...
        if self.data is not None:
            if self.type in ['LONG', 'SHORT']:
                if replaceNullInts:
                    ...
                    data = NUM.array(self.data, dtype = float)
...
class SSDataObject(object):
    ...
    def __init__(self, inputFC, templateFC = None, explicitSpatialRef = None,
        ...
        if self.dataType in UTILS.dataTypeNoTable:
    ...
    def testFields(self, fieldList, fields = [], types = [0,1,2,3,5,6]):
    ...
    def obtainData(self, masterField = None, fields = [],
                   ...
                   types = [0,1,2,3,4,5,6], minNumObs = 0, warnNumObs = 0,
        ...
        if self.isTable:
            ...
            self.__obtainDataTable(fields = fields, types = types,
                    ...
                    if numFields:
                        ...
                        for fieldInd, fieldName in enumerate(fields):
                            ...
                            if fieldValue not in [None, 'nan']:
                                ...
                                if typeField in [NUM.float, NUM.float32]:
                                    self.fields[fieldName].data[c] = NUM.nan
                                elif typeField.kind in ['U', 'S']:
                                    self.fields[fieldName].data[c] = 'nan'
                                elif typeField.kind == "M":
                                    self.fields[fieldName].data[c] = 'nat'
                                else:
                                    self.fields[fieldName].data[c] = NUM.iinfo(typeField).min
    ...
    def __obtainDataTable(self,fields = [],
                              types = [0,1,2,3,4,5,6],
                              minNumObs = 0, warnNumObs = 0,
                              explicitBadRecordID = None,
    ...
    def obtainDataGA(self, masterField = None, fields = [], types = [0,1,2,3,5,6],
                     minNumObs = 0, warnNumObs = 0):
        """Takes a list of field names and returns it in a dictionary
        structure.
        ...
        for row in UTILS.ssRange(numObs):
            ...
            if numFields:
                ...
                for fieldInd, fieldName in enumerate(fields):
                    ...
                    self.fields[fieldName].data[row] = fieldValue
    ...
    def addFields2FC(self, candidateFields, fieldOrder = []):
        ...
        if hasJoin:
            ...
            ARC._ss.update_table_from_dataobject(self,self.inputFC,
                                                        self.masterField.split(".")[1],
                                                        [ f  for f in candidateFields.values()],
                                                        addFields)
            return


        #### Populate Output Feature Class with Values ####
        ARCPY.SetProgressor("step", ARCPY.GetIDMessage(220118),
                            0, self.numObs, 1)
        outRows = DA.UpdateCursor(self.inputFC, outputFieldNames)
        minInt = NUM.iinfo(NUM.int32).min
        try:
            for row in outRows:
                masterID = row[0]
                ARCPY.SetProgressorPosition()
                if masterID in self.master2Order:
                    ...
                    for fieldName in fieldKeys:
                        ...
                        #### Check Null values in Int Fields ####
                        if field.data.dtype == NUM.int32 and fieldValue == minInt:
                            fieldValue = None
    ...
    def output2NewFC(self, outputFC, candidateFields, appendFields = [],
        ...
        if self.isTable:
            ...
            outIsShapeFile = UTILS.isShapeFile(self.outputFC, "DBF")
        ...
        if not self.isTable:
            ...
            #### Do T2T  ####
            T2T = CONV.TableToTable
            try:
                T2T(self.inputFC, outPath, outName, "", outputFieldMaps)
            except:
                ARCPY.AddIDMessage("ERROR", 210, self.outputFC)
                raise SystemExit()
