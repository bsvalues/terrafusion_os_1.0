"""-----------------------------------------------------------------------------
Name:              FlowDistance.py
Purpose:           This is the service tool that performs hydrology flow distance
                   analysis.
Author:            Esri Inc.
Created:           7/31/2017
...
if __name__ == '__main__':
    ...
    instream = arcpy.GetParameterAsText(0)
    insurf = arcpy.GetParameterAsText(1)
    outras = arcpy.GetParameterAsText(2)
    inflow = arcpy.GetParameterAsText(3)
    distancetype = arcpy.GetParameterAsText(4)
    flowdirectiontype = arcpy.GetParameterAsText(5)
    ...
    try:
        ...
        instream = rasterutils.getInDataPath(instream)
        if isinstance(instream, dict):
            instream = json.dumps(instream)
        inflow = rasterutils.getInDataPath(inflow)
        if isinstance(inflow, dict):
            inflow = json.dumps(inflow)
        ...
        # 4. Execute tool
        arcpy.gp.FlowDistance_sa(instream, insurf, outras, inflow, distancetype, flowdirectiontype, statisticsType)
