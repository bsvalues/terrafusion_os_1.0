...
def reportBadRecords(numObs, numBadObs, badIDs, label="OID",
    ...
    ARCPY.AddIDMessage("WARNING", firstID, numBadObs, numObs)
    badIDs = badIDs[0:30]
    badIDs = ", ".join(badIDs)
    ARCPY.AddIDMessage("WARNING", secondID, label, badIDs)
...
def reportDoubleAngleCases(numCases, numDoubleAngleCases, doubleAngleCases,
    ...
    if numCases == numDoubleAngleCases:
        ARCPY.AddIDMessage("WARNING", 110070)
    else:
        ARCPY.AddIDMessage("WARNING", 110071, numBadCases, numDoubleAngleCases)
        if len(doubleAngleCases) > 30:
            doubleAngleCases = doubleAngleCases[0:30]
        doubleAngleCases = ", ".join(doubleAngleCases)
        ARCPY.AddIDMessage("WARNING", 947, label, doubleAngleCases)
...
def reportBadCasesForTestUniformity(numCases, numBadCases, badCases, label="CASE",
    ...
    if numCases == numBadCases:
        ARCPY.AddIDMessage("WARNING", 110068, numCases2D, numCases3D)
    else:
        ARCPY.AddIDMessage("WARNING", 110069, numBadCases, numCases)
...
def errorNumberOfObs(numObs, minNumObs=3):
    ...
    if numObs < minNumObs:
        msg = {}
        msg["messageCode"] = u"AO_{}".format(641)
        msg["params"] = {"numFeatures": minNumObs}
        error_msg = ARCPY.GetIDMessage(84262).format(minNumObs)
    ...
    if numObs == 0 and minNumObs == 0:
        msg = {}
        msg["messageCode"] = u"AO_{}".format(401)
        msg["message"] = u"{}.".format(ARCPY.GetIDMessage(401))
        ARCPY.AddError(JSON.dumps(msg))
...
def warningNoNeighbors(numObs, numObsNoNeighs, idsNoNeighs,
    ...
    if forceNeighbor:
        if contiguity:
            ARCPY.AddIDMessage("Warning", 718, str(numObsNoNeighs))
        else:
            ARCPY.AddIDMessage("Warning", 715, str(numObsNoNeighs))
        ARCPY.AddIDMessage("Warning", 716, masterField, idsNoNeighs)
    else:
        if silentStats:
            ARCPY.AddIDMessage("Warning", 846, str(numObsNoNeighs))
...
def checkField(allFields, fieldName, types=[]):
    ...
    try:
        ...
        msg = {}
        msg["messageCode"] = u"AO_{}".format(728)
        errorMsg = u"Field {} does not exist within the analysis layer.".format(fieldName)
