...
if __name__=='__main__':
    ...
    if inOptimize == "SPEED" or inOptimize == "" or inOptimize == "#" or inOptimize == None:
        inAnalysisMethod = "PERIMETER_SIGHTLINES"

    maxView = arcpy.GetParameterAsText(4)
    maxViewField = arcpy.GetParameterAsText(5)
    if maxView == "" or maxView == None or maxView == "#":
        maxViewParam = decodeLinearUnits(maxViewField)
    else:
        maxViewParam = maxView

    minView = arcpy.GetParameterAsText(6)
    minViewField = arcpy.GetParameterAsText(7)
    if minView == "" or minView == None or minView == "#":
        minViewParam = decodeLinearUnits(minViewField)
    else:
        minViewParam = minView

    distanceIs3D = arcpy.GetParameter(8)
    if distanceIs3D == True or distanceIs3D == "3D":
        dist3D = "3D"
    else:
        dist3D = "GROUND"

    elevationObs = arcpy.GetParameterAsText(9)
    elevationObsField = arcpy.GetParameterAsText(10)
    if elevationObs == "" or elevationObs == None or elevationObs == "#":
        elevObsParam = decodeLinearUnits(elevationObsField)
    else:
        elevObsParam = elevationObs

    heightObs = arcpy.GetParameterAsText(11)
    heightObsField = arcpy.GetParameterAsText(12)
    if heightObs == "" or heightObs == None or heightObs == "#":
        heightObsParam = decodeLinearUnits(heightObsField)
    else:
        heightObsParam = heightObs

    heightTarget = arcpy.GetParameterAsText(13)
    heightTargetField = arcpy.GetParameterAsText(14)
    if heightTarget == "" or heightTarget == None or heightTarget == "#":
        heightTargetParam = decodeLinearUnits(heightTargetField)
    else:
        heightTargetParam = heightTarget

    aglOutName = arcpy.GetParameter(15)

    if aglOutName == "" or aglOutName == None:
        outAGL = ""
    else:
        outAGL = os.path.join(arcpy.env.scratchWorkspace, aglOutName)  # output path for local debug run

    #new parameters added on 5/1/2023
    verticalError = arcpy.GetParameterAsText(16)
    refractivityCoefficient = arcpy.GetParameterAsText(17)
    horizontalStartAngle = arcpy.GetParameterAsText(18)
    horizontalEndAngle = arcpy.GetParameterAsText(19)
    verticalUpperAngle = arcpy.GetParameterAsText(20)
    verticalLowerAngle = arcpy.GetParameterAsText(21)

    context = arcpy.GetParameterAsText(22)
    ...
    try:
        ...
        if aglOutName != "" and aglOutName != None and aglOutName != "#":
            ...
            iid2, isurl2, aisurl2, outras2 = rasterutils.getOutRasterPath(aglOutName)
        ...
        # inner_radius_is_3d == outer_radius_is_3d in this code
        arcpy.gp.Viewshed2_sa(inras2, InputLayer, outras, outras2, "FREQUENCY",  verticalError, "", refractivityCoefficient,
                              heightTargetParam, elevObsParam, heightObsParam, minViewParam, dist3D, maxViewParam,
        ...
        arcpy.SetParameterAsText(23, json.dumps(outval))

        if outras2 != "":
            outval2 = {"itemId": iid2, "url": isurl2}
            arcpy.SetParameterAsText(24, json.dumps(outval2))
