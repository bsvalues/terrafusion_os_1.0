"""-----------------------------------------------------------------------------
Name:              SurfaceParameters.py
Purpose:           This is the service tool that performs surface parameters analysis
Author:            Esri Inc.
Created:           12/29/2021
...
if __name__ == '__main__':
    ...
    localsurf = arcpy.GetParameterAsText(3)
    neighbordist = arcpy.GetParameterAsText(4)
    adaptiveneighbor = arcpy.GetParameterAsText(5)
    zunit = arcpy.GetParameterAsText(6)
    outslope = arcpy.GetParameterAsText(7)
    projectazimuths = arcpy.GetParameterAsText(8)
    equatorialaspect = arcpy.GetParameterAsText(9)
    inmask = arcpy.GetParameterAsText(10)
    ...
    try:
        ...
        hostedgp = agolgp.HostedGP(11, 1)
        if rasterutils.checkIfFeatureCollection(inmask):
            Input, InputLayerCount = aolutils.getHostedLayerX(hostedgp, "inmask", 10)
            inmask = Input.name
        # Now parsing the input raster
        else:
            inmask = rasterutils.getInDataPath(inmask)
            if inmask.find("/FeatureServer/") > -1 \
                    or inmask.find("/MapServer/") > -1:
                Input, InputLayerCount = aolutils.getHostedLayerX(hostedgp, "inmask", 10)
                inmask = Input.name
            else:
                if isinstance(inmask, dict):
                    inmask = json.dumps(inmask)
        ...
        # 4. Execute tool
        arcpy.gp.SurfaceParameters_sa(insurf, outsurf, parameter, localsurf, neighbordist,
                                      adaptiveneighbor, zunit, outslope, projectazimuths, equatorialaspect, inmask)
