...
def canChangeFieldNamd(fields, current, compareOnly = False):
    listTypes =["Z_SCORE","MIN_MAX","MAX_ABS", "ROBUST"]
    if type(current) !=  str:
        return False
    for fld in fields:
...
class ToolValidator(object):
    ...
    def updateParameters(self):
        ...
        if parameters[2]. value not in ['Z-SCORE', 'MIN-MAX', 'MAXABS', "ROBUST"]:
            parameters[2].value = "Z-SCORE"
        ...
        if parameters[2].value == "MIN-MAX":
            ...
            if parameters[3].value is not None and  parameters[4].value is not None:
                ...
                if parameters[3].value >= parameters[4].value:
                    parameters[4].setIDMessage("ERROR", 110365, parameters[3].value)
