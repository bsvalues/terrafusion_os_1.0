"""-------------------------------------------------------------------------
    Tool:               Train Text Classification Model (GeoAI Tools)
    Source Name:        trainTextClassificationModel.py
    ...
    Optional model_arguments: Pretrained Model
                        ...
                        Remove HTML Tags
                        Remove URLs
    Description:        Trains Text Classifier model.
------------------------------------------------------------------------"""
...
def read_file(path, col_list):
    col_list_first = [col_list[0]]
    col_list_first.extend(col_list[1])
    col_list = col_list_first
...
def execute():
    try:
        ...
        if backbone_model == "":
            kwargs["backbone"] = "bert-base-cased"
        else:
        ...
        try:
            ...
            if "invalid literal for int() with base 10" in str(e):
                arcpy.AddIDMessage("ERROR", 260300)
                exit()
            else:
                arcpy.AddIDMessage("ERROR", 260057)
                exit()
        ...
        if not pretrained_model:
            ...
            try:
                training_model_object = TextClassifier(data_bunch, **kwargs)
                if arcpy.env.isCancelled:
                    arcpy.AddIDMessage("ERROR", 571)
                    exit()
            except Exception as e:
            ...
            try:
                if backbone_model not in ["mistral"]:
                    training_model_object = TextClassifier.from_model(
        ...
        accuracy = LOCALE.format_string(
            "%0.2f", float(training_model_object.accuracy())
        )
        arcpy.AddIDMessage("INFORMATIVE", 260058, str(accuracy))
