...
class Geopackage(object):
    ...
    def get_tiletable_name(self, cursor):
        res = cursor.execute('SELECT count(*) from gpkg_tile_matrix_set')
        idx = res.fetchall()[0][0]
        return 'tiles' if idx == 0 else 'tiles_%s' % idx
    ...
    def _srs_id_exists(self, cursor, wkt):
        exists = cursor.execute(
            'SELECT count(srs_id) from gpkg_spatial_ref_sys '
            'where srs_id=%s' % wkt)
        return exists.fetchall()[0][0] > 0
    ...
    def __create_schema(self):
        ...
        with self.__db_con as db_con:
            ...
            self.tile_table_name = self.get_tiletable_name(cursor)
            ...
            if not self._srs_id_exists(cursor, 3857):
                wkt = """
            ...
            if not self._srs_id_exists(cursor, 4326):
                wkt = """
            ...
            if not self._srs_id_exists(cursor, 3395):
                wkt = """
            ...
            if not self._srs_id_exists(cursor, 9804):
                wkt = """
            ...
            if not self._srs_id_exists(cursor, -1):
                wkt = """undefined"""
                cursor.execute(
                    """
                    INSERT INTO gpkg_spatial_ref_sys (
            ...
            """, (self.tile_table_name, "tiles",
                  "Raster Tiles:%s" % self.tile_table_name,
    ...
    def update_metadata(self, metadata):
        ...
        with self.__db_con as db_con:
            ...
            for level in metadata:
                cursor.execute(
                    ...
                    (self.tile_table_name, level.zoom, level.matrix_width,
                     ...
                     self.__projection.pixel_size(
                         level.zoom), self.__projection.pixel_size(level.zoom)))
            contents_stmt = """
            ...
            cursor.execute(tile_matrix_set_stmt,
                           (self.tile_table_name, self.__srs, top_level.min_x,
    ...
    def assimilate(self, source):
        ...
        with self.__db_con as db_con:
            ...
            try:
                cursor.execute("""INSERT OR REPLACE INTO %s
                (zoom_level, tile_column, tile_row, tile_data)
                SELECT zoom_level, tile_column, tile_row, tile_data
                FROM source.tiles;""" % (self.tile_table_name))
                cursor.execute("detach source;")
            except Error as err:
                logger.info("Error: {}".format(type(err)))
                logger.info("Error msg:".format(err))
                raise
            remove(source)
...
class TempDB(object):
    ...
    def __init__(self, filename):
        ...
        self.__file_path = os.path.join(filename, self.name)
        self.__db_con = connect(self.__file_path)
        with self.__db_con as db_con:
            cursor = db_con.cursor()
            stmt = """
...
def file_count(base_dir):
    ...
    """
    logger.info("Calculating number of tiles, this could take a while...")
    file_list = []
    ...
    for root, sub_folders, files in walk(base_dir):
        temp_list = [
            os.path.join(root, f) for f in files if f.endswith(IMAGE_TYPES)
        ]
        file_list += temp_list
    logger.info("Found {} total tiles.".format(len(file_list)))
...
def allocate(cores, pool, file_list, extra_args):
    ...
    """
    if cores == 1:
        logger.info("Spawning worker with {} files".format(len(file_list)))
        return [pool.apply_async(sqlite_worker, [file_list, extra_args])]
    else:
        files = len(file_list)
        head = allocate(int(cores / 2), pool, file_list[:int(files / 2)],
                        extra_args)
        tail = allocate(int(cores / 2), pool, file_list[int(files / 2):],
                        extra_args)
        return head + tail
...
def build_lut(file_list, lower_left, srs):
    ...
    for zoom in zoom_levels:
        ...
        if zoom - 1 in [item for item in zoom_levels if item == (zoom - 1)]:
            ...
            x_vals = [
                int(item['x']) for item in file_list if int(item['z']) == zoom
            ]
            y_vals = [
                int(item['y']) for item in file_list if int(item['z']) == zoom
            ]
            ...
            x_width_max = max(
                [item['x'] for item in file_list if item['z'] == level.zoom])
            x_width_min = min(
                [item['x'] for item in file_list if item['z'] == level.zoom])
            level.matrix_width = (x_width_max - x_width_min) + 1
            y_height_max = max(
                [item['y'] for item in file_list if item['z'] == level.zoom])
            y_height_min = min(
                [item['y'] for item in file_list if item['z'] == level.zoom])
            level.matrix_height = (y_height_max - y_height_min) + 1
        level.min_x, level.min_y, level.max_x, level.max_y = calculate_top_left(
            level, projection, lower_left)
...
def calculate_top_left(level, projection, lower_left):
    if lower_left:
        ...
        min_x, max_y = projection.get_coord(level.zoom, level.min_tile_row,
                                            level.max_tile_col + 1)
        max_x, min_y = projection.get_coord(level.zoom, level.max_tile_row + 1,
                                            level.min_tile_col)
    else:
...
def build_lut_nsg(file_list, lower_left, srs, renumber):
    ...
    # Initialize a projection class
    if srs != 4326:
        logger.info("NSG Profile requires that -srs be set to 4326")
        exit(1)
    ...
    for zoom in zoom_levels:
        ...
        level.matrix_width, level.matrix_height = projection.get_matrix_size(
            level.zoom)
...
def combine_worker_dbs(out_geopackage):
    ...
    """
    base_dir = os.path.split(out_geopackage.file_path)[0]
    if base_dir == "":
        base_dir = "."
    glob_path = os.path.join(base_dir + '/*.gpkg.part')
    file_list = glob(glob_path)
    logger.info("Merging temporary databases...")
    ...
    for tdb in file_list:
        ...
        if counter != len(status) - 1:
            ...
            counter = 0
    logger.info(" All geopackages merged!")
...
def main(arg_list):
    ...
    if len(files) == 0:
        ...
        logger.info(" Ensure the correct source tile directory was specified.")
        exit(1)
    ...
    root_dir, _ = os.path.split(arg_list.output_file)
    # Build the tile matrix info object
    if arg_list.nsg_profile:
        tile_info = build_lut_nsg(files, lower_left, arg_list.srs,
                                  arg_list.renumber)
    else:
    ...
    if arg_list.threading:
        ...
        logger.info("Running in singhle thread")
        extra_args = dict(root_dir=root_dir,
                          tile_info=tile_info,
                          lower_left=lower_left,
                          srs=arg_list.srs,
...
if __name__ == '__main__':
    ...
    PARSER.add_argument(
        ...
        default=False,
        help=
        "Enable re-numbering tiles/zoom levels from standard Geodetic to NSG geodetic. Only valid"
    ...
    group.add_argument(
        ...
        dest="nsg_profile",
        help=
        "Enforce NSG Profile Requirements on output GeoPackage. Currently Requires data to"
    ...
    if not os.path.exists(ARG_LIST.source_folder):
        PARSER.print_usage()
        logger.info("Ensure that TMS directory exists and out file does not.")
        exit(1)
    if ARG_LIST.q is not None and ARG_LIST.imagery == 'png':
        PARSER.print_usage()
        logger.info("-q cannot be used with png")
        exit(1)
    if ARG_LIST.nsg_profile and ARG_LIST.srs != 4326:
        PARSER.print_usage()
        logger.info("-nsg requires that -srs be set to 4326")
        exit(1)
    if not ARG_LIST.nsg_profile and ARG_LIST.renumber:
        PARSER.print_usage()
        logger.info("-renumber requires that the -nsg flag also be active")
