...
def createBuffers(in_features, distances, field, units, dissolve_type, ring_type,
    ...
    analysisutils.AddTimerMessage(beginTime, "Describe {}".format(in_features))
...
def getFieldAlias(units):
    ...
    return "Buffer distance in {}".format(units.title())
...
def singleBuffer(in_features, out_features, distance, field, units, side_type, end_type, dissolve_type, geodesic, gcscode, calcField):

    startTime = time.time()

    # If there is units to apply, apply it
    if units.lower() not in ['default', '', '#']:
        bufdistance = "{} {}".format(distance, units)
    else:
    ...
    if geodesic:
        ...
        #spref = arcpy.env.outputCoordinateSystem
    ...
    if field:
        if (not dissolve_type or dissolve_type == "NONE"):
            fields = "{};{}".format(bufdistance, distancefieldname)
...
def ringBuffer(in_features, out_features, distances, units, geodesic, oidfieldname, spref, calcField):
    ...
    #debugUtils.debugFields(out_features)
