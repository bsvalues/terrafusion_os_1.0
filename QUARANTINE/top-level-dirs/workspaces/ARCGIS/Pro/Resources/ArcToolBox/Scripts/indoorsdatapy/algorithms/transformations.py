...
def make_similarity(src, dst, do_explicit=False):
    """
...
def projective_transform(coords, params, inverse=False):
    ...
    out = np.zeros(coords.shape)
    if inverse:
        out[:, 0] = (a2 * b0 - a0 * b2 + (b2 - b0 * c1) * x + (
                a0 * c1 - a2) * y) \
                    / (a1 * b2 - a2 * b1 + (b1 * c1 - b2 * c0) * x + (
                a2 * c0 - a1 * c1) * y)
        out[:, 1] = (a0 * b1 - a1 * b0 + (b0 * c0 - b1) * x + (
                a1 - a0 * c0) * y) \
                    / (a1 * b2 - a2 * b1 + (b1 * c1 - b2 * c0) * x + (
                a2 * c0 - a1 * c1) * y)
    else:
...
class HelmertHelper:
    @staticmethod
    def make_similarity_transform(src, dst, do_explicit=False):
        return make_similarity(src, dst, do_explicit)

    @staticmethod
    def similarity_transform(coords, params, inverse=False):
        return similarity_transform(coords, params, inverse)

    @staticmethod
    def transform_params2linear(translation_x, translation_y, scale, rotation):
        """

        :param translation_x: x offset (m)
        :param translation_y: y offset (m)
        :param scale: Scaling factor
        :param rotation: float rotation (degrees)
        :return:
        Linear params a0, a1, b0, b1
        """
        a0 = translation_x
        a1 = scale * np.cos(np.deg2rad(rotation))
        b0 = translation_y
        b1 = scale * np.sin(np.deg2rad(rotation))
        return a0, a1, b0, b1

    @staticmethod
    def transform_linear2params(a0, a1, b0, b1):
        """

        :param a0: linear param x offset (m)
        :param a1: linear param scale & rot
        :param b0: linear param z offset (m)
        :param b1: linear param scale & rot
        :return:
        transformation parameters
        translation_x, translation_y, scale, rotation
        """
        translation_x = a0
        translation_y = b0
        scale = np.sqrt(a1 * a1 + b1 * b1)
        rotation = np.arctan2(b1, a1)
        return translation_x, translation_y, scale, rotation

    @staticmethod
    def similarity_transform_positions2(positions, translation_x,
                                        translation_y, scale, rotation):
        """Apply similarity transform to positions"""
        params = HelmertHelper.transform_params2linear(
            translation_x, translation_y, scale, rotation)

        return similarity_transform(positions[["x", "y"]].values, params)

    @staticmethod
    def similarity_transform_positions(positions, params):
        """Apply similarity transform to positions"""
        return similarity_transform(positions[["x", "y"]].values, params)
