...
def create_name(workspace, name, extension):
    ...
    name = u'_'.join([str(i) for i in name])
...
def create_expression(in_data, field_name, value):
    ...
    if isinstance(value, numbers.Number):
        ...
        if not value.tzname():
            return u''' %s = timestamp '%s' ''' % ( delimited_field, value )
        else:
            return u''' %s = timestamp '%s' ''' % ( delimited_field, value.strftime('%Y-%m-%d %H:%M:%S') + ' ' +  value.tzname().replace('UTC', '') )
...
def select(datasettype, *args):
    """
    Data type non-specific Select tool handling

    :param datasettype: arcpy.Describe datasettype keyword
    :param args: arguments for Select/TableSelect tools
    :return:
    """

    feature_data = datasettype == 'FeatureClass'
...
def split_by_atts(in_data, out_workspace, fields):
    ...
    datasettype = arcpy.Describe(in_data).datasettype
    ...
    for i in unique_values:
        ...
        select(datasettype, in_data, output, expression)
        values_string = u', '.join([str(v) for v in i])
