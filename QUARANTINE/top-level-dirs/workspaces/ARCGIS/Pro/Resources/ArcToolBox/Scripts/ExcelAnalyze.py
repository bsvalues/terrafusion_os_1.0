...
Source: ExcelAnalyze.py
Author: ESRI

This function returns information about an excel file (*.xls)
...
from ExcelToTable import gen_out_fields
import xlrd
import json
...
def sheet_to_table_schema(workbook, sheet, validate_workspace, eval_count=None):
    ...
    gdbfields = gen_out_fields(sheet, eval_count=eval_count, check_for_MGRS=True, 
                               validate_workspace=validate_workspace, wb_datemode=workbook.datemode)
    fields = []
    for f in gdbfields:
...
def excel_analyze(in_excel, validate_workspace, eval_count=None):
    ...
    sheets = []
    for sheetId, sheet in enumerate(workbook.sheets()):
        fields = sheet_to_table_schema(workbook, sheet, validate_workspace, eval_count)
        sheet = {"name": sheet.name, "id":sheetId, "fields": fields }
        sheets.append(sheet)

    sheets_wrapper = { "sheets" : sheets }

    result = json.dumps(sheets_wrapper)
    return result
