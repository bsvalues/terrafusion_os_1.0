'''----------------------------------------------------------------------------------
 ...
 Required Arguments:  An input feature class
                        An output text file
...
def execute():
    ...
    try:
        ...
        if len(sys.argv) < 4:
            raise Exception(msgNotEnoughParams)
        inputFC = sys.argv[1]
        outFile = open(sys.argv[2], "w")

        arg3poss = ['default python output', 'locale decimal point', 'comma', 'period', '$sep$']
        if sys.argv[3].lower() not in arg3poss:
            raise Exception(msgUseValidDecimalPointSep)

        if sys.argv[3].lower() == arg3poss[1]:
            locale.setlocale(locale.LC_ALL, '')
            sepchar = locale.localeconv()['decimal_point']
        elif sys.argv[3].lower() == arg3poss[2]:
            sepchar = ','
        elif sys.argv[3].lower() == arg3poss[3]:
            sepchar = '.'
        elif sys.argv[3].lower() == arg3poss[4]:
            sepchar = '$SEP$'
        elif sys.argv[3].lower() == arg3poss[0]:
            sepchar = ""
        ...
        while inRow:
            ...
            if inDesc.ShapeType.lower() == "point":
                pnt = feat.getpart()
                outLine = str(inRow.GetValue(inDesc.OIDFieldName)) + " " + str(pnt.x) + " " + str(pnt.y) + " " + str(
                    pnt.z) + " " + str(pnt.m) + "\n"
                if sepchar == "":
                ...
                while partnum < partcount:
                    ...
                    outLine = str(partnum) + " " + str(pnt.x) + " " + str(pnt.y) + " " + str(pnt.z) + " " + str(
                        pnt.m) + "\n"
                    if sepchar == "":
                        outFile.write(outLine)
                    else:
                ...
                while partnum < partcount:
                    ...
                    pnt_count = 0
                    while pnt:
                        outLine = str(pnt_count) + " " + str(pnt.x) + " " + str(pnt.y) + " " + str(pnt.z) + " " + str(
                            pnt.m) + "\n"
                        if sepchar == "":
                            outFile.write(outLine)
                        else:
