...
def filterRois(rects, img_w, img_h, roi_min_area, roi_max_area, roi_min_side, roi_max_side, roi_max_aspect_ratio):
    ...
    # groupedRectangles, weights = cv2.groupRectangles(np.asanyarray(rectsInput, float).tolist(), 1, 0.3)
    # groupedRectangles = nms_python(np.asarray(rectsInput, float), 0.5)
...
def compute_targets(proposals, gt_rois, iou_threshold, normalize_means, normalize_stds):
    ...
    ex_gt_overlaps = bbox_overlaps(
        np.ascontiguousarray(proposals, dtype=float),
        np.ascontiguousarray(gt_rois, dtype=float))
