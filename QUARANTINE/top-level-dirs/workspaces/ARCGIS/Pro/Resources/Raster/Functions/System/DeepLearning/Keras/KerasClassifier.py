...
class ChildObjectDetector:
    ...
    def vectorize(self, **pixelBlocks):
        ...
        with self.graph.as_default():
            ...
            session.run(init)
            labels, confidences, classvalues = self.prediction(im=batch_images_resize, clf=clf)

        return rings, confidences, labels, classvalues

    def prediction(self, im, clf, image_dim = 64):
        # Need image to be in shape (?, 64, 64, 3)
        pr = clf.predict_on_batch(im)

        labels, confidences, classvalues = [], [], []
        for i in range(0, len(pr)):
            if np.argmax(pr[i]) == 0:
                labels.append(self.json_info['Classes'][0]['Name'])
                classvalues.append(self.json_info['Classes'][0]['Value'])
                confidences.append(pr[i][0])

            elif np.argmax(pr[i]) == 1:
                labels.append(self.json_info['Classes'][1]['Name'])
                classvalues.append(self.json_info['Classes'][1]['Value'])
                confidences.append(pr[i][1])

        return labels, confidences, classvalues
