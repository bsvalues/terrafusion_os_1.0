...
msg = """arcpy expects to run inside a Conda environment, which ArcGIS
...
Or by running the proenv.bat file in the terminal:
     C:\\> {}bin\\python\\scripts\\proenv.bat
"""
...
if sys.platform.startswith("win"):
    ...
    # previously checked sys.version, but it is no longer modified in Python 3.7
    if not os.path.exists(os.path.join(sys.prefix, 'conda-meta')):
        ...
        for (key_name, root_key) in root_keys:
            try:
            ...
            if key:
                try:
                    ...
                    pass
        if not install_dir:
            install_dir = "<ArcGIS Pro install directory>\\"
...
if sys.platform.startswith("win"):
    ...
    from arcpy.toolbox_linux import *
...
from arcgisscripting import SignInToPortal as _SignInToPortal
from arcgisscripting import ImportCredentials as _ImportCredentials
from arcgisscripting import ClearCredentials as _ClearCredentials
from arcpy import mp
from arcpy import sharing
from arcpy import metadata
import arcpy.charts
from arcpy.charts import Chart
from arcpy.charts import TimeBinningProperties
from arcpy.charts import Guide
...
# importing nax makes arcpy.na available which is not desirable on native linux since
# arcpy.na is deprecated on native linux
...
from arcpy.sa import RasterInfo

import arcpy.utils as utils
import arcpy.ia
from arcpy.ia import Mensuration
import arcpy._utbx
from arcpy.ia import Render
from arcpy.ia.util import GetSTACInfo

from arcpy._aio import AIO, AIOFileOpen, CloudPathType

from .cmanagers import EnvManager, PauseDrawing, ViewContext
...
# find site-package toolbox modules
sitepackpths = [pathlib.Path(f) for f in sys.path if (os.path.basename(f) == 'site-packages')]
module_paths += [f.glob('*/esri/arcpy/*.py') for f in sitepackpths]

# add toolbox modules to arcpy
for arcpy_module in chain.from_iterable(module_paths):
    try:
        loader = importlib.machinery.SourceFileLoader(arcpy_module.stem, str(arcpy_module))
        _arcpy_module = loader.load_module()
        ...
        pass
del sitepackpths, chain, pathlib, local_appdata, uuid_paths, module_paths
...
def GetPackageInfo(filename):
    ...
    return gp.getpackageinfo(filename)
...
def GetAllMessages():
    """GetAllMessages()
       ...
       Returns the message types, return codes and message strings from the
       previously run tool."""
    return gp.getAllMessages()
...
def SetMessageLevels(levels="NORMAL"):
    """SetMessageLevels(levels)

       Specifies whether geoprocessing tool messages will include additional
       categories of informative messages.

          levels(List):
       The levels of messages that will be included in geoprocessing tool
       messages. Valid message types are:

        * NORMAL:   Only standard tool messages will be included.

        * COMMANDSYNTAX:   The tool messages will include a command syntax
        message.

        * DIAGNOSTICS:   The tool messages will include diagnostic messages.

        * PROJECTIONTRANSFORMATION:    The tool messages will include
        geographic transformation-related messages."""
    k_levels = ['NORMAL', 'COMMANDSYNTAX', 'DIAGNOSTICS',
                'PROJECTIONTRANSFORMATION']

    if isinstance(levels, (list, tuple)):
        m_levels = [i.upper() if hasattr(i, 'upper') else i for i in levels]
    elif isinstance(levels, str):
        m_levels = [levels.upper()]
    else:
        raise TypeError(levels)

    for level in m_levels:
        if level not in k_levels:
            raise ValueError(f'{level} is not a valid value; supported values' \
                             f' are {", ".join(k_levels)}.')

    command_str = "SetGPMessageLevels " + " ".join(m_levels)

    r = gp.command(command_str)
    if r != "SUCCEEDED":
        # When early traps don't raise an exception
        raise ValueError(f"{levels} is not a valid value; supported values" \
                         f' are {", ".join(k_levels)}.')
...
def ListTransformations(from_sr, to_sr, extent=None, vertical=False,
                        first_only=False):
    ...
    return gp.listTransformations(from_sr, to_sr, extent, vertical, first_only)
...
def SetParameterSymbology(index, text):
    """SetParameterSymbology(index, text)
       ...
       """
    return gp.setParameterSymbology(index, text)
...
def ListRasterProducts(metadataFilePath):
    """ListRasterProducts(metadataFilePath)

       Returns a list of raster products associated with 
       the specified metadata file.

         metadataFilePath{String}:
       The path to the metadata file for which the list of raster products 
       should be returned."""
    return gp.listRasterProducts(metadataFilePath)
...
@_gptooldoc(None, [[["FeatureLayer", "Table", "TableView", "Dataset", "FeatureDataset", "RasterDataset", "RasterLayer", "TinLayer", "LasDataset", "LasDatasetLayer"], "", "", ""]])
...
def IsBeingEdited(dataset):
    ...
    return gp.isBeingEdited(dataset)
...
def GetParameterInfo(tool_name=None):
    ...
    class ParameterArray(list):
        def __init__(self, inner):
            super().__init__()
            fn_append = self.append
        ...
        def __getitem__(self, key):
            return self.__getattribute__(key) if isinstance(key, str) else super().__getitem__(key)
...
."""
    return gp.logLineage
def SetLogMetadata(log_metadata):
...
def FromGeohash(geohash_string):
    ...
    return gp.createObject("_geohash", geohash_string)
...
def LinearUnitConversionFactor(from_unit, to_unit):
    ...
    return gp.LinearUnitConversionFactor(from_unit, to_unit)
...
def ArealUnitConversionFactor(from_unit, to_unit):
    ...
    return gp.ArealUnitConversionFactor(from_unit, to_unit)
...
def CreateGeocodeSDDraft(loc_path, out_sddraft, service_name, server_type='ARCGIS_SERVER',
                         connection_file_path=None, copy_data_to_server=False, folder_name=None,
                         summary=None, tags=None, max_result_size=500, max_batch_size=1000,
                         suggested_batch_size=1000, supported_operations=["GEOCODE","REVERSE_GEOCODE","SUGGEST"],
    ...
    """CreateGeocodeSDDraft(loc_path, out_sddraft, service_name, {server_type},
       ...
       an existing service."""
    import arcgisscripting
    return arcgisscripting._createGeocodeSDDraft(loc_path, out_sddraft, service_name, server_type, connection_file_path,
                                                 copy_data_to_server, folder_name, summary, tags, max_result_size, max_batch_size,
                                                 suggested_batch_size, supported_operations, overwrite_existing_service)


def CreateGPSDDraft(result, out_sddraft, service_name,
                    server_type="ARCGIS_SERVER",
                    connection_file_path="", copy_data_to_server=True,
                    folder_name=None, summary=None, tags=None,
                    executionType="Asynchronous", resultMapServer=False,
                    showMessages="None", maximumRecords=1000, minInstances=1,
                    maxInstances=2, maxUsageTime=600, maxWaitTime=60,
                    maxIdleTime=1800, capabilities=None, constantValues=None,
                    choiceLists=None):
    """CreateGPSDDraft(result, out_sddraft, service_name, {server_type},
       ...
       """
    return gp.createGPSDDraft(result, out_sddraft, service_name, server_type,
                              connection_file_path, copy_data_to_server,
                              folder_name, summary, tags, executionType,
                              resultMapServer, showMessages, maximumRecords,
                              minInstances, maxInstances, maxUsageTime,
                              maxWaitTime, maxIdleTime, capabilities,
                              constantValues, choiceLists)
...
def GetPortalDescription(portal_URL=None):
    ...
    try:
        portal_desc = json.loads(gp.getPortalDescription(portal_URL))
    except (TypeError, ValueError):
        raise ValueError(gp.getidmessage(30101) % portal_URL) from None
...
def GetPortalInfo(portal_URL=None):
    """GetPortalInfo({portal_URL})
       ...
       The portal URL.
       By default it uses the URL returned by GetActivePortalURL."""
    return gp.getPortalInfo(portal_URL)
...
def DecryptPYT(toolbox=None, password=None):
    ...
    return gp.decryptPYT(toolbox, password)
...
def EncryptPYT(toolbox=None, password=None):
    ...
    return gp.encryptPYT(toolbox, password)
...
def GetImageEXIFProperties(*args, **kwargs):
    ...
    import arcgisscripting
    return arcgisscripting._getImageEXIFProperties(*args, **kwargs)
...
def GetUTMFromLocation(*args, **kwargs):
    ...
    import arcgisscripting
    return arcgisscripting._getUTMFromLocation(*args, **kwargs)
...
def SetRasterKeyMetadata(*args, **kwargs):
    ...
    import arcgisscripting
    return arcgisscripting._setRasterKeyMetadata(*args, **kwargs)
...
def GetRasterKeyMetadata(*args, **kwargs):
    ...
    import arcgisscripting
    return arcgisscripting._getRasterKeyMetadata(*args, **kwargs)

def SignInToPortal(*args, **kwargs):
    """SignInToPortal(portal_url, {username}, {password}, {cert_file}, {key_file})."""
    #identify NB_HOME keyword
    portal_url = args[0] if len(args) >= 1 else kwargs.get("portal_url")
    if portal_url and portal_url.lower() == "notebook":
        #identify NB Environment
        if os.environ.get("NB_AUTH_FILE"):
            try:
                from arcgis.gis import GIS as ARC_GIS
                #initializes NB_AUTH_FILE
                arc_gis = ARC_GIS("home")
                if arc_gis:
                    new_args = ()
                    new_kwargs = dict(
                        portal_url = arc_gis.url,
                        token = arc_gis._con.token,
                        token_referer = arc_gis._referer,
                        token_expiry = int(time.time() + arc_gis._expiration))
                    return _SignInToPortal(*new_args, **new_kwargs)
            except Exception as e:
                pass # just pass and try with original args as-is
    return _SignInToPortal(*args, **kwargs)

def ImportCredentials(*args, **kwargs):
    """ImportCredentials(secure_server_connections)."""
    return _ImportCredentials(*args, **kwargs)

def ClearCredentials(*args, **kwargs):
    """ClearCredentials(connections)."""
    return _ClearCredentials(*args, **kwargs)

def _PauseDrawing(*args, **kwargs):
    maps = []
    layouts = []
    context = args[1]
    try:
        proj = mp.ArcGISProject("current")
        if context in ViewContext.maps | ViewContext.maps_and_layouts:
            for m in proj.listMaps():
                maps.append(m._arc_object)
        if context in ViewContext.layouts | ViewContext.maps_and_layouts:
            for l in proj.listLayouts():
                layouts.append(l._arc_object)
    except OSError:
        return [], []
    import arcgisscripting
    return arcgisscripting._pauseDrawing(maps, layouts)

def _RestoreDrawing(*args, **kwargs):
    import arcgisscripting
    arcgisscripting._restoreDrawing(args[2], args[3])

def RefreshLayer(*args, **kwargs):
    """RefreshLayer(layer_name)."""
    layers = []
    layer_name = args[0]
    try: 
        proj = mp.ArcGISProject("current")
        if type(layer_name) not in (list, tuple):
            layer_name = [layer_name]
        for lyr in layer_name:
            if type(lyr) == arcpy._mp.Layer:
                layers.append(lyr._arc_object)
            elif type(lyr) == str:
                for m in proj.listMaps():
                    for layer in m.listLayers():
                        if layer.name == lyr:
                            layers.append(layer._arc_object)
    except OSError:
        return 
    
    if len(layers) > 0:
        import arcgisscripting
        arcgisscripting._refreshLayer(layers)

PauseDrawing.PauseDrawing = _PauseDrawing
PauseDrawing.RestoreDrawing = _RestoreDrawing

def SetupDebugger():
    """SetupDebugger()

       Configure the `debugpy` module to allow interactive debugging with tools that support
       it such as Visual Studio Code. This function is intended to be called automatically
       by the debugger when attaching to a Pro Python process."""
    import debugpy
