...
class Geometry:
    """Represents an abstract base for json-represented geometries on
       ...
       L{Point<_server_admin.geometry.Point>},
       L{Multipoint<_server_admin.geometry.Multipoint>},
       L{Polygon<_server_admin.geometry.Polygon>},
       L{Polyline<_server_admin.geometry.Polyline>} and
       L{Envelope<_server_admin.geometry.Envelope>} in this module for more
...
class SpatialReference(Geometry):
    ...
    def __init__(self, wkid):
        """Create a new Spatial Reference.
                ...
                >>> mysr = _server_admin.geometry.SpatialReference(4326)
                >>> mysr.name
                'GCS_WGS_1984'
                >>> mysr.wkid
                4326
                >>> myothersr = _server_admin.geometry.SpatialReference(mysr)
                >>> myothersr.wkid
                4326
    ...
    def __init__(self, x, y, spatialReference=None):
        ...
        @param spatialReference: The spatial reference, either as an instance
               ...
               have a spatial reference.
                    ...
                    >>> mysr = _server_admin.geometry.SpatialReference(4326)
                    >>> _server_admin.geometry.Point(10, 10, mysr)
...
class Polyline(Geometry):
    ...
    def __init__(self, paths=[], spatialReference=None):
        ...
        @param paths: A list of lists of points. Actual acceptable values are
                      ...
                      L{Point<_server_admin.geometry.Point>} or lists/tuples of
...
def fromJson(struct, attributes=None):
    ...
    if isinstance(struct, str) and len(struct.split(',')) == 4:
        return Envelope(*list(map(float, struct.split(','))))
    # Look for telltale attributes in the dict
    if isinstance(struct, dict):
        for key, cls in list(indicative_attributes.items()):
            if key in struct:
...
def fromGeoJson(struct, attributes=None):
    ...
    if struct['type'] == "Feature":
        ...
        if 'crs' in struct:
            ...
            members = list(map(fromGeoJson, struct['members']))
            for member in members:
                member.spatialReference = sr
            return members
        else:
            return list(map(fromGeoJson, struct['members']))
