...
shared_args.add_argument('-t', '--token',
                         ...
                         default=True)
shared_args.add_argument('--ignoressl',
                         required=False,
                         action='store_true',
                         help='Description: Disables SSL certificate verification',
                         default=False)
...
def should_ignore_ssl(ignoressl):
   if ignoressl:
        import ssl
...
def createservice(action):
    import _server_admin.admin as admin
    args = createserviceargs.parse_args()
    should_ignore_ssl(args.ignoressl)
    files = args.sdfile
    ...
    for filename in all_files:
        with action("uploading and publishing {0}".format(
            ...
            with action("deleting temporary {0} on server ({1})".format(
                ...
                try:
                    compat.urllib2.urlopen(delete_url, compat.ensure_bytes('')).read()
    ...
    if not args.list_clusters:
        ...
        with action("listing machines"):
            if args.list:
                ...
                for machine in list(itemobject.machines.keys()):
                    print("-", machine)
                print()
