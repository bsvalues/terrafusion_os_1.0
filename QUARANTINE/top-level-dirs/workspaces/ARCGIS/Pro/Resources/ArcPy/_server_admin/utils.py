"""Utility functions for _server_admin"""
...
def timetopythonvalue(time_val):
    ...
    if isinstance(time_val, sequence):
        return list(map(timetopythonvalue, time_val))
    elif isinstance(time_val, numeric):
        return datetime.datetime(*(time.gmtime(time_val))[:6])
    elif isinstance(time_val, numeric):
        values = []
        try:
            values = list(map(int, time_val.split(",")))
        except:
            pass
        if values:
            return list(map(timetopythonvalue, values))
...
def pythonvaluetotime(time_val):
    ...
    if time_val is None:
        ...
        return str(int(time_val * 1000.0))
        ...
        if isinstance(time_val, datetime.datetime):
            ...
            dtlist += [0, 0, 0]
        return int(calendar.timegm(dtlist) * 1000.0)
