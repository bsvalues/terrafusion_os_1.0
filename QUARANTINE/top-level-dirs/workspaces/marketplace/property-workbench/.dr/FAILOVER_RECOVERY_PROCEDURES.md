# Failover & Recovery Procedures for property-workbench
...
1. **Local Storage**
   - Path: `/var/backups/property-workbench`
   - Retention: 30 days
   - Purpose: Quick recovery

2. **AWS Cloud**
   - Bucket: `terrafusion-backups-property-workbench`
   - Region: us-east-1
   - Retention: 365 days
   - Purpose: Geographic redundancy

3. **Azure Cloud**
   - Container: `terrafusion-backups-property-workbench`
   - Region: eastus
   - Retention: 365 days
   - Purpose: Geographic redundancy
...
|--------|--------|--------|
| RTO (Recovery Time Objective) | 60 minutes | Monitored |
| RPO (Recovery Point Objective) | 2 hours | Monitored |
...
|----------|--------|-----------|-------------------|
| Primary database down | Automatic failover | 60 min | Automated system |
| Data corruption detected | Stop writes, restore from backup | 120 min | DBA + Team lead |
| Network partition | Manual failover assessment | 180 min | Operations manager |
| Multi-region failure | Initiate full disaster recovery | 300 min | Disaster recovery team |
