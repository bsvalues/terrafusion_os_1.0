...
describe('Terra Flow API Tests', () => {
  ...
  beforeAll(async () => {
    ...
    authToken = await generateTestToken({
      ...
      scopes: ['terra-flow:read', 'terra-flow:write'],
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /health', async () => {
      const response = await testClient
        .get('/api/v1/terra-flow/health')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'terra-flow');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /data', async () => {
      const response = await testClient
        .get('/api/v1/terra-flow/data')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'terra-flow');
    ...
    test('should handle POST /operations', async () => {
      const response = await testClient
        .post('/api/v1/terra-flow/operations')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'terra-flow');
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/terra-flow/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'terra-flow-api-tests',
  security_level: 'MEDIUM',
  endpoints_tested: 6,
