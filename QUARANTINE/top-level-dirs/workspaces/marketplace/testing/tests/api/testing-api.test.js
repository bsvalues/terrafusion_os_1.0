...
describe('Testing API Tests', () => {
  ...
  beforeAll(async () => {
    ...
    authToken = await generateTestToken({
      workspace: 'testing',
      scopes: ['testing:read', 'testing:write'],
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /health', async () => {
      const response = await testClient
        .get('/api/v1/testing/health')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'testing');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /data', async () => {
      const response = await testClient
        .get('/api/v1/testing/data')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'testing');
    ...
    test('should handle POST /operations', async () => {
      const response = await testClient
        .post('/api/v1/testing/operations')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'testing');
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/testing/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'testing-api-tests',
  security_level: 'MEDIUM',
  endpoints_tested: 6,
