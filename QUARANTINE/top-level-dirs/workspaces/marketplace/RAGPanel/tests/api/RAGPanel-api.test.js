...
describe('Ragpanel API Tests', () => {
  ...
  beforeAll(async () => {
    ...
    authToken = await generateTestToken({
      workspace: 'RAGPanel',
      scopes: ['RAGPanel:read', 'RAGPanel:write'],
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /health', async () => {
      const response = await testClient
        .get('/api/v1/RAGPanel/health')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'RAGPanel');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /data', async () => {
      const response = await testClient
        .get('/api/v1/RAGPanel/data')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'RAGPanel');
    ...
    test('should handle POST /operations', async () => {
      const response = await testClient
        .post('/api/v1/RAGPanel/operations')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'RAGPanel');
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/RAGPanel/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'RAGPanel-api-tests',
  security_level: 'MEDIUM',
  endpoints_tested: 6,
