...
describe('Leafscope API Tests', () => {
  ...
  beforeAll(async () => {
    ...
    authToken = await generateTestToken({
      workspace: 'LeafScope',
      scopes: ['LeafScope:read', 'LeafScope:write'],
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /health', async () => {
      const response = await testClient
        .get('/api/v1/LeafScope/health')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'LeafScope');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /data', async () => {
      const response = await testClient
        .get('/api/v1/LeafScope/data')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'LeafScope');
    ...
    test('should handle POST /operations', async () => {
      const response = await testClient
        .post('/api/v1/LeafScope/operations')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'LeafScope');
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/LeafScope/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'LeafScope-api-tests',
  security_level: 'MEDIUM',
  endpoints_tested: 6,
