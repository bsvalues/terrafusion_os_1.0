...
describe('Specialized API Tests', () => {
  let authToken;
  let testClient;
  ...
  beforeAll(async () => {
    ...
    authToken = await generateTestToken({
      ...
      scopes: ['specialized:read', 'specialized:write'],
  ...
  describe('🏥 Health & Monitoring', () => {
    ...
    test('should provide metrics endpoint', async () => {
      const response = await testClient
        .get('/api/v1/specialized/metrics')
        .set('Authorization', `Bearer ${authToken}`)
        .expect(200);
  ...
  describe('🎯 Primary Endpoints', () => {
    test('should handle GET /health', async () => {
      const response = await testClient
        .get('/api/v1/specialized/health')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'specialized');

      expect([200, 201, 204]).toContain(response.status);
    });
    test('should handle GET /data', async () => {
      const response = await testClient
        .get('/api/v1/specialized/data')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'specialized');
    ...
    test('should handle POST /operations', async () => {
      const response = await testClient
        .post('/api/v1/specialized/operations')
        .set('Authorization', `Bearer ${authToken}`)
        .set('X-TerraFusion-Workspace', 'specialized');
  ...
  afterAll(async () => {
    ...
    await testClient
      .delete('/api/v1/specialized/test-cleanup')
      .set('Authorization', `Bearer ${authToken}`);
  });
});

module.exports = {
  testSuite: 'specialized-api-tests',
  security_level: 'MEDIUM',
  endpoints_tested: 6,
