...
export class CMAAutomationEngine {
  ...
  async findComparableProperties(targetProperty: Property): Promise<ComparableProperty[]> {
    try {
      ...
      const adjusted = topComparables.map(comp => 
        this.calculateAdjustments(comp, targetProperty)
      );
  ...
  private async searchComparableCandidates(property: Property): Promise<any[]> {
    ...
    for (let i = 0; i < 20; i++) {
      candidates.push({
        ...
        soldPrice: property.price ? property.price * (0.8 + Math.random() * 0.4) : 300000 + Math.random() * 200000,
        listPrice: property.price ? property.price * (0.85 + Math.random() * 0.3) : 320000 + Math.random() * 180000,
  ...
  private calculateSimilarityScore(comparable: any, property: Property): number {
    ...
    // Size similarity
    const sizeRatio = Math.min(comparable.sqft, property.sqft) / Math.max(comparable.sqft, property.sqft);
    score *= sizeRatio;
  ...
  private calculateAdjustments(comparable: any, property: Property): ComparableProperty {
    ...
    adjustments.features = bedroomDiff * this.adjustmentFactors.bedroom + 
                          bathroomDiff * this.adjustmentFactors.bathroom;
    ...
    adjustments.location = comparable.soldPrice * 
      (comparable.location.distance / this.maxDistance) * this.adjustmentFactors.location;
  ...
  private calculateValueFromComparables(comparables: ComparableProperty[]): number {
    ...
    const weightedSum = comparables.reduce((sum, comp) => 
      sum + (comp.adjustedPrice * comp.relevanceWeight), 0);
  ...
  private getSeasonalityAdjustment(): number {
    ...
    const seasonalFactors = [0.98, 0.99, 1.02, 1.05, 1.08, 1.06, 1.04, 1.02, 1.00, 0.99, 0.97, 0.96];
  ...
  private calculateConfidenceInterval(comparables: ComparableProperty[], predictedValue: number): any {
  ...
  private getMethodologyDescription(comparables: ComparableProperty[], marketAdjustments: any): string[] {
    return [
  ...
  private calculateCompetitiveAnalysis(property: Property, activeListings: any[], recentSales: any[]): any {
    const allPrices = [...activeListings.map(l => l.listPrice), ...recentSales.map(s => s.soldPrice)];
    ...
    const marketPricePerSqft = allPricesPerSqft.reduce((sum, price) => sum + price, 0) / allPricesPerSqft.length;
  ...
  private compareOverall(property: Property, competitor: any): 'better' | 'similar' | 'worse' {
    const propertyScore = property.sqft * 0.4 + property.bedrooms * 20 + property.bathrooms * 15 + 
                         (2025 - property.yearBuilt) * 0.1;
    const competitorScore = competitor.sqft * 0.4 + competitor.bedrooms * 20 + competitor.bathrooms * 15 + 
  ...
  private determineMarketPosition(property: Property, competitiveAnalysis: any): 'underpriced' | 'fairly_priced' | 'overpriced' {
    const priceRatio = (property.price || competitiveAnalysis.averagePrice) / competitiveAnalysis.averagePrice;
  ...
  private async generatePricingRecommendation(property: Property, analysis: any, competitors: any[]): Promise<any> {
  ...
  private assessRecency(comparables: ComparableProperty[]): number {
    const avgAge = comparables.reduce((sum, comp) => {
