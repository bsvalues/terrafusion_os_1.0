...
// Cast ReactFlow components to avoid type conflicts
    ...
    <div className="flex flex-col h-[calc(100vh-6rem)] gap-6">
      <div className="flex items-center justify-between">
        <div className="space-y-2">
          <Label htmlFor="pipeline-name" className="tf-clarity-gradient">Pipeline Name</Label>
          <Input
            ...
            className="w-64 tf-transcendence-glow"
          />
        </div>
        <Button
      ...
      <div className="flex gap-6 flex-1">
        <div className="w-48 space-y-4">
          <h2 className="font-semibold tf-clarity-gradient">Nodes</h2>
          <div className="space-y-2">
            {['source', 'transform', 'filter', 'join', 'output'].map((type) => (
              <div
                key={type}
                className="p-2 border rounded cursor-move bg-background tf-government-badge"
                draggable
                onDragStart={(e) => {
        ...
        <div className="flex-1 border rounded-lg bg-background/50 tf-transcendence-glow">
          <ReactFlowComponent
            nodes={nodes}
            edges={edges}
