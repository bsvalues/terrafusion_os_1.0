      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>Current status breakdown of all workflows</CardDescription>
          </div>
          <div className="bg-blue-100 p-2 rounded-full">
          ...
          {chartData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                ...
                <Tooltip formatter={(value: number) => [`${value} workflows`, 'Count']} />
                <Legend />
              </PieChart>
            </ResponsiveContainer>
          ) : (
...
const WorkflowTypeChart = ({ workflows }: { workflows: Workflow[] }) => {
  const chartData = useMemo(() => {
    ...
    return Object.entries(typeCounts).map(([type, count]) => ({
      type: workflowTypeLabels[type as WorkflowType] || type,
      count
    }));
  }, [workflows]);
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>Distribution by workflow category</CardDescription>
          </div>
...
const RecentActivity = ({ events }: { events: WorkflowEvent[] }) => {
  ...
  const getEventIcon = (eventType: string) => {
    switch (eventType) {
      case 'created': return <FileCheck className="h-4 w-4 text-blue-600" />;
      case 'status_changed': return <Activity className="h-4 w-4 text-green-600" />;
      case 'assigned': return <UserIcon className="h-4 w-4 text-purple-600" />;
      case 'comment_added': return <FileText className="h-4 w-4 text-orange-600" />;
        ...
        {recentEvents.length > 0 ? (
            ...
            <div className="space-y-4">
              {recentEvents.map((event) => (
                <div key={event.id} className="flex items-start gap-3">
                  <div className="bg-neutral-100 p-2 rounded-full h-8 w-8 flex items-center justify-center mt-0.5">
          ...
          <div className="flex flex-col items-center justify-center h-[250px] text-center">
            ...
            <p className="text-sm text-muted-foreground mt-1">
              Activity will appear here as workflows are updated
            </p>
          </div>
        )}
      </CardContent>
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>Your document types overview</CardDescription>
          </div>
          <div className="bg-emerald-100 p-2 rounded-full">
            <FileDigit className="h-5 w-5 text-emerald-600" />
          </div>
        </div>
      </CardHeader>
      <CardContent>
        <div className="h-[240px]">
          {classificationData.length > 0 ? (
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={classificationData} layout="horizontal">
                <CartesianGrid strokeDasharray="3 3" />
                <XAxis type="number" />
            ...
            <div className="flex flex-col items-center justify-center h-full text-center">
              <FileText className="h-12 w-12 text-gray-100 mb-3" />
              <h3 className="text-base font-medium text-neutral-600">No Documents</h3>
              <p className="text-sm text-muted-foreground mt-1">
                Document classification will appear here
              </p>
            </div>
          )}
        </div>
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>Your high priority incomplete tasks</CardDescription>
          </div>
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>Personalized workflow completion insights</CardDescription>
          </div>
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          ...
          <div className="p-3 bg-green-50 rounded-lg">
            ...
            <div className="mt-2 flex items-end gap-1">
              ...
              <span className="text-sm text-green-600 mb-0.5">completed</span>
            </div>
          </div>
        </div>
...
const WorkflowProgress = ({ workflows, states }: { workflows: Workflow[], states: any[] }) => {
  const [, navigate] = useLocation();
    ...
    return workflows
      ...
      .map(workflow => {
        ...
        return {
          ...
          progress: steps.length > 0 ? Math.round(((currentStepIndex + 1) / steps.length) * 100) : 0,
  ...
  return (
    <Card className="h-full">
      <CardHeader className="pb-2">
        ...
        <CardDescription>Track your active workflows</CardDescription>
      </CardHeader>
      <CardContent>
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>Personal vs. team workload</CardDescription>
          </div>
          <div className="bg-blue-100 p-2 rounded-full">
    ...
    return workflows
      ...
      .map(workflow => {
        ...
        if (workflow.priority === 'high') {
          deadline = addDays(now, Math.floor(Math.random() * 3) + 1);
        } else if (workflow.priority === 'medium') {
          deadline = addDays(now, Math.floor(Math.random() * 7) + 3);
        } else {
          deadline = addDays(now, Math.floor(Math.random() * 14) + 7);
        }
      ...
      <CardHeader className="pb-2">
        <div className="flex justify-between items-center">
          <div>
            ...
            <CardDescription>Your workflow due dates</CardDescription>
          </div>
          ...
          <ScrollArea className="h-[250px] pr-4">
            <div className="space-y-4">
              {deadlines.map((deadline) => (
                <div key={deadline.workflowId} className="flex items-start gap-3">
                  <div className={`p-2 rounded-full h-8 w-8 flex items-center justify-center mt-0.5 ${
                    deadline.daysRemaining <= 1 ? 'bg-red-100' : 
                    deadline.daysRemaining <= 3 ? 'bg-orange-100' : 'bg-blue-100'
                  }`}>
                    <Hourglass className={`h-4 w-4 ${
                      deadline.daysRemaining <= 1 ? 'text-red-600' : 
                      deadline.daysRemaining <= 3 ? 'text-orange-600' : 'text-blue-600'
                    }`} />
                  </div>
                  <div className="flex-1">
                    <div className="flex justify-between">
                      <h4 className="text-sm font-medium">{deadline.workflowTitle}</h4>
                      <Badge variant={deadline.priority === 'high' ? 'destructive' : 'outline'} className="text-xs">
                        {deadline.daysRemaining <= 0 ? 'Overdue' : `${deadline.daysRemaining}d`}
                      </Badge>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">
                      Due: {format(deadline.deadline, 'MMM dd, yyyy')}
                    </p>
                  </div>
                </div>
              ))}
...
export default function DashboardPage() {
  ...
  const { data: workflowStates, isLoading: isStatesLoading } = useQuery({
    queryKey: ['workflow-states'],
    queryFn: getQueryFn('/api/workflow-states')
  });

  const { data: workflowEvents, isLoading: isEventsLoading } = useQuery({
    queryKey: ['workflow-events'],
    queryFn: getQueryFn('/api/workflow-events')
  });

  const { data: checklistItems, isLoading: isChecklistLoading } = useQuery({
