...
const UserPresence: React.FC<UserPresenceProps> = ({
  ...
  const [presenceData, setPresenceData] = useState<UserPresenceData[]>(users);
  ...
  const [sortBy, setSortBy] = useState<'name' | 'status' | 'role' | 'joinedAt' | 'activity'>('name');
  ...
  const samplePresenceData: UserPresenceData[] = useMemo(() => [
    {
      ...
      activity: {
        currentAction: 'Editing Environmental Sensors layer',
    ...
    },
    {
      ...
      activity: {
        currentAction: 'Presenting analysis results',
    ...
    },
    {
      ...
      activity: {
        currentAction: 'Reviewing analysis results',
  ...
  useEffect(() => {
    if (users.length === 0) {
      setPresenceData(samplePresenceData);
    } else {
    ...
    if (realTimeUpdates) {
      updateIntervalRef.current = setInterval(() => {
        setPresenceData(prev => prev.map(user => {
          ...
          if (activityUpdate < 0.1) { // 10% chance of status change
  ...
  const filteredAndSortedUsers = useMemo(() => {
    let filtered = presenceData.filter(user => {
      // Status filter
      ...
      // Device filter
      if (filter.devices.length > 0 && !user.devices.some(device => filter.devices.includes(device.type))) {
        return false;
      }

      // Offline filter
      if (!filter.showOffline && user.status === 'offline') {
        return false;
      }
    ...
    filtered.sort((a, b) => {
      ...
      switch (sortBy) {
        case 'name':
          ...
          break;
        case 'status':
          const statusOrder = { 'online': 1, 'presenting': 2, 'busy': 3, 'away': 4, 'idle': 5, 'dnd': 6, 'offline': 7 };
  ...
  const handleUserSelect = (user: UserPresenceData) => {
  ...
  const getStatusDisplay = (status: UserPresenceData['status']) => {
    switch (status) {
      case 'online': return { icon: Circle, color: 'text-green-500', bg: 'bg-green-500' };
      case 'presenting': return { icon: ScreenShare, color: 'text-purple-500', bg: 'bg-purple-500' };
      case 'busy': return { icon: Circle, color: 'text-red-500', bg: 'bg-red-500' };
      case 'away': return { icon: Schedule, color: 'text-yellow-500', bg: 'bg-yellow-500' };
      case 'idle': return { icon: AccessTime, color: 'text-gray-500', bg: 'bg-gray-500' };
      case 'dnd': return { icon: Circle, color: 'text-red-600', bg: 'bg-red-600' };
      default: return { icon: Circle, color: 'text-gray-400', bg: 'bg-gray-400' };
    }
  };

  // Get role icon
  const getRoleIcon = (role: UserPresenceData['role']) => {
    switch (role) {
      case 'owner': return <SupervisorAccount className="h-3 w-3 text-yellow-600" />;
      case 'admin': return <AdminPanelSettings className="h-3 w-3 text-red-600" />;
      case 'editor': return <Edit className="h-3 w-3 text-blue-600" />;
      case 'viewer': return <Visibility className="h-3 w-3 text-green-600" />;
      case 'guest': return <Person className="h-3 w-3 text-purple-600" />;
      case 'observer': return <Visibility className="h-3 w-3 text-gray-600" />;
      default: return <Person className="h-3 w-3 text-gray-600" />;
    }
  };

  // Get device icon
  const getDeviceIcon = (deviceType: UserDevice['type']) => {
    switch (deviceType) {
      case 'desktop': return <Computer className="h-3 w-3" />;
      case 'mobile': return <Smartphone className="h-3 w-3" />;
      case 'tablet': return <Tablet className="h-3 w-3" />;
      case 'watch': return <Watch className="h-3 w-3" />;
      default: return <DeviceHub className="h-3 w-3" />;
    }
  };

  // Get network icon
  const getNetworkIcon = (network: UserDevice['performance']['network']) => {
    switch (network) {
      case 'fast': return <SignalWifi4Bar className="h-3 w-3 text-green-600" />;
      case 'medium': return <SignalWifi4Bar className="h-3 w-3 text-yellow-600" />;
      case 'slow': return <Battery1Bar className="h-3 w-3 text-red-600" />;
      case 'offline': return <SignalWifiOff className="h-3 w-3 text-gray-600" />;
      default: return <SignalWifi4Bar className="h-3 w-3 text-gray-600" />;
    }
  };
      ...
      >
        <div className="flex items-start gap-3">
          <div className="relative flex-shrink-0">
            ...
            {/* Status indicator */}
            <div className={`absolute -bottom-0.5 -right-0.5 h-4 w-4 rounded-full border-2 border-white ${statusDisplay.bg} flex items-center justify-center`}>
              {user.status === 'presenting' && <ScreenShare className="h-2 w-2 text-white" />}
            </div>
            
            {/* Following indicator */}
            {user.collaborationData.isFollowing && (
              <div className="absolute -top-1 -left-1 h-3 w-3 rounded-full bg-blue-500 border border-white flex items-center justify-center">
                <PersonPin className="h-2 w-2 text-white" />
              </div>
            )}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            {viewMode === 'detailed' && (
              <div className="space-y-1 text-xs text-muted-foreground">
                {user.activity.currentAction && (
                  <div className="flex items-center gap-1">
                    <Edit className="h-3 w-3" />
                    <span className="truncate">{user.activity.currentAction}</span>
                  </div>
                )}
                
                {user.location?.isShared && user.location.city && (
                  <div className="flex items-center gap-1">
                ...
                <div className="flex items-center gap-1">
                  ...
                  <span>Active {formatDuration(user.activity.totalSessionTime)}</span>
                </div>
                
                {primaryDevice && (
                  <div className="flex items-center gap-2">
                    {getNetworkIcon(primaryDevice.performance.network)}
                    <Progress value={100 - primaryDevice.performance.cpu} className="flex-1 h-1" />
                    {primaryDevice.performance.battery && (
                      <span>{primaryDevice.performance.battery}%</span>
                    )}
                  </div>
                )}
              </div>
            ...
            {user.collaborationData.followers && user.collaborationData.followers.length > 0 && (
              <div className="flex items-center gap-1 mt-2">
                <PersonPin className="h-3 w-3 text-blue-600" />
                <span className="text-xs text-blue-600">
                  {user.collaborationData.followers.length} follower{user.collaborationData.followers.length > 1 ? 's' : ''}
                </span>
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-1">
            {!isCurrentUser && (
              <div className="flex gap-1">
                ...
                {onFollowUser && !user.collaborationData.followers?.includes(currentUser.id) && (
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
                ...
                {onVoiceCall && user.devices.some(d => d.capabilities.audio) && (
                  <TooltipProvider>
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <Button
            ...
            {allowUserManagement && !isCurrentUser && (
              <Button
                size="sm"
                variant="ghost"
                onClick={(e) => {
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="flex items-center gap-2">
              ...
              User Presence ({filteredAndSortedUsers.length})
            ...
            <div className="flex items-center gap-2">
              <Button
                ...
                onClick={() => setViewMode(
                  ...
                  viewMode === 'detailed' ? 'grid' : 'compact'
                )}
              >
                {viewMode === 'compact' ? 'Compact' : 
                 viewMode === 'detailed' ? 'Detailed' : 'Grid'}
              </Button>
            ...
            <div className="flex items-center gap-2 text-xs">
              ...
              {bulkActions.length > 0 && (
                <div className="flex items-center gap-1 ml-auto">
                  <span className="text-muted-foreground">{bulkActions.length} selected</span>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="h-full flex flex-col">
            <TabsList className="grid w-full grid-cols-4 mb-4">
              <TabsTrigger value="online">
                Online ({usersByStatus.online?.length || 0})
              </TabsTrigger>
              <TabsTrigger value="all">
                All ({filteredAndSortedUsers.length})
              </TabsTrigger>
            ...
            <TabsContent value="online" className="flex-1 flex flex-col min-h-0">
              <ScrollArea className="flex-1">
                <div className={`space-y-3 ${viewMode === 'grid' ? 'grid grid-cols-2 gap-3 space-y-0' : ''}`}>
                  {usersByStatus.online?.map(renderUserCard) || (
            ...
            <TabsContent value="activity" className="flex-1">
              <div className="text-center py-8">
                ...
                <p className="text-muted-foreground">
                  Real-time user activity and collaboration events.
                </p>
              </div>
            </TabsContent>
            ...
            <TabsContent value="stats" className="flex-1">
              <div className="text-center py-8">
                ...
                <h3 className="text-lg font-medium mb-2">Presence Statistics</h3>
                <p className="text-muted-foreground">
                  User engagement and collaboration analytics.
                </p>
              </div>
            </TabsContent>
          </Tabs>
