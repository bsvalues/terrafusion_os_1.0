                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                    ...
                    <div className="space-y-1 leading-none">
                      <FormLabel>
                        Use secure connection (SFTP)
                      </FormLabel>
                      <FormDescription>
                        Enable this if the server requires an encrypted connection
                      </FormDescription>
                    </div>
                  </FormItem>
                )}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
            <TabsList className="grid w-full grid-cols-3">
              ...
              <TabsTrigger value="transfers">
                Transfers 
                {transfers.filter(t => t.status === 'in_progress').length > 0 && (
              ...
              {loadingFiles ? (
                <div className="flex justify-center py-8">
                  <div className="text-center">
                    <RefreshIcon className="animate-spin mx-auto mb-2" size={24} />
                ...
                <div className="border rounded-lg">
                  <div className="bg-gray-50 px-4 py-2 border-b flex items-center">
                    <Checkbox 
                      ...
                      className="mr-3"
                    />
                    <div className="grid grid-cols-4 w-full text-sm font-medium text-gray-700">
                      <div>Name</div>
                      <div>Size</div>
                      <div>Modified</div>
                      <div>Actions</div>
                  ...
                  <ScrollArea className="max-h-96">
                    {files.length === 0 ? (
                      ...
                      files.map((file) => (
                        <div key={file.name} className="px-4 py-2 border-b hover:bg-gray-50 flex items-center">
                          <Checkbox 
                            ...
                            <div className="flex space-x-1">
                              ...
                              <TooltipProvider>
                                <Tooltip>
                                  ...
                                  <TooltipContent>
                                    <p>Delete {file.isDirectory ? 'directory' : 'file'}</p>
                                  </TooltipContent>
                                </Tooltip>
                              </TooltipProvider>
                            </div>
            ...
            <TabsContent value="upload" className="space-y-4">
              <Alert>
                ...
                <AlertDescription>
                  Select the type of data you're uploading and choose your files. Files will be uploaded to the current directory: {currentPath}
                </AlertDescription>
              </Alert>
            ...
            <TabsContent value="transfers" className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-medium">File Transfers</h3>
                <Button 
                  variant="outline" 
                  size="sm" 
                  onClick={clearCompletedTransfers}
                  disabled={transfers.filter(t => t.status === 'completed' || t.status === 'failed').length === 0}
                >
                  Clear Completed
                </Button>
              </div>
              ...
              {transfers.length === 0 ? (
                <div className="text-center py-8 text-gray-500">
                  <UploadIcon size={48} className="mx-auto mb-2 opacity-50" />
                  <p>No transfers in progress</p>
                </div>
              ) : (
                <div className="space-y-2">
                  {transfers.map((transfer, index) => (
                    <Card key={`${transfer.filename}-${index}`}>
                      <CardContent className="pt-4">
                        <div className="flex items-center justify-between mb-2">
                          <div className="flex items-center space-x-2">
                            {transfer.direction === 'upload' ? (
                              ...
                              <DownloadIcon size={16} className="text-green-500" />
                            )}
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            <Badge 
                              variant={
                                ...
                                transfer.status === 'in_progress' ? 'secondary' : 'outline'
                              }
                            >
                        ...
                        <div className="space-y-2">
                          <Progress value={transfer.percentComplete} className="w-full" />
                          <div className="flex justify-between text-xs text-gray-600">
                            <span>
                              {formatBytes(transfer.bytesTransferred)} / {formatBytes(transfer.totalBytes)}
                            </span>
                            <span>{transfer.percentComplete}%</span>
                          </div>
                          {transfer.error && (
