...
const NeuralNetwork3D: React.FC<{
...
}> = ({ agents, connections, signals, clusters, visualizationMode, showConnections, activityLevel }) => {
  ...
  const getAgentColor = (agent: NeuralAgent) => {
    switch (agent.type) {
      case 'SUPREME_COMMANDER': return '#ff00ff';
      case 'FIELD_GENERAL': return '#ff6b6b';
      case 'SQUAD_LEADER': return '#4ecdc4';
      case 'SPECIALIST': return '#45b7d1';
      case 'WORKER': return '#96ceb4';
      default: return '#ffffff';
    }
  };

  const getAgentSize = (agent: NeuralAgent) => {
    const baseSize = {
  ...
  const getConnectionColor = (connection: NeuralConnection) => {
    switch (connection.type) {
      case 'COMMAND': return '#ff0000';
      case 'DATA': return '#00ff00';
      case 'FEEDBACK': return '#0000ff';
      case 'COORDINATION': return '#ffff00';
      case 'LEARNING': return '#ff00ff';
      default: return '#ffffff';
    }
  };

  const getSignalColor = (signal: NeuralSignal) => {
    switch (signal.type) {
      case 'COMMAND': return '#ff4444';
      case 'DATA': return '#44ff44';
      case 'LEARNING': return '#4444ff';
  ...
  const layeredAgents = useMemo(() => {
    if (visualizationMode !== 'LAYERS') return agents;
  ...
  return (
    <group ref={groupRef}>
        ...
        <group>
          ...
          {agents.map((agent) => (
            <group key={agent.id} position={agent.position}>
              {/* Main agent node */}
              <mesh>
                <sphereGeometry args={[getAgentSize(agent), 16, 16]} />
              ...
              {/* Activation aura */}
              <mesh>
                <sphereGeometry args={[getAgentSize(agent) * 2, 12, 12]} />
                <meshBasicMaterial
                  color={getAgentColor(agent)}
                  transparent
              ...
              {agent.processingLoad > 0.7 && (
                <mesh rotation={[0, time * 5, 0]}>
                  <torusGeometry args={[getAgentSize(agent) * 1.5, getAgentSize(agent) * 0.3, 8, 16]} />
                  <meshBasicMaterial
                    color="#ffffff"
                    transparent
                    opacity={0.6}
              ...
              {agent.type === 'SUPREME_COMMANDER' && (
                <Billboard position={[0, getAgentSize(agent) + 0.5, 0]}>
                  <Text
                    fontSize={0.2}
                    color="#ff00ff"
                    anchorX="center"
      ...
      {visualizationMode === 'LAYERS' && (
        <group>
          {/* Neural layers */}
          {Array.from({ length: 11 }, (_, layer) => {
            const agentsInLayer = layeredAgents.filter(a => a.neuralLayer === layer);
            const layerActivation = agentsInLayer.reduce((sum, a) => sum + a.activation, 0) / agentsInLayer.length;
            
            return (
              <group key={layer}>
                {/* Layer ring */}
                <mesh position={[0, (layer - 5) * 2, 0]} rotation={[Math.PI / 2, 0, 0]}>
                  <ringGeometry args={[3 + layer * 1.5 - 0.1, 3 + layer * 1.5 + 0.1, 32]} />
                  <meshBasicMaterial
                    color={layer === 0 ? '#ff00ff' : layer < 3 ? '#ff6b6b' : layer < 8 ? '#4ecdc4' : '#96ceb4'}
                    transparent
                    opacity={0.3 + layerActivation * 0.4}
                  />
                </mesh>
                ...
                {agentsInLayer.map((agent) => (
                  <group key={agent.id} position={agent.position}>
                    <mesh>
                      <sphereGeometry args={[getAgentSize(agent), 12, 12]} />
                      <meshStandardMaterial
                        color={getAgentColor(agent)}
                        emissive={getAgentColor(agent)}
                        emissiveIntensity={agent.activation * 0.3}
                      />
                    </mesh>
                  </group>
                ))}
                ...
                {/* Layer label */}
                <Billboard position={[0, (layer - 5) * 2 + 1, 0]}>
                  <Text
                    fontSize={0.3}
                    color="white"
                    anchorX="center"
          ...
          {clusters.map((cluster) => (
            <group key={cluster.id} position={cluster.position}>
              ...
              {cluster.agents.map((agentId) => {
                ...
                return (
                  <mesh key={agentId} position={localPos}>
                    <sphereGeometry args={[getAgentSize(agent), 8, 8]} />
                    <meshStandardMaterial
                      color={getAgentColor(agent)}
                      emissive={getAgentColor(agent)}
                      emissiveIntensity={agent.activation * 0.3}
              ...
              {/* Cluster label */}
              <Billboard position={[0, cluster.radius + 1, 0]}>
                <Text
                  fontSize={0.4}
                  color="#00ffee"
                  anchorX="center"
              ...
              <Billboard position={[0, -cluster.radius - 1, 0]}>
                <Text
                  ...
                  anchorY="middle"
                >
                  {cluster.throughput.toFixed(0)} ops/s
                </Text>
              </Billboard>
            </group>
          ))}
      ...
      {showConnections && connections.map((connection) => {
        ...
        const fromPos = positions.find(a => a.id === connection.from)?.position || fromAgent.position;
        const toPos = positions.find(a => a.id === connection.to)?.position || toAgent.position;

        return (
          <line key={`${connection.from}-${connection.to}`}>
            <bufferGeometry>
              <bufferAttribute
      ...
      {/* Neural signals */}
      {visualizationMode === 'SIGNALS' && signals.map((signal) => {
        ...
        return (
          <group key={signal.id}>
            ...
            <line>
              <bufferGeometry>
                <bufferAttribute
                  ...
                  array={new Float32Array(
                    signal.path.flatMap(pos => [pos.x, pos.y, pos.z])
                  )}
                  itemSize={3}
                />
              </bufferGeometry>
            ...
            {/* Moving signal */}
            <mesh position={interpolatedPos}>
              <sphereGeometry args={[0.1, 8, 8]} />
              <meshBasicMaterial
                color={getSignalColor(signal)}
                emissive={getSignalColor(signal)}
                emissiveIntensity={signal.intensity}
              />
            </mesh>
...
}> = ({ stats, onVisualizationModeChange, onConnectionsToggle, showConnections, visualizationMode }) => {
  return (
    <Card sx={{ mb: 2, background: 'rgba(0, 0, 0, 0.9)', color: 'white' }}>
      <CardContent>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={6}>
            <FormControl fullWidth size="small">
              ...
              >
                <MenuItem value="AGENTS">Individual Agents</MenuItem>
                <MenuItem value="LAYERS">Neural Layers</MenuItem>
                <MenuItem value="CLUSTERS">Agent Clusters</MenuItem>
                <MenuItem value="SIGNALS">Signal Flow</MenuItem>
              </Select>
            </FormControl>
          </Grid>
          <Grid item xs={6}>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={6} md={3}>
            <Typography variant="caption" color="grey.400">Total Agents</Typography>
            <Typography variant="h6" color="#00ffee">
              {stats.totalAgents.toLocaleString()}
            </Typography>
          </Grid>
          <Grid item xs={6} md={3}>
            <Typography variant="caption" color="grey.400">Active Now</Typography>
            <Typography variant="h6" color="#00ff00">
              {stats.activeAgents.toLocaleString()}
            </Typography>
          </Grid>
          <Grid item xs={6} md={3}>
            <Typography variant="caption" color="grey.400">Avg Activation</Typography>
            <Typography variant="h6" color="#ffaa00">
              {(stats.averageActivation * 100).toFixed(1)}%
            </Typography>
          </Grid>
          <Grid item xs={6} md={3}>
            <Typography variant="caption" color="grey.400">Throughput</Typography>
            <Typography variant="h6" color="#ff6b6b">
              {stats.networkThroughput.toFixed(0)}/s
            </Typography>
          </Grid>
        </Grid>
        ...
        <Box sx={{ mb: 3 }}>
          ...
          <Box sx={{ mb: 1 }}>
            ...
            <LinearProgress
              ...
              sx={{ '& .MuiLinearProgress-bar': { backgroundColor: '#ff00ff' } }}
            />
          </Box>
          <Box sx={{ mb: 1 }}>
            <Typography variant="caption" color="grey.400">
              Cluster Efficiency: {stats.clusterEfficiency.toFixed(1)}%
            </Typography>
            <LinearProgress
              variant="determinate"
              value={stats.clusterEfficiency}
              sx={{ '& .MuiLinearProgress-bar': { backgroundColor: '#00ff00' } }}
            />
          </Box>
          <Box sx={{ mb: 1 }}>
            <Typography variant="caption" color="grey.400">
              Quantum Coherence: {(stats.quantumCoherence * 100).toFixed(1)}%
            </Typography>
            <LinearProgress
              variant="determinate"
          ...
          <Box>
            <Typography variant="caption" color="grey.400">
              Consciousness Level: {stats.consciousnessLevel.toFixed(1)}%
            </Typography>
            <LinearProgress
              variant="determinate"
...
const AgentHierarchy: React.FC<{
  ...
  selectedAgent: NeuralAgent | null;
  ...
  const agentsByType = useMemo(() => {
    ...
    const order: NeuralAgent['type'][] = ['SUPREME_COMMANDER', 'FIELD_GENERAL', 'SQUAD_LEADER', 'SPECIALIST', 'WORKER'];
  ...
  const getTypeColor = (type: NeuralAgent['type']) => {
    switch (type) {
      case 'SUPREME_COMMANDER': return '#ff00ff';
      case 'FIELD_GENERAL': return '#ff6b6b';
      case 'SQUAD_LEADER': return '#4ecdc4';
  ...
  return (
    <Card sx={{ background: 'rgba(0, 0, 0, 0.8)', color: 'white' }}>
      <CardContent>
        ...
        {Object.entries(agentsByType).map(([type, agentList]) => (
          <Box key={type} sx={{ mb: 2 }}>
            <Typography 
              variant="subtitle2" 
              sx={{ color: getTypeColor(type as NeuralAgent['type']), mb: 1 }}
            >
              {type.replace(/_/g, ' ')} ({agentList.length})
            </Typography>
            
            <Box sx={{ pl: 2, maxHeight: 150, overflowY: 'auto' }}>
              {agentList.slice(0, 10).map(agent => (
                <Box
                  key={agent.id}
                  ...
                  sx={{
                    ...
                    borderRadius: 1,
                    cursor: 'pointer',
                    backgroundColor: selectedAgent?.id === agent.id ? 'rgba(0,255,238,0.2)' : 'rgba(255,255,255,0.05)',
                ...
                >
                  ...
                  <Typography variant="caption" color="grey.400">
                    {agent.specialization} • {(agent.activation * 100).toFixed(0)}% active
                  </Typography>
 ...
 */
export const NeuralNetworkTheater: React.FC = () => {
  ...
  const [visualizationMode, setVisualizationMode] = useState<'AGENTS' | 'LAYERS' | 'CLUSTERS' | 'SIGNALS'>('AGENTS');
  ...
  const generateNeuralNetwork = () => {
    console.log('🧠 Generating Neural Network with 50,000+ agents...');
    ...
    for (let i = 0; i < 4800; i++) {
      ...
      newAgents.push({
        ...
        specialization: [
          'Data Analysis', 'Process Optimization', 'Quality Assurance', 
          'Citizen Services', 'Budget Analysis', 'Compliance Monitoring'
        ][i % 6],
    ...
    for (let i = 0; i < workerCount; i++) {
      ...
      if (i % 1000 === 0) {
        ...
        newAgents.push({
          ...
          specialization: [
            'Data Entry', 'Document Processing', 'Routine Tasks',
            'Monitoring', 'Basic Analysis', 'Administrative'
          ][i % 6],
    ...
    console.log(`✅ Neural Network Generated: ${newAgents.length} visualized agents representing 50,000+ total`);
  };

  const generateConnections = (agentList: NeuralAgent[]) => {
    ...
    agentList.forEach(agent => {
      agent.connections.forEach(targetId => {
        ...
        if (target) {
          newConnections.push({
            ...
            type: ['COMMAND', 'DATA', 'FEEDBACK', 'COORDINATION', 'LEARNING'][Math.floor(Math.random() * 5)] as any
          });
        }
      });
      ...
      if (Math.random() < 0.1) {
        const sameTypeAgents = agentList.filter(a => a.type === agent.type && a.id !== agent.id);
  ...
  const generateClusters = (agentList: NeuralAgent[]) => {
    const newClusters: NeuralCluster[] = [
      {
        id: 'command_cluster',
        name: 'Command Center',
        agents: agentList.filter(a => a.type === 'SUPREME_COMMANDER' || a.type === 'FIELD_GENERAL').map(a => a.id),
      ...
      },
      {
        ...
        name: 'Operations Hub',
        agents: agentList.filter(a => a.type === 'SQUAD_LEADER').slice(0, 50).map(a => a.id),
      ...
      },
      {
        ...
        agents: agentList.filter(a => a.type === 'SPECIALIST').slice(0, 100).map(a => a.id),
  ...
  const updateNetworkActivity = () => {
    ...
    setAgents(prev => prev.map(agent => ({
      ...
      processingLoad: Math.max(10, Math.min(100, agent.processingLoad + (Math.random() - 0.5) * 5))
    })));
    ...
    })).slice(0, 30)); // Keep manageable number
  ...
  const updateNetworkStats = (agentList: NeuralAgent[]) => {
    const activeAgents = agentList.filter(a => a.activation > 0.3).length;
    const averageActivation = agentList.reduce((sum, a) => sum + a.activation, 0) / agentList.length;
      ...
      <Box sx={{ flex: 1, position: 'relative' }}>
        ...
        <Box sx={{
          ...
          <Typography variant="body2" color="grey.500" sx={{ mt: 1 }}>
            Mode: {visualizationMode} | Connections: {showConnections ? 'ON' : 'OFF'}
          </Typography>
        </Box>
      </Box>
