...
export class SpatialGISAgent extends BaseAgent {
  ...
  constructor(
    ...
    const config: AgentConfig = {
      ...
      description:
        'Specializes in spatial analysis and mapping operations for Benton County properties',
      capabilities: [],
      permissions: [],
    };
  ...
  private async getFloodZoneAnalysis(params: any): Promise<any> {
    try {
      ...
      const floodZoneResult = await this.executeMCPTool('gis.getFloodZone', {
      ...
      if (inFloodZone && zones.length > 0) {
        ...
        const hasHighRiskZones = zones.some(
          (zone: any) => zone.zone.startsWith('A') || zone.zone.startsWith('V')
        );

        // Check for special flood hazard areas
        const hasSpecialFloodHazardAreas = zones.some((zone: any) => zone.isSpecialFloodHazardArea);
      ...
      const valueImpact = inFloodZone
        ? riskLevel === 'High'
          ? {
              percentage: -15,
              description: 'High flood risk typically reduces property values by 10-15%',
            }
          : {
              percentage: -5,
              description: 'Moderate flood risk typically reduces property values by 5-10%',
            }
      ...
      await this.logActivity('flood_zone_analysis_generated', 'Flood zone analysis generated', {
        propertyId,
      });
      ...
      await this.logActivity(
        'flood_zone_analysis_error',
        `Error generating flood zone analysis: ${error.message}`
      );
  ...
  private async getWetlandsAnalysis(params: any): Promise<any> {
    try {
      ...
      const wetlandsResult = await this.executeMCPTool('gis.getWetlands', {
        parcelId: parcelNumber,
      });
      ...
      if (hasWetlands && wetlands.length > 0) {
        totalWetlandAcres = wetlands.reduce(
          (sum: number, wetland: any) => sum + (wetland.acres || 0),
          0
        );
      }

      // Calculate percentage of property covered by wetlands
      const wetlandCoveragePercent =
        propertyAcres > 0
          ? Math.min(100, Math.round((totalWetlandAcres / propertyAcres) * 100))
          : 0;
      ...
      if (hasWetlands && wetlands.length > 0) {
        const hasProtectedWetlands = wetlands.some(
          (wetland: any) => wetland.classification && wetland.classification.includes('Protected')
        );

        const hasHighValueWetlands = wetlands.some(
          (wetland: any) =>
            wetland.type &&
            (wetland.type.includes('Estuarine') || wetland.type.includes('Riverine'))
        );
        ...
        if (hasProtectedWetlands || hasHighValueWetlands || wetlandCoveragePercent > 30) {
          ...
          conservationRequirements = ['Basic wetland protection measures required'];
      ...
      const valueImpact = hasWetlands
        ? wetlandCoveragePercent > 50
          ? {
              percentage: -25,
              description:
                'Properties with significant wetland coverage typically see reduced development potential and value',
            }
          : wetlandCoveragePercent > 20
            ? {
                percentage: -15,
                description: 'Moderate wetland coverage reduces property development options',
              }
            : {
                percentage: -5,
                description: 'Minor wetland presence has limited impact on property value',
              }
      ...
      await this.logActivity('wetlands_analysis_generated', 'Wetlands analysis generated', {
        propertyId,
      });
      ...
      await this.logActivity(
        'wetlands_analysis_error',
        `Error generating wetlands analysis: ${error.message}`
      );
  ...
  private async getSchoolDistrictInfo(params: any): Promise<any> {
    try {
      ...
      const schoolDistrictResult = await this.executeMCPTool('gis.getSchoolDistrict', {
        parcelId: parcelNumber,
      });
      ...
      await this.logActivity(
        'school_district_info_generated',
        'School district information generated',
        { propertyId }
      );
      ...
      await this.logActivity(
        'school_district_info_error',
        `Error getting school district information: ${error.message}`
      );
  ...
  private async getTaxDistrictInfo(params: any): Promise<any> {
    try {
      ...
      const taxDistrictResult = await this.executeMCPTool('gis.getTaxDistrict', {
        parcelId: parcelNumber,
      });
      ...
      await this.logActivity('tax_district_info_generated', 'Tax district information generated', {
        propertyId,
      });
      ...
      await this.logActivity(
        'tax_district_info_error',
        `Error getting tax district information: ${error.message}`
      );
  ...
  private async getMultiLayerAnalysis(params: any): Promise<any> {
    try {
      ...
      if (analysisOptions.includeNeighbors) {
        const neighbors = await this.executeMCPTool('gis.getNeighboringParcels', {
          parcelId: parcelNumber,
        });
        if (neighbors?.success) {
          analysisResult.layers.neighbors = {
            count: neighbors.result.length,
            properties: neighbors.result
      ...
      if (analysisOptions.includeWetlands) {
        const wetlandsInfo = await this.executeMCPTool('gis.getWetlands', {
          parcelId: parcelNumber,
        });
      ...
      if (analysisOptions.includeSchoolDistricts) {
        const schoolDistrictInfo = await this.executeMCPTool('gis.getSchoolDistrict', {
          parcelId: parcelNumber,
        });
      ...
      if (analysisOptions.includeTaxDistricts) {
        const taxDistrictInfo = await this.executeMCPTool('gis.getTaxDistrict', {
          parcelId: parcelNumber,
        });
      ...
      await this.logActivity(
        'multi_layer_analysis_generated',
        'Multi-layer spatial analysis generated',
        { propertyId }
      );
      ...
      await this.logActivity(
        'multi_layer_analysis_error',
        `Error generating multi-layer analysis: ${error.message}`
      );
  ...
  private generateMultiLayerSummary(analysisResult: any): any {
    ...
    if (analysisResult.layers.floodZones) {
      ...
      if (inFloodZone) {
        keyFindings.push(
          `Property is located in a flood zone (${zones.map((z: any) => z.zone).join(', ')})`
        );
    ...
    if (analysisResult.layers.schoolDistricts) {
      ...
      keyFindings.push(
        `Property is in ${districts.map((d: any) => d.name).join(', ')} school district(s)`
      );
    }
    ...
    if (analysisResult.layers.taxDistricts) {
      ...
      districts.forEach((d: any) => {
        totalRate += d.taxRate || 0;
      });
      keyFindings.push(
        `Property is subject to a ${totalRate.toFixed(2)} tax rate across ${districts.length} tax district(s)`
      );
    }
    ...
    if (analysisResult.layers.zoning) {
      ...
      if (zoning.permittedUses && zoning.permittedUses.length > 0) {
        recommendations.push(
          `Consider permitted uses for this zone: ${zoning.permittedUses.slice(0, 3).join(', ')}${zoning.permittedUses.length > 3 ? '...' : ''}`
        );
      }
    }
    ...
    if (analysisResult.layers.neighbors) {
      ...
      if (count > 0) {
        ...
        const avgValue =
          properties.reduce((sum: number, p: any) => sum + (p.value || 0), 0) / properties.length;
        keyFindings.push(
          `Average neighboring property value: $${Math.round(avgValue).toLocaleString()}`
        );
      }
    }
  ...
  private async getSpatialInfo(params: any): Promise<any> {
    try {
      ...
      await this.logActivity('spatial_info_generated', 'Spatial information generated', {
        propertyId,
      });
      ...
      await this.logActivity(
        'spatial_info_error',
        `Error generating spatial information: ${error.message}`
      );
  ...
  private async getNeighborhoodMap(params: any): Promise<any> {
    try {
      ...
      const neighboringProperties = neighbors.result
        .map((parcel: any) => this.arcgisService.convertToProperty(parcel))
        .filter(Boolean);
      ...
      await this.logActivity('neighborhood_map_generated', 'Neighborhood map generated', {
        propertyId,
      });
      ...
      await this.logActivity(
        'neighborhood_map_error',
        `Error generating neighborhood map: ${error.message}`
      );
  ...
  private async searchByLocation(params: any): Promise<any> {
    try {
      const { latitude, longitude, address, radius = 1000, maxResults = 20 } = params;
      ...
      await this.logActivity(
        ...
        `Error searching by location: ${error.message}`
      );
  ...
  private async analyzeZoning(params: any): Promise<any> {
    try {
      ...
      const zoning =
        parcel.result.attributes.ZONING || property.result.extraFields?.zoning || 'Unknown';
      ...
      const marketFactors = await this.marketFactorService.getPropertyMarketFactors(propertyId);
      const landUseFactors = marketFactors.filter(
        f => f.name.includes('Development') || f.name.includes('Zoning') || f.name.includes('Land')
      );
      ...
      await this.logActivity('zoning_analysis_generated', 'Zoning analysis generated', {
        propertyId,
      });
  ...
  private analyzeNeighborhood(
    ...
    const yearsBuilt = neighbors.map(n => n.extraFields?.yearBuilt).filter(Boolean) as number[];

    const averageYearBuilt =
      yearsBuilt.length > 0
    ...
    const zoningInfo: { [key: string]: any } = {
      'R-1': {
        ...
        restrictions: [
          ...
          'Minimum setbacks: 20 ft front, 5 ft side, 20 ft rear',
        ],
      },
      'R-2': {
        ...
        restrictions: [
          ...
          'Maximum height of 35 feet',
          'Minimum setbacks: 15 ft front, 5 ft side, 15 ft rear',
        ],
      },
      'R-3': {
        ...
        restrictions: [
          'Minimum lot size of 2,000 sq ft per unit',
          'Maximum height of 45 feet',
          'Minimum setbacks: 10 ft front, 5 ft side, 10 ft rear',
        ],
      },
      'C-1': {
        ...
        restrictions: [
          'Maximum floor area ratio of 0.5',
      ...
      'C-2': {
        ...
        restrictions: [
          ...
          'Maximum height of 45 feet',
          'Minimum setbacks: 5 ft front, 0 ft side, 10 ft rear',
        ],
      },
      'I-1': {
        ...
        restrictions: [
          ...
          'Minimum setbacks: 20 ft front, 10 ft side, 20 ft rear',
        ],
      },
      'I-2': {
        ...
        restrictions: [
          ...
          'Minimum setbacks: 30 ft front, 15 ft side, 30 ft rear',
        ],
      },
      'A-1': {
        ...
        restrictions: [
          ...
          'Maximum height of 35 feet for residences',
          'Minimum setbacks: 50 ft front, 20 ft side, 50 ft rear',
        ],
      },
    };
