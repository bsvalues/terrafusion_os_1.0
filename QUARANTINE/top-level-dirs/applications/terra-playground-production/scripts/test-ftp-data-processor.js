...
async function testFileParsing(filePath, entityType) {
  ...
  try {
    ...
    logger.info(
      `✓ Successfully parsed ${parseResult.records.length} records from ${entityType} file`
    );
    return true;
  } catch (error) {
...
async function runAllTests() {
  ...
  if (testsPassed === totalTests) {
    ...
    logger.error(
      `Some FTP data processor tests failed (${totalTests - testsPassed}/${totalTests} failed)`
    );
    return false;
  }
}
