...
echo TerraFusion Frontend Startup Script
echo ================================
...
REM Start frontend with security measures
echo Starting TerraFusion Frontend...
start /B npm run start:frontend -- --port %PORT% --https --cert "%APPDATA%\TerraFusion\security\cert.pem" --key "%APPDATA%\TerraFusion\security\key.pem"

REM Health check
timeout /t 10 /nobreak
curl -k https://localhost:%PORT%/health
if errorlevel 1 (
    echo Frontend failed to start properly. Check logs for details.
    exit /b 1
)
