    ...
    try {
      ...
      return `
        ...
        <body><>
          ...
          <p
</> class="error">${error instanceof Error ? error.message : 'Unknown error'}</p>
          <div class="actions"><>

            <button onclick="window.parent.postMessage({ type: 'webview.action', action: 'retry' }, '*')">Retry</button>
            <button
</> class="secondary" onclick="window.parent.postMessage({ type: 'webview.action', action: 'close' }, '*')">Close</button>
          </div>
          <script>
