  ...
  "recommendations": [
    {
      ...
      throw new Error(`AI recommendation generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
...
Respond in JSON format with the same structure as before, but focus on user-specific improvements.`;
      ...
      throw new Error(`Personalized recommendation generation failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
  }
