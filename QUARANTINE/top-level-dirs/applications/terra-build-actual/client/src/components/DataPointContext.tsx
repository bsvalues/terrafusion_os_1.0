...
import { Info, HelpCircle, BarChart3, TrendingUp  } from '@mui/icons-material';
...
className="h-16 w-full flex items-end gap-1">
...
className="opacity-0 group-hover:opacity-100 absolute -top-6 left-1/2 transform -translate-x-1/2 text-[10px] font-medium bg-primary text-white px-1 py-0.5 rounded">
...
</>>{trendData[trendData.length - 1]?.date}</span>
            </div>
          </div>
        )}
...
className="font-medium">{formatValue(item.value)}</span>
                  </div>
