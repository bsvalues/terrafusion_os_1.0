...
import { Database, HardDrive, Refresh, Clock  } from '@mui/icons-material';
...
</>>Current database connectivity information</CardDescription>
      </CardHeader>
      
      <CardContent>
        {isLoading ? (
          <div className="flex justify-center items-center py-4">
            <Refresh className="animate-spin h-6 w-6 text-primary" />
...
variant={status?.supabase.available ? 'default' : 'outline'} className={status?.supabase.available ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-100'}>
...
variant={status?.supabase.configured ? 'outline' : 'default'} className={status?.supabase.configured ? '' : 'opacity-70'}>
...
variant={status?.postgres.available ? 'default' : 'outline'} className={status?.postgres.available ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-100' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-100'}>
...
variant={status?.postgres.configured ? 'outline' : 'default'} className={status?.postgres.configured ? '' : 'opacity-70'}>
