...
import { MapPin, Search, Filter, Layers, Plus, Minus, Home, X, ChevronRight, ChevronDown  } from '@mui/icons-material';
...
className="mt-2 text-sm text-gray-600">Loading map data...</p>
          </div>
        </div>
      )}
...
className="absolute" style={{ top: '50%', left: '0%', width: '100%', height: '3px', backgroundColor: '#94A3B8' }}></div>
...
size="sm" variant="secondary" onClick={handleZoomOut}>
<>
...
size="sm" variant="secondary" onClick={handleResetMap}>
...
className="ml-2 inline-flex items-center justify-center text-xs h-5 w-5 text-white bg-primary rounded-full">
                {markers.length}
              </span>
            </div>
...
</>>${valueRange[1].toLocaleString()}</span>
                      </div>
                    </div>
...
className="text-xs text-muted-foreground mt-1">ID: {marker.property.propId}</div>
<>
...
className="text-sm mt-1">${marker.property.assessedValue?.toLocaleString() || 'Not assessed'}</div>
                  </div>
                </div>
              </div>
            ))}
...
size="sm" variant="ghost" onClick={clearSelectedMarker}>
                    <X className="h-4 w-4" />
                  </Button>
                </div>
...
className="font-medium">{selectedMarker.property.address || 'No address available'}</p>
                    </div>
...
</>>{selectedMarker.property.propId}</p>
                      </div>
                      <div>
<>
...
</>>{selectedMarker.property.propertyType || 'Unknown'}</p>
                      </div>
                    </div>
...
</>>{selectedMarker.property.owner || 'Owner information not available'}</p>
                    </div>
...
className="text-lg font-semibold">${selectedMarker.property.assessedValue?.toLocaleString() || 'Not assessed'}</p>
                    </div>
...
</>>{selectedMarker.property.buildingType}</p>
                      </div>
                    )}
...
</>>{selectedMarker.property.yearBuilt}</p>
                        </div>
                      )}
...
</>>{selectedMarker.property.squareFeet.toLocaleString()}</p>
                        </div>
                      )}
                    </div>
