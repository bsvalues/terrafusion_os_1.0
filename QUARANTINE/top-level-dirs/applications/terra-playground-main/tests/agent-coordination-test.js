...
async function runAgentCoordinationTest() {
  ...
  try {
    ...
    const reportResult = await agentSystem.executeCapability('reporting', 'runReport', {
    ...
    const exportResult = await agentSystem.executeCapability('data_ingestion', 'exportToFTP', {
      content: {
        propertyId: TEST_PROPERTY_ID,
