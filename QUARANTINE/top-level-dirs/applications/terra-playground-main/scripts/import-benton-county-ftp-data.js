...
async function importPropertyData() {
  ...
  try {
    ...
    if (!connected) {
      throw new Error(
        'Failed to connect to Benton County FTP server. Check credentials and try again.'
      );
    }
    ...
    const propertyDataFiles = files.filter(
      file =>
    ...
    for (const file of propertyDataFiles) {
      ...
      try {
        ...
        const commitResult = await dataImportService.commitStagedProperties(
          stagedIds.map(p => p.id)
        );
