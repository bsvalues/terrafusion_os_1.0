...
import { Loader2, Wifi, WifiOff, AlertCircle  } from '@mui/icons-material';
...
<>
          <div className={`h-3 w-3 rounded-full ${getStatusColor()}`}></div>
          <h3
</> className="text-sm font-medium">Agent System</h3>
        </div>
...
<>
              ...
              <span
</> className="font-semibold">{agentCount}</span>
            </div>
            <div className="flex justify-between">
<>
              <span>Last Activity:</span>
              <span
</>>{formatTimestamp(lastMessageTime)}</span>
            </div>
          </div>
