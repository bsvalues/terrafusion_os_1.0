...
export function registerDemoRoutes(app: express.Express, storage: IStorage) {
    ...
    try {
      ...
      const demoProperties = properties.map(property => {
        ...
        return {
          ...
          landValue: extraFields.landValue
            ? Number(extraFields.landValue)
            : Math.round(assessedValue * 0.4),
  ...
  router.get('/properties/:id/tax-assessment', async (req, res) => {
    try {
      ...
      const totalExemptionValue = exemptions.reduce(
        (sum: number, exemption: PropertyExemption) => sum + (exemption.value || 0),
        0
      );
      ...
      const totalSpecialAssessments = specialAssessments.reduce(
        (sum: number, assessment: SpecialAssessment) => sum + (assessment.amount || 0),
        0
      );
        ...
        return {
          ...
          changeFromPriorYear:
            i === 0
              ...
              : parseFloat(
                  (
                    (taxAmount / (assessedValue * taxRate * (1 - (i - 1) * 0.005)) - 1) *
                    100
                  ).toFixed(2)
                ),
        };
          ...
          factors: [
            {
              factor: 'Location',
              impact: 'high',
              description: 'Property is in a desirable area with good school district',
            },
            {
              factor: 'Recent Sales',
              impact: 'medium',
              description: 'Comparable sales in the area suggest stable to increasing values',
            },
            {
              factor: 'Property Condition',
              impact: 'medium',
              description: 'Property appears to be in good condition based on available data',
            },
          ],
        },
        taxAssessment: {
        ...
        improvementOpportunities: [
          {
            ...
            description: 'Additional property details could improve assessment accuracy',
          },
          {
            opportunity: 'Exemption Review',
            description: 'Property may qualify for additional exemptions based on profile',
          },
        ],
