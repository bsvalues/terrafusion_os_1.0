...
export class PropertyIntelligenceAgent {
  ...
  private async handleComparablePropertiesTask(taskId: string, task: any): Promise<void> {
    ...
    const result = await this.findComparableProperties(propertyId, count || 5, criteria, options);
    ...
    try {
      ...
      if (riskTypes.includes('environmental')) {
        try {
          ...
          logger.warn(
            `Could not get environmental risks for property ${propertyId}: ${error.message}`
          );
        }
      }
