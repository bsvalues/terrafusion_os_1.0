...
import { createSampleEvents, initMaintenanceSystem } from './lib/maintenance/init';
  ...
  window.addEventListener('unhandledrejection', event => {
    ...
    try {
      if (event.reason && typeof event.reason === 'object' && event.reason.message) {
        const source = event.reason.message.includes('WebSocket')
          ? 'WebSocket'
          : event.reason.message.includes('Yjs')
            ? 'Yjs'
            : 'Other';
