Create view [CO\MDP].GIS_ManuHome_inParks as
SELECT DISTINCT 
...
left join 
 ...
 -- LEFT JOIN (SELECT [Parcel_ID],ROW_NUMBER() over (partition by prop_id ORDER BY [OBJECTID] DESC) AS order_id,[Prop_ID],--Geometry,[shape].STCentroid().STX as XCoord,[shape].STCentroid().STY as YCoord ,      [CENTROID_X] as x      ,[CENTROID_Y] as yFROM --[Benton_spatial_data].[dbo].[spatial_coords]
--[Benton_spatial_data].[dbo].[parcel]) as coords ON pa.child_prop_id = coords.Prop_ID AND coords.order_id = 1


WHERE pv.prop_val_yr = (select appr_yr from pacs_oltp.dbo.pacs_system)---you can change as needed
AND pv.cycle = 1---you can change as needed
