...
export class AIAgentManager extends EventEmitter {
  ...
  async createSwarm(config: SwarmConfiguration): Promise<AgentSwarm> {
    ...
    const coordinator = Array.from(this.agents.values()).find(
      agent => agent.type === config.coordinatorType && agent.status === 'idle'
    );
  ...
  async deployAgents(
    ...
    if (availableAgents.length < agentCount) {
      console.log(
        `   Warning: Only ${availableAgents.length} agents available (requested ${agentCount})`
      );
    }
  ...
  private async assignTaskToSwarm(task: TaskAssignment): Promise<void> {
    ...
    console.log(
      `📋 Task ${task.taskId} distributed across swarm ${swarm.name} (${subtasks.length} subtasks)`
    );
  ...
  private updateAgentMetrics(): void {
    for (const agent of this.agents.values()) {
      ...
      agent.resources.cpuUsage = Math.max(
        0,
        Math.min(100, agent.resources.cpuUsage + (Math.random() - 0.5) * 10)
      );
      agent.resources.memoryUsage = Math.max(
        0,
        Math.min(100, agent.resources.memoryUsage + (Math.random() - 0.5) * 5)
      );
      agent.resources.networkLatency = Math.max(
        1,
        agent.resources.networkLatency + (Math.random() - 0.5) * 20
      );
    }
  }
  ...
  private optimizeResourceAllocation(): void {
    ...
    const overloadedAgents = Array.from(this.agents.values()).filter(
      agent => agent.resources.cpuUsage > 80 || agent.resources.memoryUsage > 80
    );
  ...
  private redistributeTasks(overloadedAgents: AIAgent[]): void {
    const availableAgents = Array.from(this.agents.values()).filter(
      agent => agent.status === 'idle' && agent.resources.cpuUsage < 50
    );
